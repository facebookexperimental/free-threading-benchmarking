# Results vs. 3.12.6

- fork: python
- ref: 2be2dd5fc219a5c252d7
- machine: linux-x86_64
- commit hash: 2be2dd5
- commit date: 2026-02-21
- overall geometric mean: 1.133x faster
- HPT reliability: 100.00%
- HPT 99th percentile: 1.06x faster
- Memory change: 1.23x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260221-vultr-x86_64-python-2be2dd5fc219a5c252d7-3.15.0a6+-2be2dd5 |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| 2to3           | 264 ms                                                 | 249 ms: 1.06x faster                                                   |
| docutils       | 2.64 sec                                               | 2.76 sec: 1.05x slower                                                 |
| html5lib       | 63.6 ms                                                | 64.7 ms: 1.02x slower                                                  |
| Geometric mean | (ref)                                                  | 1.00x slower                                                           |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260221-vultr-x86_64-python-2be2dd5fc219a5c252d7-3.15.0a6+-2be2dd5 |
|----------------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| async_tree_io_tg           | 1.11 sec                                               | 572 ms: 1.94x faster                                                   |
| async_tree_none            | 464 ms                                                 | 240 ms: 1.93x faster                                                   |
| async_tree_io              | 1.08 sec                                               | 561 ms: 1.93x faster                                                   |
| async_tree_memoization_tg  | 560 ms                                                 | 297 ms: 1.89x faster                                                   |
| async_tree_memoization     | 555 ms                                                 | 296 ms: 1.88x faster                                                   |
| async_tree_none_tg         | 446 ms                                                 | 250 ms: 1.78x faster                                                   |
| async_tree_cpu_io_mixed    | 715 ms                                                 | 475 ms: 1.51x faster                                                   |
| async_tree_cpu_io_mixed_tg | 723 ms                                                 | 485 ms: 1.49x faster                                                   |
| asyncio_tcp                | 519 ms                                                 | 449 ms: 1.15x faster                                                   |
| asyncio_tcp_ssl            | 1.51 sec                                               | 1.47 sec: 1.02x faster                                                 |
| coroutines                 | 23.9 ms                                                | 23.5 ms: 1.02x faster                                                  |
| asyncio_websockets         | 517 ms                                                 | 543 ms: 1.05x slower                                                   |
| async_generators           | 384 ms                                                 | 408 ms: 1.06x slower                                                   |
| Geometric mean             | (ref)                                                  | 1.44x faster                                                           |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260221-vultr-x86_64-python-2be2dd5fc219a5c252d7-3.15.0a6+-2be2dd5 |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| float          | 80.8 ms                                                | 58.7 ms: 1.38x faster                                                  |
| nbody          | 89.3 ms                                                | 73.1 ms: 1.22x faster                                                  |
| pidigits       | 184 ms                                                 | 198 ms: 1.07x slower                                                   |
| Geometric mean | (ref)                                                  | 1.16x faster                                                           |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260221-vultr-x86_64-python-2be2dd5fc219a5c252d7-3.15.0a6+-2be2dd5 |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| regex_compile  | 142 ms                                                 | 126 ms: 1.13x faster                                                   |
| regex_effbot   | 3.17 ms                                                | 2.94 ms: 1.08x faster                                                  |
| regex_v8       | 20.6 ms                                                | 22.1 ms: 1.07x slower                                                  |
| regex_dna      | 168 ms                                                 | 191 ms: 1.14x slower                                                   |
| Geometric mean | (ref)                                                  | 1.00x slower                                                           |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260221-vultr-x86_64-python-2be2dd5fc219a5c252d7-3.15.0a6+-2be2dd5 |
|----------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| tomli_loads          | 2.11 sec                                               | 1.60 sec: 1.32x faster                                                 |
| xml_etree_iterparse  | 96.7 ms                                                | 83.6 ms: 1.16x faster                                                  |
| unpickle_pure_python | 221 us                                                 | 196 us: 1.12x faster                                                   |
| json_dumps           | 10.4 ms                                                | 9.37 ms: 1.11x faster                                                  |
| xml_etree_generate   | 85.2 ms                                                | 77.9 ms: 1.09x faster                                                  |
| xml_etree_process    | 59.0 ms                                                | 54.0 ms: 1.09x faster                                                  |
| xml_etree_parse      | 139 ms                                                 | 130 ms: 1.07x faster                                                   |
| pickle_pure_python   | 308 us                                                 | 315 us: 1.02x slower                                                   |
| pickle_dict          | 31.8 us                                                | 33.4 us: 1.05x slower                                                  |
| json_loads           | 26.5 us                                                | 28.0 us: 1.05x slower                                                  |
| unpickle             | 14.1 us                                                | 14.8 us: 1.05x slower                                                  |
| pickle_list          | 4.77 us                                                | 5.13 us: 1.08x slower                                                  |
| unpickle_list        | 4.67 us                                                | 5.19 us: 1.11x slower                                                  |
| pickle               | 10.9 us                                                | 12.8 us: 1.17x slower                                                  |
| Geometric mean       | (ref)                                                  | 1.03x faster                                                           |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260221-vultr-x86_64-python-2be2dd5fc219a5c252d7-3.15.0a6+-2be2dd5 |
|------------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| python_startup_no_site | 7.16 ms                                                | 7.48 ms: 1.04x slower                                                  |
| python_startup         | 9.93 ms                                                | 12.6 ms: 1.27x slower                                                  |
| Geometric mean         | (ref)                                                  | 1.15x slower                                                           |

Benchmarks with tag 'template':
===============================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260221-vultr-x86_64-python-2be2dd5fc219a5c252d7-3.15.0a6+-2be2dd5 |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| mako           | 11.0 ms                                                | 10.4 ms: 1.05x faster                                                  |
| Geometric mean | (ref)                                                  | 1.03x faster                                                           |

Benchmark hidden because not significant (1): django_template

All benchmarks:
===============

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260221-vultr-x86_64-python-2be2dd5fc219a5c252d7-3.15.0a6+-2be2dd5 |
|----------------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| richards                   | 45.9 ms                                                | 17.6 ms: 2.61x faster                                                  |
| richards_super             | 51.9 ms                                                | 21.4 ms: 2.43x faster                                                  |
| mdp                        | 2.42 sec                                               | 1.21 sec: 1.99x faster                                                 |
| async_tree_io_tg           | 1.11 sec                                               | 572 ms: 1.94x faster                                                   |
| async_tree_none            | 464 ms                                                 | 240 ms: 1.93x faster                                                   |
| async_tree_io              | 1.08 sec                                               | 561 ms: 1.93x faster                                                   |
| async_tree_memoization_tg  | 560 ms                                                 | 297 ms: 1.89x faster                                                   |
| async_tree_memoization     | 555 ms                                                 | 296 ms: 1.88x faster                                                   |
| deepcopy_memo              | 40.3 us                                                | 22.2 us: 1.81x faster                                                  |
| async_tree_none_tg         | 446 ms                                                 | 250 ms: 1.78x faster                                                   |
| deepcopy                   | 352 us                                                 | 226 us: 1.56x faster                                                   |
| async_tree_cpu_io_mixed    | 715 ms                                                 | 475 ms: 1.51x faster                                                   |
| scimark_sor                | 130 ms                                                 | 86.6 ms: 1.50x faster                                                  |
| async_tree_cpu_io_mixed_tg | 723 ms                                                 | 485 ms: 1.49x faster                                                   |
| scimark_lu                 | 114 ms                                                 | 79.0 ms: 1.44x faster                                                  |
| spectral_norm              | 110 ms                                                 | 78.2 ms: 1.41x faster                                                  |
| go                         | 139 ms                                                 | 100 ms: 1.39x faster                                                   |
| float                      | 80.8 ms                                                | 58.7 ms: 1.38x faster                                                  |
| comprehensions             | 19.8 us                                                | 14.8 us: 1.34x faster                                                  |
| scimark_fft                | 342 ms                                                 | 256 ms: 1.33x faster                                                   |
| logging_silent             | 109 ns                                                 | 81.8 ns: 1.33x faster                                                  |
| scimark_monte_carlo        | 68.4 ms                                                | 51.6 ms: 1.33x faster                                                  |
| tomli_loads                | 2.11 sec                                               | 1.60 sec: 1.32x faster                                                 |
| deepcopy_reduce            | 3.08 us                                                | 2.38 us: 1.29x faster                                                  |
| deltablue                  | 3.45 ms                                                | 2.72 ms: 1.27x faster                                                  |
| pathlib                    | 21.5 ms                                                | 17.6 ms: 1.23x faster                                                  |
| pyflate                    | 448 ms                                                 | 366 ms: 1.23x faster                                                   |
| nbody                      | 89.3 ms                                                | 73.1 ms: 1.22x faster                                                  |
| bpe_tokeniser              | 4.74 sec                                               | 4.00 sec: 1.18x faster                                                 |
| chaos                      | 62.8 ms                                                | 53.2 ms: 1.18x faster                                                  |
| crypto_pyaes               | 76.6 ms                                                | 65.3 ms: 1.17x faster                                                  |
| pprint_safe_repr           | 743 ms                                                 | 636 ms: 1.17x faster                                                   |
| xml_etree_iterparse        | 96.7 ms                                                | 83.6 ms: 1.16x faster                                                  |
| asyncio_tcp                | 519 ms                                                 | 449 ms: 1.15x faster                                                   |
| pprint_pformat             | 1.52 sec                                               | 1.33 sec: 1.14x faster                                                 |
| regex_compile              | 142 ms                                                 | 126 ms: 1.13x faster                                                   |
| unpickle_pure_python       | 221 us                                                 | 196 us: 1.12x faster                                                   |
| logging_simple             | 6.63 us                                                | 5.97 us: 1.11x faster                                                  |
| dulwich_log                | 78.9 ms                                                | 71.1 ms: 1.11x faster                                                  |
| generators                 | 32.2 ms                                                | 29.1 ms: 1.11x faster                                                  |
| json_dumps                 | 10.4 ms                                                | 9.37 ms: 1.11x faster                                                  |
| logging_format             | 7.35 us                                                | 6.65 us: 1.11x faster                                                  |
| xml_etree_generate         | 85.2 ms                                                | 77.9 ms: 1.09x faster                                                  |
| pylint                     | 319 ms                                                 | 292 ms: 1.09x faster                                                   |
| hexiom                     | 6.17 ms                                                | 5.65 ms: 1.09x faster                                                  |
| xml_etree_process          | 59.0 ms                                                | 54.0 ms: 1.09x faster                                                  |
| regex_effbot               | 3.17 ms                                                | 2.94 ms: 1.08x faster                                                  |
| fannkuch                   | 372 ms                                                 | 346 ms: 1.08x faster                                                   |
| nqueens                    | 80.1 ms                                                | 74.5 ms: 1.07x faster                                                  |
| raytrace                   | 299 ms                                                 | 280 ms: 1.07x faster                                                   |
| xml_etree_parse            | 139 ms                                                 | 130 ms: 1.07x faster                                                   |
| 2to3                       | 264 ms                                                 | 249 ms: 1.06x faster                                                   |
| meteor_contest             | 104 ms                                                 | 98.1 ms: 1.06x faster                                                  |
| mako                       | 11.0 ms                                                | 10.4 ms: 1.05x faster                                                  |
| scimark_sparse_mat_mult    | 4.39 ms                                                | 4.19 ms: 1.05x faster                                                  |
| sympy_integrate            | 20.5 ms                                                | 19.6 ms: 1.05x faster                                                  |
| typing_runtime_protocols   | 163 us                                                 | 158 us: 1.03x faster                                                   |
| json                       | 5.02 ms                                                | 4.90 ms: 1.02x faster                                                  |
| asyncio_tcp_ssl            | 1.51 sec                                               | 1.47 sec: 1.02x faster                                                 |
| sympy_sum                  | 166 ms                                                 | 162 ms: 1.02x faster                                                   |
| sqlite_synth               | 2.20 us                                                | 2.16 us: 1.02x faster                                                  |
| coroutines                 | 23.9 ms                                                | 23.5 ms: 1.02x faster                                                  |
| sympy_str                  | 292 ms                                                 | 286 ms: 1.02x faster                                                   |
| pycparser                  | 1.17 sec                                               | 1.15 sec: 1.01x faster                                                 |
| html5lib                   | 63.6 ms                                                | 64.7 ms: 1.02x slower                                                  |
| pickle_pure_python         | 308 us                                                 | 315 us: 1.02x slower                                                   |
| thrift                     | 791 us                                                 | 817 us: 1.03x slower                                                   |
| python_startup_no_site     | 7.16 ms                                                | 7.48 ms: 1.04x slower                                                  |
| docutils                   | 2.64 sec                                               | 2.76 sec: 1.05x slower                                                 |
| sympy_expand               | 468 ms                                                 | 490 ms: 1.05x slower                                                   |
| pickle_dict                | 31.8 us                                                | 33.4 us: 1.05x slower                                                  |
| asyncio_websockets         | 517 ms                                                 | 543 ms: 1.05x slower                                                   |
| json_loads                 | 26.5 us                                                | 28.0 us: 1.05x slower                                                  |
| unpickle                   | 14.1 us                                                | 14.8 us: 1.05x slower                                                  |
| async_generators           | 384 ms                                                 | 408 ms: 1.06x slower                                                   |
| pidigits                   | 184 ms                                                 | 198 ms: 1.07x slower                                                   |
| regex_v8                   | 20.6 ms                                                | 22.1 ms: 1.07x slower                                                  |
| pickle_list                | 4.77 us                                                | 5.13 us: 1.08x slower                                                  |
| unpickle_list              | 4.67 us                                                | 5.19 us: 1.11x slower                                                  |
| regex_dna                  | 168 ms                                                 | 191 ms: 1.14x slower                                                   |
| pickle                     | 10.9 us                                                | 12.8 us: 1.17x slower                                                  |
| coverage                   | 71.4 ms                                                | 83.4 ms: 1.17x slower                                                  |
| python_startup             | 9.93 ms                                                | 12.6 ms: 1.27x slower                                                  |
| gc_traversal               | 3.46 ms                                                | 4.66 ms: 1.35x slower                                                  |
| unpack_sequence            | 52.1 ns                                                | 70.8 ns: 1.36x slower                                                  |
| bench_thread_pool          | 941 us                                                 | 1.31 ms: 1.40x slower                                                  |
| create_gc_cycles           | 1.09 ms                                                | 1.90 ms: 1.74x slower                                                  |
| bench_mp_pool              | 10.8 ms                                                | 223 ms: 20.61x slower                                                  |
| telco                      | 6.53 ms                                                | 160 ms: 24.45x slower                                                  |
| Geometric mean             | (ref)                                                  | 1.07x faster                                                           |

Benchmark hidden because not significant (1): django_template
Ignored benchmarks (15) of results/bm-20240906-3.12.6-a4a2d2b/bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b.json: aiohttp, chameleon, dask, flaskblogging, genshi_text, genshi_xml, gunicorn, mypy2, sqlalchemy_declarative, sqlalchemy_imperative, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http
Ignored benchmarks (10) of results/bm-20260221-3.15.0a6+-2be2dd5-JIT/bm-20260221-vultr-x86_64-python-2be2dd5fc219a5c252d7-3.15.0a6+-2be2dd5.json: connected_components, k_core, many_optionals, shortest_path, sphinx, sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile, subparsers

- Geometric mean (including insignificant results): 1.133x faster

# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.09x
- 95% likely to have a speedup of 1.07x
- 99% likely to have a speedup of 1.06x

# Memory
- memory change: 1.23x