# Results vs. 3.13.0rc2

- fork: python
- ref: 46295677a13f141b8d52
- machine: linux-x86_64
- commit hash: 4629567
- commit date: 2025-12-10
- overall geometric mean: 1.066x faster
- HPT reliability: 99.99%
- HPT 99th percentile: 1.02x faster
- Memory change: 1.18x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20251210-vultr-x86_64-python-46295677a13f141b8d52-3.15.0a2+-4629567 |
|----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| 2to3           | 260 ms                                                       | 247 ms: 1.05x faster                                                   |
| docutils       | 2.62 sec                                                     | 2.73 sec: 1.04x slower                                                 |
| html5lib       | 67.0 ms                                                      | 66.1 ms: 1.01x faster                                                  |
| Geometric mean | (ref)                                                        | 1.01x faster                                                           |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20251210-vultr-x86_64-python-46295677a13f141b8d52-3.15.0a2+-4629567 |
|----------------------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| async_tree_io              | 876 ms                                                       | 552 ms: 1.59x faster                                                   |
| async_tree_io_tg           | 913 ms                                                       | 581 ms: 1.57x faster                                                   |
| async_tree_memoization     | 461 ms                                                       | 300 ms: 1.54x faster                                                   |
| async_tree_none            | 354 ms                                                       | 241 ms: 1.47x faster                                                   |
| async_tree_none_tg         | 336 ms                                                       | 240 ms: 1.40x faster                                                   |
| async_tree_cpu_io_mixed    | 666 ms                                                       | 487 ms: 1.37x faster                                                   |
| async_tree_memoization_tg  | 414 ms                                                       | 306 ms: 1.35x faster                                                   |
| async_tree_cpu_io_mixed_tg | 638 ms                                                       | 485 ms: 1.31x faster                                                   |
| async_generators           | 377 ms                                                       | 365 ms: 1.03x faster                                                   |
| coroutines                 | 23.6 ms                                                      | 23.9 ms: 1.01x slower                                                  |
| asyncio_websockets         | 520 ms                                                       | 544 ms: 1.05x slower                                                   |
| Geometric mean             | (ref)                                                        | 1.31x faster                                                           |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20251210-vultr-x86_64-python-46295677a13f141b8d52-3.15.0a2+-4629567 |
|----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| float          | 77.5 ms                                                      | 60.9 ms: 1.27x faster                                                  |
| pidigits       | 217 ms                                                       | 194 ms: 1.11x faster                                                   |
| nbody          | 85.1 ms                                                      | 84.4 ms: 1.01x faster                                                  |
| Geometric mean | (ref)                                                        | 1.13x faster                                                           |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20251210-vultr-x86_64-python-46295677a13f141b8d52-3.15.0a2+-4629567 |
|----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| regex_effbot   | 3.08 ms                                                      | 2.78 ms: 1.11x faster                                                  |
| regex_compile  | 132 ms                                                       | 124 ms: 1.06x faster                                                   |
| regex_dna      | 180 ms                                                       | 176 ms: 1.02x faster                                                   |
| regex_v8       | 22.7 ms                                                      | 22.4 ms: 1.01x faster                                                  |
| Geometric mean | (ref)                                                        | 1.05x faster                                                           |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20251210-vultr-x86_64-python-46295677a13f141b8d52-3.15.0a2+-4629567 |
|----------------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| xml_etree_iterparse  | 94.9 ms                                                      | 82.6 ms: 1.15x faster                                                  |
| unpickle_pure_python | 210 us                                                       | 186 us: 1.13x faster                                                   |
| json_dumps           | 10.5 ms                                                      | 9.36 ms: 1.13x faster                                                  |
| xml_etree_process    | 59.3 ms                                                      | 54.5 ms: 1.09x faster                                                  |
| xml_etree_generate   | 85.4 ms                                                      | 78.7 ms: 1.09x faster                                                  |
| xml_etree_parse      | 136 ms                                                       | 129 ms: 1.06x faster                                                   |
| pickle_pure_python   | 294 us                                                       | 292 us: 1.01x faster                                                   |
| json_loads           | 27.0 us                                                      | 28.4 us: 1.05x slower                                                  |
| tomli_loads          | 2.01 sec                                                     | 2.28 sec: 1.14x slower                                                 |
| Geometric mean       | (ref)                                                        | 1.05x faster                                                           |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20251210-vultr-x86_64-python-46295677a13f141b8d52-3.15.0a2+-4629567 |
|------------------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| python_startup_no_site | 7.39 ms                                                      | 7.36 ms: 1.00x faster                                                  |
| python_startup         | 11.0 ms                                                      | 12.6 ms: 1.15x slower                                                  |
| Geometric mean         | (ref)                                                        | 1.07x slower                                                           |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20251210-vultr-x86_64-python-46295677a13f141b8d52-3.15.0a2+-4629567 |
|-----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| mako            | 11.3 ms                                                      | 10.4 ms: 1.09x faster                                                  |
| genshi_text     | 21.5 ms                                                      | 20.9 ms: 1.03x faster                                                  |
| django_template | 34.1 ms                                                      | 36.3 ms: 1.06x slower                                                  |
| genshi_xml      | 48.8 ms                                                      | 53.3 ms: 1.09x slower                                                  |
| Geometric mean  | (ref)                                                        | 1.01x slower                                                           |

All benchmarks:
===============

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20251210-vultr-x86_64-python-46295677a13f141b8d52-3.15.0a2+-4629567 |
|----------------------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| richards                   | 45.2 ms                                                      | 21.2 ms: 2.13x faster                                                  |
| richards_super             | 51.6 ms                                                      | 25.6 ms: 2.02x faster                                                  |
| mdp                        | 2.36 sec                                                     | 1.33 sec: 1.77x faster                                                 |
| async_tree_io              | 876 ms                                                       | 552 ms: 1.59x faster                                                   |
| async_tree_io_tg           | 913 ms                                                       | 581 ms: 1.57x faster                                                   |
| deepcopy_memo              | 39.1 us                                                      | 25.1 us: 1.56x faster                                                  |
| async_tree_memoization     | 461 ms                                                       | 300 ms: 1.54x faster                                                   |
| async_tree_none            | 354 ms                                                       | 241 ms: 1.47x faster                                                   |
| deepcopy                   | 355 us                                                       | 247 us: 1.44x faster                                                   |
| async_tree_none_tg         | 336 ms                                                       | 240 ms: 1.40x faster                                                   |
| go                         | 141 ms                                                       | 101 ms: 1.39x faster                                                   |
| scimark_sor                | 134 ms                                                       | 97.7 ms: 1.38x faster                                                  |
| scimark_fft                | 349 ms                                                       | 255 ms: 1.37x faster                                                   |
| async_tree_cpu_io_mixed    | 666 ms                                                       | 487 ms: 1.37x faster                                                   |
| async_tree_memoization_tg  | 414 ms                                                       | 306 ms: 1.35x faster                                                   |
| async_tree_cpu_io_mixed_tg | 638 ms                                                       | 485 ms: 1.31x faster                                                   |
| spectral_norm              | 111 ms                                                       | 84.6 ms: 1.31x faster                                                  |
| float                      | 77.5 ms                                                      | 60.9 ms: 1.27x faster                                                  |
| deepcopy_reduce            | 3.11 us                                                      | 2.52 us: 1.24x faster                                                  |
| logging_silent             | 103 ns                                                       | 86.6 ns: 1.18x faster                                                  |
| pyflate                    | 449 ms                                                       | 380 ms: 1.18x faster                                                   |
| scimark_monte_carlo        | 65.4 ms                                                      | 55.4 ms: 1.18x faster                                                  |
| xml_etree_iterparse        | 94.9 ms                                                      | 82.6 ms: 1.15x faster                                                  |
| pprint_safe_repr           | 738 ms                                                       | 652 ms: 1.13x faster                                                   |
| unpickle_pure_python       | 210 us                                                       | 186 us: 1.13x faster                                                   |
| json_dumps                 | 10.5 ms                                                      | 9.36 ms: 1.13x faster                                                  |
| pidigits                   | 217 ms                                                       | 194 ms: 1.11x faster                                                   |
| bpe_tokeniser              | 4.45 sec                                                     | 4.00 sec: 1.11x faster                                                 |
| scimark_sparse_mat_mult    | 4.71 ms                                                      | 4.24 ms: 1.11x faster                                                  |
| regex_effbot               | 3.08 ms                                                      | 2.78 ms: 1.11x faster                                                  |
| scimark_lu                 | 113 ms                                                       | 102 ms: 1.10x faster                                                   |
| deltablue                  | 3.12 ms                                                      | 2.84 ms: 1.10x faster                                                  |
| pprint_pformat             | 1.50 sec                                                     | 1.37 sec: 1.09x faster                                                 |
| xml_etree_process          | 59.3 ms                                                      | 54.5 ms: 1.09x faster                                                  |
| mako                       | 11.3 ms                                                      | 10.4 ms: 1.09x faster                                                  |
| xml_etree_generate         | 85.4 ms                                                      | 78.7 ms: 1.09x faster                                                  |
| pathlib                    | 19.2 ms                                                      | 17.8 ms: 1.08x faster                                                  |
| regex_compile              | 132 ms                                                       | 124 ms: 1.06x faster                                                   |
| xml_etree_parse            | 136 ms                                                       | 129 ms: 1.06x faster                                                   |
| logging_simple             | 6.16 us                                                      | 5.86 us: 1.05x faster                                                  |
| dulwich_log                | 74.8 ms                                                      | 71.2 ms: 1.05x faster                                                  |
| 2to3                       | 260 ms                                                       | 247 ms: 1.05x faster                                                   |
| chaos                      | 57.3 ms                                                      | 54.6 ms: 1.05x faster                                                  |
| meteor_contest             | 102 ms                                                       | 97.0 ms: 1.05x faster                                                  |
| thrift                     | 778 us                                                       | 747 us: 1.04x faster                                                   |
| comprehensions             | 16.5 us                                                      | 15.9 us: 1.03x faster                                                  |
| async_generators           | 377 ms                                                       | 365 ms: 1.03x faster                                                   |
| genshi_text                | 21.5 ms                                                      | 20.9 ms: 1.03x faster                                                  |
| fannkuch                   | 370 ms                                                       | 359 ms: 1.03x faster                                                   |
| coverage                   | 83.0 ms                                                      | 80.8 ms: 1.03x faster                                                  |
| regex_dna                  | 180 ms                                                       | 176 ms: 1.02x faster                                                   |
| logging_format             | 6.84 us                                                      | 6.72 us: 1.02x faster                                                  |
| regex_v8                   | 22.7 ms                                                      | 22.4 ms: 1.01x faster                                                  |
| html5lib                   | 67.0 ms                                                      | 66.1 ms: 1.01x faster                                                  |
| generators                 | 28.8 ms                                                      | 28.4 ms: 1.01x faster                                                  |
| sqlite_synth               | 2.21 us                                                      | 2.18 us: 1.01x faster                                                  |
| pickle_pure_python         | 294 us                                                       | 292 us: 1.01x faster                                                   |
| nbody                      | 85.1 ms                                                      | 84.4 ms: 1.01x faster                                                  |
| python_startup_no_site     | 7.39 ms                                                      | 7.36 ms: 1.00x faster                                                  |
| hexiom                     | 5.99 ms                                                      | 6.06 ms: 1.01x slower                                                  |
| typing_runtime_protocols   | 155 us                                                       | 157 us: 1.01x slower                                                   |
| coroutines                 | 23.6 ms                                                      | 23.9 ms: 1.01x slower                                                  |
| json                       | 4.93 ms                                                      | 5.03 ms: 1.02x slower                                                  |
| docutils                   | 2.62 sec                                                     | 2.73 sec: 1.04x slower                                                 |
| asyncio_websockets         | 520 ms                                                       | 544 ms: 1.05x slower                                                   |
| sympy_integrate            | 19.8 ms                                                      | 20.8 ms: 1.05x slower                                                  |
| json_loads                 | 27.0 us                                                      | 28.4 us: 1.05x slower                                                  |
| raytrace                   | 253 ms                                                       | 267 ms: 1.06x slower                                                   |
| django_template            | 34.1 ms                                                      | 36.3 ms: 1.06x slower                                                  |
| nqueens                    | 78.6 ms                                                      | 84.4 ms: 1.07x slower                                                  |
| genshi_xml                 | 48.8 ms                                                      | 53.3 ms: 1.09x slower                                                  |
| sympy_sum                  | 156 ms                                                       | 172 ms: 1.10x slower                                                   |
| sympy_expand               | 457 ms                                                       | 505 ms: 1.10x slower                                                   |
| sympy_str                  | 275 ms                                                       | 308 ms: 1.12x slower                                                   |
| tomli_loads                | 2.01 sec                                                     | 2.28 sec: 1.14x slower                                                 |
| python_startup             | 11.0 ms                                                      | 12.6 ms: 1.15x slower                                                  |
| gc_traversal               | 3.14 ms                                                      | 4.20 ms: 1.34x slower                                                  |
| create_gc_cycles           | 1.34 ms                                                      | 1.89 ms: 1.41x slower                                                  |
| bench_thread_pool          | 919 us                                                       | 1.32 ms: 1.44x slower                                                  |
| telco                      | 7.82 ms                                                      | 161 ms: 20.63x slower                                                  |
| bench_mp_pool              | 11.0 ms                                                      | 239 ms: 21.73x slower                                                  |
| Geometric mean             | (ref)                                                        | 1.02x faster                                                           |

Benchmark hidden because not significant (3): pylint, pycparser, crypto_pyaes
Ignored benchmarks (18) of results/bm-20240906-3.13.0rc2-ec61006/bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006.json: aiohttp, asyncio_tcp, asyncio_tcp_ssl, chameleon, dask, flaskblogging, gunicorn, pickle, pickle_dict, pickle_list, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http, unpack_sequence, unpickle, unpickle_list
Ignored benchmarks (10) of results/bm-20251210-3.15.0a2+-4629567-JIT/bm-20251210-vultr-x86_64-python-46295677a13f141b8d52-3.15.0a2+-4629567.json: connected_components, k_core, many_optionals, shortest_path, sphinx, sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile, subparsers

- Geometric mean (including insignificant results): 1.066x faster

# HPT report

- Reliability score: 99.99% likely to be faster
- 90% likely to have a speedup of 1.04x
- 95% likely to have a speedup of 1.03x
- 99% likely to have a speedup of 1.02x

# Memory
- memory change: 1.18x