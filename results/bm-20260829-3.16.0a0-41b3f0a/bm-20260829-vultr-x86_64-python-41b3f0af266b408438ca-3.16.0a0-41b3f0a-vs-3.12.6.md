# Results vs. 3.12.6

- fork: python
- ref: 41b3f0af266b408438ca
- machine: linux-x86_64
- commit hash: 41b3f0a
- commit date: 2026-08-29
- overall geometric mean: 1.067x faster
- HPT reliability: 100.00%
- HPT 99th percentile: 1.02x faster
- Memory change: 1.15x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260829-vultr-x86_64-python-41b3f0af266b408438ca-3.16.0a0-41b3f0a |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| 2to3           | 264 ms                                                 | 262 ms: 1.01x faster                                                  |
| docutils       | 2.64 sec                                               | 2.39 sec: 1.11x faster                                                |
| html5lib       | 63.6 ms                                                | 60.3 ms: 1.05x faster                                                 |
| Geometric mean | (ref)                                                  | 1.06x faster                                                          |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260829-vultr-x86_64-python-41b3f0af266b408438ca-3.16.0a0-41b3f0a |
|----------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| async_tree_none            | 464 ms                                                 | 293 ms: 1.59x faster                                                  |
| async_tree_memoization_tg  | 560 ms                                                 | 368 ms: 1.52x faster                                                  |
| async_tree_io              | 1.08 sec                                               | 721 ms: 1.50x faster                                                  |
| async_tree_none_tg         | 446 ms                                                 | 300 ms: 1.49x faster                                                  |
| async_tree_memoization     | 555 ms                                                 | 375 ms: 1.48x faster                                                  |
| async_tree_io_tg           | 1.11 sec                                               | 763 ms: 1.45x faster                                                  |
| async_tree_cpu_io_mixed    | 715 ms                                                 | 540 ms: 1.32x faster                                                  |
| async_tree_cpu_io_mixed_tg | 723 ms                                                 | 561 ms: 1.29x faster                                                  |
| asyncio_tcp                | 519 ms                                                 | 448 ms: 1.16x faster                                                  |
| async_generators           | 384 ms                                                 | 346 ms: 1.11x faster                                                  |
| coroutines                 | 23.9 ms                                                | 22.9 ms: 1.04x faster                                                 |
| asyncio_tcp_ssl            | 1.51 sec                                               | 1.45 sec: 1.04x faster                                                |
| asyncio_websockets         | 517 ms                                                 | 543 ms: 1.05x slower                                                  |
| Geometric mean             | (ref)                                                  | 1.29x faster                                                          |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260829-vultr-x86_64-python-41b3f0af266b408438ca-3.16.0a0-41b3f0a |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| float          | 80.8 ms                                                | 72.3 ms: 1.12x faster                                                 |
| pidigits       | 184 ms                                                 | 184 ms: 1.00x faster                                                  |
| Geometric mean | (ref)                                                  | 1.04x faster                                                          |

Benchmark hidden because not significant (1): nbody

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260829-vultr-x86_64-python-41b3f0af266b408438ca-3.16.0a0-41b3f0a |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| regex_effbot   | 3.17 ms                                                | 2.74 ms: 1.16x faster                                                 |
| regex_compile  | 142 ms                                                 | 149 ms: 1.05x slower                                                  |
| regex_v8       | 20.6 ms                                                | 22.6 ms: 1.10x slower                                                 |
| regex_dna      | 168 ms                                                 | 186 ms: 1.11x slower                                                  |
| Geometric mean | (ref)                                                  | 1.02x slower                                                          |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260829-vultr-x86_64-python-41b3f0af266b408438ca-3.16.0a0-41b3f0a |
|----------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| tomli_loads          | 2.11 sec                                               | 1.83 sec: 1.15x faster                                                |
| json_dumps           | 10.4 ms                                                | 9.38 ms: 1.10x faster                                                 |
| xml_etree_iterparse  | 96.7 ms                                                | 90.2 ms: 1.07x faster                                                 |
| unpickle_pure_python | 221 us                                                 | 211 us: 1.05x faster                                                  |
| pickle_pure_python   | 308 us                                                 | 301 us: 1.02x faster                                                  |
| json_loads           | 26.5 us                                                | 27.2 us: 1.02x slower                                                 |
| xml_etree_generate   | 85.2 ms                                                | 87.4 ms: 1.03x slower                                                 |
| xml_etree_parse      | 139 ms                                                 | 143 ms: 1.03x slower                                                  |
| xml_etree_process    | 59.0 ms                                                | 61.8 ms: 1.05x slower                                                 |
| unpickle             | 14.1 us                                                | 14.8 us: 1.05x slower                                                 |
| pickle_dict          | 31.8 us                                                | 34.5 us: 1.09x slower                                                 |
| unpickle_list        | 4.67 us                                                | 5.09 us: 1.09x slower                                                 |
| pickle_list          | 4.77 us                                                | 5.47 us: 1.15x slower                                                 |
| pickle               | 10.9 us                                                | 13.2 us: 1.21x slower                                                 |
| Geometric mean       | (ref)                                                  | 1.02x slower                                                          |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260829-vultr-x86_64-python-41b3f0af266b408438ca-3.16.0a0-41b3f0a |
|------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| python_startup_no_site | 7.16 ms                                                | 7.76 ms: 1.08x slower                                                 |
| python_startup         | 9.93 ms                                                | 13.0 ms: 1.31x slower                                                 |
| Geometric mean         | (ref)                                                  | 1.19x slower                                                          |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260829-vultr-x86_64-python-41b3f0af266b408438ca-3.16.0a0-41b3f0a |
|-----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| django_template | 34.7 ms                                                | 36.4 ms: 1.05x slower                                                 |
| mako            | 11.0 ms                                                | 11.9 ms: 1.08x slower                                                 |
| Geometric mean  | (ref)                                                  | 1.07x slower                                                          |

All benchmarks:
===============

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260829-vultr-x86_64-python-41b3f0af266b408438ca-3.16.0a0-41b3f0a |
|----------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| pylint                     | 319 ms                                                 | 113 ms: 2.81x faster                                                  |
| mdp                        | 2.42 sec                                               | 1.14 sec: 2.13x faster                                                |
| async_tree_none            | 464 ms                                                 | 293 ms: 1.59x faster                                                  |
| async_tree_memoization_tg  | 560 ms                                                 | 368 ms: 1.52x faster                                                  |
| deepcopy                   | 352 us                                                 | 234 us: 1.50x faster                                                  |
| async_tree_io              | 1.08 sec                                               | 721 ms: 1.50x faster                                                  |
| deepcopy_memo              | 40.3 us                                                | 27.0 us: 1.50x faster                                                 |
| async_tree_none_tg         | 446 ms                                                 | 300 ms: 1.49x faster                                                  |
| async_tree_memoization     | 555 ms                                                 | 375 ms: 1.48x faster                                                  |
| async_tree_io_tg           | 1.11 sec                                               | 763 ms: 1.45x faster                                                  |
| go                         | 139 ms                                                 | 102 ms: 1.36x faster                                                  |
| typing_runtime_protocols   | 163 us                                                 | 122 us: 1.33x faster                                                  |
| async_tree_cpu_io_mixed    | 715 ms                                                 | 540 ms: 1.32x faster                                                  |
| async_tree_cpu_io_mixed_tg | 723 ms                                                 | 561 ms: 1.29x faster                                                  |
| comprehensions             | 19.8 us                                                | 15.8 us: 1.26x faster                                                 |
| scimark_sor                | 130 ms                                                 | 105 ms: 1.23x faster                                                  |
| pathlib                    | 21.5 ms                                                | 17.5 ms: 1.23x faster                                                 |
| spectral_norm              | 110 ms                                                 | 90.2 ms: 1.22x faster                                                 |
| raytrace                   | 299 ms                                                 | 250 ms: 1.20x faster                                                  |
| chaos                      | 62.8 ms                                                | 52.9 ms: 1.19x faster                                                 |
| dulwich_log                | 78.9 ms                                                | 67.0 ms: 1.18x faster                                                 |
| deepcopy_reduce            | 3.08 us                                                | 2.62 us: 1.18x faster                                                 |
| generators                 | 32.2 ms                                                | 27.8 ms: 1.16x faster                                                 |
| asyncio_tcp                | 519 ms                                                 | 448 ms: 1.16x faster                                                  |
| regex_effbot               | 3.17 ms                                                | 2.74 ms: 1.16x faster                                                 |
| tomli_loads                | 2.11 sec                                               | 1.83 sec: 1.15x faster                                                |
| logging_silent             | 109 ns                                                 | 95.1 ns: 1.15x faster                                                 |
| pyflate                    | 448 ms                                                 | 392 ms: 1.14x faster                                                  |
| crypto_pyaes               | 76.6 ms                                                | 67.4 ms: 1.14x faster                                                 |
| bpe_tokeniser              | 4.74 sec                                               | 4.22 sec: 1.12x faster                                                |
| float                      | 80.8 ms                                                | 72.3 ms: 1.12x faster                                                 |
| scimark_fft                | 342 ms                                                 | 307 ms: 1.11x faster                                                  |
| async_generators           | 384 ms                                                 | 346 ms: 1.11x faster                                                  |
| docutils                   | 2.64 sec                                               | 2.39 sec: 1.11x faster                                                |
| json_dumps                 | 10.4 ms                                                | 9.38 ms: 1.10x faster                                                 |
| hexiom                     | 6.17 ms                                                | 5.62 ms: 1.10x faster                                                 |
| nqueens                    | 80.1 ms                                                | 73.1 ms: 1.10x faster                                                 |
| scimark_monte_carlo        | 68.4 ms                                                | 62.8 ms: 1.09x faster                                                 |
| unpack_sequence            | 52.1 ns                                                | 48.0 ns: 1.09x faster                                                 |
| logging_simple             | 6.63 us                                                | 6.18 us: 1.07x faster                                                 |
| xml_etree_iterparse        | 96.7 ms                                                | 90.2 ms: 1.07x faster                                                 |
| sympy_integrate            | 20.5 ms                                                | 19.4 ms: 1.06x faster                                                 |
| html5lib                   | 63.6 ms                                                | 60.3 ms: 1.05x faster                                                 |
| pycparser                  | 1.17 sec                                               | 1.11 sec: 1.05x faster                                                |
| unpickle_pure_python       | 221 us                                                 | 211 us: 1.05x faster                                                  |
| coroutines                 | 23.9 ms                                                | 22.9 ms: 1.04x faster                                                 |
| logging_format             | 7.35 us                                                | 7.04 us: 1.04x faster                                                 |
| deltablue                  | 3.45 ms                                                | 3.30 ms: 1.04x faster                                                 |
| asyncio_tcp_ssl            | 1.51 sec                                               | 1.45 sec: 1.04x faster                                                |
| sympy_str                  | 292 ms                                                 | 281 ms: 1.04x faster                                                  |
| sympy_sum                  | 166 ms                                                 | 161 ms: 1.03x faster                                                  |
| pickle_pure_python         | 308 us                                                 | 301 us: 1.02x faster                                                  |
| meteor_contest             | 104 ms                                                 | 101 ms: 1.02x faster                                                  |
| json                       | 5.02 ms                                                | 4.92 ms: 1.02x faster                                                 |
| richards                   | 45.9 ms                                                | 45.0 ms: 1.02x faster                                                 |
| pprint_pformat             | 1.52 sec                                               | 1.49 sec: 1.02x faster                                                |
| thrift                     | 791 us                                                 | 778 us: 1.02x faster                                                  |
| richards_super             | 51.9 ms                                                | 51.1 ms: 1.01x faster                                                 |
| pprint_safe_repr           | 743 ms                                                 | 734 ms: 1.01x faster                                                  |
| 2to3                       | 264 ms                                                 | 262 ms: 1.01x faster                                                  |
| pidigits                   | 184 ms                                                 | 184 ms: 1.00x faster                                                  |
| json_loads                 | 26.5 us                                                | 27.2 us: 1.02x slower                                                 |
| xml_etree_generate         | 85.2 ms                                                | 87.4 ms: 1.03x slower                                                 |
| sympy_expand               | 468 ms                                                 | 481 ms: 1.03x slower                                                  |
| xml_etree_parse            | 139 ms                                                 | 143 ms: 1.03x slower                                                  |
| scimark_sparse_mat_mult    | 4.39 ms                                                | 4.59 ms: 1.04x slower                                                 |
| regex_compile              | 142 ms                                                 | 149 ms: 1.05x slower                                                  |
| xml_etree_process          | 59.0 ms                                                | 61.8 ms: 1.05x slower                                                 |
| django_template            | 34.7 ms                                                | 36.4 ms: 1.05x slower                                                 |
| asyncio_websockets         | 517 ms                                                 | 543 ms: 1.05x slower                                                  |
| unpickle                   | 14.1 us                                                | 14.8 us: 1.05x slower                                                 |
| mako                       | 11.0 ms                                                | 11.9 ms: 1.08x slower                                                 |
| python_startup_no_site     | 7.16 ms                                                | 7.76 ms: 1.08x slower                                                 |
| pickle_dict                | 31.8 us                                                | 34.5 us: 1.09x slower                                                 |
| unpickle_list              | 4.67 us                                                | 5.09 us: 1.09x slower                                                 |
| regex_v8                   | 20.6 ms                                                | 22.6 ms: 1.10x slower                                                 |
| regex_dna                  | 168 ms                                                 | 186 ms: 1.11x slower                                                  |
| gc_traversal               | 3.46 ms                                                | 3.86 ms: 1.12x slower                                                 |
| pickle_list                | 4.77 us                                                | 5.47 us: 1.15x slower                                                 |
| coverage                   | 71.4 ms                                                | 83.2 ms: 1.16x slower                                                 |
| pickle                     | 10.9 us                                                | 13.2 us: 1.21x slower                                                 |
| python_startup             | 9.93 ms                                                | 13.0 ms: 1.31x slower                                                 |
| bench_thread_pool          | 941 us                                                 | 1.35 ms: 1.44x slower                                                 |
| create_gc_cycles           | 1.09 ms                                                | 1.67 ms: 1.53x slower                                                 |
| bench_mp_pool              | 10.8 ms                                                | 242 ms: 22.38x slower                                                 |
| telco                      | 6.53 ms                                                | 163 ms: 25.05x slower                                                 |
| Geometric mean             | (ref)                                                  | 1.02x faster                                                          |

Benchmark hidden because not significant (4): fannkuch, sqlite_synth, scimark_lu, nbody
Ignored benchmarks (15) of results/bm-20240906-3.12.6-a4a2d2b/bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b.json: aiohttp, chameleon, dask, flaskblogging, genshi_text, genshi_xml, gunicorn, mypy2, sqlalchemy_declarative, sqlalchemy_imperative, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http
Ignored benchmarks (10) of results/bm-20260829-3.16.0a0-41b3f0a/bm-20260829-vultr-x86_64-python-41b3f0af266b408438ca-3.16.0a0-41b3f0a.json: connected_components, k_core, many_optionals, shortest_path, sphinx, sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile, subparsers

- Geometric mean (including insignificant results): 1.067x faster

# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.04x
- 95% likely to have a speedup of 1.03x
- 99% likely to have a speedup of 1.02x

# Memory
- memory change: 1.15x