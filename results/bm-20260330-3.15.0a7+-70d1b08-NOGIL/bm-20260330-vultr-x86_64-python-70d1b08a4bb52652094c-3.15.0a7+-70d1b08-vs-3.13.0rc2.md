# Results vs. 3.13.0rc2

- fork: python
- ref: 70d1b08a4bb52652094c
- machine: linux-x86_64
- commit hash: 70d1b08
- commit date: 2026-03-30
- overall geometric mean: 1.057x slower
- HPT reliability: 78.00%
- HPT 99th percentile: 1.00x slower
- Memory change: 1.41x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260330-vultr-x86_64-python-70d1b08a4bb52652094c-3.15.0a7+-70d1b08 |
|----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| 2to3           | 260 ms                                                       | 281 ms: 1.08x slower                                                   |
| html5lib       | 67.0 ms                                                      | 65.8 ms: 1.02x faster                                                  |
| Geometric mean | (ref)                                                        | 1.02x slower                                                           |

Benchmark hidden because not significant (1): docutils

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260330-vultr-x86_64-python-70d1b08a4bb52652094c-3.15.0a7+-70d1b08 |
|----------------------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| async_tree_io_tg           | 913 ms                                                       | 609 ms: 1.50x faster                                                   |
| async_tree_io              | 876 ms                                                       | 643 ms: 1.36x faster                                                   |
| async_tree_memoization     | 461 ms                                                       | 353 ms: 1.30x faster                                                   |
| async_tree_memoization_tg  | 414 ms                                                       | 324 ms: 1.28x faster                                                   |
| async_tree_none_tg         | 336 ms                                                       | 264 ms: 1.27x faster                                                   |
| async_tree_cpu_io_mixed_tg | 638 ms                                                       | 514 ms: 1.24x faster                                                   |
| async_tree_cpu_io_mixed    | 666 ms                                                       | 541 ms: 1.23x faster                                                   |
| async_tree_none            | 354 ms                                                       | 289 ms: 1.22x faster                                                   |
| asyncio_websockets         | 520 ms                                                       | 515 ms: 1.01x faster                                                   |
| async_generators           | 377 ms                                                       | 374 ms: 1.01x faster                                                   |
| Geometric mean             | (ref)                                                        | 1.21x faster                                                           |

Benchmark hidden because not significant (1): coroutines

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260330-vultr-x86_64-python-70d1b08a4bb52652094c-3.15.0a7+-70d1b08 |
|----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| pidigits       | 217 ms                                                       | 188 ms: 1.15x faster                                                   |
| float          | 77.5 ms                                                      | 74.1 ms: 1.05x faster                                                  |
| nbody          | 85.1 ms                                                      | 124 ms: 1.45x slower                                                   |
| Geometric mean | (ref)                                                        | 1.06x slower                                                           |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260330-vultr-x86_64-python-70d1b08a4bb52652094c-3.15.0a7+-70d1b08 |
|----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| regex_effbot   | 3.08 ms                                                      | 2.79 ms: 1.11x faster                                                  |
| regex_v8       | 22.7 ms                                                      | 20.9 ms: 1.09x faster                                                  |
| regex_dna      | 180 ms                                                       | 174 ms: 1.04x faster                                                   |
| regex_compile  | 132 ms                                                       | 142 ms: 1.07x slower                                                   |
| Geometric mean | (ref)                                                        | 1.04x faster                                                           |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260330-vultr-x86_64-python-70d1b08a4bb52652094c-3.15.0a7+-70d1b08 |
|----------------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| xml_etree_iterparse  | 94.9 ms                                                      | 88.4 ms: 1.07x faster                                                  |
| tomli_loads          | 2.01 sec                                                     | 1.91 sec: 1.05x faster                                                 |
| xml_etree_parse      | 136 ms                                                       | 132 ms: 1.03x faster                                                   |
| json_dumps           | 10.5 ms                                                      | 11.0 ms: 1.05x slower                                                  |
| json_loads           | 27.0 us                                                      | 29.9 us: 1.11x slower                                                  |
| xml_etree_generate   | 85.4 ms                                                      | 96.0 ms: 1.12x slower                                                  |
| unpickle_pure_python | 210 us                                                       | 236 us: 1.13x slower                                                   |
| pickle_pure_python   | 294 us                                                       | 336 us: 1.14x slower                                                   |
| xml_etree_process    | 59.3 ms                                                      | 68.4 ms: 1.15x slower                                                  |
| Geometric mean       | (ref)                                                        | 1.06x slower                                                           |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260330-vultr-x86_64-python-70d1b08a4bb52652094c-3.15.0a7+-70d1b08 |
|------------------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| python_startup_no_site | 7.39 ms                                                      | 9.91 ms: 1.34x slower                                                  |
| python_startup         | 11.0 ms                                                      | 16.1 ms: 1.47x slower                                                  |
| Geometric mean         | (ref)                                                        | 1.40x slower                                                           |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260330-vultr-x86_64-python-70d1b08a4bb52652094c-3.15.0a7+-70d1b08 |
|-----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| genshi_xml      | 48.8 ms                                                      | 55.5 ms: 1.14x slower                                                  |
| genshi_text     | 21.5 ms                                                      | 25.5 ms: 1.19x slower                                                  |
| django_template | 34.1 ms                                                      | 40.9 ms: 1.20x slower                                                  |
| mako            | 11.3 ms                                                      | 15.9 ms: 1.41x slower                                                  |
| Geometric mean  | (ref)                                                        | 1.23x slower                                                           |

All benchmarks:
===============

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260330-vultr-x86_64-python-70d1b08a4bb52652094c-3.15.0a7+-70d1b08 |
|----------------------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| mdp                        | 2.36 sec                                                     | 1.32 sec: 1.78x faster                                                 |
| gc_traversal               | 3.14 ms                                                      | 1.93 ms: 1.63x faster                                                  |
| bench_mp_pool              | 11.0 ms                                                      | 7.05 ms: 1.56x faster                                                  |
| async_tree_io_tg           | 913 ms                                                       | 609 ms: 1.50x faster                                                   |
| async_tree_io              | 876 ms                                                       | 643 ms: 1.36x faster                                                   |
| deepcopy                   | 355 us                                                       | 267 us: 1.33x faster                                                   |
| async_tree_memoization     | 461 ms                                                       | 353 ms: 1.30x faster                                                   |
| async_tree_memoization_tg  | 414 ms                                                       | 324 ms: 1.28x faster                                                   |
| async_tree_none_tg         | 336 ms                                                       | 264 ms: 1.27x faster                                                   |
| deepcopy_memo              | 39.1 us                                                      | 31.3 us: 1.25x faster                                                  |
| async_tree_cpu_io_mixed_tg | 638 ms                                                       | 514 ms: 1.24x faster                                                   |
| async_tree_cpu_io_mixed    | 666 ms                                                       | 541 ms: 1.23x faster                                                   |
| async_tree_none            | 354 ms                                                       | 289 ms: 1.22x faster                                                   |
| go                         | 141 ms                                                       | 121 ms: 1.17x faster                                                   |
| pidigits                   | 217 ms                                                       | 188 ms: 1.15x faster                                                   |
| sqlite_synth               | 2.21 us                                                      | 1.93 us: 1.15x faster                                                  |
| scimark_sor                | 134 ms                                                       | 121 ms: 1.11x faster                                                   |
| regex_effbot               | 3.08 ms                                                      | 2.79 ms: 1.11x faster                                                  |
| pathlib                    | 19.2 ms                                                      | 17.4 ms: 1.10x faster                                                  |
| deepcopy_reduce            | 3.11 us                                                      | 2.87 us: 1.09x faster                                                  |
| regex_v8                   | 22.7 ms                                                      | 20.9 ms: 1.09x faster                                                  |
| xml_etree_iterparse        | 94.9 ms                                                      | 88.4 ms: 1.07x faster                                                  |
| pylint                     | 317 ms                                                       | 298 ms: 1.06x faster                                                   |
| dulwich_log                | 74.8 ms                                                      | 70.5 ms: 1.06x faster                                                  |
| tomli_loads                | 2.01 sec                                                     | 1.91 sec: 1.05x faster                                                 |
| float                      | 77.5 ms                                                      | 74.1 ms: 1.05x faster                                                  |
| regex_dna                  | 180 ms                                                       | 174 ms: 1.04x faster                                                   |
| spectral_norm              | 111 ms                                                       | 108 ms: 1.03x faster                                                   |
| xml_etree_parse            | 136 ms                                                       | 132 ms: 1.03x faster                                                   |
| bpe_tokeniser              | 4.45 sec                                                     | 4.32 sec: 1.03x faster                                                 |
| html5lib                   | 67.0 ms                                                      | 65.8 ms: 1.02x faster                                                  |
| logging_silent             | 103 ns                                                       | 101 ns: 1.02x faster                                                   |
| asyncio_websockets         | 520 ms                                                       | 515 ms: 1.01x faster                                                   |
| scimark_fft                | 349 ms                                                       | 346 ms: 1.01x faster                                                   |
| async_generators           | 377 ms                                                       | 374 ms: 1.01x faster                                                   |
| pycparser                  | 1.12 sec                                                     | 1.11 sec: 1.01x faster                                                 |
| json                       | 4.93 ms                                                      | 5.11 ms: 1.04x slower                                                  |
| json_dumps                 | 10.5 ms                                                      | 11.0 ms: 1.05x slower                                                  |
| pyflate                    | 449 ms                                                       | 472 ms: 1.05x slower                                                   |
| chaos                      | 57.3 ms                                                      | 61.0 ms: 1.06x slower                                                  |
| regex_compile              | 132 ms                                                       | 142 ms: 1.07x slower                                                   |
| pprint_safe_repr           | 738 ms                                                       | 792 ms: 1.07x slower                                                   |
| 2to3                       | 260 ms                                                       | 281 ms: 1.08x slower                                                   |
| logging_simple             | 6.16 us                                                      | 6.69 us: 1.09x slower                                                  |
| hexiom                     | 5.99 ms                                                      | 6.51 ms: 1.09x slower                                                  |
| sympy_integrate            | 19.8 ms                                                      | 21.7 ms: 1.09x slower                                                  |
| pprint_pformat             | 1.50 sec                                                     | 1.64 sec: 1.10x slower                                                 |
| scimark_lu                 | 113 ms                                                       | 125 ms: 1.11x slower                                                   |
| json_loads                 | 27.0 us                                                      | 29.9 us: 1.11x slower                                                  |
| logging_format             | 6.84 us                                                      | 7.59 us: 1.11x slower                                                  |
| deltablue                  | 3.12 ms                                                      | 3.47 ms: 1.11x slower                                                  |
| comprehensions             | 16.5 us                                                      | 18.4 us: 1.12x slower                                                  |
| sympy_str                  | 275 ms                                                       | 308 ms: 1.12x slower                                                   |
| xml_etree_generate         | 85.4 ms                                                      | 96.0 ms: 1.12x slower                                                  |
| unpickle_pure_python       | 210 us                                                       | 236 us: 1.13x slower                                                   |
| nqueens                    | 78.6 ms                                                      | 88.5 ms: 1.13x slower                                                  |
| create_gc_cycles           | 1.34 ms                                                      | 1.51 ms: 1.13x slower                                                  |
| sympy_expand               | 457 ms                                                       | 517 ms: 1.13x slower                                                   |
| sympy_sum                  | 156 ms                                                       | 177 ms: 1.14x slower                                                   |
| genshi_xml                 | 48.8 ms                                                      | 55.5 ms: 1.14x slower                                                  |
| pickle_pure_python         | 294 us                                                       | 336 us: 1.14x slower                                                   |
| generators                 | 28.8 ms                                                      | 33.0 ms: 1.14x slower                                                  |
| xml_etree_process          | 59.3 ms                                                      | 68.4 ms: 1.15x slower                                                  |
| thrift                     | 778 us                                                       | 899 us: 1.16x slower                                                   |
| richards_super             | 51.6 ms                                                      | 60.2 ms: 1.16x slower                                                  |
| richards                   | 45.2 ms                                                      | 52.7 ms: 1.17x slower                                                  |
| scimark_sparse_mat_mult    | 4.71 ms                                                      | 5.50 ms: 1.17x slower                                                  |
| raytrace                   | 253 ms                                                       | 298 ms: 1.18x slower                                                   |
| genshi_text                | 21.5 ms                                                      | 25.5 ms: 1.19x slower                                                  |
| scimark_monte_carlo        | 65.4 ms                                                      | 78.3 ms: 1.20x slower                                                  |
| django_template            | 34.1 ms                                                      | 40.9 ms: 1.20x slower                                                  |
| meteor_contest             | 102 ms                                                       | 123 ms: 1.21x slower                                                   |
| fannkuch                   | 370 ms                                                       | 456 ms: 1.23x slower                                                   |
| typing_runtime_protocols   | 155 us                                                       | 194 us: 1.25x slower                                                   |
| crypto_pyaes               | 67.9 ms                                                      | 87.7 ms: 1.29x slower                                                  |
| python_startup_no_site     | 7.39 ms                                                      | 9.91 ms: 1.34x slower                                                  |
| coverage                   | 83.0 ms                                                      | 112 ms: 1.35x slower                                                   |
| mako                       | 11.3 ms                                                      | 15.9 ms: 1.41x slower                                                  |
| nbody                      | 85.1 ms                                                      | 124 ms: 1.45x slower                                                   |
| python_startup             | 11.0 ms                                                      | 16.1 ms: 1.47x slower                                                  |
| bench_thread_pool          | 919 us                                                       | 1.46 ms: 1.59x slower                                                  |
| telco                      | 7.82 ms                                                      | 172 ms: 21.97x slower                                                  |
| Geometric mean             | (ref)                                                        | 1.06x slower                                                           |

Benchmark hidden because not significant (2): coroutines, docutils
Ignored benchmarks (18) of results/bm-20240906-3.13.0rc2-ec61006/bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006.json: aiohttp, asyncio_tcp, asyncio_tcp_ssl, chameleon, dask, flaskblogging, gunicorn, pickle, pickle_dict, pickle_list, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http, unpack_sequence, unpickle, unpickle_list
Ignored benchmarks (10) of results/bm-20260330-3.15.0a7+-70d1b08-NOGIL/bm-20260330-vultr-x86_64-python-70d1b08a4bb52652094c-3.15.0a7+-70d1b08.json: connected_components, k_core, many_optionals, shortest_path, sphinx, sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile, subparsers

- Geometric mean (including insignificant results): 1.057x slower

# HPT report

- Reliability score: 78.00% likely to be slow
- 90% likely to have a slowdown of 1.00x
- 95% likely to have a slowdown of 1.00x
- 99% likely to have a slowdown of 1.00x

# Memory
- memory change: 1.41x