# Results vs. 3.12.6

- fork: python
- ref: 432ddd99e2b06a75a4f4
- machine: linux-x86_64
- commit hash: 432ddd9
- commit date: 2026-02-09
- overall geometric mean: 1.081x faster
- HPT reliability: 100.00%
- HPT 99th percentile: 1.05x faster
- Memory change: 1.17x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260209-vultr-x86_64-python-432ddd99e2b06a75a4f4-3.15.0a5+-432ddd9 |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| 2to3           | 264 ms                                                 | 246 ms: 1.07x faster                                                   |
| docutils       | 2.64 sec                                               | 2.52 sec: 1.05x faster                                                 |
| html5lib       | 63.6 ms                                                | 59.6 ms: 1.07x faster                                                  |
| Geometric mean | (ref)                                                  | 1.06x faster                                                           |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260209-vultr-x86_64-python-432ddd99e2b06a75a4f4-3.15.0a5+-432ddd9 |
|----------------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| async_tree_io_tg           | 1.11 sec                                               | 556 ms: 2.00x faster                                                   |
| async_tree_memoization_tg  | 560 ms                                                 | 292 ms: 1.92x faster                                                   |
| async_tree_io              | 1.08 sec                                               | 565 ms: 1.92x faster                                                   |
| async_tree_none            | 464 ms                                                 | 244 ms: 1.90x faster                                                   |
| async_tree_memoization     | 555 ms                                                 | 296 ms: 1.87x faster                                                   |
| async_tree_none_tg         | 446 ms                                                 | 247 ms: 1.81x faster                                                   |
| async_tree_cpu_io_mixed    | 715 ms                                                 | 480 ms: 1.49x faster                                                   |
| async_tree_cpu_io_mixed_tg | 723 ms                                                 | 487 ms: 1.48x faster                                                   |
| async_generators           | 384 ms                                                 | 334 ms: 1.15x faster                                                   |
| coroutines                 | 23.9 ms                                                | 23.1 ms: 1.04x faster                                                  |
| asyncio_websockets         | 517 ms                                                 | 544 ms: 1.05x slower                                                   |
| Geometric mean             | (ref)                                                  | 1.54x faster                                                           |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260209-vultr-x86_64-python-432ddd99e2b06a75a4f4-3.15.0a5+-432ddd9 |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| float          | 80.8 ms                                                | 70.5 ms: 1.15x faster                                                  |
| pidigits       | 184 ms                                                 | 197 ms: 1.07x slower                                                   |
| Geometric mean | (ref)                                                  | 1.02x faster                                                           |

Benchmark hidden because not significant (1): nbody

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260209-vultr-x86_64-python-432ddd99e2b06a75a4f4-3.15.0a5+-432ddd9 |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| regex_compile  | 142 ms                                                 | 122 ms: 1.16x faster                                                   |
| regex_effbot   | 3.17 ms                                                | 2.79 ms: 1.14x faster                                                  |
| regex_dna      | 168 ms                                                 | 172 ms: 1.02x slower                                                   |
| regex_v8       | 20.6 ms                                                | 21.3 ms: 1.03x slower                                                  |
| Geometric mean | (ref)                                                  | 1.06x faster                                                           |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260209-vultr-x86_64-python-432ddd99e2b06a75a4f4-3.15.0a5+-432ddd9 |
|----------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| tomli_loads          | 2.11 sec                                               | 1.83 sec: 1.15x faster                                                 |
| xml_etree_iterparse  | 96.7 ms                                                | 85.3 ms: 1.13x faster                                                  |
| xml_etree_parse      | 139 ms                                                 | 132 ms: 1.05x faster                                                   |
| json_dumps           | 10.4 ms                                                | 9.90 ms: 1.05x faster                                                  |
| unpickle_pure_python | 221 us                                                 | 211 us: 1.05x faster                                                   |
| xml_etree_generate   | 85.2 ms                                                | 83.3 ms: 1.02x faster                                                  |
| xml_etree_process    | 59.0 ms                                                | 58.2 ms: 1.01x faster                                                  |
| json_loads           | 26.5 us                                                | 28.2 us: 1.06x slower                                                  |
| Geometric mean       | (ref)                                                  | 1.04x faster                                                           |

Benchmark hidden because not significant (1): pickle_pure_python

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260209-vultr-x86_64-python-432ddd99e2b06a75a4f4-3.15.0a5+-432ddd9 |
|------------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| python_startup_no_site | 7.16 ms                                                | 7.30 ms: 1.02x slower                                                  |
| python_startup         | 9.93 ms                                                | 12.5 ms: 1.26x slower                                                  |
| Geometric mean         | (ref)                                                  | 1.13x slower                                                           |

Benchmarks with tag 'template':
===============================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260209-vultr-x86_64-python-432ddd99e2b06a75a4f4-3.15.0a5+-432ddd9 |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| genshi_text    | 22.8 ms                                                | 20.9 ms: 1.09x faster                                                  |
| genshi_xml     | 50.2 ms                                                | 48.5 ms: 1.03x faster                                                  |
| mako           | 11.0 ms                                                | 11.5 ms: 1.05x slower                                                  |
| Geometric mean | (ref)                                                  | 1.02x faster                                                           |

Benchmark hidden because not significant (1): django_template

All benchmarks:
===============

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260209-vultr-x86_64-python-432ddd99e2b06a75a4f4-3.15.0a5+-432ddd9 |
|----------------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| mdp                        | 2.42 sec                                               | 1.14 sec: 2.12x faster                                                 |
| async_tree_io_tg           | 1.11 sec                                               | 556 ms: 2.00x faster                                                   |
| async_tree_memoization_tg  | 560 ms                                                 | 292 ms: 1.92x faster                                                   |
| async_tree_io              | 1.08 sec                                               | 565 ms: 1.92x faster                                                   |
| async_tree_none            | 464 ms                                                 | 244 ms: 1.90x faster                                                   |
| async_tree_memoization     | 555 ms                                                 | 296 ms: 1.87x faster                                                   |
| async_tree_none_tg         | 446 ms                                                 | 247 ms: 1.81x faster                                                   |
| deepcopy_memo              | 40.3 us                                                | 25.9 us: 1.55x faster                                                  |
| deepcopy                   | 352 us                                                 | 232 us: 1.52x faster                                                   |
| async_tree_cpu_io_mixed    | 715 ms                                                 | 480 ms: 1.49x faster                                                   |
| async_tree_cpu_io_mixed_tg | 723 ms                                                 | 487 ms: 1.48x faster                                                   |
| go                         | 139 ms                                                 | 106 ms: 1.32x faster                                                   |
| pathlib                    | 21.5 ms                                                | 17.4 ms: 1.24x faster                                                  |
| comprehensions             | 19.8 us                                                | 16.1 us: 1.23x faster                                                  |
| deepcopy_reduce            | 3.08 us                                                | 2.54 us: 1.21x faster                                                  |
| scimark_sor                | 130 ms                                                 | 108 ms: 1.20x faster                                                   |
| raytrace                   | 299 ms                                                 | 252 ms: 1.19x faster                                                   |
| spectral_norm              | 110 ms                                                 | 93.8 ms: 1.17x faster                                                  |
| dulwich_log                | 78.9 ms                                                | 67.6 ms: 1.17x faster                                                  |
| regex_compile              | 142 ms                                                 | 122 ms: 1.16x faster                                                   |
| tomli_loads                | 2.11 sec                                               | 1.83 sec: 1.15x faster                                                 |
| async_generators           | 384 ms                                                 | 334 ms: 1.15x faster                                                   |
| float                      | 80.8 ms                                                | 70.5 ms: 1.15x faster                                                  |
| regex_effbot               | 3.17 ms                                                | 2.79 ms: 1.14x faster                                                  |
| bpe_tokeniser              | 4.74 sec                                               | 4.18 sec: 1.13x faster                                                 |
| xml_etree_iterparse        | 96.7 ms                                                | 85.3 ms: 1.13x faster                                                  |
| chaos                      | 62.8 ms                                                | 55.4 ms: 1.13x faster                                                  |
| pylint                     | 319 ms                                                 | 284 ms: 1.12x faster                                                   |
| crypto_pyaes               | 76.6 ms                                                | 68.6 ms: 1.12x faster                                                  |
| logging_simple             | 6.63 us                                                | 5.95 us: 1.12x faster                                                  |
| scimark_fft                | 342 ms                                                 | 307 ms: 1.11x faster                                                   |
| logging_silent             | 109 ns                                                 | 98.0 ns: 1.11x faster                                                  |
| logging_format             | 7.35 us                                                | 6.67 us: 1.10x faster                                                  |
| generators                 | 32.2 ms                                                | 29.3 ms: 1.10x faster                                                  |
| genshi_text                | 22.8 ms                                                | 20.9 ms: 1.09x faster                                                  |
| scimark_monte_carlo        | 68.4 ms                                                | 62.7 ms: 1.09x faster                                                  |
| pyflate                    | 448 ms                                                 | 411 ms: 1.09x faster                                                   |
| hexiom                     | 6.17 ms                                                | 5.69 ms: 1.08x faster                                                  |
| pycparser                  | 1.17 sec                                               | 1.08 sec: 1.08x faster                                                 |
| richards                   | 45.9 ms                                                | 42.5 ms: 1.08x faster                                                  |
| pprint_safe_repr           | 743 ms                                                 | 689 ms: 1.08x faster                                                   |
| pprint_pformat             | 1.52 sec                                               | 1.41 sec: 1.08x faster                                                 |
| sympy_integrate            | 20.5 ms                                                | 19.1 ms: 1.08x faster                                                  |
| typing_runtime_protocols   | 163 us                                                 | 152 us: 1.07x faster                                                   |
| sympy_sum                  | 166 ms                                                 | 155 ms: 1.07x faster                                                   |
| richards_super             | 51.9 ms                                                | 48.5 ms: 1.07x faster                                                  |
| 2to3                       | 264 ms                                                 | 246 ms: 1.07x faster                                                   |
| html5lib                   | 63.6 ms                                                | 59.6 ms: 1.07x faster                                                  |
| scimark_lu                 | 114 ms                                                 | 107 ms: 1.07x faster                                                   |
| sympy_str                  | 292 ms                                                 | 276 ms: 1.06x faster                                                   |
| xml_etree_parse            | 139 ms                                                 | 132 ms: 1.05x faster                                                   |
| docutils                   | 2.64 sec                                               | 2.52 sec: 1.05x faster                                                 |
| json_dumps                 | 10.4 ms                                                | 9.90 ms: 1.05x faster                                                  |
| unpickle_pure_python       | 221 us                                                 | 211 us: 1.05x faster                                                   |
| coroutines                 | 23.9 ms                                                | 23.1 ms: 1.04x faster                                                  |
| nqueens                    | 80.1 ms                                                | 77.3 ms: 1.04x faster                                                  |
| genshi_xml                 | 50.2 ms                                                | 48.5 ms: 1.03x faster                                                  |
| meteor_contest             | 104 ms                                                 | 101 ms: 1.03x faster                                                   |
| deltablue                  | 3.45 ms                                                | 3.37 ms: 1.02x faster                                                  |
| xml_etree_generate         | 85.2 ms                                                | 83.3 ms: 1.02x faster                                                  |
| thrift                     | 791 us                                                 | 776 us: 1.02x faster                                                   |
| xml_etree_process          | 59.0 ms                                                | 58.2 ms: 1.01x faster                                                  |
| python_startup_no_site     | 7.16 ms                                                | 7.30 ms: 1.02x slower                                                  |
| scimark_sparse_mat_mult    | 4.39 ms                                                | 4.48 ms: 1.02x slower                                                  |
| regex_dna                  | 168 ms                                                 | 172 ms: 1.02x slower                                                   |
| sqlite_synth               | 2.20 us                                                | 2.25 us: 1.02x slower                                                  |
| regex_v8                   | 20.6 ms                                                | 21.3 ms: 1.03x slower                                                  |
| sympy_expand               | 468 ms                                                 | 484 ms: 1.04x slower                                                   |
| mako                       | 11.0 ms                                                | 11.5 ms: 1.05x slower                                                  |
| fannkuch                   | 372 ms                                                 | 392 ms: 1.05x slower                                                   |
| asyncio_websockets         | 517 ms                                                 | 544 ms: 1.05x slower                                                   |
| json_loads                 | 26.5 us                                                | 28.2 us: 1.06x slower                                                  |
| pidigits                   | 184 ms                                                 | 197 ms: 1.07x slower                                                   |
| coverage                   | 71.4 ms                                                | 80.9 ms: 1.13x slower                                                  |
| python_startup             | 9.93 ms                                                | 12.5 ms: 1.26x slower                                                  |
| gc_traversal               | 3.46 ms                                                | 4.42 ms: 1.28x slower                                                  |
| bench_thread_pool          | 941 us                                                 | 1.31 ms: 1.39x slower                                                  |
| create_gc_cycles           | 1.09 ms                                                | 1.85 ms: 1.70x slower                                                  |
| telco                      | 6.53 ms                                                | 156 ms: 23.90x slower                                                  |
| bench_mp_pool              | 10.8 ms                                                | 270 ms: 25.04x slower                                                  |
| Geometric mean             | (ref)                                                  | 1.04x faster                                                           |

Benchmark hidden because not significant (4): django_template, pickle_pure_python, json, nbody
Ignored benchmarks (21) of results/bm-20240906-3.12.6-a4a2d2b/bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b.json: aiohttp, asyncio_tcp, asyncio_tcp_ssl, chameleon, dask, flaskblogging, gunicorn, mypy2, pickle, pickle_dict, pickle_list, sqlalchemy_declarative, sqlalchemy_imperative, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http, unpack_sequence, unpickle, unpickle_list
Ignored benchmarks (10) of results/bm-20260209-3.15.0a5+-432ddd9/bm-20260209-vultr-x86_64-python-432ddd99e2b06a75a4f4-3.15.0a5+-432ddd9.json: connected_components, k_core, many_optionals, shortest_path, sphinx, sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile, subparsers

- Geometric mean (including insignificant results): 1.081x faster

# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.07x
- 95% likely to have a speedup of 1.06x
- 99% likely to have a speedup of 1.05x

# Memory
- memory change: 1.17x