# Results vs. 3.12.6

- fork: python
- ref: a7bb524fef61f77ede01
- machine: linux-x86_64
- commit hash: a7bb524
- commit date: 2026-08-17
- overall geometric mean: 1.046x slower
- HPT reliability: 91.88%
- HPT 99th percentile: 1.00x slower
- Memory change: 1.37x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260817-vultr-x86_64-python-a7bb524fef61f77ede01-3.16.0a0-a7bb524 |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| 2to3           | 264 ms                                                 | 298 ms: 1.13x slower                                                  |
| docutils       | 2.64 sec                                               | 2.94 sec: 1.12x slower                                                |
| html5lib       | 63.6 ms                                                | 66.2 ms: 1.04x slower                                                 |
| Geometric mean | (ref)                                                  | 1.09x slower                                                          |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260817-vultr-x86_64-python-a7bb524fef61f77ede01-3.16.0a0-a7bb524 |
|----------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| async_tree_io_tg           | 1.11 sec                                               | 700 ms: 1.59x faster                                                  |
| async_tree_io              | 1.08 sec                                               | 712 ms: 1.52x faster                                                  |
| async_tree_memoization_tg  | 560 ms                                                 | 404 ms: 1.38x faster                                                  |
| async_tree_none            | 464 ms                                                 | 343 ms: 1.35x faster                                                  |
| async_tree_none_tg         | 446 ms                                                 | 337 ms: 1.33x faster                                                  |
| async_tree_memoization     | 555 ms                                                 | 425 ms: 1.31x faster                                                  |
| async_tree_cpu_io_mixed_tg | 723 ms                                                 | 681 ms: 1.06x faster                                                  |
| async_tree_cpu_io_mixed    | 715 ms                                                 | 701 ms: 1.02x faster                                                  |
| asyncio_websockets         | 517 ms                                                 | 510 ms: 1.01x faster                                                  |
| coroutines                 | 23.9 ms                                                | 24.6 ms: 1.03x slower                                                 |
| async_generators           | 384 ms                                                 | 396 ms: 1.03x slower                                                  |
| Geometric mean             | (ref)                                                  | 1.21x faster                                                          |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260817-vultr-x86_64-python-a7bb524fef61f77ede01-3.16.0a0-a7bb524 |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| float          | 80.8 ms                                                | 84.3 ms: 1.04x slower                                                 |
| pidigits       | 184 ms                                                 | 215 ms: 1.17x slower                                                  |
| nbody          | 89.3 ms                                                | 120 ms: 1.35x slower                                                  |
| Geometric mean | (ref)                                                  | 1.18x slower                                                          |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260817-vultr-x86_64-python-a7bb524fef61f77ede01-3.16.0a0-a7bb524 |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| regex_effbot   | 3.17 ms                                                | 2.95 ms: 1.07x faster                                                 |
| regex_v8       | 20.6 ms                                                | 20.9 ms: 1.02x slower                                                 |
| regex_dna      | 168 ms                                                 | 181 ms: 1.08x slower                                                  |
| regex_compile  | 142 ms                                                 | 171 ms: 1.20x slower                                                  |
| Geometric mean | (ref)                                                  | 1.05x slower                                                          |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260817-vultr-x86_64-python-a7bb524fef61f77ede01-3.16.0a0-a7bb524 |
|----------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| tomli_loads          | 2.11 sec                                               | 1.96 sec: 1.08x faster                                                |
| json_dumps           | 10.4 ms                                                | 10.1 ms: 1.03x faster                                                 |
| xml_etree_iterparse  | 96.7 ms                                                | 95.1 ms: 1.02x faster                                                 |
| unpickle_pure_python | 221 us                                                 | 240 us: 1.09x slower                                                  |
| pickle_pure_python   | 308 us                                                 | 337 us: 1.09x slower                                                  |
| json_loads           | 26.5 us                                                | 30.5 us: 1.15x slower                                                 |
| xml_etree_generate   | 85.2 ms                                                | 101 ms: 1.18x slower                                                  |
| xml_etree_process    | 59.0 ms                                                | 75.0 ms: 1.27x slower                                                 |
| Geometric mean       | (ref)                                                  | 1.07x slower                                                          |

Benchmark hidden because not significant (1): xml_etree_parse

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260817-vultr-x86_64-python-a7bb524fef61f77ede01-3.16.0a0-a7bb524 |
|------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| python_startup_no_site | 7.16 ms                                                | 9.27 ms: 1.29x slower                                                 |
| python_startup         | 9.93 ms                                                | 15.7 ms: 1.58x slower                                                 |
| Geometric mean         | (ref)                                                  | 1.43x slower                                                          |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260817-vultr-x86_64-python-a7bb524fef61f77ede01-3.16.0a0-a7bb524 |
|-----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| django_template | 34.7 ms                                                | 41.7 ms: 1.20x slower                                                 |
| mako            | 11.0 ms                                                | 15.8 ms: 1.44x slower                                                 |
| Geometric mean  | (ref)                                                  | 1.31x slower                                                          |

All benchmarks:
===============

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260817-vultr-x86_64-python-a7bb524fef61f77ede01-3.16.0a0-a7bb524 |
|----------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| pylint                     | 319 ms                                                 | 131 ms: 2.43x faster                                                  |
| gc_traversal               | 3.46 ms                                                | 1.79 ms: 1.93x faster                                                 |
| mdp                        | 2.42 sec                                               | 1.31 sec: 1.85x faster                                                |
| bench_mp_pool              | 10.8 ms                                                | 6.78 ms: 1.59x faster                                                 |
| async_tree_io_tg           | 1.11 sec                                               | 700 ms: 1.59x faster                                                  |
| async_tree_io              | 1.08 sec                                               | 712 ms: 1.52x faster                                                  |
| async_tree_memoization_tg  | 560 ms                                                 | 404 ms: 1.38x faster                                                  |
| async_tree_none            | 464 ms                                                 | 343 ms: 1.35x faster                                                  |
| async_tree_none_tg         | 446 ms                                                 | 337 ms: 1.33x faster                                                  |
| async_tree_memoization     | 555 ms                                                 | 425 ms: 1.31x faster                                                  |
| deepcopy                   | 352 us                                                 | 272 us: 1.29x faster                                                  |
| deepcopy_memo              | 40.3 us                                                | 31.9 us: 1.26x faster                                                 |
| pathlib                    | 21.5 ms                                                | 17.9 ms: 1.20x faster                                                 |
| go                         | 139 ms                                                 | 120 ms: 1.16x faster                                                  |
| sqlite_synth               | 2.20 us                                                | 1.95 us: 1.13x faster                                                 |
| dulwich_log                | 78.9 ms                                                | 70.7 ms: 1.12x faster                                                 |
| comprehensions             | 19.8 us                                                | 17.9 us: 1.11x faster                                                 |
| typing_runtime_protocols   | 163 us                                                 | 149 us: 1.10x faster                                                  |
| bpe_tokeniser              | 4.74 sec                                               | 4.38 sec: 1.08x faster                                                |
| tomli_loads                | 2.11 sec                                               | 1.96 sec: 1.08x faster                                                |
| regex_effbot               | 3.17 ms                                                | 2.95 ms: 1.07x faster                                                 |
| scimark_sor                | 130 ms                                                 | 121 ms: 1.07x faster                                                  |
| async_tree_cpu_io_mixed_tg | 723 ms                                                 | 681 ms: 1.06x faster                                                  |
| chaos                      | 62.8 ms                                                | 60.7 ms: 1.04x faster                                                 |
| spectral_norm              | 110 ms                                                 | 106 ms: 1.03x faster                                                  |
| deepcopy_reduce            | 3.08 us                                                | 2.99 us: 1.03x faster                                                 |
| json_dumps                 | 10.4 ms                                                | 10.1 ms: 1.03x faster                                                 |
| async_tree_cpu_io_mixed    | 715 ms                                                 | 701 ms: 1.02x faster                                                  |
| raytrace                   | 299 ms                                                 | 293 ms: 1.02x faster                                                  |
| xml_etree_iterparse        | 96.7 ms                                                | 95.1 ms: 1.02x faster                                                 |
| logging_silent             | 109 ns                                                 | 107 ns: 1.02x faster                                                  |
| asyncio_websockets         | 517 ms                                                 | 510 ms: 1.01x faster                                                  |
| pycparser                  | 1.17 sec                                               | 1.18 sec: 1.01x slower                                                |
| regex_v8                   | 20.6 ms                                                | 20.9 ms: 1.02x slower                                                 |
| coroutines                 | 23.9 ms                                                | 24.6 ms: 1.03x slower                                                 |
| async_generators           | 384 ms                                                 | 396 ms: 1.03x slower                                                  |
| pyflate                    | 448 ms                                                 | 464 ms: 1.04x slower                                                  |
| generators                 | 32.2 ms                                                | 33.5 ms: 1.04x slower                                                 |
| html5lib                   | 63.6 ms                                                | 66.2 ms: 1.04x slower                                                 |
| float                      | 80.8 ms                                                | 84.3 ms: 1.04x slower                                                 |
| logging_simple             | 6.63 us                                                | 6.98 us: 1.05x slower                                                 |
| hexiom                     | 6.17 ms                                                | 6.55 ms: 1.06x slower                                                 |
| json                       | 5.02 ms                                                | 5.38 ms: 1.07x slower                                                 |
| sympy_integrate            | 20.5 ms                                                | 22.0 ms: 1.07x slower                                                 |
| deltablue                  | 3.45 ms                                                | 3.70 ms: 1.07x slower                                                 |
| regex_dna                  | 168 ms                                                 | 181 ms: 1.08x slower                                                  |
| unpickle_pure_python       | 221 us                                                 | 240 us: 1.09x slower                                                  |
| sympy_sum                  | 166 ms                                                 | 182 ms: 1.09x slower                                                  |
| pickle_pure_python         | 308 us                                                 | 337 us: 1.09x slower                                                  |
| logging_format             | 7.35 us                                                | 8.07 us: 1.10x slower                                                 |
| sympy_str                  | 292 ms                                                 | 321 ms: 1.10x slower                                                  |
| nqueens                    | 80.1 ms                                                | 88.8 ms: 1.11x slower                                                 |
| docutils                   | 2.64 sec                                               | 2.94 sec: 1.12x slower                                                |
| pprint_safe_repr           | 743 ms                                                 | 832 ms: 1.12x slower                                                  |
| scimark_lu                 | 114 ms                                                 | 129 ms: 1.13x slower                                                  |
| 2to3                       | 264 ms                                                 | 298 ms: 1.13x slower                                                  |
| pprint_pformat             | 1.52 sec                                               | 1.72 sec: 1.13x slower                                                |
| richards                   | 45.9 ms                                                | 52.4 ms: 1.14x slower                                                 |
| scimark_monte_carlo        | 68.4 ms                                                | 78.1 ms: 1.14x slower                                                 |
| json_loads                 | 26.5 us                                                | 30.5 us: 1.15x slower                                                 |
| sympy_expand               | 468 ms                                                 | 540 ms: 1.15x slower                                                  |
| richards_super             | 51.9 ms                                                | 60.0 ms: 1.16x slower                                                 |
| crypto_pyaes               | 76.6 ms                                                | 89.3 ms: 1.17x slower                                                 |
| pidigits                   | 184 ms                                                 | 215 ms: 1.17x slower                                                  |
| thrift                     | 791 us                                                 | 935 us: 1.18x slower                                                  |
| scimark_sparse_mat_mult    | 4.39 ms                                                | 5.19 ms: 1.18x slower                                                 |
| xml_etree_generate         | 85.2 ms                                                | 101 ms: 1.18x slower                                                  |
| regex_compile              | 142 ms                                                 | 171 ms: 1.20x slower                                                  |
| django_template            | 34.7 ms                                                | 41.7 ms: 1.20x slower                                                 |
| meteor_contest             | 104 ms                                                 | 125 ms: 1.21x slower                                                  |
| fannkuch                   | 372 ms                                                 | 464 ms: 1.25x slower                                                  |
| create_gc_cycles           | 1.09 ms                                                | 1.38 ms: 1.26x slower                                                 |
| xml_etree_process          | 59.0 ms                                                | 75.0 ms: 1.27x slower                                                 |
| python_startup_no_site     | 7.16 ms                                                | 9.27 ms: 1.29x slower                                                 |
| nbody                      | 89.3 ms                                                | 120 ms: 1.35x slower                                                  |
| mako                       | 11.0 ms                                                | 15.8 ms: 1.44x slower                                                 |
| python_startup             | 9.93 ms                                                | 15.7 ms: 1.58x slower                                                 |
| bench_thread_pool          | 941 us                                                 | 1.49 ms: 1.58x slower                                                 |
| coverage                   | 71.4 ms                                                | 115 ms: 1.61x slower                                                  |
| telco                      | 6.53 ms                                                | 178 ms: 27.21x slower                                                 |
| Geometric mean             | (ref)                                                  | 1.05x slower                                                          |

Benchmark hidden because not significant (2): scimark_fft, xml_etree_parse
Ignored benchmarks (23) of results/bm-20240906-3.12.6-a4a2d2b/bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b.json: aiohttp, asyncio_tcp, asyncio_tcp_ssl, chameleon, dask, flaskblogging, genshi_text, genshi_xml, gunicorn, mypy2, pickle, pickle_dict, pickle_list, sqlalchemy_declarative, sqlalchemy_imperative, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http, unpack_sequence, unpickle, unpickle_list
Ignored benchmarks (10) of results/bm-20260817-3.16.0a0-a7bb524-NOGIL/bm-20260817-vultr-x86_64-python-a7bb524fef61f77ede01-3.16.0a0-a7bb524.json: connected_components, k_core, many_optionals, shortest_path, sphinx, sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile, subparsers

- Geometric mean (including insignificant results): 1.046x slower

# HPT report

- Reliability score: 91.88% likely to be slow
- 90% likely to have a slowdown of 1.00x
- 95% likely to have a slowdown of 1.00x
- 99% likely to have a slowdown of 1.00x

# Memory
- memory change: 1.37x