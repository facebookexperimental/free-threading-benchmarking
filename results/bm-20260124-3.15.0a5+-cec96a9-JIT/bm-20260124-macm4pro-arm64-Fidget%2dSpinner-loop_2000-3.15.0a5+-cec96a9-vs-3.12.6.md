# Results vs. 3.12.6

- fork: Fidget-Spinner
- ref: loop_2000
- machine: darwin-arm64
- commit hash: cec96a9
- commit date: 2026-01-24
- overall geometric mean: 1.263x faster
- HPT reliability: 100.00%
- HPT 99th percentile: 1.15x faster
- Memory change: 1.25x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260124-macm4pro-arm64-Fidget%2dSpinner-loop_2000-3.15.0a5+-cec96a9 |
|----------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| 2to3           | 114 ms                                                   | 109 ms: 1.05x faster                                                    |
| docutils       | 1.02 sec                                                 | 983 ms: 1.04x faster                                                    |
| html5lib       | 23.0 ms                                                  | 20.1 ms: 1.15x faster                                                   |
| sphinx         | 434 ms                                                   | 385 ms: 1.13x faster                                                    |
| Geometric mean | (ref)                                                    | 1.09x faster                                                            |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                        | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260124-macm4pro-arm64-Fidget%2dSpinner-loop_2000-3.15.0a5+-cec96a9 |
|----------------------------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| async_tree_eager_io              | 496 ms                                                   | 215 ms: 2.31x faster                                                    |
| async_tree_io_tg                 | 480 ms                                                   | 214 ms: 2.24x faster                                                    |
| async_tree_io                    | 459 ms                                                   | 222 ms: 2.06x faster                                                    |
| async_tree_eager_io_tg           | 446 ms                                                   | 216 ms: 2.06x faster                                                    |
| async_tree_none                  | 178 ms                                                   | 94.7 ms: 1.88x faster                                                   |
| async_tree_none_tg               | 172 ms                                                   | 95.3 ms: 1.81x faster                                                   |
| async_tree_memoization_tg        | 231 ms                                                   | 133 ms: 1.73x faster                                                    |
| async_tree_memoization           | 223 ms                                                   | 134 ms: 1.66x faster                                                    |
| async_tree_cpu_io_mixed          | 333 ms                                                   | 241 ms: 1.38x faster                                                    |
| async_tree_cpu_io_mixed_tg       | 338 ms                                                   | 247 ms: 1.37x faster                                                    |
| coroutines                       | 13.6 ms                                                  | 10.1 ms: 1.35x faster                                                   |
| async_tree_eager_memoization     | 132 ms                                                   | 105 ms: 1.26x faster                                                    |
| async_tree_eager                 | 45.6 ms                                                  | 36.7 ms: 1.24x faster                                                   |
| async_generators                 | 206 ms                                                   | 179 ms: 1.15x faster                                                    |
| async_tree_eager_cpu_io_mixed    | 231 ms                                                   | 215 ms: 1.07x faster                                                    |
| asyncio_websockets               | 190 ms                                                   | 184 ms: 1.03x faster                                                    |
| async_tree_eager_memoization_tg  | 113 ms                                                   | 119 ms: 1.05x slower                                                    |
| async_tree_eager_cpu_io_mixed_tg | 213 ms                                                   | 235 ms: 1.10x slower                                                    |
| async_tree_eager_tg              | 32.1 ms                                                  | 75.9 ms: 2.36x slower                                                   |
| Geometric mean                   | (ref)                                                    | 1.37x faster                                                            |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260124-macm4pro-arm64-Fidget%2dSpinner-loop_2000-3.15.0a5+-cec96a9 |
|----------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| float          | 37.9 ms                                                  | 23.0 ms: 1.64x faster                                                   |
| nbody          | 54.2 ms                                                  | 39.7 ms: 1.36x faster                                                   |
| pidigits       | 161 ms                                                   | 159 ms: 1.02x faster                                                    |
| Geometric mean | (ref)                                                    | 1.32x faster                                                            |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260124-macm4pro-arm64-Fidget%2dSpinner-loop_2000-3.15.0a5+-cec96a9 |
|----------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| regex_compile  | 54.6 ms                                                  | 40.1 ms: 1.36x faster                                                   |
| regex_effbot   | 1.67 ms                                                  | 1.45 ms: 1.15x faster                                                   |
| regex_dna      | 99.6 ms                                                  | 95.8 ms: 1.04x faster                                                   |
| regex_v8       | 9.59 ms                                                  | 9.83 ms: 1.03x slower                                                   |
| Geometric mean | (ref)                                                    | 1.12x faster                                                            |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260124-macm4pro-arm64-Fidget%2dSpinner-loop_2000-3.15.0a5+-cec96a9 |
|----------------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| tomli_loads          | 957 ms                                                   | 669 ms: 1.43x faster                                                    |
| unpickle_pure_python | 103 us                                                   | 75.4 us: 1.36x faster                                                   |
| xml_etree_iterparse  | 51.6 ms                                                  | 38.8 ms: 1.33x faster                                                   |
| xml_etree_generate   | 38.9 ms                                                  | 30.8 ms: 1.26x faster                                                   |
| pickle_pure_python   | 139 us                                                   | 111 us: 1.26x faster                                                    |
| xml_etree_process    | 26.7 ms                                                  | 21.6 ms: 1.24x faster                                                   |
| json_dumps           | 4.26 ms                                                  | 3.60 ms: 1.18x faster                                                   |
| xml_etree_parse      | 67.9 ms                                                  | 64.3 ms: 1.06x faster                                                   |
| json_loads           | 10.9 us                                                  | 10.5 us: 1.03x faster                                                   |
| Geometric mean       | (ref)                                                    | 1.23x faster                                                            |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260124-macm4pro-arm64-Fidget%2dSpinner-loop_2000-3.15.0a5+-cec96a9 |
|------------------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| python_startup         | 8.01 ms                                                  | 8.72 ms: 1.09x slower                                                   |
| python_startup_no_site | 5.71 ms                                                  | 6.30 ms: 1.10x slower                                                   |
| Geometric mean         | (ref)                                                    | 1.10x slower                                                            |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260124-macm4pro-arm64-Fidget%2dSpinner-loop_2000-3.15.0a5+-cec96a9 |
|-----------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| mako            | 4.77 ms                                                  | 3.86 ms: 1.24x faster                                                   |
| genshi_text     | 10.5 ms                                                  | 8.67 ms: 1.21x faster                                                   |
| genshi_xml      | 21.8 ms                                                  | 20.0 ms: 1.09x faster                                                   |
| django_template | 13.6 ms                                                  | 13.6 ms: 1.00x faster                                                   |
| Geometric mean  | (ref)                                                    | 1.13x faster                                                            |

All benchmarks:
===============

| Benchmark                        | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260124-macm4pro-arm64-Fidget%2dSpinner-loop_2000-3.15.0a5+-cec96a9 |
|----------------------------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| subparsers                       | 20.8 ms                                                  | 3.81 ms: 5.45x faster                                                   |
| richards                         | 22.4 ms                                                  | 9.18 ms: 2.44x faster                                                   |
| async_tree_eager_io              | 496 ms                                                   | 215 ms: 2.31x faster                                                    |
| richards_super                   | 25.4 ms                                                  | 11.1 ms: 2.29x faster                                                   |
| async_tree_io_tg                 | 480 ms                                                   | 214 ms: 2.24x faster                                                    |
| async_tree_io                    | 459 ms                                                   | 222 ms: 2.06x faster                                                    |
| async_tree_eager_io_tg           | 446 ms                                                   | 216 ms: 2.06x faster                                                    |
| async_tree_none                  | 178 ms                                                   | 94.7 ms: 1.88x faster                                                   |
| mdp                              | 1.09 sec                                                 | 591 ms: 1.85x faster                                                    |
| scimark_lu                       | 51.3 ms                                                  | 28.0 ms: 1.83x faster                                                   |
| async_tree_none_tg               | 172 ms                                                   | 95.3 ms: 1.81x faster                                                   |
| deepcopy                         | 161 us                                                   | 91.5 us: 1.77x faster                                                   |
| deepcopy_memo                    | 18.3 us                                                  | 10.6 us: 1.73x faster                                                   |
| async_tree_memoization_tg        | 231 ms                                                   | 133 ms: 1.73x faster                                                    |
| async_tree_memoization           | 223 ms                                                   | 134 ms: 1.66x faster                                                    |
| float                            | 37.9 ms                                                  | 23.0 ms: 1.64x faster                                                   |
| comprehensions                   | 9.84 us                                                  | 6.17 us: 1.60x faster                                                   |
| scimark_sor                      | 61.0 ms                                                  | 38.6 ms: 1.58x faster                                                   |
| deltablue                        | 1.73 ms                                                  | 1.13 ms: 1.53x faster                                                   |
| go                               | 70.0 ms                                                  | 46.5 ms: 1.51x faster                                                   |
| deepcopy_reduce                  | 1.46 us                                                  | 974 ns: 1.50x faster                                                    |
| logging_silent                   | 50.9 ns                                                  | 35.0 ns: 1.45x faster                                                   |
| scimark_fft                      | 142 ms                                                   | 98.2 ms: 1.44x faster                                                   |
| pyflate                          | 216 ms                                                   | 150 ms: 1.44x faster                                                    |
| tomli_loads                      | 957 ms                                                   | 669 ms: 1.43x faster                                                    |
| scimark_monte_carlo              | 32.2 ms                                                  | 22.8 ms: 1.41x faster                                                   |
| spectral_norm                    | 54.4 ms                                                  | 39.2 ms: 1.39x faster                                                   |
| raytrace                         | 145 ms                                                   | 105 ms: 1.38x faster                                                    |
| async_tree_cpu_io_mixed          | 333 ms                                                   | 241 ms: 1.38x faster                                                    |
| async_tree_cpu_io_mixed_tg       | 338 ms                                                   | 247 ms: 1.37x faster                                                    |
| nbody                            | 54.2 ms                                                  | 39.7 ms: 1.36x faster                                                   |
| unpickle_pure_python             | 103 us                                                   | 75.4 us: 1.36x faster                                                   |
| regex_compile                    | 54.6 ms                                                  | 40.1 ms: 1.36x faster                                                   |
| coroutines                       | 13.6 ms                                                  | 10.1 ms: 1.35x faster                                                   |
| chaos                            | 28.9 ms                                                  | 21.6 ms: 1.34x faster                                                   |
| xml_etree_iterparse              | 51.6 ms                                                  | 38.8 ms: 1.33x faster                                                   |
| pprint_pformat                   | 665 ms                                                   | 503 ms: 1.32x faster                                                    |
| pprint_safe_repr                 | 328 ms                                                   | 250 ms: 1.31x faster                                                    |
| k_core                           | 1.12 sec                                                 | 859 ms: 1.30x faster                                                    |
| bpe_tokeniser                    | 2.24 sec                                                 | 1.77 sec: 1.27x faster                                                  |
| xml_etree_generate               | 38.9 ms                                                  | 30.8 ms: 1.26x faster                                                   |
| crypto_pyaes                     | 38.8 ms                                                  | 30.7 ms: 1.26x faster                                                   |
| async_tree_eager_memoization     | 132 ms                                                   | 105 ms: 1.26x faster                                                    |
| pickle_pure_python               | 139 us                                                   | 111 us: 1.26x faster                                                    |
| async_tree_eager                 | 45.6 ms                                                  | 36.7 ms: 1.24x faster                                                   |
| mako                             | 4.77 ms                                                  | 3.86 ms: 1.24x faster                                                   |
| xml_etree_process                | 26.7 ms                                                  | 21.6 ms: 1.24x faster                                                   |
| logging_simple                   | 2.57 us                                                  | 2.09 us: 1.23x faster                                                   |
| logging_format                   | 2.80 us                                                  | 2.29 us: 1.23x faster                                                   |
| nqueens                          | 43.5 ms                                                  | 35.5 ms: 1.22x faster                                                   |
| genshi_text                      | 10.5 ms                                                  | 8.67 ms: 1.21x faster                                                   |
| json_dumps                       | 4.26 ms                                                  | 3.60 ms: 1.18x faster                                                   |
| generators                       | 21.9 ms                                                  | 18.6 ms: 1.18x faster                                                   |
| pathlib                          | 12.4 ms                                                  | 10.6 ms: 1.17x faster                                                   |
| fannkuch                         | 176 ms                                                   | 150 ms: 1.17x faster                                                    |
| async_generators                 | 206 ms                                                   | 179 ms: 1.15x faster                                                    |
| regex_effbot                     | 1.67 ms                                                  | 1.45 ms: 1.15x faster                                                   |
| hexiom                           | 3.04 ms                                                  | 2.64 ms: 1.15x faster                                                   |
| dulwich_log                      | 21.3 ms                                                  | 18.4 ms: 1.15x faster                                                   |
| typing_runtime_protocols         | 71.0 us                                                  | 61.7 us: 1.15x faster                                                   |
| pycparser                        | 497 ms                                                   | 433 ms: 1.15x faster                                                    |
| html5lib                         | 23.0 ms                                                  | 20.1 ms: 1.15x faster                                                   |
| sphinx                           | 434 ms                                                   | 385 ms: 1.13x faster                                                    |
| scimark_sparse_mat_mult          | 2.08 ms                                                  | 1.87 ms: 1.11x faster                                                   |
| genshi_xml                       | 21.8 ms                                                  | 20.0 ms: 1.09x faster                                                   |
| async_tree_eager_cpu_io_mixed    | 231 ms                                                   | 215 ms: 1.07x faster                                                    |
| meteor_contest                   | 47.7 ms                                                  | 44.6 ms: 1.07x faster                                                   |
| thrift                           | 322 us                                                   | 301 us: 1.07x faster                                                    |
| json                             | 1.93 ms                                                  | 1.82 ms: 1.06x faster                                                   |
| xml_etree_parse                  | 67.9 ms                                                  | 64.3 ms: 1.06x faster                                                   |
| 2to3                             | 114 ms                                                   | 109 ms: 1.05x faster                                                    |
| sympy_integrate                  | 8.02 ms                                                  | 7.68 ms: 1.04x faster                                                   |
| regex_dna                        | 99.6 ms                                                  | 95.8 ms: 1.04x faster                                                   |
| docutils                         | 1.02 sec                                                 | 983 ms: 1.04x faster                                                    |
| asyncio_websockets               | 190 ms                                                   | 184 ms: 1.03x faster                                                    |
| json_loads                       | 10.9 us                                                  | 10.5 us: 1.03x faster                                                   |
| connected_components             | 201 ms                                                   | 196 ms: 1.02x faster                                                    |
| pidigits                         | 161 ms                                                   | 159 ms: 1.02x faster                                                    |
| shortest_path                    | 219 ms                                                   | 216 ms: 1.02x faster                                                    |
| dask                             | 528 ms                                                   | 522 ms: 1.01x faster                                                    |
| sqlite_synth                     | 967 ns                                                   | 959 ns: 1.01x faster                                                    |
| telco                            | 2.61 ms                                                  | 2.59 ms: 1.01x faster                                                   |
| django_template                  | 13.6 ms                                                  | 13.6 ms: 1.00x faster                                                   |
| sympy_sum                        | 57.6 ms                                                  | 58.7 ms: 1.02x slower                                                   |
| bench_thread_pool                | 419 us                                                   | 429 us: 1.02x slower                                                    |
| regex_v8                         | 9.59 ms                                                  | 9.83 ms: 1.03x slower                                                   |
| sympy_str                        | 104 ms                                                   | 107 ms: 1.03x slower                                                    |
| sympy_expand                     | 167 ms                                                   | 176 ms: 1.05x slower                                                    |
| async_tree_eager_memoization_tg  | 113 ms                                                   | 119 ms: 1.05x slower                                                    |
| create_gc_cycles                 | 830 us                                                   | 898 us: 1.08x slower                                                    |
| python_startup                   | 8.01 ms                                                  | 8.72 ms: 1.09x slower                                                   |
| async_tree_eager_cpu_io_mixed_tg | 213 ms                                                   | 235 ms: 1.10x slower                                                    |
| python_startup_no_site           | 5.71 ms                                                  | 6.30 ms: 1.10x slower                                                   |
| bench_mp_pool                    | 39.7 ms                                                  | 44.2 ms: 1.11x slower                                                   |
| coverage                         | 26.9 ms                                                  | 32.4 ms: 1.21x slower                                                   |
| many_optionals                   | 195 us                                                   | 244 us: 1.25x slower                                                    |
| async_tree_eager_tg              | 32.1 ms                                                  | 75.9 ms: 2.36x slower                                                   |
| Geometric mean                   | (ref)                                                    | 1.26x faster                                                            |

Benchmark hidden because not significant (2): pylint, gc_traversal
Ignored benchmarks (10) of results/bm-20240906-3.12.6-a4a2d2b/bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b.json: chameleon, djangocms, gevent_hub, sqlalchemy_declarative, sqlalchemy_imperative, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http
Ignored benchmarks (4) of results/bm-20260124-3.15.0a5+-cec96a9-JIT/bm-20260124-macm4pro-arm64-Fidget%2dSpinner-loop_2000-3.15.0a5+-cec96a9.json: sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile

- Geometric mean (including insignificant results): 1.263x faster

# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.18x
- 95% likely to have a speedup of 1.16x
- 99% likely to have a speedup of 1.15x

# Memory
- memory change: 1.25x