# Results vs. 3.13.0rc2

- fork: python
- ref: dc62b622524bf49eb539
- machine: linux-x86_64
- commit hash: dc62b62
- commit date: 2025-11-24
- overall geometric mean: 1.065x faster
- HPT reliability: 99.99%
- HPT 99th percentile: 1.02x faster
- Memory change: 1.18x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20251124-vultr-x86_64-python-dc62b622524bf49eb539-3.15.0a2+-dc62b62 |
|----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| 2to3           | 260 ms                                                       | 250 ms: 1.04x faster                                                   |
| html5lib       | 67.0 ms                                                      | 67.9 ms: 1.01x slower                                                  |
| Geometric mean | (ref)                                                        | 1.01x faster                                                           |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20251124-vultr-x86_64-python-dc62b622524bf49eb539-3.15.0a2+-dc62b62 |
|----------------------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| async_tree_io              | 876 ms                                                       | 559 ms: 1.57x faster                                                   |
| async_tree_io_tg           | 913 ms                                                       | 601 ms: 1.52x faster                                                   |
| async_tree_memoization     | 461 ms                                                       | 315 ms: 1.46x faster                                                   |
| async_tree_none            | 354 ms                                                       | 248 ms: 1.43x faster                                                   |
| async_tree_cpu_io_mixed    | 666 ms                                                       | 485 ms: 1.37x faster                                                   |
| async_tree_none_tg         | 336 ms                                                       | 247 ms: 1.36x faster                                                   |
| async_tree_memoization_tg  | 414 ms                                                       | 312 ms: 1.33x faster                                                   |
| async_tree_cpu_io_mixed_tg | 638 ms                                                       | 487 ms: 1.31x faster                                                   |
| async_generators           | 377 ms                                                       | 363 ms: 1.04x faster                                                   |
| coroutines                 | 23.6 ms                                                      | 24.4 ms: 1.03x slower                                                  |
| asyncio_websockets         | 520 ms                                                       | 545 ms: 1.05x slower                                                   |
| Geometric mean             | (ref)                                                        | 1.28x faster                                                           |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20251124-vultr-x86_64-python-dc62b622524bf49eb539-3.15.0a2+-dc62b62 |
|----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| float          | 77.5 ms                                                      | 60.2 ms: 1.29x faster                                                  |
| pidigits       | 217 ms                                                       | 198 ms: 1.09x faster                                                   |
| nbody          | 85.1 ms                                                      | 86.2 ms: 1.01x slower                                                  |
| Geometric mean | (ref)                                                        | 1.11x faster                                                           |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20251124-vultr-x86_64-python-dc62b622524bf49eb539-3.15.0a2+-dc62b62 |
|----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| regex_effbot   | 3.08 ms                                                      | 2.87 ms: 1.07x faster                                                  |
| regex_compile  | 132 ms                                                       | 127 ms: 1.04x faster                                                   |
| regex_v8       | 22.7 ms                                                      | 22.3 ms: 1.02x faster                                                  |
| regex_dna      | 180 ms                                                       | 182 ms: 1.01x slower                                                   |
| Geometric mean | (ref)                                                        | 1.03x faster                                                           |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20251124-vultr-x86_64-python-dc62b622524bf49eb539-3.15.0a2+-dc62b62 |
|----------------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| xml_etree_iterparse  | 94.9 ms                                                      | 82.6 ms: 1.15x faster                                                  |
| unpickle_pure_python | 210 us                                                       | 183 us: 1.14x faster                                                   |
| json_dumps           | 10.5 ms                                                      | 9.34 ms: 1.13x faster                                                  |
| xml_etree_process    | 59.3 ms                                                      | 53.5 ms: 1.11x faster                                                  |
| xml_etree_generate   | 85.4 ms                                                      | 77.1 ms: 1.11x faster                                                  |
| tomli_loads          | 2.01 sec                                                     | 1.85 sec: 1.08x faster                                                 |
| xml_etree_parse      | 136 ms                                                       | 130 ms: 1.05x faster                                                   |
| json_loads           | 27.0 us                                                      | 28.7 us: 1.06x slower                                                  |
| Geometric mean       | (ref)                                                        | 1.08x faster                                                           |

Benchmark hidden because not significant (1): pickle_pure_python

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20251124-vultr-x86_64-python-dc62b622524bf49eb539-3.15.0a2+-dc62b62 |
|------------------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| python_startup_no_site | 7.39 ms                                                      | 7.27 ms: 1.02x faster                                                  |
| python_startup         | 11.0 ms                                                      | 12.4 ms: 1.13x slower                                                  |
| Geometric mean         | (ref)                                                        | 1.05x slower                                                           |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20251124-vultr-x86_64-python-dc62b622524bf49eb539-3.15.0a2+-dc62b62 |
|-----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| mako            | 11.3 ms                                                      | 10.5 ms: 1.08x faster                                                  |
| genshi_text     | 21.5 ms                                                      | 21.2 ms: 1.02x faster                                                  |
| django_template | 34.1 ms                                                      | 36.3 ms: 1.06x slower                                                  |
| genshi_xml      | 48.8 ms                                                      | 54.3 ms: 1.11x slower                                                  |
| Geometric mean  | (ref)                                                        | 1.02x slower                                                           |

All benchmarks:
===============

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20251124-vultr-x86_64-python-dc62b622524bf49eb539-3.15.0a2+-dc62b62 |
|----------------------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| richards                   | 45.2 ms                                                      | 21.0 ms: 2.15x faster                                                  |
| richards_super             | 51.6 ms                                                      | 25.7 ms: 2.01x faster                                                  |
| mdp                        | 2.36 sec                                                     | 1.33 sec: 1.77x faster                                                 |
| async_tree_io              | 876 ms                                                       | 559 ms: 1.57x faster                                                   |
| deepcopy_memo              | 39.1 us                                                      | 25.0 us: 1.56x faster                                                  |
| async_tree_io_tg           | 913 ms                                                       | 601 ms: 1.52x faster                                                   |
| async_tree_memoization     | 461 ms                                                       | 315 ms: 1.46x faster                                                   |
| async_tree_none            | 354 ms                                                       | 248 ms: 1.43x faster                                                   |
| deepcopy                   | 355 us                                                       | 250 us: 1.42x faster                                                   |
| scimark_fft                | 349 ms                                                       | 250 ms: 1.40x faster                                                   |
| scimark_sor                | 134 ms                                                       | 97.7 ms: 1.38x faster                                                  |
| async_tree_cpu_io_mixed    | 666 ms                                                       | 485 ms: 1.37x faster                                                   |
| go                         | 141 ms                                                       | 103 ms: 1.36x faster                                                   |
| async_tree_none_tg         | 336 ms                                                       | 247 ms: 1.36x faster                                                   |
| spectral_norm              | 111 ms                                                       | 82.8 ms: 1.34x faster                                                  |
| async_tree_memoization_tg  | 414 ms                                                       | 312 ms: 1.33x faster                                                   |
| async_tree_cpu_io_mixed_tg | 638 ms                                                       | 487 ms: 1.31x faster                                                   |
| float                      | 77.5 ms                                                      | 60.2 ms: 1.29x faster                                                  |
| pyflate                    | 449 ms                                                       | 370 ms: 1.21x faster                                                   |
| deepcopy_reduce            | 3.11 us                                                      | 2.58 us: 1.20x faster                                                  |
| logging_silent             | 103 ns                                                       | 85.4 ns: 1.20x faster                                                  |
| scimark_monte_carlo        | 65.4 ms                                                      | 55.4 ms: 1.18x faster                                                  |
| scimark_sparse_mat_mult    | 4.71 ms                                                      | 4.06 ms: 1.16x faster                                                  |
| xml_etree_iterparse        | 94.9 ms                                                      | 82.6 ms: 1.15x faster                                                  |
| unpickle_pure_python       | 210 us                                                       | 183 us: 1.14x faster                                                   |
| json_dumps                 | 10.5 ms                                                      | 9.34 ms: 1.13x faster                                                  |
| scimark_lu                 | 113 ms                                                       | 101 ms: 1.12x faster                                                   |
| xml_etree_process          | 59.3 ms                                                      | 53.5 ms: 1.11x faster                                                  |
| xml_etree_generate         | 85.4 ms                                                      | 77.1 ms: 1.11x faster                                                  |
| bpe_tokeniser              | 4.45 sec                                                     | 4.01 sec: 1.11x faster                                                 |
| pprint_safe_repr           | 738 ms                                                       | 670 ms: 1.10x faster                                                   |
| pidigits                   | 217 ms                                                       | 198 ms: 1.09x faster                                                   |
| deltablue                  | 3.12 ms                                                      | 2.87 ms: 1.09x faster                                                  |
| tomli_loads                | 2.01 sec                                                     | 1.85 sec: 1.08x faster                                                 |
| mako                       | 11.3 ms                                                      | 10.5 ms: 1.08x faster                                                  |
| logging_simple             | 6.16 us                                                      | 5.73 us: 1.08x faster                                                  |
| regex_effbot               | 3.08 ms                                                      | 2.87 ms: 1.07x faster                                                  |
| pathlib                    | 19.2 ms                                                      | 17.9 ms: 1.07x faster                                                  |
| pprint_pformat             | 1.50 sec                                                     | 1.40 sec: 1.07x faster                                                 |
| logging_format             | 6.84 us                                                      | 6.46 us: 1.06x faster                                                  |
| xml_etree_parse            | 136 ms                                                       | 130 ms: 1.05x faster                                                   |
| chaos                      | 57.3 ms                                                      | 54.8 ms: 1.05x faster                                                  |
| regex_compile              | 132 ms                                                       | 127 ms: 1.04x faster                                                   |
| async_generators           | 377 ms                                                       | 363 ms: 1.04x faster                                                   |
| thrift                     | 778 us                                                       | 748 us: 1.04x faster                                                   |
| 2to3                       | 260 ms                                                       | 250 ms: 1.04x faster                                                   |
| meteor_contest             | 102 ms                                                       | 98.3 ms: 1.03x faster                                                  |
| comprehensions             | 16.5 us                                                      | 16.0 us: 1.03x faster                                                  |
| regex_v8                   | 22.7 ms                                                      | 22.3 ms: 1.02x faster                                                  |
| python_startup_no_site     | 7.39 ms                                                      | 7.27 ms: 1.02x faster                                                  |
| genshi_text                | 21.5 ms                                                      | 21.2 ms: 1.02x faster                                                  |
| fannkuch                   | 370 ms                                                       | 364 ms: 1.01x faster                                                   |
| coverage                   | 83.0 ms                                                      | 82.0 ms: 1.01x faster                                                  |
| crypto_pyaes               | 67.9 ms                                                      | 67.2 ms: 1.01x faster                                                  |
| hexiom                     | 5.99 ms                                                      | 6.02 ms: 1.00x slower                                                  |
| dulwich_log                | 74.8 ms                                                      | 75.3 ms: 1.01x slower                                                  |
| regex_dna                  | 180 ms                                                       | 182 ms: 1.01x slower                                                   |
| html5lib                   | 67.0 ms                                                      | 67.9 ms: 1.01x slower                                                  |
| nbody                      | 85.1 ms                                                      | 86.2 ms: 1.01x slower                                                  |
| json                       | 4.93 ms                                                      | 5.03 ms: 1.02x slower                                                  |
| typing_runtime_protocols   | 155 us                                                       | 158 us: 1.02x slower                                                   |
| generators                 | 28.8 ms                                                      | 29.8 ms: 1.03x slower                                                  |
| coroutines                 | 23.6 ms                                                      | 24.4 ms: 1.03x slower                                                  |
| raytrace                   | 253 ms                                                       | 263 ms: 1.04x slower                                                   |
| asyncio_websockets         | 520 ms                                                       | 545 ms: 1.05x slower                                                   |
| json_loads                 | 27.0 us                                                      | 28.7 us: 1.06x slower                                                  |
| sympy_integrate            | 19.8 ms                                                      | 21.1 ms: 1.06x slower                                                  |
| django_template            | 34.1 ms                                                      | 36.3 ms: 1.06x slower                                                  |
| nqueens                    | 78.6 ms                                                      | 83.6 ms: 1.06x slower                                                  |
| sympy_expand               | 457 ms                                                       | 508 ms: 1.11x slower                                                   |
| genshi_xml                 | 48.8 ms                                                      | 54.3 ms: 1.11x slower                                                  |
| sympy_sum                  | 156 ms                                                       | 173 ms: 1.12x slower                                                   |
| python_startup             | 11.0 ms                                                      | 12.4 ms: 1.13x slower                                                  |
| sympy_str                  | 275 ms                                                       | 314 ms: 1.14x slower                                                   |
| bench_mp_pool              | 11.0 ms                                                      | 13.1 ms: 1.19x slower                                                  |
| bench_thread_pool          | 919 us                                                       | 1.10 ms: 1.19x slower                                                  |
| gc_traversal               | 3.14 ms                                                      | 4.25 ms: 1.35x slower                                                  |
| create_gc_cycles           | 1.34 ms                                                      | 1.89 ms: 1.42x slower                                                  |
| telco                      | 7.82 ms                                                      | 161 ms: 20.59x slower                                                  |
| Geometric mean             | (ref)                                                        | 1.06x faster                                                           |

Benchmark hidden because not significant (4): pylint, sqlite_synth, pycparser, pickle_pure_python
Ignored benchmarks (19) of results/bm-20240906-3.13.0rc2-ec61006/bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006.json: aiohttp, asyncio_tcp, asyncio_tcp_ssl, chameleon, dask, docutils, flaskblogging, gunicorn, pickle, pickle_dict, pickle_list, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http, unpack_sequence, unpickle, unpickle_list
Ignored benchmarks (7) of results/bm-20251124-3.15.0a2+-dc62b62-JIT/bm-20251124-vultr-x86_64-python-dc62b622524bf49eb539-3.15.0a2+-dc62b62.json: many_optionals, sphinx, sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile, subparsers

- Geometric mean (including insignificant results): 1.065x faster

# HPT report

- Reliability score: 99.99% likely to be faster
- 90% likely to have a speedup of 1.04x
- 95% likely to have a speedup of 1.03x
- 99% likely to have a speedup of 1.02x

# Memory
- memory change: 1.18x