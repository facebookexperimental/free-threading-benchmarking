# Results vs. 3.12.6

- fork: python
- ref: 7a845ce16548bf94e777
- machine: linux-x86_64
- commit hash: 7a845ce
- commit date: 2026-08-16
- overall geometric mean: 1.066x faster
- HPT reliability: 100.00%
- HPT 99th percentile: 1.02x faster
- Memory change: 1.15x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260816-vultr-x86_64-python-7a845ce16548bf94e777-3.16.0a0-7a845ce |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| 2to3           | 264 ms                                                 | 259 ms: 1.02x faster                                                  |
| docutils       | 2.64 sec                                               | 2.37 sec: 1.11x faster                                                |
| html5lib       | 63.6 ms                                                | 58.8 ms: 1.08x faster                                                 |
| Geometric mean | (ref)                                                  | 1.07x faster                                                          |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260816-vultr-x86_64-python-7a845ce16548bf94e777-3.16.0a0-7a845ce |
|----------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| async_tree_none            | 464 ms                                                 | 295 ms: 1.58x faster                                                  |
| async_tree_memoization_tg  | 560 ms                                                 | 365 ms: 1.53x faster                                                  |
| async_tree_io              | 1.08 sec                                               | 721 ms: 1.50x faster                                                  |
| async_tree_none_tg         | 446 ms                                                 | 301 ms: 1.48x faster                                                  |
| async_tree_io_tg           | 1.11 sec                                               | 759 ms: 1.46x faster                                                  |
| async_tree_memoization     | 555 ms                                                 | 380 ms: 1.46x faster                                                  |
| async_tree_cpu_io_mixed    | 715 ms                                                 | 540 ms: 1.32x faster                                                  |
| async_tree_cpu_io_mixed_tg | 723 ms                                                 | 556 ms: 1.30x faster                                                  |
| async_generators           | 384 ms                                                 | 345 ms: 1.11x faster                                                  |
| asyncio_websockets         | 517 ms                                                 | 544 ms: 1.05x slower                                                  |
| Geometric mean             | (ref)                                                  | 1.32x faster                                                          |

Benchmark hidden because not significant (1): coroutines

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260816-vultr-x86_64-python-7a845ce16548bf94e777-3.16.0a0-7a845ce |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| float          | 80.8 ms                                                | 73.4 ms: 1.10x faster                                                 |
| nbody          | 89.3 ms                                                | 93.4 ms: 1.05x slower                                                 |
| pidigits       | 184 ms                                                 | 194 ms: 1.06x slower                                                  |
| Geometric mean | (ref)                                                  | 1.00x slower                                                          |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260816-vultr-x86_64-python-7a845ce16548bf94e777-3.16.0a0-7a845ce |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| regex_effbot   | 3.17 ms                                                | 2.66 ms: 1.19x faster                                                 |
| regex_compile  | 142 ms                                                 | 149 ms: 1.04x slower                                                  |
| regex_v8       | 20.6 ms                                                | 21.8 ms: 1.06x slower                                                 |
| regex_dna      | 168 ms                                                 | 181 ms: 1.08x slower                                                  |
| Geometric mean | (ref)                                                  | 1.00x faster                                                          |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260816-vultr-x86_64-python-7a845ce16548bf94e777-3.16.0a0-7a845ce |
|----------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| tomli_loads          | 2.11 sec                                               | 1.83 sec: 1.15x faster                                                |
| json_dumps           | 10.4 ms                                                | 9.26 ms: 1.12x faster                                                 |
| xml_etree_iterparse  | 96.7 ms                                                | 91.3 ms: 1.06x faster                                                 |
| unpickle_pure_python | 221 us                                                 | 213 us: 1.04x faster                                                  |
| pickle_pure_python   | 308 us                                                 | 301 us: 1.02x faster                                                  |
| xml_etree_generate   | 85.2 ms                                                | 88.1 ms: 1.03x slower                                                 |
| json_loads           | 26.5 us                                                | 27.5 us: 1.04x slower                                                 |
| xml_etree_parse      | 139 ms                                                 | 144 ms: 1.04x slower                                                  |
| xml_etree_process    | 59.0 ms                                                | 62.4 ms: 1.06x slower                                                 |
| Geometric mean       | (ref)                                                  | 1.02x faster                                                          |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260816-vultr-x86_64-python-7a845ce16548bf94e777-3.16.0a0-7a845ce |
|------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| python_startup_no_site | 7.16 ms                                                | 7.08 ms: 1.01x faster                                                 |
| python_startup         | 9.93 ms                                                | 12.6 ms: 1.27x slower                                                 |
| Geometric mean         | (ref)                                                  | 1.12x slower                                                          |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260816-vultr-x86_64-python-7a845ce16548bf94e777-3.16.0a0-7a845ce |
|-----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| mako            | 11.0 ms                                                | 11.7 ms: 1.07x slower                                                 |
| django_template | 34.7 ms                                                | 37.2 ms: 1.07x slower                                                 |
| Geometric mean  | (ref)                                                  | 1.07x slower                                                          |

All benchmarks:
===============

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260816-vultr-x86_64-python-7a845ce16548bf94e777-3.16.0a0-7a845ce |
|----------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| pylint                     | 319 ms                                                 | 114 ms: 2.80x faster                                                  |
| mdp                        | 2.42 sec                                               | 1.14 sec: 2.12x faster                                                |
| async_tree_none            | 464 ms                                                 | 295 ms: 1.58x faster                                                  |
| async_tree_memoization_tg  | 560 ms                                                 | 365 ms: 1.53x faster                                                  |
| deepcopy                   | 352 us                                                 | 234 us: 1.50x faster                                                  |
| async_tree_io              | 1.08 sec                                               | 721 ms: 1.50x faster                                                  |
| deepcopy_memo              | 40.3 us                                                | 26.8 us: 1.50x faster                                                 |
| async_tree_none_tg         | 446 ms                                                 | 301 ms: 1.48x faster                                                  |
| async_tree_io_tg           | 1.11 sec                                               | 759 ms: 1.46x faster                                                  |
| async_tree_memoization     | 555 ms                                                 | 380 ms: 1.46x faster                                                  |
| typing_runtime_protocols   | 163 us                                                 | 121 us: 1.36x faster                                                  |
| go                         | 139 ms                                                 | 103 ms: 1.35x faster                                                  |
| async_tree_cpu_io_mixed    | 715 ms                                                 | 540 ms: 1.32x faster                                                  |
| async_tree_cpu_io_mixed_tg | 723 ms                                                 | 556 ms: 1.30x faster                                                  |
| comprehensions             | 19.8 us                                                | 15.8 us: 1.26x faster                                                 |
| scimark_sor                | 130 ms                                                 | 108 ms: 1.21x faster                                                  |
| pathlib                    | 21.5 ms                                                | 17.9 ms: 1.20x faster                                                 |
| raytrace                   | 299 ms                                                 | 251 ms: 1.19x faster                                                  |
| regex_effbot               | 3.17 ms                                                | 2.66 ms: 1.19x faster                                                 |
| chaos                      | 62.8 ms                                                | 52.8 ms: 1.19x faster                                                 |
| deepcopy_reduce            | 3.08 us                                                | 2.59 us: 1.19x faster                                                 |
| dulwich_log                | 78.9 ms                                                | 66.9 ms: 1.18x faster                                                 |
| spectral_norm              | 110 ms                                                 | 94.5 ms: 1.17x faster                                                 |
| tomli_loads                | 2.11 sec                                               | 1.83 sec: 1.15x faster                                                |
| logging_silent             | 109 ns                                                 | 95.4 ns: 1.14x faster                                                 |
| generators                 | 32.2 ms                                                | 28.4 ms: 1.13x faster                                                 |
| pyflate                    | 448 ms                                                 | 395 ms: 1.13x faster                                                  |
| crypto_pyaes               | 76.6 ms                                                | 68.2 ms: 1.12x faster                                                 |
| json_dumps                 | 10.4 ms                                                | 9.26 ms: 1.12x faster                                                 |
| async_generators           | 384 ms                                                 | 345 ms: 1.11x faster                                                  |
| docutils                   | 2.64 sec                                               | 2.37 sec: 1.11x faster                                                |
| bpe_tokeniser              | 4.74 sec                                               | 4.27 sec: 1.11x faster                                                |
| logging_simple             | 6.63 us                                                | 6.01 us: 1.10x faster                                                 |
| float                      | 80.8 ms                                                | 73.4 ms: 1.10x faster                                                 |
| nqueens                    | 80.1 ms                                                | 73.1 ms: 1.09x faster                                                 |
| logging_format             | 7.35 us                                                | 6.79 us: 1.08x faster                                                 |
| scimark_fft                | 342 ms                                                 | 316 ms: 1.08x faster                                                  |
| hexiom                     | 6.17 ms                                                | 5.70 ms: 1.08x faster                                                 |
| html5lib                   | 63.6 ms                                                | 58.8 ms: 1.08x faster                                                 |
| scimark_monte_carlo        | 68.4 ms                                                | 63.3 ms: 1.08x faster                                                 |
| sympy_integrate            | 20.5 ms                                                | 19.2 ms: 1.07x faster                                                 |
| xml_etree_iterparse        | 96.7 ms                                                | 91.3 ms: 1.06x faster                                                 |
| sympy_sum                  | 166 ms                                                 | 158 ms: 1.05x faster                                                  |
| sympy_str                  | 292 ms                                                 | 279 ms: 1.05x faster                                                  |
| pycparser                  | 1.17 sec                                               | 1.12 sec: 1.04x faster                                                |
| unpickle_pure_python       | 221 us                                                 | 213 us: 1.04x faster                                                  |
| deltablue                  | 3.45 ms                                                | 3.34 ms: 1.03x faster                                                 |
| pprint_pformat             | 1.52 sec                                               | 1.48 sec: 1.03x faster                                                |
| richards                   | 45.9 ms                                                | 44.8 ms: 1.03x faster                                                 |
| json                       | 5.02 ms                                                | 4.91 ms: 1.02x faster                                                 |
| pprint_safe_repr           | 743 ms                                                 | 728 ms: 1.02x faster                                                  |
| pickle_pure_python         | 308 us                                                 | 301 us: 1.02x faster                                                  |
| 2to3                       | 264 ms                                                 | 259 ms: 1.02x faster                                                  |
| richards_super             | 51.9 ms                                                | 51.1 ms: 1.02x faster                                                 |
| thrift                     | 791 us                                                 | 780 us: 1.01x faster                                                  |
| python_startup_no_site     | 7.16 ms                                                | 7.08 ms: 1.01x faster                                                 |
| meteor_contest             | 104 ms                                                 | 103 ms: 1.00x faster                                                  |
| sympy_expand               | 468 ms                                                 | 476 ms: 1.02x slower                                                  |
| scimark_lu                 | 114 ms                                                 | 116 ms: 1.02x slower                                                  |
| sqlite_synth               | 2.20 us                                                | 2.26 us: 1.03x slower                                                 |
| xml_etree_generate         | 85.2 ms                                                | 88.1 ms: 1.03x slower                                                 |
| json_loads                 | 26.5 us                                                | 27.5 us: 1.04x slower                                                 |
| xml_etree_parse            | 139 ms                                                 | 144 ms: 1.04x slower                                                  |
| regex_compile              | 142 ms                                                 | 149 ms: 1.04x slower                                                  |
| nbody                      | 89.3 ms                                                | 93.4 ms: 1.05x slower                                                 |
| asyncio_websockets         | 517 ms                                                 | 544 ms: 1.05x slower                                                  |
| pidigits                   | 184 ms                                                 | 194 ms: 1.06x slower                                                  |
| regex_v8                   | 20.6 ms                                                | 21.8 ms: 1.06x slower                                                 |
| xml_etree_process          | 59.0 ms                                                | 62.4 ms: 1.06x slower                                                 |
| mako                       | 11.0 ms                                                | 11.7 ms: 1.07x slower                                                 |
| gc_traversal               | 3.46 ms                                                | 3.70 ms: 1.07x slower                                                 |
| scimark_sparse_mat_mult    | 4.39 ms                                                | 4.70 ms: 1.07x slower                                                 |
| django_template            | 34.7 ms                                                | 37.2 ms: 1.07x slower                                                 |
| regex_dna                  | 168 ms                                                 | 181 ms: 1.08x slower                                                  |
| coverage                   | 71.4 ms                                                | 82.6 ms: 1.16x slower                                                 |
| python_startup             | 9.93 ms                                                | 12.6 ms: 1.27x slower                                                 |
| bench_thread_pool          | 941 us                                                 | 1.35 ms: 1.43x slower                                                 |
| create_gc_cycles           | 1.09 ms                                                | 1.66 ms: 1.52x slower                                                 |
| bench_mp_pool              | 10.8 ms                                                | 257 ms: 23.82x slower                                                 |
| telco                      | 6.53 ms                                                | 157 ms: 24.12x slower                                                 |
| Geometric mean             | (ref)                                                  | 1.02x faster                                                          |

Benchmark hidden because not significant (2): coroutines, fannkuch
Ignored benchmarks (23) of results/bm-20240906-3.12.6-a4a2d2b/bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b.json: aiohttp, asyncio_tcp, asyncio_tcp_ssl, chameleon, dask, flaskblogging, genshi_text, genshi_xml, gunicorn, mypy2, pickle, pickle_dict, pickle_list, sqlalchemy_declarative, sqlalchemy_imperative, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http, unpack_sequence, unpickle, unpickle_list
Ignored benchmarks (10) of results/bm-20260816-3.16.0a0-7a845ce/bm-20260816-vultr-x86_64-python-7a845ce16548bf94e777-3.16.0a0-7a845ce.json: connected_components, k_core, many_optionals, shortest_path, sphinx, sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile, subparsers

- Geometric mean (including insignificant results): 1.066x faster

# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.04x
- 95% likely to have a speedup of 1.04x
- 99% likely to have a speedup of 1.02x

# Memory
- memory change: 1.15x