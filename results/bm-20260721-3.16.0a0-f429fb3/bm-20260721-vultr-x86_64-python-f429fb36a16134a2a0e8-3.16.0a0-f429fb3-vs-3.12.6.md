# Results vs. 3.12.6

- fork: python
- ref: f429fb36a16134a2a0e8
- machine: linux-x86_64
- commit hash: f429fb3
- commit date: 2026-07-21
- overall geometric mean: 1.071x faster
- HPT reliability: 100.00%
- HPT 99th percentile: 1.03x faster
- Memory change: 1.15x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260721-vultr-x86_64-python-f429fb36a16134a2a0e8-3.16.0a0-f429fb3 |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| 2to3           | 264 ms                                                 | 258 ms: 1.02x faster                                                  |
| docutils       | 2.64 sec                                               | 2.35 sec: 1.12x faster                                                |
| html5lib       | 63.6 ms                                                | 57.4 ms: 1.11x faster                                                 |
| Geometric mean | (ref)                                                  | 1.08x faster                                                          |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260721-vultr-x86_64-python-f429fb36a16134a2a0e8-3.16.0a0-f429fb3 |
|----------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| async_tree_none            | 464 ms                                                 | 290 ms: 1.60x faster                                                  |
| async_tree_memoization_tg  | 560 ms                                                 | 363 ms: 1.54x faster                                                  |
| async_tree_io              | 1.08 sec                                               | 715 ms: 1.51x faster                                                  |
| async_tree_none_tg         | 446 ms                                                 | 299 ms: 1.49x faster                                                  |
| async_tree_memoization     | 555 ms                                                 | 374 ms: 1.48x faster                                                  |
| async_tree_io_tg           | 1.11 sec                                               | 758 ms: 1.46x faster                                                  |
| async_tree_cpu_io_mixed    | 715 ms                                                 | 538 ms: 1.33x faster                                                  |
| async_tree_cpu_io_mixed_tg | 723 ms                                                 | 555 ms: 1.30x faster                                                  |
| async_generators           | 384 ms                                                 | 340 ms: 1.13x faster                                                  |
| asyncio_websockets         | 517 ms                                                 | 544 ms: 1.05x slower                                                  |
| Geometric mean             | (ref)                                                  | 1.33x faster                                                          |

Benchmark hidden because not significant (1): coroutines

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260721-vultr-x86_64-python-f429fb36a16134a2a0e8-3.16.0a0-f429fb3 |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| float          | 80.8 ms                                                | 74.0 ms: 1.09x faster                                                 |
| pidigits       | 184 ms                                                 | 184 ms: 1.00x faster                                                  |
| nbody          | 89.3 ms                                                | 92.3 ms: 1.03x slower                                                 |
| Geometric mean | (ref)                                                  | 1.02x faster                                                          |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260721-vultr-x86_64-python-f429fb36a16134a2a0e8-3.16.0a0-f429fb3 |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| regex_effbot   | 3.17 ms                                                | 2.60 ms: 1.22x faster                                                 |
| regex_v8       | 20.6 ms                                                | 21.1 ms: 1.02x slower                                                 |
| regex_dna      | 168 ms                                                 | 175 ms: 1.04x slower                                                  |
| regex_compile  | 142 ms                                                 | 149 ms: 1.04x slower                                                  |
| Geometric mean | (ref)                                                  | 1.02x faster                                                          |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260721-vultr-x86_64-python-f429fb36a16134a2a0e8-3.16.0a0-f429fb3 |
|----------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| tomli_loads          | 2.11 sec                                               | 1.86 sec: 1.13x faster                                                |
| xml_etree_iterparse  | 96.7 ms                                                | 90.6 ms: 1.07x faster                                                 |
| unpickle_pure_python | 221 us                                                 | 209 us: 1.05x faster                                                  |
| json_dumps           | 10.4 ms                                                | 9.95 ms: 1.04x faster                                                 |
| json_loads           | 26.5 us                                                | 25.9 us: 1.02x faster                                                 |
| pickle_pure_python   | 308 us                                                 | 303 us: 1.01x faster                                                  |
| xml_etree_generate   | 85.2 ms                                                | 88.2 ms: 1.04x slower                                                 |
| xml_etree_parse      | 139 ms                                                 | 144 ms: 1.04x slower                                                  |
| xml_etree_process    | 59.0 ms                                                | 62.3 ms: 1.06x slower                                                 |
| Geometric mean       | (ref)                                                  | 1.02x faster                                                          |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260721-vultr-x86_64-python-f429fb36a16134a2a0e8-3.16.0a0-f429fb3 |
|------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| python_startup_no_site | 7.16 ms                                                | 7.04 ms: 1.02x faster                                                 |
| python_startup         | 9.93 ms                                                | 12.5 ms: 1.25x slower                                                 |
| Geometric mean         | (ref)                                                  | 1.11x slower                                                          |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260721-vultr-x86_64-python-f429fb36a16134a2a0e8-3.16.0a0-f429fb3 |
|-----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| django_template | 34.7 ms                                                | 36.7 ms: 1.06x slower                                                 |
| mako            | 11.0 ms                                                | 12.1 ms: 1.10x slower                                                 |
| Geometric mean  | (ref)                                                  | 1.08x slower                                                          |

All benchmarks:
===============

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260721-vultr-x86_64-python-f429fb36a16134a2a0e8-3.16.0a0-f429fb3 |
|----------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| pylint                     | 319 ms                                                 | 114 ms: 2.79x faster                                                  |
| mdp                        | 2.42 sec                                               | 1.13 sec: 2.13x faster                                                |
| async_tree_none            | 464 ms                                                 | 290 ms: 1.60x faster                                                  |
| async_tree_memoization_tg  | 560 ms                                                 | 363 ms: 1.54x faster                                                  |
| deepcopy                   | 352 us                                                 | 232 us: 1.52x faster                                                  |
| async_tree_io              | 1.08 sec                                               | 715 ms: 1.51x faster                                                  |
| deepcopy_memo              | 40.3 us                                                | 26.7 us: 1.51x faster                                                 |
| async_tree_none_tg         | 446 ms                                                 | 299 ms: 1.49x faster                                                  |
| async_tree_memoization     | 555 ms                                                 | 374 ms: 1.48x faster                                                  |
| async_tree_io_tg           | 1.11 sec                                               | 758 ms: 1.46x faster                                                  |
| typing_runtime_protocols   | 163 us                                                 | 119 us: 1.38x faster                                                  |
| go                         | 139 ms                                                 | 102 ms: 1.36x faster                                                  |
| async_tree_cpu_io_mixed    | 715 ms                                                 | 538 ms: 1.33x faster                                                  |
| async_tree_cpu_io_mixed_tg | 723 ms                                                 | 555 ms: 1.30x faster                                                  |
| comprehensions             | 19.8 us                                                | 15.6 us: 1.27x faster                                                 |
| scimark_sor                | 130 ms                                                 | 106 ms: 1.23x faster                                                  |
| regex_effbot               | 3.17 ms                                                | 2.60 ms: 1.22x faster                                                 |
| pathlib                    | 21.5 ms                                                | 17.7 ms: 1.22x faster                                                 |
| deepcopy_reduce            | 3.08 us                                                | 2.54 us: 1.21x faster                                                 |
| raytrace                   | 299 ms                                                 | 251 ms: 1.19x faster                                                  |
| chaos                      | 62.8 ms                                                | 53.7 ms: 1.17x faster                                                 |
| dulwich_log                | 78.9 ms                                                | 67.6 ms: 1.17x faster                                                 |
| generators                 | 32.2 ms                                                | 27.7 ms: 1.16x faster                                                 |
| spectral_norm              | 110 ms                                                 | 95.1 ms: 1.16x faster                                                 |
| pyflate                    | 448 ms                                                 | 388 ms: 1.15x faster                                                  |
| logging_silent             | 109 ns                                                 | 95.2 ns: 1.14x faster                                                 |
| async_generators           | 384 ms                                                 | 340 ms: 1.13x faster                                                  |
| tomli_loads                | 2.11 sec                                               | 1.86 sec: 1.13x faster                                                |
| bpe_tokeniser              | 4.74 sec                                               | 4.20 sec: 1.13x faster                                                |
| docutils                   | 2.64 sec                                               | 2.35 sec: 1.12x faster                                                |
| crypto_pyaes               | 76.6 ms                                                | 68.6 ms: 1.12x faster                                                 |
| logging_simple             | 6.63 us                                                | 5.99 us: 1.11x faster                                                 |
| html5lib                   | 63.6 ms                                                | 57.4 ms: 1.11x faster                                                 |
| hexiom                     | 6.17 ms                                                | 5.59 ms: 1.10x faster                                                 |
| float                      | 80.8 ms                                                | 74.0 ms: 1.09x faster                                                 |
| json                       | 5.02 ms                                                | 4.67 ms: 1.08x faster                                                 |
| nqueens                    | 80.1 ms                                                | 74.6 ms: 1.07x faster                                                 |
| sympy_integrate            | 20.5 ms                                                | 19.1 ms: 1.07x faster                                                 |
| logging_format             | 7.35 us                                                | 6.85 us: 1.07x faster                                                 |
| scimark_monte_carlo        | 68.4 ms                                                | 64.0 ms: 1.07x faster                                                 |
| xml_etree_iterparse        | 96.7 ms                                                | 90.6 ms: 1.07x faster                                                 |
| scimark_fft                | 342 ms                                                 | 321 ms: 1.06x faster                                                  |
| deltablue                  | 3.45 ms                                                | 3.25 ms: 1.06x faster                                                 |
| sympy_sum                  | 166 ms                                                 | 157 ms: 1.05x faster                                                  |
| unpickle_pure_python       | 221 us                                                 | 209 us: 1.05x faster                                                  |
| sympy_str                  | 292 ms                                                 | 279 ms: 1.04x faster                                                  |
| json_dumps                 | 10.4 ms                                                | 9.95 ms: 1.04x faster                                                 |
| pycparser                  | 1.17 sec                                               | 1.13 sec: 1.03x faster                                                |
| pprint_pformat             | 1.52 sec                                               | 1.48 sec: 1.03x faster                                                |
| pprint_safe_repr           | 743 ms                                                 | 724 ms: 1.03x faster                                                  |
| json_loads                 | 26.5 us                                                | 25.9 us: 1.02x faster                                                 |
| 2to3                       | 264 ms                                                 | 258 ms: 1.02x faster                                                  |
| richards                   | 45.9 ms                                                | 45.1 ms: 1.02x faster                                                 |
| python_startup_no_site     | 7.16 ms                                                | 7.04 ms: 1.02x faster                                                 |
| meteor_contest             | 104 ms                                                 | 102 ms: 1.02x faster                                                  |
| pickle_pure_python         | 308 us                                                 | 303 us: 1.01x faster                                                  |
| richards_super             | 51.9 ms                                                | 51.2 ms: 1.01x faster                                                 |
| fannkuch                   | 372 ms                                                 | 368 ms: 1.01x faster                                                  |
| thrift                     | 791 us                                                 | 784 us: 1.01x faster                                                  |
| pidigits                   | 184 ms                                                 | 184 ms: 1.00x faster                                                  |
| sympy_expand               | 468 ms                                                 | 473 ms: 1.01x slower                                                  |
| regex_v8                   | 20.6 ms                                                | 21.1 ms: 1.02x slower                                                 |
| nbody                      | 89.3 ms                                                | 92.3 ms: 1.03x slower                                                 |
| xml_etree_generate         | 85.2 ms                                                | 88.2 ms: 1.04x slower                                                 |
| xml_etree_parse            | 139 ms                                                 | 144 ms: 1.04x slower                                                  |
| sqlite_synth               | 2.20 us                                                | 2.29 us: 1.04x slower                                                 |
| regex_dna                  | 168 ms                                                 | 175 ms: 1.04x slower                                                  |
| regex_compile              | 142 ms                                                 | 149 ms: 1.04x slower                                                  |
| scimark_lu                 | 114 ms                                                 | 120 ms: 1.05x slower                                                  |
| asyncio_websockets         | 517 ms                                                 | 544 ms: 1.05x slower                                                  |
| xml_etree_process          | 59.0 ms                                                | 62.3 ms: 1.06x slower                                                 |
| django_template            | 34.7 ms                                                | 36.7 ms: 1.06x slower                                                 |
| gc_traversal               | 3.46 ms                                                | 3.70 ms: 1.07x slower                                                 |
| scimark_sparse_mat_mult    | 4.39 ms                                                | 4.77 ms: 1.09x slower                                                 |
| mako                       | 11.0 ms                                                | 12.1 ms: 1.10x slower                                                 |
| coverage                   | 71.4 ms                                                | 84.2 ms: 1.18x slower                                                 |
| python_startup             | 9.93 ms                                                | 12.5 ms: 1.25x slower                                                 |
| bench_thread_pool          | 941 us                                                 | 1.34 ms: 1.42x slower                                                 |
| create_gc_cycles           | 1.09 ms                                                | 1.68 ms: 1.54x slower                                                 |
| telco                      | 6.53 ms                                                | 157 ms: 24.07x slower                                                 |
| bench_mp_pool              | 10.8 ms                                                | 284 ms: 26.33x slower                                                 |
| Geometric mean             | (ref)                                                  | 1.02x faster                                                          |

Benchmark hidden because not significant (1): coroutines
Ignored benchmarks (23) of results/bm-20240906-3.12.6-a4a2d2b/bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b.json: aiohttp, asyncio_tcp, asyncio_tcp_ssl, chameleon, dask, flaskblogging, genshi_text, genshi_xml, gunicorn, mypy2, pickle, pickle_dict, pickle_list, sqlalchemy_declarative, sqlalchemy_imperative, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http, unpack_sequence, unpickle, unpickle_list
Ignored benchmarks (10) of results/bm-20260721-3.16.0a0-f429fb3/bm-20260721-vultr-x86_64-python-f429fb36a16134a2a0e8-3.16.0a0-f429fb3.json: connected_components, k_core, many_optionals, shortest_path, sphinx, sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile, subparsers

- Geometric mean (including insignificant results): 1.071x faster

# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.05x
- 95% likely to have a speedup of 1.04x
- 99% likely to have a speedup of 1.03x

# Memory
- memory change: 1.15x