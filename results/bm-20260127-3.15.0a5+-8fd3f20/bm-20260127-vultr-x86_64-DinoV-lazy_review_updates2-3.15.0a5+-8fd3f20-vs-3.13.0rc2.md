# Results vs. 3.13.0rc2

- fork: DinoV
- ref: lazy_review_updates2
- machine: linux-x86_64
- commit hash: 8fd3f20
- commit date: 2026-01-27
- overall geometric mean: 1.074x slower
- HPT reliability: 99.61%
- HPT 99th percentile: 1.00x slower
- Memory change: 1.18x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260127-vultr-x86_64-DinoV-lazy_review_updates2-3.15.0a5+-8fd3f20 |
|----------------|:------------------------------------------------------------:|:---------------------------------------------------------------------:|
| 2to3           | 260 ms                                                       | 281 ms: 1.08x slower                                                  |
| docutils       | 2.62 sec                                                     | 2.74 sec: 1.05x slower                                                |
| html5lib       | 67.0 ms                                                      | 63.5 ms: 1.06x faster                                                 |
| Geometric mean | (ref)                                                        | 1.02x slower                                                          |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260127-vultr-x86_64-DinoV-lazy_review_updates2-3.15.0a5+-8fd3f20 |
|----------------------------|:------------------------------------------------------------:|:---------------------------------------------------------------------:|
| async_tree_io_tg           | 913 ms                                                       | 627 ms: 1.46x faster                                                  |
| async_tree_memoization     | 461 ms                                                       | 323 ms: 1.43x faster                                                  |
| async_tree_io              | 876 ms                                                       | 642 ms: 1.36x faster                                                  |
| async_tree_none            | 354 ms                                                       | 266 ms: 1.33x faster                                                  |
| async_tree_memoization_tg  | 414 ms                                                       | 322 ms: 1.29x faster                                                  |
| async_tree_none_tg         | 336 ms                                                       | 272 ms: 1.24x faster                                                  |
| async_tree_cpu_io_mixed    | 666 ms                                                       | 560 ms: 1.19x faster                                                  |
| async_tree_cpu_io_mixed_tg | 638 ms                                                       | 563 ms: 1.13x faster                                                  |
| asyncio_websockets         | 520 ms                                                       | 546 ms: 1.05x slower                                                  |
| coroutines                 | 23.6 ms                                                      | 25.5 ms: 1.08x slower                                                 |
| Geometric mean             | (ref)                                                        | 1.20x faster                                                          |

Benchmark hidden because not significant (1): async_generators

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260127-vultr-x86_64-DinoV-lazy_review_updates2-3.15.0a5+-8fd3f20 |
|----------------|:------------------------------------------------------------:|:---------------------------------------------------------------------:|
| pidigits       | 217 ms                                                       | 190 ms: 1.14x faster                                                  |
| float          | 77.5 ms                                                      | 76.5 ms: 1.01x faster                                                 |
| nbody          | 85.1 ms                                                      | 105 ms: 1.24x slower                                                  |
| Geometric mean | (ref)                                                        | 1.02x slower                                                          |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260127-vultr-x86_64-DinoV-lazy_review_updates2-3.15.0a5+-8fd3f20 |
|----------------|:------------------------------------------------------------:|:---------------------------------------------------------------------:|
| regex_effbot   | 3.08 ms                                                      | 2.70 ms: 1.14x faster                                                 |
| regex_dna      | 180 ms                                                       | 177 ms: 1.02x faster                                                  |
| regex_v8       | 22.7 ms                                                      | 22.4 ms: 1.01x faster                                                 |
| regex_compile  | 132 ms                                                       | 140 ms: 1.06x slower                                                  |
| Geometric mean | (ref)                                                        | 1.03x faster                                                          |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260127-vultr-x86_64-DinoV-lazy_review_updates2-3.15.0a5+-8fd3f20 |
|----------------------|:------------------------------------------------------------:|:---------------------------------------------------------------------:|
| tomli_loads          | 2.01 sec                                                     | 2.05 sec: 1.02x slower                                                |
| json_dumps           | 10.5 ms                                                      | 11.3 ms: 1.07x slower                                                 |
| xml_etree_parse      | 136 ms                                                       | 146 ms: 1.07x slower                                                  |
| json_loads           | 27.0 us                                                      | 29.8 us: 1.10x slower                                                 |
| unpickle_pure_python | 210 us                                                       | 240 us: 1.14x slower                                                  |
| pickle_pure_python   | 294 us                                                       | 348 us: 1.18x slower                                                  |
| xml_etree_generate   | 85.4 ms                                                      | 101 ms: 1.19x slower                                                  |
| xml_etree_process    | 59.3 ms                                                      | 71.4 ms: 1.20x slower                                                 |
| Geometric mean       | (ref)                                                        | 1.11x slower                                                          |

Benchmark hidden because not significant (1): xml_etree_iterparse

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260127-vultr-x86_64-DinoV-lazy_review_updates2-3.15.0a5+-8fd3f20 |
|------------------------|:------------------------------------------------------------:|:---------------------------------------------------------------------:|
| python_startup_no_site | 7.39 ms                                                      | 7.81 ms: 1.06x slower                                                 |
| python_startup         | 11.0 ms                                                      | 13.5 ms: 1.23x slower                                                 |
| Geometric mean         | (ref)                                                        | 1.14x slower                                                          |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260127-vultr-x86_64-DinoV-lazy_review_updates2-3.15.0a5+-8fd3f20 |
|-----------------|:------------------------------------------------------------:|:---------------------------------------------------------------------:|
| genshi_text     | 21.5 ms                                                      | 25.1 ms: 1.16x slower                                                 |
| genshi_xml      | 48.8 ms                                                      | 56.9 ms: 1.17x slower                                                 |
| mako            | 11.3 ms                                                      | 13.5 ms: 1.19x slower                                                 |
| django_template | 34.1 ms                                                      | 42.2 ms: 1.24x slower                                                 |
| Geometric mean  | (ref)                                                        | 1.19x slower                                                          |

All benchmarks:
===============

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260127-vultr-x86_64-DinoV-lazy_review_updates2-3.15.0a5+-8fd3f20 |
|----------------------------|:------------------------------------------------------------:|:---------------------------------------------------------------------:|
| mdp                        | 2.36 sec                                                     | 1.36 sec: 1.74x faster                                                |
| async_tree_io_tg           | 913 ms                                                       | 627 ms: 1.46x faster                                                  |
| async_tree_memoization     | 461 ms                                                       | 323 ms: 1.43x faster                                                  |
| deepcopy_memo              | 39.1 us                                                      | 28.2 us: 1.39x faster                                                 |
| async_tree_io              | 876 ms                                                       | 642 ms: 1.36x faster                                                  |
| async_tree_none            | 354 ms                                                       | 266 ms: 1.33x faster                                                  |
| deepcopy                   | 355 us                                                       | 270 us: 1.32x faster                                                  |
| async_tree_memoization_tg  | 414 ms                                                       | 322 ms: 1.29x faster                                                  |
| go                         | 141 ms                                                       | 111 ms: 1.27x faster                                                  |
| async_tree_none_tg         | 336 ms                                                       | 272 ms: 1.24x faster                                                  |
| async_tree_cpu_io_mixed    | 666 ms                                                       | 560 ms: 1.19x faster                                                  |
| regex_effbot               | 3.08 ms                                                      | 2.70 ms: 1.14x faster                                                 |
| pidigits                   | 217 ms                                                       | 190 ms: 1.14x faster                                                  |
| async_tree_cpu_io_mixed_tg | 638 ms                                                       | 563 ms: 1.13x faster                                                  |
| scimark_sor                | 134 ms                                                       | 124 ms: 1.08x faster                                                  |
| html5lib                   | 67.0 ms                                                      | 63.5 ms: 1.06x faster                                                 |
| spectral_norm              | 111 ms                                                       | 108 ms: 1.03x faster                                                  |
| regex_dna                  | 180 ms                                                       | 177 ms: 1.02x faster                                                  |
| deepcopy_reduce            | 3.11 us                                                      | 3.05 us: 1.02x faster                                                 |
| pyflate                    | 449 ms                                                       | 442 ms: 1.02x faster                                                  |
| regex_v8                   | 22.7 ms                                                      | 22.4 ms: 1.01x faster                                                 |
| float                      | 77.5 ms                                                      | 76.5 ms: 1.01x faster                                                 |
| dulwich_log                | 74.8 ms                                                      | 74.0 ms: 1.01x faster                                                 |
| tomli_loads                | 2.01 sec                                                     | 2.05 sec: 1.02x slower                                                |
| pathlib                    | 19.2 ms                                                      | 19.8 ms: 1.03x slower                                                 |
| generators                 | 28.8 ms                                                      | 30.1 ms: 1.04x slower                                                 |
| docutils                   | 2.62 sec                                                     | 2.74 sec: 1.05x slower                                                |
| hexiom                     | 5.99 ms                                                      | 6.28 ms: 1.05x slower                                                 |
| asyncio_websockets         | 520 ms                                                       | 546 ms: 1.05x slower                                                  |
| python_startup_no_site     | 7.39 ms                                                      | 7.81 ms: 1.06x slower                                                 |
| sympy_integrate            | 19.8 ms                                                      | 21.0 ms: 1.06x slower                                                 |
| regex_compile              | 132 ms                                                       | 140 ms: 1.06x slower                                                  |
| meteor_contest             | 102 ms                                                       | 109 ms: 1.07x slower                                                  |
| json_dumps                 | 10.5 ms                                                      | 11.3 ms: 1.07x slower                                                 |
| scimark_fft                | 349 ms                                                       | 374 ms: 1.07x slower                                                  |
| comprehensions             | 16.5 us                                                      | 17.6 us: 1.07x slower                                                 |
| xml_etree_parse            | 136 ms                                                       | 146 ms: 1.07x slower                                                  |
| logging_silent             | 103 ns                                                       | 110 ns: 1.07x slower                                                  |
| coroutines                 | 23.6 ms                                                      | 25.5 ms: 1.08x slower                                                 |
| 2to3                       | 260 ms                                                       | 281 ms: 1.08x slower                                                  |
| scimark_monte_carlo        | 65.4 ms                                                      | 70.9 ms: 1.08x slower                                                 |
| scimark_sparse_mat_mult    | 4.71 ms                                                      | 5.12 ms: 1.09x slower                                                 |
| pycparser                  | 1.12 sec                                                     | 1.22 sec: 1.09x slower                                                |
| scimark_lu                 | 113 ms                                                       | 123 ms: 1.09x slower                                                  |
| richards_super             | 51.6 ms                                                      | 56.8 ms: 1.10x slower                                                 |
| sympy_sum                  | 156 ms                                                       | 172 ms: 1.10x slower                                                  |
| json_loads                 | 27.0 us                                                      | 29.8 us: 1.10x slower                                                 |
| json                       | 4.93 ms                                                      | 5.46 ms: 1.11x slower                                                 |
| bpe_tokeniser              | 4.45 sec                                                     | 4.97 sec: 1.12x slower                                                |
| richards                   | 45.2 ms                                                      | 50.7 ms: 1.12x slower                                                 |
| logging_simple             | 6.16 us                                                      | 6.96 us: 1.13x slower                                                 |
| pprint_pformat             | 1.50 sec                                                     | 1.69 sec: 1.13x slower                                                |
| coverage                   | 83.0 ms                                                      | 94.0 ms: 1.13x slower                                                 |
| pprint_safe_repr           | 738 ms                                                       | 836 ms: 1.13x slower                                                  |
| chaos                      | 57.3 ms                                                      | 65.2 ms: 1.14x slower                                                 |
| sympy_str                  | 275 ms                                                       | 314 ms: 1.14x slower                                                  |
| unpickle_pure_python       | 210 us                                                       | 240 us: 1.14x slower                                                  |
| genshi_text                | 21.5 ms                                                      | 25.1 ms: 1.16x slower                                                 |
| genshi_xml                 | 48.8 ms                                                      | 56.9 ms: 1.17x slower                                                 |
| crypto_pyaes               | 67.9 ms                                                      | 79.3 ms: 1.17x slower                                                 |
| sympy_expand               | 457 ms                                                       | 536 ms: 1.17x slower                                                  |
| nqueens                    | 78.6 ms                                                      | 92.3 ms: 1.17x slower                                                 |
| deltablue                  | 3.12 ms                                                      | 3.67 ms: 1.18x slower                                                 |
| raytrace                   | 253 ms                                                       | 298 ms: 1.18x slower                                                  |
| pickle_pure_python         | 294 us                                                       | 348 us: 1.18x slower                                                  |
| thrift                     | 778 us                                                       | 921 us: 1.18x slower                                                  |
| xml_etree_generate         | 85.4 ms                                                      | 101 ms: 1.19x slower                                                  |
| mako                       | 11.3 ms                                                      | 13.5 ms: 1.19x slower                                                 |
| logging_format             | 6.84 us                                                      | 8.23 us: 1.20x slower                                                 |
| xml_etree_process          | 59.3 ms                                                      | 71.4 ms: 1.20x slower                                                 |
| typing_runtime_protocols   | 155 us                                                       | 187 us: 1.21x slower                                                  |
| python_startup             | 11.0 ms                                                      | 13.5 ms: 1.23x slower                                                 |
| django_template            | 34.1 ms                                                      | 42.2 ms: 1.24x slower                                                 |
| nbody                      | 85.1 ms                                                      | 105 ms: 1.24x slower                                                  |
| fannkuch                   | 370 ms                                                       | 458 ms: 1.24x slower                                                  |
| sqlite_synth               | 2.21 us                                                      | 2.81 us: 1.27x slower                                                 |
| gc_traversal               | 3.14 ms                                                      | 4.36 ms: 1.39x slower                                                 |
| create_gc_cycles           | 1.34 ms                                                      | 2.04 ms: 1.53x slower                                                 |
| bench_thread_pool          | 919 us                                                       | 1.42 ms: 1.55x slower                                                 |
| bench_mp_pool              | 11.0 ms                                                      | 194 ms: 17.62x slower                                                 |
| telco                      | 7.82 ms                                                      | 190 ms: 24.30x slower                                                 |
| Geometric mean             | (ref)                                                        | 1.12x slower                                                          |

Benchmark hidden because not significant (3): pylint, xml_etree_iterparse, async_generators
Ignored benchmarks (18) of results/bm-20240906-3.13.0rc2-ec61006/bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006.json: aiohttp, asyncio_tcp, asyncio_tcp_ssl, chameleon, dask, flaskblogging, gunicorn, pickle, pickle_dict, pickle_list, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http, unpack_sequence, unpickle, unpickle_list
Ignored benchmarks (10) of results/bm-20260127-3.15.0a5+-8fd3f20/bm-20260127-vultr-x86_64-DinoV-lazy_review_updates2-3.15.0a5+-8fd3f20.json: connected_components, k_core, many_optionals, shortest_path, sphinx, sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile, subparsers

- Geometric mean (including insignificant results): 1.074x slower

# HPT report

- Reliability score: 99.61% likely to be slow
- 90% likely to have a slowdown of 1.04x
- 95% likely to have a slowdown of 1.02x
- 99% likely to have a slowdown of 1.00x

# Memory
- memory change: 1.18x