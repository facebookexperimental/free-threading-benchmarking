# Results vs. 3.13.0rc2

- fork: python
- ref: 6b632ce36b950c7753c5
- machine: linux-x86_64
- commit hash: 6b632ce
- commit date: 2026-05-02
- overall geometric mean: 1.050x faster
- HPT reliability: 100.00%
- HPT 99th percentile: 1.03x faster
- Memory change: 1.17x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260502-vultr-x86_64-python-6b632ce36b950c7753c5-3.15.0a8+-6b632ce |
|----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| 2to3           | 260 ms                                                       | 256 ms: 1.02x faster                                                   |
| docutils       | 2.62 sec                                                     | 2.33 sec: 1.12x faster                                                 |
| html5lib       | 67.0 ms                                                      | 57.7 ms: 1.16x faster                                                  |
| Geometric mean | (ref)                                                        | 1.10x faster                                                           |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260502-vultr-x86_64-python-6b632ce36b950c7753c5-3.15.0a8+-6b632ce |
|----------------------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| async_tree_cpu_io_mixed    | 666 ms                                                       | 527 ms: 1.26x faster                                                   |
| async_tree_memoization     | 461 ms                                                       | 369 ms: 1.25x faster                                                   |
| async_tree_none            | 354 ms                                                       | 285 ms: 1.24x faster                                                   |
| async_tree_io              | 876 ms                                                       | 721 ms: 1.21x faster                                                   |
| async_tree_io_tg           | 913 ms                                                       | 763 ms: 1.20x faster                                                   |
| async_tree_cpu_io_mixed_tg | 638 ms                                                       | 543 ms: 1.17x faster                                                   |
| async_tree_none_tg         | 336 ms                                                       | 294 ms: 1.14x faster                                                   |
| asyncio_tcp                | 505 ms                                                       | 443 ms: 1.14x faster                                                   |
| async_generators           | 377 ms                                                       | 331 ms: 1.14x faster                                                   |
| async_tree_memoization_tg  | 414 ms                                                       | 363 ms: 1.14x faster                                                   |
| coroutines                 | 23.6 ms                                                      | 22.4 ms: 1.05x faster                                                  |
| asyncio_tcp_ssl            | 1.51 sec                                                     | 1.48 sec: 1.02x faster                                                 |
| asyncio_websockets         | 520 ms                                                       | 543 ms: 1.04x slower                                                   |
| Geometric mean             | (ref)                                                        | 1.15x faster                                                           |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260502-vultr-x86_64-python-6b632ce36b950c7753c5-3.15.0a8+-6b632ce |
|----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| pidigits       | 217 ms                                                       | 192 ms: 1.13x faster                                                   |
| float          | 77.5 ms                                                      | 73.4 ms: 1.06x faster                                                  |
| nbody          | 85.1 ms                                                      | 84.5 ms: 1.01x faster                                                  |
| Geometric mean | (ref)                                                        | 1.06x faster                                                           |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260502-vultr-x86_64-python-6b632ce36b950c7753c5-3.15.0a8+-6b632ce |
|----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| regex_compile  | 132 ms                                                       | 122 ms: 1.09x faster                                                   |
| regex_v8       | 22.7 ms                                                      | 22.3 ms: 1.02x faster                                                  |
| regex_dna      | 180 ms                                                       | 184 ms: 1.02x slower                                                   |
| Geometric mean | (ref)                                                        | 1.02x faster                                                           |

Benchmark hidden because not significant (1): regex_effbot

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260502-vultr-x86_64-python-6b632ce36b950c7753c5-3.15.0a8+-6b632ce |
|----------------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| json_dumps           | 10.5 ms                                                      | 9.08 ms: 1.16x faster                                                  |
| pickle_dict          | 32.5 us                                                      | 28.2 us: 1.15x faster                                                  |
| tomli_loads          | 2.01 sec                                                     | 1.82 sec: 1.10x faster                                                 |
| pickle_list          | 4.93 us                                                      | 4.65 us: 1.06x faster                                                  |
| unpickle_list        | 4.71 us                                                      | 4.48 us: 1.05x faster                                                  |
| json_loads           | 27.0 us                                                      | 26.0 us: 1.04x faster                                                  |
| xml_etree_iterparse  | 94.9 ms                                                      | 93.1 ms: 1.02x faster                                                  |
| xml_etree_generate   | 85.4 ms                                                      | 86.1 ms: 1.01x slower                                                  |
| unpickle_pure_python | 210 us                                                       | 213 us: 1.01x slower                                                   |
| pickle_pure_python   | 294 us                                                       | 307 us: 1.04x slower                                                   |
| unpickle             | 14.3 us                                                      | 15.1 us: 1.06x slower                                                  |
| pickle               | 11.3 us                                                      | 12.9 us: 1.14x slower                                                  |
| xml_etree_parse      | 136 ms                                                       | 157 ms: 1.15x slower                                                   |
| Geometric mean       | (ref)                                                        | 1.01x faster                                                           |

Benchmark hidden because not significant (1): xml_etree_process

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260502-vultr-x86_64-python-6b632ce36b950c7753c5-3.15.0a8+-6b632ce |
|------------------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| python_startup_no_site | 7.39 ms                                                      | 7.00 ms: 1.06x faster                                                  |
| python_startup         | 11.0 ms                                                      | 12.7 ms: 1.16x slower                                                  |
| Geometric mean         | (ref)                                                        | 1.05x slower                                                           |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260502-vultr-x86_64-python-6b632ce36b950c7753c5-3.15.0a8+-6b632ce |
|-----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| django_template | 34.1 ms                                                      | 33.2 ms: 1.03x faster                                                  |
| mako            | 11.3 ms                                                      | 12.8 ms: 1.13x slower                                                  |
| Geometric mean  | (ref)                                                        | 1.05x slower                                                           |

All benchmarks:
===============

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260502-vultr-x86_64-python-6b632ce36b950c7753c5-3.15.0a8+-6b632ce |
|----------------------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| pylint                     | 317 ms                                                       | 114 ms: 2.78x faster                                                   |
| mdp                        | 2.36 sec                                                     | 1.13 sec: 2.09x faster                                                 |
| deepcopy                   | 355 us                                                       | 219 us: 1.62x faster                                                   |
| deepcopy_memo              | 39.1 us                                                      | 25.8 us: 1.51x faster                                                  |
| go                         | 141 ms                                                       | 102 ms: 1.39x faster                                                   |
| deepcopy_reduce            | 3.11 us                                                      | 2.38 us: 1.31x faster                                                  |
| spectral_norm              | 111 ms                                                       | 85.6 ms: 1.30x faster                                                  |
| scimark_sor                | 134 ms                                                       | 104 ms: 1.30x faster                                                   |
| async_tree_cpu_io_mixed    | 666 ms                                                       | 527 ms: 1.26x faster                                                   |
| logging_silent             | 103 ns                                                       | 81.9 ns: 1.25x faster                                                  |
| async_tree_memoization     | 461 ms                                                       | 369 ms: 1.25x faster                                                   |
| async_tree_none            | 354 ms                                                       | 285 ms: 1.24x faster                                                   |
| async_tree_io              | 876 ms                                                       | 721 ms: 1.21x faster                                                   |
| scimark_fft                | 349 ms                                                       | 289 ms: 1.21x faster                                                   |
| async_tree_io_tg           | 913 ms                                                       | 763 ms: 1.20x faster                                                   |
| async_tree_cpu_io_mixed_tg | 638 ms                                                       | 543 ms: 1.17x faster                                                   |
| richards                   | 45.2 ms                                                      | 38.6 ms: 1.17x faster                                                  |
| html5lib                   | 67.0 ms                                                      | 57.7 ms: 1.16x faster                                                  |
| json_dumps                 | 10.5 ms                                                      | 9.08 ms: 1.16x faster                                                  |
| richards_super             | 51.6 ms                                                      | 44.6 ms: 1.16x faster                                                  |
| pickle_dict                | 32.5 us                                                      | 28.2 us: 1.15x faster                                                  |
| async_tree_none_tg         | 336 ms                                                       | 294 ms: 1.14x faster                                                   |
| asyncio_tcp                | 505 ms                                                       | 443 ms: 1.14x faster                                                   |
| async_generators           | 377 ms                                                       | 331 ms: 1.14x faster                                                   |
| async_tree_memoization_tg  | 414 ms                                                       | 363 ms: 1.14x faster                                                   |
| pidigits                   | 217 ms                                                       | 192 ms: 1.13x faster                                                   |
| pyflate                    | 449 ms                                                       | 398 ms: 1.13x faster                                                   |
| docutils                   | 2.62 sec                                                     | 2.33 sec: 1.12x faster                                                 |
| comprehensions             | 16.5 us                                                      | 14.7 us: 1.12x faster                                                  |
| pathlib                    | 19.2 ms                                                      | 17.2 ms: 1.12x faster                                                  |
| scimark_monte_carlo        | 65.4 ms                                                      | 59.1 ms: 1.11x faster                                                  |
| dulwich_log                | 74.8 ms                                                      | 67.8 ms: 1.10x faster                                                  |
| logging_simple             | 6.16 us                                                      | 5.59 us: 1.10x faster                                                  |
| tomli_loads                | 2.01 sec                                                     | 1.82 sec: 1.10x faster                                                 |
| nqueens                    | 78.6 ms                                                      | 71.6 ms: 1.10x faster                                                  |
| chaos                      | 57.3 ms                                                      | 52.3 ms: 1.10x faster                                                  |
| regex_compile              | 132 ms                                                       | 122 ms: 1.09x faster                                                   |
| sympy_integrate            | 19.8 ms                                                      | 18.3 ms: 1.08x faster                                                  |
| bpe_tokeniser              | 4.45 sec                                                     | 4.14 sec: 1.07x faster                                                 |
| logging_format             | 6.84 us                                                      | 6.42 us: 1.07x faster                                                  |
| scimark_sparse_mat_mult    | 4.71 ms                                                      | 4.42 ms: 1.06x faster                                                  |
| hexiom                     | 5.99 ms                                                      | 5.64 ms: 1.06x faster                                                  |
| pickle_list                | 4.93 us                                                      | 4.65 us: 1.06x faster                                                  |
| python_startup_no_site     | 7.39 ms                                                      | 7.00 ms: 1.06x faster                                                  |
| float                      | 77.5 ms                                                      | 73.4 ms: 1.06x faster                                                  |
| deltablue                  | 3.12 ms                                                      | 2.97 ms: 1.05x faster                                                  |
| coroutines                 | 23.6 ms                                                      | 22.4 ms: 1.05x faster                                                  |
| unpickle_list              | 4.71 us                                                      | 4.48 us: 1.05x faster                                                  |
| scimark_lu                 | 113 ms                                                       | 108 ms: 1.05x faster                                                   |
| raytrace                   | 253 ms                                                       | 243 ms: 1.04x faster                                                   |
| coverage                   | 83.0 ms                                                      | 79.9 ms: 1.04x faster                                                  |
| sympy_sum                  | 156 ms                                                       | 150 ms: 1.04x faster                                                   |
| json_loads                 | 27.0 us                                                      | 26.0 us: 1.04x faster                                                  |
| typing_runtime_protocols   | 155 us                                                       | 149 us: 1.04x faster                                                   |
| thrift                     | 778 us                                                       | 753 us: 1.03x faster                                                   |
| django_template            | 34.1 ms                                                      | 33.2 ms: 1.03x faster                                                  |
| pycparser                  | 1.12 sec                                                     | 1.09 sec: 1.02x faster                                                 |
| sympy_str                  | 275 ms                                                       | 268 ms: 1.02x faster                                                   |
| asyncio_tcp_ssl            | 1.51 sec                                                     | 1.48 sec: 1.02x faster                                                 |
| xml_etree_iterparse        | 94.9 ms                                                      | 93.1 ms: 1.02x faster                                                  |
| regex_v8                   | 22.7 ms                                                      | 22.3 ms: 1.02x faster                                                  |
| fannkuch                   | 370 ms                                                       | 364 ms: 1.02x faster                                                   |
| sympy_expand               | 457 ms                                                       | 450 ms: 1.02x faster                                                   |
| 2to3                       | 260 ms                                                       | 256 ms: 1.02x faster                                                   |
| sqlite_synth               | 2.21 us                                                      | 2.18 us: 1.01x faster                                                  |
| pprint_safe_repr           | 738 ms                                                       | 729 ms: 1.01x faster                                                   |
| nbody                      | 85.1 ms                                                      | 84.5 ms: 1.01x faster                                                  |
| pprint_pformat             | 1.50 sec                                                     | 1.49 sec: 1.00x faster                                                 |
| xml_etree_generate         | 85.4 ms                                                      | 86.1 ms: 1.01x slower                                                  |
| json                       | 4.93 ms                                                      | 4.97 ms: 1.01x slower                                                  |
| unpickle_pure_python       | 210 us                                                       | 213 us: 1.01x slower                                                   |
| regex_dna                  | 180 ms                                                       | 184 ms: 1.02x slower                                                   |
| meteor_contest             | 102 ms                                                       | 106 ms: 1.04x slower                                                   |
| pickle_pure_python         | 294 us                                                       | 307 us: 1.04x slower                                                   |
| unpack_sequence            | 44.8 ns                                                      | 46.7 ns: 1.04x slower                                                  |
| asyncio_websockets         | 520 ms                                                       | 543 ms: 1.04x slower                                                   |
| crypto_pyaes               | 67.9 ms                                                      | 71.1 ms: 1.05x slower                                                  |
| unpickle                   | 14.3 us                                                      | 15.1 us: 1.06x slower                                                  |
| mako                       | 11.3 ms                                                      | 12.8 ms: 1.13x slower                                                  |
| pickle                     | 11.3 us                                                      | 12.9 us: 1.14x slower                                                  |
| xml_etree_parse            | 136 ms                                                       | 157 ms: 1.15x slower                                                   |
| python_startup             | 11.0 ms                                                      | 12.7 ms: 1.16x slower                                                  |
| bench_thread_pool          | 919 us                                                       | 1.29 ms: 1.41x slower                                                  |
| gc_traversal               | 3.14 ms                                                      | 4.52 ms: 1.44x slower                                                  |
| create_gc_cycles           | 1.34 ms                                                      | 1.94 ms: 1.45x slower                                                  |
| telco                      | 7.82 ms                                                      | 155 ms: 19.85x slower                                                  |
| bench_mp_pool              | 11.0 ms                                                      | 249 ms: 22.61x slower                                                  |
| Geometric mean             | (ref)                                                        | 1.01x faster                                                           |

Benchmark hidden because not significant (3): generators, regex_effbot, xml_etree_process
Ignored benchmarks (12) of results/bm-20240906-3.13.0rc2-ec61006/bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006.json: aiohttp, chameleon, dask, flaskblogging, genshi_text, genshi_xml, gunicorn, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http
Ignored benchmarks (10) of results/bm-20260502-3.15.0a8+-6b632ce-CLANG/bm-20260502-vultr-x86_64-python-6b632ce36b950c7753c5-3.15.0a8+-6b632ce.json: connected_components, k_core, many_optionals, shortest_path, sphinx, sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile, subparsers

- Geometric mean (including insignificant results): 1.050x faster

# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.05x
- 95% likely to have a speedup of 1.04x
- 99% likely to have a speedup of 1.03x

# Memory
- memory change: 1.17x