# Results vs. 3.13.0rc2

- fork: python
- ref: e2118b0ac21191bfeecd
- machine: darwin-arm64
- commit hash: e2118b0
- commit date: 2026-08-25
- overall geometric mean: 1.058x slower
- HPT reliability: 99.47%
- HPT 99th percentile: 1.00x slower
- Memory change: 1.27x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-macm4pro-arm64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260825-macm4pro-arm64-python-e2118b0ac21191bfeecd-3.16.0a0-e2118b0 |
|----------------|:--------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| 2to3           | 112 ms                                                         | 134 ms: 1.20x slower                                                    |
| docutils       | 1.05 sec                                                       | 1.09 sec: 1.04x slower                                                  |
| html5lib       | 23.1 ms                                                        | 24.1 ms: 1.04x slower                                                   |
| sphinx         | 409 ms                                                         | 458 ms: 1.12x slower                                                    |
| Geometric mean | (ref)                                                          | 1.10x slower                                                            |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                        | bm-20240906-macm4pro-arm64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260825-macm4pro-arm64-python-e2118b0ac21191bfeecd-3.16.0a0-e2118b0 |
|----------------------------------|:--------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| async_tree_eager_io              | 525 ms                                                         | 309 ms: 1.70x faster                                                    |
| async_tree_eager_io_tg           | 521 ms                                                         | 307 ms: 1.70x faster                                                    |
| async_tree_io_tg                 | 405 ms                                                         | 311 ms: 1.30x faster                                                    |
| async_tree_io                    | 386 ms                                                         | 316 ms: 1.22x faster                                                    |
| async_generators                 | 193 ms                                                         | 166 ms: 1.17x faster                                                    |
| asyncio_websockets               | 194 ms                                                         | 184 ms: 1.05x faster                                                    |
| async_tree_none                  | 142 ms                                                         | 146 ms: 1.03x slower                                                    |
| async_tree_memoization           | 184 ms                                                         | 190 ms: 1.03x slower                                                    |
| async_tree_cpu_io_mixed          | 294 ms                                                         | 305 ms: 1.04x slower                                                    |
| async_tree_eager_memoization     | 122 ms                                                         | 127 ms: 1.04x slower                                                    |
| async_tree_eager_cpu_io_mixed    | 225 ms                                                         | 237 ms: 1.05x slower                                                    |
| async_tree_none_tg               | 133 ms                                                         | 146 ms: 1.10x slower                                                    |
| coroutines                       | 10.8 ms                                                        | 12.8 ms: 1.19x slower                                                   |
| async_tree_eager                 | 43.2 ms                                                        | 52.1 ms: 1.21x slower                                                   |
| async_tree_eager_cpu_io_mixed_tg | 208 ms                                                         | 278 ms: 1.33x slower                                                    |
| async_tree_eager_memoization_tg  | 103 ms                                                         | 158 ms: 1.54x slower                                                    |
| async_tree_eager_tg              | 28.9 ms                                                        | 118 ms: 4.08x slower                                                    |
| Geometric mean                   | (ref)                                                          | 1.06x slower                                                            |

Benchmark hidden because not significant (2): async_tree_cpu_io_mixed_tg, async_tree_memoization_tg

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-macm4pro-arm64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260825-macm4pro-arm64-python-e2118b0ac21191bfeecd-3.16.0a0-e2118b0 |
|----------------|:--------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| pidigits       | 166 ms                                                         | 163 ms: 1.02x faster                                                    |
| float          | 31.4 ms                                                        | 35.1 ms: 1.12x slower                                                   |
| nbody          | 42.5 ms                                                        | 64.2 ms: 1.51x slower                                                   |
| Geometric mean | (ref)                                                          | 1.18x slower                                                            |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-macm4pro-arm64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260825-macm4pro-arm64-python-e2118b0ac21191bfeecd-3.16.0a0-e2118b0 |
|----------------|:--------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| regex_v8       | 10.7 ms                                                        | 9.08 ms: 1.18x faster                                                   |
| regex_effbot   | 1.61 ms                                                        | 1.39 ms: 1.16x faster                                                   |
| regex_dna      | 94.6 ms                                                        | 91.6 ms: 1.03x faster                                                   |
| regex_compile  | 47.9 ms                                                        | 68.2 ms: 1.42x slower                                                   |
| Geometric mean | (ref)                                                          | 1.00x slower                                                            |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-macm4pro-arm64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260825-macm4pro-arm64-python-e2118b0ac21191bfeecd-3.16.0a0-e2118b0 |
|----------------------|:--------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| json_dumps           | 4.65 ms                                                        | 3.72 ms: 1.25x faster                                                   |
| xml_etree_iterparse  | 46.1 ms                                                        | 42.2 ms: 1.09x faster                                                   |
| tomli_loads          | 1000 ms                                                        | 941 ms: 1.06x faster                                                    |
| xml_etree_parse      | 62.4 ms                                                        | 59.5 ms: 1.05x faster                                                   |
| json_loads           | 10.8 us                                                        | 11.0 us: 1.02x slower                                                   |
| xml_etree_generate   | 35.8 ms                                                        | 39.5 ms: 1.10x slower                                                   |
| unpickle_pure_python | 99.5 us                                                        | 119 us: 1.20x slower                                                    |
| xml_etree_process    | 25.4 ms                                                        | 31.7 ms: 1.25x slower                                                   |
| pickle_pure_python   | 130 us                                                         | 165 us: 1.27x slower                                                    |
| Geometric mean       | (ref)                                                          | 1.04x slower                                                            |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240906-macm4pro-arm64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260825-macm4pro-arm64-python-e2118b0ac21191bfeecd-3.16.0a0-e2118b0 |
|------------------------|:--------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| python_startup         | 8.63 ms                                                        | 10.2 ms: 1.18x slower                                                   |
| python_startup_no_site | 5.95 ms                                                        | 7.60 ms: 1.28x slower                                                   |
| Geometric mean         | (ref)                                                          | 1.23x slower                                                            |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240906-macm4pro-arm64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260825-macm4pro-arm64-python-e2118b0ac21191bfeecd-3.16.0a0-e2118b0 |
|-----------------|:--------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| mako            | 4.41 ms                                                        | 5.97 ms: 1.35x slower                                                   |
| django_template | 12.5 ms                                                        | 17.7 ms: 1.42x slower                                                   |
| Geometric mean  | (ref)                                                          | 1.39x slower                                                            |

All benchmarks:
===============

| Benchmark                        | bm-20240906-macm4pro-arm64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260825-macm4pro-arm64-python-e2118b0ac21191bfeecd-3.16.0a0-e2118b0 |
|----------------------------------|:--------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| gc_traversal                     | 2.04 ms                                                        | 764 us: 2.67x faster                                                    |
| create_gc_cycles                 | 993 us                                                         | 485 us: 2.05x faster                                                    |
| pylint                           | 106 ms                                                         | 54.1 ms: 1.95x faster                                                   |
| mdp                              | 1.06 sec                                                       | 618 ms: 1.71x faster                                                    |
| async_tree_eager_io              | 525 ms                                                         | 309 ms: 1.70x faster                                                    |
| async_tree_eager_io_tg           | 521 ms                                                         | 307 ms: 1.70x faster                                                    |
| k_core                           | 1.46 sec                                                       | 1.01 sec: 1.45x faster                                                  |
| subparsers                       | 6.26 ms                                                        | 4.52 ms: 1.38x faster                                                   |
| async_tree_io_tg                 | 405 ms                                                         | 311 ms: 1.30x faster                                                    |
| json_dumps                       | 4.65 ms                                                        | 3.72 ms: 1.25x faster                                                   |
| deepcopy                         | 145 us                                                         | 118 us: 1.23x faster                                                    |
| async_tree_io                    | 386 ms                                                         | 316 ms: 1.22x faster                                                    |
| regex_v8                         | 10.7 ms                                                        | 9.08 ms: 1.18x faster                                                   |
| async_generators                 | 193 ms                                                         | 166 ms: 1.17x faster                                                    |
| regex_effbot                     | 1.61 ms                                                        | 1.39 ms: 1.16x faster                                                   |
| sqlite_synth                     | 948 ns                                                         | 819 ns: 1.16x faster                                                    |
| xml_etree_iterparse              | 46.1 ms                                                        | 42.2 ms: 1.09x faster                                                   |
| go                               | 72.6 ms                                                        | 66.6 ms: 1.09x faster                                                   |
| bpe_tokeniser                    | 2.13 sec                                                       | 1.96 sec: 1.09x faster                                                  |
| deepcopy_memo                    | 16.5 us                                                        | 15.4 us: 1.07x faster                                                   |
| typing_runtime_protocols         | 64.6 us                                                        | 60.6 us: 1.07x faster                                                   |
| tomli_loads                      | 1000 ms                                                        | 941 ms: 1.06x faster                                                    |
| pyflate                          | 222 ms                                                         | 211 ms: 1.05x faster                                                    |
| asyncio_websockets               | 194 ms                                                         | 184 ms: 1.05x faster                                                    |
| xml_etree_parse                  | 62.4 ms                                                        | 59.5 ms: 1.05x faster                                                   |
| scimark_sor                      | 64.0 ms                                                        | 61.2 ms: 1.05x faster                                                   |
| deepcopy_reduce                  | 1.30 us                                                        | 1.25 us: 1.03x faster                                                   |
| regex_dna                        | 94.6 ms                                                        | 91.6 ms: 1.03x faster                                                   |
| pathlib                          | 11.1 ms                                                        | 10.9 ms: 1.02x faster                                                   |
| pidigits                         | 166 ms                                                         | 163 ms: 1.02x faster                                                    |
| telco                            | 3.07 ms                                                        | 3.10 ms: 1.01x slower                                                   |
| json_loads                       | 10.8 us                                                        | 11.0 us: 1.02x slower                                                   |
| async_tree_none                  | 142 ms                                                         | 146 ms: 1.03x slower                                                    |
| async_tree_memoization           | 184 ms                                                         | 190 ms: 1.03x slower                                                    |
| async_tree_cpu_io_mixed          | 294 ms                                                         | 305 ms: 1.04x slower                                                    |
| html5lib                         | 23.1 ms                                                        | 24.1 ms: 1.04x slower                                                   |
| docutils                         | 1.05 sec                                                       | 1.09 sec: 1.04x slower                                                  |
| async_tree_eager_memoization     | 122 ms                                                         | 127 ms: 1.04x slower                                                    |
| async_tree_eager_cpu_io_mixed    | 225 ms                                                         | 237 ms: 1.05x slower                                                    |
| fannkuch                         | 179 ms                                                         | 192 ms: 1.07x slower                                                    |
| pycparser                        | 470 ms                                                         | 507 ms: 1.08x slower                                                    |
| async_tree_none_tg               | 133 ms                                                         | 146 ms: 1.10x slower                                                    |
| scimark_fft                      | 124 ms                                                         | 136 ms: 1.10x slower                                                    |
| xml_etree_generate               | 35.8 ms                                                        | 39.5 ms: 1.10x slower                                                   |
| float                            | 31.4 ms                                                        | 35.1 ms: 1.12x slower                                                   |
| sphinx                           | 409 ms                                                         | 458 ms: 1.12x slower                                                    |
| meteor_contest                   | 47.9 ms                                                        | 54.0 ms: 1.13x slower                                                   |
| shortest_path                    | 225 ms                                                         | 254 ms: 1.13x slower                                                    |
| sympy_integrate                  | 7.53 ms                                                        | 8.60 ms: 1.14x slower                                                   |
| pprint_safe_repr                 | 322 ms                                                         | 370 ms: 1.15x slower                                                    |
| bench_mp_pool                    | 37.8 ms                                                        | 43.5 ms: 1.15x slower                                                   |
| thrift                           | 309 us                                                         | 357 us: 1.16x slower                                                    |
| pprint_pformat                   | 650 ms                                                         | 761 ms: 1.17x slower                                                    |
| python_startup                   | 8.63 ms                                                        | 10.2 ms: 1.18x slower                                                   |
| connected_components             | 208 ms                                                         | 245 ms: 1.18x slower                                                    |
| logging_simple                   | 2.24 us                                                        | 2.65 us: 1.18x slower                                                   |
| nqueens                          | 37.2 ms                                                        | 44.1 ms: 1.18x slower                                                   |
| logging_format                   | 2.45 us                                                        | 2.90 us: 1.19x slower                                                   |
| coroutines                       | 10.8 ms                                                        | 12.8 ms: 1.19x slower                                                   |
| scimark_monte_carlo              | 29.9 ms                                                        | 35.6 ms: 1.19x slower                                                   |
| unpickle_pure_python             | 99.5 us                                                        | 119 us: 1.20x slower                                                    |
| sympy_sum                        | 52.3 ms                                                        | 62.7 ms: 1.20x slower                                                   |
| spectral_norm                    | 43.7 ms                                                        | 52.5 ms: 1.20x slower                                                   |
| 2to3                             | 112 ms                                                         | 134 ms: 1.20x slower                                                    |
| async_tree_eager                 | 43.2 ms                                                        | 52.1 ms: 1.21x slower                                                   |
| sympy_str                        | 95.5 ms                                                        | 116 ms: 1.21x slower                                                    |
| richards                         | 22.1 ms                                                        | 26.7 ms: 1.21x slower                                                   |
| richards_super                   | 24.7 ms                                                        | 30.2 ms: 1.22x slower                                                   |
| sympy_expand                     | 159 ms                                                         | 196 ms: 1.23x slower                                                    |
| hexiom                           | 2.85 ms                                                        | 3.51 ms: 1.23x slower                                                   |
| scimark_sparse_mat_mult          | 1.78 ms                                                        | 2.20 ms: 1.24x slower                                                   |
| logging_silent                   | 40.6 ns                                                        | 50.7 ns: 1.25x slower                                                   |
| xml_etree_process                | 25.4 ms                                                        | 31.7 ms: 1.25x slower                                                   |
| crypto_pyaes                     | 33.6 ms                                                        | 42.3 ms: 1.26x slower                                                   |
| pickle_pure_python               | 130 us                                                         | 165 us: 1.27x slower                                                    |
| python_startup_no_site           | 5.95 ms                                                        | 7.60 ms: 1.28x slower                                                   |
| comprehensions                   | 6.80 us                                                        | 8.80 us: 1.29x slower                                                   |
| chaos                            | 24.3 ms                                                        | 31.5 ms: 1.30x slower                                                   |
| coverage                         | 31.2 ms                                                        | 41.1 ms: 1.32x slower                                                   |
| async_tree_eager_cpu_io_mixed_tg | 208 ms                                                         | 278 ms: 1.33x slower                                                    |
| many_optionals                   | 200 us                                                         | 268 us: 1.34x slower                                                    |
| mako                             | 4.41 ms                                                        | 5.97 ms: 1.35x slower                                                   |
| bench_thread_pool                | 412 us                                                         | 577 us: 1.40x slower                                                    |
| django_template                  | 12.5 ms                                                        | 17.7 ms: 1.42x slower                                                   |
| regex_compile                    | 47.9 ms                                                        | 68.2 ms: 1.42x slower                                                   |
| raytrace                         | 109 ms                                                         | 158 ms: 1.45x slower                                                    |
| deltablue                        | 1.45 ms                                                        | 2.18 ms: 1.50x slower                                                   |
| nbody                            | 42.5 ms                                                        | 64.2 ms: 1.51x slower                                                   |
| async_tree_eager_memoization_tg  | 103 ms                                                         | 158 ms: 1.54x slower                                                    |
| generators                       | 15.7 ms                                                        | 24.3 ms: 1.54x slower                                                   |
| scimark_lu                       | 42.8 ms                                                        | 74.5 ms: 1.74x slower                                                   |
| async_tree_eager_tg              | 28.9 ms                                                        | 118 ms: 4.08x slower                                                    |
| Geometric mean                   | (ref)                                                          | 1.07x slower                                                            |

Benchmark hidden because not significant (4): async_tree_cpu_io_mixed_tg, async_tree_memoization_tg, dulwich_log, json
Ignored benchmarks (13) of results/bm-20240906-3.13.0rc2-ec61006/bm-20240906-macm4pro-arm64-python-v3.13.0rc2-3.13.0rc2-ec61006.json: chameleon, dask, djangocms, genshi_text, genshi_xml, gevent_hub, sqlalchemy_declarative, sqlalchemy_imperative, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http
Ignored benchmarks (4) of results/bm-20260825-3.16.0a0-e2118b0-NOGIL/bm-20260825-macm4pro-arm64-python-e2118b0ac21191bfeecd-3.16.0a0-e2118b0.json: sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile

- Geometric mean (including insignificant results): 1.058x slower

# HPT report

- Reliability score: 99.47% likely to be slow
- 90% likely to have a slowdown of 1.04x
- 95% likely to have a slowdown of 1.03x
- 99% likely to have a slowdown of 1.00x

# Memory
- memory change: 1.27x