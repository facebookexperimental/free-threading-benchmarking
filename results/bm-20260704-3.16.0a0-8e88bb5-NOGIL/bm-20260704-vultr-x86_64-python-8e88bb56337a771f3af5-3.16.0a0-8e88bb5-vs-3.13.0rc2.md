# Results vs. 3.13.0rc2

- fork: python
- ref: 8e88bb56337a771f3af5
- machine: linux-x86_64
- commit hash: 8e88bb5
- commit date: 2026-07-04
- overall geometric mean: 1.068x slower
- HPT reliability: 98.23%
- HPT 99th percentile: 1.00x slower
- Memory change: 1.36x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5 | bm-20260704-vultr-x86_64-python-8e88bb56337a771f3af5-3.16.0a0-8e88bb5 |
|----------------|:----------------------------------------------------------------------:|:---------------------------------------------------------------------:|
| 2to3           | 259 ms                                                                 | 294 ms: 1.13x slower                                                  |
| docutils       | 2.63 sec                                                               | 2.96 sec: 1.12x slower                                                |
| html5lib       | 68.6 ms                                                                | 67.7 ms: 1.01x faster                                                 |
| Geometric mean | (ref)                                                                  | 1.08x slower                                                          |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5 | bm-20260704-vultr-x86_64-python-8e88bb56337a771f3af5-3.16.0a0-8e88bb5 |
|----------------------------|:----------------------------------------------------------------------:|:---------------------------------------------------------------------:|
| async_tree_io_tg           | 901 ms                                                                 | 686 ms: 1.31x faster                                                  |
| async_tree_io              | 881 ms                                                                 | 703 ms: 1.25x faster                                                  |
| async_tree_cpu_io_mixed    | 666 ms                                                                 | 603 ms: 1.10x faster                                                  |
| asyncio_tcp                | 502 ms                                                                 | 456 ms: 1.10x faster                                                  |
| async_tree_memoization     | 459 ms                                                                 | 419 ms: 1.10x faster                                                  |
| async_tree_cpu_io_mixed_tg | 634 ms                                                                 | 580 ms: 1.09x faster                                                  |
| async_tree_none            | 353 ms                                                                 | 339 ms: 1.04x faster                                                  |
| async_tree_memoization_tg  | 410 ms                                                                 | 395 ms: 1.04x faster                                                  |
| async_tree_none_tg         | 333 ms                                                                 | 322 ms: 1.03x faster                                                  |
| asyncio_websockets         | 517 ms                                                                 | 513 ms: 1.01x faster                                                  |
| async_generators           | 375 ms                                                                 | 386 ms: 1.03x slower                                                  |
| coroutines                 | 23.3 ms                                                                | 24.6 ms: 1.06x slower                                                 |
| asyncio_tcp_ssl            | 1.51 sec                                                               | 1.70 sec: 1.12x slower                                                |
| Geometric mean             | (ref)                                                                  | 1.06x faster                                                          |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5 | bm-20260704-vultr-x86_64-python-8e88bb56337a771f3af5-3.16.0a0-8e88bb5 |
|----------------|:----------------------------------------------------------------------:|:---------------------------------------------------------------------:|
| pidigits       | 216 ms                                                                 | 184 ms: 1.18x faster                                                  |
| float          | 76.7 ms                                                                | 85.6 ms: 1.12x slower                                                 |
| nbody          | 85.3 ms                                                                | 130 ms: 1.52x slower                                                  |
| Geometric mean | (ref)                                                                  | 1.13x slower                                                          |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5 | bm-20260704-vultr-x86_64-python-8e88bb56337a771f3af5-3.16.0a0-8e88bb5 |
|----------------|:----------------------------------------------------------------------:|:---------------------------------------------------------------------:|
| regex_effbot   | 3.21 ms                                                                | 2.81 ms: 1.14x faster                                                 |
| regex_v8       | 23.2 ms                                                                | 20.5 ms: 1.13x faster                                                 |
| regex_dna      | 189 ms                                                                 | 177 ms: 1.07x faster                                                  |
| regex_compile  | 131 ms                                                                 | 167 ms: 1.27x slower                                                  |
| Geometric mean | (ref)                                                                  | 1.02x faster                                                          |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5 | bm-20260704-vultr-x86_64-python-8e88bb56337a771f3af5-3.16.0a0-8e88bb5 |
|----------------------|:----------------------------------------------------------------------:|:---------------------------------------------------------------------:|
| tomli_loads          | 2.01 sec                                                               | 1.96 sec: 1.03x faster                                                |
| json_dumps           | 10.6 ms                                                                | 10.5 ms: 1.01x faster                                                 |
| xml_etree_iterparse  | 94.4 ms                                                                | 95.4 ms: 1.01x slower                                                 |
| xml_etree_parse      | 136 ms                                                                 | 141 ms: 1.03x slower                                                  |
| pickle_dict          | 32.7 us                                                                | 34.9 us: 1.07x slower                                                 |
| json_loads           | 27.3 us                                                                | 29.6 us: 1.08x slower                                                 |
| unpickle_pure_python | 208 us                                                                 | 237 us: 1.14x slower                                                  |
| unpickle             | 14.6 us                                                                | 16.8 us: 1.15x slower                                                 |
| pickle_pure_python   | 292 us                                                                 | 338 us: 1.16x slower                                                  |
| pickle               | 11.4 us                                                                | 13.4 us: 1.18x slower                                                 |
| xml_etree_generate   | 85.4 ms                                                                | 101 ms: 1.18x slower                                                  |
| pickle_list          | 4.81 us                                                                | 5.69 us: 1.18x slower                                                 |
| unpickle_list        | 4.60 us                                                                | 5.67 us: 1.23x slower                                                 |
| xml_etree_process    | 59.2 ms                                                                | 74.1 ms: 1.25x slower                                                 |
| Geometric mean       | (ref)                                                                  | 1.11x slower                                                          |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5 | bm-20260704-vultr-x86_64-python-8e88bb56337a771f3af5-3.16.0a0-8e88bb5 |
|------------------------|:----------------------------------------------------------------------:|:---------------------------------------------------------------------:|
| python_startup_no_site | 7.41 ms                                                                | 9.18 ms: 1.24x slower                                                 |
| python_startup         | 11.0 ms                                                                | 15.4 ms: 1.40x slower                                                 |
| Geometric mean         | (ref)                                                                  | 1.31x slower                                                          |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5 | bm-20260704-vultr-x86_64-python-8e88bb56337a771f3af5-3.16.0a0-8e88bb5 |
|-----------------|:----------------------------------------------------------------------:|:---------------------------------------------------------------------:|
| django_template | 34.2 ms                                                                | 40.0 ms: 1.17x slower                                                 |
| mako            | 11.2 ms                                                                | 16.1 ms: 1.43x slower                                                 |
| Geometric mean  | (ref)                                                                  | 1.30x slower                                                          |

All benchmarks:
===============

| Benchmark                  | bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5 | bm-20260704-vultr-x86_64-python-8e88bb56337a771f3af5-3.16.0a0-8e88bb5 |
|----------------------------|:----------------------------------------------------------------------:|:---------------------------------------------------------------------:|
| pylint                     | 317 ms                                                                 | 131 ms: 2.42x faster                                                  |
| gc_traversal               | 3.32 ms                                                                | 1.77 ms: 1.88x faster                                                 |
| mdp                        | 2.34 sec                                                               | 1.30 sec: 1.81x faster                                                |
| bench_mp_pool              | 11.0 ms                                                                | 6.92 ms: 1.59x faster                                                 |
| deepcopy                   | 357 us                                                                 | 264 us: 1.35x faster                                                  |
| async_tree_io_tg           | 901 ms                                                                 | 686 ms: 1.31x faster                                                  |
| async_tree_io              | 881 ms                                                                 | 703 ms: 1.25x faster                                                  |
| deepcopy_memo              | 38.1 us                                                                | 30.6 us: 1.24x faster                                                 |
| pidigits                   | 216 ms                                                                 | 184 ms: 1.18x faster                                                  |
| go                         | 141 ms                                                                 | 120 ms: 1.17x faster                                                  |
| sqlite_synth               | 2.25 us                                                                | 1.95 us: 1.15x faster                                                 |
| regex_effbot               | 3.21 ms                                                                | 2.81 ms: 1.14x faster                                                 |
| regex_v8                   | 23.2 ms                                                                | 20.5 ms: 1.13x faster                                                 |
| async_tree_cpu_io_mixed    | 666 ms                                                                 | 603 ms: 1.10x faster                                                  |
| asyncio_tcp                | 502 ms                                                                 | 456 ms: 1.10x faster                                                  |
| pathlib                    | 19.3 ms                                                                | 17.5 ms: 1.10x faster                                                 |
| async_tree_memoization     | 459 ms                                                                 | 419 ms: 1.10x faster                                                  |
| async_tree_cpu_io_mixed_tg | 634 ms                                                                 | 580 ms: 1.09x faster                                                  |
| deepcopy_reduce            | 3.12 us                                                                | 2.86 us: 1.09x faster                                                 |
| scimark_sor                | 134 ms                                                                 | 124 ms: 1.08x faster                                                  |
| typing_runtime_protocols   | 156 us                                                                 | 145 us: 1.07x faster                                                  |
| regex_dna                  | 189 ms                                                                 | 177 ms: 1.07x faster                                                  |
| dulwich_log                | 74.5 ms                                                                | 70.2 ms: 1.06x faster                                                 |
| async_tree_none            | 353 ms                                                                 | 339 ms: 1.04x faster                                                  |
| async_tree_memoization_tg  | 410 ms                                                                 | 395 ms: 1.04x faster                                                  |
| async_tree_none_tg         | 333 ms                                                                 | 322 ms: 1.03x faster                                                  |
| bpe_tokeniser              | 4.46 sec                                                               | 4.33 sec: 1.03x faster                                                |
| tomli_loads                | 2.01 sec                                                               | 1.96 sec: 1.03x faster                                                |
| html5lib                   | 68.6 ms                                                                | 67.7 ms: 1.01x faster                                                 |
| json_dumps                 | 10.6 ms                                                                | 10.5 ms: 1.01x faster                                                 |
| asyncio_websockets         | 517 ms                                                                 | 513 ms: 1.01x faster                                                  |
| xml_etree_iterparse        | 94.4 ms                                                                | 95.4 ms: 1.01x slower                                                 |
| scimark_fft                | 348 ms                                                                 | 354 ms: 1.02x slower                                                  |
| create_gc_cycles           | 1.33 ms                                                                | 1.36 ms: 1.02x slower                                                 |
| async_generators           | 375 ms                                                                 | 386 ms: 1.03x slower                                                  |
| xml_etree_parse            | 136 ms                                                                 | 141 ms: 1.03x slower                                                  |
| pycparser                  | 1.12 sec                                                               | 1.16 sec: 1.03x slower                                                |
| comprehensions             | 16.6 us                                                                | 17.3 us: 1.04x slower                                                 |
| logging_silent             | 98.2 ns                                                                | 103 ns: 1.04x slower                                                  |
| pyflate                    | 449 ms                                                                 | 470 ms: 1.05x slower                                                  |
| spectral_norm              | 108 ms                                                                 | 113 ms: 1.05x slower                                                  |
| json                       | 4.98 ms                                                                | 5.24 ms: 1.05x slower                                                 |
| coroutines                 | 23.3 ms                                                                | 24.6 ms: 1.06x slower                                                 |
| pickle_dict                | 32.7 us                                                                | 34.9 us: 1.07x slower                                                 |
| json_loads                 | 27.3 us                                                                | 29.6 us: 1.08x slower                                                 |
| hexiom                     | 5.95 ms                                                                | 6.47 ms: 1.09x slower                                                 |
| sympy_integrate            | 19.7 ms                                                                | 21.7 ms: 1.10x slower                                                 |
| logging_simple             | 6.14 us                                                                | 6.83 us: 1.11x slower                                                 |
| float                      | 76.7 ms                                                                | 85.6 ms: 1.12x slower                                                 |
| chaos                      | 56.3 ms                                                                | 63.0 ms: 1.12x slower                                                 |
| asyncio_tcp_ssl            | 1.51 sec                                                               | 1.70 sec: 1.12x slower                                                |
| pprint_safe_repr           | 719 ms                                                                 | 806 ms: 1.12x slower                                                  |
| logging_format             | 6.92 us                                                                | 7.77 us: 1.12x slower                                                 |
| docutils                   | 2.63 sec                                                               | 2.96 sec: 1.12x slower                                                |
| nqueens                    | 77.7 ms                                                                | 88.1 ms: 1.13x slower                                                 |
| 2to3                       | 259 ms                                                                 | 294 ms: 1.13x slower                                                  |
| sympy_str                  | 274 ms                                                                 | 312 ms: 1.14x slower                                                  |
| unpickle_pure_python       | 208 us                                                                 | 237 us: 1.14x slower                                                  |
| generators                 | 28.5 ms                                                                | 32.5 ms: 1.14x slower                                                 |
| scimark_lu                 | 112 ms                                                                 | 129 ms: 1.14x slower                                                  |
| sympy_expand               | 454 ms                                                                 | 521 ms: 1.15x slower                                                  |
| pprint_pformat             | 1.46 sec                                                               | 1.67 sec: 1.15x slower                                                |
| unpickle                   | 14.6 us                                                                | 16.8 us: 1.15x slower                                                 |
| pickle_pure_python         | 292 us                                                                 | 338 us: 1.16x slower                                                  |
| scimark_sparse_mat_mult    | 4.75 ms                                                                | 5.53 ms: 1.16x slower                                                 |
| sympy_sum                  | 154 ms                                                                 | 180 ms: 1.17x slower                                                  |
| django_template            | 34.2 ms                                                                | 40.0 ms: 1.17x slower                                                 |
| deltablue                  | 3.10 ms                                                                | 3.64 ms: 1.18x slower                                                 |
| pickle                     | 11.4 us                                                                | 13.4 us: 1.18x slower                                                 |
| xml_etree_generate         | 85.4 ms                                                                | 101 ms: 1.18x slower                                                  |
| unpack_sequence            | 39.3 ns                                                                | 46.3 ns: 1.18x slower                                                 |
| pickle_list                | 4.81 us                                                                | 5.69 us: 1.18x slower                                                 |
| thrift                     | 772 us                                                                 | 916 us: 1.19x slower                                                  |
| richards                   | 44.4 ms                                                                | 53.1 ms: 1.20x slower                                                 |
| richards_super             | 50.4 ms                                                                | 60.6 ms: 1.20x slower                                                 |
| fannkuch                   | 376 ms                                                                 | 457 ms: 1.22x slower                                                  |
| scimark_monte_carlo        | 65.8 ms                                                                | 80.2 ms: 1.22x slower                                                 |
| raytrace                   | 250 ms                                                                 | 305 ms: 1.22x slower                                                  |
| unpickle_list              | 4.60 us                                                                | 5.67 us: 1.23x slower                                                 |
| python_startup_no_site     | 7.41 ms                                                                | 9.18 ms: 1.24x slower                                                 |
| xml_etree_process          | 59.2 ms                                                                | 74.1 ms: 1.25x slower                                                 |
| meteor_contest             | 101 ms                                                                 | 128 ms: 1.26x slower                                                  |
| regex_compile              | 131 ms                                                                 | 167 ms: 1.27x slower                                                  |
| crypto_pyaes               | 68.2 ms                                                                | 89.9 ms: 1.32x slower                                                 |
| coverage                   | 82.5 ms                                                                | 114 ms: 1.38x slower                                                  |
| python_startup             | 11.0 ms                                                                | 15.4 ms: 1.40x slower                                                 |
| mako                       | 11.2 ms                                                                | 16.1 ms: 1.43x slower                                                 |
| nbody                      | 85.3 ms                                                                | 130 ms: 1.52x slower                                                  |
| bench_thread_pool          | 924 us                                                                 | 1.47 ms: 1.59x slower                                                 |
| telco                      | 7.77 ms                                                                | 176 ms: 22.65x slower                                                 |
| Geometric mean             | (ref)                                                                  | 1.07x slower                                                          |
Ignored benchmarks (12) of results/bm-20240920-3.13.0rc2-4981ec5/bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5.json: aiohttp, chameleon, dask, flaskblogging, genshi_text, genshi_xml, gunicorn, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http
Ignored benchmarks (10) of results/bm-20260704-3.16.0a0-8e88bb5-NOGIL/bm-20260704-vultr-x86_64-python-8e88bb56337a771f3af5-3.16.0a0-8e88bb5.json: connected_components, k_core, many_optionals, shortest_path, sphinx, sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile, subparsers

- Geometric mean (including insignificant results): 1.068x slower

# HPT report

- Reliability score: 98.23% likely to be slow
- 90% likely to have a slowdown of 1.02x
- 95% likely to have a slowdown of 1.01x
- 99% likely to have a slowdown of 1.00x

# Memory
- memory change: 1.36x