# Results vs. 3.12.6

- fork: python
- ref: cb3b4b98d8d141c9de04
- machine: darwin-arm64
- commit hash: cb3b4b9
- commit date: 2026-05-20
- overall geometric mean: 1.099x faster
- HPT reliability: 99.58%
- HPT 99th percentile: 1.00x faster
- Memory change: 1.34x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260520-macm4pro-arm64-python-cb3b4b98d8d141c9de04-3.16.0a0-cb3b4b9 |
|----------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| docutils       | 1.02 sec                                                 | 1.06 sec: 1.04x slower                                                  |
| html5lib       | 23.0 ms                                                  | 23.2 ms: 1.01x slower                                                   |
| sphinx         | 434 ms                                                   | 444 ms: 1.03x slower                                                    |
| Geometric mean | (ref)                                                    | 1.02x slower                                                            |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                        | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260520-macm4pro-arm64-python-cb3b4b98d8d141c9de04-3.16.0a0-cb3b4b9 |
|----------------------------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| async_tree_eager_io              | 496 ms                                                   | 282 ms: 1.76x faster                                                    |
| async_tree_io                    | 459 ms                                                   | 273 ms: 1.69x faster                                                    |
| async_tree_io_tg                 | 480 ms                                                   | 290 ms: 1.66x faster                                                    |
| async_tree_eager_io_tg           | 446 ms                                                   | 277 ms: 1.61x faster                                                    |
| async_generators                 | 206 ms                                                   | 151 ms: 1.37x faster                                                    |
| async_tree_memoization_tg        | 231 ms                                                   | 173 ms: 1.33x faster                                                    |
| async_tree_none                  | 178 ms                                                   | 134 ms: 1.33x faster                                                    |
| async_tree_none_tg               | 172 ms                                                   | 133 ms: 1.30x faster                                                    |
| coroutines                       | 13.6 ms                                                  | 10.9 ms: 1.25x faster                                                   |
| async_tree_memoization           | 223 ms                                                   | 180 ms: 1.24x faster                                                    |
| async_tree_cpu_io_mixed_tg       | 338 ms                                                   | 281 ms: 1.21x faster                                                    |
| async_tree_cpu_io_mixed          | 333 ms                                                   | 285 ms: 1.17x faster                                                    |
| async_tree_eager_memoization     | 132 ms                                                   | 119 ms: 1.10x faster                                                    |
| async_tree_eager_cpu_io_mixed    | 231 ms                                                   | 226 ms: 1.02x faster                                                    |
| async_tree_eager                 | 45.6 ms                                                  | 45.7 ms: 1.00x slower                                                   |
| async_tree_eager_cpu_io_mixed_tg | 213 ms                                                   | 267 ms: 1.25x slower                                                    |
| async_tree_eager_memoization_tg  | 113 ms                                                   | 150 ms: 1.33x slower                                                    |
| async_tree_eager_tg              | 32.1 ms                                                  | 105 ms: 3.27x slower                                                    |
| Geometric mean                   | (ref)                                                    | 1.14x faster                                                            |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260520-macm4pro-arm64-python-cb3b4b98d8d141c9de04-3.16.0a0-cb3b4b9 |
|----------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| float          | 37.9 ms                                                  | 32.8 ms: 1.15x faster                                                   |
| nbody          | 54.2 ms                                                  | 56.4 ms: 1.04x slower                                                   |
| Geometric mean | (ref)                                                    | 1.04x faster                                                            |

Benchmark hidden because not significant (1): pidigits

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260520-macm4pro-arm64-python-cb3b4b98d8d141c9de04-3.16.0a0-cb3b4b9 |
|----------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| regex_effbot   | 1.67 ms                                                  | 1.44 ms: 1.16x faster                                                   |
| regex_dna      | 99.6 ms                                                  | 93.7 ms: 1.06x faster                                                   |
| regex_compile  | 54.6 ms                                                  | 51.9 ms: 1.05x faster                                                   |
| regex_v8       | 9.59 ms                                                  | 9.14 ms: 1.05x faster                                                   |
| Geometric mean | (ref)                                                    | 1.08x faster                                                            |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260520-macm4pro-arm64-python-cb3b4b98d8d141c9de04-3.16.0a0-cb3b4b9 |
|----------------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| xml_etree_iterparse  | 51.6 ms                                                  | 41.8 ms: 1.23x faster                                                   |
| xml_etree_parse      | 67.9 ms                                                  | 60.2 ms: 1.13x faster                                                   |
| tomli_loads          | 957 ms                                                   | 855 ms: 1.12x faster                                                    |
| json_dumps           | 4.26 ms                                                  | 3.90 ms: 1.09x faster                                                   |
| xml_etree_generate   | 38.9 ms                                                  | 37.7 ms: 1.03x faster                                                   |
| pickle_pure_python   | 139 us                                                   | 149 us: 1.07x slower                                                    |
| unpickle_pure_python | 103 us                                                   | 110 us: 1.07x slower                                                    |
| xml_etree_process    | 26.7 ms                                                  | 28.6 ms: 1.07x slower                                                   |
| Geometric mean       | (ref)                                                    | 1.04x faster                                                            |

Benchmark hidden because not significant (1): json_loads

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260520-macm4pro-arm64-python-cb3b4b98d8d141c9de04-3.16.0a0-cb3b4b9 |
|-----------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| django_template | 13.6 ms                                                  | 15.7 ms: 1.16x slower                                                   |
| mako            | 4.77 ms                                                  | 5.63 ms: 1.18x slower                                                   |
| Geometric mean  | (ref)                                                    | 1.17x slower                                                            |

All benchmarks:
===============

| Benchmark                        | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260520-macm4pro-arm64-python-cb3b4b98d8d141c9de04-3.16.0a0-cb3b4b9 |
|----------------------------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| subparsers                       | 20.8 ms                                                  | 4.26 ms: 4.88x faster                                                   |
| gc_traversal                     | 2.01 ms                                                  | 790 us: 2.54x faster                                                    |
| pylint                           | 128 ms                                                   | 53.6 ms: 2.39x faster                                                   |
| mdp                              | 1.09 sec                                                 | 584 ms: 1.87x faster                                                    |
| async_tree_eager_io              | 496 ms                                                   | 282 ms: 1.76x faster                                                    |
| async_tree_io                    | 459 ms                                                   | 273 ms: 1.69x faster                                                    |
| async_tree_io_tg                 | 480 ms                                                   | 290 ms: 1.66x faster                                                    |
| create_gc_cycles                 | 830 us                                                   | 514 us: 1.62x faster                                                    |
| async_tree_eager_io_tg           | 446 ms                                                   | 277 ms: 1.61x faster                                                    |
| deepcopy                         | 161 us                                                   | 105 us: 1.54x faster                                                    |
| deepcopy_memo                    | 18.3 us                                                  | 12.7 us: 1.44x faster                                                   |
| async_generators                 | 206 ms                                                   | 151 ms: 1.37x faster                                                    |
| async_tree_memoization_tg        | 231 ms                                                   | 173 ms: 1.33x faster                                                    |
| async_tree_none                  | 178 ms                                                   | 134 ms: 1.33x faster                                                    |
| async_tree_none_tg               | 172 ms                                                   | 133 ms: 1.30x faster                                                    |
| typing_runtime_protocols         | 71.0 us                                                  | 54.8 us: 1.29x faster                                                   |
| deepcopy_reduce                  | 1.46 us                                                  | 1.17 us: 1.25x faster                                                   |
| coroutines                       | 13.6 ms                                                  | 10.9 ms: 1.25x faster                                                   |
| async_tree_memoization           | 223 ms                                                   | 180 ms: 1.24x faster                                                    |
| xml_etree_iterparse              | 51.6 ms                                                  | 41.8 ms: 1.23x faster                                                   |
| sqlite_synth                     | 967 ns                                                   | 789 ns: 1.23x faster                                                    |
| comprehensions                   | 9.84 us                                                  | 8.09 us: 1.22x faster                                                   |
| async_tree_cpu_io_mixed_tg       | 338 ms                                                   | 281 ms: 1.21x faster                                                    |
| logging_silent                   | 50.9 ns                                                  | 42.5 ns: 1.20x faster                                                   |
| bpe_tokeniser                    | 2.24 sec                                                 | 1.89 sec: 1.19x faster                                                  |
| go                               | 70.0 ms                                                  | 59.2 ms: 1.18x faster                                                   |
| async_tree_cpu_io_mixed          | 333 ms                                                   | 285 ms: 1.17x faster                                                    |
| regex_effbot                     | 1.67 ms                                                  | 1.44 ms: 1.16x faster                                                   |
| pathlib                          | 12.4 ms                                                  | 10.7 ms: 1.16x faster                                                   |
| float                            | 37.9 ms                                                  | 32.8 ms: 1.15x faster                                                   |
| dulwich_log                      | 21.3 ms                                                  | 18.5 ms: 1.15x faster                                                   |
| xml_etree_parse                  | 67.9 ms                                                  | 60.2 ms: 1.13x faster                                                   |
| pyflate                          | 216 ms                                                   | 193 ms: 1.12x faster                                                    |
| raytrace                         | 145 ms                                                   | 129 ms: 1.12x faster                                                    |
| k_core                           | 1.12 sec                                                 | 997 ms: 1.12x faster                                                    |
| tomli_loads                      | 957 ms                                                   | 855 ms: 1.12x faster                                                    |
| scimark_fft                      | 142 ms                                                   | 128 ms: 1.11x faster                                                    |
| async_tree_eager_memoization     | 132 ms                                                   | 119 ms: 1.10x faster                                                    |
| scimark_sor                      | 61.0 ms                                                  | 55.6 ms: 1.10x faster                                                   |
| logging_simple                   | 2.57 us                                                  | 2.35 us: 1.09x faster                                                   |
| json_dumps                       | 4.26 ms                                                  | 3.90 ms: 1.09x faster                                                   |
| logging_format                   | 2.80 us                                                  | 2.59 us: 1.08x faster                                                   |
| pycparser                        | 497 ms                                                   | 462 ms: 1.08x faster                                                    |
| spectral_norm                    | 54.4 ms                                                  | 50.6 ms: 1.08x faster                                                   |
| nqueens                          | 43.5 ms                                                  | 40.6 ms: 1.07x faster                                                   |
| chaos                            | 28.9 ms                                                  | 27.2 ms: 1.06x faster                                                   |
| regex_dna                        | 99.6 ms                                                  | 93.7 ms: 1.06x faster                                                   |
| generators                       | 21.9 ms                                                  | 20.8 ms: 1.05x faster                                                   |
| regex_compile                    | 54.6 ms                                                  | 51.9 ms: 1.05x faster                                                   |
| regex_v8                         | 9.59 ms                                                  | 9.14 ms: 1.05x faster                                                   |
| xml_etree_generate               | 38.9 ms                                                  | 37.7 ms: 1.03x faster                                                   |
| fannkuch                         | 176 ms                                                   | 171 ms: 1.03x faster                                                    |
| deltablue                        | 1.73 ms                                                  | 1.68 ms: 1.02x faster                                                   |
| async_tree_eager_cpu_io_mixed    | 231 ms                                                   | 226 ms: 1.02x faster                                                    |
| json                             | 1.93 ms                                                  | 1.89 ms: 1.02x faster                                                   |
| sympy_integrate                  | 8.02 ms                                                  | 8.04 ms: 1.00x slower                                                   |
| async_tree_eager                 | 45.6 ms                                                  | 45.7 ms: 1.00x slower                                                   |
| html5lib                         | 23.0 ms                                                  | 23.2 ms: 1.01x slower                                                   |
| pprint_safe_repr                 | 328 ms                                                   | 334 ms: 1.02x slower                                                    |
| sphinx                           | 434 ms                                                   | 444 ms: 1.03x slower                                                    |
| scimark_monte_carlo              | 32.2 ms                                                  | 33.1 ms: 1.03x slower                                                   |
| pprint_pformat                   | 665 ms                                                   | 685 ms: 1.03x slower                                                    |
| richards                         | 22.4 ms                                                  | 23.2 ms: 1.03x slower                                                   |
| hexiom                           | 3.04 ms                                                  | 3.15 ms: 1.04x slower                                                   |
| docutils                         | 1.02 sec                                                 | 1.06 sec: 1.04x slower                                                  |
| nbody                            | 54.2 ms                                                  | 56.4 ms: 1.04x slower                                                   |
| richards_super                   | 25.4 ms                                                  | 26.4 ms: 1.04x slower                                                   |
| sympy_str                        | 104 ms                                                   | 110 ms: 1.05x slower                                                    |
| sympy_sum                        | 57.6 ms                                                  | 60.8 ms: 1.06x slower                                                   |
| thrift                           | 322 us                                                   | 341 us: 1.06x slower                                                    |
| scimark_sparse_mat_mult          | 2.08 ms                                                  | 2.21 ms: 1.06x slower                                                   |
| pickle_pure_python               | 139 us                                                   | 149 us: 1.07x slower                                                    |
| unpickle_pure_python             | 103 us                                                   | 110 us: 1.07x slower                                                    |
| xml_etree_process                | 26.7 ms                                                  | 28.6 ms: 1.07x slower                                                   |
| sympy_expand                     | 167 ms                                                   | 186 ms: 1.12x slower                                                    |
| crypto_pyaes                     | 38.8 ms                                                  | 44.1 ms: 1.13x slower                                                   |
| shortest_path                    | 219 ms                                                   | 249 ms: 1.14x slower                                                    |
| bench_mp_pool                    | 39.7 ms                                                  | 45.4 ms: 1.14x slower                                                   |
| meteor_contest                   | 47.7 ms                                                  | 54.9 ms: 1.15x slower                                                   |
| django_template                  | 13.6 ms                                                  | 15.7 ms: 1.16x slower                                                   |
| telco                            | 2.61 ms                                                  | 3.06 ms: 1.17x slower                                                   |
| mako                             | 4.77 ms                                                  | 5.63 ms: 1.18x slower                                                   |
| scimark_lu                       | 51.3 ms                                                  | 61.6 ms: 1.20x slower                                                   |
| connected_components             | 201 ms                                                   | 244 ms: 1.21x slower                                                    |
| async_tree_eager_cpu_io_mixed_tg | 213 ms                                                   | 267 ms: 1.25x slower                                                    |
| many_optionals                   | 195 us                                                   | 258 us: 1.32x slower                                                    |
| async_tree_eager_memoization_tg  | 113 ms                                                   | 150 ms: 1.33x slower                                                    |
| bench_thread_pool                | 419 us                                                   | 566 us: 1.35x slower                                                    |
| coverage                         | 26.9 ms                                                  | 38.9 ms: 1.45x slower                                                   |
| async_tree_eager_tg              | 32.1 ms                                                  | 105 ms: 3.27x slower                                                    |
| Geometric mean                   | (ref)                                                    | 1.09x faster                                                            |

Benchmark hidden because not significant (3): pidigits, dask, json_loads
Ignored benchmarks (16) of results/bm-20240906-3.12.6-a4a2d2b/bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b.json: 2to3, asyncio_websockets, chameleon, djangocms, genshi_text, genshi_xml, gevent_hub, python_startup, python_startup_no_site, sqlalchemy_declarative, sqlalchemy_imperative, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http
Ignored benchmarks (4) of results/bm-20260520-3.16.0a0-cb3b4b9-NOGIL/bm-20260520-macm4pro-arm64-python-cb3b4b98d8d141c9de04-3.16.0a0-cb3b4b9.json: sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile

- Geometric mean (including insignificant results): 1.099x faster

# HPT report

- Reliability score: 99.58% likely to be faster
- 90% likely to have a speedup of 1.03x
- 95% likely to have a speedup of 1.02x
- 99% likely to have a speedup of 1.00x

# Memory
- memory change: 1.34x