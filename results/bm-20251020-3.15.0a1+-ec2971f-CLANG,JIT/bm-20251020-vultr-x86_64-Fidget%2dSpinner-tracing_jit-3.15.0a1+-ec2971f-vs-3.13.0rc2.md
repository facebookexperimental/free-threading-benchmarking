# Results vs. 3.13.0rc2

- fork: Fidget-Spinner
- ref: tracing_jit
- machine: linux-x86_64
- commit hash: ec2971f
- commit date: 2025-10-20
- overall geometric mean: 1.079x faster
- HPT reliability: 100.00%
- HPT 99th percentile: 1.03x faster
- Memory change: 1.21x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5 | bm-20251020-vultr-x86_64-Fidget%2dSpinner-tracing_jit-3.15.0a1+-ec2971f |
|----------------|:----------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| 2to3           | 259 ms                                                                 | 252 ms: 1.03x faster                                                    |
| docutils       | 2.63 sec                                                               | 2.61 sec: 1.01x faster                                                  |
| html5lib       | 68.6 ms                                                                | 63.7 ms: 1.08x faster                                                   |
| Geometric mean | (ref)                                                                  | 1.04x faster                                                            |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5 | bm-20251020-vultr-x86_64-Fidget%2dSpinner-tracing_jit-3.15.0a1+-ec2971f |
|----------------------------|:----------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| async_tree_memoization     | 459 ms                                                                 | 308 ms: 1.49x faster                                                    |
| async_tree_io              | 881 ms                                                                 | 596 ms: 1.48x faster                                                    |
| async_tree_io_tg           | 901 ms                                                                 | 613 ms: 1.47x faster                                                    |
| async_tree_cpu_io_mixed    | 666 ms                                                                 | 475 ms: 1.40x faster                                                    |
| async_tree_none            | 353 ms                                                                 | 257 ms: 1.37x faster                                                    |
| async_tree_none_tg         | 333 ms                                                                 | 245 ms: 1.36x faster                                                    |
| async_tree_cpu_io_mixed_tg | 634 ms                                                                 | 474 ms: 1.34x faster                                                    |
| async_tree_memoization_tg  | 410 ms                                                                 | 307 ms: 1.33x faster                                                    |
| async_generators           | 375 ms                                                                 | 334 ms: 1.12x faster                                                    |
| coroutines                 | 23.3 ms                                                                | 21.9 ms: 1.06x faster                                                   |
| asyncio_websockets         | 517 ms                                                                 | 544 ms: 1.05x slower                                                    |
| Geometric mean             | (ref)                                                                  | 1.30x faster                                                            |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5 | bm-20251020-vultr-x86_64-Fidget%2dSpinner-tracing_jit-3.15.0a1+-ec2971f |
|----------------|:----------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| float          | 76.7 ms                                                                | 62.0 ms: 1.24x faster                                                   |
| pidigits       | 216 ms                                                                 | 192 ms: 1.13x faster                                                    |
| nbody          | 85.3 ms                                                                | 99.8 ms: 1.17x slower                                                   |
| Geometric mean | (ref)                                                                  | 1.06x faster                                                            |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5 | bm-20251020-vultr-x86_64-Fidget%2dSpinner-tracing_jit-3.15.0a1+-ec2971f |
|----------------|:----------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| regex_effbot   | 3.21 ms                                                                | 2.94 ms: 1.09x faster                                                   |
| regex_v8       | 23.2 ms                                                                | 21.8 ms: 1.06x faster                                                   |
| regex_dna      | 189 ms                                                                 | 177 ms: 1.06x faster                                                    |
| regex_compile  | 131 ms                                                                 | 138 ms: 1.05x slower                                                    |
| Geometric mean | (ref)                                                                  | 1.04x faster                                                            |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5 | bm-20251020-vultr-x86_64-Fidget%2dSpinner-tracing_jit-3.15.0a1+-ec2971f |
|----------------------|:----------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| json_dumps           | 10.6 ms                                                                | 8.74 ms: 1.22x faster                                                   |
| xml_etree_process    | 59.2 ms                                                                | 52.0 ms: 1.14x faster                                                   |
| unpickle_pure_python | 208 us                                                                 | 183 us: 1.14x faster                                                    |
| xml_etree_generate   | 85.4 ms                                                                | 75.5 ms: 1.13x faster                                                   |
| json_loads           | 27.3 us                                                                | 25.9 us: 1.05x faster                                                   |
| tomli_loads          | 2.01 sec                                                               | 1.97 sec: 1.02x faster                                                  |
| pickle_pure_python   | 292 us                                                                 | 305 us: 1.05x slower                                                    |
| xml_etree_parse      | 136 ms                                                                 | 143 ms: 1.05x slower                                                    |
| Geometric mean       | (ref)                                                                  | 1.06x faster                                                            |

Benchmark hidden because not significant (1): xml_etree_iterparse

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5 | bm-20251020-vultr-x86_64-Fidget%2dSpinner-tracing_jit-3.15.0a1+-ec2971f |
|------------------------|:----------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| python_startup_no_site | 7.41 ms                                                                | 7.40 ms: 1.00x faster                                                   |
| python_startup         | 11.0 ms                                                                | 12.7 ms: 1.15x slower                                                   |
| Geometric mean         | (ref)                                                                  | 1.07x slower                                                            |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5 | bm-20251020-vultr-x86_64-Fidget%2dSpinner-tracing_jit-3.15.0a1+-ec2971f |
|-----------------|:----------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| genshi_text     | 21.7 ms                                                                | 20.0 ms: 1.09x faster                                                   |
| mako            | 11.2 ms                                                                | 10.9 ms: 1.03x faster                                                   |
| django_template | 34.2 ms                                                                | 33.5 ms: 1.02x faster                                                   |
| Geometric mean  | (ref)                                                                  | 1.04x faster                                                            |

Benchmark hidden because not significant (1): genshi_xml

All benchmarks:
===============

| Benchmark                  | bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5 | bm-20251020-vultr-x86_64-Fidget%2dSpinner-tracing_jit-3.15.0a1+-ec2971f |
|----------------------------|:----------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| richards                   | 44.4 ms                                                                | 21.9 ms: 2.03x faster                                                   |
| mdp                        | 2.34 sec                                                               | 1.18 sec: 1.99x faster                                                  |
| richards_super             | 50.4 ms                                                                | 26.5 ms: 1.90x faster                                                   |
| deepcopy_memo              | 38.1 us                                                                | 21.9 us: 1.74x faster                                                   |
| deepcopy                   | 357 us                                                                 | 228 us: 1.57x faster                                                    |
| async_tree_memoization     | 459 ms                                                                 | 308 ms: 1.49x faster                                                    |
| async_tree_io              | 881 ms                                                                 | 596 ms: 1.48x faster                                                    |
| async_tree_io_tg           | 901 ms                                                                 | 613 ms: 1.47x faster                                                    |
| scimark_fft                | 348 ms                                                                 | 242 ms: 1.44x faster                                                    |
| async_tree_cpu_io_mixed    | 666 ms                                                                 | 475 ms: 1.40x faster                                                    |
| scimark_sor                | 134 ms                                                                 | 97.0 ms: 1.38x faster                                                   |
| async_tree_none            | 353 ms                                                                 | 257 ms: 1.37x faster                                                    |
| async_tree_none_tg         | 333 ms                                                                 | 245 ms: 1.36x faster                                                    |
| async_tree_cpu_io_mixed_tg | 634 ms                                                                 | 474 ms: 1.34x faster                                                    |
| async_tree_memoization_tg  | 410 ms                                                                 | 307 ms: 1.33x faster                                                    |
| deepcopy_reduce            | 3.12 us                                                                | 2.35 us: 1.33x faster                                                   |
| spectral_norm              | 108 ms                                                                 | 83.5 ms: 1.29x faster                                                   |
| logging_silent             | 98.2 ns                                                                | 78.4 ns: 1.25x faster                                                   |
| float                      | 76.7 ms                                                                | 62.0 ms: 1.24x faster                                                   |
| json_dumps                 | 10.6 ms                                                                | 8.74 ms: 1.22x faster                                                   |
| scimark_sparse_mat_mult    | 4.75 ms                                                                | 3.91 ms: 1.21x faster                                                   |
| scimark_monte_carlo        | 65.8 ms                                                                | 54.4 ms: 1.21x faster                                                   |
| scimark_lu                 | 112 ms                                                                 | 93.6 ms: 1.20x faster                                                   |
| go                         | 141 ms                                                                 | 120 ms: 1.17x faster                                                    |
| xml_etree_process          | 59.2 ms                                                                | 52.0 ms: 1.14x faster                                                   |
| comprehensions             | 16.6 us                                                                | 14.6 us: 1.14x faster                                                   |
| pyflate                    | 449 ms                                                                 | 395 ms: 1.14x faster                                                    |
| unpickle_pure_python       | 208 us                                                                 | 183 us: 1.14x faster                                                    |
| pylint                     | 317 ms                                                                 | 280 ms: 1.13x faster                                                    |
| xml_etree_generate         | 85.4 ms                                                                | 75.5 ms: 1.13x faster                                                   |
| bpe_tokeniser              | 4.46 sec                                                               | 3.94 sec: 1.13x faster                                                  |
| pidigits                   | 216 ms                                                                 | 192 ms: 1.13x faster                                                    |
| deltablue                  | 3.10 ms                                                                | 2.75 ms: 1.12x faster                                                   |
| async_generators           | 375 ms                                                                 | 334 ms: 1.12x faster                                                    |
| coverage                   | 82.5 ms                                                                | 74.6 ms: 1.11x faster                                                   |
| pprint_safe_repr           | 719 ms                                                                 | 655 ms: 1.10x faster                                                    |
| regex_effbot               | 3.21 ms                                                                | 2.94 ms: 1.09x faster                                                   |
| genshi_text                | 21.7 ms                                                                | 20.0 ms: 1.09x faster                                                   |
| pprint_pformat             | 1.46 sec                                                               | 1.34 sec: 1.08x faster                                                  |
| html5lib                   | 68.6 ms                                                                | 63.7 ms: 1.08x faster                                                   |
| coroutines                 | 23.3 ms                                                                | 21.9 ms: 1.06x faster                                                   |
| regex_v8                   | 23.2 ms                                                                | 21.8 ms: 1.06x faster                                                   |
| regex_dna                  | 189 ms                                                                 | 177 ms: 1.06x faster                                                    |
| typing_runtime_protocols   | 156 us                                                                 | 147 us: 1.06x faster                                                    |
| chaos                      | 56.3 ms                                                                | 53.1 ms: 1.06x faster                                                   |
| thrift                     | 772 us                                                                 | 728 us: 1.06x faster                                                    |
| sympy_integrate            | 19.7 ms                                                                | 18.7 ms: 1.06x faster                                                   |
| json_loads                 | 27.3 us                                                                | 25.9 us: 1.05x faster                                                   |
| logging_format             | 6.92 us                                                                | 6.59 us: 1.05x faster                                                   |
| fannkuch                   | 376 ms                                                                 | 360 ms: 1.04x faster                                                    |
| crypto_pyaes               | 68.2 ms                                                                | 65.8 ms: 1.04x faster                                                   |
| logging_simple             | 6.14 us                                                                | 5.93 us: 1.04x faster                                                   |
| sqlite_synth               | 2.25 us                                                                | 2.17 us: 1.04x faster                                                   |
| json                       | 4.98 ms                                                                | 4.83 ms: 1.03x faster                                                   |
| 2to3                       | 259 ms                                                                 | 252 ms: 1.03x faster                                                    |
| generators                 | 28.5 ms                                                                | 27.7 ms: 1.03x faster                                                   |
| mako                       | 11.2 ms                                                                | 10.9 ms: 1.03x faster                                                   |
| hexiom                     | 5.95 ms                                                                | 5.81 ms: 1.03x faster                                                   |
| django_template            | 34.2 ms                                                                | 33.5 ms: 1.02x faster                                                   |
| tomli_loads                | 2.01 sec                                                               | 1.97 sec: 1.02x faster                                                  |
| dulwich_log                | 74.5 ms                                                                | 73.6 ms: 1.01x faster                                                   |
| docutils                   | 2.63 sec                                                               | 2.61 sec: 1.01x faster                                                  |
| python_startup_no_site     | 7.41 ms                                                                | 7.40 ms: 1.00x faster                                                   |
| pathlib                    | 19.3 ms                                                                | 19.4 ms: 1.01x slower                                                   |
| sympy_str                  | 274 ms                                                                 | 279 ms: 1.02x slower                                                    |
| nqueens                    | 77.7 ms                                                                | 80.1 ms: 1.03x slower                                                   |
| bench_thread_pool          | 924 us                                                                 | 963 us: 1.04x slower                                                    |
| meteor_contest             | 101 ms                                                                 | 106 ms: 1.05x slower                                                    |
| pickle_pure_python         | 292 us                                                                 | 305 us: 1.05x slower                                                    |
| xml_etree_parse            | 136 ms                                                                 | 143 ms: 1.05x slower                                                    |
| sympy_expand               | 454 ms                                                                 | 477 ms: 1.05x slower                                                    |
| asyncio_websockets         | 517 ms                                                                 | 544 ms: 1.05x slower                                                    |
| regex_compile              | 131 ms                                                                 | 138 ms: 1.05x slower                                                    |
| pycparser                  | 1.12 sec                                                               | 1.19 sec: 1.06x slower                                                  |
| raytrace                   | 250 ms                                                                 | 266 ms: 1.06x slower                                                    |
| bench_mp_pool              | 11.0 ms                                                                | 12.3 ms: 1.12x slower                                                   |
| python_startup             | 11.0 ms                                                                | 12.7 ms: 1.15x slower                                                   |
| nbody                      | 85.3 ms                                                                | 99.8 ms: 1.17x slower                                                   |
| gc_traversal               | 3.32 ms                                                                | 4.31 ms: 1.30x slower                                                   |
| create_gc_cycles           | 1.33 ms                                                                | 2.04 ms: 1.53x slower                                                   |
| telco                      | 7.77 ms                                                                | 159 ms: 20.47x slower                                                   |
| Geometric mean             | (ref)                                                                  | 1.08x faster                                                            |

Benchmark hidden because not significant (3): genshi_xml, sympy_sum, xml_etree_iterparse
Ignored benchmarks (18) of results/bm-20240920-3.13.0rc2-4981ec5/bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5.json: aiohttp, asyncio_tcp, asyncio_tcp_ssl, chameleon, dask, flaskblogging, gunicorn, pickle, pickle_dict, pickle_list, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http, unpack_sequence, unpickle, unpickle_list
Ignored benchmarks (10) of results/bm-20251020-3.15.0a1+-ec2971f-CLANG,JIT/bm-20251020-vultr-x86_64-Fidget%2dSpinner-tracing_jit-3.15.0a1+-ec2971f.json: connected_components, k_core, many_optionals, shortest_path, sphinx, sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile, subparsers

- Geometric mean (including insignificant results): 1.079x faster

# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.05x
- 95% likely to have a speedup of 1.04x
- 99% likely to have a speedup of 1.03x

# Memory
- memory change: 1.21x