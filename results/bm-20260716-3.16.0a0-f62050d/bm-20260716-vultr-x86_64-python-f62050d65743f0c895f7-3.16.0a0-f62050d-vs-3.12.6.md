# Results vs. 3.12.6

- fork: python
- ref: f62050d65743f0c895f7
- machine: linux-x86_64
- commit hash: f62050d
- commit date: 2026-07-16
- overall geometric mean: 1.071x faster
- HPT reliability: 100.00%
- HPT 99th percentile: 1.04x faster
- Memory change: 1.14x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260716-vultr-x86_64-python-f62050d65743f0c895f7-3.16.0a0-f62050d |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| 2to3           | 264 ms                                                 | 258 ms: 1.02x faster                                                  |
| docutils       | 2.64 sec                                               | 2.42 sec: 1.09x faster                                                |
| html5lib       | 63.6 ms                                                | 58.4 ms: 1.09x faster                                                 |
| Geometric mean | (ref)                                                  | 1.07x faster                                                          |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260716-vultr-x86_64-python-f62050d65743f0c895f7-3.16.0a0-f62050d |
|----------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| async_tree_none            | 464 ms                                                 | 290 ms: 1.60x faster                                                  |
| async_tree_memoization_tg  | 560 ms                                                 | 360 ms: 1.56x faster                                                  |
| async_tree_io              | 1.08 sec                                               | 716 ms: 1.51x faster                                                  |
| async_tree_none_tg         | 446 ms                                                 | 298 ms: 1.50x faster                                                  |
| async_tree_memoization     | 555 ms                                                 | 373 ms: 1.49x faster                                                  |
| async_tree_io_tg           | 1.11 sec                                               | 761 ms: 1.46x faster                                                  |
| async_tree_cpu_io_mixed    | 715 ms                                                 | 542 ms: 1.32x faster                                                  |
| async_tree_cpu_io_mixed_tg | 723 ms                                                 | 557 ms: 1.30x faster                                                  |
| async_generators           | 384 ms                                                 | 335 ms: 1.15x faster                                                  |
| coroutines                 | 23.9 ms                                                | 22.8 ms: 1.05x faster                                                 |
| asyncio_websockets         | 517 ms                                                 | 544 ms: 1.05x slower                                                  |
| Geometric mean             | (ref)                                                  | 1.34x faster                                                          |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260716-vultr-x86_64-python-f62050d65743f0c895f7-3.16.0a0-f62050d |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| float          | 80.8 ms                                                | 74.8 ms: 1.08x faster                                                 |
| pidigits       | 184 ms                                                 | 192 ms: 1.04x slower                                                  |
| nbody          | 89.3 ms                                                | 94.4 ms: 1.06x slower                                                 |
| Geometric mean | (ref)                                                  | 1.01x slower                                                          |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260716-vultr-x86_64-python-f62050d65743f0c895f7-3.16.0a0-f62050d |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| regex_effbot   | 3.17 ms                                                | 2.80 ms: 1.13x faster                                                 |
| regex_compile  | 142 ms                                                 | 148 ms: 1.04x slower                                                  |
| regex_v8       | 20.6 ms                                                | 21.6 ms: 1.05x slower                                                 |
| regex_dna      | 168 ms                                                 | 187 ms: 1.12x slower                                                  |
| Geometric mean | (ref)                                                  | 1.02x slower                                                          |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260716-vultr-x86_64-python-f62050d65743f0c895f7-3.16.0a0-f62050d |
|----------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| tomli_loads          | 2.11 sec                                               | 1.82 sec: 1.16x faster                                                |
| xml_etree_iterparse  | 96.7 ms                                                | 89.0 ms: 1.09x faster                                                 |
| json_dumps           | 10.4 ms                                                | 9.81 ms: 1.06x faster                                                 |
| unpickle_pure_python | 221 us                                                 | 212 us: 1.04x faster                                                  |
| xml_etree_generate   | 85.2 ms                                                | 86.0 ms: 1.01x slower                                                 |
| xml_etree_process    | 59.0 ms                                                | 60.7 ms: 1.03x slower                                                 |
| xml_etree_parse      | 139 ms                                                 | 143 ms: 1.03x slower                                                  |
| json_loads           | 26.5 us                                                | 27.5 us: 1.04x slower                                                 |
| Geometric mean       | (ref)                                                  | 1.02x faster                                                          |

Benchmark hidden because not significant (1): pickle_pure_python

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260716-vultr-x86_64-python-f62050d65743f0c895f7-3.16.0a0-f62050d |
|------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| python_startup_no_site | 7.16 ms                                                | 7.05 ms: 1.02x faster                                                 |
| python_startup         | 9.93 ms                                                | 12.4 ms: 1.25x slower                                                 |
| Geometric mean         | (ref)                                                  | 1.11x slower                                                          |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260716-vultr-x86_64-python-f62050d65743f0c895f7-3.16.0a0-f62050d |
|-----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| django_template | 34.7 ms                                                | 35.9 ms: 1.03x slower                                                 |
| mako            | 11.0 ms                                                | 12.1 ms: 1.10x slower                                                 |
| Geometric mean  | (ref)                                                  | 1.07x slower                                                          |

All benchmarks:
===============

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260716-vultr-x86_64-python-f62050d65743f0c895f7-3.16.0a0-f62050d |
|----------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| pylint                     | 319 ms                                                 | 114 ms: 2.78x faster                                                  |
| mdp                        | 2.42 sec                                               | 1.14 sec: 2.12x faster                                                |
| async_tree_none            | 464 ms                                                 | 290 ms: 1.60x faster                                                  |
| async_tree_memoization_tg  | 560 ms                                                 | 360 ms: 1.56x faster                                                  |
| async_tree_io              | 1.08 sec                                               | 716 ms: 1.51x faster                                                  |
| deepcopy_memo              | 40.3 us                                                | 26.7 us: 1.51x faster                                                 |
| deepcopy                   | 352 us                                                 | 235 us: 1.50x faster                                                  |
| async_tree_none_tg         | 446 ms                                                 | 298 ms: 1.50x faster                                                  |
| async_tree_memoization     | 555 ms                                                 | 373 ms: 1.49x faster                                                  |
| async_tree_io_tg           | 1.11 sec                                               | 761 ms: 1.46x faster                                                  |
| typing_runtime_protocols   | 163 us                                                 | 116 us: 1.41x faster                                                  |
| go                         | 139 ms                                                 | 102 ms: 1.36x faster                                                  |
| async_tree_cpu_io_mixed    | 715 ms                                                 | 542 ms: 1.32x faster                                                  |
| async_tree_cpu_io_mixed_tg | 723 ms                                                 | 557 ms: 1.30x faster                                                  |
| comprehensions             | 19.8 us                                                | 15.9 us: 1.24x faster                                                 |
| deepcopy_reduce            | 3.08 us                                                | 2.52 us: 1.22x faster                                                 |
| pathlib                    | 21.5 ms                                                | 17.6 ms: 1.22x faster                                                 |
| scimark_sor                | 130 ms                                                 | 107 ms: 1.21x faster                                                  |
| raytrace                   | 299 ms                                                 | 250 ms: 1.20x faster                                                  |
| spectral_norm              | 110 ms                                                 | 93.6 ms: 1.18x faster                                                 |
| chaos                      | 62.8 ms                                                | 53.7 ms: 1.17x faster                                                 |
| pyflate                    | 448 ms                                                 | 386 ms: 1.16x faster                                                  |
| tomli_loads                | 2.11 sec                                               | 1.82 sec: 1.16x faster                                                |
| generators                 | 32.2 ms                                                | 28.0 ms: 1.15x faster                                                 |
| async_generators           | 384 ms                                                 | 335 ms: 1.15x faster                                                  |
| dulwich_log                | 78.9 ms                                                | 69.5 ms: 1.13x faster                                                 |
| logging_simple             | 6.63 us                                                | 5.85 us: 1.13x faster                                                 |
| regex_effbot               | 3.17 ms                                                | 2.80 ms: 1.13x faster                                                 |
| logging_silent             | 109 ns                                                 | 96.6 ns: 1.13x faster                                                 |
| bpe_tokeniser              | 4.74 sec                                               | 4.20 sec: 1.13x faster                                                |
| crypto_pyaes               | 76.6 ms                                                | 68.8 ms: 1.11x faster                                                 |
| logging_format             | 7.35 us                                                | 6.69 us: 1.10x faster                                                 |
| hexiom                     | 6.17 ms                                                | 5.63 ms: 1.10x faster                                                 |
| docutils                   | 2.64 sec                                               | 2.42 sec: 1.09x faster                                                |
| html5lib                   | 63.6 ms                                                | 58.4 ms: 1.09x faster                                                 |
| xml_etree_iterparse        | 96.7 ms                                                | 89.0 ms: 1.09x faster                                                 |
| scimark_monte_carlo        | 68.4 ms                                                | 63.1 ms: 1.08x faster                                                 |
| float                      | 80.8 ms                                                | 74.8 ms: 1.08x faster                                                 |
| nqueens                    | 80.1 ms                                                | 74.3 ms: 1.08x faster                                                 |
| sympy_integrate            | 20.5 ms                                                | 19.1 ms: 1.07x faster                                                 |
| richards                   | 45.9 ms                                                | 42.8 ms: 1.07x faster                                                 |
| scimark_fft                | 342 ms                                                 | 319 ms: 1.07x faster                                                  |
| sympy_str                  | 292 ms                                                 | 274 ms: 1.07x faster                                                  |
| deltablue                  | 3.45 ms                                                | 3.24 ms: 1.06x faster                                                 |
| richards_super             | 51.9 ms                                                | 49.0 ms: 1.06x faster                                                 |
| sympy_sum                  | 166 ms                                                 | 157 ms: 1.06x faster                                                  |
| json_dumps                 | 10.4 ms                                                | 9.81 ms: 1.06x faster                                                 |
| coroutines                 | 23.9 ms                                                | 22.8 ms: 1.05x faster                                                 |
| pycparser                  | 1.17 sec                                               | 1.12 sec: 1.05x faster                                                |
| unpickle_pure_python       | 221 us                                                 | 212 us: 1.04x faster                                                  |
| pprint_pformat             | 1.52 sec                                               | 1.46 sec: 1.04x faster                                                |
| pprint_safe_repr           | 743 ms                                                 | 720 ms: 1.03x faster                                                  |
| 2to3                       | 264 ms                                                 | 258 ms: 1.02x faster                                                  |
| thrift                     | 791 us                                                 | 777 us: 1.02x faster                                                  |
| sympy_expand               | 468 ms                                                 | 459 ms: 1.02x faster                                                  |
| meteor_contest             | 104 ms                                                 | 102 ms: 1.02x faster                                                  |
| python_startup_no_site     | 7.16 ms                                                | 7.05 ms: 1.02x faster                                                 |
| scimark_lu                 | 114 ms                                                 | 113 ms: 1.01x faster                                                  |
| xml_etree_generate         | 85.2 ms                                                | 86.0 ms: 1.01x slower                                                 |
| sqlite_synth               | 2.20 us                                                | 2.23 us: 1.01x slower                                                 |
| xml_etree_process          | 59.0 ms                                                | 60.7 ms: 1.03x slower                                                 |
| xml_etree_parse            | 139 ms                                                 | 143 ms: 1.03x slower                                                  |
| django_template            | 34.7 ms                                                | 35.9 ms: 1.03x slower                                                 |
| regex_compile              | 142 ms                                                 | 148 ms: 1.04x slower                                                  |
| json_loads                 | 26.5 us                                                | 27.5 us: 1.04x slower                                                 |
| pidigits                   | 184 ms                                                 | 192 ms: 1.04x slower                                                  |
| regex_v8                   | 20.6 ms                                                | 21.6 ms: 1.05x slower                                                 |
| asyncio_websockets         | 517 ms                                                 | 544 ms: 1.05x slower                                                  |
| nbody                      | 89.3 ms                                                | 94.4 ms: 1.06x slower                                                 |
| scimark_sparse_mat_mult    | 4.39 ms                                                | 4.71 ms: 1.07x slower                                                 |
| mako                       | 11.0 ms                                                | 12.1 ms: 1.10x slower                                                 |
| gc_traversal               | 3.46 ms                                                | 3.81 ms: 1.10x slower                                                 |
| regex_dna                  | 168 ms                                                 | 187 ms: 1.12x slower                                                  |
| coverage                   | 71.4 ms                                                | 82.2 ms: 1.15x slower                                                 |
| python_startup             | 9.93 ms                                                | 12.4 ms: 1.25x slower                                                 |
| bench_thread_pool          | 941 us                                                 | 1.34 ms: 1.42x slower                                                 |
| create_gc_cycles           | 1.09 ms                                                | 1.69 ms: 1.55x slower                                                 |
| bench_mp_pool              | 10.8 ms                                                | 233 ms: 21.57x slower                                                 |
| telco                      | 6.53 ms                                                | 157 ms: 24.10x slower                                                 |
| Geometric mean             | (ref)                                                  | 1.03x faster                                                          |

Benchmark hidden because not significant (3): json, fannkuch, pickle_pure_python
Ignored benchmarks (23) of results/bm-20240906-3.12.6-a4a2d2b/bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b.json: aiohttp, asyncio_tcp, asyncio_tcp_ssl, chameleon, dask, flaskblogging, genshi_text, genshi_xml, gunicorn, mypy2, pickle, pickle_dict, pickle_list, sqlalchemy_declarative, sqlalchemy_imperative, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http, unpack_sequence, unpickle, unpickle_list
Ignored benchmarks (10) of results/bm-20260716-3.16.0a0-f62050d/bm-20260716-vultr-x86_64-python-f62050d65743f0c895f7-3.16.0a0-f62050d.json: connected_components, k_core, many_optionals, shortest_path, sphinx, sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile, subparsers

- Geometric mean (including insignificant results): 1.071x faster

# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.05x
- 95% likely to have a speedup of 1.05x
- 99% likely to have a speedup of 1.04x

# Memory
- memory change: 1.14x