# Results vs. 3.13.0rc2

- fork: python
- ref: 80f9467434cecbc4e97b
- machine: linux-x86_64
- commit hash: 80f9467
- commit date: 2026-06-11
- overall geometric mean: 1.028x faster
- HPT reliability: 99.78%
- HPT 99th percentile: 1.00x faster
- Memory change: 1.16x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5 | bm-20260611-vultr-x86_64-python-80f9467434cecbc4e97b-3.16.0a0-80f9467 |
|----------------|:----------------------------------------------------------------------:|:---------------------------------------------------------------------:|
| docutils       | 2.63 sec                                                               | 2.41 sec: 1.09x faster                                                |
| html5lib       | 68.6 ms                                                                | 60.1 ms: 1.14x faster                                                 |
| Geometric mean | (ref)                                                                  | 1.12x faster                                                          |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5 | bm-20260611-vultr-x86_64-python-80f9467434cecbc4e97b-3.16.0a0-80f9467 |
|----------------------------|:----------------------------------------------------------------------:|:---------------------------------------------------------------------:|
| async_tree_io              | 881 ms                                                                 | 715 ms: 1.23x faster                                                  |
| async_tree_memoization     | 459 ms                                                                 | 373 ms: 1.23x faster                                                  |
| async_tree_cpu_io_mixed    | 666 ms                                                                 | 542 ms: 1.23x faster                                                  |
| async_tree_none            | 353 ms                                                                 | 290 ms: 1.22x faster                                                  |
| async_tree_io_tg           | 901 ms                                                                 | 762 ms: 1.18x faster                                                  |
| async_tree_cpu_io_mixed_tg | 634 ms                                                                 | 555 ms: 1.14x faster                                                  |
| async_tree_memoization_tg  | 410 ms                                                                 | 365 ms: 1.12x faster                                                  |
| async_generators           | 375 ms                                                                 | 338 ms: 1.11x faster                                                  |
| async_tree_none_tg         | 333 ms                                                                 | 301 ms: 1.11x faster                                                  |
| coroutines                 | 23.3 ms                                                                | 23.8 ms: 1.02x slower                                                 |
| asyncio_websockets         | 517 ms                                                                 | 545 ms: 1.05x slower                                                  |
| Geometric mean             | (ref)                                                                  | 1.13x faster                                                          |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5 | bm-20260611-vultr-x86_64-python-80f9467434cecbc4e97b-3.16.0a0-80f9467 |
|----------------|:----------------------------------------------------------------------:|:---------------------------------------------------------------------:|
| pidigits       | 216 ms                                                                 | 195 ms: 1.11x faster                                                  |
| float          | 76.7 ms                                                                | 75.4 ms: 1.02x faster                                                 |
| nbody          | 85.3 ms                                                                | 94.9 ms: 1.11x slower                                                 |
| Geometric mean | (ref)                                                                  | 1.00x faster                                                          |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5 | bm-20260611-vultr-x86_64-python-80f9467434cecbc4e97b-3.16.0a0-80f9467 |
|----------------|:----------------------------------------------------------------------:|:---------------------------------------------------------------------:|
| regex_effbot   | 3.21 ms                                                                | 3.00 ms: 1.07x faster                                                 |
| regex_compile  | 131 ms                                                                 | 123 ms: 1.07x faster                                                  |
| regex_v8       | 23.2 ms                                                                | 22.0 ms: 1.05x faster                                                 |
| regex_dna      | 189 ms                                                                 | 180 ms: 1.05x faster                                                  |
| Geometric mean | (ref)                                                                  | 1.06x faster                                                          |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5 | bm-20260611-vultr-x86_64-python-80f9467434cecbc4e97b-3.16.0a0-80f9467 |
|----------------------|:----------------------------------------------------------------------:|:---------------------------------------------------------------------:|
| tomli_loads          | 2.01 sec                                                               | 1.82 sec: 1.10x faster                                                |
| json_dumps           | 10.6 ms                                                                | 9.84 ms: 1.08x faster                                                 |
| xml_etree_iterparse  | 94.4 ms                                                                | 90.2 ms: 1.05x faster                                                 |
| xml_etree_generate   | 85.4 ms                                                                | 87.0 ms: 1.02x slower                                                 |
| unpickle_pure_python | 208 us                                                                 | 213 us: 1.02x slower                                                  |
| xml_etree_process    | 59.2 ms                                                                | 61.4 ms: 1.04x slower                                                 |
| xml_etree_parse      | 136 ms                                                                 | 142 ms: 1.05x slower                                                  |
| pickle_pure_python   | 292 us                                                                 | 314 us: 1.08x slower                                                  |
| Geometric mean       | (ref)                                                                  | 1.00x faster                                                          |

Benchmark hidden because not significant (1): json_loads

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5 | bm-20260611-vultr-x86_64-python-80f9467434cecbc4e97b-3.16.0a0-80f9467 |
|-----------------|:----------------------------------------------------------------------:|:---------------------------------------------------------------------:|
| django_template | 34.2 ms                                                                | 35.8 ms: 1.05x slower                                                 |
| mako            | 11.2 ms                                                                | 11.8 ms: 1.05x slower                                                 |
| Geometric mean  | (ref)                                                                  | 1.05x slower                                                          |

All benchmarks:
===============

| Benchmark                  | bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5 | bm-20260611-vultr-x86_64-python-80f9467434cecbc4e97b-3.16.0a0-80f9467 |
|----------------------------|:----------------------------------------------------------------------:|:---------------------------------------------------------------------:|
| pylint                     | 317 ms                                                                 | 114 ms: 2.77x faster                                                  |
| mdp                        | 2.34 sec                                                               | 1.15 sec: 2.04x faster                                                |
| deepcopy                   | 357 us                                                                 | 232 us: 1.54x faster                                                  |
| deepcopy_memo              | 38.1 us                                                                | 26.6 us: 1.43x faster                                                 |
| go                         | 141 ms                                                                 | 106 ms: 1.33x faster                                                  |
| typing_runtime_protocols   | 156 us                                                                 | 119 us: 1.31x faster                                                  |
| async_tree_io              | 881 ms                                                                 | 715 ms: 1.23x faster                                                  |
| async_tree_memoization     | 459 ms                                                                 | 373 ms: 1.23x faster                                                  |
| async_tree_cpu_io_mixed    | 666 ms                                                                 | 542 ms: 1.23x faster                                                  |
| deepcopy_reduce            | 3.12 us                                                                | 2.55 us: 1.22x faster                                                 |
| async_tree_none            | 353 ms                                                                 | 290 ms: 1.22x faster                                                  |
| scimark_sor                | 134 ms                                                                 | 111 ms: 1.20x faster                                                  |
| async_tree_io_tg           | 901 ms                                                                 | 762 ms: 1.18x faster                                                  |
| async_tree_cpu_io_mixed_tg | 634 ms                                                                 | 555 ms: 1.14x faster                                                  |
| html5lib                   | 68.6 ms                                                                | 60.1 ms: 1.14x faster                                                 |
| spectral_norm              | 108 ms                                                                 | 95.4 ms: 1.13x faster                                                 |
| async_tree_memoization_tg  | 410 ms                                                                 | 365 ms: 1.12x faster                                                  |
| pidigits                   | 216 ms                                                                 | 195 ms: 1.11x faster                                                  |
| async_generators           | 375 ms                                                                 | 338 ms: 1.11x faster                                                  |
| pyflate                    | 449 ms                                                                 | 406 ms: 1.11x faster                                                  |
| async_tree_none_tg         | 333 ms                                                                 | 301 ms: 1.11x faster                                                  |
| tomli_loads                | 2.01 sec                                                               | 1.82 sec: 1.10x faster                                                |
| dulwich_log                | 74.5 ms                                                                | 67.5 ms: 1.10x faster                                                 |
| pathlib                    | 19.3 ms                                                                | 17.6 ms: 1.09x faster                                                 |
| docutils                   | 2.63 sec                                                               | 2.41 sec: 1.09x faster                                                |
| json_dumps                 | 10.6 ms                                                                | 9.84 ms: 1.08x faster                                                 |
| scimark_fft                | 348 ms                                                                 | 323 ms: 1.08x faster                                                  |
| regex_effbot               | 3.21 ms                                                                | 3.00 ms: 1.07x faster                                                 |
| regex_compile              | 131 ms                                                                 | 123 ms: 1.07x faster                                                  |
| bpe_tokeniser              | 4.46 sec                                                               | 4.21 sec: 1.06x faster                                                |
| regex_v8                   | 23.2 ms                                                                | 22.0 ms: 1.05x faster                                                 |
| comprehensions             | 16.6 us                                                                | 15.8 us: 1.05x faster                                                 |
| regex_dna                  | 189 ms                                                                 | 180 ms: 1.05x faster                                                  |
| xml_etree_iterparse        | 94.4 ms                                                                | 90.2 ms: 1.05x faster                                                 |
| generators                 | 28.5 ms                                                                | 27.4 ms: 1.04x faster                                                 |
| richards                   | 44.4 ms                                                                | 42.9 ms: 1.03x faster                                                 |
| sympy_integrate            | 19.7 ms                                                                | 19.1 ms: 1.03x faster                                                 |
| nqueens                    | 77.7 ms                                                                | 75.4 ms: 1.03x faster                                                 |
| hexiom                     | 5.95 ms                                                                | 5.78 ms: 1.03x faster                                                 |
| richards_super             | 50.4 ms                                                                | 49.3 ms: 1.02x faster                                                 |
| scimark_monte_carlo        | 65.8 ms                                                                | 64.7 ms: 1.02x faster                                                 |
| logging_format             | 6.92 us                                                                | 6.80 us: 1.02x faster                                                 |
| float                      | 76.7 ms                                                                | 75.4 ms: 1.02x faster                                                 |
| logging_simple             | 6.14 us                                                                | 6.04 us: 1.02x faster                                                 |
| chaos                      | 56.3 ms                                                                | 55.5 ms: 1.01x faster                                                 |
| meteor_contest             | 101 ms                                                                 | 99.9 ms: 1.01x faster                                                 |
| coverage                   | 82.5 ms                                                                | 83.4 ms: 1.01x slower                                                 |
| sqlite_synth               | 2.25 us                                                                | 2.28 us: 1.01x slower                                                 |
| pprint_safe_repr           | 719 ms                                                                 | 729 ms: 1.01x slower                                                  |
| sympy_sum                  | 154 ms                                                                 | 157 ms: 1.02x slower                                                  |
| pprint_pformat             | 1.46 sec                                                               | 1.48 sec: 1.02x slower                                                |
| xml_etree_generate         | 85.4 ms                                                                | 87.0 ms: 1.02x slower                                                 |
| sympy_expand               | 454 ms                                                                 | 462 ms: 1.02x slower                                                  |
| coroutines                 | 23.3 ms                                                                | 23.8 ms: 1.02x slower                                                 |
| unpickle_pure_python       | 208 us                                                                 | 213 us: 1.02x slower                                                  |
| scimark_lu                 | 112 ms                                                                 | 115 ms: 1.02x slower                                                  |
| json                       | 4.98 ms                                                                | 5.10 ms: 1.02x slower                                                 |
| thrift                     | 772 us                                                                 | 791 us: 1.03x slower                                                  |
| raytrace                   | 250 ms                                                                 | 259 ms: 1.04x slower                                                  |
| xml_etree_process          | 59.2 ms                                                                | 61.4 ms: 1.04x slower                                                 |
| crypto_pyaes               | 68.2 ms                                                                | 70.8 ms: 1.04x slower                                                 |
| xml_etree_parse            | 136 ms                                                                 | 142 ms: 1.05x slower                                                  |
| django_template            | 34.2 ms                                                                | 35.8 ms: 1.05x slower                                                 |
| mako                       | 11.2 ms                                                                | 11.8 ms: 1.05x slower                                                 |
| asyncio_websockets         | 517 ms                                                                 | 545 ms: 1.05x slower                                                  |
| scimark_sparse_mat_mult    | 4.75 ms                                                                | 5.01 ms: 1.05x slower                                                 |
| pickle_pure_python         | 292 us                                                                 | 314 us: 1.08x slower                                                  |
| deltablue                  | 3.10 ms                                                                | 3.39 ms: 1.10x slower                                                 |
| nbody                      | 85.3 ms                                                                | 94.9 ms: 1.11x slower                                                 |
| gc_traversal               | 3.32 ms                                                                | 3.84 ms: 1.16x slower                                                 |
| create_gc_cycles           | 1.33 ms                                                                | 1.81 ms: 1.36x slower                                                 |
| bench_thread_pool          | 924 us                                                                 | 1.33 ms: 1.44x slower                                                 |
| telco                      | 7.77 ms                                                                | 160 ms: 20.60x slower                                                 |
| bench_mp_pool              | 11.0 ms                                                                | 250 ms: 22.73x slower                                                 |
| Geometric mean             | (ref)                                                                  | 1.02x slower                                                          |

Benchmark hidden because not significant (5): logging_silent, pycparser, json_loads, fannkuch, sympy_str
Ignored benchmarks (23) of results/bm-20240920-3.13.0rc2-4981ec5/bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5.json: 2to3, aiohttp, asyncio_tcp, asyncio_tcp_ssl, chameleon, dask, flaskblogging, genshi_text, genshi_xml, gunicorn, pickle, pickle_dict, pickle_list, python_startup, python_startup_no_site, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http, unpack_sequence, unpickle, unpickle_list
Ignored benchmarks (10) of results/bm-20260611-3.16.0a0-80f9467/bm-20260611-vultr-x86_64-python-80f9467434cecbc4e97b-3.16.0a0-80f9467.json: connected_components, k_core, many_optionals, shortest_path, sphinx, sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile, subparsers

- Geometric mean (including insignificant results): 1.028x faster

# HPT report

- Reliability score: 99.78% likely to be faster
- 90% likely to have a speedup of 1.02x
- 95% likely to have a speedup of 1.01x
- 99% likely to have a speedup of 1.00x

# Memory
- memory change: 1.16x