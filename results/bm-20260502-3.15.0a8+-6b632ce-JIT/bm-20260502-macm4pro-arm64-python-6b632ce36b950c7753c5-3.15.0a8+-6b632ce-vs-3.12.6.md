# Results vs. 3.12.6

- fork: python
- ref: 6b632ce36b950c7753c5
- machine: darwin-arm64
- commit hash: 6b632ce
- commit date: 2026-05-02
- overall geometric mean: 1.249x faster
- HPT reliability: 100.00%
- HPT 99th percentile: 1.14x faster
- Memory change: 1.26x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260502-macm4pro-arm64-python-6b632ce36b950c7753c5-3.15.0a8+-6b632ce |
|----------------|:--------------------------------------------------------:|:------------------------------------------------------------------------:|
| 2to3           | 114 ms                                                   | 116 ms: 1.02x slower                                                     |
| docutils       | 1.02 sec                                                 | 942 ms: 1.08x faster                                                     |
| html5lib       | 23.0 ms                                                  | 19.8 ms: 1.16x faster                                                    |
| sphinx         | 434 ms                                                   | 404 ms: 1.07x faster                                                     |
| Geometric mean | (ref)                                                    | 1.07x faster                                                             |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                        | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260502-macm4pro-arm64-python-6b632ce36b950c7753c5-3.15.0a8+-6b632ce |
|----------------------------------|:--------------------------------------------------------:|:------------------------------------------------------------------------:|
| async_tree_eager_io              | 496 ms                                                   | 293 ms: 1.69x faster                                                     |
| async_tree_none                  | 178 ms                                                   | 108 ms: 1.65x faster                                                     |
| async_tree_io                    | 459 ms                                                   | 290 ms: 1.58x faster                                                     |
| async_tree_io_tg                 | 480 ms                                                   | 307 ms: 1.56x faster                                                     |
| async_tree_none_tg               | 172 ms                                                   | 115 ms: 1.49x faster                                                     |
| async_tree_memoization_tg        | 231 ms                                                   | 165 ms: 1.40x faster                                                     |
| async_tree_eager_io_tg           | 446 ms                                                   | 323 ms: 1.38x faster                                                     |
| async_tree_memoization           | 223 ms                                                   | 166 ms: 1.34x faster                                                     |
| coroutines                       | 13.6 ms                                                  | 10.1 ms: 1.34x faster                                                    |
| async_tree_eager                 | 45.6 ms                                                  | 36.4 ms: 1.25x faster                                                    |
| async_tree_cpu_io_mixed          | 333 ms                                                   | 272 ms: 1.23x faster                                                     |
| async_tree_cpu_io_mixed_tg       | 338 ms                                                   | 284 ms: 1.19x faster                                                     |
| async_tree_eager_memoization     | 132 ms                                                   | 112 ms: 1.18x faster                                                     |
| async_generators                 | 206 ms                                                   | 183 ms: 1.13x faster                                                     |
| async_tree_eager_cpu_io_mixed    | 231 ms                                                   | 222 ms: 1.04x faster                                                     |
| asyncio_websockets               | 190 ms                                                   | 189 ms: 1.01x faster                                                     |
| async_tree_eager_cpu_io_mixed_tg | 213 ms                                                   | 260 ms: 1.23x slower                                                     |
| async_tree_eager_memoization_tg  | 113 ms                                                   | 149 ms: 1.32x slower                                                     |
| async_tree_eager_tg              | 32.1 ms                                                  | 97.5 ms: 3.04x slower                                                    |
| Geometric mean                   | (ref)                                                    | 1.17x faster                                                             |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260502-macm4pro-arm64-python-6b632ce36b950c7753c5-3.15.0a8+-6b632ce |
|----------------|:--------------------------------------------------------:|:------------------------------------------------------------------------:|
| float          | 37.9 ms                                                  | 23.6 ms: 1.61x faster                                                    |
| nbody          | 54.2 ms                                                  | 34.1 ms: 1.59x faster                                                    |
| pidigits       | 161 ms                                                   | 165 ms: 1.02x slower                                                     |
| Geometric mean | (ref)                                                    | 1.36x faster                                                             |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260502-macm4pro-arm64-python-6b632ce36b950c7753c5-3.15.0a8+-6b632ce |
|----------------|:--------------------------------------------------------:|:------------------------------------------------------------------------:|
| regex_compile  | 54.6 ms                                                  | 39.6 ms: 1.38x faster                                                    |
| regex_effbot   | 1.67 ms                                                  | 1.49 ms: 1.12x faster                                                    |
| regex_dna      | 99.6 ms                                                  | 96.9 ms: 1.03x faster                                                    |
| regex_v8       | 9.59 ms                                                  | 9.86 ms: 1.03x slower                                                    |
| Geometric mean | (ref)                                                    | 1.11x faster                                                             |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260502-macm4pro-arm64-python-6b632ce36b950c7753c5-3.15.0a8+-6b632ce |
|----------------------|:--------------------------------------------------------:|:------------------------------------------------------------------------:|
| tomli_loads          | 957 ms                                                   | 629 ms: 1.52x faster                                                     |
| unpickle_pure_python | 103 us                                                   | 75.7 us: 1.36x faster                                                    |
| xml_etree_iterparse  | 51.6 ms                                                  | 40.9 ms: 1.26x faster                                                    |
| pickle_pure_python   | 139 us                                                   | 112 us: 1.25x faster                                                     |
| xml_etree_process    | 26.7 ms                                                  | 22.1 ms: 1.21x faster                                                    |
| json_dumps           | 4.26 ms                                                  | 3.54 ms: 1.20x faster                                                    |
| xml_etree_generate   | 38.9 ms                                                  | 32.6 ms: 1.19x faster                                                    |
| xml_etree_parse      | 67.9 ms                                                  | 66.0 ms: 1.03x faster                                                    |
| json_loads           | 10.9 us                                                  | 10.6 us: 1.02x faster                                                    |
| Geometric mean       | (ref)                                                    | 1.22x faster                                                             |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260502-macm4pro-arm64-python-6b632ce36b950c7753c5-3.15.0a8+-6b632ce |
|------------------------|:--------------------------------------------------------:|:------------------------------------------------------------------------:|
| python_startup_no_site | 5.71 ms                                                  | 5.96 ms: 1.04x slower                                                    |
| python_startup         | 8.01 ms                                                  | 8.80 ms: 1.10x slower                                                    |
| Geometric mean         | (ref)                                                    | 1.07x slower                                                             |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260502-macm4pro-arm64-python-6b632ce36b950c7753c5-3.15.0a8+-6b632ce |
|-----------------|:--------------------------------------------------------:|:------------------------------------------------------------------------:|
| mako            | 4.77 ms                                                  | 3.93 ms: 1.22x faster                                                    |
| django_template | 13.6 ms                                                  | 12.6 ms: 1.08x faster                                                    |
| Geometric mean  | (ref)                                                    | 1.15x faster                                                             |

All benchmarks:
===============

| Benchmark                        | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260502-macm4pro-arm64-python-6b632ce36b950c7753c5-3.15.0a8+-6b632ce |
|----------------------------------|:--------------------------------------------------------:|:------------------------------------------------------------------------:|
| subparsers                       | 20.8 ms                                                  | 3.63 ms: 5.71x faster                                                    |
| pylint                           | 128 ms                                                   | 56.6 ms: 2.26x faster                                                    |
| richards_super                   | 25.4 ms                                                  | 11.8 ms: 2.15x faster                                                    |
| richards                         | 22.4 ms                                                  | 10.4 ms: 2.15x faster                                                    |
| mdp                              | 1.09 sec                                                 | 523 ms: 2.09x faster                                                     |
| scimark_sor                      | 61.0 ms                                                  | 32.9 ms: 1.85x faster                                                    |
| scimark_lu                       | 51.3 ms                                                  | 29.4 ms: 1.74x faster                                                    |
| deepcopy                         | 161 us                                                   | 93.9 us: 1.72x faster                                                    |
| comprehensions                   | 9.84 us                                                  | 5.74 us: 1.71x faster                                                    |
| async_tree_eager_io              | 496 ms                                                   | 293 ms: 1.69x faster                                                     |
| async_tree_none                  | 178 ms                                                   | 108 ms: 1.65x faster                                                     |
| deepcopy_memo                    | 18.3 us                                                  | 11.1 us: 1.65x faster                                                    |
| float                            | 37.9 ms                                                  | 23.6 ms: 1.61x faster                                                    |
| spectral_norm                    | 54.4 ms                                                  | 33.9 ms: 1.61x faster                                                    |
| nbody                            | 54.2 ms                                                  | 34.1 ms: 1.59x faster                                                    |
| async_tree_io                    | 459 ms                                                   | 290 ms: 1.58x faster                                                     |
| async_tree_io_tg                 | 480 ms                                                   | 307 ms: 1.56x faster                                                     |
| logging_silent                   | 50.9 ns                                                  | 32.6 ns: 1.56x faster                                                    |
| tomli_loads                      | 957 ms                                                   | 629 ms: 1.52x faster                                                     |
| async_tree_none_tg               | 172 ms                                                   | 115 ms: 1.49x faster                                                     |
| deepcopy_reduce                  | 1.46 us                                                  | 991 ns: 1.47x faster                                                     |
| scimark_fft                      | 142 ms                                                   | 97.4 ms: 1.46x faster                                                    |
| pyflate                          | 216 ms                                                   | 149 ms: 1.45x faster                                                     |
| go                               | 70.0 ms                                                  | 48.5 ms: 1.44x faster                                                    |
| nqueens                          | 43.5 ms                                                  | 30.3 ms: 1.43x faster                                                    |
| raytrace                         | 145 ms                                                   | 101 ms: 1.43x faster                                                     |
| deltablue                        | 1.73 ms                                                  | 1.21 ms: 1.43x faster                                                    |
| logging_format                   | 2.80 us                                                  | 1.99 us: 1.41x faster                                                    |
| logging_simple                   | 2.57 us                                                  | 1.83 us: 1.41x faster                                                    |
| chaos                            | 28.9 ms                                                  | 20.6 ms: 1.40x faster                                                    |
| async_tree_memoization_tg        | 231 ms                                                   | 165 ms: 1.40x faster                                                     |
| async_tree_eager_io_tg           | 446 ms                                                   | 323 ms: 1.38x faster                                                     |
| regex_compile                    | 54.6 ms                                                  | 39.6 ms: 1.38x faster                                                    |
| scimark_monte_carlo              | 32.2 ms                                                  | 23.6 ms: 1.36x faster                                                    |
| unpickle_pure_python             | 103 us                                                   | 75.7 us: 1.36x faster                                                    |
| async_tree_memoization           | 223 ms                                                   | 166 ms: 1.34x faster                                                     |
| coroutines                       | 13.6 ms                                                  | 10.1 ms: 1.34x faster                                                    |
| pprint_safe_repr                 | 328 ms                                                   | 250 ms: 1.31x faster                                                     |
| pprint_pformat                   | 665 ms                                                   | 507 ms: 1.31x faster                                                     |
| xml_etree_iterparse              | 51.6 ms                                                  | 40.9 ms: 1.26x faster                                                    |
| async_tree_eager                 | 45.6 ms                                                  | 36.4 ms: 1.25x faster                                                    |
| pickle_pure_python               | 139 us                                                   | 112 us: 1.25x faster                                                     |
| hexiom                           | 3.04 ms                                                  | 2.47 ms: 1.23x faster                                                    |
| generators                       | 21.9 ms                                                  | 17.8 ms: 1.23x faster                                                    |
| async_tree_cpu_io_mixed          | 333 ms                                                   | 272 ms: 1.23x faster                                                     |
| bpe_tokeniser                    | 2.24 sec                                                 | 1.84 sec: 1.22x faster                                                   |
| mako                             | 4.77 ms                                                  | 3.93 ms: 1.22x faster                                                    |
| xml_etree_process                | 26.7 ms                                                  | 22.1 ms: 1.21x faster                                                    |
| crypto_pyaes                     | 38.8 ms                                                  | 32.1 ms: 1.21x faster                                                    |
| json_dumps                       | 4.26 ms                                                  | 3.54 ms: 1.20x faster                                                    |
| scimark_sparse_mat_mult          | 2.08 ms                                                  | 1.72 ms: 1.20x faster                                                    |
| fannkuch                         | 176 ms                                                   | 146 ms: 1.20x faster                                                     |
| dulwich_log                      | 21.3 ms                                                  | 17.7 ms: 1.20x faster                                                    |
| xml_etree_generate               | 38.9 ms                                                  | 32.6 ms: 1.19x faster                                                    |
| async_tree_cpu_io_mixed_tg       | 338 ms                                                   | 284 ms: 1.19x faster                                                     |
| k_core                           | 1.12 sec                                                 | 942 ms: 1.19x faster                                                     |
| async_tree_eager_memoization     | 132 ms                                                   | 112 ms: 1.18x faster                                                     |
| typing_runtime_protocols         | 71.0 us                                                  | 60.4 us: 1.18x faster                                                    |
| pathlib                          | 12.4 ms                                                  | 10.6 ms: 1.16x faster                                                    |
| html5lib                         | 23.0 ms                                                  | 19.8 ms: 1.16x faster                                                    |
| async_generators                 | 206 ms                                                   | 183 ms: 1.13x faster                                                     |
| regex_effbot                     | 1.67 ms                                                  | 1.49 ms: 1.12x faster                                                    |
| sympy_str                        | 104 ms                                                   | 95.5 ms: 1.09x faster                                                    |
| sympy_sum                        | 57.6 ms                                                  | 53.0 ms: 1.09x faster                                                    |
| docutils                         | 1.02 sec                                                 | 942 ms: 1.08x faster                                                     |
| django_template                  | 13.6 ms                                                  | 12.6 ms: 1.08x faster                                                    |
| sphinx                           | 434 ms                                                   | 404 ms: 1.07x faster                                                     |
| sympy_integrate                  | 8.02 ms                                                  | 7.51 ms: 1.07x faster                                                    |
| sympy_expand                     | 167 ms                                                   | 157 ms: 1.07x faster                                                     |
| pycparser                        | 497 ms                                                   | 467 ms: 1.06x faster                                                     |
| thrift                           | 322 us                                                   | 305 us: 1.05x faster                                                     |
| sqlite_synth                     | 967 ns                                                   | 925 ns: 1.04x faster                                                     |
| json                             | 1.93 ms                                                  | 1.86 ms: 1.04x faster                                                    |
| async_tree_eager_cpu_io_mixed    | 231 ms                                                   | 222 ms: 1.04x faster                                                     |
| telco                            | 2.61 ms                                                  | 2.53 ms: 1.03x faster                                                    |
| meteor_contest                   | 47.7 ms                                                  | 46.3 ms: 1.03x faster                                                    |
| xml_etree_parse                  | 67.9 ms                                                  | 66.0 ms: 1.03x faster                                                    |
| gc_traversal                     | 2.01 ms                                                  | 1.95 ms: 1.03x faster                                                    |
| regex_dna                        | 99.6 ms                                                  | 96.9 ms: 1.03x faster                                                    |
| json_loads                       | 10.9 us                                                  | 10.6 us: 1.02x faster                                                    |
| dask                             | 528 ms                                                   | 523 ms: 1.01x faster                                                     |
| asyncio_websockets               | 190 ms                                                   | 189 ms: 1.01x faster                                                     |
| connected_components             | 201 ms                                                   | 201 ms: 1.00x slower                                                     |
| bench_thread_pool                | 419 us                                                   | 427 us: 1.02x slower                                                     |
| 2to3                             | 114 ms                                                   | 116 ms: 1.02x slower                                                     |
| pidigits                         | 161 ms                                                   | 165 ms: 1.02x slower                                                     |
| regex_v8                         | 9.59 ms                                                  | 9.86 ms: 1.03x slower                                                    |
| python_startup_no_site           | 5.71 ms                                                  | 5.96 ms: 1.04x slower                                                    |
| create_gc_cycles                 | 830 us                                                   | 872 us: 1.05x slower                                                     |
| python_startup                   | 8.01 ms                                                  | 8.80 ms: 1.10x slower                                                    |
| many_optionals                   | 195 us                                                   | 230 us: 1.18x slower                                                     |
| bench_mp_pool                    | 39.7 ms                                                  | 47.7 ms: 1.20x slower                                                    |
| async_tree_eager_cpu_io_mixed_tg | 213 ms                                                   | 260 ms: 1.23x slower                                                     |
| coverage                         | 26.9 ms                                                  | 33.5 ms: 1.25x slower                                                    |
| async_tree_eager_memoization_tg  | 113 ms                                                   | 149 ms: 1.32x slower                                                     |
| async_tree_eager_tg              | 32.1 ms                                                  | 97.5 ms: 3.04x slower                                                    |
| Geometric mean                   | (ref)                                                    | 1.24x faster                                                             |

Benchmark hidden because not significant (1): shortest_path
Ignored benchmarks (12) of results/bm-20240906-3.12.6-a4a2d2b/bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b.json: chameleon, djangocms, genshi_text, genshi_xml, gevent_hub, sqlalchemy_declarative, sqlalchemy_imperative, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http
Ignored benchmarks (12) of results/bm-20260502-3.15.0a8+-6b632ce-JIT/bm-20260502-macm4pro-arm64-python-6b632ce36b950c7753c5-3.15.0a8+-6b632ce.json: asyncio_tcp, asyncio_tcp_ssl, pickle, pickle_dict, pickle_list, sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile, unpack_sequence, unpickle, unpickle_list

- Geometric mean (including insignificant results): 1.249x faster

# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.19x
- 95% likely to have a speedup of 1.17x
- 99% likely to have a speedup of 1.14x

# Memory
- memory change: 1.26x