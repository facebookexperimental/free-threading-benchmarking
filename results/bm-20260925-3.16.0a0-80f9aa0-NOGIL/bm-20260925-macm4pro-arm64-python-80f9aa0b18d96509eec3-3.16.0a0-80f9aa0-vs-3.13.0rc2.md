# Results vs. 3.13.0rc2

- fork: python
- ref: 80f9aa0b18d96509eec3
- machine: darwin-arm64
- commit hash: 80f9aa0
- commit date: 2026-09-25
- overall geometric mean: 1.071x slower
- HPT reliability: 99.79%
- HPT 99th percentile: 1.02x slower
- Memory change: 1.29x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-macm4pro-arm64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260925-macm4pro-arm64-python-80f9aa0b18d96509eec3-3.16.0a0-80f9aa0 |
|----------------|:--------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| 2to3           | 112 ms                                                         | 137 ms: 1.23x slower                                                    |
| docutils       | 1.05 sec                                                       | 1.10 sec: 1.05x slower                                                  |
| html5lib       | 23.1 ms                                                        | 23.9 ms: 1.03x slower                                                   |
| sphinx         | 409 ms                                                         | 469 ms: 1.15x slower                                                    |
| Geometric mean | (ref)                                                          | 1.11x slower                                                            |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                        | bm-20240906-macm4pro-arm64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260925-macm4pro-arm64-python-80f9aa0b18d96509eec3-3.16.0a0-80f9aa0 |
|----------------------------------|:--------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| async_tree_eager_io_tg           | 521 ms                                                         | 306 ms: 1.70x faster                                                    |
| async_tree_eager_io              | 525 ms                                                         | 316 ms: 1.66x faster                                                    |
| async_tree_io_tg                 | 405 ms                                                         | 310 ms: 1.31x faster                                                    |
| async_tree_io                    | 386 ms                                                         | 321 ms: 1.20x faster                                                    |
| async_generators                 | 193 ms                                                         | 167 ms: 1.16x faster                                                    |
| asyncio_websockets               | 194 ms                                                         | 185 ms: 1.05x faster                                                    |
| async_tree_cpu_io_mixed_tg       | 301 ms                                                         | 306 ms: 1.02x slower                                                    |
| async_tree_cpu_io_mixed          | 294 ms                                                         | 317 ms: 1.08x slower                                                    |
| async_tree_memoization           | 184 ms                                                         | 203 ms: 1.10x slower                                                    |
| async_tree_none_tg               | 133 ms                                                         | 147 ms: 1.11x slower                                                    |
| async_tree_none                  | 142 ms                                                         | 164 ms: 1.15x slower                                                    |
| async_tree_eager_cpu_io_mixed    | 225 ms                                                         | 266 ms: 1.18x slower                                                    |
| coroutines                       | 10.8 ms                                                        | 12.7 ms: 1.19x slower                                                   |
| async_tree_eager_memoization     | 122 ms                                                         | 150 ms: 1.23x slower                                                    |
| async_tree_eager_cpu_io_mixed_tg | 208 ms                                                         | 284 ms: 1.37x slower                                                    |
| async_tree_eager_memoization_tg  | 103 ms                                                         | 167 ms: 1.62x slower                                                    |
| async_tree_eager                 | 43.2 ms                                                        | 71.0 ms: 1.64x slower                                                   |
| async_tree_eager_tg              | 28.9 ms                                                        | 119 ms: 4.11x slower                                                    |
| Geometric mean                   | (ref)                                                          | 1.11x slower                                                            |

Benchmark hidden because not significant (1): async_tree_memoization_tg

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-macm4pro-arm64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260925-macm4pro-arm64-python-80f9aa0b18d96509eec3-3.16.0a0-80f9aa0 |
|----------------|:--------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| pidigits       | 166 ms                                                         | 164 ms: 1.01x faster                                                    |
| float          | 31.4 ms                                                        | 35.2 ms: 1.12x slower                                                   |
| nbody          | 42.5 ms                                                        | 62.3 ms: 1.47x slower                                                   |
| Geometric mean | (ref)                                                          | 1.18x slower                                                            |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-macm4pro-arm64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260925-macm4pro-arm64-python-80f9aa0b18d96509eec3-3.16.0a0-80f9aa0 |
|----------------|:--------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| regex_effbot   | 1.61 ms                                                        | 1.33 ms: 1.21x faster                                                   |
| regex_v8       | 10.7 ms                                                        | 9.20 ms: 1.16x faster                                                   |
| regex_dna      | 94.6 ms                                                        | 91.2 ms: 1.04x faster                                                   |
| regex_compile  | 47.9 ms                                                        | 69.7 ms: 1.45x slower                                                   |
| Geometric mean | (ref)                                                          | 1.00x faster                                                            |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-macm4pro-arm64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260925-macm4pro-arm64-python-80f9aa0b18d96509eec3-3.16.0a0-80f9aa0 |
|----------------------|:--------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| json_dumps           | 4.65 ms                                                        | 3.77 ms: 1.24x faster                                                   |
| xml_etree_iterparse  | 46.1 ms                                                        | 43.2 ms: 1.07x faster                                                   |
| tomli_loads          | 1000 ms                                                        | 949 ms: 1.05x faster                                                    |
| xml_etree_parse      | 62.4 ms                                                        | 60.7 ms: 1.03x faster                                                   |
| json_loads           | 10.8 us                                                        | 11.1 us: 1.03x slower                                                   |
| xml_etree_generate   | 35.8 ms                                                        | 39.5 ms: 1.10x slower                                                   |
| unpickle_pure_python | 99.5 us                                                        | 119 us: 1.19x slower                                                    |
| xml_etree_process    | 25.4 ms                                                        | 31.3 ms: 1.23x slower                                                   |
| pickle_pure_python   | 130 us                                                         | 165 us: 1.26x slower                                                    |
| Geometric mean       | (ref)                                                          | 1.04x slower                                                            |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240906-macm4pro-arm64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260925-macm4pro-arm64-python-80f9aa0b18d96509eec3-3.16.0a0-80f9aa0 |
|------------------------|:--------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| python_startup         | 8.63 ms                                                        | 10.1 ms: 1.17x slower                                                   |
| python_startup_no_site | 5.95 ms                                                        | 7.37 ms: 1.24x slower                                                   |
| Geometric mean         | (ref)                                                          | 1.20x slower                                                            |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240906-macm4pro-arm64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260925-macm4pro-arm64-python-80f9aa0b18d96509eec3-3.16.0a0-80f9aa0 |
|-----------------|:--------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| mako            | 4.41 ms                                                        | 6.02 ms: 1.36x slower                                                   |
| django_template | 12.5 ms                                                        | 18.1 ms: 1.45x slower                                                   |
| Geometric mean  | (ref)                                                          | 1.41x slower                                                            |

All benchmarks:
===============

| Benchmark                        | bm-20240906-macm4pro-arm64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260925-macm4pro-arm64-python-80f9aa0b18d96509eec3-3.16.0a0-80f9aa0 |
|----------------------------------|:--------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| gc_traversal                     | 2.04 ms                                                        | 777 us: 2.63x faster                                                    |
| create_gc_cycles                 | 993 us                                                         | 492 us: 2.02x faster                                                    |
| pylint                           | 106 ms                                                         | 53.9 ms: 1.96x faster                                                   |
| async_tree_eager_io_tg           | 521 ms                                                         | 306 ms: 1.70x faster                                                    |
| mdp                              | 1.06 sec                                                       | 636 ms: 1.66x faster                                                    |
| async_tree_eager_io              | 525 ms                                                         | 316 ms: 1.66x faster                                                    |
| k_core                           | 1.46 sec                                                       | 1.01 sec: 1.45x faster                                                  |
| subparsers                       | 6.26 ms                                                        | 4.59 ms: 1.36x faster                                                   |
| async_tree_io_tg                 | 405 ms                                                         | 310 ms: 1.31x faster                                                    |
| json_dumps                       | 4.65 ms                                                        | 3.77 ms: 1.24x faster                                                   |
| deepcopy                         | 145 us                                                         | 119 us: 1.22x faster                                                    |
| regex_effbot                     | 1.61 ms                                                        | 1.33 ms: 1.21x faster                                                   |
| async_tree_io                    | 386 ms                                                         | 321 ms: 1.20x faster                                                    |
| regex_v8                         | 10.7 ms                                                        | 9.20 ms: 1.16x faster                                                   |
| async_generators                 | 193 ms                                                         | 167 ms: 1.16x faster                                                    |
| sqlite_synth                     | 948 ns                                                         | 820 ns: 1.15x faster                                                    |
| go                               | 72.6 ms                                                        | 67.1 ms: 1.08x faster                                                   |
| xml_etree_iterparse              | 46.1 ms                                                        | 43.2 ms: 1.07x faster                                                   |
| bpe_tokeniser                    | 2.13 sec                                                       | 2.00 sec: 1.06x faster                                                  |
| typing_runtime_protocols         | 64.6 us                                                        | 61.0 us: 1.06x faster                                                   |
| tomli_loads                      | 1000 ms                                                        | 949 ms: 1.05x faster                                                    |
| asyncio_websockets               | 194 ms                                                         | 185 ms: 1.05x faster                                                    |
| pyflate                          | 222 ms                                                         | 213 ms: 1.04x faster                                                    |
| regex_dna                        | 94.6 ms                                                        | 91.2 ms: 1.04x faster                                                   |
| xml_etree_parse                  | 62.4 ms                                                        | 60.7 ms: 1.03x faster                                                   |
| deepcopy_reduce                  | 1.30 us                                                        | 1.27 us: 1.02x faster                                                   |
| pidigits                         | 166 ms                                                         | 164 ms: 1.01x faster                                                    |
| scimark_sor                      | 64.0 ms                                                        | 63.4 ms: 1.01x faster                                                   |
| dulwich_log                      | 19.8 ms                                                        | 19.9 ms: 1.00x slower                                                   |
| deepcopy_memo                    | 16.5 us                                                        | 16.6 us: 1.01x slower                                                   |
| async_tree_cpu_io_mixed_tg       | 301 ms                                                         | 306 ms: 1.02x slower                                                    |
| json                             | 1.94 ms                                                        | 1.99 ms: 1.03x slower                                                   |
| telco                            | 3.07 ms                                                        | 3.15 ms: 1.03x slower                                                   |
| json_loads                       | 10.8 us                                                        | 11.1 us: 1.03x slower                                                   |
| html5lib                         | 23.1 ms                                                        | 23.9 ms: 1.03x slower                                                   |
| docutils                         | 1.05 sec                                                       | 1.10 sec: 1.05x slower                                                  |
| fannkuch                         | 179 ms                                                         | 189 ms: 1.06x slower                                                    |
| pycparser                        | 470 ms                                                         | 504 ms: 1.07x slower                                                    |
| async_tree_cpu_io_mixed          | 294 ms                                                         | 317 ms: 1.08x slower                                                    |
| async_tree_memoization           | 184 ms                                                         | 203 ms: 1.10x slower                                                    |
| xml_etree_generate               | 35.8 ms                                                        | 39.5 ms: 1.10x slower                                                   |
| async_tree_none_tg               | 133 ms                                                         | 147 ms: 1.11x slower                                                    |
| scimark_fft                      | 124 ms                                                         | 138 ms: 1.12x slower                                                    |
| float                            | 31.4 ms                                                        | 35.2 ms: 1.12x slower                                                   |
| sympy_integrate                  | 7.53 ms                                                        | 8.45 ms: 1.12x slower                                                   |
| shortest_path                    | 225 ms                                                         | 254 ms: 1.13x slower                                                    |
| sphinx                           | 409 ms                                                         | 469 ms: 1.15x slower                                                    |
| meteor_contest                   | 47.9 ms                                                        | 55.1 ms: 1.15x slower                                                   |
| async_tree_none                  | 142 ms                                                         | 164 ms: 1.15x slower                                                    |
| thrift                           | 309 us                                                         | 358 us: 1.16x slower                                                    |
| bench_mp_pool                    | 37.8 ms                                                        | 44.0 ms: 1.16x slower                                                   |
| connected_components             | 208 ms                                                         | 243 ms: 1.17x slower                                                    |
| python_startup                   | 8.63 ms                                                        | 10.1 ms: 1.17x slower                                                   |
| logging_simple                   | 2.24 us                                                        | 2.64 us: 1.18x slower                                                   |
| async_tree_eager_cpu_io_mixed    | 225 ms                                                         | 266 ms: 1.18x slower                                                    |
| logging_format                   | 2.45 us                                                        | 2.89 us: 1.18x slower                                                   |
| coroutines                       | 10.8 ms                                                        | 12.7 ms: 1.19x slower                                                   |
| pprint_safe_repr                 | 322 ms                                                         | 383 ms: 1.19x slower                                                    |
| unpickle_pure_python             | 99.5 us                                                        | 119 us: 1.19x slower                                                    |
| sympy_sum                        | 52.3 ms                                                        | 62.6 ms: 1.20x slower                                                   |
| scimark_monte_carlo              | 29.9 ms                                                        | 35.8 ms: 1.20x slower                                                   |
| nqueens                          | 37.2 ms                                                        | 44.7 ms: 1.20x slower                                                   |
| spectral_norm                    | 43.7 ms                                                        | 52.7 ms: 1.21x slower                                                   |
| sympy_str                        | 95.5 ms                                                        | 115 ms: 1.21x slower                                                    |
| richards                         | 22.1 ms                                                        | 26.7 ms: 1.21x slower                                                   |
| pprint_pformat                   | 650 ms                                                         | 789 ms: 1.21x slower                                                    |
| richards_super                   | 24.7 ms                                                        | 30.2 ms: 1.22x slower                                                   |
| sympy_expand                     | 159 ms                                                         | 195 ms: 1.23x slower                                                    |
| 2to3                             | 112 ms                                                         | 137 ms: 1.23x slower                                                    |
| async_tree_eager_memoization     | 122 ms                                                         | 150 ms: 1.23x slower                                                    |
| xml_etree_process                | 25.4 ms                                                        | 31.3 ms: 1.23x slower                                                   |
| python_startup_no_site           | 5.95 ms                                                        | 7.37 ms: 1.24x slower                                                   |
| scimark_sparse_mat_mult          | 1.78 ms                                                        | 2.21 ms: 1.24x slower                                                   |
| hexiom                           | 2.85 ms                                                        | 3.56 ms: 1.25x slower                                                   |
| crypto_pyaes                     | 33.6 ms                                                        | 42.4 ms: 1.26x slower                                                   |
| pickle_pure_python               | 130 us                                                         | 165 us: 1.26x slower                                                    |
| chaos                            | 24.3 ms                                                        | 31.0 ms: 1.28x slower                                                   |
| logging_silent                   | 40.6 ns                                                        | 51.8 ns: 1.28x slower                                                   |
| coverage                         | 31.2 ms                                                        | 40.4 ms: 1.29x slower                                                   |
| comprehensions                   | 6.80 us                                                        | 8.93 us: 1.31x slower                                                   |
| raytrace                         | 109 ms                                                         | 148 ms: 1.36x slower                                                    |
| mako                             | 4.41 ms                                                        | 6.02 ms: 1.36x slower                                                   |
| async_tree_eager_cpu_io_mixed_tg | 208 ms                                                         | 284 ms: 1.37x slower                                                    |
| many_optionals                   | 200 us                                                         | 275 us: 1.37x slower                                                    |
| deltablue                        | 1.45 ms                                                        | 2.02 ms: 1.40x slower                                                   |
| bench_thread_pool                | 412 us                                                         | 589 us: 1.43x slower                                                    |
| django_template                  | 12.5 ms                                                        | 18.1 ms: 1.45x slower                                                   |
| regex_compile                    | 47.9 ms                                                        | 69.7 ms: 1.45x slower                                                   |
| nbody                            | 42.5 ms                                                        | 62.3 ms: 1.47x slower                                                   |
| generators                       | 15.7 ms                                                        | 24.8 ms: 1.58x slower                                                   |
| scimark_lu                       | 42.8 ms                                                        | 69.3 ms: 1.62x slower                                                   |
| async_tree_eager_memoization_tg  | 103 ms                                                         | 167 ms: 1.62x slower                                                    |
| async_tree_eager                 | 43.2 ms                                                        | 71.0 ms: 1.64x slower                                                   |
| async_tree_eager_tg              | 28.9 ms                                                        | 119 ms: 4.11x slower                                                    |
| Geometric mean                   | (ref)                                                          | 1.08x slower                                                            |

Benchmark hidden because not significant (2): pathlib, async_tree_memoization_tg
Ignored benchmarks (13) of results/bm-20240906-3.13.0rc2-ec61006/bm-20240906-macm4pro-arm64-python-v3.13.0rc2-3.13.0rc2-ec61006.json: chameleon, dask, djangocms, genshi_text, genshi_xml, gevent_hub, sqlalchemy_declarative, sqlalchemy_imperative, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http
Ignored benchmarks (4) of results/bm-20260925-3.16.0a0-80f9aa0-NOGIL/bm-20260925-macm4pro-arm64-python-80f9aa0b18d96509eec3-3.16.0a0-80f9aa0.json: sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile

- Geometric mean (including insignificant results): 1.071x slower

# HPT report

- Reliability score: 99.79% likely to be slow
- 90% likely to have a slowdown of 1.06x
- 95% likely to have a slowdown of 1.05x
- 99% likely to have a slowdown of 1.02x

# Memory
- memory change: 1.29x