# Results vs. 3.12.6

- fork: python
- ref: 7a918411a300ddeef06d
- machine: linux-x86_64
- commit hash: 7a91841
- commit date: 2026-09-05
- overall geometric mean: 1.165x faster
- HPT reliability: 100.00%
- HPT 99th percentile: 1.09x faster
- Memory change: 1.20x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260905-vultr-x86_64-python-7a918411a300ddeef06d-3.16.0a0-7a91841 |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| 2to3           | 264 ms                                                 | 259 ms: 1.02x faster                                                  |
| docutils       | 2.64 sec                                               | 2.49 sec: 1.06x faster                                                |
| html5lib       | 63.6 ms                                                | 60.4 ms: 1.05x faster                                                 |
| Geometric mean | (ref)                                                  | 1.04x faster                                                          |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260905-vultr-x86_64-python-7a918411a300ddeef06d-3.16.0a0-7a91841 |
|----------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| async_tree_none            | 464 ms                                                 | 269 ms: 1.73x faster                                                  |
| async_tree_memoization_tg  | 560 ms                                                 | 339 ms: 1.65x faster                                                  |
| async_tree_io              | 1.08 sec                                               | 678 ms: 1.60x faster                                                  |
| async_tree_memoization     | 555 ms                                                 | 348 ms: 1.59x faster                                                  |
| async_tree_none_tg         | 446 ms                                                 | 280 ms: 1.59x faster                                                  |
| async_tree_io_tg           | 1.11 sec                                               | 722 ms: 1.54x faster                                                  |
| async_tree_cpu_io_mixed    | 715 ms                                                 | 523 ms: 1.37x faster                                                  |
| async_tree_cpu_io_mixed_tg | 723 ms                                                 | 535 ms: 1.35x faster                                                  |
| asyncio_tcp                | 519 ms                                                 | 445 ms: 1.17x faster                                                  |
| async_generators           | 384 ms                                                 | 359 ms: 1.07x faster                                                  |
| asyncio_tcp_ssl            | 1.51 sec                                               | 1.47 sec: 1.03x faster                                                |
| asyncio_websockets         | 517 ms                                                 | 544 ms: 1.05x slower                                                  |
| Geometric mean             | (ref)                                                  | 1.33x faster                                                          |

Benchmark hidden because not significant (1): coroutines

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260905-vultr-x86_64-python-7a918411a300ddeef06d-3.16.0a0-7a91841 |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| float          | 80.8 ms                                                | 55.4 ms: 1.46x faster                                                 |
| nbody          | 89.3 ms                                                | 62.4 ms: 1.43x faster                                                 |
| pidigits       | 184 ms                                                 | 183 ms: 1.00x faster                                                  |
| Geometric mean | (ref)                                                  | 1.28x faster                                                          |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260905-vultr-x86_64-python-7a918411a300ddeef06d-3.16.0a0-7a91841 |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| regex_effbot   | 3.17 ms                                                | 2.72 ms: 1.17x faster                                                 |
| regex_dna      | 168 ms                                                 | 165 ms: 1.02x faster                                                  |
| regex_v8       | 20.6 ms                                                | 21.3 ms: 1.03x slower                                                 |
| Geometric mean | (ref)                                                  | 1.04x faster                                                          |

Benchmark hidden because not significant (1): regex_compile

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260905-vultr-x86_64-python-7a918411a300ddeef06d-3.16.0a0-7a91841 |
|----------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| unpickle_pure_python | 221 us                                                 | 182 us: 1.21x faster                                                  |
| pickle_pure_python   | 308 us                                                 | 260 us: 1.18x faster                                                  |
| json_dumps           | 10.4 ms                                                | 8.83 ms: 1.17x faster                                                 |
| tomli_loads          | 2.11 sec                                               | 1.80 sec: 1.17x faster                                                |
| xml_etree_iterparse  | 96.7 ms                                                | 89.2 ms: 1.08x faster                                                 |
| json_loads           | 26.5 us                                                | 24.8 us: 1.07x faster                                                 |
| xml_etree_process    | 59.0 ms                                                | 58.4 ms: 1.01x faster                                                 |
| xml_etree_generate   | 85.2 ms                                                | 84.5 ms: 1.01x faster                                                 |
| xml_etree_parse      | 139 ms                                                 | 144 ms: 1.04x slower                                                  |
| pickle_dict          | 31.8 us                                                | 33.4 us: 1.05x slower                                                 |
| unpickle             | 14.1 us                                                | 15.1 us: 1.08x slower                                                 |
| unpickle_list        | 4.67 us                                                | 5.27 us: 1.13x slower                                                 |
| pickle_list          | 4.77 us                                                | 5.73 us: 1.20x slower                                                 |
| pickle               | 10.9 us                                                | 13.2 us: 1.21x slower                                                 |
| Geometric mean       | (ref)                                                  | 1.01x faster                                                          |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260905-vultr-x86_64-python-7a918411a300ddeef06d-3.16.0a0-7a91841 |
|------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| python_startup_no_site | 7.16 ms                                                | 7.88 ms: 1.10x slower                                                 |
| python_startup         | 9.93 ms                                                | 13.2 ms: 1.33x slower                                                 |
| Geometric mean         | (ref)                                                  | 1.21x slower                                                          |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260905-vultr-x86_64-python-7a918411a300ddeef06d-3.16.0a0-7a91841 |
|-----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| mako            | 11.0 ms                                                | 9.99 ms: 1.10x faster                                                 |
| django_template | 34.7 ms                                                | 40.9 ms: 1.18x slower                                                 |
| Geometric mean  | (ref)                                                  | 1.03x slower                                                          |

All benchmarks:
===============

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260905-vultr-x86_64-python-7a918411a300ddeef06d-3.16.0a0-7a91841 |
|----------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| pylint                     | 319 ms                                                 | 115 ms: 2.77x faster                                                  |
| richards_super             | 51.9 ms                                                | 20.8 ms: 2.49x faster                                                 |
| richards                   | 45.9 ms                                                | 19.0 ms: 2.42x faster                                                 |
| mdp                        | 2.42 sec                                               | 1.22 sec: 1.98x faster                                                |
| deepcopy_memo              | 40.3 us                                                | 22.1 us: 1.82x faster                                                 |
| async_tree_none            | 464 ms                                                 | 269 ms: 1.73x faster                                                  |
| scimark_sor                | 130 ms                                                 | 77.1 ms: 1.68x faster                                                 |
| async_tree_memoization_tg  | 560 ms                                                 | 339 ms: 1.65x faster                                                  |
| async_tree_io              | 1.08 sec                                               | 678 ms: 1.60x faster                                                  |
| async_tree_memoization     | 555 ms                                                 | 348 ms: 1.59x faster                                                  |
| async_tree_none_tg         | 446 ms                                                 | 280 ms: 1.59x faster                                                  |
| spectral_norm              | 110 ms                                                 | 69.6 ms: 1.58x faster                                                 |
| async_tree_io_tg           | 1.11 sec                                               | 722 ms: 1.54x faster                                                  |
| typing_runtime_protocols   | 163 us                                                 | 109 us: 1.50x faster                                                  |
| deepcopy                   | 352 us                                                 | 236 us: 1.49x faster                                                  |
| float                      | 80.8 ms                                                | 55.4 ms: 1.46x faster                                                 |
| nbody                      | 89.3 ms                                                | 62.4 ms: 1.43x faster                                                 |
| go                         | 139 ms                                                 | 98.5 ms: 1.41x faster                                                 |
| comprehensions             | 19.8 us                                                | 14.3 us: 1.38x faster                                                 |
| async_tree_cpu_io_mixed    | 715 ms                                                 | 523 ms: 1.37x faster                                                  |
| scimark_lu                 | 114 ms                                                 | 84.2 ms: 1.36x faster                                                 |
| async_tree_cpu_io_mixed_tg | 723 ms                                                 | 535 ms: 1.35x faster                                                  |
| scimark_fft                | 342 ms                                                 | 255 ms: 1.34x faster                                                  |
| chaos                      | 62.8 ms                                                | 47.2 ms: 1.33x faster                                                 |
| scimark_monte_carlo        | 68.4 ms                                                | 52.2 ms: 1.31x faster                                                 |
| deltablue                  | 3.45 ms                                                | 2.70 ms: 1.28x faster                                                 |
| pyflate                    | 448 ms                                                 | 355 ms: 1.26x faster                                                  |
| logging_format             | 7.35 us                                                | 5.97 us: 1.23x faster                                                 |
| nqueens                    | 80.1 ms                                                | 65.2 ms: 1.23x faster                                                 |
| logging_simple             | 6.63 us                                                | 5.40 us: 1.23x faster                                                 |
| crypto_pyaes               | 76.6 ms                                                | 62.4 ms: 1.23x faster                                                 |
| pathlib                    | 21.5 ms                                                | 17.6 ms: 1.22x faster                                                 |
| raytrace                   | 299 ms                                                 | 245 ms: 1.22x faster                                                  |
| unpickle_pure_python       | 221 us                                                 | 182 us: 1.21x faster                                                  |
| pprint_safe_repr           | 743 ms                                                 | 615 ms: 1.21x faster                                                  |
| deepcopy_reduce            | 3.08 us                                                | 2.57 us: 1.20x faster                                                 |
| bpe_tokeniser              | 4.74 sec                                               | 3.97 sec: 1.19x faster                                                |
| pprint_pformat             | 1.52 sec                                               | 1.28 sec: 1.19x faster                                                |
| pickle_pure_python         | 308 us                                                 | 260 us: 1.18x faster                                                  |
| json_dumps                 | 10.4 ms                                                | 8.83 ms: 1.17x faster                                                 |
| tomli_loads                | 2.11 sec                                               | 1.80 sec: 1.17x faster                                                |
| asyncio_tcp                | 519 ms                                                 | 445 ms: 1.17x faster                                                  |
| regex_effbot               | 3.17 ms                                                | 2.72 ms: 1.17x faster                                                 |
| dulwich_log                | 78.9 ms                                                | 69.9 ms: 1.13x faster                                                 |
| json                       | 5.02 ms                                                | 4.54 ms: 1.11x faster                                                 |
| fannkuch                   | 372 ms                                                 | 336 ms: 1.11x faster                                                  |
| generators                 | 32.2 ms                                                | 29.1 ms: 1.11x faster                                                 |
| logging_silent             | 109 ns                                                 | 98.8 ns: 1.10x faster                                                 |
| mako                       | 11.0 ms                                                | 9.99 ms: 1.10x faster                                                 |
| scimark_sparse_mat_mult    | 4.39 ms                                                | 4.01 ms: 1.10x faster                                                 |
| hexiom                     | 6.17 ms                                                | 5.64 ms: 1.09x faster                                                 |
| xml_etree_iterparse        | 96.7 ms                                                | 89.2 ms: 1.08x faster                                                 |
| async_generators           | 384 ms                                                 | 359 ms: 1.07x faster                                                  |
| json_loads                 | 26.5 us                                                | 24.8 us: 1.07x faster                                                 |
| docutils                   | 2.64 sec                                               | 2.49 sec: 1.06x faster                                                |
| meteor_contest             | 104 ms                                                 | 97.8 ms: 1.06x faster                                                 |
| html5lib                   | 63.6 ms                                                | 60.4 ms: 1.05x faster                                                 |
| sqlite_synth               | 2.20 us                                                | 2.13 us: 1.04x faster                                                 |
| asyncio_tcp_ssl            | 1.51 sec                                               | 1.47 sec: 1.03x faster                                                |
| sympy_integrate            | 20.5 ms                                                | 20.1 ms: 1.02x faster                                                 |
| regex_dna                  | 168 ms                                                 | 165 ms: 1.02x faster                                                  |
| 2to3                       | 264 ms                                                 | 259 ms: 1.02x faster                                                  |
| thrift                     | 791 us                                                 | 780 us: 1.01x faster                                                  |
| xml_etree_process          | 59.0 ms                                                | 58.4 ms: 1.01x faster                                                 |
| xml_etree_generate         | 85.2 ms                                                | 84.5 ms: 1.01x faster                                                 |
| pidigits                   | 184 ms                                                 | 183 ms: 1.00x faster                                                  |
| sympy_expand               | 468 ms                                                 | 472 ms: 1.01x slower                                                  |
| sympy_sum                  | 166 ms                                                 | 169 ms: 1.02x slower                                                  |
| regex_v8                   | 20.6 ms                                                | 21.3 ms: 1.03x slower                                                 |
| xml_etree_parse            | 139 ms                                                 | 144 ms: 1.04x slower                                                  |
| pickle_dict                | 31.8 us                                                | 33.4 us: 1.05x slower                                                 |
| asyncio_websockets         | 517 ms                                                 | 544 ms: 1.05x slower                                                  |
| unpickle                   | 14.1 us                                                | 15.1 us: 1.08x slower                                                 |
| python_startup_no_site     | 7.16 ms                                                | 7.88 ms: 1.10x slower                                                 |
| unpickle_list              | 4.67 us                                                | 5.27 us: 1.13x slower                                                 |
| coverage                   | 71.4 ms                                                | 82.6 ms: 1.16x slower                                                 |
| gc_traversal               | 3.46 ms                                                | 4.07 ms: 1.18x slower                                                 |
| django_template            | 34.7 ms                                                | 40.9 ms: 1.18x slower                                                 |
| pickle_list                | 4.77 us                                                | 5.73 us: 1.20x slower                                                 |
| pickle                     | 10.9 us                                                | 13.2 us: 1.21x slower                                                 |
| unpack_sequence            | 52.1 ns                                                | 63.3 ns: 1.22x slower                                                 |
| python_startup             | 9.93 ms                                                | 13.2 ms: 1.33x slower                                                 |
| bench_thread_pool          | 941 us                                                 | 1.32 ms: 1.40x slower                                                 |
| create_gc_cycles           | 1.09 ms                                                | 1.69 ms: 1.55x slower                                                 |
| bench_mp_pool              | 10.8 ms                                                | 183 ms: 16.92x slower                                                 |
| telco                      | 6.53 ms                                                | 135 ms: 20.70x slower                                                 |
| Geometric mean             | (ref)                                                  | 1.10x faster                                                          |

Benchmark hidden because not significant (4): pycparser, regex_compile, coroutines, sympy_str
Ignored benchmarks (15) of results/bm-20240906-3.12.6-a4a2d2b/bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b.json: aiohttp, chameleon, dask, flaskblogging, genshi_text, genshi_xml, gunicorn, mypy2, sqlalchemy_declarative, sqlalchemy_imperative, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http
Ignored benchmarks (10) of results/bm-20260905-3.16.0a0-7a91841-JIT/bm-20260905-vultr-x86_64-python-7a918411a300ddeef06d-3.16.0a0-7a91841.json: connected_components, k_core, many_optionals, shortest_path, sphinx, sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile, subparsers

- Geometric mean (including insignificant results): 1.165x faster

# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.12x
- 95% likely to have a speedup of 1.11x
- 99% likely to have a speedup of 1.09x

# Memory
- memory change: 1.20x