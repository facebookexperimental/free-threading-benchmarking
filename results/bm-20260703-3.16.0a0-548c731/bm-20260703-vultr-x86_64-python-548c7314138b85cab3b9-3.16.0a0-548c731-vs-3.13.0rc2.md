# Results vs. 3.13.0rc2

- fork: python
- ref: 548c7314138b85cab3b9
- machine: linux-x86_64
- commit hash: 548c731
- commit date: 2026-07-03
- overall geometric mean: 1.037x faster
- HPT reliability: 100.00%
- HPT 99th percentile: 1.01x faster
- Memory change: 1.13x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5 | bm-20260703-vultr-x86_64-python-548c7314138b85cab3b9-3.16.0a0-548c731 |
|----------------|:----------------------------------------------------------------------:|:---------------------------------------------------------------------:|
| 2to3           | 259 ms                                                                 | 257 ms: 1.01x faster                                                  |
| docutils       | 2.63 sec                                                               | 2.39 sec: 1.10x faster                                                |
| html5lib       | 68.6 ms                                                                | 60.0 ms: 1.14x faster                                                 |
| Geometric mean | (ref)                                                                  | 1.08x faster                                                          |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5 | bm-20260703-vultr-x86_64-python-548c7314138b85cab3b9-3.16.0a0-548c731 |
|----------------------------|:----------------------------------------------------------------------:|:---------------------------------------------------------------------:|
| async_tree_io              | 881 ms                                                                 | 711 ms: 1.24x faster                                                  |
| async_tree_cpu_io_mixed    | 666 ms                                                                 | 540 ms: 1.23x faster                                                  |
| async_tree_memoization     | 459 ms                                                                 | 377 ms: 1.22x faster                                                  |
| async_tree_none            | 353 ms                                                                 | 292 ms: 1.21x faster                                                  |
| async_tree_io_tg           | 901 ms                                                                 | 761 ms: 1.18x faster                                                  |
| async_tree_cpu_io_mixed_tg | 634 ms                                                                 | 556 ms: 1.14x faster                                                  |
| async_tree_memoization_tg  | 410 ms                                                                 | 366 ms: 1.12x faster                                                  |
| async_tree_none_tg         | 333 ms                                                                 | 301 ms: 1.11x faster                                                  |
| async_generators           | 375 ms                                                                 | 344 ms: 1.09x faster                                                  |
| coroutines                 | 23.3 ms                                                                | 23.5 ms: 1.01x slower                                                 |
| asyncio_websockets         | 517 ms                                                                 | 544 ms: 1.05x slower                                                  |
| Geometric mean             | (ref)                                                                  | 1.13x faster                                                          |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5 | bm-20260703-vultr-x86_64-python-548c7314138b85cab3b9-3.16.0a0-548c731 |
|----------------|:----------------------------------------------------------------------:|:---------------------------------------------------------------------:|
| pidigits       | 216 ms                                                                 | 187 ms: 1.15x faster                                                  |
| float          | 76.7 ms                                                                | 73.5 ms: 1.04x faster                                                 |
| nbody          | 85.3 ms                                                                | 95.5 ms: 1.12x slower                                                 |
| Geometric mean | (ref)                                                                  | 1.03x faster                                                          |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5 | bm-20260703-vultr-x86_64-python-548c7314138b85cab3b9-3.16.0a0-548c731 |
|----------------|:----------------------------------------------------------------------:|:---------------------------------------------------------------------:|
| regex_effbot   | 3.21 ms                                                                | 2.74 ms: 1.17x faster                                                 |
| regex_v8       | 23.2 ms                                                                | 22.0 ms: 1.06x faster                                                 |
| regex_dna      | 189 ms                                                                 | 187 ms: 1.01x faster                                                  |
| regex_compile  | 131 ms                                                                 | 147 ms: 1.12x slower                                                  |
| Geometric mean | (ref)                                                                  | 1.03x faster                                                          |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5 | bm-20260703-vultr-x86_64-python-548c7314138b85cab3b9-3.16.0a0-548c731 |
|----------------------|:----------------------------------------------------------------------:|:---------------------------------------------------------------------:|
| tomli_loads          | 2.01 sec                                                               | 1.83 sec: 1.10x faster                                                |
| json_dumps           | 10.6 ms                                                                | 9.85 ms: 1.08x faster                                                 |
| xml_etree_iterparse  | 94.4 ms                                                                | 89.1 ms: 1.06x faster                                                 |
| unpickle_pure_python | 208 us                                                                 | 210 us: 1.01x slower                                                  |
| xml_etree_process    | 59.2 ms                                                                | 60.6 ms: 1.02x slower                                                 |
| xml_etree_parse      | 136 ms                                                                 | 144 ms: 1.06x slower                                                  |
| pickle_pure_python   | 292 us                                                                 | 309 us: 1.06x slower                                                  |
| Geometric mean       | (ref)                                                                  | 1.01x faster                                                          |

Benchmark hidden because not significant (2): json_loads, xml_etree_generate

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5 | bm-20260703-vultr-x86_64-python-548c7314138b85cab3b9-3.16.0a0-548c731 |
|------------------------|:----------------------------------------------------------------------:|:---------------------------------------------------------------------:|
| python_startup_no_site | 7.41 ms                                                                | 6.93 ms: 1.07x faster                                                 |
| python_startup         | 11.0 ms                                                                | 12.3 ms: 1.11x slower                                                 |
| Geometric mean         | (ref)                                                                  | 1.02x slower                                                          |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5 | bm-20260703-vultr-x86_64-python-548c7314138b85cab3b9-3.16.0a0-548c731 |
|-----------------|:----------------------------------------------------------------------:|:---------------------------------------------------------------------:|
| django_template | 34.2 ms                                                                | 35.6 ms: 1.04x slower                                                 |
| mako            | 11.2 ms                                                                | 12.0 ms: 1.07x slower                                                 |
| Geometric mean  | (ref)                                                                  | 1.05x slower                                                          |

All benchmarks:
===============

| Benchmark                  | bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5 | bm-20260703-vultr-x86_64-python-548c7314138b85cab3b9-3.16.0a0-548c731 |
|----------------------------|:----------------------------------------------------------------------:|:---------------------------------------------------------------------:|
| pylint                     | 317 ms                                                                 | 114 ms: 2.79x faster                                                  |
| mdp                        | 2.34 sec                                                               | 1.13 sec: 2.06x faster                                                |
| deepcopy                   | 357 us                                                                 | 229 us: 1.56x faster                                                  |
| deepcopy_memo              | 38.1 us                                                                | 26.5 us: 1.44x faster                                                 |
| go                         | 141 ms                                                                 | 103 ms: 1.36x faster                                                  |
| typing_runtime_protocols   | 156 us                                                                 | 116 us: 1.34x faster                                                  |
| deepcopy_reduce            | 3.12 us                                                                | 2.49 us: 1.25x faster                                                 |
| scimark_sor                | 134 ms                                                                 | 108 ms: 1.24x faster                                                  |
| async_tree_io              | 881 ms                                                                 | 711 ms: 1.24x faster                                                  |
| async_tree_cpu_io_mixed    | 666 ms                                                                 | 540 ms: 1.23x faster                                                  |
| async_tree_memoization     | 459 ms                                                                 | 377 ms: 1.22x faster                                                  |
| async_tree_none            | 353 ms                                                                 | 292 ms: 1.21x faster                                                  |
| async_tree_io_tg           | 901 ms                                                                 | 761 ms: 1.18x faster                                                  |
| regex_effbot               | 3.21 ms                                                                | 2.74 ms: 1.17x faster                                                 |
| pyflate                    | 449 ms                                                                 | 384 ms: 1.17x faster                                                  |
| pidigits                   | 216 ms                                                                 | 187 ms: 1.15x faster                                                  |
| html5lib                   | 68.6 ms                                                                | 60.0 ms: 1.14x faster                                                 |
| async_tree_cpu_io_mixed_tg | 634 ms                                                                 | 556 ms: 1.14x faster                                                  |
| spectral_norm              | 108 ms                                                                 | 94.4 ms: 1.14x faster                                                 |
| async_tree_memoization_tg  | 410 ms                                                                 | 366 ms: 1.12x faster                                                  |
| dulwich_log                | 74.5 ms                                                                | 67.0 ms: 1.11x faster                                                 |
| scimark_fft                | 348 ms                                                                 | 314 ms: 1.11x faster                                                  |
| async_tree_none_tg         | 333 ms                                                                 | 301 ms: 1.11x faster                                                  |
| docutils                   | 2.63 sec                                                               | 2.39 sec: 1.10x faster                                                |
| tomli_loads                | 2.01 sec                                                               | 1.83 sec: 1.10x faster                                                |
| async_generators           | 375 ms                                                                 | 344 ms: 1.09x faster                                                  |
| pathlib                    | 19.3 ms                                                                | 17.7 ms: 1.09x faster                                                 |
| json_dumps                 | 10.6 ms                                                                | 9.85 ms: 1.08x faster                                                 |
| python_startup_no_site     | 7.41 ms                                                                | 6.93 ms: 1.07x faster                                                 |
| bpe_tokeniser              | 4.46 sec                                                               | 4.20 sec: 1.06x faster                                                |
| xml_etree_iterparse        | 94.4 ms                                                                | 89.1 ms: 1.06x faster                                                 |
| regex_v8                   | 23.2 ms                                                                | 22.0 ms: 1.06x faster                                                 |
| nqueens                    | 77.7 ms                                                                | 73.6 ms: 1.06x faster                                                 |
| comprehensions             | 16.6 us                                                                | 15.7 us: 1.05x faster                                                 |
| richards                   | 44.4 ms                                                                | 42.5 ms: 1.05x faster                                                 |
| float                      | 76.7 ms                                                                | 73.5 ms: 1.04x faster                                                 |
| chaos                      | 56.3 ms                                                                | 54.0 ms: 1.04x faster                                                 |
| richards_super             | 50.4 ms                                                                | 48.4 ms: 1.04x faster                                                 |
| hexiom                     | 5.95 ms                                                                | 5.74 ms: 1.04x faster                                                 |
| sympy_integrate            | 19.7 ms                                                                | 19.1 ms: 1.03x faster                                                 |
| scimark_monte_carlo        | 65.8 ms                                                                | 63.8 ms: 1.03x faster                                                 |
| logging_simple             | 6.14 us                                                                | 5.97 us: 1.03x faster                                                 |
| scimark_sparse_mat_mult    | 4.75 ms                                                                | 4.63 ms: 1.03x faster                                                 |
| logging_format             | 6.92 us                                                                | 6.76 us: 1.02x faster                                                 |
| sqlite_synth               | 2.25 us                                                                | 2.22 us: 1.01x faster                                                 |
| meteor_contest             | 101 ms                                                                 | 100.0 ms: 1.01x faster                                                |
| pprint_safe_repr           | 719 ms                                                                 | 709 ms: 1.01x faster                                                  |
| fannkuch                   | 376 ms                                                                 | 371 ms: 1.01x faster                                                  |
| thrift                     | 772 us                                                                 | 762 us: 1.01x faster                                                  |
| 2to3                       | 259 ms                                                                 | 257 ms: 1.01x faster                                                  |
| generators                 | 28.5 ms                                                                | 28.3 ms: 1.01x faster                                                 |
| json                       | 4.98 ms                                                                | 4.94 ms: 1.01x faster                                                 |
| pycparser                  | 1.12 sec                                                               | 1.11 sec: 1.01x faster                                                |
| regex_dna                  | 189 ms                                                                 | 187 ms: 1.01x faster                                                  |
| pprint_pformat             | 1.46 sec                                                               | 1.44 sec: 1.01x faster                                                |
| sympy_str                  | 274 ms                                                                 | 272 ms: 1.01x faster                                                  |
| sympy_expand               | 454 ms                                                                 | 456 ms: 1.01x slower                                                  |
| coroutines                 | 23.3 ms                                                                | 23.5 ms: 1.01x slower                                                 |
| unpickle_pure_python       | 208 us                                                                 | 210 us: 1.01x slower                                                  |
| logging_silent             | 98.2 ns                                                                | 99.2 ns: 1.01x slower                                                 |
| scimark_lu                 | 112 ms                                                                 | 114 ms: 1.01x slower                                                  |
| sympy_sum                  | 154 ms                                                                 | 157 ms: 1.02x slower                                                  |
| crypto_pyaes               | 68.2 ms                                                                | 69.3 ms: 1.02x slower                                                 |
| xml_etree_process          | 59.2 ms                                                                | 60.6 ms: 1.02x slower                                                 |
| django_template            | 34.2 ms                                                                | 35.6 ms: 1.04x slower                                                 |
| asyncio_websockets         | 517 ms                                                                 | 544 ms: 1.05x slower                                                  |
| deltablue                  | 3.10 ms                                                                | 3.28 ms: 1.06x slower                                                 |
| xml_etree_parse            | 136 ms                                                                 | 144 ms: 1.06x slower                                                  |
| pickle_pure_python         | 292 us                                                                 | 309 us: 1.06x slower                                                  |
| mako                       | 11.2 ms                                                                | 12.0 ms: 1.07x slower                                                 |
| python_startup             | 11.0 ms                                                                | 12.3 ms: 1.11x slower                                                 |
| nbody                      | 85.3 ms                                                                | 95.5 ms: 1.12x slower                                                 |
| regex_compile              | 131 ms                                                                 | 147 ms: 1.12x slower                                                  |
| gc_traversal               | 3.32 ms                                                                | 3.73 ms: 1.12x slower                                                 |
| create_gc_cycles           | 1.33 ms                                                                | 1.68 ms: 1.26x slower                                                 |
| bench_thread_pool          | 924 us                                                                 | 1.34 ms: 1.45x slower                                                 |
| telco                      | 7.77 ms                                                                | 158 ms: 20.29x slower                                                 |
| bench_mp_pool              | 11.0 ms                                                                | 276 ms: 25.11x slower                                                 |
| Geometric mean             | (ref)                                                                  | 1.01x slower                                                          |

Benchmark hidden because not significant (4): json_loads, raytrace, coverage, xml_etree_generate
Ignored benchmarks (20) of results/bm-20240920-3.13.0rc2-4981ec5/bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5.json: aiohttp, asyncio_tcp, asyncio_tcp_ssl, chameleon, dask, flaskblogging, genshi_text, genshi_xml, gunicorn, pickle, pickle_dict, pickle_list, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http, unpack_sequence, unpickle, unpickle_list
Ignored benchmarks (10) of results/bm-20260703-3.16.0a0-548c731/bm-20260703-vultr-x86_64-python-548c7314138b85cab3b9-3.16.0a0-548c731.json: connected_components, k_core, many_optionals, shortest_path, sphinx, sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile, subparsers

- Geometric mean (including insignificant results): 1.037x faster

# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.02x
- 95% likely to have a speedup of 1.01x
- 99% likely to have a speedup of 1.01x

# Memory
- memory change: 1.13x