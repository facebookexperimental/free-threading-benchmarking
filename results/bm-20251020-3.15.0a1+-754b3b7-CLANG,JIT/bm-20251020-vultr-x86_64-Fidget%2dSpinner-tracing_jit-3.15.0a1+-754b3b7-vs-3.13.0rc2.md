# Results vs. 3.13.0rc2

- fork: Fidget-Spinner
- ref: tracing_jit
- machine: linux-x86_64
- commit hash: 754b3b7
- commit date: 2025-10-20
- overall geometric mean: 1.068x faster
- HPT reliability: 100.00%
- HPT 99th percentile: 1.02x faster
- Memory change: 1.20x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5 | bm-20251020-vultr-x86_64-Fidget%2dSpinner-tracing_jit-3.15.0a1+-754b3b7 |
|----------------|:----------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| 2to3           | 259 ms                                                                 | 253 ms: 1.02x faster                                                    |
| docutils       | 2.63 sec                                                               | 2.58 sec: 1.02x faster                                                  |
| html5lib       | 68.6 ms                                                                | 66.0 ms: 1.04x faster                                                   |
| Geometric mean | (ref)                                                                  | 1.03x faster                                                            |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5 | bm-20251020-vultr-x86_64-Fidget%2dSpinner-tracing_jit-3.15.0a1+-754b3b7 |
|----------------------------|:----------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| async_tree_memoization     | 459 ms                                                                 | 303 ms: 1.51x faster                                                    |
| async_tree_io              | 881 ms                                                                 | 593 ms: 1.49x faster                                                    |
| async_tree_io_tg           | 901 ms                                                                 | 607 ms: 1.48x faster                                                    |
| async_tree_cpu_io_mixed    | 666 ms                                                                 | 470 ms: 1.42x faster                                                    |
| async_tree_none_tg         | 333 ms                                                                 | 243 ms: 1.37x faster                                                    |
| async_tree_none            | 353 ms                                                                 | 258 ms: 1.37x faster                                                    |
| async_tree_memoization_tg  | 410 ms                                                                 | 305 ms: 1.35x faster                                                    |
| async_tree_cpu_io_mixed_tg | 634 ms                                                                 | 473 ms: 1.34x faster                                                    |
| async_generators           | 375 ms                                                                 | 334 ms: 1.12x faster                                                    |
| coroutines                 | 23.3 ms                                                                | 22.4 ms: 1.04x faster                                                   |
| asyncio_websockets         | 517 ms                                                                 | 544 ms: 1.05x slower                                                    |
| Geometric mean             | (ref)                                                                  | 1.30x faster                                                            |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5 | bm-20251020-vultr-x86_64-Fidget%2dSpinner-tracing_jit-3.15.0a1+-754b3b7 |
|----------------|:----------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| float          | 76.7 ms                                                                | 62.3 ms: 1.23x faster                                                   |
| pidigits       | 216 ms                                                                 | 194 ms: 1.12x faster                                                    |
| nbody          | 85.3 ms                                                                | 98.8 ms: 1.16x slower                                                   |
| Geometric mean | (ref)                                                                  | 1.06x faster                                                            |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5 | bm-20251020-vultr-x86_64-Fidget%2dSpinner-tracing_jit-3.15.0a1+-754b3b7 |
|----------------|:----------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| regex_effbot   | 3.21 ms                                                                | 2.95 ms: 1.09x faster                                                   |
| regex_dna      | 189 ms                                                                 | 180 ms: 1.05x faster                                                    |
| regex_v8       | 23.2 ms                                                                | 22.5 ms: 1.03x faster                                                   |
| regex_compile  | 131 ms                                                                 | 137 ms: 1.04x slower                                                    |
| Geometric mean | (ref)                                                                  | 1.03x faster                                                            |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5 | bm-20251020-vultr-x86_64-Fidget%2dSpinner-tracing_jit-3.15.0a1+-754b3b7 |
|----------------------|:----------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| json_dumps           | 10.6 ms                                                                | 8.84 ms: 1.20x faster                                                   |
| unpickle_pure_python | 208 us                                                                 | 184 us: 1.13x faster                                                    |
| xml_etree_process    | 59.2 ms                                                                | 52.4 ms: 1.13x faster                                                   |
| xml_etree_generate   | 85.4 ms                                                                | 76.2 ms: 1.12x faster                                                   |
| json_loads           | 27.3 us                                                                | 25.7 us: 1.06x faster                                                   |
| xml_etree_iterparse  | 94.4 ms                                                                | 93.7 ms: 1.01x faster                                                   |
| pickle_pure_python   | 292 us                                                                 | 304 us: 1.04x slower                                                    |
| xml_etree_parse      | 136 ms                                                                 | 143 ms: 1.05x slower                                                    |
| Geometric mean       | (ref)                                                                  | 1.06x faster                                                            |

Benchmark hidden because not significant (1): tomli_loads

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5 | bm-20251020-vultr-x86_64-Fidget%2dSpinner-tracing_jit-3.15.0a1+-754b3b7 |
|------------------------|:----------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| python_startup_no_site | 7.41 ms                                                                | 7.40 ms: 1.00x faster                                                   |
| python_startup         | 11.0 ms                                                                | 12.7 ms: 1.15x slower                                                   |
| Geometric mean         | (ref)                                                                  | 1.07x slower                                                            |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5 | bm-20251020-vultr-x86_64-Fidget%2dSpinner-tracing_jit-3.15.0a1+-754b3b7 |
|-----------------|:----------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| genshi_text     | 21.7 ms                                                                | 19.8 ms: 1.10x faster                                                   |
| django_template | 34.2 ms                                                                | 34.5 ms: 1.01x slower                                                   |
| Geometric mean  | (ref)                                                                  | 1.02x faster                                                            |

Benchmark hidden because not significant (2): mako, genshi_xml

All benchmarks:
===============

| Benchmark                  | bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5 | bm-20251020-vultr-x86_64-Fidget%2dSpinner-tracing_jit-3.15.0a1+-754b3b7 |
|----------------------------|:----------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| mdp                        | 2.34 sec                                                               | 1.18 sec: 1.99x faster                                                  |
| deepcopy_memo              | 38.1 us                                                                | 22.3 us: 1.71x faster                                                   |
| richards                   | 44.4 ms                                                                | 29.0 ms: 1.53x faster                                                   |
| deepcopy                   | 357 us                                                                 | 236 us: 1.52x faster                                                    |
| async_tree_memoization     | 459 ms                                                                 | 303 ms: 1.51x faster                                                    |
| richards_super             | 50.4 ms                                                                | 33.7 ms: 1.50x faster                                                   |
| async_tree_io              | 881 ms                                                                 | 593 ms: 1.49x faster                                                    |
| async_tree_io_tg           | 901 ms                                                                 | 607 ms: 1.48x faster                                                    |
| async_tree_cpu_io_mixed    | 666 ms                                                                 | 470 ms: 1.42x faster                                                    |
| scimark_fft                | 348 ms                                                                 | 247 ms: 1.41x faster                                                    |
| async_tree_none_tg         | 333 ms                                                                 | 243 ms: 1.37x faster                                                    |
| async_tree_none            | 353 ms                                                                 | 258 ms: 1.37x faster                                                    |
| scimark_sor                | 134 ms                                                                 | 98.3 ms: 1.36x faster                                                   |
| async_tree_memoization_tg  | 410 ms                                                                 | 305 ms: 1.35x faster                                                    |
| async_tree_cpu_io_mixed_tg | 634 ms                                                                 | 473 ms: 1.34x faster                                                    |
| deepcopy_reduce            | 3.12 us                                                                | 2.37 us: 1.32x faster                                                   |
| spectral_norm              | 108 ms                                                                 | 85.1 ms: 1.26x faster                                                   |
| logging_silent             | 98.2 ns                                                                | 78.6 ns: 1.25x faster                                                   |
| float                      | 76.7 ms                                                                | 62.3 ms: 1.23x faster                                                   |
| scimark_sparse_mat_mult    | 4.75 ms                                                                | 3.93 ms: 1.21x faster                                                   |
| json_dumps                 | 10.6 ms                                                                | 8.84 ms: 1.20x faster                                                   |
| scimark_monte_carlo        | 65.8 ms                                                                | 55.8 ms: 1.18x faster                                                   |
| scimark_lu                 | 112 ms                                                                 | 96.1 ms: 1.17x faster                                                   |
| go                         | 141 ms                                                                 | 121 ms: 1.16x faster                                                    |
| pyflate                    | 449 ms                                                                 | 392 ms: 1.15x faster                                                    |
| pylint                     | 317 ms                                                                 | 277 ms: 1.14x faster                                                    |
| unpickle_pure_python       | 208 us                                                                 | 184 us: 1.13x faster                                                    |
| bpe_tokeniser              | 4.46 sec                                                               | 3.94 sec: 1.13x faster                                                  |
| xml_etree_process          | 59.2 ms                                                                | 52.4 ms: 1.13x faster                                                   |
| comprehensions             | 16.6 us                                                                | 14.8 us: 1.12x faster                                                   |
| async_generators           | 375 ms                                                                 | 334 ms: 1.12x faster                                                    |
| xml_etree_generate         | 85.4 ms                                                                | 76.2 ms: 1.12x faster                                                   |
| pidigits                   | 216 ms                                                                 | 194 ms: 1.12x faster                                                    |
| deltablue                  | 3.10 ms                                                                | 2.78 ms: 1.11x faster                                                   |
| pprint_safe_repr           | 719 ms                                                                 | 651 ms: 1.10x faster                                                    |
| coverage                   | 82.5 ms                                                                | 75.0 ms: 1.10x faster                                                   |
| genshi_text                | 21.7 ms                                                                | 19.8 ms: 1.10x faster                                                   |
| regex_effbot               | 3.21 ms                                                                | 2.95 ms: 1.09x faster                                                   |
| pprint_pformat             | 1.46 sec                                                               | 1.35 sec: 1.08x faster                                                  |
| chaos                      | 56.3 ms                                                                | 52.1 ms: 1.08x faster                                                   |
| typing_runtime_protocols   | 156 us                                                                 | 146 us: 1.06x faster                                                    |
| json_loads                 | 27.3 us                                                                | 25.7 us: 1.06x faster                                                   |
| fannkuch                   | 376 ms                                                                 | 355 ms: 1.06x faster                                                    |
| sympy_integrate            | 19.7 ms                                                                | 18.7 ms: 1.05x faster                                                   |
| sqlite_synth               | 2.25 us                                                                | 2.14 us: 1.05x faster                                                   |
| regex_dna                  | 189 ms                                                                 | 180 ms: 1.05x faster                                                    |
| logging_format             | 6.92 us                                                                | 6.61 us: 1.05x faster                                                   |
| logging_simple             | 6.14 us                                                                | 5.89 us: 1.04x faster                                                   |
| coroutines                 | 23.3 ms                                                                | 22.4 ms: 1.04x faster                                                   |
| html5lib                   | 68.6 ms                                                                | 66.0 ms: 1.04x faster                                                   |
| generators                 | 28.5 ms                                                                | 27.5 ms: 1.04x faster                                                   |
| regex_v8                   | 23.2 ms                                                                | 22.5 ms: 1.03x faster                                                   |
| 2to3                       | 259 ms                                                                 | 253 ms: 1.02x faster                                                    |
| json                       | 4.98 ms                                                                | 4.88 ms: 1.02x faster                                                   |
| crypto_pyaes               | 68.2 ms                                                                | 66.9 ms: 1.02x faster                                                   |
| docutils                   | 2.63 sec                                                               | 2.58 sec: 1.02x faster                                                  |
| thrift                     | 772 us                                                                 | 759 us: 1.02x faster                                                    |
| dulwich_log                | 74.5 ms                                                                | 73.8 ms: 1.01x faster                                                   |
| xml_etree_iterparse        | 94.4 ms                                                                | 93.7 ms: 1.01x faster                                                   |
| pathlib                    | 19.3 ms                                                                | 19.2 ms: 1.00x faster                                                   |
| sympy_sum                  | 154 ms                                                                 | 154 ms: 1.00x faster                                                    |
| python_startup_no_site     | 7.41 ms                                                                | 7.40 ms: 1.00x faster                                                   |
| django_template            | 34.2 ms                                                                | 34.5 ms: 1.01x slower                                                   |
| sympy_str                  | 274 ms                                                                 | 279 ms: 1.02x slower                                                    |
| pycparser                  | 1.12 sec                                                               | 1.14 sec: 1.02x slower                                                  |
| nqueens                    | 77.7 ms                                                                | 80.1 ms: 1.03x slower                                                   |
| regex_compile              | 131 ms                                                                 | 137 ms: 1.04x slower                                                    |
| pickle_pure_python         | 292 us                                                                 | 304 us: 1.04x slower                                                    |
| bench_thread_pool          | 924 us                                                                 | 966 us: 1.04x slower                                                    |
| asyncio_websockets         | 517 ms                                                                 | 544 ms: 1.05x slower                                                    |
| xml_etree_parse            | 136 ms                                                                 | 143 ms: 1.05x slower                                                    |
| sympy_expand               | 454 ms                                                                 | 479 ms: 1.06x slower                                                    |
| meteor_contest             | 101 ms                                                                 | 107 ms: 1.06x slower                                                    |
| raytrace                   | 250 ms                                                                 | 267 ms: 1.07x slower                                                    |
| bench_mp_pool              | 11.0 ms                                                                | 12.3 ms: 1.12x slower                                                   |
| python_startup             | 11.0 ms                                                                | 12.7 ms: 1.15x slower                                                   |
| nbody                      | 85.3 ms                                                                | 98.8 ms: 1.16x slower                                                   |
| gc_traversal               | 3.32 ms                                                                | 4.27 ms: 1.29x slower                                                   |
| create_gc_cycles           | 1.33 ms                                                                | 2.03 ms: 1.52x slower                                                   |
| telco                      | 7.77 ms                                                                | 157 ms: 20.20x slower                                                   |
| Geometric mean             | (ref)                                                                  | 1.07x faster                                                            |

Benchmark hidden because not significant (4): tomli_loads, hexiom, mako, genshi_xml
Ignored benchmarks (18) of results/bm-20240920-3.13.0rc2-4981ec5/bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5.json: aiohttp, asyncio_tcp, asyncio_tcp_ssl, chameleon, dask, flaskblogging, gunicorn, pickle, pickle_dict, pickle_list, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http, unpack_sequence, unpickle, unpickle_list
Ignored benchmarks (10) of results/bm-20251020-3.15.0a1+-754b3b7-CLANG,JIT/bm-20251020-vultr-x86_64-Fidget%2dSpinner-tracing_jit-3.15.0a1+-754b3b7.json: connected_components, k_core, many_optionals, shortest_path, sphinx, sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile, subparsers

- Geometric mean (including insignificant results): 1.068x faster

# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.04x
- 95% likely to have a speedup of 1.04x
- 99% likely to have a speedup of 1.02x

# Memory
- memory change: 1.20x