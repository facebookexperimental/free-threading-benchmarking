# Results vs. 3.12.6

- fork: kumaraditya303
- ref: jit_inline_funcptr
- machine: linux-x86_64
- commit hash: e4a2fff
- commit date: 2026-04-03
- overall geometric mean: 1.159x faster
- HPT reliability: 100.00%
- HPT 99th percentile: 1.08x faster
- Memory change: 1.21x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260403-vultr-x86_64-kumaraditya303-jit_inline_funcptr-3.15.0a7+-e4a2fff |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------------:|
| 2to3           | 264 ms                                                 | 251 ms: 1.05x faster                                                         |
| html5lib       | 63.6 ms                                                | 62.1 ms: 1.02x faster                                                        |
| Geometric mean | (ref)                                                  | 1.02x faster                                                                 |

Benchmark hidden because not significant (1): docutils

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260403-vultr-x86_64-kumaraditya303-jit_inline_funcptr-3.15.0a7+-e4a2fff |
|----------------------------|:------------------------------------------------------:|:----------------------------------------------------------------------------:|
| async_tree_io_tg           | 1.11 sec                                               | 559 ms: 1.98x faster                                                         |
| async_tree_none            | 464 ms                                                 | 235 ms: 1.98x faster                                                         |
| async_tree_memoization     | 555 ms                                                 | 288 ms: 1.93x faster                                                         |
| async_tree_io              | 1.08 sec                                               | 567 ms: 1.91x faster                                                         |
| async_tree_memoization_tg  | 560 ms                                                 | 295 ms: 1.90x faster                                                         |
| async_tree_none_tg         | 446 ms                                                 | 236 ms: 1.89x faster                                                         |
| async_tree_cpu_io_mixed    | 715 ms                                                 | 474 ms: 1.51x faster                                                         |
| async_tree_cpu_io_mixed_tg | 723 ms                                                 | 482 ms: 1.50x faster                                                         |
| async_generators           | 384 ms                                                 | 349 ms: 1.10x faster                                                         |
| coroutines                 | 23.9 ms                                                | 23.2 ms: 1.03x faster                                                        |
| asyncio_websockets         | 517 ms                                                 | 544 ms: 1.05x slower                                                         |
| Geometric mean             | (ref)                                                  | 1.55x faster                                                                 |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260403-vultr-x86_64-kumaraditya303-jit_inline_funcptr-3.15.0a7+-e4a2fff |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------------:|
| float          | 80.8 ms                                                | 57.7 ms: 1.40x faster                                                        |
| nbody          | 89.3 ms                                                | 64.0 ms: 1.39x faster                                                        |
| pidigits       | 184 ms                                                 | 187 ms: 1.01x slower                                                         |
| Geometric mean | (ref)                                                  | 1.24x faster                                                                 |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260403-vultr-x86_64-kumaraditya303-jit_inline_funcptr-3.15.0a7+-e4a2fff |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------------:|
| regex_effbot   | 3.17 ms                                                | 2.72 ms: 1.16x faster                                                        |
| regex_compile  | 142 ms                                                 | 126 ms: 1.13x faster                                                         |
| regex_dna      | 168 ms                                                 | 171 ms: 1.02x slower                                                         |
| regex_v8       | 20.6 ms                                                | 22.2 ms: 1.08x slower                                                        |
| Geometric mean | (ref)                                                  | 1.05x faster                                                                 |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260403-vultr-x86_64-kumaraditya303-jit_inline_funcptr-3.15.0a7+-e4a2fff |
|----------------------|:------------------------------------------------------:|:----------------------------------------------------------------------------:|
| tomli_loads          | 2.11 sec                                               | 1.52 sec: 1.38x faster                                                       |
| json_dumps           | 10.4 ms                                                | 8.73 ms: 1.19x faster                                                        |
| xml_etree_iterparse  | 96.7 ms                                                | 83.0 ms: 1.17x faster                                                        |
| xml_etree_process    | 59.0 ms                                                | 53.4 ms: 1.11x faster                                                        |
| unpickle_pure_python | 221 us                                                 | 200 us: 1.10x faster                                                         |
| xml_etree_parse      | 139 ms                                                 | 131 ms: 1.06x faster                                                         |
| xml_etree_generate   | 85.2 ms                                                | 80.6 ms: 1.06x faster                                                        |
| json_loads           | 26.5 us                                                | 26.9 us: 1.01x slower                                                        |
| pickle_pure_python   | 308 us                                                 | 319 us: 1.04x slower                                                         |
| Geometric mean       | (ref)                                                  | 1.11x faster                                                                 |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260403-vultr-x86_64-kumaraditya303-jit_inline_funcptr-3.15.0a7+-e4a2fff |
|------------------------|:------------------------------------------------------:|:----------------------------------------------------------------------------:|
| python_startup_no_site | 7.16 ms                                                | 7.54 ms: 1.05x slower                                                        |
| python_startup         | 9.93 ms                                                | 12.8 ms: 1.28x slower                                                        |
| Geometric mean         | (ref)                                                  | 1.16x slower                                                                 |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260403-vultr-x86_64-kumaraditya303-jit_inline_funcptr-3.15.0a7+-e4a2fff |
|-----------------|:------------------------------------------------------:|:----------------------------------------------------------------------------:|
| genshi_text     | 22.8 ms                                                | 20.4 ms: 1.12x faster                                                        |
| django_template | 34.7 ms                                                | 32.1 ms: 1.08x faster                                                        |
| mako            | 11.0 ms                                                | 10.9 ms: 1.01x faster                                                        |
| genshi_xml      | 50.2 ms                                                | 51.6 ms: 1.03x slower                                                        |
| Geometric mean  | (ref)                                                  | 1.04x faster                                                                 |

All benchmarks:
===============

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260403-vultr-x86_64-kumaraditya303-jit_inline_funcptr-3.15.0a7+-e4a2fff |
|----------------------------|:------------------------------------------------------:|:----------------------------------------------------------------------------:|
| richards                   | 45.9 ms                                                | 18.5 ms: 2.48x faster                                                        |
| richards_super             | 51.9 ms                                                | 21.4 ms: 2.42x faster                                                        |
| mdp                        | 2.42 sec                                               | 1.19 sec: 2.04x faster                                                       |
| async_tree_io_tg           | 1.11 sec                                               | 559 ms: 1.98x faster                                                         |
| async_tree_none            | 464 ms                                                 | 235 ms: 1.98x faster                                                         |
| async_tree_memoization     | 555 ms                                                 | 288 ms: 1.93x faster                                                         |
| async_tree_io              | 1.08 sec                                               | 567 ms: 1.91x faster                                                         |
| async_tree_memoization_tg  | 560 ms                                                 | 295 ms: 1.90x faster                                                         |
| async_tree_none_tg         | 446 ms                                                 | 236 ms: 1.89x faster                                                         |
| deepcopy_memo              | 40.3 us                                                | 22.5 us: 1.79x faster                                                        |
| scimark_sor                | 130 ms                                                 | 78.1 ms: 1.66x faster                                                        |
| deepcopy                   | 352 us                                                 | 222 us: 1.59x faster                                                         |
| async_tree_cpu_io_mixed    | 715 ms                                                 | 474 ms: 1.51x faster                                                         |
| spectral_norm              | 110 ms                                                 | 73.0 ms: 1.51x faster                                                        |
| async_tree_cpu_io_mixed_tg | 723 ms                                                 | 482 ms: 1.50x faster                                                         |
| logging_silent             | 109 ns                                                 | 73.7 ns: 1.48x faster                                                        |
| scimark_lu                 | 114 ms                                                 | 78.8 ms: 1.45x faster                                                        |
| float                      | 80.8 ms                                                | 57.7 ms: 1.40x faster                                                        |
| nbody                      | 89.3 ms                                                | 64.0 ms: 1.39x faster                                                        |
| tomli_loads                | 2.11 sec                                               | 1.52 sec: 1.38x faster                                                       |
| go                         | 139 ms                                                 | 101 ms: 1.38x faster                                                         |
| scimark_monte_carlo        | 68.4 ms                                                | 50.6 ms: 1.35x faster                                                        |
| scimark_fft                | 342 ms                                                 | 253 ms: 1.35x faster                                                         |
| comprehensions             | 19.8 us                                                | 14.7 us: 1.35x faster                                                        |
| deltablue                  | 3.45 ms                                                | 2.65 ms: 1.30x faster                                                        |
| chaos                      | 62.8 ms                                                | 49.0 ms: 1.28x faster                                                        |
| pyflate                    | 448 ms                                                 | 351 ms: 1.28x faster                                                         |
| deepcopy_reduce            | 3.08 us                                                | 2.48 us: 1.24x faster                                                        |
| logging_format             | 7.35 us                                                | 6.00 us: 1.22x faster                                                        |
| bpe_tokeniser              | 4.74 sec                                               | 3.87 sec: 1.22x faster                                                       |
| pprint_safe_repr           | 743 ms                                                 | 609 ms: 1.22x faster                                                         |
| pathlib                    | 21.5 ms                                                | 17.7 ms: 1.21x faster                                                        |
| logging_simple             | 6.63 us                                                | 5.46 us: 1.21x faster                                                        |
| crypto_pyaes               | 76.6 ms                                                | 64.2 ms: 1.19x faster                                                        |
| json_dumps                 | 10.4 ms                                                | 8.73 ms: 1.19x faster                                                        |
| pprint_pformat             | 1.52 sec                                               | 1.29 sec: 1.18x faster                                                       |
| xml_etree_iterparse        | 96.7 ms                                                | 83.0 ms: 1.17x faster                                                        |
| regex_effbot               | 3.17 ms                                                | 2.72 ms: 1.16x faster                                                        |
| nqueens                    | 80.1 ms                                                | 69.9 ms: 1.15x faster                                                        |
| regex_compile              | 142 ms                                                 | 126 ms: 1.13x faster                                                         |
| genshi_text                | 22.8 ms                                                | 20.4 ms: 1.12x faster                                                        |
| scimark_sparse_mat_mult    | 4.39 ms                                                | 3.92 ms: 1.12x faster                                                        |
| dulwich_log                | 78.9 ms                                                | 70.8 ms: 1.11x faster                                                        |
| fannkuch                   | 372 ms                                                 | 334 ms: 1.11x faster                                                         |
| generators                 | 32.2 ms                                                | 29.1 ms: 1.11x faster                                                        |
| xml_etree_process          | 59.0 ms                                                | 53.4 ms: 1.11x faster                                                        |
| unpickle_pure_python       | 221 us                                                 | 200 us: 1.10x faster                                                         |
| async_generators           | 384 ms                                                 | 349 ms: 1.10x faster                                                         |
| pylint                     | 319 ms                                                 | 292 ms: 1.09x faster                                                         |
| hexiom                     | 6.17 ms                                                | 5.69 ms: 1.08x faster                                                        |
| django_template            | 34.7 ms                                                | 32.1 ms: 1.08x faster                                                        |
| typing_runtime_protocols   | 163 us                                                 | 152 us: 1.07x faster                                                         |
| xml_etree_parse            | 139 ms                                                 | 131 ms: 1.06x faster                                                         |
| xml_etree_generate         | 85.2 ms                                                | 80.6 ms: 1.06x faster                                                        |
| 2to3                       | 264 ms                                                 | 251 ms: 1.05x faster                                                         |
| raytrace                   | 299 ms                                                 | 286 ms: 1.05x faster                                                         |
| coroutines                 | 23.9 ms                                                | 23.2 ms: 1.03x faster                                                        |
| sqlite_synth               | 2.20 us                                                | 2.14 us: 1.03x faster                                                        |
| meteor_contest             | 104 ms                                                 | 101 ms: 1.03x faster                                                         |
| pycparser                  | 1.17 sec                                               | 1.14 sec: 1.02x faster                                                       |
| html5lib                   | 63.6 ms                                                | 62.1 ms: 1.02x faster                                                        |
| sympy_integrate            | 20.5 ms                                                | 20.1 ms: 1.02x faster                                                        |
| json                       | 5.02 ms                                                | 4.94 ms: 1.02x faster                                                        |
| mako                       | 11.0 ms                                                | 10.9 ms: 1.01x faster                                                        |
| json_loads                 | 26.5 us                                                | 26.9 us: 1.01x slower                                                        |
| pidigits                   | 184 ms                                                 | 187 ms: 1.01x slower                                                         |
| sympy_str                  | 292 ms                                                 | 296 ms: 1.02x slower                                                         |
| thrift                     | 791 us                                                 | 805 us: 1.02x slower                                                         |
| sympy_sum                  | 166 ms                                                 | 169 ms: 1.02x slower                                                         |
| regex_dna                  | 168 ms                                                 | 171 ms: 1.02x slower                                                         |
| sympy_expand               | 468 ms                                                 | 479 ms: 1.02x slower                                                         |
| genshi_xml                 | 50.2 ms                                                | 51.6 ms: 1.03x slower                                                        |
| pickle_pure_python         | 308 us                                                 | 319 us: 1.04x slower                                                         |
| python_startup_no_site     | 7.16 ms                                                | 7.54 ms: 1.05x slower                                                        |
| asyncio_websockets         | 517 ms                                                 | 544 ms: 1.05x slower                                                         |
| regex_v8                   | 20.6 ms                                                | 22.2 ms: 1.08x slower                                                        |
| coverage                   | 71.4 ms                                                | 82.7 ms: 1.16x slower                                                        |
| gc_traversal               | 3.46 ms                                                | 4.20 ms: 1.21x slower                                                        |
| python_startup             | 9.93 ms                                                | 12.8 ms: 1.28x slower                                                        |
| bench_thread_pool          | 941 us                                                 | 1.31 ms: 1.39x slower                                                        |
| create_gc_cycles           | 1.09 ms                                                | 1.85 ms: 1.70x slower                                                        |
| bench_mp_pool              | 10.8 ms                                                | 173 ms: 15.99x slower                                                        |
| telco                      | 6.53 ms                                                | 152 ms: 23.32x slower                                                        |
| Geometric mean             | (ref)                                                  | 1.11x faster                                                                 |

Benchmark hidden because not significant (1): docutils
Ignored benchmarks (21) of results/bm-20240906-3.12.6-a4a2d2b/bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b.json: aiohttp, asyncio_tcp, asyncio_tcp_ssl, chameleon, dask, flaskblogging, gunicorn, mypy2, pickle, pickle_dict, pickle_list, sqlalchemy_declarative, sqlalchemy_imperative, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http, unpack_sequence, unpickle, unpickle_list
Ignored benchmarks (10) of results/bm-20260403-3.15.0a7+-e4a2fff-JIT/bm-20260403-vultr-x86_64-kumaraditya303-jit_inline_funcptr-3.15.0a7+-e4a2fff.json: connected_components, k_core, many_optionals, shortest_path, sphinx, sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile, subparsers

- Geometric mean (including insignificant results): 1.159x faster

# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.11x
- 95% likely to have a speedup of 1.10x
- 99% likely to have a speedup of 1.08x

# Memory
- memory change: 1.21x