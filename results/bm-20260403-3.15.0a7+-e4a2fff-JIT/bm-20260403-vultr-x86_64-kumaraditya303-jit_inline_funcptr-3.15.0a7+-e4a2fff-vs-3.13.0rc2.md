# Results vs. 3.13.0rc2

- fork: kumaraditya303
- ref: jit_inline_funcptr
- machine: linux-x86_64
- commit hash: e4a2fff
- commit date: 2026-04-03
- overall geometric mean: 1.120x faster
- HPT reliability: 100.00%
- HPT 99th percentile: 1.05x faster
- Memory change: 1.19x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260403-vultr-x86_64-kumaraditya303-jit_inline_funcptr-3.15.0a7+-e4a2fff |
|----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------------:|
| 2to3           | 260 ms                                                       | 251 ms: 1.04x faster                                                         |
| docutils       | 2.62 sec                                                     | 2.65 sec: 1.01x slower                                                       |
| html5lib       | 67.0 ms                                                      | 62.1 ms: 1.08x faster                                                        |
| Geometric mean | (ref)                                                        | 1.03x faster                                                                 |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260403-vultr-x86_64-kumaraditya303-jit_inline_funcptr-3.15.0a7+-e4a2fff |
|----------------------------|:------------------------------------------------------------:|:----------------------------------------------------------------------------:|
| async_tree_io_tg           | 913 ms                                                       | 559 ms: 1.63x faster                                                         |
| async_tree_memoization     | 461 ms                                                       | 288 ms: 1.60x faster                                                         |
| async_tree_io              | 876 ms                                                       | 567 ms: 1.54x faster                                                         |
| async_tree_none            | 354 ms                                                       | 235 ms: 1.51x faster                                                         |
| async_tree_none_tg         | 336 ms                                                       | 236 ms: 1.42x faster                                                         |
| async_tree_cpu_io_mixed    | 666 ms                                                       | 474 ms: 1.41x faster                                                         |
| async_tree_memoization_tg  | 414 ms                                                       | 295 ms: 1.41x faster                                                         |
| async_tree_cpu_io_mixed_tg | 638 ms                                                       | 482 ms: 1.32x faster                                                         |
| async_generators           | 377 ms                                                       | 349 ms: 1.08x faster                                                         |
| coroutines                 | 23.6 ms                                                      | 23.2 ms: 1.02x faster                                                        |
| asyncio_websockets         | 520 ms                                                       | 544 ms: 1.05x slower                                                         |
| Geometric mean             | (ref)                                                        | 1.33x faster                                                                 |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260403-vultr-x86_64-kumaraditya303-jit_inline_funcptr-3.15.0a7+-e4a2fff |
|----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------------:|
| float          | 77.5 ms                                                      | 57.7 ms: 1.34x faster                                                        |
| nbody          | 85.1 ms                                                      | 64.0 ms: 1.33x faster                                                        |
| pidigits       | 217 ms                                                       | 187 ms: 1.16x faster                                                         |
| Geometric mean | (ref)                                                        | 1.27x faster                                                                 |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260403-vultr-x86_64-kumaraditya303-jit_inline_funcptr-3.15.0a7+-e4a2fff |
|----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------------:|
| regex_effbot   | 3.08 ms                                                      | 2.72 ms: 1.13x faster                                                        |
| regex_dna      | 180 ms                                                       | 171 ms: 1.05x faster                                                         |
| regex_compile  | 132 ms                                                       | 126 ms: 1.05x faster                                                         |
| regex_v8       | 22.7 ms                                                      | 22.2 ms: 1.02x faster                                                        |
| Geometric mean | (ref)                                                        | 1.06x faster                                                                 |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260403-vultr-x86_64-kumaraditya303-jit_inline_funcptr-3.15.0a7+-e4a2fff |
|----------------------|:------------------------------------------------------------:|:----------------------------------------------------------------------------:|
| tomli_loads          | 2.01 sec                                                     | 1.52 sec: 1.32x faster                                                       |
| json_dumps           | 10.5 ms                                                      | 8.73 ms: 1.21x faster                                                        |
| xml_etree_iterparse  | 94.9 ms                                                      | 83.0 ms: 1.14x faster                                                        |
| xml_etree_process    | 59.3 ms                                                      | 53.4 ms: 1.11x faster                                                        |
| xml_etree_generate   | 85.4 ms                                                      | 80.6 ms: 1.06x faster                                                        |
| unpickle_pure_python | 210 us                                                       | 200 us: 1.05x faster                                                         |
| xml_etree_parse      | 136 ms                                                       | 131 ms: 1.04x faster                                                         |
| json_loads           | 27.0 us                                                      | 26.9 us: 1.01x faster                                                        |
| pickle_pure_python   | 294 us                                                       | 319 us: 1.08x slower                                                         |
| Geometric mean       | (ref)                                                        | 1.09x faster                                                                 |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260403-vultr-x86_64-kumaraditya303-jit_inline_funcptr-3.15.0a7+-e4a2fff |
|------------------------|:------------------------------------------------------------:|:----------------------------------------------------------------------------:|
| python_startup_no_site | 7.39 ms                                                      | 7.54 ms: 1.02x slower                                                        |
| python_startup         | 11.0 ms                                                      | 12.8 ms: 1.16x slower                                                        |
| Geometric mean         | (ref)                                                        | 1.09x slower                                                                 |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260403-vultr-x86_64-kumaraditya303-jit_inline_funcptr-3.15.0a7+-e4a2fff |
|-----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------------:|
| django_template | 34.1 ms                                                      | 32.1 ms: 1.06x faster                                                        |
| genshi_text     | 21.5 ms                                                      | 20.4 ms: 1.06x faster                                                        |
| mako            | 11.3 ms                                                      | 10.9 ms: 1.04x faster                                                        |
| genshi_xml      | 48.8 ms                                                      | 51.6 ms: 1.06x slower                                                        |
| Geometric mean  | (ref)                                                        | 1.03x faster                                                                 |

All benchmarks:
===============

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260403-vultr-x86_64-kumaraditya303-jit_inline_funcptr-3.15.0a7+-e4a2fff |
|----------------------------|:------------------------------------------------------------:|:----------------------------------------------------------------------------:|
| richards                   | 45.2 ms                                                      | 18.5 ms: 2.45x faster                                                        |
| richards_super             | 51.6 ms                                                      | 21.4 ms: 2.41x faster                                                        |
| mdp                        | 2.36 sec                                                     | 1.19 sec: 1.99x faster                                                       |
| deepcopy_memo              | 39.1 us                                                      | 22.5 us: 1.74x faster                                                        |
| scimark_sor                | 134 ms                                                       | 78.1 ms: 1.72x faster                                                        |
| async_tree_io_tg           | 913 ms                                                       | 559 ms: 1.63x faster                                                         |
| deepcopy                   | 355 us                                                       | 222 us: 1.60x faster                                                         |
| async_tree_memoization     | 461 ms                                                       | 288 ms: 1.60x faster                                                         |
| async_tree_io              | 876 ms                                                       | 567 ms: 1.54x faster                                                         |
| spectral_norm              | 111 ms                                                       | 73.0 ms: 1.52x faster                                                        |
| async_tree_none            | 354 ms                                                       | 235 ms: 1.51x faster                                                         |
| scimark_lu                 | 113 ms                                                       | 78.8 ms: 1.43x faster                                                        |
| async_tree_none_tg         | 336 ms                                                       | 236 ms: 1.42x faster                                                         |
| async_tree_cpu_io_mixed    | 666 ms                                                       | 474 ms: 1.41x faster                                                         |
| async_tree_memoization_tg  | 414 ms                                                       | 295 ms: 1.41x faster                                                         |
| go                         | 141 ms                                                       | 101 ms: 1.39x faster                                                         |
| logging_silent             | 103 ns                                                       | 73.7 ns: 1.39x faster                                                        |
| scimark_fft                | 349 ms                                                       | 253 ms: 1.38x faster                                                         |
| float                      | 77.5 ms                                                      | 57.7 ms: 1.34x faster                                                        |
| nbody                      | 85.1 ms                                                      | 64.0 ms: 1.33x faster                                                        |
| async_tree_cpu_io_mixed_tg | 638 ms                                                       | 482 ms: 1.32x faster                                                         |
| tomli_loads                | 2.01 sec                                                     | 1.52 sec: 1.32x faster                                                       |
| scimark_monte_carlo        | 65.4 ms                                                      | 50.6 ms: 1.29x faster                                                        |
| pyflate                    | 449 ms                                                       | 351 ms: 1.28x faster                                                         |
| deepcopy_reduce            | 3.11 us                                                      | 2.48 us: 1.25x faster                                                        |
| pprint_safe_repr           | 738 ms                                                       | 609 ms: 1.21x faster                                                         |
| json_dumps                 | 10.5 ms                                                      | 8.73 ms: 1.21x faster                                                        |
| scimark_sparse_mat_mult    | 4.71 ms                                                      | 3.92 ms: 1.20x faster                                                        |
| deltablue                  | 3.12 ms                                                      | 2.65 ms: 1.18x faster                                                        |
| chaos                      | 57.3 ms                                                      | 49.0 ms: 1.17x faster                                                        |
| pprint_pformat             | 1.50 sec                                                     | 1.29 sec: 1.16x faster                                                       |
| pidigits                   | 217 ms                                                       | 187 ms: 1.16x faster                                                         |
| bpe_tokeniser              | 4.45 sec                                                     | 3.87 sec: 1.15x faster                                                       |
| xml_etree_iterparse        | 94.9 ms                                                      | 83.0 ms: 1.14x faster                                                        |
| logging_format             | 6.84 us                                                      | 6.00 us: 1.14x faster                                                        |
| regex_effbot               | 3.08 ms                                                      | 2.72 ms: 1.13x faster                                                        |
| logging_simple             | 6.16 us                                                      | 5.46 us: 1.13x faster                                                        |
| nqueens                    | 78.6 ms                                                      | 69.9 ms: 1.12x faster                                                        |
| comprehensions             | 16.5 us                                                      | 14.7 us: 1.12x faster                                                        |
| xml_etree_process          | 59.3 ms                                                      | 53.4 ms: 1.11x faster                                                        |
| fannkuch                   | 370 ms                                                       | 334 ms: 1.11x faster                                                         |
| pylint                     | 317 ms                                                       | 292 ms: 1.09x faster                                                         |
| async_generators           | 377 ms                                                       | 349 ms: 1.08x faster                                                         |
| pathlib                    | 19.2 ms                                                      | 17.7 ms: 1.08x faster                                                        |
| html5lib                   | 67.0 ms                                                      | 62.1 ms: 1.08x faster                                                        |
| django_template            | 34.1 ms                                                      | 32.1 ms: 1.06x faster                                                        |
| xml_etree_generate         | 85.4 ms                                                      | 80.6 ms: 1.06x faster                                                        |
| crypto_pyaes               | 67.9 ms                                                      | 64.2 ms: 1.06x faster                                                        |
| genshi_text                | 21.5 ms                                                      | 20.4 ms: 1.06x faster                                                        |
| dulwich_log                | 74.8 ms                                                      | 70.8 ms: 1.06x faster                                                        |
| hexiom                     | 5.99 ms                                                      | 5.69 ms: 1.05x faster                                                        |
| regex_dna                  | 180 ms                                                       | 171 ms: 1.05x faster                                                         |
| unpickle_pure_python       | 210 us                                                       | 200 us: 1.05x faster                                                         |
| regex_compile              | 132 ms                                                       | 126 ms: 1.05x faster                                                         |
| xml_etree_parse            | 136 ms                                                       | 131 ms: 1.04x faster                                                         |
| mako                       | 11.3 ms                                                      | 10.9 ms: 1.04x faster                                                        |
| 2to3                       | 260 ms                                                       | 251 ms: 1.04x faster                                                         |
| sqlite_synth               | 2.21 us                                                      | 2.14 us: 1.03x faster                                                        |
| regex_v8                   | 22.7 ms                                                      | 22.2 ms: 1.02x faster                                                        |
| coroutines                 | 23.6 ms                                                      | 23.2 ms: 1.02x faster                                                        |
| typing_runtime_protocols   | 155 us                                                       | 152 us: 1.01x faster                                                         |
| meteor_contest             | 102 ms                                                       | 101 ms: 1.01x faster                                                         |
| json_loads                 | 27.0 us                                                      | 26.9 us: 1.01x faster                                                        |
| generators                 | 28.8 ms                                                      | 29.1 ms: 1.01x slower                                                        |
| docutils                   | 2.62 sec                                                     | 2.65 sec: 1.01x slower                                                       |
| sympy_integrate            | 19.8 ms                                                      | 20.1 ms: 1.02x slower                                                        |
| python_startup_no_site     | 7.39 ms                                                      | 7.54 ms: 1.02x slower                                                        |
| pycparser                  | 1.12 sec                                                     | 1.14 sec: 1.02x slower                                                       |
| thrift                     | 778 us                                                       | 805 us: 1.03x slower                                                         |
| asyncio_websockets         | 520 ms                                                       | 544 ms: 1.05x slower                                                         |
| sympy_expand               | 457 ms                                                       | 479 ms: 1.05x slower                                                         |
| genshi_xml                 | 48.8 ms                                                      | 51.6 ms: 1.06x slower                                                        |
| sympy_str                  | 275 ms                                                       | 296 ms: 1.08x slower                                                         |
| pickle_pure_python         | 294 us                                                       | 319 us: 1.08x slower                                                         |
| sympy_sum                  | 156 ms                                                       | 169 ms: 1.09x slower                                                         |
| raytrace                   | 253 ms                                                       | 286 ms: 1.13x slower                                                         |
| python_startup             | 11.0 ms                                                      | 12.8 ms: 1.16x slower                                                        |
| gc_traversal               | 3.14 ms                                                      | 4.20 ms: 1.34x slower                                                        |
| create_gc_cycles           | 1.34 ms                                                      | 1.85 ms: 1.39x slower                                                        |
| bench_thread_pool          | 919 us                                                       | 1.31 ms: 1.43x slower                                                        |
| bench_mp_pool              | 11.0 ms                                                      | 173 ms: 15.71x slower                                                        |
| telco                      | 7.82 ms                                                      | 152 ms: 19.45x slower                                                        |
| Geometric mean             | (ref)                                                        | 1.08x faster                                                                 |

Benchmark hidden because not significant (2): coverage, json
Ignored benchmarks (18) of results/bm-20240906-3.13.0rc2-ec61006/bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006.json: aiohttp, asyncio_tcp, asyncio_tcp_ssl, chameleon, dask, flaskblogging, gunicorn, pickle, pickle_dict, pickle_list, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http, unpack_sequence, unpickle, unpickle_list
Ignored benchmarks (10) of results/bm-20260403-3.15.0a7+-e4a2fff-JIT/bm-20260403-vultr-x86_64-kumaraditya303-jit_inline_funcptr-3.15.0a7+-e4a2fff.json: connected_components, k_core, many_optionals, shortest_path, sphinx, sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile, subparsers

- Geometric mean (including insignificant results): 1.120x faster

# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.08x
- 95% likely to have a speedup of 1.07x
- 99% likely to have a speedup of 1.05x

# Memory
- memory change: 1.19x