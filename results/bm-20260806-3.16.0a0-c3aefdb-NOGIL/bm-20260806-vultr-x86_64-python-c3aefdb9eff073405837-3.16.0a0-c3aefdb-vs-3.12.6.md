# Results vs. 3.12.6

- fork: python
- ref: c3aefdb9eff073405837
- machine: linux-x86_64
- commit hash: c3aefdb
- commit date: 2026-08-06
- overall geometric mean: 1.034x slower
- HPT reliability: 81.30%
- HPT 99th percentile: 1.00x slower
- Memory change: 1.37x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260806-vultr-x86_64-python-c3aefdb9eff073405837-3.16.0a0-c3aefdb |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| 2to3           | 264 ms                                                 | 298 ms: 1.13x slower                                                  |
| docutils       | 2.64 sec                                               | 2.92 sec: 1.11x slower                                                |
| html5lib       | 63.6 ms                                                | 67.0 ms: 1.05x slower                                                 |
| Geometric mean | (ref)                                                  | 1.10x slower                                                          |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260806-vultr-x86_64-python-c3aefdb9eff073405837-3.16.0a0-c3aefdb |
|----------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| async_tree_io_tg           | 1.11 sec                                               | 683 ms: 1.63x faster                                                  |
| async_tree_io              | 1.08 sec                                               | 703 ms: 1.54x faster                                                  |
| async_tree_memoization_tg  | 560 ms                                                 | 392 ms: 1.43x faster                                                  |
| async_tree_none_tg         | 446 ms                                                 | 322 ms: 1.38x faster                                                  |
| async_tree_none            | 464 ms                                                 | 342 ms: 1.36x faster                                                  |
| async_tree_memoization     | 555 ms                                                 | 419 ms: 1.32x faster                                                  |
| async_tree_cpu_io_mixed_tg | 723 ms                                                 | 576 ms: 1.25x faster                                                  |
| async_tree_cpu_io_mixed    | 715 ms                                                 | 604 ms: 1.18x faster                                                  |
| asyncio_websockets         | 517 ms                                                 | 514 ms: 1.01x faster                                                  |
| async_generators           | 384 ms                                                 | 388 ms: 1.01x slower                                                  |
| coroutines                 | 23.9 ms                                                | 24.6 ms: 1.03x slower                                                 |
| Geometric mean             | (ref)                                                  | 1.26x faster                                                          |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260806-vultr-x86_64-python-c3aefdb9eff073405837-3.16.0a0-c3aefdb |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| pidigits       | 184 ms                                                 | 189 ms: 1.03x slower                                                  |
| float          | 80.8 ms                                                | 83.3 ms: 1.03x slower                                                 |
| nbody          | 89.3 ms                                                | 118 ms: 1.32x slower                                                  |
| Geometric mean | (ref)                                                  | 1.12x slower                                                          |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260806-vultr-x86_64-python-c3aefdb9eff073405837-3.16.0a0-c3aefdb |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| regex_effbot   | 3.17 ms                                                | 2.91 ms: 1.09x faster                                                 |
| regex_v8       | 20.6 ms                                                | 20.1 ms: 1.03x faster                                                 |
| regex_dna      | 168 ms                                                 | 176 ms: 1.05x slower                                                  |
| regex_compile  | 142 ms                                                 | 169 ms: 1.19x slower                                                  |
| Geometric mean | (ref)                                                  | 1.03x slower                                                          |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260806-vultr-x86_64-python-c3aefdb9eff073405837-3.16.0a0-c3aefdb |
|----------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| tomli_loads          | 2.11 sec                                               | 1.95 sec: 1.08x faster                                                |
| xml_etree_parse      | 139 ms                                                 | 139 ms: 1.01x slower                                                  |
| json_dumps           | 10.4 ms                                                | 10.6 ms: 1.03x slower                                                 |
| unpickle_pure_python | 221 us                                                 | 237 us: 1.07x slower                                                  |
| pickle_pure_python   | 308 us                                                 | 333 us: 1.08x slower                                                  |
| json_loads           | 26.5 us                                                | 30.6 us: 1.15x slower                                                 |
| xml_etree_generate   | 85.2 ms                                                | 101 ms: 1.19x slower                                                  |
| xml_etree_process    | 59.0 ms                                                | 74.9 ms: 1.27x slower                                                 |
| Geometric mean       | (ref)                                                  | 1.08x slower                                                          |

Benchmark hidden because not significant (1): xml_etree_iterparse

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260806-vultr-x86_64-python-c3aefdb9eff073405837-3.16.0a0-c3aefdb |
|------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| python_startup_no_site | 7.16 ms                                                | 9.25 ms: 1.29x slower                                                 |
| python_startup         | 9.93 ms                                                | 15.7 ms: 1.58x slower                                                 |
| Geometric mean         | (ref)                                                  | 1.43x slower                                                          |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260806-vultr-x86_64-python-c3aefdb9eff073405837-3.16.0a0-c3aefdb |
|-----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| django_template | 34.7 ms                                                | 41.0 ms: 1.18x slower                                                 |
| mako            | 11.0 ms                                                | 16.1 ms: 1.46x slower                                                 |
| Geometric mean  | (ref)                                                  | 1.31x slower                                                          |

All benchmarks:
===============

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260806-vultr-x86_64-python-c3aefdb9eff073405837-3.16.0a0-c3aefdb |
|----------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| pylint                     | 319 ms                                                 | 131 ms: 2.44x faster                                                  |
| gc_traversal               | 3.46 ms                                                | 1.78 ms: 1.94x faster                                                 |
| mdp                        | 2.42 sec                                               | 1.30 sec: 1.86x faster                                                |
| async_tree_io_tg           | 1.11 sec                                               | 683 ms: 1.63x faster                                                  |
| async_tree_io              | 1.08 sec                                               | 703 ms: 1.54x faster                                                  |
| async_tree_memoization_tg  | 560 ms                                                 | 392 ms: 1.43x faster                                                  |
| async_tree_none_tg         | 446 ms                                                 | 322 ms: 1.38x faster                                                  |
| async_tree_none            | 464 ms                                                 | 342 ms: 1.36x faster                                                  |
| async_tree_memoization     | 555 ms                                                 | 419 ms: 1.32x faster                                                  |
| deepcopy                   | 352 us                                                 | 271 us: 1.30x faster                                                  |
| deepcopy_memo              | 40.3 us                                                | 32.0 us: 1.26x faster                                                 |
| async_tree_cpu_io_mixed_tg | 723 ms                                                 | 576 ms: 1.25x faster                                                  |
| pathlib                    | 21.5 ms                                                | 17.8 ms: 1.21x faster                                                 |
| async_tree_cpu_io_mixed    | 715 ms                                                 | 604 ms: 1.18x faster                                                  |
| go                         | 139 ms                                                 | 120 ms: 1.16x faster                                                  |
| sqlite_synth               | 2.20 us                                                | 1.92 us: 1.15x faster                                                 |
| dulwich_log                | 78.9 ms                                                | 70.8 ms: 1.11x faster                                                 |
| typing_runtime_protocols   | 163 us                                                 | 147 us: 1.11x faster                                                  |
| comprehensions             | 19.8 us                                                | 18.0 us: 1.10x faster                                                 |
| regex_effbot               | 3.17 ms                                                | 2.91 ms: 1.09x faster                                                 |
| scimark_sor                | 130 ms                                                 | 120 ms: 1.09x faster                                                  |
| bpe_tokeniser              | 4.74 sec                                               | 4.36 sec: 1.09x faster                                                |
| tomli_loads                | 2.11 sec                                               | 1.95 sec: 1.08x faster                                                |
| logging_silent             | 109 ns                                                 | 102 ns: 1.06x faster                                                  |
| chaos                      | 62.8 ms                                                | 60.3 ms: 1.04x faster                                                 |
| raytrace                   | 299 ms                                                 | 288 ms: 1.04x faster                                                  |
| deepcopy_reduce            | 3.08 us                                                | 2.98 us: 1.03x faster                                                 |
| scimark_fft                | 342 ms                                                 | 331 ms: 1.03x faster                                                  |
| pycparser                  | 1.17 sec                                               | 1.14 sec: 1.03x faster                                                |
| regex_v8                   | 20.6 ms                                                | 20.1 ms: 1.03x faster                                                 |
| spectral_norm              | 110 ms                                                 | 109 ms: 1.02x faster                                                  |
| asyncio_websockets         | 517 ms                                                 | 514 ms: 1.01x faster                                                  |
| xml_etree_parse            | 139 ms                                                 | 139 ms: 1.01x slower                                                  |
| async_generators           | 384 ms                                                 | 388 ms: 1.01x slower                                                  |
| pidigits                   | 184 ms                                                 | 189 ms: 1.03x slower                                                  |
| json_dumps                 | 10.4 ms                                                | 10.6 ms: 1.03x slower                                                 |
| coroutines                 | 23.9 ms                                                | 24.6 ms: 1.03x slower                                                 |
| float                      | 80.8 ms                                                | 83.3 ms: 1.03x slower                                                 |
| pyflate                    | 448 ms                                                 | 463 ms: 1.03x slower                                                  |
| regex_dna                  | 168 ms                                                 | 176 ms: 1.05x slower                                                  |
| html5lib                   | 63.6 ms                                                | 67.0 ms: 1.05x slower                                                 |
| logging_simple             | 6.63 us                                                | 7.00 us: 1.06x slower                                                 |
| sympy_integrate            | 20.5 ms                                                | 21.9 ms: 1.06x slower                                                 |
| json                       | 5.02 ms                                                | 5.36 ms: 1.07x slower                                                 |
| deltablue                  | 3.45 ms                                                | 3.68 ms: 1.07x slower                                                 |
| hexiom                     | 6.17 ms                                                | 6.63 ms: 1.07x slower                                                 |
| unpickle_pure_python       | 221 us                                                 | 237 us: 1.07x slower                                                  |
| pickle_pure_python         | 308 us                                                 | 333 us: 1.08x slower                                                  |
| sympy_sum                  | 166 ms                                                 | 180 ms: 1.09x slower                                                  |
| sympy_str                  | 292 ms                                                 | 318 ms: 1.09x slower                                                  |
| nqueens                    | 80.1 ms                                                | 87.6 ms: 1.09x slower                                                 |
| pprint_safe_repr           | 743 ms                                                 | 816 ms: 1.10x slower                                                  |
| logging_format             | 7.35 us                                                | 8.09 us: 1.10x slower                                                 |
| docutils                   | 2.64 sec                                               | 2.92 sec: 1.11x slower                                                |
| pprint_pformat             | 1.52 sec                                               | 1.70 sec: 1.12x slower                                                |
| scimark_lu                 | 114 ms                                                 | 128 ms: 1.12x slower                                                  |
| scimark_monte_carlo        | 68.4 ms                                                | 76.8 ms: 1.12x slower                                                 |
| 2to3                       | 264 ms                                                 | 298 ms: 1.13x slower                                                  |
| richards                   | 45.9 ms                                                | 52.2 ms: 1.14x slower                                                 |
| sympy_expand               | 468 ms                                                 | 535 ms: 1.14x slower                                                  |
| crypto_pyaes               | 76.6 ms                                                | 88.2 ms: 1.15x slower                                                 |
| richards_super             | 51.9 ms                                                | 59.8 ms: 1.15x slower                                                 |
| json_loads                 | 26.5 us                                                | 30.6 us: 1.15x slower                                                 |
| scimark_sparse_mat_mult    | 4.39 ms                                                | 5.08 ms: 1.16x slower                                                 |
| thrift                     | 791 us                                                 | 919 us: 1.16x slower                                                  |
| django_template            | 34.7 ms                                                | 41.0 ms: 1.18x slower                                                 |
| xml_etree_generate         | 85.2 ms                                                | 101 ms: 1.19x slower                                                  |
| regex_compile              | 142 ms                                                 | 169 ms: 1.19x slower                                                  |
| meteor_contest             | 104 ms                                                 | 125 ms: 1.21x slower                                                  |
| bench_mp_pool              | 10.8 ms                                                | 13.2 ms: 1.22x slower                                                 |
| create_gc_cycles           | 1.09 ms                                                | 1.37 ms: 1.25x slower                                                 |
| fannkuch                   | 372 ms                                                 | 469 ms: 1.26x slower                                                  |
| xml_etree_process          | 59.0 ms                                                | 74.9 ms: 1.27x slower                                                 |
| python_startup_no_site     | 7.16 ms                                                | 9.25 ms: 1.29x slower                                                 |
| nbody                      | 89.3 ms                                                | 118 ms: 1.32x slower                                                  |
| mako                       | 11.0 ms                                                | 16.1 ms: 1.46x slower                                                 |
| bench_thread_pool          | 941 us                                                 | 1.49 ms: 1.58x slower                                                 |
| python_startup             | 9.93 ms                                                | 15.7 ms: 1.58x slower                                                 |
| coverage                   | 71.4 ms                                                | 114 ms: 1.60x slower                                                  |
| telco                      | 6.53 ms                                                | 173 ms: 26.53x slower                                                 |
| Geometric mean             | (ref)                                                  | 1.04x slower                                                          |

Benchmark hidden because not significant (2): xml_etree_iterparse, generators
Ignored benchmarks (23) of results/bm-20240906-3.12.6-a4a2d2b/bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b.json: aiohttp, asyncio_tcp, asyncio_tcp_ssl, chameleon, dask, flaskblogging, genshi_text, genshi_xml, gunicorn, mypy2, pickle, pickle_dict, pickle_list, sqlalchemy_declarative, sqlalchemy_imperative, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http, unpack_sequence, unpickle, unpickle_list
Ignored benchmarks (10) of results/bm-20260806-3.16.0a0-c3aefdb-NOGIL/bm-20260806-vultr-x86_64-python-c3aefdb9eff073405837-3.16.0a0-c3aefdb.json: connected_components, k_core, many_optionals, shortest_path, sphinx, sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile, subparsers

- Geometric mean (including insignificant results): 1.034x slower

# HPT report

- Reliability score: 81.30% likely to be slow
- 90% likely to have a slowdown of 1.00x
- 95% likely to have a slowdown of 1.00x
- 99% likely to have a slowdown of 1.00x

# Memory
- memory change: 1.37x