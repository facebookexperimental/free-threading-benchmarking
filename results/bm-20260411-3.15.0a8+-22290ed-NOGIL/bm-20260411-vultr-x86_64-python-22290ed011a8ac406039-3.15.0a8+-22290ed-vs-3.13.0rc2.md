# Results vs. 3.13.0rc2

- fork: python
- ref: 22290ed011a8ac406039
- machine: linux-x86_64
- commit hash: 22290ed
- commit date: 2026-04-11
- overall geometric mean: 1.062x slower
- HPT reliability: 93.58%
- HPT 99th percentile: 1.00x slower
- Memory change: 1.42x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260411-vultr-x86_64-python-22290ed011a8ac406039-3.15.0a8+-22290ed |
|----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| 2to3           | 260 ms                                                       | 284 ms: 1.09x slower                                                   |
| docutils       | 2.62 sec                                                     | 2.64 sec: 1.01x slower                                                 |
| html5lib       | 67.0 ms                                                      | 66.1 ms: 1.01x faster                                                  |
| Geometric mean | (ref)                                                        | 1.03x slower                                                           |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260411-vultr-x86_64-python-22290ed011a8ac406039-3.15.0a8+-22290ed |
|----------------------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| async_tree_io_tg           | 913 ms                                                       | 617 ms: 1.48x faster                                                   |
| async_tree_io              | 876 ms                                                       | 655 ms: 1.34x faster                                                   |
| async_tree_memoization     | 461 ms                                                       | 350 ms: 1.32x faster                                                   |
| async_tree_none_tg         | 336 ms                                                       | 262 ms: 1.28x faster                                                   |
| async_tree_memoization_tg  | 414 ms                                                       | 328 ms: 1.26x faster                                                   |
| async_tree_cpu_io_mixed    | 666 ms                                                       | 534 ms: 1.25x faster                                                   |
| async_tree_cpu_io_mixed_tg | 638 ms                                                       | 512 ms: 1.25x faster                                                   |
| async_tree_none            | 354 ms                                                       | 294 ms: 1.20x faster                                                   |
| asyncio_websockets         | 520 ms                                                       | 510 ms: 1.02x faster                                                   |
| async_generators           | 377 ms                                                       | 380 ms: 1.01x slower                                                   |
| coroutines                 | 23.6 ms                                                      | 24.7 ms: 1.05x slower                                                  |
| Geometric mean             | (ref)                                                        | 1.20x faster                                                           |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260411-vultr-x86_64-python-22290ed011a8ac406039-3.15.0a8+-22290ed |
|----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| pidigits       | 217 ms                                                       | 180 ms: 1.20x faster                                                   |
| float          | 77.5 ms                                                      | 74.7 ms: 1.04x faster                                                  |
| nbody          | 85.1 ms                                                      | 128 ms: 1.51x slower                                                   |
| Geometric mean | (ref)                                                        | 1.07x slower                                                           |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260411-vultr-x86_64-python-22290ed011a8ac406039-3.15.0a8+-22290ed |
|----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| regex_effbot   | 3.08 ms                                                      | 2.92 ms: 1.06x faster                                                  |
| regex_v8       | 22.7 ms                                                      | 21.7 ms: 1.05x faster                                                  |
| regex_dna      | 180 ms                                                       | 184 ms: 1.02x slower                                                   |
| regex_compile  | 132 ms                                                       | 141 ms: 1.07x slower                                                   |
| Geometric mean | (ref)                                                        | 1.00x faster                                                           |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260411-vultr-x86_64-python-22290ed011a8ac406039-3.15.0a8+-22290ed |
|----------------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| xml_etree_iterparse  | 94.9 ms                                                      | 87.4 ms: 1.09x faster                                                  |
| tomli_loads          | 2.01 sec                                                     | 1.96 sec: 1.03x faster                                                 |
| xml_etree_parse      | 136 ms                                                       | 133 ms: 1.02x faster                                                   |
| json_dumps           | 10.5 ms                                                      | 10.7 ms: 1.02x slower                                                  |
| xml_etree_generate   | 85.4 ms                                                      | 95.7 ms: 1.12x slower                                                  |
| unpickle_pure_python | 210 us                                                       | 236 us: 1.12x slower                                                   |
| pickle_pure_python   | 294 us                                                       | 339 us: 1.15x slower                                                   |
| xml_etree_process    | 59.3 ms                                                      | 68.5 ms: 1.15x slower                                                  |
| json_loads           | 27.0 us                                                      | 31.7 us: 1.17x slower                                                  |
| Geometric mean       | (ref)                                                        | 1.06x slower                                                           |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260411-vultr-x86_64-python-22290ed011a8ac406039-3.15.0a8+-22290ed |
|------------------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| python_startup_no_site | 7.39 ms                                                      | 9.89 ms: 1.34x slower                                                  |
| python_startup         | 11.0 ms                                                      | 16.1 ms: 1.47x slower                                                  |
| Geometric mean         | (ref)                                                        | 1.40x slower                                                           |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260411-vultr-x86_64-python-22290ed011a8ac406039-3.15.0a8+-22290ed |
|-----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| genshi_xml      | 48.8 ms                                                      | 54.1 ms: 1.11x slower                                                  |
| django_template | 34.1 ms                                                      | 40.1 ms: 1.17x slower                                                  |
| genshi_text     | 21.5 ms                                                      | 25.5 ms: 1.18x slower                                                  |
| mako            | 11.3 ms                                                      | 15.9 ms: 1.40x slower                                                  |
| Geometric mean  | (ref)                                                        | 1.21x slower                                                           |

All benchmarks:
===============

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260411-vultr-x86_64-python-22290ed011a8ac406039-3.15.0a8+-22290ed |
|----------------------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| mdp                        | 2.36 sec                                                     | 1.31 sec: 1.80x faster                                                 |
| gc_traversal               | 3.14 ms                                                      | 1.98 ms: 1.58x faster                                                  |
| bench_mp_pool              | 11.0 ms                                                      | 7.08 ms: 1.55x faster                                                  |
| async_tree_io_tg           | 913 ms                                                       | 617 ms: 1.48x faster                                                   |
| deepcopy                   | 355 us                                                       | 263 us: 1.35x faster                                                   |
| async_tree_io              | 876 ms                                                       | 655 ms: 1.34x faster                                                   |
| async_tree_memoization     | 461 ms                                                       | 350 ms: 1.32x faster                                                   |
| async_tree_none_tg         | 336 ms                                                       | 262 ms: 1.28x faster                                                   |
| async_tree_memoization_tg  | 414 ms                                                       | 328 ms: 1.26x faster                                                   |
| async_tree_cpu_io_mixed    | 666 ms                                                       | 534 ms: 1.25x faster                                                   |
| async_tree_cpu_io_mixed_tg | 638 ms                                                       | 512 ms: 1.25x faster                                                   |
| deepcopy_memo              | 39.1 us                                                      | 31.4 us: 1.24x faster                                                  |
| async_tree_none            | 354 ms                                                       | 294 ms: 1.20x faster                                                   |
| pidigits                   | 217 ms                                                       | 180 ms: 1.20x faster                                                   |
| go                         | 141 ms                                                       | 121 ms: 1.16x faster                                                   |
| sqlite_synth               | 2.21 us                                                      | 1.92 us: 1.15x faster                                                  |
| scimark_sor                | 134 ms                                                       | 122 ms: 1.10x faster                                                   |
| xml_etree_iterparse        | 94.9 ms                                                      | 87.4 ms: 1.09x faster                                                  |
| pathlib                    | 19.2 ms                                                      | 17.9 ms: 1.07x faster                                                  |
| deepcopy_reduce            | 3.11 us                                                      | 2.93 us: 1.06x faster                                                  |
| pylint                     | 317 ms                                                       | 299 ms: 1.06x faster                                                   |
| regex_effbot               | 3.08 ms                                                      | 2.92 ms: 1.06x faster                                                  |
| dulwich_log                | 74.8 ms                                                      | 71.4 ms: 1.05x faster                                                  |
| regex_v8                   | 22.7 ms                                                      | 21.7 ms: 1.05x faster                                                  |
| float                      | 77.5 ms                                                      | 74.7 ms: 1.04x faster                                                  |
| tomli_loads                | 2.01 sec                                                     | 1.96 sec: 1.03x faster                                                 |
| xml_etree_parse            | 136 ms                                                       | 133 ms: 1.02x faster                                                   |
| spectral_norm              | 111 ms                                                       | 109 ms: 1.02x faster                                                   |
| asyncio_websockets         | 520 ms                                                       | 510 ms: 1.02x faster                                                   |
| bpe_tokeniser              | 4.45 sec                                                     | 4.38 sec: 1.02x faster                                                 |
| html5lib                   | 67.0 ms                                                      | 66.1 ms: 1.01x faster                                                  |
| scimark_fft                | 349 ms                                                       | 348 ms: 1.00x faster                                                   |
| logging_silent             | 103 ns                                                       | 103 ns: 1.01x slower                                                   |
| pycparser                  | 1.12 sec                                                     | 1.13 sec: 1.01x slower                                                 |
| async_generators           | 377 ms                                                       | 380 ms: 1.01x slower                                                   |
| docutils                   | 2.62 sec                                                     | 2.64 sec: 1.01x slower                                                 |
| json_dumps                 | 10.5 ms                                                      | 10.7 ms: 1.02x slower                                                  |
| pyflate                    | 449 ms                                                       | 456 ms: 1.02x slower                                                   |
| regex_dna                  | 180 ms                                                       | 184 ms: 1.02x slower                                                   |
| coroutines                 | 23.6 ms                                                      | 24.7 ms: 1.05x slower                                                  |
| regex_compile              | 132 ms                                                       | 141 ms: 1.07x slower                                                   |
| comprehensions             | 16.5 us                                                      | 17.6 us: 1.07x slower                                                  |
| chaos                      | 57.3 ms                                                      | 62.0 ms: 1.08x slower                                                  |
| logging_simple             | 6.16 us                                                      | 6.67 us: 1.08x slower                                                  |
| pprint_safe_repr           | 738 ms                                                       | 801 ms: 1.09x slower                                                   |
| 2to3                       | 260 ms                                                       | 284 ms: 1.09x slower                                                   |
| json                       | 4.93 ms                                                      | 5.39 ms: 1.09x slower                                                  |
| hexiom                     | 5.99 ms                                                      | 6.56 ms: 1.09x slower                                                  |
| sympy_integrate            | 19.8 ms                                                      | 21.9 ms: 1.10x slower                                                  |
| pprint_pformat             | 1.50 sec                                                     | 1.65 sec: 1.10x slower                                                 |
| genshi_xml                 | 48.8 ms                                                      | 54.1 ms: 1.11x slower                                                  |
| nqueens                    | 78.6 ms                                                      | 87.4 ms: 1.11x slower                                                  |
| logging_format             | 6.84 us                                                      | 7.63 us: 1.11x slower                                                  |
| xml_etree_generate         | 85.4 ms                                                      | 95.7 ms: 1.12x slower                                                  |
| unpickle_pure_python       | 210 us                                                       | 236 us: 1.12x slower                                                   |
| deltablue                  | 3.12 ms                                                      | 3.51 ms: 1.12x slower                                                  |
| scimark_lu                 | 113 ms                                                       | 127 ms: 1.13x slower                                                   |
| generators                 | 28.8 ms                                                      | 32.6 ms: 1.13x slower                                                  |
| sympy_str                  | 275 ms                                                       | 312 ms: 1.14x slower                                                   |
| sympy_sum                  | 156 ms                                                       | 177 ms: 1.14x slower                                                   |
| richards_super             | 51.6 ms                                                      | 59.2 ms: 1.15x slower                                                  |
| richards                   | 45.2 ms                                                      | 51.9 ms: 1.15x slower                                                  |
| thrift                     | 778 us                                                       | 895 us: 1.15x slower                                                   |
| pickle_pure_python         | 294 us                                                       | 339 us: 1.15x slower                                                   |
| xml_etree_process          | 59.3 ms                                                      | 68.5 ms: 1.15x slower                                                  |
| scimark_sparse_mat_mult    | 4.71 ms                                                      | 5.44 ms: 1.16x slower                                                  |
| sympy_expand               | 457 ms                                                       | 532 ms: 1.16x slower                                                   |
| json_loads                 | 27.0 us                                                      | 31.7 us: 1.17x slower                                                  |
| django_template            | 34.1 ms                                                      | 40.1 ms: 1.17x slower                                                  |
| genshi_text                | 21.5 ms                                                      | 25.5 ms: 1.18x slower                                                  |
| create_gc_cycles           | 1.34 ms                                                      | 1.58 ms: 1.18x slower                                                  |
| raytrace                   | 253 ms                                                       | 304 ms: 1.20x slower                                                   |
| scimark_monte_carlo        | 65.4 ms                                                      | 79.2 ms: 1.21x slower                                                  |
| meteor_contest             | 102 ms                                                       | 123 ms: 1.21x slower                                                   |
| typing_runtime_protocols   | 155 us                                                       | 190 us: 1.23x slower                                                   |
| fannkuch                   | 370 ms                                                       | 456 ms: 1.23x slower                                                   |
| crypto_pyaes               | 67.9 ms                                                      | 87.8 ms: 1.29x slower                                                  |
| coverage                   | 83.0 ms                                                      | 109 ms: 1.31x slower                                                   |
| python_startup_no_site     | 7.39 ms                                                      | 9.89 ms: 1.34x slower                                                  |
| mako                       | 11.3 ms                                                      | 15.9 ms: 1.40x slower                                                  |
| python_startup             | 11.0 ms                                                      | 16.1 ms: 1.47x slower                                                  |
| nbody                      | 85.1 ms                                                      | 128 ms: 1.51x slower                                                   |
| bench_thread_pool          | 919 us                                                       | 1.47 ms: 1.60x slower                                                  |
| telco                      | 7.82 ms                                                      | 171 ms: 21.85x slower                                                  |
| Geometric mean             | (ref)                                                        | 1.06x slower                                                           |
Ignored benchmarks (18) of results/bm-20240906-3.13.0rc2-ec61006/bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006.json: aiohttp, asyncio_tcp, asyncio_tcp_ssl, chameleon, dask, flaskblogging, gunicorn, pickle, pickle_dict, pickle_list, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http, unpack_sequence, unpickle, unpickle_list
Ignored benchmarks (10) of results/bm-20260411-3.15.0a8+-22290ed-NOGIL/bm-20260411-vultr-x86_64-python-22290ed011a8ac406039-3.15.0a8+-22290ed.json: connected_components, k_core, many_optionals, shortest_path, sphinx, sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile, subparsers

- Geometric mean (including insignificant results): 1.062x slower

# HPT report

- Reliability score: 93.58% likely to be slow
- 90% likely to have a slowdown of 1.00x
- 95% likely to have a slowdown of 1.00x
- 99% likely to have a slowdown of 1.00x

# Memory
- memory change: 1.42x