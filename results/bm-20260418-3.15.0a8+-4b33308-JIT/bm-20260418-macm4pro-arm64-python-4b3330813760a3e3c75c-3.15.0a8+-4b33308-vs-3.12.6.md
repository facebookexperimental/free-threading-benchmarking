# Results vs. 3.12.6

- fork: python
- ref: 4b3330813760a3e3c75c
- machine: darwin-arm64
- commit hash: 4b33308
- commit date: 2026-04-18
- overall geometric mean: 1.289x faster
- HPT reliability: 100.00%
- HPT 99th percentile: 1.14x faster
- Memory change: 1.29x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260418-macm4pro-arm64-python-4b3330813760a3e3c75c-3.15.0a8+-4b33308 |
|----------------|:--------------------------------------------------------:|:------------------------------------------------------------------------:|
| 2to3           | 114 ms                                                   | 118 ms: 1.04x slower                                                     |
| docutils       | 1.02 sec                                                 | 1.00 sec: 1.02x faster                                                   |
| html5lib       | 23.0 ms                                                  | 19.1 ms: 1.20x faster                                                    |
| sphinx         | 434 ms                                                   | 396 ms: 1.10x faster                                                     |
| Geometric mean | (ref)                                                    | 1.07x faster                                                             |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                        | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260418-macm4pro-arm64-python-4b3330813760a3e3c75c-3.15.0a8+-4b33308 |
|----------------------------------|:--------------------------------------------------------:|:------------------------------------------------------------------------:|
| async_tree_eager_io              | 496 ms                                                   | 202 ms: 2.45x faster                                                     |
| async_tree_io_tg                 | 480 ms                                                   | 205 ms: 2.34x faster                                                     |
| async_tree_eager_io_tg           | 446 ms                                                   | 206 ms: 2.16x faster                                                     |
| async_tree_io                    | 459 ms                                                   | 215 ms: 2.14x faster                                                     |
| async_tree_none                  | 178 ms                                                   | 91.3 ms: 1.95x faster                                                    |
| async_tree_none_tg               | 172 ms                                                   | 92.6 ms: 1.86x faster                                                    |
| async_tree_memoization_tg        | 231 ms                                                   | 130 ms: 1.78x faster                                                     |
| async_tree_memoization           | 223 ms                                                   | 126 ms: 1.77x faster                                                     |
| async_tree_cpu_io_mixed_tg       | 338 ms                                                   | 247 ms: 1.37x faster                                                     |
| async_tree_cpu_io_mixed          | 333 ms                                                   | 246 ms: 1.35x faster                                                     |
| async_tree_eager_memoization     | 132 ms                                                   | 103 ms: 1.28x faster                                                     |
| async_tree_eager                 | 45.6 ms                                                  | 37.1 ms: 1.23x faster                                                    |
| coroutines                       | 13.6 ms                                                  | 11.1 ms: 1.23x faster                                                    |
| async_generators                 | 206 ms                                                   | 186 ms: 1.11x faster                                                     |
| async_tree_eager_cpu_io_mixed    | 231 ms                                                   | 219 ms: 1.06x faster                                                     |
| asyncio_websockets               | 190 ms                                                   | 189 ms: 1.01x faster                                                     |
| async_tree_eager_memoization_tg  | 113 ms                                                   | 118 ms: 1.05x slower                                                     |
| async_tree_eager_cpu_io_mixed_tg | 213 ms                                                   | 239 ms: 1.12x slower                                                     |
| async_tree_eager_tg              | 32.1 ms                                                  | 77.6 ms: 2.41x slower                                                    |
| Geometric mean                   | (ref)                                                    | 1.38x faster                                                             |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260418-macm4pro-arm64-python-4b3330813760a3e3c75c-3.15.0a8+-4b33308 |
|----------------|:--------------------------------------------------------:|:------------------------------------------------------------------------:|
| float          | 37.9 ms                                                  | 21.2 ms: 1.78x faster                                                    |
| nbody          | 54.2 ms                                                  | 34.2 ms: 1.59x faster                                                    |
| pidigits       | 161 ms                                                   | 164 ms: 1.02x slower                                                     |
| Geometric mean | (ref)                                                    | 1.41x faster                                                             |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260418-macm4pro-arm64-python-4b3330813760a3e3c75c-3.15.0a8+-4b33308 |
|----------------|:--------------------------------------------------------:|:------------------------------------------------------------------------:|
| regex_compile  | 54.6 ms                                                  | 39.8 ms: 1.37x faster                                                    |
| regex_effbot   | 1.67 ms                                                  | 1.47 ms: 1.13x faster                                                    |
| regex_dna      | 99.6 ms                                                  | 95.4 ms: 1.04x faster                                                    |
| Geometric mean | (ref)                                                    | 1.13x faster                                                             |

Benchmark hidden because not significant (1): regex_v8

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260418-macm4pro-arm64-python-4b3330813760a3e3c75c-3.15.0a8+-4b33308 |
|----------------------|:--------------------------------------------------------:|:------------------------------------------------------------------------:|
| tomli_loads          | 957 ms                                                   | 628 ms: 1.52x faster                                                     |
| unpickle_pure_python | 103 us                                                   | 71.6 us: 1.44x faster                                                    |
| xml_etree_iterparse  | 51.6 ms                                                  | 37.9 ms: 1.36x faster                                                    |
| pickle_pure_python   | 139 us                                                   | 111 us: 1.25x faster                                                     |
| xml_etree_process    | 26.7 ms                                                  | 21.8 ms: 1.23x faster                                                    |
| json_dumps           | 4.26 ms                                                  | 3.48 ms: 1.22x faster                                                    |
| xml_etree_generate   | 38.9 ms                                                  | 32.5 ms: 1.20x faster                                                    |
| xml_etree_parse      | 67.9 ms                                                  | 63.6 ms: 1.07x faster                                                    |
| json_loads           | 10.9 us                                                  | 10.5 us: 1.03x faster                                                    |
| Geometric mean       | (ref)                                                    | 1.25x faster                                                             |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260418-macm4pro-arm64-python-4b3330813760a3e3c75c-3.15.0a8+-4b33308 |
|------------------------|:--------------------------------------------------------:|:------------------------------------------------------------------------:|
| python_startup_no_site | 5.71 ms                                                  | 5.95 ms: 1.04x slower                                                    |
| python_startup         | 8.01 ms                                                  | 8.73 ms: 1.09x slower                                                    |
| Geometric mean         | (ref)                                                    | 1.07x slower                                                             |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260418-macm4pro-arm64-python-4b3330813760a3e3c75c-3.15.0a8+-4b33308 |
|-----------------|:--------------------------------------------------------:|:------------------------------------------------------------------------:|
| mako            | 4.77 ms                                                  | 3.96 ms: 1.21x faster                                                    |
| genshi_text     | 10.5 ms                                                  | 8.74 ms: 1.20x faster                                                    |
| django_template | 13.6 ms                                                  | 12.7 ms: 1.07x faster                                                    |
| genshi_xml      | 21.8 ms                                                  | 20.5 ms: 1.06x faster                                                    |
| Geometric mean  | (ref)                                                    | 1.13x faster                                                             |

All benchmarks:
===============

| Benchmark                        | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260418-macm4pro-arm64-python-4b3330813760a3e3c75c-3.15.0a8+-4b33308 |
|----------------------------------|:--------------------------------------------------------:|:------------------------------------------------------------------------:|
| subparsers                       | 20.8 ms                                                  | 3.59 ms: 5.79x faster                                                    |
| pylint                           | 128 ms                                                   | 50.9 ms: 2.52x faster                                                    |
| async_tree_eager_io              | 496 ms                                                   | 202 ms: 2.45x faster                                                     |
| async_tree_io_tg                 | 480 ms                                                   | 205 ms: 2.34x faster                                                     |
| richards_super                   | 25.4 ms                                                  | 11.3 ms: 2.25x faster                                                    |
| richards                         | 22.4 ms                                                  | 10.1 ms: 2.23x faster                                                    |
| async_tree_eager_io_tg           | 446 ms                                                   | 206 ms: 2.16x faster                                                     |
| async_tree_io                    | 459 ms                                                   | 215 ms: 2.14x faster                                                     |
| mdp                              | 1.09 sec                                                 | 520 ms: 2.10x faster                                                     |
| async_tree_none                  | 178 ms                                                   | 91.3 ms: 1.95x faster                                                    |
| async_tree_none_tg               | 172 ms                                                   | 92.6 ms: 1.86x faster                                                    |
| scimark_sor                      | 61.0 ms                                                  | 33.2 ms: 1.84x faster                                                    |
| scimark_lu                       | 51.3 ms                                                  | 28.4 ms: 1.81x faster                                                    |
| float                            | 37.9 ms                                                  | 21.2 ms: 1.78x faster                                                    |
| async_tree_memoization_tg        | 231 ms                                                   | 130 ms: 1.78x faster                                                     |
| async_tree_memoization           | 223 ms                                                   | 126 ms: 1.77x faster                                                     |
| deepcopy                         | 161 us                                                   | 94.5 us: 1.71x faster                                                    |
| comprehensions                   | 9.84 us                                                  | 5.76 us: 1.71x faster                                                    |
| deepcopy_memo                    | 18.3 us                                                  | 11.0 us: 1.66x faster                                                    |
| spectral_norm                    | 54.4 ms                                                  | 34.0 ms: 1.60x faster                                                    |
| nbody                            | 54.2 ms                                                  | 34.2 ms: 1.59x faster                                                    |
| deltablue                        | 1.73 ms                                                  | 1.12 ms: 1.54x faster                                                    |
| tomli_loads                      | 957 ms                                                   | 628 ms: 1.52x faster                                                     |
| logging_silent                   | 50.9 ns                                                  | 33.7 ns: 1.51x faster                                                    |
| pyflate                          | 216 ms                                                   | 149 ms: 1.45x faster                                                     |
| scimark_fft                      | 142 ms                                                   | 97.8 ms: 1.45x faster                                                    |
| raytrace                         | 145 ms                                                   | 101 ms: 1.44x faster                                                     |
| unpickle_pure_python             | 103 us                                                   | 71.6 us: 1.44x faster                                                    |
| go                               | 70.0 ms                                                  | 49.0 ms: 1.43x faster                                                    |
| chaos                            | 28.9 ms                                                  | 20.2 ms: 1.43x faster                                                    |
| deepcopy_reduce                  | 1.46 us                                                  | 1.03 us: 1.42x faster                                                    |
| logging_simple                   | 2.57 us                                                  | 1.81 us: 1.42x faster                                                    |
| logging_format                   | 2.80 us                                                  | 1.97 us: 1.42x faster                                                    |
| scimark_monte_carlo              | 32.2 ms                                                  | 23.2 ms: 1.39x faster                                                    |
| async_tree_cpu_io_mixed_tg       | 338 ms                                                   | 247 ms: 1.37x faster                                                     |
| regex_compile                    | 54.6 ms                                                  | 39.8 ms: 1.37x faster                                                    |
| xml_etree_iterparse              | 51.6 ms                                                  | 37.9 ms: 1.36x faster                                                    |
| nqueens                          | 43.5 ms                                                  | 32.1 ms: 1.35x faster                                                    |
| async_tree_cpu_io_mixed          | 333 ms                                                   | 246 ms: 1.35x faster                                                     |
| pprint_pformat                   | 665 ms                                                   | 510 ms: 1.30x faster                                                     |
| pprint_safe_repr                 | 328 ms                                                   | 253 ms: 1.29x faster                                                     |
| k_core                           | 1.12 sec                                                 | 868 ms: 1.29x faster                                                     |
| async_tree_eager_memoization     | 132 ms                                                   | 103 ms: 1.28x faster                                                     |
| bpe_tokeniser                    | 2.24 sec                                                 | 1.79 sec: 1.25x faster                                                   |
| pickle_pure_python               | 139 us                                                   | 111 us: 1.25x faster                                                     |
| hexiom                           | 3.04 ms                                                  | 2.46 ms: 1.23x faster                                                    |
| async_tree_eager                 | 45.6 ms                                                  | 37.1 ms: 1.23x faster                                                    |
| coroutines                       | 13.6 ms                                                  | 11.1 ms: 1.23x faster                                                    |
| xml_etree_process                | 26.7 ms                                                  | 21.8 ms: 1.23x faster                                                    |
| json_dumps                       | 4.26 ms                                                  | 3.48 ms: 1.22x faster                                                    |
| crypto_pyaes                     | 38.8 ms                                                  | 31.9 ms: 1.22x faster                                                    |
| mako                             | 4.77 ms                                                  | 3.96 ms: 1.21x faster                                                    |
| html5lib                         | 23.0 ms                                                  | 19.1 ms: 1.20x faster                                                    |
| genshi_text                      | 10.5 ms                                                  | 8.74 ms: 1.20x faster                                                    |
| xml_etree_generate               | 38.9 ms                                                  | 32.5 ms: 1.20x faster                                                    |
| scimark_sparse_mat_mult          | 2.08 ms                                                  | 1.75 ms: 1.19x faster                                                    |
| dulwich_log                      | 21.3 ms                                                  | 17.9 ms: 1.19x faster                                                    |
| pathlib                          | 12.4 ms                                                  | 10.6 ms: 1.16x faster                                                    |
| generators                       | 21.9 ms                                                  | 19.1 ms: 1.15x faster                                                    |
| regex_effbot                     | 1.67 ms                                                  | 1.47 ms: 1.13x faster                                                    |
| typing_runtime_protocols         | 71.0 us                                                  | 62.7 us: 1.13x faster                                                    |
| fannkuch                         | 176 ms                                                   | 157 ms: 1.12x faster                                                     |
| async_generators                 | 206 ms                                                   | 186 ms: 1.11x faster                                                     |
| pycparser                        | 497 ms                                                   | 453 ms: 1.10x faster                                                     |
| sphinx                           | 434 ms                                                   | 396 ms: 1.10x faster                                                     |
| django_template                  | 13.6 ms                                                  | 12.7 ms: 1.07x faster                                                    |
| xml_etree_parse                  | 67.9 ms                                                  | 63.6 ms: 1.07x faster                                                    |
| genshi_xml                       | 21.8 ms                                                  | 20.5 ms: 1.06x faster                                                    |
| async_tree_eager_cpu_io_mixed    | 231 ms                                                   | 219 ms: 1.06x faster                                                     |
| sympy_sum                        | 57.6 ms                                                  | 54.7 ms: 1.05x faster                                                    |
| sympy_str                        | 104 ms                                                   | 99.3 ms: 1.05x faster                                                    |
| json                             | 1.93 ms                                                  | 1.84 ms: 1.05x faster                                                    |
| regex_dna                        | 99.6 ms                                                  | 95.4 ms: 1.04x faster                                                    |
| sympy_integrate                  | 8.02 ms                                                  | 7.71 ms: 1.04x faster                                                    |
| json_loads                       | 10.9 us                                                  | 10.5 us: 1.03x faster                                                    |
| thrift                           | 322 us                                                   | 313 us: 1.03x faster                                                     |
| sqlite_synth                     | 967 ns                                                   | 944 ns: 1.02x faster                                                     |
| meteor_contest                   | 47.7 ms                                                  | 46.6 ms: 1.02x faster                                                    |
| docutils                         | 1.02 sec                                                 | 1.00 sec: 1.02x faster                                                   |
| sympy_expand                     | 167 ms                                                   | 164 ms: 1.02x faster                                                     |
| dask                             | 528 ms                                                   | 521 ms: 1.01x faster                                                     |
| asyncio_websockets               | 190 ms                                                   | 189 ms: 1.01x faster                                                     |
| telco                            | 2.61 ms                                                  | 2.63 ms: 1.01x slower                                                    |
| pidigits                         | 161 ms                                                   | 164 ms: 1.02x slower                                                     |
| 2to3                             | 114 ms                                                   | 118 ms: 1.04x slower                                                     |
| bench_thread_pool                | 419 us                                                   | 436 us: 1.04x slower                                                     |
| python_startup_no_site           | 5.71 ms                                                  | 5.95 ms: 1.04x slower                                                    |
| async_tree_eager_memoization_tg  | 113 ms                                                   | 118 ms: 1.05x slower                                                     |
| python_startup                   | 8.01 ms                                                  | 8.73 ms: 1.09x slower                                                    |
| async_tree_eager_cpu_io_mixed_tg | 213 ms                                                   | 239 ms: 1.12x slower                                                     |
| create_gc_cycles                 | 830 us                                                   | 978 us: 1.18x slower                                                     |
| bench_mp_pool                    | 39.7 ms                                                  | 47.3 ms: 1.19x slower                                                    |
| many_optionals                   | 195 us                                                   | 236 us: 1.21x slower                                                     |
| gc_traversal                     | 2.01 ms                                                  | 2.44 ms: 1.22x slower                                                    |
| coverage                         | 26.9 ms                                                  | 33.2 ms: 1.24x slower                                                    |
| async_tree_eager_tg              | 32.1 ms                                                  | 77.6 ms: 2.41x slower                                                    |
| Geometric mean                   | (ref)                                                    | 1.28x faster                                                             |

Benchmark hidden because not significant (3): shortest_path, connected_components, regex_v8
Ignored benchmarks (10) of results/bm-20240906-3.12.6-a4a2d2b/bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b.json: chameleon, djangocms, gevent_hub, sqlalchemy_declarative, sqlalchemy_imperative, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http
Ignored benchmarks (12) of results/bm-20260418-3.15.0a8+-4b33308-JIT/bm-20260418-macm4pro-arm64-python-4b3330813760a3e3c75c-3.15.0a8+-4b33308.json: asyncio_tcp, asyncio_tcp_ssl, pickle, pickle_dict, pickle_list, sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile, unpack_sequence, unpickle, unpickle_list

- Geometric mean (including insignificant results): 1.289x faster

# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.19x
- 95% likely to have a speedup of 1.18x
- 99% likely to have a speedup of 1.14x

# Memory
- memory change: 1.29x