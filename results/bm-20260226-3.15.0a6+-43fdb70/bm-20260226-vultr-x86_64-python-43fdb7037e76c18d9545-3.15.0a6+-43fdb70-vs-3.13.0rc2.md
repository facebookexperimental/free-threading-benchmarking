# Results vs. 3.13.0rc2

- fork: python
- ref: 43fdb7037e76c18d9545
- machine: linux-x86_64
- commit hash: 43fdb70
- commit date: 2026-02-26
- overall geometric mean: 1.034x faster
- HPT reliability: 100.00%
- HPT 99th percentile: 1.02x faster
- Memory change: 1.16x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260226-vultr-x86_64-python-43fdb7037e76c18d9545-3.15.0a6+-43fdb70 |
|----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| 2to3           | 260 ms                                                       | 251 ms: 1.03x faster                                                   |
| docutils       | 2.62 sec                                                     | 2.55 sec: 1.03x faster                                                 |
| html5lib       | 67.0 ms                                                      | 61.5 ms: 1.09x faster                                                  |
| Geometric mean | (ref)                                                        | 1.05x faster                                                           |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260226-vultr-x86_64-python-43fdb7037e76c18d9545-3.15.0a6+-43fdb70 |
|----------------------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| async_tree_io_tg           | 913 ms                                                       | 565 ms: 1.62x faster                                                   |
| async_tree_io              | 876 ms                                                       | 585 ms: 1.50x faster                                                   |
| async_tree_memoization     | 461 ms                                                       | 310 ms: 1.49x faster                                                   |
| async_tree_none            | 354 ms                                                       | 248 ms: 1.43x faster                                                   |
| async_tree_none_tg         | 336 ms                                                       | 240 ms: 1.40x faster                                                   |
| async_tree_memoization_tg  | 414 ms                                                       | 301 ms: 1.38x faster                                                   |
| async_tree_cpu_io_mixed    | 666 ms                                                       | 561 ms: 1.19x faster                                                   |
| async_generators           | 377 ms                                                       | 337 ms: 1.12x faster                                                   |
| async_tree_cpu_io_mixed_tg | 638 ms                                                       | 577 ms: 1.10x faster                                                   |
| coroutines                 | 23.6 ms                                                      | 23.5 ms: 1.00x faster                                                  |
| asyncio_websockets         | 520 ms                                                       | 544 ms: 1.05x slower                                                   |
| Geometric mean             | (ref)                                                        | 1.27x faster                                                           |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260226-vultr-x86_64-python-43fdb7037e76c18d9545-3.15.0a6+-43fdb70 |
|----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| float          | 77.5 ms                                                      | 68.1 ms: 1.14x faster                                                  |
| pidigits       | 217 ms                                                       | 228 ms: 1.05x slower                                                   |
| nbody          | 85.1 ms                                                      | 91.2 ms: 1.07x slower                                                  |
| Geometric mean | (ref)                                                        | 1.00x faster                                                           |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260226-vultr-x86_64-python-43fdb7037e76c18d9545-3.15.0a6+-43fdb70 |
|----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| regex_compile  | 132 ms                                                       | 123 ms: 1.07x faster                                                   |
| regex_effbot   | 3.08 ms                                                      | 2.95 ms: 1.04x faster                                                  |
| regex_v8       | 22.7 ms                                                      | 22.3 ms: 1.02x faster                                                  |
| regex_dna      | 180 ms                                                       | 178 ms: 1.01x faster                                                   |
| Geometric mean | (ref)                                                        | 1.04x faster                                                           |

Benchmarks with tag 'serialize':
================================

| Benchmark           | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260226-vultr-x86_64-python-43fdb7037e76c18d9545-3.15.0a6+-43fdb70 |
|---------------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| xml_etree_iterparse | 94.9 ms                                                      | 85.0 ms: 1.12x faster                                                  |
| tomli_loads         | 2.01 sec                                                     | 1.82 sec: 1.10x faster                                                 |
| json_dumps          | 10.5 ms                                                      | 10.0 ms: 1.05x faster                                                  |
| xml_etree_parse     | 136 ms                                                       | 131 ms: 1.04x faster                                                   |
| xml_etree_generate  | 85.4 ms                                                      | 84.6 ms: 1.01x faster                                                  |
| json_loads          | 27.0 us                                                      | 28.0 us: 1.04x slower                                                  |
| pickle_pure_python  | 294 us                                                       | 307 us: 1.04x slower                                                   |
| Geometric mean      | (ref)                                                        | 1.02x faster                                                           |

Benchmark hidden because not significant (2): xml_etree_process, unpickle_pure_python

Benchmarks with tag 'startup':
==============================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260226-vultr-x86_64-python-43fdb7037e76c18d9545-3.15.0a6+-43fdb70 |
|----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| python_startup | 11.0 ms                                                      | 12.6 ms: 1.14x slower                                                  |
| Geometric mean | (ref)                                                        | 1.07x slower                                                           |

Benchmark hidden because not significant (1): python_startup_no_site

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260226-vultr-x86_64-python-43fdb7037e76c18d9545-3.15.0a6+-43fdb70 |
|-----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| genshi_text     | 21.5 ms                                                      | 21.0 ms: 1.03x faster                                                  |
| genshi_xml      | 48.8 ms                                                      | 49.1 ms: 1.01x slower                                                  |
| mako            | 11.3 ms                                                      | 11.6 ms: 1.02x slower                                                  |
| django_template | 34.1 ms                                                      | 34.8 ms: 1.02x slower                                                  |
| Geometric mean  | (ref)                                                        | 1.01x slower                                                           |

All benchmarks:
===============

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260226-vultr-x86_64-python-43fdb7037e76c18d9545-3.15.0a6+-43fdb70 |
|----------------------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| mdp                        | 2.36 sec                                                     | 1.14 sec: 2.06x faster                                                 |
| async_tree_io_tg           | 913 ms                                                       | 565 ms: 1.62x faster                                                   |
| deepcopy_memo              | 39.1 us                                                      | 25.6 us: 1.53x faster                                                  |
| deepcopy                   | 355 us                                                       | 233 us: 1.53x faster                                                   |
| async_tree_io              | 876 ms                                                       | 585 ms: 1.50x faster                                                   |
| async_tree_memoization     | 461 ms                                                       | 310 ms: 1.49x faster                                                   |
| async_tree_none            | 354 ms                                                       | 248 ms: 1.43x faster                                                   |
| async_tree_none_tg         | 336 ms                                                       | 240 ms: 1.40x faster                                                   |
| async_tree_memoization_tg  | 414 ms                                                       | 301 ms: 1.38x faster                                                   |
| go                         | 141 ms                                                       | 106 ms: 1.33x faster                                                   |
| scimark_sor                | 134 ms                                                       | 107 ms: 1.26x faster                                                   |
| spectral_norm              | 111 ms                                                       | 89.6 ms: 1.24x faster                                                  |
| deepcopy_reduce            | 3.11 us                                                      | 2.56 us: 1.22x faster                                                  |
| async_tree_cpu_io_mixed    | 666 ms                                                       | 561 ms: 1.19x faster                                                   |
| scimark_fft                | 349 ms                                                       | 305 ms: 1.15x faster                                                   |
| float                      | 77.5 ms                                                      | 68.1 ms: 1.14x faster                                                  |
| async_generators           | 377 ms                                                       | 337 ms: 1.12x faster                                                   |
| xml_etree_iterparse        | 94.9 ms                                                      | 85.0 ms: 1.12x faster                                                  |
| pylint                     | 317 ms                                                       | 285 ms: 1.11x faster                                                   |
| pathlib                    | 19.2 ms                                                      | 17.3 ms: 1.11x faster                                                  |
| async_tree_cpu_io_mixed_tg | 638 ms                                                       | 577 ms: 1.10x faster                                                   |
| tomli_loads                | 2.01 sec                                                     | 1.82 sec: 1.10x faster                                                 |
| dulwich_log                | 74.8 ms                                                      | 68.0 ms: 1.10x faster                                                  |
| pyflate                    | 449 ms                                                       | 410 ms: 1.09x faster                                                   |
| html5lib                   | 67.0 ms                                                      | 61.5 ms: 1.09x faster                                                  |
| richards_super             | 51.6 ms                                                      | 48.1 ms: 1.07x faster                                                  |
| scimark_sparse_mat_mult    | 4.71 ms                                                      | 4.39 ms: 1.07x faster                                                  |
| richards                   | 45.2 ms                                                      | 42.2 ms: 1.07x faster                                                  |
| hexiom                     | 5.99 ms                                                      | 5.59 ms: 1.07x faster                                                  |
| regex_compile              | 132 ms                                                       | 123 ms: 1.07x faster                                                   |
| bpe_tokeniser              | 4.45 sec                                                     | 4.23 sec: 1.05x faster                                                 |
| json_dumps                 | 10.5 ms                                                      | 10.0 ms: 1.05x faster                                                  |
| chaos                      | 57.3 ms                                                      | 54.6 ms: 1.05x faster                                                  |
| scimark_monte_carlo        | 65.4 ms                                                      | 62.3 ms: 1.05x faster                                                  |
| regex_effbot               | 3.08 ms                                                      | 2.95 ms: 1.04x faster                                                  |
| logging_silent             | 103 ns                                                       | 98.4 ns: 1.04x faster                                                  |
| pprint_safe_repr           | 738 ms                                                       | 708 ms: 1.04x faster                                                   |
| logging_simple             | 6.16 us                                                      | 5.92 us: 1.04x faster                                                  |
| pprint_pformat             | 1.50 sec                                                     | 1.44 sec: 1.04x faster                                                 |
| sympy_integrate            | 19.8 ms                                                      | 19.1 ms: 1.04x faster                                                  |
| xml_etree_parse            | 136 ms                                                       | 131 ms: 1.04x faster                                                   |
| scimark_lu                 | 113 ms                                                       | 109 ms: 1.04x faster                                                   |
| 2to3                       | 260 ms                                                       | 251 ms: 1.03x faster                                                   |
| genshi_text                | 21.5 ms                                                      | 21.0 ms: 1.03x faster                                                  |
| docutils                   | 2.62 sec                                                     | 2.55 sec: 1.03x faster                                                 |
| pycparser                  | 1.12 sec                                                     | 1.10 sec: 1.02x faster                                                 |
| regex_v8                   | 22.7 ms                                                      | 22.3 ms: 1.02x faster                                                  |
| nqueens                    | 78.6 ms                                                      | 77.1 ms: 1.02x faster                                                  |
| logging_format             | 6.84 us                                                      | 6.74 us: 1.02x faster                                                  |
| raytrace                   | 253 ms                                                       | 250 ms: 1.01x faster                                                   |
| comprehensions             | 16.5 us                                                      | 16.3 us: 1.01x faster                                                  |
| regex_dna                  | 180 ms                                                       | 178 ms: 1.01x faster                                                   |
| xml_etree_generate         | 85.4 ms                                                      | 84.6 ms: 1.01x faster                                                  |
| crypto_pyaes               | 67.9 ms                                                      | 67.5 ms: 1.01x faster                                                  |
| coroutines                 | 23.6 ms                                                      | 23.5 ms: 1.00x faster                                                  |
| sympy_sum                  | 156 ms                                                       | 156 ms: 1.01x slower                                                   |
| genshi_xml                 | 48.8 ms                                                      | 49.1 ms: 1.01x slower                                                  |
| sympy_str                  | 275 ms                                                       | 276 ms: 1.01x slower                                                   |
| sqlite_synth               | 2.21 us                                                      | 2.24 us: 1.01x slower                                                  |
| mako                       | 11.3 ms                                                      | 11.6 ms: 1.02x slower                                                  |
| django_template            | 34.1 ms                                                      | 34.8 ms: 1.02x slower                                                  |
| json                       | 4.93 ms                                                      | 5.05 ms: 1.03x slower                                                  |
| fannkuch                   | 370 ms                                                       | 383 ms: 1.04x slower                                                   |
| typing_runtime_protocols   | 155 us                                                       | 160 us: 1.04x slower                                                   |
| json_loads                 | 27.0 us                                                      | 28.0 us: 1.04x slower                                                  |
| pickle_pure_python         | 294 us                                                       | 307 us: 1.04x slower                                                   |
| asyncio_websockets         | 520 ms                                                       | 544 ms: 1.05x slower                                                   |
| pidigits                   | 217 ms                                                       | 228 ms: 1.05x slower                                                   |
| sympy_expand               | 457 ms                                                       | 482 ms: 1.06x slower                                                   |
| deltablue                  | 3.12 ms                                                      | 3.33 ms: 1.06x slower                                                  |
| nbody                      | 85.1 ms                                                      | 91.2 ms: 1.07x slower                                                  |
| python_startup             | 11.0 ms                                                      | 12.6 ms: 1.14x slower                                                  |
| gc_traversal               | 3.14 ms                                                      | 4.24 ms: 1.35x slower                                                  |
| create_gc_cycles           | 1.34 ms                                                      | 1.85 ms: 1.38x slower                                                  |
| bench_thread_pool          | 919 us                                                       | 1.32 ms: 1.43x slower                                                  |
| telco                      | 7.82 ms                                                      | 157 ms: 20.09x slower                                                  |
| bench_mp_pool              | 11.0 ms                                                      | 310 ms: 28.22x slower                                                  |
| Geometric mean             | (ref)                                                        | 1.01x slower                                                           |

Benchmark hidden because not significant (7): meteor_contest, generators, thrift, python_startup_no_site, coverage, xml_etree_process, unpickle_pure_python
Ignored benchmarks (18) of results/bm-20240906-3.13.0rc2-ec61006/bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006.json: aiohttp, asyncio_tcp, asyncio_tcp_ssl, chameleon, dask, flaskblogging, gunicorn, pickle, pickle_dict, pickle_list, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http, unpack_sequence, unpickle, unpickle_list
Ignored benchmarks (10) of results/bm-20260226-3.15.0a6+-43fdb70/bm-20260226-vultr-x86_64-python-43fdb7037e76c18d9545-3.15.0a6+-43fdb70.json: connected_components, k_core, many_optionals, shortest_path, sphinx, sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile, subparsers

- Geometric mean (including insignificant results): 1.034x faster

# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.02x
- 95% likely to have a speedup of 1.02x
- 99% likely to have a speedup of 1.02x

# Memory
- memory change: 1.16x