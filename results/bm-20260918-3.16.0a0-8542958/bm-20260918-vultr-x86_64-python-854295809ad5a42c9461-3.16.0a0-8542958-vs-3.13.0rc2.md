# Results vs. 3.13.0rc2

- fork: python
- ref: 854295809ad5a42c9461
- machine: linux-x86_64
- commit hash: 8542958
- commit date: 2026-09-18
- overall geometric mean: 1.028x faster
- HPT reliability: 99.98%
- HPT 99th percentile: 1.01x faster
- Memory change: 1.14x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260918-vultr-x86_64-python-854295809ad5a42c9461-3.16.0a0-8542958 |
|----------------|:------------------------------------------------------------:|:---------------------------------------------------------------------:|
| 2to3           | 260 ms                                                       | 258 ms: 1.01x faster                                                  |
| docutils       | 2.62 sec                                                     | 2.37 sec: 1.10x faster                                                |
| html5lib       | 67.0 ms                                                      | 57.9 ms: 1.16x faster                                                 |
| Geometric mean | (ref)                                                        | 1.09x faster                                                          |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260918-vultr-x86_64-python-854295809ad5a42c9461-3.16.0a0-8542958 |
|----------------------------|:------------------------------------------------------------:|:---------------------------------------------------------------------:|
| async_tree_io_tg           | 913 ms                                                       | 760 ms: 1.20x faster                                                  |
| async_tree_cpu_io_mixed    | 666 ms                                                       | 559 ms: 1.19x faster                                                  |
| async_tree_cpu_io_mixed_tg | 638 ms                                                       | 556 ms: 1.15x faster                                                  |
| async_tree_memoization     | 461 ms                                                       | 405 ms: 1.14x faster                                                  |
| async_tree_memoization_tg  | 414 ms                                                       | 366 ms: 1.13x faster                                                  |
| async_tree_none_tg         | 336 ms                                                       | 299 ms: 1.13x faster                                                  |
| async_tree_io              | 876 ms                                                       | 783 ms: 1.12x faster                                                  |
| async_generators           | 377 ms                                                       | 349 ms: 1.08x faster                                                  |
| async_tree_none            | 354 ms                                                       | 332 ms: 1.07x faster                                                  |
| coroutines                 | 23.6 ms                                                      | 23.9 ms: 1.02x slower                                                 |
| asyncio_websockets         | 520 ms                                                       | 544 ms: 1.05x slower                                                  |
| Geometric mean             | (ref)                                                        | 1.10x faster                                                          |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260918-vultr-x86_64-python-854295809ad5a42c9461-3.16.0a0-8542958 |
|----------------|:------------------------------------------------------------:|:---------------------------------------------------------------------:|
| pidigits       | 217 ms                                                       | 191 ms: 1.14x faster                                                  |
| float          | 77.5 ms                                                      | 71.8 ms: 1.08x faster                                                 |
| nbody          | 85.1 ms                                                      | 92.8 ms: 1.09x slower                                                 |
| Geometric mean | (ref)                                                        | 1.04x faster                                                          |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260918-vultr-x86_64-python-854295809ad5a42c9461-3.16.0a0-8542958 |
|----------------|:------------------------------------------------------------:|:---------------------------------------------------------------------:|
| regex_effbot   | 3.08 ms                                                      | 2.52 ms: 1.23x faster                                                 |
| regex_v8       | 22.7 ms                                                      | 20.7 ms: 1.10x faster                                                 |
| regex_dna      | 180 ms                                                       | 179 ms: 1.01x faster                                                  |
| regex_compile  | 132 ms                                                       | 149 ms: 1.13x slower                                                  |
| Geometric mean | (ref)                                                        | 1.05x faster                                                          |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260918-vultr-x86_64-python-854295809ad5a42c9461-3.16.0a0-8542958 |
|----------------------|:------------------------------------------------------------:|:---------------------------------------------------------------------:|
| json_dumps           | 10.5 ms                                                      | 9.44 ms: 1.12x faster                                                 |
| tomli_loads          | 2.01 sec                                                     | 1.81 sec: 1.11x faster                                                |
| xml_etree_iterparse  | 94.9 ms                                                      | 92.4 ms: 1.03x faster                                                 |
| json_loads           | 27.0 us                                                      | 27.2 us: 1.01x slower                                                 |
| unpickle_pure_python | 210 us                                                       | 212 us: 1.01x slower                                                  |
| pickle_pure_python   | 294 us                                                       | 308 us: 1.05x slower                                                  |
| xml_etree_generate   | 85.4 ms                                                      | 89.4 ms: 1.05x slower                                                 |
| xml_etree_parse      | 136 ms                                                       | 143 ms: 1.05x slower                                                  |
| xml_etree_process    | 59.3 ms                                                      | 63.5 ms: 1.07x slower                                                 |
| Geometric mean       | (ref)                                                        | 1.00x faster                                                          |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260918-vultr-x86_64-python-854295809ad5a42c9461-3.16.0a0-8542958 |
|------------------------|:------------------------------------------------------------:|:---------------------------------------------------------------------:|
| python_startup_no_site | 7.39 ms                                                      | 7.75 ms: 1.05x slower                                                 |
| python_startup         | 11.0 ms                                                      | 12.9 ms: 1.18x slower                                                 |
| Geometric mean         | (ref)                                                        | 1.11x slower                                                          |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260918-vultr-x86_64-python-854295809ad5a42c9461-3.16.0a0-8542958 |
|-----------------|:------------------------------------------------------------:|:---------------------------------------------------------------------:|
| mako            | 11.3 ms                                                      | 12.0 ms: 1.06x slower                                                 |
| django_template | 34.1 ms                                                      | 36.9 ms: 1.08x slower                                                 |
| Geometric mean  | (ref)                                                        | 1.07x slower                                                          |

All benchmarks:
===============

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260918-vultr-x86_64-python-854295809ad5a42c9461-3.16.0a0-8542958 |
|----------------------------|:------------------------------------------------------------:|:---------------------------------------------------------------------:|
| pylint                     | 317 ms                                                       | 113 ms: 2.81x faster                                                  |
| mdp                        | 2.36 sec                                                     | 1.13 sec: 2.08x faster                                                |
| deepcopy                   | 355 us                                                       | 228 us: 1.56x faster                                                  |
| deepcopy_memo              | 39.1 us                                                      | 27.7 us: 1.41x faster                                                 |
| go                         | 141 ms                                                       | 102 ms: 1.38x faster                                                  |
| typing_runtime_protocols   | 155 us                                                       | 118 us: 1.32x faster                                                  |
| scimark_sor                | 134 ms                                                       | 106 ms: 1.26x faster                                                  |
| regex_effbot               | 3.08 ms                                                      | 2.52 ms: 1.23x faster                                                 |
| deepcopy_reduce            | 3.11 us                                                      | 2.55 us: 1.22x faster                                                 |
| async_tree_io_tg           | 913 ms                                                       | 760 ms: 1.20x faster                                                  |
| spectral_norm              | 111 ms                                                       | 92.5 ms: 1.20x faster                                                 |
| async_tree_cpu_io_mixed    | 666 ms                                                       | 559 ms: 1.19x faster                                                  |
| html5lib                   | 67.0 ms                                                      | 57.9 ms: 1.16x faster                                                 |
| pyflate                    | 449 ms                                                       | 388 ms: 1.16x faster                                                  |
| async_tree_cpu_io_mixed_tg | 638 ms                                                       | 556 ms: 1.15x faster                                                  |
| async_tree_memoization     | 461 ms                                                       | 405 ms: 1.14x faster                                                  |
| pidigits                   | 217 ms                                                       | 191 ms: 1.14x faster                                                  |
| async_tree_memoization_tg  | 414 ms                                                       | 366 ms: 1.13x faster                                                  |
| dulwich_log                | 74.8 ms                                                      | 66.3 ms: 1.13x faster                                                 |
| async_tree_none_tg         | 336 ms                                                       | 299 ms: 1.13x faster                                                  |
| async_tree_io              | 876 ms                                                       | 783 ms: 1.12x faster                                                  |
| json_dumps                 | 10.5 ms                                                      | 9.44 ms: 1.12x faster                                                 |
| scimark_fft                | 349 ms                                                       | 315 ms: 1.11x faster                                                  |
| tomli_loads                | 2.01 sec                                                     | 1.81 sec: 1.11x faster                                                |
| docutils                   | 2.62 sec                                                     | 2.37 sec: 1.10x faster                                                |
| regex_v8                   | 22.7 ms                                                      | 20.7 ms: 1.10x faster                                                 |
| logging_silent             | 103 ns                                                       | 94.1 ns: 1.09x faster                                                 |
| async_generators           | 377 ms                                                       | 349 ms: 1.08x faster                                                  |
| chaos                      | 57.3 ms                                                      | 53.1 ms: 1.08x faster                                                 |
| pathlib                    | 19.2 ms                                                      | 17.8 ms: 1.08x faster                                                 |
| float                      | 77.5 ms                                                      | 71.8 ms: 1.08x faster                                                 |
| async_tree_none            | 354 ms                                                       | 332 ms: 1.07x faster                                                  |
| comprehensions             | 16.5 us                                                      | 15.5 us: 1.06x faster                                                 |
| nqueens                    | 78.6 ms                                                      | 74.1 ms: 1.06x faster                                                 |
| bpe_tokeniser              | 4.45 sec                                                     | 4.23 sec: 1.05x faster                                                |
| hexiom                     | 5.99 ms                                                      | 5.72 ms: 1.05x faster                                                 |
| scimark_monte_carlo        | 65.4 ms                                                      | 62.7 ms: 1.04x faster                                                 |
| scimark_sparse_mat_mult    | 4.71 ms                                                      | 4.52 ms: 1.04x faster                                                 |
| generators                 | 28.8 ms                                                      | 27.8 ms: 1.03x faster                                                 |
| xml_etree_iterparse        | 94.9 ms                                                      | 92.4 ms: 1.03x faster                                                 |
| sympy_integrate            | 19.8 ms                                                      | 19.4 ms: 1.02x faster                                                 |
| logging_simple             | 6.16 us                                                      | 6.03 us: 1.02x faster                                                 |
| pprint_safe_repr           | 738 ms                                                       | 722 ms: 1.02x faster                                                  |
| raytrace                   | 253 ms                                                       | 248 ms: 1.02x faster                                                  |
| richards                   | 45.2 ms                                                      | 44.5 ms: 1.02x faster                                                 |
| meteor_contest             | 102 ms                                                       | 100 ms: 1.01x faster                                                  |
| regex_dna                  | 180 ms                                                       | 179 ms: 1.01x faster                                                  |
| 2to3                       | 260 ms                                                       | 258 ms: 1.01x faster                                                  |
| pprint_pformat             | 1.50 sec                                                     | 1.49 sec: 1.01x faster                                                |
| crypto_pyaes               | 67.9 ms                                                      | 67.6 ms: 1.00x faster                                                 |
| richards_super             | 51.6 ms                                                      | 51.4 ms: 1.00x faster                                                 |
| json_loads                 | 27.0 us                                                      | 27.2 us: 1.01x slower                                                 |
| sqlite_synth               | 2.21 us                                                      | 2.23 us: 1.01x slower                                                 |
| unpickle_pure_python       | 210 us                                                       | 212 us: 1.01x slower                                                  |
| thrift                     | 778 us                                                       | 788 us: 1.01x slower                                                  |
| json                       | 4.93 ms                                                      | 4.99 ms: 1.01x slower                                                 |
| sympy_str                  | 275 ms                                                       | 279 ms: 1.01x slower                                                  |
| coroutines                 | 23.6 ms                                                      | 23.9 ms: 1.02x slower                                                 |
| fannkuch                   | 370 ms                                                       | 377 ms: 1.02x slower                                                  |
| sympy_sum                  | 156 ms                                                       | 159 ms: 1.02x slower                                                  |
| scimark_lu                 | 113 ms                                                       | 118 ms: 1.04x slower                                                  |
| sympy_expand               | 457 ms                                                       | 477 ms: 1.04x slower                                                  |
| asyncio_websockets         | 520 ms                                                       | 544 ms: 1.05x slower                                                  |
| pickle_pure_python         | 294 us                                                       | 308 us: 1.05x slower                                                  |
| xml_etree_generate         | 85.4 ms                                                      | 89.4 ms: 1.05x slower                                                 |
| python_startup_no_site     | 7.39 ms                                                      | 7.75 ms: 1.05x slower                                                 |
| xml_etree_parse            | 136 ms                                                       | 143 ms: 1.05x slower                                                  |
| mako                       | 11.3 ms                                                      | 12.0 ms: 1.06x slower                                                 |
| deltablue                  | 3.12 ms                                                      | 3.32 ms: 1.06x slower                                                 |
| xml_etree_process          | 59.3 ms                                                      | 63.5 ms: 1.07x slower                                                 |
| django_template            | 34.1 ms                                                      | 36.9 ms: 1.08x slower                                                 |
| nbody                      | 85.1 ms                                                      | 92.8 ms: 1.09x slower                                                 |
| regex_compile              | 132 ms                                                       | 149 ms: 1.13x slower                                                  |
| python_startup             | 11.0 ms                                                      | 12.9 ms: 1.18x slower                                                 |
| gc_traversal               | 3.14 ms                                                      | 3.74 ms: 1.19x slower                                                 |
| create_gc_cycles           | 1.34 ms                                                      | 1.69 ms: 1.27x slower                                                 |
| bench_thread_pool          | 919 us                                                       | 1.34 ms: 1.46x slower                                                 |
| telco                      | 7.82 ms                                                      | 158 ms: 20.25x slower                                                 |
| bench_mp_pool              | 11.0 ms                                                      | 227 ms: 20.68x slower                                                 |
| Geometric mean             | (ref)                                                        | 1.01x slower                                                          |

Benchmark hidden because not significant (3): pycparser, coverage, logging_format
Ignored benchmarks (20) of results/bm-20240906-3.13.0rc2-ec61006/bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006.json: aiohttp, asyncio_tcp, asyncio_tcp_ssl, chameleon, dask, flaskblogging, genshi_text, genshi_xml, gunicorn, pickle, pickle_dict, pickle_list, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http, unpack_sequence, unpickle, unpickle_list
Ignored benchmarks (10) of results/bm-20260918-3.16.0a0-8542958/bm-20260918-vultr-x86_64-python-854295809ad5a42c9461-3.16.0a0-8542958.json: connected_components, k_core, many_optionals, shortest_path, sphinx, sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile, subparsers

- Geometric mean (including insignificant results): 1.028x faster

# HPT report

- Reliability score: 99.98% likely to be faster
- 90% likely to have a speedup of 1.02x
- 95% likely to have a speedup of 1.01x
- 99% likely to have a speedup of 1.01x

# Memory
- memory change: 1.14x