# Results vs. 3.12.6

- fork: Fidget-Spinner
- ref: tracing_jit
- machine: linux-x86_64
- commit hash: 01c2d73
- commit date: 2025-10-28
- overall geometric mean: 1.099x faster
- HPT reliability: 100.00%
- HPT 99th percentile: 1.04x faster
- Memory change: 1.19x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20251028-vultr-x86_64-Fidget%2dSpinner-tracing_jit-3.15.0a1+-01c2d73 |
|----------------|:------------------------------------------------------:|:-----------------------------------------------------------------------:|
| 2to3           | 264 ms                                                 | 253 ms: 1.04x faster                                                    |
| docutils       | 2.64 sec                                               | 2.74 sec: 1.04x slower                                                  |
| html5lib       | 63.6 ms                                                | 68.5 ms: 1.08x slower                                                   |
| Geometric mean | (ref)                                                  | 1.02x slower                                                            |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20251028-vultr-x86_64-Fidget%2dSpinner-tracing_jit-3.15.0a1+-01c2d73 |
|----------------------------|:------------------------------------------------------:|:-----------------------------------------------------------------------:|
| async_tree_io              | 1.08 sec                                               | 557 ms: 1.94x faster                                                    |
| async_tree_io_tg           | 1.11 sec                                               | 589 ms: 1.88x faster                                                    |
| async_tree_none            | 464 ms                                                 | 249 ms: 1.86x faster                                                    |
| async_tree_memoization_tg  | 560 ms                                                 | 307 ms: 1.82x faster                                                    |
| async_tree_none_tg         | 446 ms                                                 | 244 ms: 1.82x faster                                                    |
| async_tree_memoization     | 555 ms                                                 | 310 ms: 1.79x faster                                                    |
| async_tree_cpu_io_mixed_tg | 723 ms                                                 | 482 ms: 1.50x faster                                                    |
| async_tree_cpu_io_mixed    | 715 ms                                                 | 488 ms: 1.46x faster                                                    |
| async_generators           | 384 ms                                                 | 363 ms: 1.06x faster                                                    |
| coroutines                 | 23.9 ms                                                | 24.2 ms: 1.01x slower                                                   |
| asyncio_websockets         | 517 ms                                                 | 545 ms: 1.05x slower                                                    |
| Geometric mean             | (ref)                                                  | 1.50x faster                                                            |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20251028-vultr-x86_64-Fidget%2dSpinner-tracing_jit-3.15.0a1+-01c2d73 |
|----------------|:------------------------------------------------------:|:-----------------------------------------------------------------------:|
| float          | 80.8 ms                                                | 60.5 ms: 1.33x faster                                                   |
| nbody          | 89.3 ms                                                | 84.6 ms: 1.06x faster                                                   |
| pidigits       | 184 ms                                                 | 193 ms: 1.05x slower                                                    |
| Geometric mean | (ref)                                                  | 1.10x faster                                                            |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20251028-vultr-x86_64-Fidget%2dSpinner-tracing_jit-3.15.0a1+-01c2d73 |
|----------------|:------------------------------------------------------:|:-----------------------------------------------------------------------:|
| regex_effbot   | 3.17 ms                                                | 2.57 ms: 1.23x faster                                                   |
| regex_compile  | 142 ms                                                 | 126 ms: 1.13x faster                                                    |
| regex_dna      | 168 ms                                                 | 172 ms: 1.02x slower                                                    |
| regex_v8       | 20.6 ms                                                | 21.4 ms: 1.04x slower                                                   |
| Geometric mean | (ref)                                                  | 1.07x faster                                                            |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20251028-vultr-x86_64-Fidget%2dSpinner-tracing_jit-3.15.0a1+-01c2d73 |
|----------------------|:------------------------------------------------------:|:-----------------------------------------------------------------------:|
| unpickle_pure_python | 221 us                                                 | 184 us: 1.20x faster                                                    |
| xml_etree_iterparse  | 96.7 ms                                                | 82.8 ms: 1.17x faster                                                   |
| tomli_loads          | 2.11 sec                                               | 1.86 sec: 1.13x faster                                                  |
| json_dumps           | 10.4 ms                                                | 9.40 ms: 1.10x faster                                                   |
| xml_etree_process    | 59.0 ms                                                | 54.5 ms: 1.08x faster                                                   |
| xml_etree_parse      | 139 ms                                                 | 128 ms: 1.08x faster                                                    |
| xml_etree_generate   | 85.2 ms                                                | 79.6 ms: 1.07x faster                                                   |
| pickle_pure_python   | 308 us                                                 | 302 us: 1.02x faster                                                    |
| json_loads           | 26.5 us                                                | 28.6 us: 1.08x slower                                                   |
| Geometric mean       | (ref)                                                  | 1.08x faster                                                            |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20251028-vultr-x86_64-Fidget%2dSpinner-tracing_jit-3.15.0a1+-01c2d73 |
|------------------------|:------------------------------------------------------:|:-----------------------------------------------------------------------:|
| python_startup_no_site | 7.16 ms                                                | 7.25 ms: 1.01x slower                                                   |
| python_startup         | 9.93 ms                                                | 12.4 ms: 1.25x slower                                                   |
| Geometric mean         | (ref)                                                  | 1.13x slower                                                            |

Benchmarks with tag 'template':
===============================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20251028-vultr-x86_64-Fidget%2dSpinner-tracing_jit-3.15.0a1+-01c2d73 |
|----------------|:------------------------------------------------------:|:-----------------------------------------------------------------------:|
| genshi_text    | 22.8 ms                                                | 21.3 ms: 1.07x faster                                                   |
| mako           | 11.0 ms                                                | 10.6 ms: 1.04x faster                                                   |
| genshi_xml     | 50.2 ms                                                | 54.4 ms: 1.08x slower                                                   |
| Geometric mean | (ref)                                                  | 1.01x faster                                                            |

Benchmark hidden because not significant (1): django_template

All benchmarks:
===============

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20251028-vultr-x86_64-Fidget%2dSpinner-tracing_jit-3.15.0a1+-01c2d73 |
|----------------------------|:------------------------------------------------------:|:-----------------------------------------------------------------------:|
| richards                   | 45.9 ms                                                | 21.4 ms: 2.15x faster                                                   |
| richards_super             | 51.9 ms                                                | 25.4 ms: 2.04x faster                                                   |
| async_tree_io              | 1.08 sec                                               | 557 ms: 1.94x faster                                                    |
| async_tree_io_tg           | 1.11 sec                                               | 589 ms: 1.88x faster                                                    |
| async_tree_none            | 464 ms                                                 | 249 ms: 1.86x faster                                                    |
| async_tree_memoization_tg  | 560 ms                                                 | 307 ms: 1.82x faster                                                    |
| async_tree_none_tg         | 446 ms                                                 | 244 ms: 1.82x faster                                                    |
| mdp                        | 2.42 sec                                               | 1.34 sec: 1.81x faster                                                  |
| async_tree_memoization     | 555 ms                                                 | 310 ms: 1.79x faster                                                    |
| deepcopy_memo              | 40.3 us                                                | 23.1 us: 1.75x faster                                                   |
| async_tree_cpu_io_mixed_tg | 723 ms                                                 | 482 ms: 1.50x faster                                                    |
| async_tree_cpu_io_mixed    | 715 ms                                                 | 488 ms: 1.46x faster                                                    |
| deepcopy                   | 352 us                                                 | 251 us: 1.40x faster                                                    |
| float                      | 80.8 ms                                                | 60.5 ms: 1.33x faster                                                   |
| scimark_sor                | 130 ms                                                 | 98.5 ms: 1.32x faster                                                   |
| scimark_fft                | 342 ms                                                 | 261 ms: 1.31x faster                                                    |
| spectral_norm              | 110 ms                                                 | 84.5 ms: 1.30x faster                                                   |
| logging_silent             | 109 ns                                                 | 86.9 ns: 1.25x faster                                                   |
| comprehensions             | 19.8 us                                                | 16.0 us: 1.24x faster                                                   |
| regex_effbot               | 3.17 ms                                                | 2.57 ms: 1.23x faster                                                   |
| deltablue                  | 3.45 ms                                                | 2.82 ms: 1.22x faster                                                   |
| deepcopy_reduce            | 3.08 us                                                | 2.56 us: 1.20x faster                                                   |
| unpickle_pure_python       | 221 us                                                 | 184 us: 1.20x faster                                                    |
| scimark_monte_carlo        | 68.4 ms                                                | 57.5 ms: 1.19x faster                                                   |
| pathlib                    | 21.5 ms                                                | 18.2 ms: 1.19x faster                                                   |
| pyflate                    | 448 ms                                                 | 380 ms: 1.18x faster                                                    |
| bpe_tokeniser              | 4.74 sec                                               | 4.03 sec: 1.17x faster                                                  |
| go                         | 139 ms                                                 | 119 ms: 1.17x faster                                                    |
| xml_etree_iterparse        | 96.7 ms                                                | 82.8 ms: 1.17x faster                                                   |
| chaos                      | 62.8 ms                                                | 54.2 ms: 1.16x faster                                                   |
| generators                 | 32.2 ms                                                | 28.3 ms: 1.14x faster                                                   |
| crypto_pyaes               | 76.6 ms                                                | 67.2 ms: 1.14x faster                                                   |
| logging_simple             | 6.63 us                                                | 5.82 us: 1.14x faster                                                   |
| regex_compile              | 142 ms                                                 | 126 ms: 1.13x faster                                                    |
| tomli_loads                | 2.11 sec                                               | 1.86 sec: 1.13x faster                                                  |
| scimark_lu                 | 114 ms                                                 | 101 ms: 1.13x faster                                                    |
| logging_format             | 7.35 us                                                | 6.60 us: 1.11x faster                                                   |
| raytrace                   | 299 ms                                                 | 271 ms: 1.10x faster                                                    |
| json_dumps                 | 10.4 ms                                                | 9.40 ms: 1.10x faster                                                   |
| dulwich_log                | 78.9 ms                                                | 72.4 ms: 1.09x faster                                                   |
| xml_etree_process          | 59.0 ms                                                | 54.5 ms: 1.08x faster                                                   |
| xml_etree_parse            | 139 ms                                                 | 128 ms: 1.08x faster                                                    |
| pprint_pformat             | 1.52 sec                                               | 1.41 sec: 1.08x faster                                                  |
| xml_etree_generate         | 85.2 ms                                                | 79.6 ms: 1.07x faster                                                   |
| genshi_text                | 22.8 ms                                                | 21.3 ms: 1.07x faster                                                   |
| typing_runtime_protocols   | 163 us                                                 | 153 us: 1.07x faster                                                    |
| async_generators           | 384 ms                                                 | 363 ms: 1.06x faster                                                    |
| nbody                      | 89.3 ms                                                | 84.6 ms: 1.06x faster                                                   |
| pprint_safe_repr           | 743 ms                                                 | 710 ms: 1.05x faster                                                    |
| scimark_sparse_mat_mult    | 4.39 ms                                                | 4.20 ms: 1.05x faster                                                   |
| mako                       | 11.0 ms                                                | 10.6 ms: 1.04x faster                                                   |
| 2to3                       | 264 ms                                                 | 253 ms: 1.04x faster                                                    |
| meteor_contest             | 104 ms                                                 | 99.5 ms: 1.04x faster                                                   |
| pickle_pure_python         | 308 us                                                 | 302 us: 1.02x faster                                                    |
| sympy_integrate            | 20.5 ms                                                | 20.2 ms: 1.02x faster                                                   |
| pycparser                  | 1.17 sec                                               | 1.15 sec: 1.01x faster                                                  |
| sympy_sum                  | 166 ms                                                 | 167 ms: 1.01x slower                                                    |
| coroutines                 | 23.9 ms                                                | 24.2 ms: 1.01x slower                                                   |
| python_startup_no_site     | 7.16 ms                                                | 7.25 ms: 1.01x slower                                                   |
| hexiom                     | 6.17 ms                                                | 6.25 ms: 1.01x slower                                                   |
| regex_dna                  | 168 ms                                                 | 172 ms: 1.02x slower                                                    |
| docutils                   | 2.64 sec                                               | 2.74 sec: 1.04x slower                                                  |
| regex_v8                   | 20.6 ms                                                | 21.4 ms: 1.04x slower                                                   |
| sympy_str                  | 292 ms                                                 | 304 ms: 1.04x slower                                                    |
| pidigits                   | 184 ms                                                 | 193 ms: 1.05x slower                                                    |
| asyncio_websockets         | 517 ms                                                 | 545 ms: 1.05x slower                                                    |
| nqueens                    | 80.1 ms                                                | 85.5 ms: 1.07x slower                                                   |
| sympy_expand               | 468 ms                                                 | 501 ms: 1.07x slower                                                    |
| json_loads                 | 26.5 us                                                | 28.6 us: 1.08x slower                                                   |
| bench_thread_pool          | 941 us                                                 | 1.01 ms: 1.08x slower                                                   |
| html5lib                   | 63.6 ms                                                | 68.5 ms: 1.08x slower                                                   |
| genshi_xml                 | 50.2 ms                                                | 54.4 ms: 1.08x slower                                                   |
| coverage                   | 71.4 ms                                                | 81.8 ms: 1.15x slower                                                   |
| bench_mp_pool              | 10.8 ms                                                | 12.5 ms: 1.15x slower                                                   |
| python_startup             | 9.93 ms                                                | 12.4 ms: 1.25x slower                                                   |
| gc_traversal               | 3.46 ms                                                | 4.55 ms: 1.32x slower                                                   |
| create_gc_cycles           | 1.09 ms                                                | 1.93 ms: 1.77x slower                                                   |
| telco                      | 6.53 ms                                                | 164 ms: 25.09x slower                                                   |
| Geometric mean             | (ref)                                                  | 1.09x faster                                                            |

Benchmark hidden because not significant (6): pylint, fannkuch, django_template, thrift, json, sqlite_synth
Ignored benchmarks (21) of results/bm-20240906-3.12.6-a4a2d2b/bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b.json: aiohttp, asyncio_tcp, asyncio_tcp_ssl, chameleon, dask, flaskblogging, gunicorn, mypy2, pickle, pickle_dict, pickle_list, sqlalchemy_declarative, sqlalchemy_imperative, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http, unpack_sequence, unpickle, unpickle_list
Ignored benchmarks (10) of results/bm-20251028-3.15.0a1+-01c2d73-JIT/bm-20251028-vultr-x86_64-Fidget%2dSpinner-tracing_jit-3.15.0a1+-01c2d73.json: connected_components, k_core, many_optionals, shortest_path, sphinx, sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile, subparsers

- Geometric mean (including insignificant results): 1.099x faster

# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.06x
- 95% likely to have a speedup of 1.06x
- 99% likely to have a speedup of 1.04x

# Memory
- memory change: 1.19x