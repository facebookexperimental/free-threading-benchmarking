# Results vs. 3.12.6

- fork: python
- ref: 7b4165b3b07638d8aeab
- machine: linux-x86_64
- commit hash: 7b4165b
- commit date: 2026-08-01
- overall geometric mean: 1.097x faster
- HPT reliability: 100.00%
- HPT 99th percentile: 1.05x faster
- Memory change: 1.18x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260801-vultr-x86_64-python-7b4165b3b07638d8aeab-3.16.0a0-7b4165b |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| 2to3           | 264 ms                                                 | 256 ms: 1.03x faster                                                  |
| docutils       | 2.64 sec                                               | 2.29 sec: 1.15x faster                                                |
| html5lib       | 63.6 ms                                                | 56.6 ms: 1.12x faster                                                 |
| Geometric mean | (ref)                                                  | 1.10x faster                                                          |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260801-vultr-x86_64-python-7b4165b3b07638d8aeab-3.16.0a0-7b4165b |
|----------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| async_tree_none            | 464 ms                                                 | 281 ms: 1.65x faster                                                  |
| async_tree_memoization_tg  | 560 ms                                                 | 359 ms: 1.56x faster                                                  |
| async_tree_none_tg         | 446 ms                                                 | 291 ms: 1.53x faster                                                  |
| async_tree_io              | 1.08 sec                                               | 711 ms: 1.52x faster                                                  |
| async_tree_memoization     | 555 ms                                                 | 367 ms: 1.51x faster                                                  |
| async_tree_io_tg           | 1.11 sec                                               | 765 ms: 1.45x faster                                                  |
| async_tree_cpu_io_mixed    | 715 ms                                                 | 529 ms: 1.35x faster                                                  |
| async_tree_cpu_io_mixed_tg | 723 ms                                                 | 550 ms: 1.31x faster                                                  |
| asyncio_tcp                | 519 ms                                                 | 438 ms: 1.18x faster                                                  |
| async_generators           | 384 ms                                                 | 330 ms: 1.17x faster                                                  |
| coroutines                 | 23.9 ms                                                | 23.1 ms: 1.03x faster                                                 |
| asyncio_tcp_ssl            | 1.51 sec                                               | 1.48 sec: 1.02x faster                                                |
| asyncio_websockets         | 517 ms                                                 | 544 ms: 1.05x slower                                                  |
| Geometric mean             | (ref)                                                  | 1.31x faster                                                          |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260801-vultr-x86_64-python-7b4165b3b07638d8aeab-3.16.0a0-7b4165b |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| float          | 80.8 ms                                                | 72.0 ms: 1.12x faster                                                 |
| nbody          | 89.3 ms                                                | 84.8 ms: 1.05x faster                                                 |
| pidigits       | 184 ms                                                 | 191 ms: 1.03x slower                                                  |
| Geometric mean | (ref)                                                  | 1.05x faster                                                          |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260801-vultr-x86_64-python-7b4165b3b07638d8aeab-3.16.0a0-7b4165b |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| regex_compile  | 142 ms                                                 | 144 ms: 1.01x slower                                                  |
| regex_v8       | 20.6 ms                                                | 22.2 ms: 1.08x slower                                                 |
| regex_dna      | 168 ms                                                 | 191 ms: 1.14x slower                                                  |
| Geometric mean | (ref)                                                  | 1.05x slower                                                          |

Benchmark hidden because not significant (1): regex_effbot

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260801-vultr-x86_64-python-7b4165b3b07638d8aeab-3.16.0a0-7b4165b |
|----------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| tomli_loads          | 2.11 sec                                               | 1.83 sec: 1.15x faster                                                |
| json_dumps           | 10.4 ms                                                | 9.28 ms: 1.12x faster                                                 |
| unpickle_list        | 4.67 us                                                | 4.23 us: 1.10x faster                                                 |
| pickle_dict          | 31.8 us                                                | 29.3 us: 1.09x faster                                                 |
| unpickle_pure_python | 221 us                                                 | 206 us: 1.07x faster                                                  |
| xml_etree_iterparse  | 96.7 ms                                                | 92.6 ms: 1.04x faster                                                 |
| pickle_pure_python   | 308 us                                                 | 298 us: 1.03x faster                                                  |
| xml_etree_process    | 59.0 ms                                                | 57.9 ms: 1.02x faster                                                 |
| xml_etree_generate   | 85.2 ms                                                | 83.8 ms: 1.02x faster                                                 |
| json_loads           | 26.5 us                                                | 26.7 us: 1.01x slower                                                 |
| unpickle             | 14.1 us                                                | 14.4 us: 1.02x slower                                                 |
| xml_etree_parse      | 139 ms                                                 | 159 ms: 1.15x slower                                                  |
| pickle_list          | 4.77 us                                                | 5.49 us: 1.15x slower                                                 |
| pickle               | 10.9 us                                                | 12.9 us: 1.18x slower                                                 |
| Geometric mean       | (ref)                                                  | 1.01x faster                                                          |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260801-vultr-x86_64-python-7b4165b3b07638d8aeab-3.16.0a0-7b4165b |
|------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| python_startup_no_site | 7.16 ms                                                | 7.21 ms: 1.01x slower                                                 |
| python_startup         | 9.93 ms                                                | 12.6 ms: 1.27x slower                                                 |
| Geometric mean         | (ref)                                                  | 1.13x slower                                                          |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260801-vultr-x86_64-python-7b4165b3b07638d8aeab-3.16.0a0-7b4165b |
|-----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| django_template | 34.7 ms                                                | 33.8 ms: 1.03x faster                                                 |
| mako            | 11.0 ms                                                | 12.4 ms: 1.13x slower                                                 |
| Geometric mean  | (ref)                                                  | 1.05x slower                                                          |

All benchmarks:
===============

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260801-vultr-x86_64-python-7b4165b3b07638d8aeab-3.16.0a0-7b4165b |
|----------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| pylint                     | 319 ms                                                 | 111 ms: 2.87x faster                                                  |
| mdp                        | 2.42 sec                                               | 1.07 sec: 2.25x faster                                                |
| async_tree_none            | 464 ms                                                 | 281 ms: 1.65x faster                                                  |
| deepcopy                   | 352 us                                                 | 215 us: 1.64x faster                                                  |
| deepcopy_memo              | 40.3 us                                                | 24.7 us: 1.63x faster                                                 |
| async_tree_memoization_tg  | 560 ms                                                 | 359 ms: 1.56x faster                                                  |
| async_tree_none_tg         | 446 ms                                                 | 291 ms: 1.53x faster                                                  |
| async_tree_io              | 1.08 sec                                               | 711 ms: 1.52x faster                                                  |
| async_tree_memoization     | 555 ms                                                 | 367 ms: 1.51x faster                                                  |
| typing_runtime_protocols   | 163 us                                                 | 111 us: 1.47x faster                                                  |
| async_tree_io_tg           | 1.11 sec                                               | 765 ms: 1.45x faster                                                  |
| go                         | 139 ms                                                 | 99.9 ms: 1.39x faster                                                 |
| comprehensions             | 19.8 us                                                | 14.3 us: 1.38x faster                                                 |
| async_tree_cpu_io_mixed    | 715 ms                                                 | 529 ms: 1.35x faster                                                  |
| async_tree_cpu_io_mixed_tg | 723 ms                                                 | 550 ms: 1.31x faster                                                  |
| deepcopy_reduce            | 3.08 us                                                | 2.38 us: 1.29x faster                                                 |
| pathlib                    | 21.5 ms                                                | 16.9 ms: 1.28x faster                                                 |
| spectral_norm              | 110 ms                                                 | 86.9 ms: 1.27x faster                                                 |
| scimark_sor                | 130 ms                                                 | 104 ms: 1.25x faster                                                  |
| logging_silent             | 109 ns                                                 | 87.4 ns: 1.25x faster                                                 |
| scimark_fft                | 342 ms                                                 | 275 ms: 1.24x faster                                                  |
| raytrace                   | 299 ms                                                 | 241 ms: 1.24x faster                                                  |
| chaos                      | 62.8 ms                                                | 51.0 ms: 1.23x faster                                                 |
| unpack_sequence            | 52.1 ns                                                | 43.3 ns: 1.20x faster                                                 |
| dulwich_log                | 78.9 ms                                                | 66.1 ms: 1.19x faster                                                 |
| scimark_monte_carlo        | 68.4 ms                                                | 57.5 ms: 1.19x faster                                                 |
| asyncio_tcp                | 519 ms                                                 | 438 ms: 1.18x faster                                                  |
| deltablue                  | 3.45 ms                                                | 2.91 ms: 1.18x faster                                                 |
| richards                   | 45.9 ms                                                | 39.1 ms: 1.17x faster                                                 |
| async_generators           | 384 ms                                                 | 330 ms: 1.17x faster                                                  |
| bpe_tokeniser              | 4.74 sec                                               | 4.08 sec: 1.16x faster                                                |
| logging_simple             | 6.63 us                                                | 5.72 us: 1.16x faster                                                 |
| richards_super             | 51.9 ms                                                | 44.8 ms: 1.16x faster                                                 |
| docutils                   | 2.64 sec                                               | 2.29 sec: 1.15x faster                                                |
| tomli_loads                | 2.11 sec                                               | 1.83 sec: 1.15x faster                                                |
| nqueens                    | 80.1 ms                                                | 69.8 ms: 1.15x faster                                                 |
| logging_format             | 7.35 us                                                | 6.43 us: 1.14x faster                                                 |
| generators                 | 32.2 ms                                                | 28.4 ms: 1.14x faster                                                 |
| sympy_integrate            | 20.5 ms                                                | 18.2 ms: 1.13x faster                                                 |
| html5lib                   | 63.6 ms                                                | 56.6 ms: 1.12x faster                                                 |
| float                      | 80.8 ms                                                | 72.0 ms: 1.12x faster                                                 |
| pyflate                    | 448 ms                                                 | 400 ms: 1.12x faster                                                  |
| hexiom                     | 6.17 ms                                                | 5.51 ms: 1.12x faster                                                 |
| json_dumps                 | 10.4 ms                                                | 9.28 ms: 1.12x faster                                                 |
| sympy_sum                  | 166 ms                                                 | 149 ms: 1.11x faster                                                  |
| unpickle_list              | 4.67 us                                                | 4.23 us: 1.10x faster                                                 |
| crypto_pyaes               | 76.6 ms                                                | 69.6 ms: 1.10x faster                                                 |
| sympy_str                  | 292 ms                                                 | 266 ms: 1.10x faster                                                  |
| scimark_sparse_mat_mult    | 4.39 ms                                                | 4.02 ms: 1.09x faster                                                 |
| pickle_dict                | 31.8 us                                                | 29.3 us: 1.09x faster                                                 |
| scimark_lu                 | 114 ms                                                 | 106 ms: 1.08x faster                                                  |
| unpickle_pure_python       | 221 us                                                 | 206 us: 1.07x faster                                                  |
| thrift                     | 791 us                                                 | 749 us: 1.06x faster                                                  |
| nbody                      | 89.3 ms                                                | 84.8 ms: 1.05x faster                                                 |
| xml_etree_iterparse        | 96.7 ms                                                | 92.6 ms: 1.04x faster                                                 |
| pycparser                  | 1.17 sec                                               | 1.13 sec: 1.04x faster                                                |
| sympy_expand               | 468 ms                                                 | 452 ms: 1.04x faster                                                  |
| coroutines                 | 23.9 ms                                                | 23.1 ms: 1.03x faster                                                 |
| pickle_pure_python         | 308 us                                                 | 298 us: 1.03x faster                                                  |
| fannkuch                   | 372 ms                                                 | 361 ms: 1.03x faster                                                  |
| 2to3                       | 264 ms                                                 | 256 ms: 1.03x faster                                                  |
| django_template            | 34.7 ms                                                | 33.8 ms: 1.03x faster                                                 |
| pprint_safe_repr           | 743 ms                                                 | 726 ms: 1.02x faster                                                  |
| asyncio_tcp_ssl            | 1.51 sec                                               | 1.48 sec: 1.02x faster                                                |
| pprint_pformat             | 1.52 sec                                               | 1.49 sec: 1.02x faster                                                |
| xml_etree_process          | 59.0 ms                                                | 57.9 ms: 1.02x faster                                                 |
| xml_etree_generate         | 85.2 ms                                                | 83.8 ms: 1.02x faster                                                 |
| json                       | 5.02 ms                                                | 4.95 ms: 1.02x faster                                                 |
| python_startup_no_site     | 7.16 ms                                                | 7.21 ms: 1.01x slower                                                 |
| json_loads                 | 26.5 us                                                | 26.7 us: 1.01x slower                                                 |
| regex_compile              | 142 ms                                                 | 144 ms: 1.01x slower                                                  |
| unpickle                   | 14.1 us                                                | 14.4 us: 1.02x slower                                                 |
| meteor_contest             | 104 ms                                                 | 107 ms: 1.03x slower                                                  |
| pidigits                   | 184 ms                                                 | 191 ms: 1.03x slower                                                  |
| asyncio_websockets         | 517 ms                                                 | 544 ms: 1.05x slower                                                  |
| regex_v8                   | 20.6 ms                                                | 22.2 ms: 1.08x slower                                                 |
| coverage                   | 71.4 ms                                                | 80.0 ms: 1.12x slower                                                 |
| mako                       | 11.0 ms                                                | 12.4 ms: 1.13x slower                                                 |
| regex_dna                  | 168 ms                                                 | 191 ms: 1.14x slower                                                  |
| xml_etree_parse            | 139 ms                                                 | 159 ms: 1.15x slower                                                  |
| pickle_list                | 4.77 us                                                | 5.49 us: 1.15x slower                                                 |
| pickle                     | 10.9 us                                                | 12.9 us: 1.18x slower                                                 |
| gc_traversal               | 3.46 ms                                                | 4.20 ms: 1.21x slower                                                 |
| python_startup             | 9.93 ms                                                | 12.6 ms: 1.27x slower                                                 |
| bench_thread_pool          | 941 us                                                 | 1.29 ms: 1.38x slower                                                 |
| create_gc_cycles           | 1.09 ms                                                | 1.83 ms: 1.67x slower                                                 |
| telco                      | 6.53 ms                                                | 155 ms: 23.79x slower                                                 |
| bench_mp_pool              | 10.8 ms                                                | 266 ms: 24.60x slower                                                 |
| Geometric mean             | (ref)                                                  | 1.05x faster                                                          |

Benchmark hidden because not significant (2): regex_effbot, sqlite_synth
Ignored benchmarks (15) of results/bm-20240906-3.12.6-a4a2d2b/bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b.json: aiohttp, chameleon, dask, flaskblogging, genshi_text, genshi_xml, gunicorn, mypy2, sqlalchemy_declarative, sqlalchemy_imperative, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http
Ignored benchmarks (10) of results/bm-20260801-3.16.0a0-7b4165b-CLANG/bm-20260801-vultr-x86_64-python-7b4165b3b07638d8aeab-3.16.0a0-7b4165b.json: connected_components, k_core, many_optionals, shortest_path, sphinx, sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile, subparsers

- Geometric mean (including insignificant results): 1.097x faster

# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.09x
- 95% likely to have a speedup of 1.08x
- 99% likely to have a speedup of 1.05x

# Memory
- memory change: 1.18x