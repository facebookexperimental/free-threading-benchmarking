# Results vs. 3.12.6

- fork: Fidget-Spinner
- ref: resume_tracing
- machine: linux-x86_64
- commit hash: 2e9b980
- commit date: 2026-03-15
- overall geometric mean: 1.143x faster
- HPT reliability: 100.00%
- HPT 99th percentile: 1.07x faster
- Memory change: 1.20x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260315-vultr-x86_64-Fidget%2dSpinner-resume_tracing-3.15.0a7+-2e9b980 |
|----------------|:------------------------------------------------------:|:--------------------------------------------------------------------------:|
| 2to3           | 264 ms                                                 | 250 ms: 1.06x faster                                                       |
| docutils       | 2.64 sec                                               | 2.61 sec: 1.01x faster                                                     |
| html5lib       | 63.6 ms                                                | 60.9 ms: 1.04x faster                                                      |
| Geometric mean | (ref)                                                  | 1.04x faster                                                               |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260315-vultr-x86_64-Fidget%2dSpinner-resume_tracing-3.15.0a7+-2e9b980 |
|----------------------------|:------------------------------------------------------:|:--------------------------------------------------------------------------:|
| async_tree_io_tg           | 1.11 sec                                               | 551 ms: 2.01x faster                                                       |
| async_tree_none            | 464 ms                                                 | 232 ms: 2.00x faster                                                       |
| async_tree_memoization     | 555 ms                                                 | 284 ms: 1.96x faster                                                       |
| async_tree_io              | 1.08 sec                                               | 567 ms: 1.91x faster                                                       |
| async_tree_memoization_tg  | 560 ms                                                 | 294 ms: 1.90x faster                                                       |
| async_tree_none_tg         | 446 ms                                                 | 236 ms: 1.89x faster                                                       |
| async_tree_cpu_io_mixed    | 715 ms                                                 | 492 ms: 1.45x faster                                                       |
| async_tree_cpu_io_mixed_tg | 723 ms                                                 | 502 ms: 1.44x faster                                                       |
| async_generators           | 384 ms                                                 | 354 ms: 1.08x faster                                                       |
| coroutines                 | 23.9 ms                                                | 22.1 ms: 1.08x faster                                                      |
| asyncio_websockets         | 517 ms                                                 | 545 ms: 1.05x slower                                                       |
| Geometric mean             | (ref)                                                  | 1.55x faster                                                               |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260315-vultr-x86_64-Fidget%2dSpinner-resume_tracing-3.15.0a7+-2e9b980 |
|----------------|:------------------------------------------------------:|:--------------------------------------------------------------------------:|
| float          | 80.8 ms                                                | 60.0 ms: 1.35x faster                                                      |
| nbody          | 89.3 ms                                                | 76.8 ms: 1.16x faster                                                      |
| pidigits       | 184 ms                                                 | 208 ms: 1.13x slower                                                       |
| Geometric mean | (ref)                                                  | 1.11x faster                                                               |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260315-vultr-x86_64-Fidget%2dSpinner-resume_tracing-3.15.0a7+-2e9b980 |
|----------------|:------------------------------------------------------:|:--------------------------------------------------------------------------:|
| regex_effbot   | 3.17 ms                                                | 2.71 ms: 1.17x faster                                                      |
| regex_compile  | 142 ms                                                 | 124 ms: 1.15x faster                                                       |
| regex_dna      | 168 ms                                                 | 168 ms: 1.00x slower                                                       |
| regex_v8       | 20.6 ms                                                | 20.9 ms: 1.01x slower                                                      |
| Geometric mean | (ref)                                                  | 1.07x faster                                                               |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260315-vultr-x86_64-Fidget%2dSpinner-resume_tracing-3.15.0a7+-2e9b980 |
|----------------------|:------------------------------------------------------:|:--------------------------------------------------------------------------:|
| tomli_loads          | 2.11 sec                                               | 1.51 sec: 1.40x faster                                                     |
| xml_etree_iterparse  | 96.7 ms                                                | 83.3 ms: 1.16x faster                                                      |
| unpickle_pure_python | 221 us                                                 | 197 us: 1.12x faster                                                       |
| json_dumps           | 10.4 ms                                                | 9.32 ms: 1.11x faster                                                      |
| xml_etree_process    | 59.0 ms                                                | 53.1 ms: 1.11x faster                                                      |
| xml_etree_generate   | 85.2 ms                                                | 79.8 ms: 1.07x faster                                                      |
| xml_etree_parse      | 139 ms                                                 | 131 ms: 1.06x faster                                                       |
| pickle_pure_python   | 308 us                                                 | 311 us: 1.01x slower                                                       |
| json_loads           | 26.5 us                                                | 26.9 us: 1.01x slower                                                      |
| Geometric mean       | (ref)                                                  | 1.11x faster                                                               |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260315-vultr-x86_64-Fidget%2dSpinner-resume_tracing-3.15.0a7+-2e9b980 |
|------------------------|:------------------------------------------------------:|:--------------------------------------------------------------------------:|
| python_startup_no_site | 7.16 ms                                                | 7.52 ms: 1.05x slower                                                      |
| python_startup         | 9.93 ms                                                | 12.7 ms: 1.28x slower                                                      |
| Geometric mean         | (ref)                                                  | 1.16x slower                                                               |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260315-vultr-x86_64-Fidget%2dSpinner-resume_tracing-3.15.0a7+-2e9b980 |
|-----------------|:------------------------------------------------------:|:--------------------------------------------------------------------------:|
| genshi_text     | 22.8 ms                                                | 20.7 ms: 1.10x faster                                                      |
| django_template | 34.7 ms                                                | 31.9 ms: 1.09x faster                                                      |
| mako            | 11.0 ms                                                | 10.5 ms: 1.04x faster                                                      |
| genshi_xml      | 50.2 ms                                                | 53.3 ms: 1.06x slower                                                      |
| Geometric mean  | (ref)                                                  | 1.04x faster                                                               |

All benchmarks:
===============

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260315-vultr-x86_64-Fidget%2dSpinner-resume_tracing-3.15.0a7+-2e9b980 |
|----------------------------|:------------------------------------------------------:|:--------------------------------------------------------------------------:|
| richards                   | 45.9 ms                                                | 18.3 ms: 2.50x faster                                                      |
| richards_super             | 51.9 ms                                                | 22.1 ms: 2.35x faster                                                      |
| async_tree_io_tg           | 1.11 sec                                               | 551 ms: 2.01x faster                                                       |
| async_tree_none            | 464 ms                                                 | 232 ms: 2.00x faster                                                       |
| mdp                        | 2.42 sec                                               | 1.21 sec: 2.00x faster                                                     |
| async_tree_memoization     | 555 ms                                                 | 284 ms: 1.96x faster                                                       |
| async_tree_io              | 1.08 sec                                               | 567 ms: 1.91x faster                                                       |
| async_tree_memoization_tg  | 560 ms                                                 | 294 ms: 1.90x faster                                                       |
| async_tree_none_tg         | 446 ms                                                 | 236 ms: 1.89x faster                                                       |
| deepcopy_memo              | 40.3 us                                                | 22.4 us: 1.80x faster                                                      |
| deepcopy                   | 352 us                                                 | 225 us: 1.57x faster                                                       |
| scimark_sor                | 130 ms                                                 | 83.6 ms: 1.55x faster                                                      |
| async_tree_cpu_io_mixed    | 715 ms                                                 | 492 ms: 1.45x faster                                                       |
| async_tree_cpu_io_mixed_tg | 723 ms                                                 | 502 ms: 1.44x faster                                                       |
| scimark_lu                 | 114 ms                                                 | 79.8 ms: 1.43x faster                                                      |
| spectral_norm              | 110 ms                                                 | 78.7 ms: 1.40x faster                                                      |
| tomli_loads                | 2.11 sec                                               | 1.51 sec: 1.40x faster                                                     |
| go                         | 139 ms                                                 | 103 ms: 1.36x faster                                                       |
| logging_silent             | 109 ns                                                 | 80.6 ns: 1.35x faster                                                      |
| float                      | 80.8 ms                                                | 60.0 ms: 1.35x faster                                                      |
| comprehensions             | 19.8 us                                                | 14.9 us: 1.33x faster                                                      |
| scimark_monte_carlo        | 68.4 ms                                                | 51.7 ms: 1.32x faster                                                      |
| scimark_fft                | 342 ms                                                 | 261 ms: 1.31x faster                                                       |
| deepcopy_reduce            | 3.08 us                                                | 2.35 us: 1.31x faster                                                      |
| deltablue                  | 3.45 ms                                                | 2.66 ms: 1.30x faster                                                      |
| logging_simple             | 6.63 us                                                | 5.25 us: 1.26x faster                                                      |
| pathlib                    | 21.5 ms                                                | 17.3 ms: 1.24x faster                                                      |
| pyflate                    | 448 ms                                                 | 361 ms: 1.24x faster                                                       |
| logging_format             | 7.35 us                                                | 6.06 us: 1.21x faster                                                      |
| chaos                      | 62.8 ms                                                | 52.2 ms: 1.20x faster                                                      |
| bpe_tokeniser              | 4.74 sec                                               | 3.94 sec: 1.20x faster                                                     |
| pprint_safe_repr           | 743 ms                                                 | 628 ms: 1.18x faster                                                       |
| pprint_pformat             | 1.52 sec                                               | 1.29 sec: 1.18x faster                                                     |
| regex_effbot               | 3.17 ms                                                | 2.71 ms: 1.17x faster                                                      |
| nbody                      | 89.3 ms                                                | 76.8 ms: 1.16x faster                                                      |
| crypto_pyaes               | 76.6 ms                                                | 66.0 ms: 1.16x faster                                                      |
| xml_etree_iterparse        | 96.7 ms                                                | 83.3 ms: 1.16x faster                                                      |
| regex_compile              | 142 ms                                                 | 124 ms: 1.15x faster                                                       |
| dulwich_log                | 78.9 ms                                                | 69.6 ms: 1.13x faster                                                      |
| unpickle_pure_python       | 221 us                                                 | 197 us: 1.12x faster                                                       |
| json_dumps                 | 10.4 ms                                                | 9.32 ms: 1.11x faster                                                      |
| xml_etree_process          | 59.0 ms                                                | 53.1 ms: 1.11x faster                                                      |
| nqueens                    | 80.1 ms                                                | 72.7 ms: 1.10x faster                                                      |
| genshi_text                | 22.8 ms                                                | 20.7 ms: 1.10x faster                                                      |
| django_template            | 34.7 ms                                                | 31.9 ms: 1.09x faster                                                      |
| async_generators           | 384 ms                                                 | 354 ms: 1.08x faster                                                       |
| typing_runtime_protocols   | 163 us                                                 | 151 us: 1.08x faster                                                       |
| coroutines                 | 23.9 ms                                                | 22.1 ms: 1.08x faster                                                      |
| fannkuch                   | 372 ms                                                 | 347 ms: 1.07x faster                                                       |
| xml_etree_generate         | 85.2 ms                                                | 79.8 ms: 1.07x faster                                                      |
| meteor_contest             | 104 ms                                                 | 97.4 ms: 1.06x faster                                                      |
| hexiom                     | 6.17 ms                                                | 5.81 ms: 1.06x faster                                                      |
| xml_etree_parse            | 139 ms                                                 | 131 ms: 1.06x faster                                                       |
| 2to3                       | 264 ms                                                 | 250 ms: 1.06x faster                                                       |
| pylint                     | 319 ms                                                 | 302 ms: 1.05x faster                                                       |
| mako                       | 11.0 ms                                                | 10.5 ms: 1.04x faster                                                      |
| html5lib                   | 63.6 ms                                                | 60.9 ms: 1.04x faster                                                      |
| json                       | 5.02 ms                                                | 4.84 ms: 1.04x faster                                                      |
| sympy_integrate            | 20.5 ms                                                | 20.1 ms: 1.02x faster                                                      |
| sqlite_synth               | 2.20 us                                                | 2.16 us: 1.02x faster                                                      |
| thrift                     | 791 us                                                 | 779 us: 1.02x faster                                                       |
| docutils                   | 2.64 sec                                               | 2.61 sec: 1.01x faster                                                     |
| raytrace                   | 299 ms                                                 | 297 ms: 1.01x faster                                                       |
| regex_dna                  | 168 ms                                                 | 168 ms: 1.00x slower                                                       |
| pickle_pure_python         | 308 us                                                 | 311 us: 1.01x slower                                                       |
| json_loads                 | 26.5 us                                                | 26.9 us: 1.01x slower                                                      |
| regex_v8                   | 20.6 ms                                                | 20.9 ms: 1.01x slower                                                      |
| sympy_sum                  | 166 ms                                                 | 168 ms: 1.01x slower                                                       |
| sympy_expand               | 468 ms                                                 | 482 ms: 1.03x slower                                                       |
| python_startup_no_site     | 7.16 ms                                                | 7.52 ms: 1.05x slower                                                      |
| asyncio_websockets         | 517 ms                                                 | 545 ms: 1.05x slower                                                       |
| genshi_xml                 | 50.2 ms                                                | 53.3 ms: 1.06x slower                                                      |
| generators                 | 32.2 ms                                                | 35.3 ms: 1.09x slower                                                      |
| pidigits                   | 184 ms                                                 | 208 ms: 1.13x slower                                                       |
| coverage                   | 71.4 ms                                                | 82.6 ms: 1.16x slower                                                      |
| gc_traversal               | 3.46 ms                                                | 4.27 ms: 1.23x slower                                                      |
| python_startup             | 9.93 ms                                                | 12.7 ms: 1.28x slower                                                      |
| bench_thread_pool          | 941 us                                                 | 1.32 ms: 1.41x slower                                                      |
| create_gc_cycles           | 1.09 ms                                                | 1.83 ms: 1.68x slower                                                      |
| bench_mp_pool              | 10.8 ms                                                | 169 ms: 15.63x slower                                                      |
| telco                      | 6.53 ms                                                | 155 ms: 23.80x slower                                                      |
| Geometric mean             | (ref)                                                  | 1.10x faster                                                               |

Benchmark hidden because not significant (3): scimark_sparse_mat_mult, pycparser, sympy_str
Ignored benchmarks (21) of results/bm-20240906-3.12.6-a4a2d2b/bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b.json: aiohttp, asyncio_tcp, asyncio_tcp_ssl, chameleon, dask, flaskblogging, gunicorn, mypy2, pickle, pickle_dict, pickle_list, sqlalchemy_declarative, sqlalchemy_imperative, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http, unpack_sequence, unpickle, unpickle_list
Ignored benchmarks (10) of results/bm-20260315-3.15.0a7+-2e9b980-JIT/bm-20260315-vultr-x86_64-Fidget%2dSpinner-resume_tracing-3.15.0a7+-2e9b980.json: connected_components, k_core, many_optionals, shortest_path, sphinx, sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile, subparsers

- Geometric mean (including insignificant results): 1.143x faster

# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.09x
- 95% likely to have a speedup of 1.09x
- 99% likely to have a speedup of 1.07x

# Memory
- memory change: 1.20x