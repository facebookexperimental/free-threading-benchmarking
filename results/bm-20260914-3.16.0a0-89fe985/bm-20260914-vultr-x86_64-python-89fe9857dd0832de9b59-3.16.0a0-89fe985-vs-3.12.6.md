# Results vs. 3.12.6

- fork: python
- ref: 89fe9857dd0832de9b59
- machine: linux-x86_64
- commit hash: 89fe985
- commit date: 2026-09-14
- overall geometric mean: 1.059x faster
- HPT reliability: 100.00%
- HPT 99th percentile: 1.03x faster
- Memory change: 1.15x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260914-vultr-x86_64-python-89fe9857dd0832de9b59-3.16.0a0-89fe985 |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| 2to3           | 264 ms                                                 | 259 ms: 1.02x faster                                                  |
| docutils       | 2.64 sec                                               | 2.38 sec: 1.11x faster                                                |
| html5lib       | 63.6 ms                                                | 60.1 ms: 1.06x faster                                                 |
| Geometric mean | (ref)                                                  | 1.06x faster                                                          |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260914-vultr-x86_64-python-89fe9857dd0832de9b59-3.16.0a0-89fe985 |
|----------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| async_tree_memoization_tg  | 560 ms                                                 | 370 ms: 1.51x faster                                                  |
| async_tree_none_tg         | 446 ms                                                 | 304 ms: 1.47x faster                                                  |
| async_tree_io_tg           | 1.11 sec                                               | 762 ms: 1.46x faster                                                  |
| async_tree_none            | 464 ms                                                 | 331 ms: 1.40x faster                                                  |
| async_tree_io              | 1.08 sec                                               | 781 ms: 1.39x faster                                                  |
| async_tree_memoization     | 555 ms                                                 | 405 ms: 1.37x faster                                                  |
| async_tree_cpu_io_mixed_tg | 723 ms                                                 | 556 ms: 1.30x faster                                                  |
| async_tree_cpu_io_mixed    | 715 ms                                                 | 563 ms: 1.27x faster                                                  |
| async_generators           | 384 ms                                                 | 348 ms: 1.11x faster                                                  |
| coroutines                 | 23.9 ms                                                | 23.1 ms: 1.04x faster                                                 |
| asyncio_websockets         | 517 ms                                                 | 544 ms: 1.05x slower                                                  |
| Geometric mean             | (ref)                                                  | 1.28x faster                                                          |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260914-vultr-x86_64-python-89fe9857dd0832de9b59-3.16.0a0-89fe985 |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| float          | 80.8 ms                                                | 73.6 ms: 1.10x faster                                                 |
| pidigits       | 184 ms                                                 | 184 ms: 1.00x faster                                                  |
| nbody          | 89.3 ms                                                | 93.9 ms: 1.05x slower                                                 |
| Geometric mean | (ref)                                                  | 1.02x faster                                                          |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260914-vultr-x86_64-python-89fe9857dd0832de9b59-3.16.0a0-89fe985 |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| regex_effbot   | 3.17 ms                                                | 2.72 ms: 1.16x faster                                                 |
| regex_v8       | 20.6 ms                                                | 21.1 ms: 1.03x slower                                                 |
| regex_compile  | 142 ms                                                 | 148 ms: 1.04x slower                                                  |
| regex_dna      | 168 ms                                                 | 181 ms: 1.08x slower                                                  |
| Geometric mean | (ref)                                                  | 1.00x faster                                                          |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260914-vultr-x86_64-python-89fe9857dd0832de9b59-3.16.0a0-89fe985 |
|----------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| tomli_loads          | 2.11 sec                                               | 1.82 sec: 1.16x faster                                                |
| json_dumps           | 10.4 ms                                                | 9.39 ms: 1.10x faster                                                 |
| xml_etree_iterparse  | 96.7 ms                                                | 92.2 ms: 1.05x faster                                                 |
| unpickle_pure_python | 221 us                                                 | 213 us: 1.03x faster                                                  |
| json_loads           | 26.5 us                                                | 27.1 us: 1.02x slower                                                 |
| xml_etree_parse      | 139 ms                                                 | 144 ms: 1.04x slower                                                  |
| xml_etree_generate   | 85.2 ms                                                | 88.5 ms: 1.04x slower                                                 |
| xml_etree_process    | 59.0 ms                                                | 63.2 ms: 1.07x slower                                                 |
| Geometric mean       | (ref)                                                  | 1.02x faster                                                          |

Benchmark hidden because not significant (1): pickle_pure_python

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260914-vultr-x86_64-python-89fe9857dd0832de9b59-3.16.0a0-89fe985 |
|------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| python_startup_no_site | 7.16 ms                                                | 7.70 ms: 1.07x slower                                                 |
| python_startup         | 9.93 ms                                                | 12.9 ms: 1.30x slower                                                 |
| Geometric mean         | (ref)                                                  | 1.18x slower                                                          |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260914-vultr-x86_64-python-89fe9857dd0832de9b59-3.16.0a0-89fe985 |
|-----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| django_template | 34.7 ms                                                | 36.6 ms: 1.06x slower                                                 |
| mako            | 11.0 ms                                                | 11.8 ms: 1.07x slower                                                 |
| Geometric mean  | (ref)                                                  | 1.06x slower                                                          |

All benchmarks:
===============

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260914-vultr-x86_64-python-89fe9857dd0832de9b59-3.16.0a0-89fe985 |
|----------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| pylint                     | 319 ms                                                 | 113 ms: 2.83x faster                                                  |
| mdp                        | 2.42 sec                                               | 1.14 sec: 2.13x faster                                                |
| deepcopy                   | 352 us                                                 | 232 us: 1.52x faster                                                  |
| async_tree_memoization_tg  | 560 ms                                                 | 370 ms: 1.51x faster                                                  |
| deepcopy_memo              | 40.3 us                                                | 26.9 us: 1.50x faster                                                 |
| async_tree_none_tg         | 446 ms                                                 | 304 ms: 1.47x faster                                                  |
| async_tree_io_tg           | 1.11 sec                                               | 762 ms: 1.46x faster                                                  |
| async_tree_none            | 464 ms                                                 | 331 ms: 1.40x faster                                                  |
| async_tree_io              | 1.08 sec                                               | 781 ms: 1.39x faster                                                  |
| async_tree_memoization     | 555 ms                                                 | 405 ms: 1.37x faster                                                  |
| typing_runtime_protocols   | 163 us                                                 | 121 us: 1.35x faster                                                  |
| go                         | 139 ms                                                 | 106 ms: 1.31x faster                                                  |
| async_tree_cpu_io_mixed_tg | 723 ms                                                 | 556 ms: 1.30x faster                                                  |
| async_tree_cpu_io_mixed    | 715 ms                                                 | 563 ms: 1.27x faster                                                  |
| comprehensions             | 19.8 us                                                | 16.0 us: 1.24x faster                                                 |
| pathlib                    | 21.5 ms                                                | 17.6 ms: 1.22x faster                                                 |
| deepcopy_reduce            | 3.08 us                                                | 2.56 us: 1.20x faster                                                 |
| raytrace                   | 299 ms                                                 | 251 ms: 1.19x faster                                                  |
| scimark_sor                | 130 ms                                                 | 110 ms: 1.18x faster                                                  |
| spectral_norm              | 110 ms                                                 | 93.2 ms: 1.18x faster                                                 |
| chaos                      | 62.8 ms                                                | 53.6 ms: 1.17x faster                                                 |
| regex_effbot               | 3.17 ms                                                | 2.72 ms: 1.16x faster                                                 |
| pyflate                    | 448 ms                                                 | 385 ms: 1.16x faster                                                  |
| tomli_loads                | 2.11 sec                                               | 1.82 sec: 1.16x faster                                                |
| dulwich_log                | 78.9 ms                                                | 68.4 ms: 1.15x faster                                                 |
| crypto_pyaes               | 76.6 ms                                                | 67.1 ms: 1.14x faster                                                 |
| bpe_tokeniser              | 4.74 sec                                               | 4.23 sec: 1.12x faster                                                |
| logging_simple             | 6.63 us                                                | 5.96 us: 1.11x faster                                                 |
| docutils                   | 2.64 sec                                               | 2.38 sec: 1.11x faster                                                |
| logging_silent             | 109 ns                                                 | 98.3 ns: 1.11x faster                                                 |
| async_generators           | 384 ms                                                 | 348 ms: 1.11x faster                                                  |
| json_dumps                 | 10.4 ms                                                | 9.39 ms: 1.10x faster                                                 |
| float                      | 80.8 ms                                                | 73.6 ms: 1.10x faster                                                 |
| scimark_monte_carlo        | 68.4 ms                                                | 62.5 ms: 1.10x faster                                                 |
| logging_format             | 7.35 us                                                | 6.72 us: 1.09x faster                                                 |
| generators                 | 32.2 ms                                                | 29.7 ms: 1.08x faster                                                 |
| scimark_fft                | 342 ms                                                 | 315 ms: 1.08x faster                                                  |
| hexiom                     | 6.17 ms                                                | 5.70 ms: 1.08x faster                                                 |
| nqueens                    | 80.1 ms                                                | 74.1 ms: 1.08x faster                                                 |
| sympy_integrate            | 20.5 ms                                                | 19.3 ms: 1.06x faster                                                 |
| html5lib                   | 63.6 ms                                                | 60.1 ms: 1.06x faster                                                 |
| xml_etree_iterparse        | 96.7 ms                                                | 92.2 ms: 1.05x faster                                                 |
| pycparser                  | 1.17 sec                                               | 1.12 sec: 1.05x faster                                                |
| sympy_sum                  | 166 ms                                                 | 159 ms: 1.04x faster                                                  |
| sympy_str                  | 292 ms                                                 | 279 ms: 1.04x faster                                                  |
| coroutines                 | 23.9 ms                                                | 23.1 ms: 1.04x faster                                                 |
| unpickle_pure_python       | 221 us                                                 | 213 us: 1.03x faster                                                  |
| pprint_pformat             | 1.52 sec                                               | 1.47 sec: 1.03x faster                                                |
| pprint_safe_repr           | 743 ms                                                 | 721 ms: 1.03x faster                                                  |
| json                       | 5.02 ms                                                | 4.90 ms: 1.03x faster                                                 |
| deltablue                  | 3.45 ms                                                | 3.38 ms: 1.02x faster                                                 |
| meteor_contest             | 104 ms                                                 | 102 ms: 1.02x faster                                                  |
| 2to3                       | 264 ms                                                 | 259 ms: 1.02x faster                                                  |
| richards                   | 45.9 ms                                                | 45.4 ms: 1.01x faster                                                 |
| thrift                     | 791 us                                                 | 783 us: 1.01x faster                                                  |
| fannkuch                   | 372 ms                                                 | 371 ms: 1.00x faster                                                  |
| pidigits                   | 184 ms                                                 | 184 ms: 1.00x faster                                                  |
| scimark_sparse_mat_mult    | 4.39 ms                                                | 4.43 ms: 1.01x slower                                                 |
| json_loads                 | 26.5 us                                                | 27.1 us: 1.02x slower                                                 |
| sympy_expand               | 468 ms                                                 | 478 ms: 1.02x slower                                                  |
| regex_v8                   | 20.6 ms                                                | 21.1 ms: 1.03x slower                                                 |
| scimark_lu                 | 114 ms                                                 | 118 ms: 1.03x slower                                                  |
| xml_etree_parse            | 139 ms                                                 | 144 ms: 1.04x slower                                                  |
| xml_etree_generate         | 85.2 ms                                                | 88.5 ms: 1.04x slower                                                 |
| regex_compile              | 142 ms                                                 | 148 ms: 1.04x slower                                                  |
| nbody                      | 89.3 ms                                                | 93.9 ms: 1.05x slower                                                 |
| asyncio_websockets         | 517 ms                                                 | 544 ms: 1.05x slower                                                  |
| django_template            | 34.7 ms                                                | 36.6 ms: 1.06x slower                                                 |
| xml_etree_process          | 59.0 ms                                                | 63.2 ms: 1.07x slower                                                 |
| mako                       | 11.0 ms                                                | 11.8 ms: 1.07x slower                                                 |
| python_startup_no_site     | 7.16 ms                                                | 7.70 ms: 1.07x slower                                                 |
| regex_dna                  | 168 ms                                                 | 181 ms: 1.08x slower                                                  |
| coverage                   | 71.4 ms                                                | 82.8 ms: 1.16x slower                                                 |
| gc_traversal               | 3.46 ms                                                | 4.08 ms: 1.18x slower                                                 |
| python_startup             | 9.93 ms                                                | 12.9 ms: 1.30x slower                                                 |
| bench_thread_pool          | 941 us                                                 | 1.35 ms: 1.43x slower                                                 |
| create_gc_cycles           | 1.09 ms                                                | 1.69 ms: 1.55x slower                                                 |
| bench_mp_pool              | 10.8 ms                                                | 257 ms: 23.75x slower                                                 |
| telco                      | 6.53 ms                                                | 161 ms: 24.70x slower                                                 |
| Geometric mean             | (ref)                                                  | 1.01x faster                                                          |

Benchmark hidden because not significant (3): richards_super, pickle_pure_python, sqlite_synth
Ignored benchmarks (23) of results/bm-20240906-3.12.6-a4a2d2b/bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b.json: aiohttp, asyncio_tcp, asyncio_tcp_ssl, chameleon, dask, flaskblogging, genshi_text, genshi_xml, gunicorn, mypy2, pickle, pickle_dict, pickle_list, sqlalchemy_declarative, sqlalchemy_imperative, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http, unpack_sequence, unpickle, unpickle_list
Ignored benchmarks (10) of results/bm-20260914-3.16.0a0-89fe985/bm-20260914-vultr-x86_64-python-89fe9857dd0832de9b59-3.16.0a0-89fe985.json: connected_components, k_core, many_optionals, shortest_path, sphinx, sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile, subparsers

- Geometric mean (including insignificant results): 1.059x faster

# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.04x
- 95% likely to have a speedup of 1.04x
- 99% likely to have a speedup of 1.03x

# Memory
- memory change: 1.15x