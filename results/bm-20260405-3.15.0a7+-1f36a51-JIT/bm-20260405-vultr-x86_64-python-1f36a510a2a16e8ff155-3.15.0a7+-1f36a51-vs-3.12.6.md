# Results vs. 3.12.6

- fork: python
- ref: 1f36a510a2a16e8ff155
- machine: linux-x86_64
- commit hash: 1f36a51
- commit date: 2026-04-05
- overall geometric mean: 1.166x faster
- HPT reliability: 100.00%
- HPT 99th percentile: 1.08x faster
- Memory change: 1.21x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260405-vultr-x86_64-python-1f36a510a2a16e8ff155-3.15.0a7+-1f36a51 |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| 2to3           | 264 ms                                                 | 250 ms: 1.05x faster                                                   |
| docutils       | 2.64 sec                                               | 2.66 sec: 1.01x slower                                                 |
| html5lib       | 63.6 ms                                                | 61.9 ms: 1.03x faster                                                  |
| Geometric mean | (ref)                                                  | 1.02x faster                                                           |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260405-vultr-x86_64-python-1f36a510a2a16e8ff155-3.15.0a7+-1f36a51 |
|----------------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| async_tree_io_tg           | 1.11 sec                                               | 547 ms: 2.03x faster                                                   |
| async_tree_none            | 464 ms                                                 | 229 ms: 2.03x faster                                                   |
| async_tree_memoization     | 555 ms                                                 | 275 ms: 2.02x faster                                                   |
| async_tree_memoization_tg  | 560 ms                                                 | 282 ms: 1.99x faster                                                   |
| async_tree_io              | 1.08 sec                                               | 561 ms: 1.93x faster                                                   |
| async_tree_none_tg         | 446 ms                                                 | 233 ms: 1.91x faster                                                   |
| async_tree_cpu_io_mixed    | 715 ms                                                 | 468 ms: 1.53x faster                                                   |
| async_tree_cpu_io_mixed_tg | 723 ms                                                 | 477 ms: 1.51x faster                                                   |
| asyncio_tcp                | 519 ms                                                 | 447 ms: 1.16x faster                                                   |
| async_generators           | 384 ms                                                 | 353 ms: 1.09x faster                                                   |
| asyncio_tcp_ssl            | 1.51 sec                                               | 1.48 sec: 1.02x faster                                                 |
| coroutines                 | 23.9 ms                                                | 23.6 ms: 1.01x faster                                                  |
| asyncio_websockets         | 517 ms                                                 | 545 ms: 1.05x slower                                                   |
| Geometric mean             | (ref)                                                  | 1.49x faster                                                           |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260405-vultr-x86_64-python-1f36a510a2a16e8ff155-3.15.0a7+-1f36a51 |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| float          | 80.8 ms                                                | 58.0 ms: 1.39x faster                                                  |
| nbody          | 89.3 ms                                                | 65.3 ms: 1.37x faster                                                  |
| pidigits       | 184 ms                                                 | 187 ms: 1.01x slower                                                   |
| Geometric mean | (ref)                                                  | 1.23x faster                                                           |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260405-vultr-x86_64-python-1f36a510a2a16e8ff155-3.15.0a7+-1f36a51 |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| regex_effbot   | 3.17 ms                                                | 2.67 ms: 1.19x faster                                                  |
| regex_compile  | 142 ms                                                 | 130 ms: 1.10x faster                                                   |
| regex_v8       | 20.6 ms                                                | 22.1 ms: 1.07x slower                                                  |
| Geometric mean | (ref)                                                  | 1.05x faster                                                           |

Benchmark hidden because not significant (1): regex_dna

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260405-vultr-x86_64-python-1f36a510a2a16e8ff155-3.15.0a7+-1f36a51 |
|----------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| tomli_loads          | 2.11 sec                                               | 1.50 sec: 1.40x faster                                                 |
| json_dumps           | 10.4 ms                                                | 8.84 ms: 1.17x faster                                                  |
| xml_etree_iterparse  | 96.7 ms                                                | 82.6 ms: 1.17x faster                                                  |
| xml_etree_process    | 59.0 ms                                                | 53.2 ms: 1.11x faster                                                  |
| unpickle_pure_python | 221 us                                                 | 200 us: 1.10x faster                                                   |
| xml_etree_generate   | 85.2 ms                                                | 79.6 ms: 1.07x faster                                                  |
| xml_etree_parse      | 139 ms                                                 | 131 ms: 1.05x faster                                                   |
| pickle_dict          | 31.8 us                                                | 32.4 us: 1.02x slower                                                  |
| json_loads           | 26.5 us                                                | 27.1 us: 1.02x slower                                                  |
| pickle_pure_python   | 308 us                                                 | 316 us: 1.03x slower                                                   |
| unpickle             | 14.1 us                                                | 14.6 us: 1.04x slower                                                  |
| pickle_list          | 4.77 us                                                | 5.20 us: 1.09x slower                                                  |
| unpickle_list        | 4.67 us                                                | 5.29 us: 1.13x slower                                                  |
| pickle               | 10.9 us                                                | 12.8 us: 1.16x slower                                                  |
| Geometric mean       | (ref)                                                  | 1.04x faster                                                           |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260405-vultr-x86_64-python-1f36a510a2a16e8ff155-3.15.0a7+-1f36a51 |
|------------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| python_startup_no_site | 7.16 ms                                                | 7.52 ms: 1.05x slower                                                  |
| python_startup         | 9.93 ms                                                | 12.8 ms: 1.29x slower                                                  |
| Geometric mean         | (ref)                                                  | 1.16x slower                                                           |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260405-vultr-x86_64-python-1f36a510a2a16e8ff155-3.15.0a7+-1f36a51 |
|-----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| genshi_text     | 22.8 ms                                                | 19.7 ms: 1.16x faster                                                  |
| django_template | 34.7 ms                                                | 33.0 ms: 1.05x faster                                                  |
| mako            | 11.0 ms                                                | 10.8 ms: 1.02x faster                                                  |
| Geometric mean  | (ref)                                                  | 1.05x faster                                                           |

Benchmark hidden because not significant (1): genshi_xml

All benchmarks:
===============

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260405-vultr-x86_64-python-1f36a510a2a16e8ff155-3.15.0a7+-1f36a51 |
|----------------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| richards                   | 45.9 ms                                                | 18.6 ms: 2.47x faster                                                  |
| richards_super             | 51.9 ms                                                | 21.8 ms: 2.38x faster                                                  |
| mdp                        | 2.42 sec                                               | 1.17 sec: 2.06x faster                                                 |
| async_tree_io_tg           | 1.11 sec                                               | 547 ms: 2.03x faster                                                   |
| async_tree_none            | 464 ms                                                 | 229 ms: 2.03x faster                                                   |
| async_tree_memoization     | 555 ms                                                 | 275 ms: 2.02x faster                                                   |
| async_tree_memoization_tg  | 560 ms                                                 | 282 ms: 1.99x faster                                                   |
| async_tree_io              | 1.08 sec                                               | 561 ms: 1.93x faster                                                   |
| async_tree_none_tg         | 446 ms                                                 | 233 ms: 1.91x faster                                                   |
| deepcopy_memo              | 40.3 us                                                | 22.3 us: 1.80x faster                                                  |
| scimark_sor                | 130 ms                                                 | 76.5 ms: 1.69x faster                                                  |
| deepcopy                   | 352 us                                                 | 221 us: 1.60x faster                                                   |
| spectral_norm              | 110 ms                                                 | 69.7 ms: 1.58x faster                                                  |
| async_tree_cpu_io_mixed    | 715 ms                                                 | 468 ms: 1.53x faster                                                   |
| async_tree_cpu_io_mixed_tg | 723 ms                                                 | 477 ms: 1.51x faster                                                   |
| scimark_lu                 | 114 ms                                                 | 76.8 ms: 1.49x faster                                                  |
| logging_silent             | 109 ns                                                 | 74.1 ns: 1.47x faster                                                  |
| tomli_loads                | 2.11 sec                                               | 1.50 sec: 1.40x faster                                                 |
| go                         | 139 ms                                                 | 99.7 ms: 1.40x faster                                                  |
| scimark_fft                | 342 ms                                                 | 245 ms: 1.39x faster                                                   |
| float                      | 80.8 ms                                                | 58.0 ms: 1.39x faster                                                  |
| comprehensions             | 19.8 us                                                | 14.4 us: 1.37x faster                                                  |
| nbody                      | 89.3 ms                                                | 65.3 ms: 1.37x faster                                                  |
| scimark_monte_carlo        | 68.4 ms                                                | 50.3 ms: 1.36x faster                                                  |
| deepcopy_reduce            | 3.08 us                                                | 2.36 us: 1.31x faster                                                  |
| deltablue                  | 3.45 ms                                                | 2.67 ms: 1.29x faster                                                  |
| logging_simple             | 6.63 us                                                | 5.19 us: 1.28x faster                                                  |
| pyflate                    | 448 ms                                                 | 357 ms: 1.25x faster                                                   |
| pathlib                    | 21.5 ms                                                | 17.2 ms: 1.25x faster                                                  |
| chaos                      | 62.8 ms                                                | 50.3 ms: 1.25x faster                                                  |
| logging_format             | 7.35 us                                                | 5.90 us: 1.25x faster                                                  |
| pprint_safe_repr           | 743 ms                                                 | 598 ms: 1.24x faster                                                   |
| pprint_pformat             | 1.52 sec                                               | 1.23 sec: 1.23x faster                                                 |
| bpe_tokeniser              | 4.74 sec                                               | 3.92 sec: 1.21x faster                                                 |
| crypto_pyaes               | 76.6 ms                                                | 64.1 ms: 1.20x faster                                                  |
| regex_effbot               | 3.17 ms                                                | 2.67 ms: 1.19x faster                                                  |
| json_dumps                 | 10.4 ms                                                | 8.84 ms: 1.17x faster                                                  |
| xml_etree_iterparse        | 96.7 ms                                                | 82.6 ms: 1.17x faster                                                  |
| asyncio_tcp                | 519 ms                                                 | 447 ms: 1.16x faster                                                   |
| genshi_text                | 22.8 ms                                                | 19.7 ms: 1.16x faster                                                  |
| scimark_sparse_mat_mult    | 4.39 ms                                                | 3.83 ms: 1.15x faster                                                  |
| dulwich_log                | 78.9 ms                                                | 69.2 ms: 1.14x faster                                                  |
| nqueens                    | 80.1 ms                                                | 70.3 ms: 1.14x faster                                                  |
| fannkuch                   | 372 ms                                                 | 333 ms: 1.12x faster                                                   |
| xml_etree_process          | 59.0 ms                                                | 53.2 ms: 1.11x faster                                                  |
| unpickle_pure_python       | 221 us                                                 | 200 us: 1.10x faster                                                   |
| regex_compile              | 142 ms                                                 | 130 ms: 1.10x faster                                                   |
| typing_runtime_protocols   | 163 us                                                 | 149 us: 1.10x faster                                                   |
| pylint                     | 319 ms                                                 | 292 ms: 1.09x faster                                                   |
| async_generators           | 384 ms                                                 | 353 ms: 1.09x faster                                                   |
| hexiom                     | 6.17 ms                                                | 5.67 ms: 1.09x faster                                                  |
| xml_etree_generate         | 85.2 ms                                                | 79.6 ms: 1.07x faster                                                  |
| meteor_contest             | 104 ms                                                 | 97.4 ms: 1.06x faster                                                  |
| xml_etree_parse            | 139 ms                                                 | 131 ms: 1.05x faster                                                   |
| 2to3                       | 264 ms                                                 | 250 ms: 1.05x faster                                                   |
| django_template            | 34.7 ms                                                | 33.0 ms: 1.05x faster                                                  |
| generators                 | 32.2 ms                                                | 30.8 ms: 1.05x faster                                                  |
| raytrace                   | 299 ms                                                 | 288 ms: 1.04x faster                                                   |
| json                       | 5.02 ms                                                | 4.87 ms: 1.03x faster                                                  |
| pycparser                  | 1.17 sec                                               | 1.13 sec: 1.03x faster                                                 |
| html5lib                   | 63.6 ms                                                | 61.9 ms: 1.03x faster                                                  |
| sqlite_synth               | 2.20 us                                                | 2.14 us: 1.03x faster                                                  |
| asyncio_tcp_ssl            | 1.51 sec                                               | 1.48 sec: 1.02x faster                                                 |
| mako                       | 11.0 ms                                                | 10.8 ms: 1.02x faster                                                  |
| coroutines                 | 23.9 ms                                                | 23.6 ms: 1.01x faster                                                  |
| sympy_integrate            | 20.5 ms                                                | 20.3 ms: 1.01x faster                                                  |
| docutils                   | 2.64 sec                                               | 2.66 sec: 1.01x slower                                                 |
| sympy_str                  | 292 ms                                                 | 295 ms: 1.01x slower                                                   |
| pidigits                   | 184 ms                                                 | 187 ms: 1.01x slower                                                   |
| pickle_dict                | 31.8 us                                                | 32.4 us: 1.02x slower                                                  |
| json_loads                 | 26.5 us                                                | 27.1 us: 1.02x slower                                                  |
| sympy_sum                  | 166 ms                                                 | 170 ms: 1.02x slower                                                   |
| sympy_expand               | 468 ms                                                 | 479 ms: 1.02x slower                                                   |
| pickle_pure_python         | 308 us                                                 | 316 us: 1.03x slower                                                   |
| unpickle                   | 14.1 us                                                | 14.6 us: 1.04x slower                                                  |
| python_startup_no_site     | 7.16 ms                                                | 7.52 ms: 1.05x slower                                                  |
| asyncio_websockets         | 517 ms                                                 | 545 ms: 1.05x slower                                                   |
| regex_v8                   | 20.6 ms                                                | 22.1 ms: 1.07x slower                                                  |
| pickle_list                | 4.77 us                                                | 5.20 us: 1.09x slower                                                  |
| unpickle_list              | 4.67 us                                                | 5.29 us: 1.13x slower                                                  |
| coverage                   | 71.4 ms                                                | 81.9 ms: 1.15x slower                                                  |
| pickle                     | 10.9 us                                                | 12.8 us: 1.16x slower                                                  |
| gc_traversal               | 3.46 ms                                                | 4.14 ms: 1.20x slower                                                  |
| python_startup             | 9.93 ms                                                | 12.8 ms: 1.29x slower                                                  |
| unpack_sequence            | 52.1 ns                                                | 69.9 ns: 1.34x slower                                                  |
| bench_thread_pool          | 941 us                                                 | 1.32 ms: 1.40x slower                                                  |
| create_gc_cycles           | 1.09 ms                                                | 1.87 ms: 1.72x slower                                                  |
| bench_mp_pool              | 10.8 ms                                                | 174 ms: 16.09x slower                                                  |
| telco                      | 6.53 ms                                                | 148 ms: 22.73x slower                                                  |
| Geometric mean             | (ref)                                                  | 1.10x faster                                                           |

Benchmark hidden because not significant (3): regex_dna, genshi_xml, thrift
Ignored benchmarks (13) of results/bm-20240906-3.12.6-a4a2d2b/bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b.json: aiohttp, chameleon, dask, flaskblogging, gunicorn, mypy2, sqlalchemy_declarative, sqlalchemy_imperative, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http
Ignored benchmarks (10) of results/bm-20260405-3.15.0a7+-1f36a51-JIT/bm-20260405-vultr-x86_64-python-1f36a510a2a16e8ff155-3.15.0a7+-1f36a51.json: connected_components, k_core, many_optionals, shortest_path, sphinx, sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile, subparsers

- Geometric mean (including insignificant results): 1.166x faster

# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.11x
- 95% likely to have a speedup of 1.10x
- 99% likely to have a speedup of 1.08x

# Memory
- memory change: 1.21x