# Results vs. 3.12.6

- fork: python
- ref: ee68f5f46a14e2bbf22d
- machine: linux-x86_64
- commit hash: ee68f5f
- commit date: 2026-08-11
- overall geometric mean: 1.069x faster
- HPT reliability: 100.00%
- HPT 99th percentile: 1.02x faster
- Memory change: 1.15x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260811-vultr-x86_64-python-ee68f5f46a14e2bbf22d-3.16.0a0-ee68f5f |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| 2to3           | 264 ms                                                 | 259 ms: 1.02x faster                                                  |
| docutils       | 2.64 sec                                               | 2.38 sec: 1.11x faster                                                |
| html5lib       | 63.6 ms                                                | 59.6 ms: 1.07x faster                                                 |
| Geometric mean | (ref)                                                  | 1.06x faster                                                          |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260811-vultr-x86_64-python-ee68f5f46a14e2bbf22d-3.16.0a0-ee68f5f |
|----------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| async_tree_none            | 464 ms                                                 | 295 ms: 1.58x faster                                                  |
| async_tree_memoization_tg  | 560 ms                                                 | 365 ms: 1.53x faster                                                  |
| async_tree_io              | 1.08 sec                                               | 721 ms: 1.50x faster                                                  |
| async_tree_none_tg         | 446 ms                                                 | 299 ms: 1.49x faster                                                  |
| async_tree_memoization     | 555 ms                                                 | 378 ms: 1.47x faster                                                  |
| async_tree_io_tg           | 1.11 sec                                               | 761 ms: 1.46x faster                                                  |
| async_tree_cpu_io_mixed    | 715 ms                                                 | 544 ms: 1.31x faster                                                  |
| async_tree_cpu_io_mixed_tg | 723 ms                                                 | 560 ms: 1.29x faster                                                  |
| async_generators           | 384 ms                                                 | 348 ms: 1.10x faster                                                  |
| asyncio_websockets         | 517 ms                                                 | 544 ms: 1.05x slower                                                  |
| Geometric mean             | (ref)                                                  | 1.32x faster                                                          |

Benchmark hidden because not significant (1): coroutines

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260811-vultr-x86_64-python-ee68f5f46a14e2bbf22d-3.16.0a0-ee68f5f |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| float          | 80.8 ms                                                | 72.6 ms: 1.11x faster                                                 |
| pidigits       | 184 ms                                                 | 184 ms: 1.00x faster                                                  |
| nbody          | 89.3 ms                                                | 90.6 ms: 1.01x slower                                                 |
| Geometric mean | (ref)                                                  | 1.03x faster                                                          |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260811-vultr-x86_64-python-ee68f5f46a14e2bbf22d-3.16.0a0-ee68f5f |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| regex_effbot   | 3.17 ms                                                | 2.55 ms: 1.24x faster                                                 |
| regex_v8       | 20.6 ms                                                | 21.0 ms: 1.02x slower                                                 |
| regex_compile  | 142 ms                                                 | 148 ms: 1.04x slower                                                  |
| regex_dna      | 168 ms                                                 | 182 ms: 1.08x slower                                                  |
| Geometric mean | (ref)                                                  | 1.02x faster                                                          |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260811-vultr-x86_64-python-ee68f5f46a14e2bbf22d-3.16.0a0-ee68f5f |
|----------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| tomli_loads          | 2.11 sec                                               | 1.85 sec: 1.14x faster                                                |
| json_dumps           | 10.4 ms                                                | 9.33 ms: 1.11x faster                                                 |
| xml_etree_iterparse  | 96.7 ms                                                | 91.2 ms: 1.06x faster                                                 |
| unpickle_pure_python | 221 us                                                 | 212 us: 1.04x faster                                                  |
| pickle_pure_python   | 308 us                                                 | 304 us: 1.01x faster                                                  |
| json_loads           | 26.5 us                                                | 27.0 us: 1.02x slower                                                 |
| xml_etree_parse      | 139 ms                                                 | 143 ms: 1.03x slower                                                  |
| xml_etree_generate   | 85.2 ms                                                | 88.0 ms: 1.03x slower                                                 |
| xml_etree_process    | 59.0 ms                                                | 62.4 ms: 1.06x slower                                                 |
| Geometric mean       | (ref)                                                  | 1.02x faster                                                          |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260811-vultr-x86_64-python-ee68f5f46a14e2bbf22d-3.16.0a0-ee68f5f |
|------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| python_startup_no_site | 7.16 ms                                                | 7.09 ms: 1.01x faster                                                 |
| python_startup         | 9.93 ms                                                | 12.6 ms: 1.27x slower                                                 |
| Geometric mean         | (ref)                                                  | 1.12x slower                                                          |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260811-vultr-x86_64-python-ee68f5f46a14e2bbf22d-3.16.0a0-ee68f5f |
|-----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| django_template | 34.7 ms                                                | 36.3 ms: 1.05x slower                                                 |
| mako            | 11.0 ms                                                | 12.0 ms: 1.09x slower                                                 |
| Geometric mean  | (ref)                                                  | 1.07x slower                                                          |

All benchmarks:
===============

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260811-vultr-x86_64-python-ee68f5f46a14e2bbf22d-3.16.0a0-ee68f5f |
|----------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| pylint                     | 319 ms                                                 | 115 ms: 2.78x faster                                                  |
| mdp                        | 2.42 sec                                               | 1.14 sec: 2.13x faster                                                |
| async_tree_none            | 464 ms                                                 | 295 ms: 1.58x faster                                                  |
| async_tree_memoization_tg  | 560 ms                                                 | 365 ms: 1.53x faster                                                  |
| deepcopy_memo              | 40.3 us                                                | 26.4 us: 1.53x faster                                                 |
| async_tree_io              | 1.08 sec                                               | 721 ms: 1.50x faster                                                  |
| async_tree_none_tg         | 446 ms                                                 | 299 ms: 1.49x faster                                                  |
| deepcopy                   | 352 us                                                 | 237 us: 1.48x faster                                                  |
| async_tree_memoization     | 555 ms                                                 | 378 ms: 1.47x faster                                                  |
| async_tree_io_tg           | 1.11 sec                                               | 761 ms: 1.46x faster                                                  |
| typing_runtime_protocols   | 163 us                                                 | 120 us: 1.36x faster                                                  |
| go                         | 139 ms                                                 | 103 ms: 1.35x faster                                                  |
| async_tree_cpu_io_mixed    | 715 ms                                                 | 544 ms: 1.31x faster                                                  |
| async_tree_cpu_io_mixed_tg | 723 ms                                                 | 560 ms: 1.29x faster                                                  |
| comprehensions             | 19.8 us                                                | 15.6 us: 1.27x faster                                                 |
| regex_effbot               | 3.17 ms                                                | 2.55 ms: 1.24x faster                                                 |
| scimark_sor                | 130 ms                                                 | 106 ms: 1.23x faster                                                  |
| raytrace                   | 299 ms                                                 | 249 ms: 1.20x faster                                                  |
| spectral_norm              | 110 ms                                                 | 91.8 ms: 1.20x faster                                                 |
| deepcopy_reduce            | 3.08 us                                                | 2.57 us: 1.20x faster                                                 |
| pathlib                    | 21.5 ms                                                | 18.0 ms: 1.20x faster                                                 |
| chaos                      | 62.8 ms                                                | 52.6 ms: 1.19x faster                                                 |
| dulwich_log                | 78.9 ms                                                | 67.7 ms: 1.17x faster                                                 |
| generators                 | 32.2 ms                                                | 27.8 ms: 1.16x faster                                                 |
| tomli_loads                | 2.11 sec                                               | 1.85 sec: 1.14x faster                                                |
| pyflate                    | 448 ms                                                 | 394 ms: 1.14x faster                                                  |
| crypto_pyaes               | 76.6 ms                                                | 67.3 ms: 1.14x faster                                                 |
| logging_silent             | 109 ns                                                 | 96.4 ns: 1.13x faster                                                 |
| bpe_tokeniser              | 4.74 sec                                               | 4.24 sec: 1.12x faster                                                |
| float                      | 80.8 ms                                                | 72.6 ms: 1.11x faster                                                 |
| docutils                   | 2.64 sec                                               | 2.38 sec: 1.11x faster                                                |
| json_dumps                 | 10.4 ms                                                | 9.33 ms: 1.11x faster                                                 |
| async_generators           | 384 ms                                                 | 348 ms: 1.10x faster                                                  |
| scimark_fft                | 342 ms                                                 | 311 ms: 1.10x faster                                                  |
| scimark_monte_carlo        | 68.4 ms                                                | 62.7 ms: 1.09x faster                                                 |
| nqueens                    | 80.1 ms                                                | 73.9 ms: 1.08x faster                                                 |
| sympy_integrate            | 20.5 ms                                                | 19.1 ms: 1.08x faster                                                 |
| hexiom                     | 6.17 ms                                                | 5.74 ms: 1.08x faster                                                 |
| html5lib                   | 63.6 ms                                                | 59.6 ms: 1.07x faster                                                 |
| logging_simple             | 6.63 us                                                | 6.22 us: 1.07x faster                                                 |
| xml_etree_iterparse        | 96.7 ms                                                | 91.2 ms: 1.06x faster                                                 |
| sympy_sum                  | 166 ms                                                 | 158 ms: 1.05x faster                                                  |
| logging_format             | 7.35 us                                                | 7.00 us: 1.05x faster                                                 |
| sympy_str                  | 292 ms                                                 | 278 ms: 1.05x faster                                                  |
| pycparser                  | 1.17 sec                                               | 1.12 sec: 1.04x faster                                                |
| unpickle_pure_python       | 221 us                                                 | 212 us: 1.04x faster                                                  |
| deltablue                  | 3.45 ms                                                | 3.32 ms: 1.04x faster                                                 |
| json                       | 5.02 ms                                                | 4.87 ms: 1.03x faster                                                 |
| richards                   | 45.9 ms                                                | 44.9 ms: 1.02x faster                                                 |
| 2to3                       | 264 ms                                                 | 259 ms: 1.02x faster                                                  |
| thrift                     | 791 us                                                 | 778 us: 1.02x faster                                                  |
| richards_super             | 51.9 ms                                                | 51.1 ms: 1.01x faster                                                 |
| pickle_pure_python         | 308 us                                                 | 304 us: 1.01x faster                                                  |
| pprint_safe_repr           | 743 ms                                                 | 735 ms: 1.01x faster                                                  |
| pprint_pformat             | 1.52 sec                                               | 1.50 sec: 1.01x faster                                                |
| python_startup_no_site     | 7.16 ms                                                | 7.09 ms: 1.01x faster                                                 |
| meteor_contest             | 104 ms                                                 | 103 ms: 1.01x faster                                                  |
| pidigits                   | 184 ms                                                 | 184 ms: 1.00x faster                                                  |
| sympy_expand               | 468 ms                                                 | 474 ms: 1.01x slower                                                  |
| nbody                      | 89.3 ms                                                | 90.6 ms: 1.01x slower                                                 |
| scimark_lu                 | 114 ms                                                 | 116 ms: 1.02x slower                                                  |
| json_loads                 | 26.5 us                                                | 27.0 us: 1.02x slower                                                 |
| regex_v8                   | 20.6 ms                                                | 21.0 ms: 1.02x slower                                                 |
| xml_etree_parse            | 139 ms                                                 | 143 ms: 1.03x slower                                                  |
| xml_etree_generate         | 85.2 ms                                                | 88.0 ms: 1.03x slower                                                 |
| scimark_sparse_mat_mult    | 4.39 ms                                                | 4.55 ms: 1.03x slower                                                 |
| regex_compile              | 142 ms                                                 | 148 ms: 1.04x slower                                                  |
| django_template            | 34.7 ms                                                | 36.3 ms: 1.05x slower                                                 |
| asyncio_websockets         | 517 ms                                                 | 544 ms: 1.05x slower                                                  |
| xml_etree_process          | 59.0 ms                                                | 62.4 ms: 1.06x slower                                                 |
| gc_traversal               | 3.46 ms                                                | 3.72 ms: 1.08x slower                                                 |
| regex_dna                  | 168 ms                                                 | 182 ms: 1.08x slower                                                  |
| mako                       | 11.0 ms                                                | 12.0 ms: 1.09x slower                                                 |
| coverage                   | 71.4 ms                                                | 82.5 ms: 1.15x slower                                                 |
| python_startup             | 9.93 ms                                                | 12.6 ms: 1.27x slower                                                 |
| bench_thread_pool          | 941 us                                                 | 1.34 ms: 1.42x slower                                                 |
| create_gc_cycles           | 1.09 ms                                                | 1.67 ms: 1.53x slower                                                 |
| bench_mp_pool              | 10.8 ms                                                | 233 ms: 21.59x slower                                                 |
| telco                      | 6.53 ms                                                | 162 ms: 24.76x slower                                                 |
| Geometric mean             | (ref)                                                  | 1.03x faster                                                          |

Benchmark hidden because not significant (3): coroutines, fannkuch, sqlite_synth
Ignored benchmarks (23) of results/bm-20240906-3.12.6-a4a2d2b/bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b.json: aiohttp, asyncio_tcp, asyncio_tcp_ssl, chameleon, dask, flaskblogging, genshi_text, genshi_xml, gunicorn, mypy2, pickle, pickle_dict, pickle_list, sqlalchemy_declarative, sqlalchemy_imperative, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http, unpack_sequence, unpickle, unpickle_list
Ignored benchmarks (10) of results/bm-20260811-3.16.0a0-ee68f5f/bm-20260811-vultr-x86_64-python-ee68f5f46a14e2bbf22d-3.16.0a0-ee68f5f.json: connected_components, k_core, many_optionals, shortest_path, sphinx, sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile, subparsers

- Geometric mean (including insignificant results): 1.069x faster

# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.04x
- 95% likely to have a speedup of 1.04x
- 99% likely to have a speedup of 1.02x

# Memory
- memory change: 1.15x