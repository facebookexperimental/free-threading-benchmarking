# Results vs. 3.13.0rc2

- fork: python
- ref: 852ec189784d6f11e6c2
- machine: linux-x86_64
- commit hash: 852ec18
- commit date: 2026-02-19
- overall geometric mean: 1.041x faster
- HPT reliability: 100.00%
- HPT 99th percentile: 1.02x faster
- Memory change: 1.16x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260219-vultr-x86_64-python-852ec189784d6f11e6c2-3.15.0a6+-852ec18 |
|----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| 2to3           | 260 ms                                                       | 248 ms: 1.05x faster                                                   |
| docutils       | 2.62 sec                                                     | 2.57 sec: 1.02x faster                                                 |
| html5lib       | 67.0 ms                                                      | 60.9 ms: 1.10x faster                                                  |
| Geometric mean | (ref)                                                        | 1.05x faster                                                           |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260219-vultr-x86_64-python-852ec189784d6f11e6c2-3.15.0a6+-852ec18 |
|----------------------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| async_tree_io_tg           | 913 ms                                                       | 573 ms: 1.60x faster                                                   |
| async_tree_memoization     | 461 ms                                                       | 303 ms: 1.52x faster                                                   |
| async_tree_io              | 876 ms                                                       | 582 ms: 1.50x faster                                                   |
| async_tree_none            | 354 ms                                                       | 246 ms: 1.44x faster                                                   |
| async_tree_cpu_io_mixed    | 666 ms                                                       | 479 ms: 1.39x faster                                                   |
| async_tree_memoization_tg  | 414 ms                                                       | 299 ms: 1.38x faster                                                   |
| async_tree_none_tg         | 336 ms                                                       | 252 ms: 1.34x faster                                                   |
| async_tree_cpu_io_mixed_tg | 638 ms                                                       | 486 ms: 1.31x faster                                                   |
| async_generators           | 377 ms                                                       | 337 ms: 1.12x faster                                                   |
| coroutines                 | 23.6 ms                                                      | 23.1 ms: 1.02x faster                                                  |
| asyncio_websockets         | 520 ms                                                       | 545 ms: 1.05x slower                                                   |
| Geometric mean             | (ref)                                                        | 1.31x faster                                                           |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260219-vultr-x86_64-python-852ec189784d6f11e6c2-3.15.0a6+-852ec18 |
|----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| float          | 77.5 ms                                                      | 69.3 ms: 1.12x faster                                                  |
| pidigits       | 217 ms                                                       | 210 ms: 1.03x faster                                                   |
| nbody          | 85.1 ms                                                      | 91.3 ms: 1.07x slower                                                  |
| Geometric mean | (ref)                                                        | 1.02x faster                                                           |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260219-vultr-x86_64-python-852ec189784d6f11e6c2-3.15.0a6+-852ec18 |
|----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| regex_compile  | 132 ms                                                       | 122 ms: 1.08x faster                                                   |
| regex_v8       | 22.7 ms                                                      | 22.8 ms: 1.01x slower                                                  |
| regex_effbot   | 3.08 ms                                                      | 3.15 ms: 1.02x slower                                                  |
| regex_dna      | 180 ms                                                       | 186 ms: 1.03x slower                                                   |
| Geometric mean | (ref)                                                        | 1.00x faster                                                           |

Benchmarks with tag 'serialize':
================================

| Benchmark           | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260219-vultr-x86_64-python-852ec189784d6f11e6c2-3.15.0a6+-852ec18 |
|---------------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| xml_etree_iterparse | 94.9 ms                                                      | 85.4 ms: 1.11x faster                                                  |
| tomli_loads         | 2.01 sec                                                     | 1.82 sec: 1.10x faster                                                 |
| json_dumps          | 10.5 ms                                                      | 9.83 ms: 1.07x faster                                                  |
| xml_etree_parse     | 136 ms                                                       | 131 ms: 1.04x faster                                                   |
| xml_etree_generate  | 85.4 ms                                                      | 83.7 ms: 1.02x faster                                                  |
| xml_etree_process   | 59.3 ms                                                      | 58.6 ms: 1.01x faster                                                  |
| pickle_pure_python  | 294 us                                                       | 303 us: 1.03x slower                                                   |
| json_loads          | 27.0 us                                                      | 28.0 us: 1.04x slower                                                  |
| Geometric mean      | (ref)                                                        | 1.03x faster                                                           |

Benchmark hidden because not significant (1): unpickle_pure_python

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260219-vultr-x86_64-python-852ec189784d6f11e6c2-3.15.0a6+-852ec18 |
|------------------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| python_startup_no_site | 7.39 ms                                                      | 7.41 ms: 1.00x slower                                                  |
| python_startup         | 11.0 ms                                                      | 12.5 ms: 1.14x slower                                                  |
| Geometric mean         | (ref)                                                        | 1.07x slower                                                           |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260219-vultr-x86_64-python-852ec189784d6f11e6c2-3.15.0a6+-852ec18 |
|-----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| genshi_text     | 21.5 ms                                                      | 21.1 ms: 1.02x faster                                                  |
| genshi_xml      | 48.8 ms                                                      | 49.7 ms: 1.02x slower                                                  |
| django_template | 34.1 ms                                                      | 35.0 ms: 1.03x slower                                                  |
| mako            | 11.3 ms                                                      | 11.8 ms: 1.04x slower                                                  |
| Geometric mean  | (ref)                                                        | 1.01x slower                                                           |

All benchmarks:
===============

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260219-vultr-x86_64-python-852ec189784d6f11e6c2-3.15.0a6+-852ec18 |
|----------------------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| mdp                        | 2.36 sec                                                     | 1.15 sec: 2.05x faster                                                 |
| async_tree_io_tg           | 913 ms                                                       | 573 ms: 1.60x faster                                                   |
| deepcopy                   | 355 us                                                       | 230 us: 1.54x faster                                                   |
| async_tree_memoization     | 461 ms                                                       | 303 ms: 1.52x faster                                                   |
| async_tree_io              | 876 ms                                                       | 582 ms: 1.50x faster                                                   |
| deepcopy_memo              | 39.1 us                                                      | 26.1 us: 1.50x faster                                                  |
| async_tree_none            | 354 ms                                                       | 246 ms: 1.44x faster                                                   |
| async_tree_cpu_io_mixed    | 666 ms                                                       | 479 ms: 1.39x faster                                                   |
| async_tree_memoization_tg  | 414 ms                                                       | 299 ms: 1.38x faster                                                   |
| go                         | 141 ms                                                       | 104 ms: 1.36x faster                                                   |
| async_tree_none_tg         | 336 ms                                                       | 252 ms: 1.34x faster                                                   |
| async_tree_cpu_io_mixed_tg | 638 ms                                                       | 486 ms: 1.31x faster                                                   |
| scimark_sor                | 134 ms                                                       | 106 ms: 1.27x faster                                                   |
| deepcopy_reduce            | 3.11 us                                                      | 2.49 us: 1.25x faster                                                  |
| spectral_norm              | 111 ms                                                       | 91.5 ms: 1.21x faster                                                  |
| scimark_fft                | 349 ms                                                       | 309 ms: 1.13x faster                                                   |
| dulwich_log                | 74.8 ms                                                      | 66.2 ms: 1.13x faster                                                  |
| async_generators           | 377 ms                                                       | 337 ms: 1.12x faster                                                   |
| pylint                     | 317 ms                                                       | 284 ms: 1.12x faster                                                   |
| float                      | 77.5 ms                                                      | 69.3 ms: 1.12x faster                                                  |
| xml_etree_iterparse        | 94.9 ms                                                      | 85.4 ms: 1.11x faster                                                  |
| pathlib                    | 19.2 ms                                                      | 17.4 ms: 1.10x faster                                                  |
| tomli_loads                | 2.01 sec                                                     | 1.82 sec: 1.10x faster                                                 |
| html5lib                   | 67.0 ms                                                      | 60.9 ms: 1.10x faster                                                  |
| scimark_sparse_mat_mult    | 4.71 ms                                                      | 4.32 ms: 1.09x faster                                                  |
| richards                   | 45.2 ms                                                      | 41.6 ms: 1.09x faster                                                  |
| pyflate                    | 449 ms                                                       | 415 ms: 1.08x faster                                                   |
| regex_compile              | 132 ms                                                       | 122 ms: 1.08x faster                                                   |
| richards_super             | 51.6 ms                                                      | 47.9 ms: 1.08x faster                                                  |
| json_dumps                 | 10.5 ms                                                      | 9.83 ms: 1.07x faster                                                  |
| pprint_safe_repr           | 738 ms                                                       | 689 ms: 1.07x faster                                                   |
| bpe_tokeniser              | 4.45 sec                                                     | 4.16 sec: 1.07x faster                                                 |
| hexiom                     | 5.99 ms                                                      | 5.61 ms: 1.07x faster                                                  |
| pprint_pformat             | 1.50 sec                                                     | 1.40 sec: 1.07x faster                                                 |
| scimark_monte_carlo        | 65.4 ms                                                      | 61.5 ms: 1.06x faster                                                  |
| logging_silent             | 103 ns                                                       | 96.7 ns: 1.06x faster                                                  |
| chaos                      | 57.3 ms                                                      | 54.4 ms: 1.05x faster                                                  |
| 2to3                       | 260 ms                                                       | 248 ms: 1.05x faster                                                   |
| sympy_integrate            | 19.8 ms                                                      | 19.0 ms: 1.04x faster                                                  |
| xml_etree_parse            | 136 ms                                                       | 131 ms: 1.04x faster                                                   |
| generators                 | 28.8 ms                                                      | 27.8 ms: 1.04x faster                                                  |
| scimark_lu                 | 113 ms                                                       | 109 ms: 1.03x faster                                                   |
| logging_simple             | 6.16 us                                                      | 5.98 us: 1.03x faster                                                  |
| pidigits                   | 217 ms                                                       | 210 ms: 1.03x faster                                                   |
| pycparser                  | 1.12 sec                                                     | 1.09 sec: 1.03x faster                                                 |
| comprehensions             | 16.5 us                                                      | 16.0 us: 1.03x faster                                                  |
| genshi_text                | 21.5 ms                                                      | 21.1 ms: 1.02x faster                                                  |
| coroutines                 | 23.6 ms                                                      | 23.1 ms: 1.02x faster                                                  |
| xml_etree_generate         | 85.4 ms                                                      | 83.7 ms: 1.02x faster                                                  |
| docutils                   | 2.62 sec                                                     | 2.57 sec: 1.02x faster                                                 |
| coverage                   | 83.0 ms                                                      | 81.7 ms: 1.02x faster                                                  |
| raytrace                   | 253 ms                                                       | 249 ms: 1.01x faster                                                   |
| logging_format             | 6.84 us                                                      | 6.74 us: 1.01x faster                                                  |
| xml_etree_process          | 59.3 ms                                                      | 58.6 ms: 1.01x faster                                                  |
| nqueens                    | 78.6 ms                                                      | 78.0 ms: 1.01x faster                                                  |
| meteor_contest             | 102 ms                                                       | 101 ms: 1.01x faster                                                   |
| python_startup_no_site     | 7.39 ms                                                      | 7.41 ms: 1.00x slower                                                  |
| regex_v8                   | 22.7 ms                                                      | 22.8 ms: 1.01x slower                                                  |
| typing_runtime_protocols   | 155 us                                                       | 157 us: 1.02x slower                                                   |
| genshi_xml                 | 48.8 ms                                                      | 49.7 ms: 1.02x slower                                                  |
| regex_effbot               | 3.08 ms                                                      | 3.15 ms: 1.02x slower                                                  |
| django_template            | 34.1 ms                                                      | 35.0 ms: 1.03x slower                                                  |
| pickle_pure_python         | 294 us                                                       | 303 us: 1.03x slower                                                   |
| json                       | 4.93 ms                                                      | 5.08 ms: 1.03x slower                                                  |
| regex_dna                  | 180 ms                                                       | 186 ms: 1.03x slower                                                   |
| json_loads                 | 27.0 us                                                      | 28.0 us: 1.04x slower                                                  |
| mako                       | 11.3 ms                                                      | 11.8 ms: 1.04x slower                                                  |
| deltablue                  | 3.12 ms                                                      | 3.24 ms: 1.04x slower                                                  |
| asyncio_websockets         | 520 ms                                                       | 545 ms: 1.05x slower                                                   |
| fannkuch                   | 370 ms                                                       | 388 ms: 1.05x slower                                                   |
| nbody                      | 85.1 ms                                                      | 91.3 ms: 1.07x slower                                                  |
| sympy_expand               | 457 ms                                                       | 492 ms: 1.08x slower                                                   |
| python_startup             | 11.0 ms                                                      | 12.5 ms: 1.14x slower                                                  |
| gc_traversal               | 3.14 ms                                                      | 4.23 ms: 1.35x slower                                                  |
| create_gc_cycles           | 1.34 ms                                                      | 1.87 ms: 1.40x slower                                                  |
| bench_thread_pool          | 919 us                                                       | 1.32 ms: 1.43x slower                                                  |
| telco                      | 7.82 ms                                                      | 155 ms: 19.80x slower                                                  |
| bench_mp_pool              | 11.0 ms                                                      | 263 ms: 23.96x slower                                                  |
| Geometric mean             | (ref)                                                        | 1.00x slower                                                           |

Benchmark hidden because not significant (6): sympy_str, thrift, sympy_sum, crypto_pyaes, unpickle_pure_python, sqlite_synth
Ignored benchmarks (18) of results/bm-20240906-3.13.0rc2-ec61006/bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006.json: aiohttp, asyncio_tcp, asyncio_tcp_ssl, chameleon, dask, flaskblogging, gunicorn, pickle, pickle_dict, pickle_list, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http, unpack_sequence, unpickle, unpickle_list
Ignored benchmarks (10) of results/bm-20260219-3.15.0a6+-852ec18/bm-20260219-vultr-x86_64-python-852ec189784d6f11e6c2-3.15.0a6+-852ec18.json: connected_components, k_core, many_optionals, shortest_path, sphinx, sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile, subparsers

- Geometric mean (including insignificant results): 1.041x faster

# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.03x
- 95% likely to have a speedup of 1.03x
- 99% likely to have a speedup of 1.02x

# Memory
- memory change: 1.16x