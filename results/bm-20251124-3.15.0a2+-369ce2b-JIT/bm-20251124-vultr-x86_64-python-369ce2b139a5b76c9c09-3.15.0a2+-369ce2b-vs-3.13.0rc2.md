# Results vs. 3.13.0rc2

- fork: python
- ref: 369ce2b139a5b76c9c09
- machine: linux-x86_64
- commit hash: 369ce2b
- commit date: 2025-11-24
- overall geometric mean: 1.064x faster
- HPT reliability: 99.99%
- HPT 99th percentile: 1.02x faster
- Memory change: 1.18x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20251124-vultr-x86_64-python-369ce2b139a5b76c9c09-3.15.0a2+-369ce2b |
|----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| 2to3           | 260 ms                                                       | 249 ms: 1.04x faster                                                   |
| Geometric mean | (ref)                                                        | 1.02x faster                                                           |

Benchmark hidden because not significant (1): html5lib

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20251124-vultr-x86_64-python-369ce2b139a5b76c9c09-3.15.0a2+-369ce2b |
|----------------------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| async_tree_io              | 876 ms                                                       | 563 ms: 1.56x faster                                                   |
| async_tree_io_tg           | 913 ms                                                       | 603 ms: 1.51x faster                                                   |
| async_tree_memoization     | 461 ms                                                       | 310 ms: 1.49x faster                                                   |
| async_tree_none            | 354 ms                                                       | 247 ms: 1.43x faster                                                   |
| async_tree_none_tg         | 336 ms                                                       | 249 ms: 1.35x faster                                                   |
| async_tree_cpu_io_mixed    | 666 ms                                                       | 496 ms: 1.34x faster                                                   |
| async_tree_memoization_tg  | 414 ms                                                       | 313 ms: 1.32x faster                                                   |
| async_tree_cpu_io_mixed_tg | 638 ms                                                       | 494 ms: 1.29x faster                                                   |
| async_generators           | 377 ms                                                       | 364 ms: 1.04x faster                                                   |
| coroutines                 | 23.6 ms                                                      | 23.8 ms: 1.01x slower                                                  |
| asyncio_websockets         | 520 ms                                                       | 545 ms: 1.05x slower                                                   |
| Geometric mean             | (ref)                                                        | 1.28x faster                                                           |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20251124-vultr-x86_64-python-369ce2b139a5b76c9c09-3.15.0a2+-369ce2b |
|----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| float          | 77.5 ms                                                      | 60.3 ms: 1.28x faster                                                  |
| pidigits       | 217 ms                                                       | 196 ms: 1.11x faster                                                   |
| nbody          | 85.1 ms                                                      | 90.3 ms: 1.06x slower                                                  |
| Geometric mean | (ref)                                                        | 1.10x faster                                                           |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20251124-vultr-x86_64-python-369ce2b139a5b76c9c09-3.15.0a2+-369ce2b |
|----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| regex_effbot   | 3.08 ms                                                      | 2.78 ms: 1.11x faster                                                  |
| regex_compile  | 132 ms                                                       | 125 ms: 1.06x faster                                                   |
| regex_v8       | 22.7 ms                                                      | 23.0 ms: 1.01x slower                                                  |
| Geometric mean | (ref)                                                        | 1.04x faster                                                           |

Benchmark hidden because not significant (1): regex_dna

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20251124-vultr-x86_64-python-369ce2b139a5b76c9c09-3.15.0a2+-369ce2b |
|----------------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| xml_etree_iterparse  | 94.9 ms                                                      | 82.0 ms: 1.16x faster                                                  |
| unpickle_pure_python | 210 us                                                       | 182 us: 1.15x faster                                                   |
| json_dumps           | 10.5 ms                                                      | 9.24 ms: 1.14x faster                                                  |
| xml_etree_process    | 59.3 ms                                                      | 53.3 ms: 1.11x faster                                                  |
| xml_etree_generate   | 85.4 ms                                                      | 76.9 ms: 1.11x faster                                                  |
| tomli_loads          | 2.01 sec                                                     | 1.83 sec: 1.10x faster                                                 |
| xml_etree_parse      | 136 ms                                                       | 129 ms: 1.05x faster                                                   |
| json_loads           | 27.0 us                                                      | 28.6 us: 1.06x slower                                                  |
| Geometric mean       | (ref)                                                        | 1.08x faster                                                           |

Benchmark hidden because not significant (1): pickle_pure_python

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20251124-vultr-x86_64-python-369ce2b139a5b76c9c09-3.15.0a2+-369ce2b |
|------------------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| python_startup_no_site | 7.39 ms                                                      | 7.24 ms: 1.02x faster                                                  |
| python_startup         | 11.0 ms                                                      | 12.4 ms: 1.13x slower                                                  |
| Geometric mean         | (ref)                                                        | 1.05x slower                                                           |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20251124-vultr-x86_64-python-369ce2b139a5b76c9c09-3.15.0a2+-369ce2b |
|-----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| mako            | 11.3 ms                                                      | 10.8 ms: 1.05x faster                                                  |
| django_template | 34.1 ms                                                      | 34.4 ms: 1.01x slower                                                  |
| genshi_xml      | 48.8 ms                                                      | 55.2 ms: 1.13x slower                                                  |
| Geometric mean  | (ref)                                                        | 1.02x slower                                                           |

Benchmark hidden because not significant (1): genshi_text

All benchmarks:
===============

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20251124-vultr-x86_64-python-369ce2b139a5b76c9c09-3.15.0a2+-369ce2b |
|----------------------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| richards                   | 45.2 ms                                                      | 20.9 ms: 2.16x faster                                                  |
| richards_super             | 51.6 ms                                                      | 25.4 ms: 2.03x faster                                                  |
| mdp                        | 2.36 sec                                                     | 1.33 sec: 1.77x faster                                                 |
| deepcopy_memo              | 39.1 us                                                      | 25.0 us: 1.56x faster                                                  |
| async_tree_io              | 876 ms                                                       | 563 ms: 1.56x faster                                                   |
| async_tree_io_tg           | 913 ms                                                       | 603 ms: 1.51x faster                                                   |
| async_tree_memoization     | 461 ms                                                       | 310 ms: 1.49x faster                                                   |
| async_tree_none            | 354 ms                                                       | 247 ms: 1.43x faster                                                   |
| deepcopy                   | 355 us                                                       | 251 us: 1.41x faster                                                   |
| scimark_sor                | 134 ms                                                       | 96.7 ms: 1.39x faster                                                  |
| go                         | 141 ms                                                       | 103 ms: 1.36x faster                                                   |
| scimark_fft                | 349 ms                                                       | 258 ms: 1.36x faster                                                   |
| async_tree_none_tg         | 336 ms                                                       | 249 ms: 1.35x faster                                                   |
| async_tree_cpu_io_mixed    | 666 ms                                                       | 496 ms: 1.34x faster                                                   |
| spectral_norm              | 111 ms                                                       | 83.2 ms: 1.34x faster                                                  |
| async_tree_memoization_tg  | 414 ms                                                       | 313 ms: 1.32x faster                                                   |
| async_tree_cpu_io_mixed_tg | 638 ms                                                       | 494 ms: 1.29x faster                                                   |
| float                      | 77.5 ms                                                      | 60.3 ms: 1.28x faster                                                  |
| logging_silent             | 103 ns                                                       | 83.6 ns: 1.23x faster                                                  |
| deepcopy_reduce            | 3.11 us                                                      | 2.60 us: 1.20x faster                                                  |
| pyflate                    | 449 ms                                                       | 382 ms: 1.17x faster                                                   |
| xml_etree_iterparse        | 94.9 ms                                                      | 82.0 ms: 1.16x faster                                                  |
| unpickle_pure_python       | 210 us                                                       | 182 us: 1.15x faster                                                   |
| scimark_monte_carlo        | 65.4 ms                                                      | 57.3 ms: 1.14x faster                                                  |
| json_dumps                 | 10.5 ms                                                      | 9.24 ms: 1.14x faster                                                  |
| scimark_sparse_mat_mult    | 4.71 ms                                                      | 4.17 ms: 1.13x faster                                                  |
| bpe_tokeniser              | 4.45 sec                                                     | 3.99 sec: 1.11x faster                                                 |
| xml_etree_process          | 59.3 ms                                                      | 53.3 ms: 1.11x faster                                                  |
| xml_etree_generate         | 85.4 ms                                                      | 76.9 ms: 1.11x faster                                                  |
| scimark_lu                 | 113 ms                                                       | 101 ms: 1.11x faster                                                   |
| regex_effbot               | 3.08 ms                                                      | 2.78 ms: 1.11x faster                                                  |
| pidigits                   | 217 ms                                                       | 196 ms: 1.11x faster                                                   |
| deltablue                  | 3.12 ms                                                      | 2.83 ms: 1.11x faster                                                  |
| tomli_loads                | 2.01 sec                                                     | 1.83 sec: 1.10x faster                                                 |
| pprint_safe_repr           | 738 ms                                                       | 674 ms: 1.09x faster                                                   |
| pathlib                    | 19.2 ms                                                      | 17.7 ms: 1.08x faster                                                  |
| pprint_pformat             | 1.50 sec                                                     | 1.39 sec: 1.08x faster                                                 |
| regex_compile              | 132 ms                                                       | 125 ms: 1.06x faster                                                   |
| xml_etree_parse            | 136 ms                                                       | 129 ms: 1.05x faster                                                   |
| mako                       | 11.3 ms                                                      | 10.8 ms: 1.05x faster                                                  |
| chaos                      | 57.3 ms                                                      | 54.5 ms: 1.05x faster                                                  |
| 2to3                       | 260 ms                                                       | 249 ms: 1.04x faster                                                   |
| logging_simple             | 6.16 us                                                      | 5.92 us: 1.04x faster                                                  |
| thrift                     | 778 us                                                       | 751 us: 1.04x faster                                                   |
| meteor_contest             | 102 ms                                                       | 98.1 ms: 1.04x faster                                                  |
| logging_format             | 6.84 us                                                      | 6.61 us: 1.04x faster                                                  |
| async_generators           | 377 ms                                                       | 364 ms: 1.04x faster                                                   |
| comprehensions             | 16.5 us                                                      | 16.0 us: 1.03x faster                                                  |
| python_startup_no_site     | 7.39 ms                                                      | 7.24 ms: 1.02x faster                                                  |
| coverage                   | 83.0 ms                                                      | 81.5 ms: 1.02x faster                                                  |
| crypto_pyaes               | 67.9 ms                                                      | 67.1 ms: 1.01x faster                                                  |
| fannkuch                   | 370 ms                                                       | 366 ms: 1.01x faster                                                   |
| generators                 | 28.8 ms                                                      | 29.0 ms: 1.01x slower                                                  |
| coroutines                 | 23.6 ms                                                      | 23.8 ms: 1.01x slower                                                  |
| django_template            | 34.1 ms                                                      | 34.4 ms: 1.01x slower                                                  |
| regex_v8                   | 22.7 ms                                                      | 23.0 ms: 1.01x slower                                                  |
| typing_runtime_protocols   | 155 us                                                       | 157 us: 1.01x slower                                                   |
| hexiom                     | 5.99 ms                                                      | 6.08 ms: 1.01x slower                                                  |
| json                       | 4.93 ms                                                      | 5.01 ms: 1.02x slower                                                  |
| raytrace                   | 253 ms                                                       | 262 ms: 1.04x slower                                                   |
| asyncio_websockets         | 520 ms                                                       | 545 ms: 1.05x slower                                                   |
| json_loads                 | 27.0 us                                                      | 28.6 us: 1.06x slower                                                  |
| sympy_integrate            | 19.8 ms                                                      | 21.0 ms: 1.06x slower                                                  |
| nbody                      | 85.1 ms                                                      | 90.3 ms: 1.06x slower                                                  |
| nqueens                    | 78.6 ms                                                      | 85.1 ms: 1.08x slower                                                  |
| sympy_expand               | 457 ms                                                       | 507 ms: 1.11x slower                                                   |
| sympy_sum                  | 156 ms                                                       | 173 ms: 1.11x slower                                                   |
| python_startup             | 11.0 ms                                                      | 12.4 ms: 1.13x slower                                                  |
| genshi_xml                 | 48.8 ms                                                      | 55.2 ms: 1.13x slower                                                  |
| sympy_str                  | 275 ms                                                       | 313 ms: 1.14x slower                                                   |
| bench_thread_pool          | 919 us                                                       | 1.10 ms: 1.19x slower                                                  |
| bench_mp_pool              | 11.0 ms                                                      | 13.2 ms: 1.20x slower                                                  |
| gc_traversal               | 3.14 ms                                                      | 4.27 ms: 1.36x slower                                                  |
| create_gc_cycles           | 1.34 ms                                                      | 1.91 ms: 1.43x slower                                                  |
| telco                      | 7.82 ms                                                      | 161 ms: 20.60x slower                                                  |
| Geometric mean             | (ref)                                                        | 1.06x faster                                                           |

Benchmark hidden because not significant (8): pylint, pycparser, sqlite_synth, pickle_pure_python, regex_dna, dulwich_log, genshi_text, html5lib
Ignored benchmarks (19) of results/bm-20240906-3.13.0rc2-ec61006/bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006.json: aiohttp, asyncio_tcp, asyncio_tcp_ssl, chameleon, dask, docutils, flaskblogging, gunicorn, pickle, pickle_dict, pickle_list, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http, unpack_sequence, unpickle, unpickle_list
Ignored benchmarks (7) of results/bm-20251124-3.15.0a2+-369ce2b-JIT/bm-20251124-vultr-x86_64-python-369ce2b139a5b76c9c09-3.15.0a2+-369ce2b.json: many_optionals, sphinx, sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile, subparsers

- Geometric mean (including insignificant results): 1.064x faster

# HPT report

- Reliability score: 99.99% likely to be faster
- 90% likely to have a speedup of 1.04x
- 95% likely to have a speedup of 1.03x
- 99% likely to have a speedup of 1.02x

# Memory
- memory change: 1.18x