# Results vs. 3.12.6

- fork: Fidget-Spinner
- ref: loop_peeling
- machine: darwin-arm64
- commit hash: e650bf3
- commit date: 2026-01-03
- overall geometric mean: 1.261x faster
- HPT reliability: 100.00%
- HPT 99th percentile: 1.15x faster
- Memory change: 1.26x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260103-macm4pro-arm64-Fidget%2dSpinner-loop_peeling-3.15.0a3+-e650bf3 |
|----------------|:--------------------------------------------------------:|:--------------------------------------------------------------------------:|
| 2to3           | 114 ms                                                   | 107 ms: 1.06x faster                                                       |
| docutils       | 1.02 sec                                                 | 1000 ms: 1.02x faster                                                      |
| html5lib       | 23.0 ms                                                  | 19.7 ms: 1.17x faster                                                      |
| sphinx         | 434 ms                                                   | 390 ms: 1.11x faster                                                       |
| Geometric mean | (ref)                                                    | 1.09x faster                                                               |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                        | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260103-macm4pro-arm64-Fidget%2dSpinner-loop_peeling-3.15.0a3+-e650bf3 |
|----------------------------------|:--------------------------------------------------------:|:--------------------------------------------------------------------------:|
| async_tree_eager_io              | 496 ms                                                   | 219 ms: 2.27x faster                                                       |
| async_tree_io_tg                 | 480 ms                                                   | 217 ms: 2.21x faster                                                       |
| async_tree_eager_io_tg           | 446 ms                                                   | 216 ms: 2.06x faster                                                       |
| async_tree_io                    | 459 ms                                                   | 225 ms: 2.04x faster                                                       |
| async_tree_none                  | 178 ms                                                   | 96.3 ms: 1.85x faster                                                      |
| async_tree_none_tg               | 172 ms                                                   | 96.1 ms: 1.79x faster                                                      |
| async_tree_memoization_tg        | 231 ms                                                   | 134 ms: 1.72x faster                                                       |
| async_tree_memoization           | 223 ms                                                   | 136 ms: 1.64x faster                                                       |
| async_tree_cpu_io_mixed          | 333 ms                                                   | 242 ms: 1.38x faster                                                       |
| async_tree_cpu_io_mixed_tg       | 338 ms                                                   | 247 ms: 1.37x faster                                                       |
| coroutines                       | 13.6 ms                                                  | 10.2 ms: 1.34x faster                                                      |
| async_tree_eager_memoization     | 132 ms                                                   | 104 ms: 1.26x faster                                                       |
| async_tree_eager                 | 45.6 ms                                                  | 37.2 ms: 1.23x faster                                                      |
| async_generators                 | 206 ms                                                   | 178 ms: 1.16x faster                                                       |
| async_tree_eager_cpu_io_mixed    | 231 ms                                                   | 213 ms: 1.08x faster                                                       |
| async_tree_eager_memoization_tg  | 113 ms                                                   | 119 ms: 1.05x slower                                                       |
| async_tree_eager_cpu_io_mixed_tg | 213 ms                                                   | 237 ms: 1.11x slower                                                       |
| async_tree_eager_tg              | 32.1 ms                                                  | 77.4 ms: 2.41x slower                                                      |
| Geometric mean                   | (ref)                                                    | 1.38x faster                                                               |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260103-macm4pro-arm64-Fidget%2dSpinner-loop_peeling-3.15.0a3+-e650bf3 |
|----------------|:--------------------------------------------------------:|:--------------------------------------------------------------------------:|
| float          | 37.9 ms                                                  | 22.2 ms: 1.71x faster                                                      |
| nbody          | 54.2 ms                                                  | 39.9 ms: 1.36x faster                                                      |
| pidigits       | 161 ms                                                   | 159 ms: 1.01x faster                                                       |
| Geometric mean | (ref)                                                    | 1.33x faster                                                               |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260103-macm4pro-arm64-Fidget%2dSpinner-loop_peeling-3.15.0a3+-e650bf3 |
|----------------|:--------------------------------------------------------:|:--------------------------------------------------------------------------:|
| regex_compile  | 54.6 ms                                                  | 40.4 ms: 1.35x faster                                                      |
| regex_effbot   | 1.67 ms                                                  | 1.44 ms: 1.16x faster                                                      |
| regex_dna      | 99.6 ms                                                  | 94.6 ms: 1.05x faster                                                      |
| regex_v8       | 9.59 ms                                                  | 9.83 ms: 1.03x slower                                                      |
| Geometric mean | (ref)                                                    | 1.13x faster                                                               |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260103-macm4pro-arm64-Fidget%2dSpinner-loop_peeling-3.15.0a3+-e650bf3 |
|----------------------|:--------------------------------------------------------:|:--------------------------------------------------------------------------:|
| unpickle_pure_python | 103 us                                                   | 72.4 us: 1.42x faster                                                      |
| xml_etree_iterparse  | 51.6 ms                                                  | 36.4 ms: 1.42x faster                                                      |
| pickle_pure_python   | 139 us                                                   | 109 us: 1.28x faster                                                       |
| xml_etree_generate   | 38.9 ms                                                  | 31.0 ms: 1.25x faster                                                      |
| tomli_loads          | 957 ms                                                   | 764 ms: 1.25x faster                                                       |
| xml_etree_process    | 26.7 ms                                                  | 21.9 ms: 1.22x faster                                                      |
| json_dumps           | 4.26 ms                                                  | 3.58 ms: 1.19x faster                                                      |
| xml_etree_parse      | 67.9 ms                                                  | 60.0 ms: 1.13x faster                                                      |
| json_loads           | 10.9 us                                                  | 10.4 us: 1.04x faster                                                      |
| Geometric mean       | (ref)                                                    | 1.24x faster                                                               |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260103-macm4pro-arm64-Fidget%2dSpinner-loop_peeling-3.15.0a3+-e650bf3 |
|------------------------|:--------------------------------------------------------:|:--------------------------------------------------------------------------:|
| python_startup         | 8.01 ms                                                  | 8.76 ms: 1.09x slower                                                      |
| python_startup_no_site | 5.71 ms                                                  | 6.32 ms: 1.11x slower                                                      |
| Geometric mean         | (ref)                                                    | 1.10x slower                                                               |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260103-macm4pro-arm64-Fidget%2dSpinner-loop_peeling-3.15.0a3+-e650bf3 |
|-----------------|:--------------------------------------------------------:|:--------------------------------------------------------------------------:|
| mako            | 4.77 ms                                                  | 3.87 ms: 1.23x faster                                                      |
| genshi_text     | 10.5 ms                                                  | 8.95 ms: 1.17x faster                                                      |
| genshi_xml      | 21.8 ms                                                  | 21.2 ms: 1.03x faster                                                      |
| django_template | 13.6 ms                                                  | 13.8 ms: 1.01x slower                                                      |
| Geometric mean  | (ref)                                                    | 1.10x faster                                                               |

All benchmarks:
===============

| Benchmark                        | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260103-macm4pro-arm64-Fidget%2dSpinner-loop_peeling-3.15.0a3+-e650bf3 |
|----------------------------------|:--------------------------------------------------------:|:--------------------------------------------------------------------------:|
| subparsers                       | 20.8 ms                                                  | 3.81 ms: 5.45x faster                                                      |
| richards                         | 22.4 ms                                                  | 9.52 ms: 2.35x faster                                                      |
| async_tree_eager_io              | 496 ms                                                   | 219 ms: 2.27x faster                                                       |
| richards_super                   | 25.4 ms                                                  | 11.3 ms: 2.24x faster                                                      |
| async_tree_io_tg                 | 480 ms                                                   | 217 ms: 2.21x faster                                                       |
| async_tree_eager_io_tg           | 446 ms                                                   | 216 ms: 2.06x faster                                                       |
| async_tree_io                    | 459 ms                                                   | 225 ms: 2.04x faster                                                       |
| mdp                              | 1.09 sec                                                 | 577 ms: 1.89x faster                                                       |
| async_tree_none                  | 178 ms                                                   | 96.3 ms: 1.85x faster                                                      |
| async_tree_none_tg               | 172 ms                                                   | 96.1 ms: 1.79x faster                                                      |
| async_tree_memoization_tg        | 231 ms                                                   | 134 ms: 1.72x faster                                                       |
| float                            | 37.9 ms                                                  | 22.2 ms: 1.71x faster                                                      |
| async_tree_memoization           | 223 ms                                                   | 136 ms: 1.64x faster                                                       |
| deepcopy                         | 161 us                                                   | 98.6 us: 1.64x faster                                                      |
| deepcopy_memo                    | 18.3 us                                                  | 11.2 us: 1.64x faster                                                      |
| go                               | 70.0 ms                                                  | 42.8 ms: 1.64x faster                                                      |
| comprehensions                   | 9.84 us                                                  | 6.13 us: 1.60x faster                                                      |
| deltablue                        | 1.73 ms                                                  | 1.11 ms: 1.55x faster                                                      |
| pyflate                          | 216 ms                                                   | 142 ms: 1.52x faster                                                       |
| logging_silent                   | 50.9 ns                                                  | 33.7 ns: 1.51x faster                                                      |
| scimark_monte_carlo              | 32.2 ms                                                  | 21.9 ms: 1.47x faster                                                      |
| deepcopy_reduce                  | 1.46 us                                                  | 1.00 us: 1.46x faster                                                      |
| scimark_fft                      | 142 ms                                                   | 97.8 ms: 1.45x faster                                                      |
| scimark_sor                      | 61.0 ms                                                  | 42.2 ms: 1.45x faster                                                      |
| scimark_lu                       | 51.3 ms                                                  | 35.9 ms: 1.43x faster                                                      |
| unpickle_pure_python             | 103 us                                                   | 72.4 us: 1.42x faster                                                      |
| xml_etree_iterparse              | 51.6 ms                                                  | 36.4 ms: 1.42x faster                                                      |
| spectral_norm                    | 54.4 ms                                                  | 39.1 ms: 1.39x faster                                                      |
| async_tree_cpu_io_mixed          | 333 ms                                                   | 242 ms: 1.38x faster                                                       |
| raytrace                         | 145 ms                                                   | 106 ms: 1.37x faster                                                       |
| async_tree_cpu_io_mixed_tg       | 338 ms                                                   | 247 ms: 1.37x faster                                                       |
| nbody                            | 54.2 ms                                                  | 39.9 ms: 1.36x faster                                                      |
| regex_compile                    | 54.6 ms                                                  | 40.4 ms: 1.35x faster                                                      |
| chaos                            | 28.9 ms                                                  | 21.6 ms: 1.34x faster                                                      |
| coroutines                       | 13.6 ms                                                  | 10.2 ms: 1.34x faster                                                      |
| pprint_safe_repr                 | 328 ms                                                   | 251 ms: 1.31x faster                                                       |
| pprint_pformat                   | 665 ms                                                   | 509 ms: 1.31x faster                                                       |
| k_core                           | 1.12 sec                                                 | 864 ms: 1.29x faster                                                       |
| crypto_pyaes                     | 38.8 ms                                                  | 30.4 ms: 1.28x faster                                                      |
| pickle_pure_python               | 139 us                                                   | 109 us: 1.28x faster                                                       |
| async_tree_eager_memoization     | 132 ms                                                   | 104 ms: 1.26x faster                                                       |
| xml_etree_generate               | 38.9 ms                                                  | 31.0 ms: 1.25x faster                                                      |
| tomli_loads                      | 957 ms                                                   | 764 ms: 1.25x faster                                                       |
| bpe_tokeniser                    | 2.24 sec                                                 | 1.81 sec: 1.24x faster                                                     |
| mako                             | 4.77 ms                                                  | 3.87 ms: 1.23x faster                                                      |
| async_tree_eager                 | 45.6 ms                                                  | 37.2 ms: 1.23x faster                                                      |
| xml_etree_process                | 26.7 ms                                                  | 21.9 ms: 1.22x faster                                                      |
| logging_simple                   | 2.57 us                                                  | 2.12 us: 1.21x faster                                                      |
| logging_format                   | 2.80 us                                                  | 2.33 us: 1.20x faster                                                      |
| generators                       | 21.9 ms                                                  | 18.3 ms: 1.20x faster                                                      |
| json_dumps                       | 4.26 ms                                                  | 3.58 ms: 1.19x faster                                                      |
| genshi_text                      | 10.5 ms                                                  | 8.95 ms: 1.17x faster                                                      |
| fannkuch                         | 176 ms                                                   | 150 ms: 1.17x faster                                                       |
| html5lib                         | 23.0 ms                                                  | 19.7 ms: 1.17x faster                                                      |
| scimark_sparse_mat_mult          | 2.08 ms                                                  | 1.79 ms: 1.16x faster                                                      |
| hexiom                           | 3.04 ms                                                  | 2.62 ms: 1.16x faster                                                      |
| async_generators                 | 206 ms                                                   | 178 ms: 1.16x faster                                                       |
| regex_effbot                     | 1.67 ms                                                  | 1.44 ms: 1.16x faster                                                      |
| dulwich_log                      | 21.3 ms                                                  | 18.4 ms: 1.15x faster                                                      |
| pathlib                          | 12.4 ms                                                  | 10.8 ms: 1.14x faster                                                      |
| pycparser                        | 497 ms                                                   | 436 ms: 1.14x faster                                                       |
| typing_runtime_protocols         | 71.0 us                                                  | 62.3 us: 1.14x faster                                                      |
| nqueens                          | 43.5 ms                                                  | 38.4 ms: 1.13x faster                                                      |
| xml_etree_parse                  | 67.9 ms                                                  | 60.0 ms: 1.13x faster                                                      |
| sphinx                           | 434 ms                                                   | 390 ms: 1.11x faster                                                       |
| async_tree_eager_cpu_io_mixed    | 231 ms                                                   | 213 ms: 1.08x faster                                                       |
| json                             | 1.93 ms                                                  | 1.81 ms: 1.07x faster                                                      |
| 2to3                             | 114 ms                                                   | 107 ms: 1.06x faster                                                       |
| thrift                           | 322 us                                                   | 304 us: 1.06x faster                                                       |
| meteor_contest                   | 47.7 ms                                                  | 45.3 ms: 1.05x faster                                                      |
| regex_dna                        | 99.6 ms                                                  | 94.6 ms: 1.05x faster                                                      |
| pylint                           | 128 ms                                                   | 122 ms: 1.05x faster                                                       |
| sympy_integrate                  | 8.02 ms                                                  | 7.65 ms: 1.05x faster                                                      |
| json_loads                       | 10.9 us                                                  | 10.4 us: 1.04x faster                                                      |
| genshi_xml                       | 21.8 ms                                                  | 21.2 ms: 1.03x faster                                                      |
| docutils                         | 1.02 sec                                                 | 1000 ms: 1.02x faster                                                      |
| telco                            | 2.61 ms                                                  | 2.55 ms: 1.02x faster                                                      |
| sqlite_synth                     | 967 ns                                                   | 955 ns: 1.01x faster                                                       |
| pidigits                         | 161 ms                                                   | 159 ms: 1.01x faster                                                       |
| connected_components             | 201 ms                                                   | 199 ms: 1.01x faster                                                       |
| shortest_path                    | 219 ms                                                   | 217 ms: 1.01x faster                                                       |
| django_template                  | 13.6 ms                                                  | 13.8 ms: 1.01x slower                                                      |
| bench_thread_pool                | 419 us                                                   | 428 us: 1.02x slower                                                       |
| regex_v8                         | 9.59 ms                                                  | 9.83 ms: 1.03x slower                                                      |
| sympy_str                        | 104 ms                                                   | 107 ms: 1.03x slower                                                       |
| async_tree_eager_memoization_tg  | 113 ms                                                   | 119 ms: 1.05x slower                                                       |
| sympy_expand                     | 167 ms                                                   | 176 ms: 1.06x slower                                                       |
| create_gc_cycles                 | 830 us                                                   | 901 us: 1.09x slower                                                       |
| python_startup                   | 8.01 ms                                                  | 8.76 ms: 1.09x slower                                                      |
| python_startup_no_site           | 5.71 ms                                                  | 6.32 ms: 1.11x slower                                                      |
| bench_mp_pool                    | 39.7 ms                                                  | 44.2 ms: 1.11x slower                                                      |
| async_tree_eager_cpu_io_mixed_tg | 213 ms                                                   | 237 ms: 1.11x slower                                                       |
| coverage                         | 26.9 ms                                                  | 32.4 ms: 1.21x slower                                                      |
| many_optionals                   | 195 us                                                   | 244 us: 1.25x slower                                                       |
| async_tree_eager_tg              | 32.1 ms                                                  | 77.4 ms: 2.41x slower                                                      |
| Geometric mean                   | (ref)                                                    | 1.26x faster                                                               |

Benchmark hidden because not significant (2): sympy_sum, gc_traversal
Ignored benchmarks (12) of results/bm-20240906-3.12.6-a4a2d2b/bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b.json: asyncio_websockets, chameleon, dask, djangocms, gevent_hub, sqlalchemy_declarative, sqlalchemy_imperative, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http
Ignored benchmarks (4) of results/bm-20260103-3.15.0a3+-e650bf3-JIT/bm-20260103-macm4pro-arm64-Fidget%2dSpinner-loop_peeling-3.15.0a3+-e650bf3.json: sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile

- Geometric mean (including insignificant results): 1.261x faster

# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.18x
- 95% likely to have a speedup of 1.17x
- 99% likely to have a speedup of 1.15x

# Memory
- memory change: 1.26x