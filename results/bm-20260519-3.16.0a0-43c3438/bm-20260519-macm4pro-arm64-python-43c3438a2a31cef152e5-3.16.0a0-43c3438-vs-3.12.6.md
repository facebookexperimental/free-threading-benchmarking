# Results vs. 3.12.6

- fork: python
- ref: 43c3438a2a31cef152e5
- machine: darwin-arm64
- commit hash: 43c3438
- commit date: 2026-05-19
- overall geometric mean: 1.118x faster
- HPT reliability: 100.00%
- HPT 99th percentile: 1.04x faster
- Memory change: 1.21x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260519-macm4pro-arm64-python-43c3438a2a31cef152e5-3.16.0a0-43c3438 |
|----------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| docutils       | 1.02 sec                                                 | 979 ms: 1.04x faster                                                    |
| html5lib       | 23.0 ms                                                  | 22.5 ms: 1.02x faster                                                   |
| sphinx         | 434 ms                                                   | 411 ms: 1.06x faster                                                    |
| Geometric mean | (ref)                                                    | 1.04x faster                                                            |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                        | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260519-macm4pro-arm64-python-43c3438a2a31cef152e5-3.16.0a0-43c3438 |
|----------------------------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| async_tree_eager_io              | 496 ms                                                   | 310 ms: 1.60x faster                                                    |
| async_tree_io                    | 459 ms                                                   | 311 ms: 1.48x faster                                                    |
| async_tree_none                  | 178 ms                                                   | 121 ms: 1.47x faster                                                    |
| async_tree_io_tg                 | 480 ms                                                   | 330 ms: 1.45x faster                                                    |
| async_generators                 | 206 ms                                                   | 147 ms: 1.41x faster                                                    |
| async_tree_none_tg               | 172 ms                                                   | 126 ms: 1.37x faster                                                    |
| async_tree_memoization_tg        | 231 ms                                                   | 172 ms: 1.34x faster                                                    |
| async_tree_eager_io_tg           | 446 ms                                                   | 338 ms: 1.32x faster                                                    |
| coroutines                       | 13.6 ms                                                  | 10.4 ms: 1.30x faster                                                   |
| async_tree_memoization           | 223 ms                                                   | 176 ms: 1.26x faster                                                    |
| async_tree_cpu_io_mixed          | 333 ms                                                   | 279 ms: 1.19x faster                                                    |
| async_tree_cpu_io_mixed_tg       | 338 ms                                                   | 290 ms: 1.17x faster                                                    |
| async_tree_eager_memoization     | 132 ms                                                   | 115 ms: 1.15x faster                                                    |
| async_tree_eager                 | 45.6 ms                                                  | 40.8 ms: 1.12x faster                                                   |
| async_tree_eager_cpu_io_mixed    | 231 ms                                                   | 223 ms: 1.03x faster                                                    |
| asyncio_websockets               | 190 ms                                                   | 185 ms: 1.03x faster                                                    |
| async_tree_eager_cpu_io_mixed_tg | 213 ms                                                   | 260 ms: 1.22x slower                                                    |
| async_tree_eager_memoization_tg  | 113 ms                                                   | 152 ms: 1.35x slower                                                    |
| async_tree_eager_tg              | 32.1 ms                                                  | 103 ms: 3.20x slower                                                    |
| Geometric mean                   | (ref)                                                    | 1.13x faster                                                            |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260519-macm4pro-arm64-python-43c3438a2a31cef152e5-3.16.0a0-43c3438 |
|----------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| float          | 37.9 ms                                                  | 30.8 ms: 1.23x faster                                                   |
| nbody          | 54.2 ms                                                  | 45.3 ms: 1.20x faster                                                   |
| pidigits       | 161 ms                                                   | 160 ms: 1.01x faster                                                    |
| Geometric mean | (ref)                                                    | 1.14x faster                                                            |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260519-macm4pro-arm64-python-43c3438a2a31cef152e5-3.16.0a0-43c3438 |
|----------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| regex_effbot   | 1.67 ms                                                  | 1.43 ms: 1.17x faster                                                   |
| regex_compile  | 54.6 ms                                                  | 47.2 ms: 1.16x faster                                                   |
| regex_dna      | 99.6 ms                                                  | 92.6 ms: 1.08x faster                                                   |
| Geometric mean | (ref)                                                    | 1.10x faster                                                            |

Benchmark hidden because not significant (1): regex_v8

Benchmarks with tag 'serialize':
================================

| Benchmark           | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260519-macm4pro-arm64-python-43c3438a2a31cef152e5-3.16.0a0-43c3438 |
|---------------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| xml_etree_iterparse | 51.6 ms                                                  | 44.0 ms: 1.17x faster                                                   |
| tomli_loads         | 957 ms                                                   | 831 ms: 1.15x faster                                                    |
| json_dumps          | 4.26 ms                                                  | 3.78 ms: 1.13x faster                                                   |
| xml_etree_generate  | 38.9 ms                                                  | 37.4 ms: 1.04x faster                                                   |
| json_loads          | 10.9 us                                                  | 10.6 us: 1.03x faster                                                   |
| xml_etree_process   | 26.7 ms                                                  | 26.2 ms: 1.02x faster                                                   |
| pickle_pure_python  | 139 us                                                   | 145 us: 1.04x slower                                                    |
| xml_etree_parse     | 67.9 ms                                                  | 70.7 ms: 1.04x slower                                                   |
| Geometric mean      | (ref)                                                    | 1.05x faster                                                            |

Benchmark hidden because not significant (1): unpickle_pure_python

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260519-macm4pro-arm64-python-43c3438a2a31cef152e5-3.16.0a0-43c3438 |
|-----------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| mako            | 4.77 ms                                                  | 4.86 ms: 1.02x slower                                                   |
| django_template | 13.6 ms                                                  | 15.2 ms: 1.12x slower                                                   |
| Geometric mean  | (ref)                                                    | 1.07x slower                                                            |

All benchmarks:
===============

| Benchmark                        | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260519-macm4pro-arm64-python-43c3438a2a31cef152e5-3.16.0a0-43c3438 |
|----------------------------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| subparsers                       | 20.8 ms                                                  | 4.23 ms: 4.91x faster                                                   |
| pylint                           | 128 ms                                                   | 56.9 ms: 2.25x faster                                                   |
| mdp                              | 1.09 sec                                                 | 522 ms: 2.09x faster                                                    |
| async_tree_eager_io              | 496 ms                                                   | 310 ms: 1.60x faster                                                    |
| deepcopy                         | 161 us                                                   | 101 us: 1.60x faster                                                    |
| deepcopy_memo                    | 18.3 us                                                  | 12.4 us: 1.48x faster                                                   |
| async_tree_io                    | 459 ms                                                   | 311 ms: 1.48x faster                                                    |
| async_tree_none                  | 178 ms                                                   | 121 ms: 1.47x faster                                                    |
| async_tree_io_tg                 | 480 ms                                                   | 330 ms: 1.45x faster                                                    |
| typing_runtime_protocols         | 71.0 us                                                  | 49.9 us: 1.42x faster                                                   |
| async_generators                 | 206 ms                                                   | 147 ms: 1.41x faster                                                    |
| async_tree_none_tg               | 172 ms                                                   | 126 ms: 1.37x faster                                                    |
| comprehensions                   | 9.84 us                                                  | 7.35 us: 1.34x faster                                                   |
| async_tree_memoization_tg        | 231 ms                                                   | 172 ms: 1.34x faster                                                    |
| async_tree_eager_io_tg           | 446 ms                                                   | 338 ms: 1.32x faster                                                    |
| coroutines                       | 13.6 ms                                                  | 10.4 ms: 1.30x faster                                                   |
| deepcopy_reduce                  | 1.46 us                                                  | 1.13 us: 1.29x faster                                                   |
| go                               | 70.0 ms                                                  | 54.1 ms: 1.29x faster                                                   |
| async_tree_memoization           | 223 ms                                                   | 176 ms: 1.26x faster                                                    |
| spectral_norm                    | 54.4 ms                                                  | 44.0 ms: 1.23x faster                                                   |
| float                            | 37.9 ms                                                  | 30.8 ms: 1.23x faster                                                   |
| raytrace                         | 145 ms                                                   | 118 ms: 1.22x faster                                                    |
| logging_silent                   | 50.9 ns                                                  | 41.7 ns: 1.22x faster                                                   |
| nbody                            | 54.2 ms                                                  | 45.3 ms: 1.20x faster                                                   |
| scimark_sor                      | 61.0 ms                                                  | 51.0 ms: 1.20x faster                                                   |
| async_tree_cpu_io_mixed          | 333 ms                                                   | 279 ms: 1.19x faster                                                    |
| scimark_fft                      | 142 ms                                                   | 120 ms: 1.18x faster                                                    |
| xml_etree_iterparse              | 51.6 ms                                                  | 44.0 ms: 1.17x faster                                                   |
| async_tree_cpu_io_mixed_tg       | 338 ms                                                   | 290 ms: 1.17x faster                                                    |
| regex_effbot                     | 1.67 ms                                                  | 1.43 ms: 1.17x faster                                                   |
| regex_compile                    | 54.6 ms                                                  | 47.2 ms: 1.16x faster                                                   |
| tomli_loads                      | 957 ms                                                   | 831 ms: 1.15x faster                                                    |
| pathlib                          | 12.4 ms                                                  | 10.7 ms: 1.15x faster                                                   |
| async_tree_eager_memoization     | 132 ms                                                   | 115 ms: 1.15x faster                                                    |
| chaos                            | 28.9 ms                                                  | 25.3 ms: 1.14x faster                                                   |
| deltablue                        | 1.73 ms                                                  | 1.52 ms: 1.14x faster                                                   |
| bpe_tokeniser                    | 2.24 sec                                                 | 1.97 sec: 1.13x faster                                                  |
| json_dumps                       | 4.26 ms                                                  | 3.78 ms: 1.13x faster                                                   |
| pyflate                          | 216 ms                                                   | 192 ms: 1.12x faster                                                    |
| dulwich_log                      | 21.3 ms                                                  | 19.0 ms: 1.12x faster                                                   |
| logging_simple                   | 2.57 us                                                  | 2.30 us: 1.12x faster                                                   |
| logging_format                   | 2.80 us                                                  | 2.51 us: 1.12x faster                                                   |
| async_tree_eager                 | 45.6 ms                                                  | 40.8 ms: 1.12x faster                                                   |
| scimark_monte_carlo              | 32.2 ms                                                  | 29.3 ms: 1.10x faster                                                   |
| generators                       | 21.9 ms                                                  | 20.0 ms: 1.10x faster                                                   |
| k_core                           | 1.12 sec                                                 | 1.02 sec: 1.10x faster                                                  |
| hexiom                           | 3.04 ms                                                  | 2.80 ms: 1.09x faster                                                   |
| regex_dna                        | 99.6 ms                                                  | 92.6 ms: 1.08x faster                                                   |
| scimark_sparse_mat_mult          | 2.08 ms                                                  | 1.94 ms: 1.07x faster                                                   |
| sympy_integrate                  | 8.02 ms                                                  | 7.54 ms: 1.06x faster                                                   |
| sphinx                           | 434 ms                                                   | 411 ms: 1.06x faster                                                    |
| richards_super                   | 25.4 ms                                                  | 24.1 ms: 1.06x faster                                                   |
| richards                         | 22.4 ms                                                  | 21.2 ms: 1.05x faster                                                   |
| docutils                         | 1.02 sec                                                 | 979 ms: 1.04x faster                                                    |
| pprint_safe_repr                 | 328 ms                                                   | 315 ms: 1.04x faster                                                    |
| gc_traversal                     | 2.01 ms                                                  | 1.93 ms: 1.04x faster                                                   |
| xml_etree_generate               | 38.9 ms                                                  | 37.4 ms: 1.04x faster                                                   |
| pprint_pformat                   | 665 ms                                                   | 642 ms: 1.04x faster                                                    |
| async_tree_eager_cpu_io_mixed    | 231 ms                                                   | 223 ms: 1.03x faster                                                    |
| asyncio_websockets               | 190 ms                                                   | 185 ms: 1.03x faster                                                    |
| fannkuch                         | 176 ms                                                   | 171 ms: 1.03x faster                                                    |
| sqlite_synth                     | 967 ns                                                   | 941 ns: 1.03x faster                                                    |
| json_loads                       | 10.9 us                                                  | 10.6 us: 1.03x faster                                                   |
| sympy_str                        | 104 ms                                                   | 102 ms: 1.02x faster                                                    |
| html5lib                         | 23.0 ms                                                  | 22.5 ms: 1.02x faster                                                   |
| xml_etree_process                | 26.7 ms                                                  | 26.2 ms: 1.02x faster                                                   |
| sympy_sum                        | 57.6 ms                                                  | 56.5 ms: 1.02x faster                                                   |
| pycparser                        | 497 ms                                                   | 489 ms: 1.02x faster                                                    |
| json                             | 1.93 ms                                                  | 1.90 ms: 1.02x faster                                                   |
| pidigits                         | 161 ms                                                   | 160 ms: 1.01x faster                                                    |
| dask                             | 528 ms                                                   | 523 ms: 1.01x faster                                                    |
| scimark_lu                       | 51.3 ms                                                  | 51.7 ms: 1.01x slower                                                   |
| mako                             | 4.77 ms                                                  | 4.86 ms: 1.02x slower                                                   |
| sympy_expand                     | 167 ms                                                   | 172 ms: 1.03x slower                                                    |
| pickle_pure_python               | 139 us                                                   | 145 us: 1.04x slower                                                    |
| meteor_contest                   | 47.7 ms                                                  | 49.6 ms: 1.04x slower                                                   |
| bench_thread_pool                | 419 us                                                   | 435 us: 1.04x slower                                                    |
| shortest_path                    | 219 ms                                                   | 228 ms: 1.04x slower                                                    |
| xml_etree_parse                  | 67.9 ms                                                  | 70.7 ms: 1.04x slower                                                   |
| create_gc_cycles                 | 830 us                                                   | 869 us: 1.05x slower                                                    |
| connected_components             | 201 ms                                                   | 211 ms: 1.05x slower                                                    |
| crypto_pyaes                     | 38.8 ms                                                  | 41.0 ms: 1.06x slower                                                   |
| telco                            | 2.61 ms                                                  | 2.84 ms: 1.09x slower                                                   |
| django_template                  | 13.6 ms                                                  | 15.2 ms: 1.12x slower                                                   |
| bench_mp_pool                    | 39.7 ms                                                  | 46.6 ms: 1.17x slower                                                   |
| async_tree_eager_cpu_io_mixed_tg | 213 ms                                                   | 260 ms: 1.22x slower                                                    |
| coverage                         | 26.9 ms                                                  | 33.1 ms: 1.23x slower                                                   |
| many_optionals                   | 195 us                                                   | 248 us: 1.27x slower                                                    |
| nqueens                          | 43.5 ms                                                  | 58.1 ms: 1.34x slower                                                   |
| async_tree_eager_memoization_tg  | 113 ms                                                   | 152 ms: 1.35x slower                                                    |
| async_tree_eager_tg              | 32.1 ms                                                  | 103 ms: 3.20x slower                                                    |
| Geometric mean                   | (ref)                                                    | 1.11x faster                                                            |

Benchmark hidden because not significant (3): regex_v8, unpickle_pure_python, thrift
Ignored benchmarks (15) of results/bm-20240906-3.12.6-a4a2d2b/bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b.json: 2to3, chameleon, djangocms, genshi_text, genshi_xml, gevent_hub, python_startup, python_startup_no_site, sqlalchemy_declarative, sqlalchemy_imperative, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http
Ignored benchmarks (4) of results/bm-20260519-3.16.0a0-43c3438/bm-20260519-macm4pro-arm64-python-43c3438a2a31cef152e5-3.16.0a0-43c3438.json: sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile

- Geometric mean (including insignificant results): 1.118x faster

# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.06x
- 95% likely to have a speedup of 1.05x
- 99% likely to have a speedup of 1.04x

# Memory
- memory change: 1.21x