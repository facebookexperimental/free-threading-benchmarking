# Results vs. 3.12.6

- fork: python
- ref: c1df6843d36233ec1da7
- machine: darwin-arm64
- commit hash: c1df684
- commit date: 2026-09-19
- overall geometric mean: 1.003x slower
- HPT reliability: 87.22%
- HPT 99th percentile: 1.00x slower
- Memory change: 1.30x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260919-macm4pro-arm64-python-c1df6843d36233ec1da7-3.16.0a0-c1df684 |
|----------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| 2to3           | 114 ms                                                   | 138 ms: 1.21x slower                                                    |
| docutils       | 1.02 sec                                                 | 1.10 sec: 1.08x slower                                                  |
| html5lib       | 23.0 ms                                                  | 24.3 ms: 1.05x slower                                                   |
| sphinx         | 434 ms                                                   | 461 ms: 1.06x slower                                                    |
| Geometric mean | (ref)                                                    | 1.10x slower                                                            |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                        | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260919-macm4pro-arm64-python-c1df6843d36233ec1da7-3.16.0a0-c1df684 |
|----------------------------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| async_tree_eager_io              | 496 ms                                                   | 320 ms: 1.55x faster                                                    |
| async_tree_io_tg                 | 480 ms                                                   | 313 ms: 1.53x faster                                                    |
| async_tree_eager_io_tg           | 446 ms                                                   | 310 ms: 1.44x faster                                                    |
| async_tree_io                    | 459 ms                                                   | 324 ms: 1.42x faster                                                    |
| async_tree_memoization_tg        | 231 ms                                                   | 187 ms: 1.23x faster                                                    |
| async_generators                 | 206 ms                                                   | 168 ms: 1.23x faster                                                    |
| async_tree_none_tg               | 172 ms                                                   | 148 ms: 1.16x faster                                                    |
| async_tree_cpu_io_mixed_tg       | 338 ms                                                   | 303 ms: 1.12x faster                                                    |
| async_tree_memoization           | 223 ms                                                   | 205 ms: 1.09x faster                                                    |
| async_tree_none                  | 178 ms                                                   | 165 ms: 1.08x faster                                                    |
| async_tree_cpu_io_mixed          | 333 ms                                                   | 314 ms: 1.06x faster                                                    |
| coroutines                       | 13.6 ms                                                  | 13.0 ms: 1.04x faster                                                   |
| asyncio_websockets               | 190 ms                                                   | 186 ms: 1.02x faster                                                    |
| async_tree_eager_cpu_io_mixed    | 231 ms                                                   | 263 ms: 1.14x slower                                                    |
| async_tree_eager_memoization     | 132 ms                                                   | 152 ms: 1.15x slower                                                    |
| async_tree_eager_cpu_io_mixed_tg | 213 ms                                                   | 281 ms: 1.32x slower                                                    |
| async_tree_eager_memoization_tg  | 113 ms                                                   | 169 ms: 1.50x slower                                                    |
| async_tree_eager                 | 45.6 ms                                                  | 71.7 ms: 1.57x slower                                                   |
| async_tree_eager_tg              | 32.1 ms                                                  | 121 ms: 3.75x slower                                                    |
| Geometric mean                   | (ref)                                                    | 1.01x slower                                                            |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260919-macm4pro-arm64-python-c1df6843d36233ec1da7-3.16.0a0-c1df684 |
|----------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| float          | 37.9 ms                                                  | 35.3 ms: 1.07x faster                                                   |
| pidigits       | 161 ms                                                   | 169 ms: 1.05x slower                                                    |
| nbody          | 54.2 ms                                                  | 62.7 ms: 1.16x slower                                                   |
| Geometric mean | (ref)                                                    | 1.04x slower                                                            |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260919-macm4pro-arm64-python-c1df6843d36233ec1da7-3.16.0a0-c1df684 |
|----------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| regex_effbot   | 1.67 ms                                                  | 1.34 ms: 1.25x faster                                                   |
| regex_dna      | 99.6 ms                                                  | 93.9 ms: 1.06x faster                                                   |
| regex_v8       | 9.59 ms                                                  | 9.24 ms: 1.04x faster                                                   |
| regex_compile  | 54.6 ms                                                  | 69.2 ms: 1.27x slower                                                   |
| Geometric mean | (ref)                                                    | 1.02x faster                                                            |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260919-macm4pro-arm64-python-c1df6843d36233ec1da7-3.16.0a0-c1df684 |
|----------------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| xml_etree_iterparse  | 51.6 ms                                                  | 41.9 ms: 1.23x faster                                                   |
| xml_etree_parse      | 67.9 ms                                                  | 59.8 ms: 1.14x faster                                                   |
| json_dumps           | 4.26 ms                                                  | 3.76 ms: 1.13x faster                                                   |
| xml_etree_generate   | 38.9 ms                                                  | 40.2 ms: 1.03x slower                                                   |
| json_loads           | 10.9 us                                                  | 11.3 us: 1.04x slower                                                   |
| unpickle_pure_python | 103 us                                                   | 121 us: 1.18x slower                                                    |
| xml_etree_process    | 26.7 ms                                                  | 31.7 ms: 1.19x slower                                                   |
| pickle_pure_python   | 139 us                                                   | 166 us: 1.19x slower                                                    |
| Geometric mean       | (ref)                                                    | 1.01x slower                                                            |

Benchmark hidden because not significant (1): tomli_loads

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260919-macm4pro-arm64-python-c1df6843d36233ec1da7-3.16.0a0-c1df684 |
|------------------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| python_startup         | 8.01 ms                                                  | 10.4 ms: 1.30x slower                                                   |
| python_startup_no_site | 5.71 ms                                                  | 7.54 ms: 1.32x slower                                                   |
| Geometric mean         | (ref)                                                    | 1.31x slower                                                            |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260919-macm4pro-arm64-python-c1df6843d36233ec1da7-3.16.0a0-c1df684 |
|-----------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| mako            | 4.77 ms                                                  | 6.07 ms: 1.27x slower                                                   |
| django_template | 13.6 ms                                                  | 18.2 ms: 1.33x slower                                                   |
| Geometric mean  | (ref)                                                    | 1.30x slower                                                            |

All benchmarks:
===============

| Benchmark                        | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260919-macm4pro-arm64-python-c1df6843d36233ec1da7-3.16.0a0-c1df684 |
|----------------------------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| subparsers                       | 20.8 ms                                                  | 4.69 ms: 4.42x faster                                                   |
| gc_traversal                     | 2.01 ms                                                  | 801 us: 2.51x faster                                                    |
| pylint                           | 128 ms                                                   | 54.3 ms: 2.36x faster                                                   |
| mdp                              | 1.09 sec                                                 | 637 ms: 1.71x faster                                                    |
| create_gc_cycles                 | 830 us                                                   | 497 us: 1.67x faster                                                    |
| async_tree_eager_io              | 496 ms                                                   | 320 ms: 1.55x faster                                                    |
| async_tree_io_tg                 | 480 ms                                                   | 313 ms: 1.53x faster                                                    |
| async_tree_eager_io_tg           | 446 ms                                                   | 310 ms: 1.44x faster                                                    |
| async_tree_io                    | 459 ms                                                   | 324 ms: 1.42x faster                                                    |
| deepcopy                         | 161 us                                                   | 119 us: 1.36x faster                                                    |
| regex_effbot                     | 1.67 ms                                                  | 1.34 ms: 1.25x faster                                                   |
| xml_etree_iterparse              | 51.6 ms                                                  | 41.9 ms: 1.23x faster                                                   |
| async_tree_memoization_tg        | 231 ms                                                   | 187 ms: 1.23x faster                                                    |
| async_generators                 | 206 ms                                                   | 168 ms: 1.23x faster                                                    |
| sqlite_synth                     | 967 ns                                                   | 815 ns: 1.19x faster                                                    |
| async_tree_none_tg               | 172 ms                                                   | 148 ms: 1.16x faster                                                    |
| typing_runtime_protocols         | 71.0 us                                                  | 61.7 us: 1.15x faster                                                   |
| deepcopy_reduce                  | 1.46 us                                                  | 1.27 us: 1.15x faster                                                   |
| xml_etree_parse                  | 67.9 ms                                                  | 59.8 ms: 1.14x faster                                                   |
| json_dumps                       | 4.26 ms                                                  | 3.76 ms: 1.13x faster                                                   |
| deepcopy_memo                    | 18.3 us                                                  | 16.2 us: 1.13x faster                                                   |
| bpe_tokeniser                    | 2.24 sec                                                 | 2.00 sec: 1.12x faster                                                  |
| async_tree_cpu_io_mixed_tg       | 338 ms                                                   | 303 ms: 1.12x faster                                                    |
| pathlib                          | 12.4 ms                                                  | 11.1 ms: 1.11x faster                                                   |
| comprehensions                   | 9.84 us                                                  | 8.90 us: 1.11x faster                                                   |
| k_core                           | 1.12 sec                                                 | 1.01 sec: 1.10x faster                                                  |
| async_tree_memoization           | 223 ms                                                   | 205 ms: 1.09x faster                                                    |
| async_tree_none                  | 178 ms                                                   | 165 ms: 1.08x faster                                                    |
| float                            | 37.9 ms                                                  | 35.3 ms: 1.07x faster                                                   |
| async_tree_cpu_io_mixed          | 333 ms                                                   | 314 ms: 1.06x faster                                                    |
| dulwich_log                      | 21.3 ms                                                  | 20.0 ms: 1.06x faster                                                   |
| regex_dna                        | 99.6 ms                                                  | 93.9 ms: 1.06x faster                                                   |
| spectral_norm                    | 54.4 ms                                                  | 51.3 ms: 1.06x faster                                                   |
| coroutines                       | 13.6 ms                                                  | 13.0 ms: 1.04x faster                                                   |
| regex_v8                         | 9.59 ms                                                  | 9.24 ms: 1.04x faster                                                   |
| go                               | 70.0 ms                                                  | 67.6 ms: 1.04x faster                                                   |
| scimark_fft                      | 142 ms                                                   | 138 ms: 1.03x faster                                                    |
| asyncio_websockets               | 190 ms                                                   | 186 ms: 1.02x faster                                                    |
| pyflate                          | 216 ms                                                   | 212 ms: 1.02x faster                                                    |
| pycparser                        | 497 ms                                                   | 505 ms: 1.02x slower                                                    |
| raytrace                         | 145 ms                                                   | 148 ms: 1.02x slower                                                    |
| scimark_sor                      | 61.0 ms                                                  | 62.4 ms: 1.02x slower                                                   |
| xml_etree_generate               | 38.9 ms                                                  | 40.2 ms: 1.03x slower                                                   |
| nqueens                          | 43.5 ms                                                  | 45.1 ms: 1.04x slower                                                   |
| json_loads                       | 10.9 us                                                  | 11.3 us: 1.04x slower                                                   |
| logging_simple                   | 2.57 us                                                  | 2.69 us: 1.05x slower                                                   |
| pidigits                         | 161 ms                                                   | 169 ms: 1.05x slower                                                    |
| json                             | 1.93 ms                                                  | 2.03 ms: 1.05x slower                                                   |
| html5lib                         | 23.0 ms                                                  | 24.3 ms: 1.05x slower                                                   |
| logging_format                   | 2.80 us                                                  | 2.96 us: 1.06x slower                                                   |
| fannkuch                         | 176 ms                                                   | 186 ms: 1.06x slower                                                    |
| chaos                            | 28.9 ms                                                  | 30.7 ms: 1.06x slower                                                   |
| sphinx                           | 434 ms                                                   | 461 ms: 1.06x slower                                                    |
| sympy_integrate                  | 8.02 ms                                                  | 8.56 ms: 1.07x slower                                                   |
| scimark_sparse_mat_mult          | 2.08 ms                                                  | 2.22 ms: 1.07x slower                                                   |
| docutils                         | 1.02 sec                                                 | 1.10 sec: 1.08x slower                                                  |
| crypto_pyaes                     | 38.8 ms                                                  | 42.1 ms: 1.08x slower                                                   |
| sympy_sum                        | 57.6 ms                                                  | 63.5 ms: 1.10x slower                                                   |
| sympy_str                        | 104 ms                                                   | 116 ms: 1.11x slower                                                    |
| scimark_monte_carlo              | 32.2 ms                                                  | 36.2 ms: 1.12x slower                                                   |
| thrift                           | 322 us                                                   | 362 us: 1.12x slower                                                    |
| bench_mp_pool                    | 39.7 ms                                                  | 44.8 ms: 1.13x slower                                                   |
| async_tree_eager_cpu_io_mixed    | 231 ms                                                   | 263 ms: 1.14x slower                                                    |
| async_tree_eager_memoization     | 132 ms                                                   | 152 ms: 1.15x slower                                                    |
| nbody                            | 54.2 ms                                                  | 62.7 ms: 1.16x slower                                                   |
| shortest_path                    | 219 ms                                                   | 255 ms: 1.16x slower                                                    |
| meteor_contest                   | 47.7 ms                                                  | 55.8 ms: 1.17x slower                                                   |
| generators                       | 21.9 ms                                                  | 25.7 ms: 1.17x slower                                                   |
| unpickle_pure_python             | 103 us                                                   | 121 us: 1.18x slower                                                    |
| pprint_safe_repr                 | 328 ms                                                   | 386 ms: 1.18x slower                                                    |
| deltablue                        | 1.73 ms                                                  | 2.04 ms: 1.18x slower                                                   |
| sympy_expand                     | 167 ms                                                   | 198 ms: 1.18x slower                                                    |
| xml_etree_process                | 26.7 ms                                                  | 31.7 ms: 1.19x slower                                                   |
| pickle_pure_python               | 139 us                                                   | 166 us: 1.19x slower                                                    |
| hexiom                           | 3.04 ms                                                  | 3.63 ms: 1.19x slower                                                   |
| telco                            | 2.61 ms                                                  | 3.13 ms: 1.20x slower                                                   |
| pprint_pformat                   | 665 ms                                                   | 800 ms: 1.20x slower                                                    |
| richards                         | 22.4 ms                                                  | 27.0 ms: 1.20x slower                                                   |
| richards_super                   | 25.4 ms                                                  | 30.7 ms: 1.21x slower                                                   |
| 2to3                             | 114 ms                                                   | 138 ms: 1.21x slower                                                    |
| connected_components             | 201 ms                                                   | 245 ms: 1.22x slower                                                    |
| regex_compile                    | 54.6 ms                                                  | 69.2 ms: 1.27x slower                                                   |
| mako                             | 4.77 ms                                                  | 6.07 ms: 1.27x slower                                                   |
| python_startup                   | 8.01 ms                                                  | 10.4 ms: 1.30x slower                                                   |
| python_startup_no_site           | 5.71 ms                                                  | 7.54 ms: 1.32x slower                                                   |
| async_tree_eager_cpu_io_mixed_tg | 213 ms                                                   | 281 ms: 1.32x slower                                                    |
| django_template                  | 13.6 ms                                                  | 18.2 ms: 1.33x slower                                                   |
| bench_thread_pool                | 419 us                                                   | 565 us: 1.35x slower                                                    |
| scimark_lu                       | 51.3 ms                                                  | 71.9 ms: 1.40x slower                                                   |
| many_optionals                   | 195 us                                                   | 275 us: 1.41x slower                                                    |
| async_tree_eager_memoization_tg  | 113 ms                                                   | 169 ms: 1.50x slower                                                    |
| coverage                         | 26.9 ms                                                  | 41.3 ms: 1.54x slower                                                   |
| async_tree_eager                 | 45.6 ms                                                  | 71.7 ms: 1.57x slower                                                   |
| async_tree_eager_tg              | 32.1 ms                                                  | 121 ms: 3.75x slower                                                    |
| Geometric mean                   | (ref)                                                    | 1.01x slower                                                            |

Benchmark hidden because not significant (2): logging_silent, tomli_loads
Ignored benchmarks (13) of results/bm-20240906-3.12.6-a4a2d2b/bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b.json: chameleon, dask, djangocms, genshi_text, genshi_xml, gevent_hub, sqlalchemy_declarative, sqlalchemy_imperative, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http
Ignored benchmarks (12) of results/bm-20260919-3.16.0a0-c1df684-NOGIL/bm-20260919-macm4pro-arm64-python-c1df6843d36233ec1da7-3.16.0a0-c1df684.json: asyncio_tcp, asyncio_tcp_ssl, pickle, pickle_dict, pickle_list, sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile, unpack_sequence, unpickle, unpickle_list

- Geometric mean (including insignificant results): 1.003x slower

# HPT report

- Reliability score: 87.22% likely to be slow
- 90% likely to have a slowdown of 1.00x
- 95% likely to have a slowdown of 1.00x
- 99% likely to have a slowdown of 1.00x

# Memory
- memory change: 1.30x