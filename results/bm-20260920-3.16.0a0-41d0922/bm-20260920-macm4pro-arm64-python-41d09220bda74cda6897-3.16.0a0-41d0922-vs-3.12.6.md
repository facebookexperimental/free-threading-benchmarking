# Results vs. 3.12.6

- fork: python
- ref: 41d09220bda74cda6897
- machine: darwin-arm64
- commit hash: 41d0922
- commit date: 2026-09-20
- overall geometric mean: 1.141x faster
- HPT reliability: 100.00%
- HPT 99th percentile: 1.06x faster
- Memory change: 1.22x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260920-macm4pro-arm64-python-41d09220bda74cda6897-3.16.0a0-41d0922 |
|----------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| 2to3           | 114 ms                                                   | 118 ms: 1.03x slower                                                    |
| docutils       | 1.02 sec                                                 | 931 ms: 1.10x faster                                                    |
| html5lib       | 23.0 ms                                                  | 21.3 ms: 1.08x faster                                                   |
| sphinx         | 434 ms                                                   | 394 ms: 1.10x faster                                                    |
| Geometric mean | (ref)                                                    | 1.06x faster                                                            |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                        | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260920-macm4pro-arm64-python-41d09220bda74cda6897-3.16.0a0-41d0922 |
|----------------------------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| async_tree_io_tg                 | 480 ms                                                   | 326 ms: 1.47x faster                                                    |
| async_tree_eager_io              | 496 ms                                                   | 341 ms: 1.46x faster                                                    |
| coroutines                       | 13.6 ms                                                  | 9.44 ms: 1.44x faster                                                   |
| async_generators                 | 206 ms                                                   | 145 ms: 1.42x faster                                                    |
| async_tree_none_tg               | 172 ms                                                   | 124 ms: 1.39x faster                                                    |
| async_tree_io                    | 459 ms                                                   | 339 ms: 1.36x faster                                                    |
| async_tree_memoization_tg        | 231 ms                                                   | 172 ms: 1.34x faster                                                    |
| async_tree_none                  | 178 ms                                                   | 135 ms: 1.32x faster                                                    |
| async_tree_eager_io_tg           | 446 ms                                                   | 339 ms: 1.32x faster                                                    |
| async_tree_memoization           | 223 ms                                                   | 182 ms: 1.22x faster                                                    |
| async_tree_cpu_io_mixed          | 333 ms                                                   | 284 ms: 1.17x faster                                                    |
| async_tree_cpu_io_mixed_tg       | 338 ms                                                   | 291 ms: 1.16x faster                                                    |
| asyncio_websockets               | 190 ms                                                   | 189 ms: 1.01x faster                                                    |
| async_tree_eager_memoization     | 132 ms                                                   | 134 ms: 1.01x slower                                                    |
| async_tree_eager_cpu_io_mixed    | 231 ms                                                   | 248 ms: 1.08x slower                                                    |
| async_tree_eager_cpu_io_mixed_tg | 213 ms                                                   | 264 ms: 1.24x slower                                                    |
| async_tree_eager_memoization_tg  | 113 ms                                                   | 153 ms: 1.36x slower                                                    |
| async_tree_eager                 | 45.6 ms                                                  | 64.4 ms: 1.41x slower                                                   |
| async_tree_eager_tg              | 32.1 ms                                                  | 103 ms: 3.20x slower                                                    |
| Geometric mean                   | (ref)                                                    | 1.07x faster                                                            |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260920-macm4pro-arm64-python-41d09220bda74cda6897-3.16.0a0-41d0922 |
|----------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| float          | 37.9 ms                                                  | 28.4 ms: 1.34x faster                                                   |
| nbody          | 54.2 ms                                                  | 44.0 ms: 1.23x faster                                                   |
| pidigits       | 161 ms                                                   | 163 ms: 1.01x slower                                                    |
| Geometric mean | (ref)                                                    | 1.18x faster                                                            |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260920-macm4pro-arm64-python-41d09220bda74cda6897-3.16.0a0-41d0922 |
|----------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| regex_effbot   | 1.67 ms                                                  | 1.32 ms: 1.27x faster                                                   |
| regex_dna      | 99.6 ms                                                  | 93.5 ms: 1.07x faster                                                   |
| regex_v8       | 9.59 ms                                                  | 9.27 ms: 1.03x faster                                                   |
| regex_compile  | 54.6 ms                                                  | 53.7 ms: 1.02x faster                                                   |
| Geometric mean | (ref)                                                    | 1.09x faster                                                            |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260920-macm4pro-arm64-python-41d09220bda74cda6897-3.16.0a0-41d0922 |
|----------------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| xml_etree_iterparse  | 51.6 ms                                                  | 43.0 ms: 1.20x faster                                                   |
| tomli_loads          | 957 ms                                                   | 800 ms: 1.20x faster                                                    |
| json_dumps           | 4.26 ms                                                  | 3.56 ms: 1.20x faster                                                   |
| xml_etree_generate   | 38.9 ms                                                  | 35.4 ms: 1.10x faster                                                   |
| xml_etree_process    | 26.7 ms                                                  | 25.1 ms: 1.07x faster                                                   |
| unpickle_pure_python | 103 us                                                   | 96.6 us: 1.07x faster                                                   |
| json_loads           | 10.9 us                                                  | 10.6 us: 1.02x faster                                                   |
| Geometric mean       | (ref)                                                    | 1.09x faster                                                            |

Benchmark hidden because not significant (2): pickle_pure_python, xml_etree_parse

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260920-macm4pro-arm64-python-41d09220bda74cda6897-3.16.0a0-41d0922 |
|------------------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| python_startup         | 8.01 ms                                                  | 9.05 ms: 1.13x slower                                                   |
| python_startup_no_site | 5.71 ms                                                  | 6.60 ms: 1.16x slower                                                   |
| Geometric mean         | (ref)                                                    | 1.14x slower                                                            |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260920-macm4pro-arm64-python-41d09220bda74cda6897-3.16.0a0-41d0922 |
|-----------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| mako            | 4.77 ms                                                  | 4.62 ms: 1.03x faster                                                   |
| django_template | 13.6 ms                                                  | 14.8 ms: 1.08x slower                                                   |
| Geometric mean  | (ref)                                                    | 1.02x slower                                                            |

All benchmarks:
===============

| Benchmark                        | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260920-macm4pro-arm64-python-41d09220bda74cda6897-3.16.0a0-41d0922 |
|----------------------------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| subparsers                       | 20.8 ms                                                  | 4.10 ms: 5.07x faster                                                   |
| pylint                           | 128 ms                                                   | 55.5 ms: 2.31x faster                                                   |
| mdp                              | 1.09 sec                                                 | 510 ms: 2.14x faster                                                    |
| deepcopy                         | 161 us                                                   | 95.5 us: 1.69x faster                                                   |
| deepcopy_memo                    | 18.3 us                                                  | 11.7 us: 1.57x faster                                                   |
| comprehensions                   | 9.84 us                                                  | 6.63 us: 1.48x faster                                                   |
| async_tree_io_tg                 | 480 ms                                                   | 326 ms: 1.47x faster                                                    |
| async_tree_eager_io              | 496 ms                                                   | 341 ms: 1.46x faster                                                    |
| typing_runtime_protocols         | 71.0 us                                                  | 48.9 us: 1.45x faster                                                   |
| deepcopy_reduce                  | 1.46 us                                                  | 1.02 us: 1.44x faster                                                   |
| coroutines                       | 13.6 ms                                                  | 9.44 ms: 1.44x faster                                                   |
| async_generators                 | 206 ms                                                   | 145 ms: 1.42x faster                                                    |
| async_tree_none_tg               | 172 ms                                                   | 124 ms: 1.39x faster                                                    |
| async_tree_io                    | 459 ms                                                   | 339 ms: 1.36x faster                                                    |
| go                               | 70.0 ms                                                  | 52.0 ms: 1.35x faster                                                   |
| async_tree_memoization_tg        | 231 ms                                                   | 172 ms: 1.34x faster                                                    |
| float                            | 37.9 ms                                                  | 28.4 ms: 1.34x faster                                                   |
| async_tree_none                  | 178 ms                                                   | 135 ms: 1.32x faster                                                    |
| async_tree_eager_io_tg           | 446 ms                                                   | 339 ms: 1.32x faster                                                    |
| generators                       | 21.9 ms                                                  | 16.8 ms: 1.31x faster                                                   |
| spectral_norm                    | 54.4 ms                                                  | 42.6 ms: 1.28x faster                                                   |
| regex_effbot                     | 1.67 ms                                                  | 1.32 ms: 1.27x faster                                                   |
| scimark_sor                      | 61.0 ms                                                  | 48.3 ms: 1.26x faster                                                   |
| logging_silent                   | 50.9 ns                                                  | 40.6 ns: 1.25x faster                                                   |
| raytrace                         | 145 ms                                                   | 116 ms: 1.25x faster                                                    |
| nqueens                          | 43.5 ms                                                  | 34.8 ms: 1.25x faster                                                   |
| nbody                            | 54.2 ms                                                  | 44.0 ms: 1.23x faster                                                   |
| async_tree_memoization           | 223 ms                                                   | 182 ms: 1.22x faster                                                    |
| scimark_fft                      | 142 ms                                                   | 116 ms: 1.22x faster                                                    |
| logging_simple                   | 2.57 us                                                  | 2.14 us: 1.20x faster                                                   |
| xml_etree_iterparse              | 51.6 ms                                                  | 43.0 ms: 1.20x faster                                                   |
| tomli_loads                      | 957 ms                                                   | 800 ms: 1.20x faster                                                    |
| json_dumps                       | 4.26 ms                                                  | 3.56 ms: 1.20x faster                                                   |
| pyflate                          | 216 ms                                                   | 181 ms: 1.19x faster                                                    |
| deltablue                        | 1.73 ms                                                  | 1.45 ms: 1.19x faster                                                   |
| dulwich_log                      | 21.3 ms                                                  | 17.9 ms: 1.18x faster                                                   |
| logging_format                   | 2.80 us                                                  | 2.37 us: 1.18x faster                                                   |
| async_tree_cpu_io_mixed          | 333 ms                                                   | 284 ms: 1.17x faster                                                    |
| scimark_monte_carlo              | 32.2 ms                                                  | 27.5 ms: 1.17x faster                                                   |
| pathlib                          | 12.4 ms                                                  | 10.6 ms: 1.16x faster                                                   |
| async_tree_cpu_io_mixed_tg       | 338 ms                                                   | 291 ms: 1.16x faster                                                    |
| bpe_tokeniser                    | 2.24 sec                                                 | 1.94 sec: 1.15x faster                                                  |
| hexiom                           | 3.04 ms                                                  | 2.63 ms: 1.15x faster                                                   |
| k_core                           | 1.12 sec                                                 | 977 ms: 1.14x faster                                                    |
| chaos                            | 28.9 ms                                                  | 25.4 ms: 1.14x faster                                                   |
| scimark_sparse_mat_mult          | 2.08 ms                                                  | 1.83 ms: 1.14x faster                                                   |
| richards                         | 22.4 ms                                                  | 20.0 ms: 1.12x faster                                                   |
| richards_super                   | 25.4 ms                                                  | 22.7 ms: 1.12x faster                                                   |
| fannkuch                         | 176 ms                                                   | 159 ms: 1.10x faster                                                    |
| sphinx                           | 434 ms                                                   | 394 ms: 1.10x faster                                                    |
| xml_etree_generate               | 38.9 ms                                                  | 35.4 ms: 1.10x faster                                                   |
| docutils                         | 1.02 sec                                                 | 931 ms: 1.10x faster                                                    |
| sympy_integrate                  | 8.02 ms                                                  | 7.33 ms: 1.09x faster                                                   |
| html5lib                         | 23.0 ms                                                  | 21.3 ms: 1.08x faster                                                   |
| xml_etree_process                | 26.7 ms                                                  | 25.1 ms: 1.07x faster                                                   |
| unpickle_pure_python             | 103 us                                                   | 96.6 us: 1.07x faster                                                   |
| regex_dna                        | 99.6 ms                                                  | 93.5 ms: 1.07x faster                                                   |
| sympy_sum                        | 57.6 ms                                                  | 54.5 ms: 1.06x faster                                                   |
| pprint_safe_repr                 | 328 ms                                                   | 311 ms: 1.06x faster                                                    |
| sympy_str                        | 104 ms                                                   | 98.8 ms: 1.06x faster                                                   |
| sqlite_synth                     | 967 ns                                                   | 918 ns: 1.05x faster                                                    |
| gc_traversal                     | 2.01 ms                                                  | 1.92 ms: 1.04x faster                                                   |
| crypto_pyaes                     | 38.8 ms                                                  | 37.2 ms: 1.04x faster                                                   |
| thrift                           | 322 us                                                   | 310 us: 1.04x faster                                                    |
| pycparser                        | 497 ms                                                   | 481 ms: 1.03x faster                                                    |
| mako                             | 4.77 ms                                                  | 4.62 ms: 1.03x faster                                                   |
| regex_v8                         | 9.59 ms                                                  | 9.27 ms: 1.03x faster                                                   |
| json                             | 1.93 ms                                                  | 1.88 ms: 1.03x faster                                                   |
| pprint_pformat                   | 665 ms                                                   | 647 ms: 1.03x faster                                                    |
| json_loads                       | 10.9 us                                                  | 10.6 us: 1.02x faster                                                   |
| regex_compile                    | 54.6 ms                                                  | 53.7 ms: 1.02x faster                                                   |
| create_gc_cycles                 | 830 us                                                   | 823 us: 1.01x faster                                                    |
| asyncio_websockets               | 190 ms                                                   | 189 ms: 1.01x faster                                                    |
| bench_thread_pool                | 419 us                                                   | 417 us: 1.00x faster                                                    |
| sympy_expand                     | 167 ms                                                   | 168 ms: 1.01x slower                                                    |
| pidigits                         | 161 ms                                                   | 163 ms: 1.01x slower                                                    |
| async_tree_eager_memoization     | 132 ms                                                   | 134 ms: 1.01x slower                                                    |
| shortest_path                    | 219 ms                                                   | 222 ms: 1.02x slower                                                    |
| connected_components             | 201 ms                                                   | 206 ms: 1.03x slower                                                    |
| 2to3                             | 114 ms                                                   | 118 ms: 1.03x slower                                                    |
| async_tree_eager_cpu_io_mixed    | 231 ms                                                   | 248 ms: 1.08x slower                                                    |
| django_template                  | 13.6 ms                                                  | 14.8 ms: 1.08x slower                                                   |
| telco                            | 2.61 ms                                                  | 2.86 ms: 1.10x slower                                                   |
| bench_mp_pool                    | 39.7 ms                                                  | 44.5 ms: 1.12x slower                                                   |
| python_startup                   | 8.01 ms                                                  | 9.05 ms: 1.13x slower                                                   |
| python_startup_no_site           | 5.71 ms                                                  | 6.60 ms: 1.16x slower                                                   |
| many_optionals                   | 195 us                                                   | 240 us: 1.23x slower                                                    |
| async_tree_eager_cpu_io_mixed_tg | 213 ms                                                   | 264 ms: 1.24x slower                                                    |
| async_tree_eager_memoization_tg  | 113 ms                                                   | 153 ms: 1.36x slower                                                    |
| async_tree_eager                 | 45.6 ms                                                  | 64.4 ms: 1.41x slower                                                   |
| async_tree_eager_tg              | 32.1 ms                                                  | 103 ms: 3.20x slower                                                    |
| Geometric mean                   | (ref)                                                    | 1.14x faster                                                            |

Benchmark hidden because not significant (4): pickle_pure_python, xml_etree_parse, meteor_contest, scimark_lu
Ignored benchmarks (14) of results/bm-20240906-3.12.6-a4a2d2b/bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b.json: chameleon, coverage, dask, djangocms, genshi_text, genshi_xml, gevent_hub, sqlalchemy_declarative, sqlalchemy_imperative, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http
Ignored benchmarks (4) of results/bm-20260920-3.16.0a0-41d0922/bm-20260920-macm4pro-arm64-python-41d09220bda74cda6897-3.16.0a0-41d0922.json: sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile

- Geometric mean (including insignificant results): 1.141x faster

# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.09x
- 95% likely to have a speedup of 1.08x
- 99% likely to have a speedup of 1.06x

# Memory
- memory change: 1.22x