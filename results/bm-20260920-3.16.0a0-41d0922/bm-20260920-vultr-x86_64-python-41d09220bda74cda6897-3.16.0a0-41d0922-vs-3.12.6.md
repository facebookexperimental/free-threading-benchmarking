# Results vs. 3.12.6

- fork: python
- ref: 41d09220bda74cda6897
- machine: linux-x86_64
- commit hash: 41d0922
- commit date: 2026-09-20
- overall geometric mean: 1.065x faster
- HPT reliability: 100.00%
- HPT 99th percentile: 1.02x faster
- Memory change: 1.16x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260920-vultr-x86_64-python-41d09220bda74cda6897-3.16.0a0-41d0922 |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| 2to3           | 264 ms                                                 | 258 ms: 1.02x faster                                                  |
| docutils       | 2.64 sec                                               | 2.33 sec: 1.13x faster                                                |
| html5lib       | 63.6 ms                                                | 56.8 ms: 1.12x faster                                                 |
| Geometric mean | (ref)                                                  | 1.09x faster                                                          |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260920-vultr-x86_64-python-41d09220bda74cda6897-3.16.0a0-41d0922 |
|----------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| async_tree_memoization_tg  | 560 ms                                                 | 366 ms: 1.53x faster                                                  |
| async_tree_none_tg         | 446 ms                                                 | 303 ms: 1.47x faster                                                  |
| async_tree_io_tg           | 1.11 sec                                               | 759 ms: 1.46x faster                                                  |
| async_tree_none            | 464 ms                                                 | 331 ms: 1.40x faster                                                  |
| async_tree_io              | 1.08 sec                                               | 775 ms: 1.40x faster                                                  |
| async_tree_memoization     | 555 ms                                                 | 404 ms: 1.37x faster                                                  |
| async_tree_cpu_io_mixed_tg | 723 ms                                                 | 555 ms: 1.30x faster                                                  |
| async_tree_cpu_io_mixed    | 715 ms                                                 | 560 ms: 1.28x faster                                                  |
| async_generators           | 384 ms                                                 | 347 ms: 1.11x faster                                                  |
| coroutines                 | 23.9 ms                                                | 23.1 ms: 1.03x faster                                                 |
| asyncio_websockets         | 517 ms                                                 | 545 ms: 1.05x slower                                                  |
| Geometric mean             | (ref)                                                  | 1.29x faster                                                          |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260920-vultr-x86_64-python-41d09220bda74cda6897-3.16.0a0-41d0922 |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| float          | 80.8 ms                                                | 72.0 ms: 1.12x faster                                                 |
| pidigits       | 184 ms                                                 | 187 ms: 1.02x slower                                                  |
| nbody          | 89.3 ms                                                | 91.2 ms: 1.02x slower                                                 |
| Geometric mean | (ref)                                                  | 1.03x faster                                                          |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260920-vultr-x86_64-python-41d09220bda74cda6897-3.16.0a0-41d0922 |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| regex_effbot   | 3.17 ms                                                | 2.62 ms: 1.21x faster                                                 |
| regex_compile  | 142 ms                                                 | 148 ms: 1.04x slower                                                  |
| regex_v8       | 20.6 ms                                                | 21.6 ms: 1.05x slower                                                 |
| regex_dna      | 168 ms                                                 | 182 ms: 1.08x slower                                                  |
| Geometric mean | (ref)                                                  | 1.00x faster                                                          |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260920-vultr-x86_64-python-41d09220bda74cda6897-3.16.0a0-41d0922 |
|----------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| tomli_loads          | 2.11 sec                                               | 1.83 sec: 1.15x faster                                                |
| json_dumps           | 10.4 ms                                                | 9.44 ms: 1.10x faster                                                 |
| unpickle_pure_python | 221 us                                                 | 209 us: 1.05x faster                                                  |
| xml_etree_iterparse  | 96.7 ms                                                | 92.0 ms: 1.05x faster                                                 |
| json_loads           | 26.5 us                                                | 27.0 us: 1.02x slower                                                 |
| xml_etree_generate   | 85.2 ms                                                | 89.0 ms: 1.04x slower                                                 |
| xml_etree_parse      | 139 ms                                                 | 146 ms: 1.06x slower                                                  |
| xml_etree_process    | 59.0 ms                                                | 63.3 ms: 1.07x slower                                                 |
| Geometric mean       | (ref)                                                  | 1.02x faster                                                          |

Benchmark hidden because not significant (1): pickle_pure_python

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260920-vultr-x86_64-python-41d09220bda74cda6897-3.16.0a0-41d0922 |
|------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| python_startup_no_site | 7.16 ms                                                | 7.74 ms: 1.08x slower                                                 |
| python_startup         | 9.93 ms                                                | 13.0 ms: 1.31x slower                                                 |
| Geometric mean         | (ref)                                                  | 1.19x slower                                                          |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260920-vultr-x86_64-python-41d09220bda74cda6897-3.16.0a0-41d0922 |
|-----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| django_template | 34.7 ms                                                | 36.8 ms: 1.06x slower                                                 |
| mako            | 11.0 ms                                                | 11.8 ms: 1.07x slower                                                 |
| Geometric mean  | (ref)                                                  | 1.07x slower                                                          |

All benchmarks:
===============

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260920-vultr-x86_64-python-41d09220bda74cda6897-3.16.0a0-41d0922 |
|----------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| pylint                     | 319 ms                                                 | 112 ms: 2.84x faster                                                  |
| mdp                        | 2.42 sec                                               | 1.13 sec: 2.14x faster                                                |
| deepcopy                   | 352 us                                                 | 227 us: 1.55x faster                                                  |
| async_tree_memoization_tg  | 560 ms                                                 | 366 ms: 1.53x faster                                                  |
| async_tree_none_tg         | 446 ms                                                 | 303 ms: 1.47x faster                                                  |
| async_tree_io_tg           | 1.11 sec                                               | 759 ms: 1.46x faster                                                  |
| deepcopy_memo              | 40.3 us                                                | 27.7 us: 1.45x faster                                                 |
| async_tree_none            | 464 ms                                                 | 331 ms: 1.40x faster                                                  |
| async_tree_io              | 1.08 sec                                               | 775 ms: 1.40x faster                                                  |
| typing_runtime_protocols   | 163 us                                                 | 118 us: 1.38x faster                                                  |
| async_tree_memoization     | 555 ms                                                 | 404 ms: 1.37x faster                                                  |
| go                         | 139 ms                                                 | 102 ms: 1.36x faster                                                  |
| async_tree_cpu_io_mixed_tg | 723 ms                                                 | 555 ms: 1.30x faster                                                  |
| async_tree_cpu_io_mixed    | 715 ms                                                 | 560 ms: 1.28x faster                                                  |
| comprehensions             | 19.8 us                                                | 15.7 us: 1.26x faster                                                 |
| deepcopy_reduce            | 3.08 us                                                | 2.51 us: 1.22x faster                                                 |
| scimark_sor                | 130 ms                                                 | 107 ms: 1.22x faster                                                  |
| spectral_norm              | 110 ms                                                 | 91.2 ms: 1.21x faster                                                 |
| regex_effbot               | 3.17 ms                                                | 2.62 ms: 1.21x faster                                                 |
| raytrace                   | 299 ms                                                 | 248 ms: 1.21x faster                                                  |
| pathlib                    | 21.5 ms                                                | 17.9 ms: 1.20x faster                                                 |
| dulwich_log                | 78.9 ms                                                | 66.3 ms: 1.19x faster                                                 |
| chaos                      | 62.8 ms                                                | 52.9 ms: 1.19x faster                                                 |
| tomli_loads                | 2.11 sec                                               | 1.83 sec: 1.15x faster                                                |
| generators                 | 32.2 ms                                                | 28.1 ms: 1.15x faster                                                 |
| pyflate                    | 448 ms                                                 | 392 ms: 1.14x faster                                                  |
| docutils                   | 2.64 sec                                               | 2.33 sec: 1.13x faster                                                |
| crypto_pyaes               | 76.6 ms                                                | 67.9 ms: 1.13x faster                                                 |
| bpe_tokeniser              | 4.74 sec                                               | 4.21 sec: 1.13x faster                                                |
| logging_simple             | 6.63 us                                                | 5.89 us: 1.13x faster                                                 |
| float                      | 80.8 ms                                                | 72.0 ms: 1.12x faster                                                 |
| html5lib                   | 63.6 ms                                                | 56.8 ms: 1.12x faster                                                 |
| async_generators           | 384 ms                                                 | 347 ms: 1.11x faster                                                  |
| logging_silent             | 109 ns                                                 | 98.7 ns: 1.10x faster                                                 |
| hexiom                     | 6.17 ms                                                | 5.62 ms: 1.10x faster                                                 |
| json_dumps                 | 10.4 ms                                                | 9.44 ms: 1.10x faster                                                 |
| logging_format             | 7.35 us                                                | 6.71 us: 1.10x faster                                                 |
| scimark_fft                | 342 ms                                                 | 312 ms: 1.09x faster                                                  |
| nqueens                    | 80.1 ms                                                | 73.6 ms: 1.09x faster                                                 |
| scimark_monte_carlo        | 68.4 ms                                                | 64.2 ms: 1.07x faster                                                 |
| sympy_integrate            | 20.5 ms                                                | 19.3 ms: 1.07x faster                                                 |
| unpickle_pure_python       | 221 us                                                 | 209 us: 1.05x faster                                                  |
| xml_etree_iterparse        | 96.7 ms                                                | 92.0 ms: 1.05x faster                                                 |
| pycparser                  | 1.17 sec                                               | 1.11 sec: 1.05x faster                                                |
| deltablue                  | 3.45 ms                                                | 3.30 ms: 1.05x faster                                                 |
| sympy_sum                  | 166 ms                                                 | 159 ms: 1.04x faster                                                  |
| sympy_str                  | 292 ms                                                 | 280 ms: 1.04x faster                                                  |
| coroutines                 | 23.9 ms                                                | 23.1 ms: 1.03x faster                                                 |
| meteor_contest             | 104 ms                                                 | 100 ms: 1.03x faster                                                  |
| richards                   | 45.9 ms                                                | 44.6 ms: 1.03x faster                                                 |
| pprint_safe_repr           | 743 ms                                                 | 725 ms: 1.02x faster                                                  |
| 2to3                       | 264 ms                                                 | 258 ms: 1.02x faster                                                  |
| pprint_pformat             | 1.52 sec                                               | 1.50 sec: 1.02x faster                                                |
| thrift                     | 791 us                                                 | 779 us: 1.02x faster                                                  |
| fannkuch                   | 372 ms                                                 | 375 ms: 1.01x slower                                                  |
| sympy_expand               | 468 ms                                                 | 472 ms: 1.01x slower                                                  |
| json_loads                 | 26.5 us                                                | 27.0 us: 1.02x slower                                                 |
| pidigits                   | 184 ms                                                 | 187 ms: 1.02x slower                                                  |
| nbody                      | 89.3 ms                                                | 91.2 ms: 1.02x slower                                                 |
| gc_traversal               | 3.46 ms                                                | 3.59 ms: 1.04x slower                                                 |
| regex_compile              | 142 ms                                                 | 148 ms: 1.04x slower                                                  |
| scimark_lu                 | 114 ms                                                 | 119 ms: 1.04x slower                                                  |
| xml_etree_generate         | 85.2 ms                                                | 89.0 ms: 1.04x slower                                                 |
| regex_v8                   | 20.6 ms                                                | 21.6 ms: 1.05x slower                                                 |
| asyncio_websockets         | 517 ms                                                 | 545 ms: 1.05x slower                                                  |
| xml_etree_parse            | 139 ms                                                 | 146 ms: 1.06x slower                                                  |
| django_template            | 34.7 ms                                                | 36.8 ms: 1.06x slower                                                 |
| scimark_sparse_mat_mult    | 4.39 ms                                                | 4.67 ms: 1.06x slower                                                 |
| mako                       | 11.0 ms                                                | 11.8 ms: 1.07x slower                                                 |
| xml_etree_process          | 59.0 ms                                                | 63.3 ms: 1.07x slower                                                 |
| python_startup_no_site     | 7.16 ms                                                | 7.74 ms: 1.08x slower                                                 |
| regex_dna                  | 168 ms                                                 | 182 ms: 1.08x slower                                                  |
| coverage                   | 71.4 ms                                                | 83.3 ms: 1.17x slower                                                 |
| python_startup             | 9.93 ms                                                | 13.0 ms: 1.31x slower                                                 |
| bench_thread_pool          | 941 us                                                 | 1.35 ms: 1.43x slower                                                 |
| create_gc_cycles           | 1.09 ms                                                | 1.68 ms: 1.54x slower                                                 |
| telco                      | 6.53 ms                                                | 159 ms: 24.31x slower                                                 |
| bench_mp_pool              | 10.8 ms                                                | 264 ms: 24.48x slower                                                 |
| Geometric mean             | (ref)                                                  | 1.02x faster                                                          |

Benchmark hidden because not significant (4): json, richards_super, pickle_pure_python, sqlite_synth
Ignored benchmarks (23) of results/bm-20240906-3.12.6-a4a2d2b/bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b.json: aiohttp, asyncio_tcp, asyncio_tcp_ssl, chameleon, dask, flaskblogging, genshi_text, genshi_xml, gunicorn, mypy2, pickle, pickle_dict, pickle_list, sqlalchemy_declarative, sqlalchemy_imperative, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http, unpack_sequence, unpickle, unpickle_list
Ignored benchmarks (10) of results/bm-20260920-3.16.0a0-41d0922/bm-20260920-vultr-x86_64-python-41d09220bda74cda6897-3.16.0a0-41d0922.json: connected_components, k_core, many_optionals, shortest_path, sphinx, sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile, subparsers

- Geometric mean (including insignificant results): 1.065x faster

# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.05x
- 95% likely to have a speedup of 1.04x
- 99% likely to have a speedup of 1.02x

# Memory
- memory change: 1.16x