# Results vs. 3.12.6

- fork: python
- ref: 5056ac552a413b394fd1
- machine: darwin-arm64
- commit hash: 5056ac5
- commit date: 2026-09-03
- overall geometric mean: 1.008x faster
- HPT reliability: 79.32%
- HPT 99th percentile: 1.00x slower
- Memory change: 1.32x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260903-macm4pro-arm64-python-5056ac552a413b394fd1-3.16.0a0-5056ac5 |
|----------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| 2to3           | 114 ms                                                   | 137 ms: 1.20x slower                                                    |
| docutils       | 1.02 sec                                                 | 1.10 sec: 1.08x slower                                                  |
| html5lib       | 23.0 ms                                                  | 24.5 ms: 1.06x slower                                                   |
| sphinx         | 434 ms                                                   | 465 ms: 1.07x slower                                                    |
| Geometric mean | (ref)                                                    | 1.10x slower                                                            |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                        | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260903-macm4pro-arm64-python-5056ac552a413b394fd1-3.16.0a0-5056ac5 |
|----------------------------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| async_tree_eager_io              | 496 ms                                                   | 308 ms: 1.61x faster                                                    |
| async_tree_io_tg                 | 480 ms                                                   | 311 ms: 1.54x faster                                                    |
| async_tree_eager_io_tg           | 446 ms                                                   | 306 ms: 1.46x faster                                                    |
| async_tree_io                    | 459 ms                                                   | 318 ms: 1.44x faster                                                    |
| async_generators                 | 206 ms                                                   | 166 ms: 1.24x faster                                                    |
| async_tree_memoization_tg        | 231 ms                                                   | 187 ms: 1.23x faster                                                    |
| async_tree_none                  | 178 ms                                                   | 150 ms: 1.19x faster                                                    |
| async_tree_none_tg               | 172 ms                                                   | 148 ms: 1.16x faster                                                    |
| async_tree_memoization           | 223 ms                                                   | 194 ms: 1.15x faster                                                    |
| async_tree_cpu_io_mixed_tg       | 338 ms                                                   | 303 ms: 1.12x faster                                                    |
| async_tree_cpu_io_mixed          | 333 ms                                                   | 310 ms: 1.07x faster                                                    |
| coroutines                       | 13.6 ms                                                  | 12.7 ms: 1.07x faster                                                   |
| asyncio_websockets               | 190 ms                                                   | 185 ms: 1.03x faster                                                    |
| async_tree_eager_memoization     | 132 ms                                                   | 129 ms: 1.02x faster                                                    |
| async_tree_eager_cpu_io_mixed    | 231 ms                                                   | 241 ms: 1.04x slower                                                    |
| async_tree_eager                 | 45.6 ms                                                  | 53.3 ms: 1.17x slower                                                   |
| async_tree_eager_cpu_io_mixed_tg | 213 ms                                                   | 281 ms: 1.32x slower                                                    |
| async_tree_eager_memoization_tg  | 113 ms                                                   | 167 ms: 1.48x slower                                                    |
| async_tree_eager_tg              | 32.1 ms                                                  | 120 ms: 3.73x slower                                                    |
| Geometric mean                   | (ref)                                                    | 1.03x faster                                                            |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260903-macm4pro-arm64-python-5056ac552a413b394fd1-3.16.0a0-5056ac5 |
|----------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| float          | 37.9 ms                                                  | 35.7 ms: 1.06x faster                                                   |
| pidigits       | 161 ms                                                   | 167 ms: 1.03x slower                                                    |
| nbody          | 54.2 ms                                                  | 62.8 ms: 1.16x slower                                                   |
| Geometric mean | (ref)                                                    | 1.04x slower                                                            |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260903-macm4pro-arm64-python-5056ac552a413b394fd1-3.16.0a0-5056ac5 |
|----------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| regex_effbot   | 1.67 ms                                                  | 1.34 ms: 1.24x faster                                                   |
| regex_dna      | 99.6 ms                                                  | 93.5 ms: 1.07x faster                                                   |
| regex_v8       | 9.59 ms                                                  | 9.19 ms: 1.04x faster                                                   |
| regex_compile  | 54.6 ms                                                  | 69.9 ms: 1.28x slower                                                   |
| Geometric mean | (ref)                                                    | 1.02x faster                                                            |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260903-macm4pro-arm64-python-5056ac552a413b394fd1-3.16.0a0-5056ac5 |
|----------------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| xml_etree_iterparse  | 51.6 ms                                                  | 41.6 ms: 1.24x faster                                                   |
| xml_etree_parse      | 67.9 ms                                                  | 58.0 ms: 1.17x faster                                                   |
| json_dumps           | 4.26 ms                                                  | 3.71 ms: 1.15x faster                                                   |
| json_loads           | 10.9 us                                                  | 11.0 us: 1.02x slower                                                   |
| xml_etree_generate   | 38.9 ms                                                  | 39.9 ms: 1.03x slower                                                   |
| unpickle_pure_python | 103 us                                                   | 120 us: 1.17x slower                                                    |
| xml_etree_process    | 26.7 ms                                                  | 31.6 ms: 1.18x slower                                                   |
| pickle_pure_python   | 139 us                                                   | 165 us: 1.18x slower                                                    |
| Geometric mean       | (ref)                                                    | 1.00x slower                                                            |

Benchmark hidden because not significant (1): tomli_loads

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260903-macm4pro-arm64-python-5056ac552a413b394fd1-3.16.0a0-5056ac5 |
|------------------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| python_startup         | 8.01 ms                                                  | 10.4 ms: 1.29x slower                                                   |
| python_startup_no_site | 5.71 ms                                                  | 7.62 ms: 1.34x slower                                                   |
| Geometric mean         | (ref)                                                    | 1.31x slower                                                            |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260903-macm4pro-arm64-python-5056ac552a413b394fd1-3.16.0a0-5056ac5 |
|-----------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| mako            | 4.77 ms                                                  | 6.06 ms: 1.27x slower                                                   |
| django_template | 13.6 ms                                                  | 17.9 ms: 1.32x slower                                                   |
| Geometric mean  | (ref)                                                    | 1.29x slower                                                            |

All benchmarks:
===============

| Benchmark                        | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260903-macm4pro-arm64-python-5056ac552a413b394fd1-3.16.0a0-5056ac5 |
|----------------------------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| subparsers                       | 20.8 ms                                                  | 4.63 ms: 4.49x faster                                                   |
| gc_traversal                     | 2.01 ms                                                  | 787 us: 2.55x faster                                                    |
| pylint                           | 128 ms                                                   | 55.0 ms: 2.33x faster                                                   |
| mdp                              | 1.09 sec                                                 | 635 ms: 1.72x faster                                                    |
| create_gc_cycles                 | 830 us                                                   | 491 us: 1.69x faster                                                    |
| async_tree_eager_io              | 496 ms                                                   | 308 ms: 1.61x faster                                                    |
| async_tree_io_tg                 | 480 ms                                                   | 311 ms: 1.54x faster                                                    |
| async_tree_eager_io_tg           | 446 ms                                                   | 306 ms: 1.46x faster                                                    |
| async_tree_io                    | 459 ms                                                   | 318 ms: 1.44x faster                                                    |
| deepcopy                         | 161 us                                                   | 121 us: 1.34x faster                                                    |
| regex_effbot                     | 1.67 ms                                                  | 1.34 ms: 1.24x faster                                                   |
| async_generators                 | 206 ms                                                   | 166 ms: 1.24x faster                                                    |
| xml_etree_iterparse              | 51.6 ms                                                  | 41.6 ms: 1.24x faster                                                   |
| async_tree_memoization_tg        | 231 ms                                                   | 187 ms: 1.23x faster                                                    |
| sqlite_synth                     | 967 ns                                                   | 811 ns: 1.19x faster                                                    |
| async_tree_none                  | 178 ms                                                   | 150 ms: 1.19x faster                                                    |
| xml_etree_parse                  | 67.9 ms                                                  | 58.0 ms: 1.17x faster                                                   |
| async_tree_none_tg               | 172 ms                                                   | 148 ms: 1.16x faster                                                    |
| typing_runtime_protocols         | 71.0 us                                                  | 61.6 us: 1.15x faster                                                   |
| async_tree_memoization           | 223 ms                                                   | 194 ms: 1.15x faster                                                    |
| json_dumps                       | 4.26 ms                                                  | 3.71 ms: 1.15x faster                                                   |
| deepcopy_reduce                  | 1.46 us                                                  | 1.29 us: 1.14x faster                                                   |
| bpe_tokeniser                    | 2.24 sec                                                 | 2.00 sec: 1.12x faster                                                  |
| async_tree_cpu_io_mixed_tg       | 338 ms                                                   | 303 ms: 1.12x faster                                                    |
| deepcopy_memo                    | 18.3 us                                                  | 16.4 us: 1.12x faster                                                   |
| comprehensions                   | 9.84 us                                                  | 8.85 us: 1.11x faster                                                   |
| k_core                           | 1.12 sec                                                 | 1.01 sec: 1.10x faster                                                  |
| pathlib                          | 12.4 ms                                                  | 11.3 ms: 1.09x faster                                                   |
| async_tree_cpu_io_mixed          | 333 ms                                                   | 310 ms: 1.07x faster                                                    |
| coroutines                       | 13.6 ms                                                  | 12.7 ms: 1.07x faster                                                   |
| dulwich_log                      | 21.3 ms                                                  | 19.9 ms: 1.07x faster                                                   |
| regex_dna                        | 99.6 ms                                                  | 93.5 ms: 1.07x faster                                                   |
| float                            | 37.9 ms                                                  | 35.7 ms: 1.06x faster                                                   |
| regex_v8                         | 9.59 ms                                                  | 9.19 ms: 1.04x faster                                                   |
| go                               | 70.0 ms                                                  | 67.3 ms: 1.04x faster                                                   |
| scimark_fft                      | 142 ms                                                   | 137 ms: 1.04x faster                                                    |
| asyncio_websockets               | 190 ms                                                   | 185 ms: 1.03x faster                                                    |
| spectral_norm                    | 54.4 ms                                                  | 52.9 ms: 1.03x faster                                                   |
| async_tree_eager_memoization     | 132 ms                                                   | 129 ms: 1.02x faster                                                    |
| pyflate                          | 216 ms                                                   | 213 ms: 1.02x faster                                                    |
| pycparser                        | 497 ms                                                   | 502 ms: 1.01x slower                                                    |
| json_loads                       | 10.9 us                                                  | 11.0 us: 1.02x slower                                                   |
| json                             | 1.93 ms                                                  | 1.97 ms: 1.02x slower                                                   |
| raytrace                         | 145 ms                                                   | 149 ms: 1.02x slower                                                    |
| xml_etree_generate               | 38.9 ms                                                  | 39.9 ms: 1.03x slower                                                   |
| scimark_sor                      | 61.0 ms                                                  | 62.8 ms: 1.03x slower                                                   |
| nqueens                          | 43.5 ms                                                  | 44.8 ms: 1.03x slower                                                   |
| pidigits                         | 161 ms                                                   | 167 ms: 1.03x slower                                                    |
| logging_simple                   | 2.57 us                                                  | 2.68 us: 1.04x slower                                                   |
| async_tree_eager_cpu_io_mixed    | 231 ms                                                   | 241 ms: 1.04x slower                                                    |
| logging_format                   | 2.80 us                                                  | 2.96 us: 1.05x slower                                                   |
| scimark_sparse_mat_mult          | 2.08 ms                                                  | 2.19 ms: 1.06x slower                                                   |
| html5lib                         | 23.0 ms                                                  | 24.5 ms: 1.06x slower                                                   |
| sympy_integrate                  | 8.02 ms                                                  | 8.54 ms: 1.06x slower                                                   |
| chaos                            | 28.9 ms                                                  | 31.0 ms: 1.07x slower                                                   |
| sphinx                           | 434 ms                                                   | 465 ms: 1.07x slower                                                    |
| docutils                         | 1.02 sec                                                 | 1.10 sec: 1.08x slower                                                  |
| fannkuch                         | 176 ms                                                   | 191 ms: 1.09x slower                                                    |
| crypto_pyaes                     | 38.8 ms                                                  | 42.4 ms: 1.09x slower                                                   |
| sympy_sum                        | 57.6 ms                                                  | 63.4 ms: 1.10x slower                                                   |
| sympy_str                        | 104 ms                                                   | 116 ms: 1.11x slower                                                    |
| scimark_monte_carlo              | 32.2 ms                                                  | 35.7 ms: 1.11x slower                                                   |
| bench_mp_pool                    | 39.7 ms                                                  | 44.5 ms: 1.12x slower                                                   |
| thrift                           | 322 us                                                   | 362 us: 1.13x slower                                                    |
| meteor_contest                   | 47.7 ms                                                  | 54.8 ms: 1.15x slower                                                   |
| generators                       | 21.9 ms                                                  | 25.2 ms: 1.15x slower                                                   |
| nbody                            | 54.2 ms                                                  | 62.8 ms: 1.16x slower                                                   |
| shortest_path                    | 219 ms                                                   | 256 ms: 1.17x slower                                                    |
| unpickle_pure_python             | 103 us                                                   | 120 us: 1.17x slower                                                    |
| pprint_safe_repr                 | 328 ms                                                   | 383 ms: 1.17x slower                                                    |
| async_tree_eager                 | 45.6 ms                                                  | 53.3 ms: 1.17x slower                                                   |
| pprint_pformat                   | 665 ms                                                   | 781 ms: 1.18x slower                                                    |
| deltablue                        | 1.73 ms                                                  | 2.03 ms: 1.18x slower                                                   |
| sympy_expand                     | 167 ms                                                   | 197 ms: 1.18x slower                                                    |
| xml_etree_process                | 26.7 ms                                                  | 31.6 ms: 1.18x slower                                                   |
| pickle_pure_python               | 139 us                                                   | 165 us: 1.18x slower                                                    |
| hexiom                           | 3.04 ms                                                  | 3.63 ms: 1.20x slower                                                   |
| richards                         | 22.4 ms                                                  | 26.9 ms: 1.20x slower                                                   |
| 2to3                             | 114 ms                                                   | 137 ms: 1.20x slower                                                    |
| richards_super                   | 25.4 ms                                                  | 30.6 ms: 1.20x slower                                                   |
| telco                            | 2.61 ms                                                  | 3.16 ms: 1.21x slower                                                   |
| connected_components             | 201 ms                                                   | 247 ms: 1.23x slower                                                    |
| mako                             | 4.77 ms                                                  | 6.06 ms: 1.27x slower                                                   |
| regex_compile                    | 54.6 ms                                                  | 69.9 ms: 1.28x slower                                                   |
| python_startup                   | 8.01 ms                                                  | 10.4 ms: 1.29x slower                                                   |
| django_template                  | 13.6 ms                                                  | 17.9 ms: 1.32x slower                                                   |
| async_tree_eager_cpu_io_mixed_tg | 213 ms                                                   | 281 ms: 1.32x slower                                                    |
| python_startup_no_site           | 5.71 ms                                                  | 7.62 ms: 1.34x slower                                                   |
| bench_thread_pool                | 419 us                                                   | 566 us: 1.35x slower                                                    |
| scimark_lu                       | 51.3 ms                                                  | 70.1 ms: 1.37x slower                                                   |
| many_optionals                   | 195 us                                                   | 273 us: 1.40x slower                                                    |
| async_tree_eager_memoization_tg  | 113 ms                                                   | 167 ms: 1.48x slower                                                    |
| coverage                         | 26.9 ms                                                  | 41.3 ms: 1.54x slower                                                   |
| async_tree_eager_tg              | 32.1 ms                                                  | 120 ms: 3.73x slower                                                    |
| Geometric mean                   | (ref)                                                    | 1.00x faster                                                            |

Benchmark hidden because not significant (2): logging_silent, tomli_loads
Ignored benchmarks (13) of results/bm-20240906-3.12.6-a4a2d2b/bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b.json: chameleon, dask, djangocms, genshi_text, genshi_xml, gevent_hub, sqlalchemy_declarative, sqlalchemy_imperative, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http
Ignored benchmarks (4) of results/bm-20260903-3.16.0a0-5056ac5-NOGIL/bm-20260903-macm4pro-arm64-python-5056ac552a413b394fd1-3.16.0a0-5056ac5.json: sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile

- Geometric mean (including insignificant results): 1.008x faster

# HPT report

- Reliability score: 79.32% likely to be slow
- 90% likely to have a slowdown of 1.00x
- 95% likely to have a slowdown of 1.00x
- 99% likely to have a slowdown of 1.00x

# Memory
- memory change: 1.32x