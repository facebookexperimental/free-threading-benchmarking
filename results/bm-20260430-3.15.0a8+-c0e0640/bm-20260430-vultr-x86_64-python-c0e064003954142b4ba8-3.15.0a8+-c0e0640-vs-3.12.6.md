# Results vs. 3.12.6

- fork: python
- ref: c0e064003954142b4ba8
- machine: linux-x86_64
- commit hash: c0e0640
- commit date: 2026-04-30
- overall geometric mean: 1.067x faster
- HPT reliability: 100.00%
- HPT 99th percentile: 1.04x faster
- Memory change: 1.15x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260430-vultr-x86_64-python-c0e064003954142b4ba8-3.15.0a8+-c0e0640 |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| 2to3           | 264 ms                                                 | 254 ms: 1.04x faster                                                   |
| docutils       | 2.64 sec                                               | 2.38 sec: 1.11x faster                                                 |
| html5lib       | 63.6 ms                                                | 59.6 ms: 1.07x faster                                                  |
| Geometric mean | (ref)                                                  | 1.07x faster                                                           |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260430-vultr-x86_64-python-c0e064003954142b4ba8-3.15.0a8+-c0e0640 |
|----------------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| async_tree_none            | 464 ms                                                 | 288 ms: 1.61x faster                                                   |
| async_tree_memoization_tg  | 560 ms                                                 | 362 ms: 1.55x faster                                                   |
| async_tree_none_tg         | 446 ms                                                 | 296 ms: 1.51x faster                                                   |
| async_tree_io              | 1.08 sec                                               | 723 ms: 1.50x faster                                                   |
| async_tree_memoization     | 555 ms                                                 | 376 ms: 1.48x faster                                                   |
| async_tree_io_tg           | 1.11 sec                                               | 762 ms: 1.46x faster                                                   |
| async_tree_cpu_io_mixed    | 715 ms                                                 | 532 ms: 1.34x faster                                                   |
| async_tree_cpu_io_mixed_tg | 723 ms                                                 | 543 ms: 1.33x faster                                                   |
| async_generators           | 384 ms                                                 | 344 ms: 1.12x faster                                                   |
| coroutines                 | 23.9 ms                                                | 22.9 ms: 1.05x faster                                                  |
| asyncio_websockets         | 517 ms                                                 | 543 ms: 1.05x slower                                                   |
| Geometric mean             | (ref)                                                  | 1.34x faster                                                           |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260430-vultr-x86_64-python-c0e064003954142b4ba8-3.15.0a8+-c0e0640 |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| float          | 80.8 ms                                                | 74.4 ms: 1.09x faster                                                  |
| pidigits       | 184 ms                                                 | 185 ms: 1.00x slower                                                   |
| nbody          | 89.3 ms                                                | 92.2 ms: 1.03x slower                                                  |
| Geometric mean | (ref)                                                  | 1.02x faster                                                           |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260430-vultr-x86_64-python-c0e064003954142b4ba8-3.15.0a8+-c0e0640 |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| regex_compile  | 142 ms                                                 | 125 ms: 1.14x faster                                                   |
| regex_effbot   | 3.17 ms                                                | 2.93 ms: 1.08x faster                                                  |
| regex_v8       | 20.6 ms                                                | 22.7 ms: 1.10x slower                                                  |
| regex_dna      | 168 ms                                                 | 187 ms: 1.11x slower                                                   |
| Geometric mean | (ref)                                                  | 1.00x faster                                                           |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260430-vultr-x86_64-python-c0e064003954142b4ba8-3.15.0a8+-c0e0640 |
|----------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| tomli_loads          | 2.11 sec                                               | 1.84 sec: 1.14x faster                                                 |
| xml_etree_iterparse  | 96.7 ms                                                | 88.1 ms: 1.10x faster                                                  |
| json_dumps           | 10.4 ms                                                | 9.63 ms: 1.08x faster                                                  |
| unpickle_pure_python | 221 us                                                 | 216 us: 1.02x faster                                                   |
| xml_etree_parse      | 139 ms                                                 | 141 ms: 1.01x slower                                                   |
| pickle_pure_python   | 308 us                                                 | 314 us: 1.02x slower                                                   |
| xml_etree_process    | 59.0 ms                                                | 60.7 ms: 1.03x slower                                                  |
| json_loads           | 26.5 us                                                | 27.6 us: 1.04x slower                                                  |
| Geometric mean       | (ref)                                                  | 1.02x faster                                                           |

Benchmark hidden because not significant (1): xml_etree_generate

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260430-vultr-x86_64-python-c0e064003954142b4ba8-3.15.0a8+-c0e0640 |
|------------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| python_startup_no_site | 7.16 ms                                                | 6.84 ms: 1.05x faster                                                  |
| python_startup         | 9.93 ms                                                | 12.5 ms: 1.26x slower                                                  |
| Geometric mean         | (ref)                                                  | 1.10x slower                                                           |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260430-vultr-x86_64-python-c0e064003954142b4ba8-3.15.0a8+-c0e0640 |
|-----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| django_template | 34.7 ms                                                | 35.1 ms: 1.01x slower                                                  |
| mako            | 11.0 ms                                                | 12.4 ms: 1.12x slower                                                  |
| Geometric mean  | (ref)                                                  | 1.07x slower                                                           |

All benchmarks:
===============

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260430-vultr-x86_64-python-c0e064003954142b4ba8-3.15.0a8+-c0e0640 |
|----------------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| pylint                     | 319 ms                                                 | 113 ms: 2.82x faster                                                   |
| mdp                        | 2.42 sec                                               | 1.14 sec: 2.12x faster                                                 |
| async_tree_none            | 464 ms                                                 | 288 ms: 1.61x faster                                                   |
| async_tree_memoization_tg  | 560 ms                                                 | 362 ms: 1.55x faster                                                   |
| async_tree_none_tg         | 446 ms                                                 | 296 ms: 1.51x faster                                                   |
| async_tree_io              | 1.08 sec                                               | 723 ms: 1.50x faster                                                   |
| async_tree_memoization     | 555 ms                                                 | 376 ms: 1.48x faster                                                   |
| deepcopy                   | 352 us                                                 | 240 us: 1.46x faster                                                   |
| deepcopy_memo              | 40.3 us                                                | 27.5 us: 1.46x faster                                                  |
| async_tree_io_tg           | 1.11 sec                                               | 762 ms: 1.46x faster                                                   |
| async_tree_cpu_io_mixed    | 715 ms                                                 | 532 ms: 1.34x faster                                                   |
| async_tree_cpu_io_mixed_tg | 723 ms                                                 | 543 ms: 1.33x faster                                                   |
| go                         | 139 ms                                                 | 106 ms: 1.31x faster                                                   |
| comprehensions             | 19.8 us                                                | 16.1 us: 1.23x faster                                                  |
| pathlib                    | 21.5 ms                                                | 17.8 ms: 1.21x faster                                                  |
| scimark_sor                | 130 ms                                                 | 109 ms: 1.19x faster                                                   |
| raytrace                   | 299 ms                                                 | 253 ms: 1.18x faster                                                   |
| generators                 | 32.2 ms                                                | 27.3 ms: 1.18x faster                                                  |
| spectral_norm              | 110 ms                                                 | 94.6 ms: 1.16x faster                                                  |
| deepcopy_reduce            | 3.08 us                                                | 2.67 us: 1.15x faster                                                  |
| dulwich_log                | 78.9 ms                                                | 68.8 ms: 1.15x faster                                                  |
| tomli_loads                | 2.11 sec                                               | 1.84 sec: 1.14x faster                                                 |
| chaos                      | 62.8 ms                                                | 55.2 ms: 1.14x faster                                                  |
| regex_compile              | 142 ms                                                 | 125 ms: 1.14x faster                                                   |
| bpe_tokeniser              | 4.74 sec                                               | 4.17 sec: 1.14x faster                                                 |
| pyflate                    | 448 ms                                                 | 399 ms: 1.12x faster                                                   |
| async_generators           | 384 ms                                                 | 344 ms: 1.12x faster                                                   |
| crypto_pyaes               | 76.6 ms                                                | 68.7 ms: 1.11x faster                                                  |
| docutils                   | 2.64 sec                                               | 2.38 sec: 1.11x faster                                                 |
| logging_silent             | 109 ns                                                 | 98.7 ns: 1.10x faster                                                  |
| xml_etree_iterparse        | 96.7 ms                                                | 88.1 ms: 1.10x faster                                                  |
| nqueens                    | 80.1 ms                                                | 73.1 ms: 1.10x faster                                                  |
| logging_simple             | 6.63 us                                                | 6.06 us: 1.09x faster                                                  |
| logging_format             | 7.35 us                                                | 6.72 us: 1.09x faster                                                  |
| float                      | 80.8 ms                                                | 74.4 ms: 1.09x faster                                                  |
| scimark_fft                | 342 ms                                                 | 315 ms: 1.08x faster                                                   |
| regex_effbot               | 3.17 ms                                                | 2.93 ms: 1.08x faster                                                  |
| scimark_monte_carlo        | 68.4 ms                                                | 63.4 ms: 1.08x faster                                                  |
| json_dumps                 | 10.4 ms                                                | 9.63 ms: 1.08x faster                                                  |
| sympy_integrate            | 20.5 ms                                                | 19.2 ms: 1.07x faster                                                  |
| html5lib                   | 63.6 ms                                                | 59.6 ms: 1.07x faster                                                  |
| sympy_sum                  | 166 ms                                                 | 157 ms: 1.06x faster                                                   |
| sympy_str                  | 292 ms                                                 | 276 ms: 1.06x faster                                                   |
| richards                   | 45.9 ms                                                | 43.6 ms: 1.05x faster                                                  |
| meteor_contest             | 104 ms                                                 | 98.6 ms: 1.05x faster                                                  |
| python_startup_no_site     | 7.16 ms                                                | 6.84 ms: 1.05x faster                                                  |
| coroutines                 | 23.9 ms                                                | 22.9 ms: 1.05x faster                                                  |
| hexiom                     | 6.17 ms                                                | 5.91 ms: 1.04x faster                                                  |
| richards_super             | 51.9 ms                                                | 49.7 ms: 1.04x faster                                                  |
| typing_runtime_protocols   | 163 us                                                 | 157 us: 1.04x faster                                                   |
| deltablue                  | 3.45 ms                                                | 3.31 ms: 1.04x faster                                                  |
| 2to3                       | 264 ms                                                 | 254 ms: 1.04x faster                                                   |
| scimark_lu                 | 114 ms                                                 | 110 ms: 1.03x faster                                                   |
| thrift                     | 791 us                                                 | 770 us: 1.03x faster                                                   |
| pycparser                  | 1.17 sec                                               | 1.14 sec: 1.03x faster                                                 |
| unpickle_pure_python       | 221 us                                                 | 216 us: 1.02x faster                                                   |
| pprint_safe_repr           | 743 ms                                                 | 727 ms: 1.02x faster                                                   |
| pprint_pformat             | 1.52 sec                                               | 1.49 sec: 1.02x faster                                                 |
| fannkuch                   | 372 ms                                                 | 366 ms: 1.02x faster                                                   |
| sympy_expand               | 468 ms                                                 | 464 ms: 1.01x faster                                                   |
| pidigits                   | 184 ms                                                 | 185 ms: 1.00x slower                                                   |
| django_template            | 34.7 ms                                                | 35.1 ms: 1.01x slower                                                  |
| xml_etree_parse            | 139 ms                                                 | 141 ms: 1.01x slower                                                   |
| pickle_pure_python         | 308 us                                                 | 314 us: 1.02x slower                                                   |
| xml_etree_process          | 59.0 ms                                                | 60.7 ms: 1.03x slower                                                  |
| nbody                      | 89.3 ms                                                | 92.2 ms: 1.03x slower                                                  |
| json_loads                 | 26.5 us                                                | 27.6 us: 1.04x slower                                                  |
| gc_traversal               | 3.46 ms                                                | 3.62 ms: 1.05x slower                                                  |
| asyncio_websockets         | 517 ms                                                 | 543 ms: 1.05x slower                                                   |
| scimark_sparse_mat_mult    | 4.39 ms                                                | 4.66 ms: 1.06x slower                                                  |
| regex_v8                   | 20.6 ms                                                | 22.7 ms: 1.10x slower                                                  |
| regex_dna                  | 168 ms                                                 | 187 ms: 1.11x slower                                                   |
| mako                       | 11.0 ms                                                | 12.4 ms: 1.12x slower                                                  |
| coverage                   | 71.4 ms                                                | 82.1 ms: 1.15x slower                                                  |
| python_startup             | 9.93 ms                                                | 12.5 ms: 1.26x slower                                                  |
| bench_thread_pool          | 941 us                                                 | 1.33 ms: 1.42x slower                                                  |
| create_gc_cycles           | 1.09 ms                                                | 1.74 ms: 1.59x slower                                                  |
| bench_mp_pool              | 10.8 ms                                                | 206 ms: 19.11x slower                                                  |
| telco                      | 6.53 ms                                                | 163 ms: 24.97x slower                                                  |
| Geometric mean             | (ref)                                                  | 1.02x faster                                                           |

Benchmark hidden because not significant (3): xml_etree_generate, sqlite_synth, json
Ignored benchmarks (23) of results/bm-20240906-3.12.6-a4a2d2b/bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b.json: aiohttp, asyncio_tcp, asyncio_tcp_ssl, chameleon, dask, flaskblogging, genshi_text, genshi_xml, gunicorn, mypy2, pickle, pickle_dict, pickle_list, sqlalchemy_declarative, sqlalchemy_imperative, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http, unpack_sequence, unpickle, unpickle_list
Ignored benchmarks (10) of results/bm-20260430-3.15.0a8+-c0e0640/bm-20260430-vultr-x86_64-python-c0e064003954142b4ba8-3.15.0a8+-c0e0640.json: connected_components, k_core, many_optionals, shortest_path, sphinx, sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile, subparsers

- Geometric mean (including insignificant results): 1.067x faster

# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.05x
- 95% likely to have a speedup of 1.05x
- 99% likely to have a speedup of 1.04x

# Memory
- memory change: 1.15x