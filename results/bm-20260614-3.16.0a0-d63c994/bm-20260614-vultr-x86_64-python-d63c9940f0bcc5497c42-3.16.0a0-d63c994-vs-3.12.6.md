# Results vs. 3.12.6

- fork: python
- ref: d63c9940f0bcc5497c42
- machine: linux-x86_64
- commit hash: d63c994
- commit date: 2026-06-14
- overall geometric mean: 1.069x faster
- HPT reliability: 100.00%
- HPT 99th percentile: 1.03x faster
- Memory change: 1.18x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260614-vultr-x86_64-python-d63c9940f0bcc5497c42-3.16.0a0-d63c994 |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| docutils       | 2.64 sec                                               | 2.39 sec: 1.10x faster                                                |
| html5lib       | 63.6 ms                                                | 60.4 ms: 1.05x faster                                                 |
| Geometric mean | (ref)                                                  | 1.08x faster                                                          |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260614-vultr-x86_64-python-d63c9940f0bcc5497c42-3.16.0a0-d63c994 |
|----------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| async_tree_none            | 464 ms                                                 | 294 ms: 1.58x faster                                                  |
| async_tree_memoization_tg  | 560 ms                                                 | 362 ms: 1.54x faster                                                  |
| async_tree_io              | 1.08 sec                                               | 721 ms: 1.50x faster                                                  |
| async_tree_none_tg         | 446 ms                                                 | 298 ms: 1.50x faster                                                  |
| async_tree_memoization     | 555 ms                                                 | 375 ms: 1.48x faster                                                  |
| async_tree_io_tg           | 1.11 sec                                               | 757 ms: 1.47x faster                                                  |
| async_tree_cpu_io_mixed    | 715 ms                                                 | 545 ms: 1.31x faster                                                  |
| async_tree_cpu_io_mixed_tg | 723 ms                                                 | 560 ms: 1.29x faster                                                  |
| async_generators           | 384 ms                                                 | 338 ms: 1.14x faster                                                  |
| coroutines                 | 23.9 ms                                                | 23.6 ms: 1.01x faster                                                 |
| asyncio_websockets         | 517 ms                                                 | 545 ms: 1.05x slower                                                  |
| Geometric mean             | (ref)                                                  | 1.32x faster                                                          |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260614-vultr-x86_64-python-d63c9940f0bcc5497c42-3.16.0a0-d63c994 |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| float          | 80.8 ms                                                | 75.2 ms: 1.07x faster                                                 |
| nbody          | 89.3 ms                                                | 96.5 ms: 1.08x slower                                                 |
| Geometric mean | (ref)                                                  | 1.00x slower                                                          |

Benchmark hidden because not significant (1): pidigits

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260614-vultr-x86_64-python-d63c9940f0bcc5497c42-3.16.0a0-d63c994 |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| regex_effbot   | 3.17 ms                                                | 2.65 ms: 1.19x faster                                                 |
| regex_compile  | 142 ms                                                 | 123 ms: 1.16x faster                                                  |
| regex_v8       | 20.6 ms                                                | 21.0 ms: 1.02x slower                                                 |
| regex_dna      | 168 ms                                                 | 177 ms: 1.05x slower                                                  |
| Geometric mean | (ref)                                                  | 1.06x faster                                                          |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260614-vultr-x86_64-python-d63c9940f0bcc5497c42-3.16.0a0-d63c994 |
|----------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| tomli_loads          | 2.11 sec                                               | 1.83 sec: 1.15x faster                                                |
| xml_etree_iterparse  | 96.7 ms                                                | 90.7 ms: 1.07x faster                                                 |
| json_dumps           | 10.4 ms                                                | 9.81 ms: 1.06x faster                                                 |
| unpickle_pure_python | 221 us                                                 | 215 us: 1.02x faster                                                  |
| pickle_pure_python   | 308 us                                                 | 313 us: 1.02x slower                                                  |
| xml_etree_generate   | 85.2 ms                                                | 87.0 ms: 1.02x slower                                                 |
| json_loads           | 26.5 us                                                | 27.2 us: 1.02x slower                                                 |
| xml_etree_parse      | 139 ms                                                 | 144 ms: 1.04x slower                                                  |
| xml_etree_process    | 59.0 ms                                                | 61.6 ms: 1.05x slower                                                 |
| Geometric mean       | (ref)                                                  | 1.02x faster                                                          |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260614-vultr-x86_64-python-d63c9940f0bcc5497c42-3.16.0a0-d63c994 |
|-----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| django_template | 34.7 ms                                                | 35.7 ms: 1.03x slower                                                 |
| mako            | 11.0 ms                                                | 11.8 ms: 1.08x slower                                                 |
| Geometric mean  | (ref)                                                  | 1.05x slower                                                          |

All benchmarks:
===============

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260614-vultr-x86_64-python-d63c9940f0bcc5497c42-3.16.0a0-d63c994 |
|----------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| pylint                     | 319 ms                                                 | 114 ms: 2.79x faster                                                  |
| mdp                        | 2.42 sec                                               | 1.14 sec: 2.13x faster                                                |
| async_tree_none            | 464 ms                                                 | 294 ms: 1.58x faster                                                  |
| async_tree_memoization_tg  | 560 ms                                                 | 362 ms: 1.54x faster                                                  |
| deepcopy_memo              | 40.3 us                                                | 26.7 us: 1.51x faster                                                 |
| async_tree_io              | 1.08 sec                                               | 721 ms: 1.50x faster                                                  |
| deepcopy                   | 352 us                                                 | 235 us: 1.50x faster                                                  |
| async_tree_none_tg         | 446 ms                                                 | 298 ms: 1.50x faster                                                  |
| async_tree_memoization     | 555 ms                                                 | 375 ms: 1.48x faster                                                  |
| async_tree_io_tg           | 1.11 sec                                               | 757 ms: 1.47x faster                                                  |
| typing_runtime_protocols   | 163 us                                                 | 118 us: 1.38x faster                                                  |
| go                         | 139 ms                                                 | 106 ms: 1.32x faster                                                  |
| async_tree_cpu_io_mixed    | 715 ms                                                 | 545 ms: 1.31x faster                                                  |
| async_tree_cpu_io_mixed_tg | 723 ms                                                 | 560 ms: 1.29x faster                                                  |
| comprehensions             | 19.8 us                                                | 15.9 us: 1.25x faster                                                 |
| pathlib                    | 21.5 ms                                                | 17.5 ms: 1.23x faster                                                 |
| deepcopy_reduce            | 3.08 us                                                | 2.54 us: 1.21x faster                                                 |
| regex_effbot               | 3.17 ms                                                | 2.65 ms: 1.19x faster                                                 |
| spectral_norm              | 110 ms                                                 | 93.7 ms: 1.18x faster                                                 |
| scimark_sor                | 130 ms                                                 | 111 ms: 1.17x faster                                                  |
| regex_compile              | 142 ms                                                 | 123 ms: 1.16x faster                                                  |
| generators                 | 32.2 ms                                                | 28.0 ms: 1.15x faster                                                 |
| tomli_loads                | 2.11 sec                                               | 1.83 sec: 1.15x faster                                                |
| raytrace                   | 299 ms                                                 | 261 ms: 1.15x faster                                                  |
| dulwich_log                | 78.9 ms                                                | 69.2 ms: 1.14x faster                                                 |
| async_generators           | 384 ms                                                 | 338 ms: 1.14x faster                                                  |
| logging_silent             | 109 ns                                                 | 96.7 ns: 1.13x faster                                                 |
| bpe_tokeniser              | 4.74 sec                                               | 4.22 sec: 1.12x faster                                                |
| chaos                      | 62.8 ms                                                | 56.0 ms: 1.12x faster                                                 |
| logging_simple             | 6.63 us                                                | 5.95 us: 1.11x faster                                                 |
| docutils                   | 2.64 sec                                               | 2.39 sec: 1.10x faster                                                |
| pyflate                    | 448 ms                                                 | 407 ms: 1.10x faster                                                  |
| logging_format             | 7.35 us                                                | 6.73 us: 1.09x faster                                                 |
| crypto_pyaes               | 76.6 ms                                                | 70.6 ms: 1.08x faster                                                 |
| nqueens                    | 80.1 ms                                                | 74.1 ms: 1.08x faster                                                 |
| sympy_integrate            | 20.5 ms                                                | 19.1 ms: 1.08x faster                                                 |
| richards                   | 45.9 ms                                                | 42.7 ms: 1.08x faster                                                 |
| float                      | 80.8 ms                                                | 75.2 ms: 1.07x faster                                                 |
| sympy_str                  | 292 ms                                                 | 273 ms: 1.07x faster                                                  |
| sympy_sum                  | 166 ms                                                 | 156 ms: 1.07x faster                                                  |
| hexiom                     | 6.17 ms                                                | 5.78 ms: 1.07x faster                                                 |
| xml_etree_iterparse        | 96.7 ms                                                | 90.7 ms: 1.07x faster                                                 |
| scimark_monte_carlo        | 68.4 ms                                                | 64.7 ms: 1.06x faster                                                 |
| json_dumps                 | 10.4 ms                                                | 9.81 ms: 1.06x faster                                                 |
| html5lib                   | 63.6 ms                                                | 60.4 ms: 1.05x faster                                                 |
| scimark_fft                | 342 ms                                                 | 325 ms: 1.05x faster                                                  |
| richards_super             | 51.9 ms                                                | 49.4 ms: 1.05x faster                                                 |
| meteor_contest             | 104 ms                                                 | 100 ms: 1.03x faster                                                  |
| unpickle_pure_python       | 221 us                                                 | 215 us: 1.02x faster                                                  |
| pprint_safe_repr           | 743 ms                                                 | 730 ms: 1.02x faster                                                  |
| sympy_expand               | 468 ms                                                 | 459 ms: 1.02x faster                                                  |
| deltablue                  | 3.45 ms                                                | 3.39 ms: 1.02x faster                                                 |
| pprint_pformat             | 1.52 sec                                               | 1.50 sec: 1.01x faster                                                |
| coroutines                 | 23.9 ms                                                | 23.6 ms: 1.01x faster                                                 |
| pycparser                  | 1.17 sec                                               | 1.19 sec: 1.02x slower                                                |
| scimark_lu                 | 114 ms                                                 | 116 ms: 1.02x slower                                                  |
| sqlite_synth               | 2.20 us                                                | 2.24 us: 1.02x slower                                                 |
| pickle_pure_python         | 308 us                                                 | 313 us: 1.02x slower                                                  |
| regex_v8                   | 20.6 ms                                                | 21.0 ms: 1.02x slower                                                 |
| xml_etree_generate         | 85.2 ms                                                | 87.0 ms: 1.02x slower                                                 |
| json_loads                 | 26.5 us                                                | 27.2 us: 1.02x slower                                                 |
| django_template            | 34.7 ms                                                | 35.7 ms: 1.03x slower                                                 |
| fannkuch                   | 372 ms                                                 | 385 ms: 1.03x slower                                                  |
| xml_etree_parse            | 139 ms                                                 | 144 ms: 1.04x slower                                                  |
| xml_etree_process          | 59.0 ms                                                | 61.6 ms: 1.05x slower                                                 |
| regex_dna                  | 168 ms                                                 | 177 ms: 1.05x slower                                                  |
| asyncio_websockets         | 517 ms                                                 | 545 ms: 1.05x slower                                                  |
| mako                       | 11.0 ms                                                | 11.8 ms: 1.08x slower                                                 |
| nbody                      | 89.3 ms                                                | 96.5 ms: 1.08x slower                                                 |
| scimark_sparse_mat_mult    | 4.39 ms                                                | 4.82 ms: 1.10x slower                                                 |
| gc_traversal               | 3.46 ms                                                | 3.89 ms: 1.12x slower                                                 |
| coverage                   | 71.4 ms                                                | 83.5 ms: 1.17x slower                                                 |
| bench_thread_pool          | 941 us                                                 | 1.35 ms: 1.43x slower                                                 |
| create_gc_cycles           | 1.09 ms                                                | 1.85 ms: 1.69x slower                                                 |
| telco                      | 6.53 ms                                                | 160 ms: 24.59x slower                                                 |
| bench_mp_pool              | 10.8 ms                                                | 282 ms: 26.11x slower                                                 |
| Geometric mean             | (ref)                                                  | 1.02x faster                                                          |

Benchmark hidden because not significant (3): thrift, pidigits, json
Ignored benchmarks (26) of results/bm-20240906-3.12.6-a4a2d2b/bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b.json: 2to3, aiohttp, asyncio_tcp, asyncio_tcp_ssl, chameleon, dask, flaskblogging, genshi_text, genshi_xml, gunicorn, mypy2, pickle, pickle_dict, pickle_list, python_startup, python_startup_no_site, sqlalchemy_declarative, sqlalchemy_imperative, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http, unpack_sequence, unpickle, unpickle_list
Ignored benchmarks (10) of results/bm-20260614-3.16.0a0-d63c994/bm-20260614-vultr-x86_64-python-d63c9940f0bcc5497c42-3.16.0a0-d63c994.json: connected_components, k_core, many_optionals, shortest_path, sphinx, sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile, subparsers

- Geometric mean (including insignificant results): 1.069x faster

# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.06x
- 95% likely to have a speedup of 1.05x
- 99% likely to have a speedup of 1.03x

# Memory
- memory change: 1.18x