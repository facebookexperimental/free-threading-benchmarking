# Results vs. 3.12.6

- fork: python
- ref: 24e5a55ccb0c5db68136
- machine: darwin-arm64
- commit hash: 24e5a55
- commit date: 2026-08-27
- overall geometric mean: 1.020x faster
- HPT reliability: 65.15%
- HPT 99th percentile: 1.00x slower
- Memory change: 1.31x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260827-macm4pro-arm64-python-24e5a55ccb0c5db68136-3.16.0a0-24e5a55 |
|----------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| 2to3           | 114 ms                                                   | 134 ms: 1.18x slower                                                    |
| docutils       | 1.02 sec                                                 | 1.08 sec: 1.06x slower                                                  |
| html5lib       | 23.0 ms                                                  | 23.9 ms: 1.04x slower                                                   |
| sphinx         | 434 ms                                                   | 458 ms: 1.06x slower                                                    |
| Geometric mean | (ref)                                                    | 1.08x slower                                                            |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                        | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260827-macm4pro-arm64-python-24e5a55ccb0c5db68136-3.16.0a0-24e5a55 |
|----------------------------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| async_tree_eager_io              | 496 ms                                                   | 311 ms: 1.59x faster                                                    |
| async_tree_io_tg                 | 480 ms                                                   | 311 ms: 1.54x faster                                                    |
| async_tree_io                    | 459 ms                                                   | 316 ms: 1.45x faster                                                    |
| async_tree_eager_io_tg           | 446 ms                                                   | 308 ms: 1.45x faster                                                    |
| async_generators                 | 206 ms                                                   | 164 ms: 1.26x faster                                                    |
| async_tree_memoization_tg        | 231 ms                                                   | 186 ms: 1.24x faster                                                    |
| async_tree_none                  | 178 ms                                                   | 146 ms: 1.22x faster                                                    |
| async_tree_none_tg               | 172 ms                                                   | 145 ms: 1.19x faster                                                    |
| async_tree_memoization           | 223 ms                                                   | 190 ms: 1.17x faster                                                    |
| async_tree_cpu_io_mixed_tg       | 338 ms                                                   | 298 ms: 1.13x faster                                                    |
| async_tree_cpu_io_mixed          | 333 ms                                                   | 304 ms: 1.10x faster                                                    |
| async_tree_eager_memoization     | 132 ms                                                   | 127 ms: 1.03x faster                                                    |
| asyncio_websockets               | 190 ms                                                   | 185 ms: 1.03x faster                                                    |
| async_tree_eager_cpu_io_mixed    | 231 ms                                                   | 236 ms: 1.02x slower                                                    |
| async_tree_eager                 | 45.6 ms                                                  | 51.8 ms: 1.14x slower                                                   |
| async_tree_eager_cpu_io_mixed_tg | 213 ms                                                   | 277 ms: 1.30x slower                                                    |
| async_tree_eager_memoization_tg  | 113 ms                                                   | 158 ms: 1.41x slower                                                    |
| async_tree_eager_tg              | 32.1 ms                                                  | 118 ms: 3.69x slower                                                    |
| Geometric mean                   | (ref)                                                    | 1.04x faster                                                            |

Benchmark hidden because not significant (1): coroutines

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260827-macm4pro-arm64-python-24e5a55ccb0c5db68136-3.16.0a0-24e5a55 |
|----------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| float          | 37.9 ms                                                  | 35.0 ms: 1.08x faster                                                   |
| pidigits       | 161 ms                                                   | 165 ms: 1.02x slower                                                    |
| nbody          | 54.2 ms                                                  | 63.1 ms: 1.16x slower                                                   |
| Geometric mean | (ref)                                                    | 1.03x slower                                                            |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260827-macm4pro-arm64-python-24e5a55ccb0c5db68136-3.16.0a0-24e5a55 |
|----------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| regex_effbot   | 1.67 ms                                                  | 1.33 ms: 1.25x faster                                                   |
| regex_dna      | 99.6 ms                                                  | 91.8 ms: 1.09x faster                                                   |
| regex_v8       | 9.59 ms                                                  | 9.14 ms: 1.05x faster                                                   |
| regex_compile  | 54.6 ms                                                  | 68.1 ms: 1.25x slower                                                   |
| Geometric mean | (ref)                                                    | 1.03x faster                                                            |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260827-macm4pro-arm64-python-24e5a55ccb0c5db68136-3.16.0a0-24e5a55 |
|----------------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| xml_etree_iterparse  | 51.6 ms                                                  | 41.2 ms: 1.25x faster                                                   |
| xml_etree_parse      | 67.9 ms                                                  | 57.4 ms: 1.18x faster                                                   |
| json_dumps           | 4.26 ms                                                  | 3.70 ms: 1.15x faster                                                   |
| tomli_loads          | 957 ms                                                   | 938 ms: 1.02x faster                                                    |
| xml_etree_generate   | 38.9 ms                                                  | 39.2 ms: 1.01x slower                                                   |
| unpickle_pure_python | 103 us                                                   | 118 us: 1.15x slower                                                    |
| xml_etree_process    | 26.7 ms                                                  | 31.3 ms: 1.17x slower                                                   |
| pickle_pure_python   | 139 us                                                   | 164 us: 1.18x slower                                                    |
| Geometric mean       | (ref)                                                    | 1.01x faster                                                            |

Benchmark hidden because not significant (1): json_loads

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260827-macm4pro-arm64-python-24e5a55ccb0c5db68136-3.16.0a0-24e5a55 |
|------------------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| python_startup         | 8.01 ms                                                  | 10.2 ms: 1.27x slower                                                   |
| python_startup_no_site | 5.71 ms                                                  | 7.50 ms: 1.31x slower                                                   |
| Geometric mean         | (ref)                                                    | 1.29x slower                                                            |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260827-macm4pro-arm64-python-24e5a55ccb0c5db68136-3.16.0a0-24e5a55 |
|-----------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| mako            | 4.77 ms                                                  | 5.85 ms: 1.23x slower                                                   |
| django_template | 13.6 ms                                                  | 17.8 ms: 1.30x slower                                                   |
| Geometric mean  | (ref)                                                    | 1.26x slower                                                            |

All benchmarks:
===============

| Benchmark                        | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260827-macm4pro-arm64-python-24e5a55ccb0c5db68136-3.16.0a0-24e5a55 |
|----------------------------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| subparsers                       | 20.8 ms                                                  | 4.52 ms: 4.60x faster                                                   |
| gc_traversal                     | 2.01 ms                                                  | 770 us: 2.61x faster                                                    |
| pylint                           | 128 ms                                                   | 54.3 ms: 2.36x faster                                                   |
| mdp                              | 1.09 sec                                                 | 617 ms: 1.77x faster                                                    |
| create_gc_cycles                 | 830 us                                                   | 484 us: 1.71x faster                                                    |
| async_tree_eager_io              | 496 ms                                                   | 311 ms: 1.59x faster                                                    |
| async_tree_io_tg                 | 480 ms                                                   | 311 ms: 1.54x faster                                                    |
| async_tree_io                    | 459 ms                                                   | 316 ms: 1.45x faster                                                    |
| async_tree_eager_io_tg           | 446 ms                                                   | 308 ms: 1.45x faster                                                    |
| deepcopy                         | 161 us                                                   | 118 us: 1.36x faster                                                    |
| async_generators                 | 206 ms                                                   | 164 ms: 1.26x faster                                                    |
| xml_etree_iterparse              | 51.6 ms                                                  | 41.2 ms: 1.25x faster                                                   |
| regex_effbot                     | 1.67 ms                                                  | 1.33 ms: 1.25x faster                                                   |
| async_tree_memoization_tg        | 231 ms                                                   | 186 ms: 1.24x faster                                                    |
| async_tree_none                  | 178 ms                                                   | 146 ms: 1.22x faster                                                    |
| deepcopy_memo                    | 18.3 us                                                  | 15.1 us: 1.21x faster                                                   |
| sqlite_synth                     | 967 ns                                                   | 805 ns: 1.20x faster                                                    |
| async_tree_none_tg               | 172 ms                                                   | 145 ms: 1.19x faster                                                    |
| typing_runtime_protocols         | 71.0 us                                                  | 59.9 us: 1.18x faster                                                   |
| xml_etree_parse                  | 67.9 ms                                                  | 57.4 ms: 1.18x faster                                                   |
| deepcopy_reduce                  | 1.46 us                                                  | 1.25 us: 1.17x faster                                                   |
| async_tree_memoization           | 223 ms                                                   | 190 ms: 1.17x faster                                                    |
| json_dumps                       | 4.26 ms                                                  | 3.70 ms: 1.15x faster                                                   |
| bpe_tokeniser                    | 2.24 sec                                                 | 1.96 sec: 1.14x faster                                                  |
| async_tree_cpu_io_mixed_tg       | 338 ms                                                   | 298 ms: 1.13x faster                                                    |
| pathlib                          | 12.4 ms                                                  | 11.0 ms: 1.12x faster                                                   |
| comprehensions                   | 9.84 us                                                  | 8.82 us: 1.12x faster                                                   |
| k_core                           | 1.12 sec                                                 | 1.01 sec: 1.11x faster                                                  |
| async_tree_cpu_io_mixed          | 333 ms                                                   | 304 ms: 1.10x faster                                                    |
| regex_dna                        | 99.6 ms                                                  | 91.8 ms: 1.09x faster                                                   |
| float                            | 37.9 ms                                                  | 35.0 ms: 1.08x faster                                                   |
| dulwich_log                      | 21.3 ms                                                  | 19.8 ms: 1.07x faster                                                   |
| go                               | 70.0 ms                                                  | 66.1 ms: 1.06x faster                                                   |
| scimark_fft                      | 142 ms                                                   | 134 ms: 1.05x faster                                                    |
| regex_v8                         | 9.59 ms                                                  | 9.14 ms: 1.05x faster                                                   |
| async_tree_eager_memoization     | 132 ms                                                   | 127 ms: 1.03x faster                                                    |
| spectral_norm                    | 54.4 ms                                                  | 52.6 ms: 1.03x faster                                                   |
| asyncio_websockets               | 190 ms                                                   | 185 ms: 1.03x faster                                                    |
| tomli_loads                      | 957 ms                                                   | 938 ms: 1.02x faster                                                    |
| pyflate                          | 216 ms                                                   | 212 ms: 1.02x faster                                                    |
| scimark_sor                      | 61.0 ms                                                  | 61.3 ms: 1.00x slower                                                   |
| xml_etree_generate               | 38.9 ms                                                  | 39.2 ms: 1.01x slower                                                   |
| pycparser                        | 497 ms                                                   | 504 ms: 1.01x slower                                                    |
| pidigits                         | 161 ms                                                   | 165 ms: 1.02x slower                                                    |
| async_tree_eager_cpu_io_mixed    | 231 ms                                                   | 236 ms: 1.02x slower                                                    |
| logging_simple                   | 2.57 us                                                  | 2.66 us: 1.03x slower                                                   |
| html5lib                         | 23.0 ms                                                  | 23.9 ms: 1.04x slower                                                   |
| scimark_sparse_mat_mult          | 2.08 ms                                                  | 2.16 ms: 1.04x slower                                                   |
| logging_format                   | 2.80 us                                                  | 2.95 us: 1.05x slower                                                   |
| sphinx                           | 434 ms                                                   | 458 ms: 1.06x slower                                                    |
| sympy_integrate                  | 8.02 ms                                                  | 8.50 ms: 1.06x slower                                                   |
| docutils                         | 1.02 sec                                                 | 1.08 sec: 1.06x slower                                                  |
| fannkuch                         | 176 ms                                                   | 188 ms: 1.07x slower                                                    |
| sympy_sum                        | 57.6 ms                                                  | 62.3 ms: 1.08x slower                                                   |
| raytrace                         | 145 ms                                                   | 157 ms: 1.08x slower                                                    |
| chaos                            | 28.9 ms                                                  | 31.7 ms: 1.10x slower                                                   |
| sympy_str                        | 104 ms                                                   | 114 ms: 1.10x slower                                                    |
| bench_mp_pool                    | 39.7 ms                                                  | 43.7 ms: 1.10x slower                                                   |
| crypto_pyaes                     | 38.8 ms                                                  | 42.9 ms: 1.10x slower                                                   |
| scimark_monte_carlo              | 32.2 ms                                                  | 35.6 ms: 1.11x slower                                                   |
| thrift                           | 322 us                                                   | 358 us: 1.11x slower                                                    |
| pprint_safe_repr                 | 328 ms                                                   | 366 ms: 1.12x slower                                                    |
| generators                       | 21.9 ms                                                  | 24.5 ms: 1.12x slower                                                   |
| meteor_contest                   | 47.7 ms                                                  | 54.1 ms: 1.13x slower                                                   |
| pprint_pformat                   | 665 ms                                                   | 754 ms: 1.13x slower                                                    |
| async_tree_eager                 | 45.6 ms                                                  | 51.8 ms: 1.14x slower                                                   |
| unpickle_pure_python             | 103 us                                                   | 118 us: 1.15x slower                                                    |
| hexiom                           | 3.04 ms                                                  | 3.51 ms: 1.16x slower                                                   |
| sympy_expand                     | 167 ms                                                   | 194 ms: 1.16x slower                                                    |
| nbody                            | 54.2 ms                                                  | 63.1 ms: 1.16x slower                                                   |
| shortest_path                    | 219 ms                                                   | 255 ms: 1.17x slower                                                    |
| xml_etree_process                | 26.7 ms                                                  | 31.3 ms: 1.17x slower                                                   |
| pickle_pure_python               | 139 us                                                   | 164 us: 1.18x slower                                                    |
| 2to3                             | 114 ms                                                   | 134 ms: 1.18x slower                                                    |
| telco                            | 2.61 ms                                                  | 3.11 ms: 1.19x slower                                                   |
| richards                         | 22.4 ms                                                  | 26.9 ms: 1.20x slower                                                   |
| richards_super                   | 25.4 ms                                                  | 30.6 ms: 1.20x slower                                                   |
| connected_components             | 201 ms                                                   | 245 ms: 1.22x slower                                                    |
| mako                             | 4.77 ms                                                  | 5.85 ms: 1.23x slower                                                   |
| regex_compile                    | 54.6 ms                                                  | 68.1 ms: 1.25x slower                                                   |
| deltablue                        | 1.73 ms                                                  | 2.17 ms: 1.26x slower                                                   |
| python_startup                   | 8.01 ms                                                  | 10.2 ms: 1.27x slower                                                   |
| async_tree_eager_cpu_io_mixed_tg | 213 ms                                                   | 277 ms: 1.30x slower                                                    |
| django_template                  | 13.6 ms                                                  | 17.8 ms: 1.30x slower                                                   |
| python_startup_no_site           | 5.71 ms                                                  | 7.50 ms: 1.31x slower                                                   |
| scimark_lu                       | 51.3 ms                                                  | 68.6 ms: 1.34x slower                                                   |
| many_optionals                   | 195 us                                                   | 268 us: 1.37x slower                                                    |
| bench_thread_pool                | 419 us                                                   | 581 us: 1.39x slower                                                    |
| async_tree_eager_memoization_tg  | 113 ms                                                   | 158 ms: 1.41x slower                                                    |
| coverage                         | 26.9 ms                                                  | 41.1 ms: 1.53x slower                                                   |
| async_tree_eager_tg              | 32.1 ms                                                  | 118 ms: 3.69x slower                                                    |
| Geometric mean                   | (ref)                                                    | 1.02x faster                                                            |

Benchmark hidden because not significant (5): json_loads, json, nqueens, logging_silent, coroutines
Ignored benchmarks (13) of results/bm-20240906-3.12.6-a4a2d2b/bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b.json: chameleon, dask, djangocms, genshi_text, genshi_xml, gevent_hub, sqlalchemy_declarative, sqlalchemy_imperative, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http
Ignored benchmarks (4) of results/bm-20260827-3.16.0a0-24e5a55-NOGIL/bm-20260827-macm4pro-arm64-python-24e5a55ccb0c5db68136-3.16.0a0-24e5a55.json: sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile

- Geometric mean (including insignificant results): 1.020x faster

# HPT report

- Reliability score: 65.15% likely to be slow
- 90% likely to have a slowdown of 1.00x
- 95% likely to have a slowdown of 1.00x
- 99% likely to have a slowdown of 1.00x

# Memory
- memory change: 1.31x