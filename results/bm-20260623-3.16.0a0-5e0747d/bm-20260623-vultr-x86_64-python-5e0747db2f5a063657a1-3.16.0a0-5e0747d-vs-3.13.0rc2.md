# Results vs. 3.13.0rc2

- fork: python
- ref: 5e0747db2f5a063657a1
- machine: linux-x86_64
- commit hash: 5e0747d
- commit date: 2026-06-23
- overall geometric mean: 1.039x faster
- HPT reliability: 100.00%
- HPT 99th percentile: 1.02x faster
- Memory change: 1.13x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5 | bm-20260623-vultr-x86_64-python-5e0747db2f5a063657a1-3.16.0a0-5e0747d |
|----------------|:----------------------------------------------------------------------:|:---------------------------------------------------------------------:|
| 2to3           | 259 ms                                                                 | 254 ms: 1.02x faster                                                  |
| docutils       | 2.63 sec                                                               | 2.39 sec: 1.10x faster                                                |
| html5lib       | 68.6 ms                                                                | 58.3 ms: 1.18x faster                                                 |
| Geometric mean | (ref)                                                                  | 1.10x faster                                                          |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5 | bm-20260623-vultr-x86_64-python-5e0747db2f5a063657a1-3.16.0a0-5e0747d |
|----------------------------|:----------------------------------------------------------------------:|:---------------------------------------------------------------------:|
| async_tree_io              | 881 ms                                                                 | 710 ms: 1.24x faster                                                  |
| async_tree_none            | 353 ms                                                                 | 287 ms: 1.23x faster                                                  |
| async_tree_cpu_io_mixed    | 666 ms                                                                 | 541 ms: 1.23x faster                                                  |
| async_tree_memoization     | 459 ms                                                                 | 374 ms: 1.23x faster                                                  |
| async_tree_io_tg           | 901 ms                                                                 | 755 ms: 1.19x faster                                                  |
| async_tree_cpu_io_mixed_tg | 634 ms                                                                 | 556 ms: 1.14x faster                                                  |
| async_tree_memoization_tg  | 410 ms                                                                 | 362 ms: 1.13x faster                                                  |
| async_tree_none_tg         | 333 ms                                                                 | 296 ms: 1.13x faster                                                  |
| async_generators           | 375 ms                                                                 | 340 ms: 1.10x faster                                                  |
| coroutines                 | 23.3 ms                                                                | 23.0 ms: 1.01x faster                                                 |
| asyncio_websockets         | 517 ms                                                                 | 543 ms: 1.05x slower                                                  |
| Geometric mean             | (ref)                                                                  | 1.14x faster                                                          |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5 | bm-20260623-vultr-x86_64-python-5e0747db2f5a063657a1-3.16.0a0-5e0747d |
|----------------|:----------------------------------------------------------------------:|:---------------------------------------------------------------------:|
| pidigits       | 216 ms                                                                 | 184 ms: 1.18x faster                                                  |
| float          | 76.7 ms                                                                | 74.3 ms: 1.03x faster                                                 |
| nbody          | 85.3 ms                                                                | 93.7 ms: 1.10x slower                                                 |
| Geometric mean | (ref)                                                                  | 1.03x faster                                                          |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5 | bm-20260623-vultr-x86_64-python-5e0747db2f5a063657a1-3.16.0a0-5e0747d |
|----------------|:----------------------------------------------------------------------:|:---------------------------------------------------------------------:|
| regex_effbot   | 3.21 ms                                                                | 2.73 ms: 1.18x faster                                                 |
| regex_compile  | 131 ms                                                                 | 123 ms: 1.07x faster                                                  |
| regex_dna      | 189 ms                                                                 | 177 ms: 1.06x faster                                                  |
| regex_v8       | 23.2 ms                                                                | 21.9 ms: 1.06x faster                                                 |
| Geometric mean | (ref)                                                                  | 1.09x faster                                                          |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5 | bm-20260623-vultr-x86_64-python-5e0747db2f5a063657a1-3.16.0a0-5e0747d |
|----------------------|:----------------------------------------------------------------------:|:---------------------------------------------------------------------:|
| json_dumps           | 10.6 ms                                                                | 9.79 ms: 1.09x faster                                                 |
| tomli_loads          | 2.01 sec                                                               | 1.85 sec: 1.09x faster                                                |
| xml_etree_iterparse  | 94.4 ms                                                                | 89.0 ms: 1.06x faster                                                 |
| json_loads           | 27.3 us                                                                | 27.2 us: 1.01x faster                                                 |
| xml_etree_generate   | 85.4 ms                                                                | 86.2 ms: 1.01x slower                                                 |
| unpickle_pure_python | 208 us                                                                 | 211 us: 1.01x slower                                                  |
| xml_etree_process    | 59.2 ms                                                                | 61.0 ms: 1.03x slower                                                 |
| xml_etree_parse      | 136 ms                                                                 | 143 ms: 1.05x slower                                                  |
| pickle_pure_python   | 292 us                                                                 | 308 us: 1.06x slower                                                  |
| Geometric mean       | (ref)                                                                  | 1.01x faster                                                          |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5 | bm-20260623-vultr-x86_64-python-5e0747db2f5a063657a1-3.16.0a0-5e0747d |
|------------------------|:----------------------------------------------------------------------:|:---------------------------------------------------------------------:|
| python_startup_no_site | 7.41 ms                                                                | 6.91 ms: 1.07x faster                                                 |
| python_startup         | 11.0 ms                                                                | 12.3 ms: 1.11x slower                                                 |
| Geometric mean         | (ref)                                                                  | 1.02x slower                                                          |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5 | bm-20260623-vultr-x86_64-python-5e0747db2f5a063657a1-3.16.0a0-5e0747d |
|-----------------|:----------------------------------------------------------------------:|:---------------------------------------------------------------------:|
| django_template | 34.2 ms                                                                | 34.9 ms: 1.02x slower                                                 |
| mako            | 11.2 ms                                                                | 11.8 ms: 1.05x slower                                                 |
| Geometric mean  | (ref)                                                                  | 1.03x slower                                                          |

All benchmarks:
===============

| Benchmark                  | bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5 | bm-20260623-vultr-x86_64-python-5e0747db2f5a063657a1-3.16.0a0-5e0747d |
|----------------------------|:----------------------------------------------------------------------:|:---------------------------------------------------------------------:|
| pylint                     | 317 ms                                                                 | 114 ms: 2.78x faster                                                  |
| mdp                        | 2.34 sec                                                               | 1.13 sec: 2.07x faster                                                |
| deepcopy                   | 357 us                                                                 | 231 us: 1.54x faster                                                  |
| deepcopy_memo              | 38.1 us                                                                | 26.6 us: 1.43x faster                                                 |
| go                         | 141 ms                                                                 | 102 ms: 1.38x faster                                                  |
| typing_runtime_protocols   | 156 us                                                                 | 117 us: 1.33x faster                                                  |
| scimark_sor                | 134 ms                                                                 | 106 ms: 1.26x faster                                                  |
| async_tree_io              | 881 ms                                                                 | 710 ms: 1.24x faster                                                  |
| async_tree_none            | 353 ms                                                                 | 287 ms: 1.23x faster                                                  |
| async_tree_cpu_io_mixed    | 666 ms                                                                 | 541 ms: 1.23x faster                                                  |
| async_tree_memoization     | 459 ms                                                                 | 374 ms: 1.23x faster                                                  |
| deepcopy_reduce            | 3.12 us                                                                | 2.55 us: 1.22x faster                                                 |
| async_tree_io_tg           | 901 ms                                                                 | 755 ms: 1.19x faster                                                  |
| html5lib                   | 68.6 ms                                                                | 58.3 ms: 1.18x faster                                                 |
| pidigits                   | 216 ms                                                                 | 184 ms: 1.18x faster                                                  |
| regex_effbot               | 3.21 ms                                                                | 2.73 ms: 1.18x faster                                                 |
| spectral_norm              | 108 ms                                                                 | 94.1 ms: 1.14x faster                                                 |
| pyflate                    | 449 ms                                                                 | 393 ms: 1.14x faster                                                  |
| async_tree_cpu_io_mixed_tg | 634 ms                                                                 | 556 ms: 1.14x faster                                                  |
| async_tree_memoization_tg  | 410 ms                                                                 | 362 ms: 1.13x faster                                                  |
| async_tree_none_tg         | 333 ms                                                                 | 296 ms: 1.13x faster                                                  |
| async_generators           | 375 ms                                                                 | 340 ms: 1.10x faster                                                  |
| docutils                   | 2.63 sec                                                               | 2.39 sec: 1.10x faster                                                |
| pathlib                    | 19.3 ms                                                                | 17.7 ms: 1.09x faster                                                 |
| scimark_fft                | 348 ms                                                                 | 321 ms: 1.09x faster                                                  |
| json_dumps                 | 10.6 ms                                                                | 9.79 ms: 1.09x faster                                                 |
| tomli_loads                | 2.01 sec                                                               | 1.85 sec: 1.09x faster                                                |
| dulwich_log                | 74.5 ms                                                                | 69.0 ms: 1.08x faster                                                 |
| python_startup_no_site     | 7.41 ms                                                                | 6.91 ms: 1.07x faster                                                 |
| regex_compile              | 131 ms                                                                 | 123 ms: 1.07x faster                                                  |
| regex_dna                  | 189 ms                                                                 | 177 ms: 1.06x faster                                                  |
| xml_etree_iterparse        | 94.4 ms                                                                | 89.0 ms: 1.06x faster                                                 |
| nqueens                    | 77.7 ms                                                                | 73.4 ms: 1.06x faster                                                 |
| regex_v8                   | 23.2 ms                                                                | 21.9 ms: 1.06x faster                                                 |
| comprehensions             | 16.6 us                                                                | 15.8 us: 1.05x faster                                                 |
| bpe_tokeniser              | 4.46 sec                                                               | 4.24 sec: 1.05x faster                                                |
| sympy_integrate            | 19.7 ms                                                                | 18.8 ms: 1.05x faster                                                 |
| logging_simple             | 6.14 us                                                                | 5.88 us: 1.04x faster                                                 |
| richards                   | 44.4 ms                                                                | 42.5 ms: 1.04x faster                                                 |
| hexiom                     | 5.95 ms                                                                | 5.70 ms: 1.04x faster                                                 |
| richards_super             | 50.4 ms                                                                | 48.5 ms: 1.04x faster                                                 |
| float                      | 76.7 ms                                                                | 74.3 ms: 1.03x faster                                                 |
| logging_format             | 6.92 us                                                                | 6.70 us: 1.03x faster                                                 |
| scimark_monte_carlo        | 65.8 ms                                                                | 63.8 ms: 1.03x faster                                                 |
| chaos                      | 56.3 ms                                                                | 54.9 ms: 1.03x faster                                                 |
| 2to3                       | 259 ms                                                                 | 254 ms: 1.02x faster                                                  |
| pycparser                  | 1.12 sec                                                               | 1.10 sec: 1.02x faster                                                |
| meteor_contest             | 101 ms                                                                 | 99.6 ms: 1.02x faster                                                 |
| logging_silent             | 98.2 ns                                                                | 96.7 ns: 1.02x faster                                                 |
| sqlite_synth               | 2.25 us                                                                | 2.22 us: 1.01x faster                                                 |
| coroutines                 | 23.3 ms                                                                | 23.0 ms: 1.01x faster                                                 |
| sympy_str                  | 274 ms                                                                 | 271 ms: 1.01x faster                                                  |
| generators                 | 28.5 ms                                                                | 28.3 ms: 1.01x faster                                                 |
| json_loads                 | 27.3 us                                                                | 27.2 us: 1.01x faster                                                 |
| pprint_pformat             | 1.46 sec                                                               | 1.45 sec: 1.00x faster                                                |
| raytrace                   | 250 ms                                                                 | 251 ms: 1.01x slower                                                  |
| thrift                     | 772 us                                                                 | 777 us: 1.01x slower                                                  |
| sympy_sum                  | 154 ms                                                                 | 156 ms: 1.01x slower                                                  |
| xml_etree_generate         | 85.4 ms                                                                | 86.2 ms: 1.01x slower                                                 |
| unpickle_pure_python       | 208 us                                                                 | 211 us: 1.01x slower                                                  |
| coverage                   | 82.5 ms                                                                | 83.8 ms: 1.02x slower                                                 |
| sympy_expand               | 454 ms                                                                 | 462 ms: 1.02x slower                                                  |
| django_template            | 34.2 ms                                                                | 34.9 ms: 1.02x slower                                                 |
| scimark_sparse_mat_mult    | 4.75 ms                                                                | 4.86 ms: 1.02x slower                                                 |
| crypto_pyaes               | 68.2 ms                                                                | 69.9 ms: 1.02x slower                                                 |
| scimark_lu                 | 112 ms                                                                 | 115 ms: 1.02x slower                                                  |
| xml_etree_process          | 59.2 ms                                                                | 61.0 ms: 1.03x slower                                                 |
| deltablue                  | 3.10 ms                                                                | 3.24 ms: 1.05x slower                                                 |
| asyncio_websockets         | 517 ms                                                                 | 543 ms: 1.05x slower                                                  |
| mako                       | 11.2 ms                                                                | 11.8 ms: 1.05x slower                                                 |
| xml_etree_parse            | 136 ms                                                                 | 143 ms: 1.05x slower                                                  |
| pickle_pure_python         | 292 us                                                                 | 308 us: 1.06x slower                                                  |
| nbody                      | 85.3 ms                                                                | 93.7 ms: 1.10x slower                                                 |
| python_startup             | 11.0 ms                                                                | 12.3 ms: 1.11x slower                                                 |
| gc_traversal               | 3.32 ms                                                                | 4.02 ms: 1.21x slower                                                 |
| create_gc_cycles           | 1.33 ms                                                                | 1.66 ms: 1.25x slower                                                 |
| bench_thread_pool          | 924 us                                                                 | 1.33 ms: 1.44x slower                                                 |
| bench_mp_pool              | 11.0 ms                                                                | 209 ms: 18.97x slower                                                 |
| telco                      | 7.77 ms                                                                | 158 ms: 20.39x slower                                                 |
| Geometric mean             | (ref)                                                                  | 1.00x slower                                                          |

Benchmark hidden because not significant (3): pprint_safe_repr, fannkuch, json
Ignored benchmarks (20) of results/bm-20240920-3.13.0rc2-4981ec5/bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5.json: aiohttp, asyncio_tcp, asyncio_tcp_ssl, chameleon, dask, flaskblogging, genshi_text, genshi_xml, gunicorn, pickle, pickle_dict, pickle_list, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http, unpack_sequence, unpickle, unpickle_list
Ignored benchmarks (10) of results/bm-20260623-3.16.0a0-5e0747d/bm-20260623-vultr-x86_64-python-5e0747db2f5a063657a1-3.16.0a0-5e0747d.json: connected_components, k_core, many_optionals, shortest_path, sphinx, sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile, subparsers

- Geometric mean (including insignificant results): 1.039x faster

# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.03x
- 95% likely to have a speedup of 1.02x
- 99% likely to have a speedup of 1.02x

# Memory
- memory change: 1.13x