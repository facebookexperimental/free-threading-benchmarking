# Results vs. 3.12.6

- fork: python
- ref: ce916dc50644bb1de940
- machine: linux-x86_64
- commit hash: ce916dc
- commit date: 2026-06-09
- overall geometric mean: 1.069x faster
- HPT reliability: 100.00%
- HPT 99th percentile: 1.04x faster
- Memory change: 1.18x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260609-vultr-x86_64-python-ce916dc50644bb1de940-3.16.0a0-ce916dc |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| docutils       | 2.64 sec                                               | 2.39 sec: 1.10x faster                                                |
| html5lib       | 63.6 ms                                                | 60.0 ms: 1.06x faster                                                 |
| Geometric mean | (ref)                                                  | 1.08x faster                                                          |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260609-vultr-x86_64-python-ce916dc50644bb1de940-3.16.0a0-ce916dc |
|----------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| async_tree_none            | 464 ms                                                 | 288 ms: 1.62x faster                                                  |
| async_tree_memoization_tg  | 560 ms                                                 | 361 ms: 1.55x faster                                                  |
| async_tree_io              | 1.08 sec                                               | 710 ms: 1.52x faster                                                  |
| async_tree_none_tg         | 446 ms                                                 | 297 ms: 1.50x faster                                                  |
| async_tree_memoization     | 555 ms                                                 | 371 ms: 1.50x faster                                                  |
| async_tree_io_tg           | 1.11 sec                                               | 747 ms: 1.49x faster                                                  |
| async_tree_cpu_io_mixed    | 715 ms                                                 | 537 ms: 1.33x faster                                                  |
| async_tree_cpu_io_mixed_tg | 723 ms                                                 | 550 ms: 1.31x faster                                                  |
| async_generators           | 384 ms                                                 | 340 ms: 1.13x faster                                                  |
| coroutines                 | 23.9 ms                                                | 23.2 ms: 1.03x faster                                                 |
| asyncio_websockets         | 517 ms                                                 | 544 ms: 1.05x slower                                                  |
| Geometric mean             | (ref)                                                  | 1.34x faster                                                          |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260609-vultr-x86_64-python-ce916dc50644bb1de940-3.16.0a0-ce916dc |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| float          | 80.8 ms                                                | 74.9 ms: 1.08x faster                                                 |
| nbody          | 89.3 ms                                                | 94.2 ms: 1.06x slower                                                 |
| Geometric mean | (ref)                                                  | 1.01x faster                                                          |

Benchmark hidden because not significant (1): pidigits

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260609-vultr-x86_64-python-ce916dc50644bb1de940-3.16.0a0-ce916dc |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| regex_compile  | 142 ms                                                 | 123 ms: 1.16x faster                                                  |
| regex_effbot   | 3.17 ms                                                | 2.99 ms: 1.06x faster                                                 |
| regex_v8       | 20.6 ms                                                | 22.0 ms: 1.07x slower                                                 |
| regex_dna      | 168 ms                                                 | 180 ms: 1.07x slower                                                  |
| Geometric mean | (ref)                                                  | 1.02x faster                                                          |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260609-vultr-x86_64-python-ce916dc50644bb1de940-3.16.0a0-ce916dc |
|----------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| tomli_loads          | 2.11 sec                                               | 1.81 sec: 1.17x faster                                                |
| xml_etree_iterparse  | 96.7 ms                                                | 88.9 ms: 1.09x faster                                                 |
| json_dumps           | 10.4 ms                                                | 9.89 ms: 1.05x faster                                                 |
| unpickle_pure_python | 221 us                                                 | 215 us: 1.02x faster                                                  |
| xml_etree_generate   | 85.2 ms                                                | 86.5 ms: 1.02x slower                                                 |
| pickle_pure_python   | 308 us                                                 | 313 us: 1.02x slower                                                  |
| xml_etree_parse      | 139 ms                                                 | 143 ms: 1.03x slower                                                  |
| xml_etree_process    | 59.0 ms                                                | 61.0 ms: 1.03x slower                                                 |
| json_loads           | 26.5 us                                                | 27.8 us: 1.05x slower                                                 |
| Geometric mean       | (ref)                                                  | 1.02x faster                                                          |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260609-vultr-x86_64-python-ce916dc50644bb1de940-3.16.0a0-ce916dc |
|-----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| django_template | 34.7 ms                                                | 35.3 ms: 1.02x slower                                                 |
| mako            | 11.0 ms                                                | 12.1 ms: 1.10x slower                                                 |
| Geometric mean  | (ref)                                                  | 1.06x slower                                                          |

All benchmarks:
===============

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260609-vultr-x86_64-python-ce916dc50644bb1de940-3.16.0a0-ce916dc |
|----------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| pylint                     | 319 ms                                                 | 115 ms: 2.78x faster                                                  |
| mdp                        | 2.42 sec                                               | 1.14 sec: 2.12x faster                                                |
| async_tree_none            | 464 ms                                                 | 288 ms: 1.62x faster                                                  |
| async_tree_memoization_tg  | 560 ms                                                 | 361 ms: 1.55x faster                                                  |
| async_tree_io              | 1.08 sec                                               | 710 ms: 1.52x faster                                                  |
| deepcopy_memo              | 40.3 us                                                | 26.7 us: 1.51x faster                                                 |
| async_tree_none_tg         | 446 ms                                                 | 297 ms: 1.50x faster                                                  |
| deepcopy                   | 352 us                                                 | 234 us: 1.50x faster                                                  |
| async_tree_memoization     | 555 ms                                                 | 371 ms: 1.50x faster                                                  |
| async_tree_io_tg           | 1.11 sec                                               | 747 ms: 1.49x faster                                                  |
| typing_runtime_protocols   | 163 us                                                 | 119 us: 1.37x faster                                                  |
| async_tree_cpu_io_mixed    | 715 ms                                                 | 537 ms: 1.33x faster                                                  |
| async_tree_cpu_io_mixed_tg | 723 ms                                                 | 550 ms: 1.31x faster                                                  |
| go                         | 139 ms                                                 | 107 ms: 1.30x faster                                                  |
| comprehensions             | 19.8 us                                                | 15.6 us: 1.27x faster                                                 |
| pathlib                    | 21.5 ms                                                | 17.7 ms: 1.22x faster                                                 |
| deepcopy_reduce            | 3.08 us                                                | 2.55 us: 1.21x faster                                                 |
| spectral_norm              | 110 ms                                                 | 93.0 ms: 1.18x faster                                                 |
| scimark_sor                | 130 ms                                                 | 110 ms: 1.18x faster                                                  |
| tomli_loads                | 2.11 sec                                               | 1.81 sec: 1.17x faster                                                |
| regex_compile              | 142 ms                                                 | 123 ms: 1.16x faster                                                  |
| generators                 | 32.2 ms                                                | 27.8 ms: 1.16x faster                                                 |
| chaos                      | 62.8 ms                                                | 55.1 ms: 1.14x faster                                                 |
| raytrace                   | 299 ms                                                 | 263 ms: 1.14x faster                                                  |
| dulwich_log                | 78.9 ms                                                | 69.3 ms: 1.14x faster                                                 |
| bpe_tokeniser              | 4.74 sec                                               | 4.18 sec: 1.13x faster                                                |
| async_generators           | 384 ms                                                 | 340 ms: 1.13x faster                                                  |
| pyflate                    | 448 ms                                                 | 398 ms: 1.13x faster                                                  |
| docutils                   | 2.64 sec                                               | 2.39 sec: 1.10x faster                                                |
| nqueens                    | 80.1 ms                                                | 73.2 ms: 1.09x faster                                                 |
| xml_etree_iterparse        | 96.7 ms                                                | 88.9 ms: 1.09x faster                                                 |
| crypto_pyaes               | 76.6 ms                                                | 70.6 ms: 1.09x faster                                                 |
| logging_silent             | 109 ns                                                 | 100 ns: 1.09x faster                                                  |
| float                      | 80.8 ms                                                | 74.9 ms: 1.08x faster                                                 |
| logging_simple             | 6.63 us                                                | 6.15 us: 1.08x faster                                                 |
| sympy_integrate            | 20.5 ms                                                | 19.1 ms: 1.07x faster                                                 |
| hexiom                     | 6.17 ms                                                | 5.76 ms: 1.07x faster                                                 |
| sympy_str                  | 292 ms                                                 | 273 ms: 1.07x faster                                                  |
| logging_format             | 7.35 us                                                | 6.89 us: 1.07x faster                                                 |
| scimark_monte_carlo        | 68.4 ms                                                | 64.5 ms: 1.06x faster                                                 |
| sympy_sum                  | 166 ms                                                 | 157 ms: 1.06x faster                                                  |
| html5lib                   | 63.6 ms                                                | 60.0 ms: 1.06x faster                                                 |
| regex_effbot               | 3.17 ms                                                | 2.99 ms: 1.06x faster                                                 |
| richards                   | 45.9 ms                                                | 43.8 ms: 1.05x faster                                                 |
| pycparser                  | 1.17 sec                                               | 1.11 sec: 1.05x faster                                                |
| json_dumps                 | 10.4 ms                                                | 9.89 ms: 1.05x faster                                                 |
| scimark_fft                | 342 ms                                                 | 328 ms: 1.04x faster                                                  |
| meteor_contest             | 104 ms                                                 | 99.9 ms: 1.04x faster                                                 |
| richards_super             | 51.9 ms                                                | 50.1 ms: 1.04x faster                                                 |
| coroutines                 | 23.9 ms                                                | 23.2 ms: 1.03x faster                                                 |
| unpickle_pure_python       | 221 us                                                 | 215 us: 1.02x faster                                                  |
| pprint_pformat             | 1.52 sec                                               | 1.49 sec: 1.02x faster                                                |
| pprint_safe_repr           | 743 ms                                                 | 732 ms: 1.01x faster                                                  |
| sympy_expand               | 468 ms                                                 | 461 ms: 1.01x faster                                                  |
| thrift                     | 791 us                                                 | 781 us: 1.01x faster                                                  |
| fannkuch                   | 372 ms                                                 | 376 ms: 1.01x slower                                                  |
| sqlite_synth               | 2.20 us                                                | 2.23 us: 1.01x slower                                                 |
| xml_etree_generate         | 85.2 ms                                                | 86.5 ms: 1.02x slower                                                 |
| pickle_pure_python         | 308 us                                                 | 313 us: 1.02x slower                                                  |
| django_template            | 34.7 ms                                                | 35.3 ms: 1.02x slower                                                 |
| xml_etree_parse            | 139 ms                                                 | 143 ms: 1.03x slower                                                  |
| json                       | 5.02 ms                                                | 5.20 ms: 1.03x slower                                                 |
| xml_etree_process          | 59.0 ms                                                | 61.0 ms: 1.03x slower                                                 |
| json_loads                 | 26.5 us                                                | 27.8 us: 1.05x slower                                                 |
| asyncio_websockets         | 517 ms                                                 | 544 ms: 1.05x slower                                                  |
| nbody                      | 89.3 ms                                                | 94.2 ms: 1.06x slower                                                 |
| regex_v8                   | 20.6 ms                                                | 22.0 ms: 1.07x slower                                                 |
| regex_dna                  | 168 ms                                                 | 180 ms: 1.07x slower                                                  |
| mako                       | 11.0 ms                                                | 12.1 ms: 1.10x slower                                                 |
| gc_traversal               | 3.46 ms                                                | 3.84 ms: 1.11x slower                                                 |
| scimark_sparse_mat_mult    | 4.39 ms                                                | 4.91 ms: 1.12x slower                                                 |
| coverage                   | 71.4 ms                                                | 83.8 ms: 1.17x slower                                                 |
| bench_thread_pool          | 941 us                                                 | 1.35 ms: 1.44x slower                                                 |
| create_gc_cycles           | 1.09 ms                                                | 1.80 ms: 1.65x slower                                                 |
| bench_mp_pool              | 10.8 ms                                                | 255 ms: 23.60x slower                                                 |
| telco                      | 6.53 ms                                                | 161 ms: 24.74x slower                                                 |
| Geometric mean             | (ref)                                                  | 1.02x faster                                                          |

Benchmark hidden because not significant (3): deltablue, scimark_lu, pidigits
Ignored benchmarks (26) of results/bm-20240906-3.12.6-a4a2d2b/bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b.json: 2to3, aiohttp, asyncio_tcp, asyncio_tcp_ssl, chameleon, dask, flaskblogging, genshi_text, genshi_xml, gunicorn, mypy2, pickle, pickle_dict, pickle_list, python_startup, python_startup_no_site, sqlalchemy_declarative, sqlalchemy_imperative, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http, unpack_sequence, unpickle, unpickle_list
Ignored benchmarks (10) of results/bm-20260609-3.16.0a0-ce916dc/bm-20260609-vultr-x86_64-python-ce916dc50644bb1de940-3.16.0a0-ce916dc.json: connected_components, k_core, many_optionals, shortest_path, sphinx, sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile, subparsers

- Geometric mean (including insignificant results): 1.069x faster

# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.05x
- 95% likely to have a speedup of 1.05x
- 99% likely to have a speedup of 1.04x

# Memory
- memory change: 1.18x