# Results vs. 3.12.6

- fork: python
- ref: 89fe9857dd0832de9b59
- machine: darwin-arm64
- commit hash: 89fe985
- commit date: 2026-09-14
- overall geometric mean: 1.001x faster
- HPT reliability: 81.06%
- HPT 99th percentile: 1.00x slower
- Memory change: 1.35x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260914-macm4pro-arm64-python-89fe9857dd0832de9b59-3.16.0a0-89fe985 |
|----------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| 2to3           | 114 ms                                                   | 137 ms: 1.20x slower                                                    |
| docutils       | 1.02 sec                                                 | 1.11 sec: 1.08x slower                                                  |
| html5lib       | 23.0 ms                                                  | 24.2 ms: 1.05x slower                                                   |
| sphinx         | 434 ms                                                   | 464 ms: 1.07x slower                                                    |
| Geometric mean | (ref)                                                    | 1.10x slower                                                            |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                        | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260914-macm4pro-arm64-python-89fe9857dd0832de9b59-3.16.0a0-89fe985 |
|----------------------------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| async_tree_eager_io              | 496 ms                                                   | 318 ms: 1.56x faster                                                    |
| async_tree_io_tg                 | 480 ms                                                   | 315 ms: 1.53x faster                                                    |
| async_tree_eager_io_tg           | 446 ms                                                   | 307 ms: 1.45x faster                                                    |
| async_tree_io                    | 459 ms                                                   | 323 ms: 1.42x faster                                                    |
| async_generators                 | 206 ms                                                   | 167 ms: 1.24x faster                                                    |
| async_tree_memoization_tg        | 231 ms                                                   | 187 ms: 1.23x faster                                                    |
| async_tree_none_tg               | 172 ms                                                   | 149 ms: 1.16x faster                                                    |
| async_tree_cpu_io_mixed_tg       | 338 ms                                                   | 301 ms: 1.13x faster                                                    |
| async_tree_memoization           | 223 ms                                                   | 205 ms: 1.09x faster                                                    |
| async_tree_none                  | 178 ms                                                   | 164 ms: 1.08x faster                                                    |
| async_tree_cpu_io_mixed          | 333 ms                                                   | 312 ms: 1.07x faster                                                    |
| coroutines                       | 13.6 ms                                                  | 12.8 ms: 1.06x faster                                                   |
| asyncio_websockets               | 190 ms                                                   | 186 ms: 1.03x faster                                                    |
| async_tree_eager_cpu_io_mixed    | 231 ms                                                   | 262 ms: 1.13x slower                                                    |
| async_tree_eager_memoization     | 132 ms                                                   | 152 ms: 1.15x slower                                                    |
| async_tree_eager_cpu_io_mixed_tg | 213 ms                                                   | 280 ms: 1.32x slower                                                    |
| async_tree_eager_memoization_tg  | 113 ms                                                   | 168 ms: 1.49x slower                                                    |
| async_tree_eager                 | 45.6 ms                                                  | 71.4 ms: 1.57x slower                                                   |
| async_tree_eager_tg              | 32.1 ms                                                  | 120 ms: 3.75x slower                                                    |
| Geometric mean                   | (ref)                                                    | 1.01x slower                                                            |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260914-macm4pro-arm64-python-89fe9857dd0832de9b59-3.16.0a0-89fe985 |
|----------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| float          | 37.9 ms                                                  | 35.5 ms: 1.07x faster                                                   |
| pidigits       | 161 ms                                                   | 168 ms: 1.04x slower                                                    |
| nbody          | 54.2 ms                                                  | 61.9 ms: 1.14x slower                                                   |
| Geometric mean | (ref)                                                    | 1.04x slower                                                            |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260914-macm4pro-arm64-python-89fe9857dd0832de9b59-3.16.0a0-89fe985 |
|----------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| regex_effbot   | 1.67 ms                                                  | 1.37 ms: 1.21x faster                                                   |
| regex_dna      | 99.6 ms                                                  | 93.1 ms: 1.07x faster                                                   |
| regex_v8       | 9.59 ms                                                  | 9.23 ms: 1.04x faster                                                   |
| regex_compile  | 54.6 ms                                                  | 69.8 ms: 1.28x slower                                                   |
| Geometric mean | (ref)                                                    | 1.01x faster                                                            |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260914-macm4pro-arm64-python-89fe9857dd0832de9b59-3.16.0a0-89fe985 |
|----------------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| xml_etree_iterparse  | 51.6 ms                                                  | 41.4 ms: 1.24x faster                                                   |
| xml_etree_parse      | 67.9 ms                                                  | 56.9 ms: 1.19x faster                                                   |
| json_dumps           | 4.26 ms                                                  | 3.78 ms: 1.13x faster                                                   |
| json_loads           | 10.9 us                                                  | 11.1 us: 1.02x slower                                                   |
| xml_etree_generate   | 38.9 ms                                                  | 39.9 ms: 1.03x slower                                                   |
| unpickle_pure_python | 103 us                                                   | 117 us: 1.14x slower                                                    |
| xml_etree_process    | 26.7 ms                                                  | 31.5 ms: 1.18x slower                                                   |
| pickle_pure_python   | 139 us                                                   | 167 us: 1.20x slower                                                    |
| Geometric mean       | (ref)                                                    | 1.00x faster                                                            |

Benchmark hidden because not significant (1): tomli_loads

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260914-macm4pro-arm64-python-89fe9857dd0832de9b59-3.16.0a0-89fe985 |
|------------------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| python_startup         | 8.01 ms                                                  | 10.4 ms: 1.30x slower                                                   |
| python_startup_no_site | 5.71 ms                                                  | 7.66 ms: 1.34x slower                                                   |
| Geometric mean         | (ref)                                                    | 1.32x slower                                                            |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260914-macm4pro-arm64-python-89fe9857dd0832de9b59-3.16.0a0-89fe985 |
|-----------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| mako            | 4.77 ms                                                  | 6.14 ms: 1.29x slower                                                   |
| django_template | 13.6 ms                                                  | 18.0 ms: 1.32x slower                                                   |
| Geometric mean  | (ref)                                                    | 1.31x slower                                                            |

All benchmarks:
===============

| Benchmark                        | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260914-macm4pro-arm64-python-89fe9857dd0832de9b59-3.16.0a0-89fe985 |
|----------------------------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| subparsers                       | 20.8 ms                                                  | 4.65 ms: 4.47x faster                                                   |
| gc_traversal                     | 2.01 ms                                                  | 786 us: 2.56x faster                                                    |
| pylint                           | 128 ms                                                   | 54.5 ms: 2.35x faster                                                   |
| mdp                              | 1.09 sec                                                 | 630 ms: 1.73x faster                                                    |
| create_gc_cycles                 | 830 us                                                   | 493 us: 1.68x faster                                                    |
| async_tree_eager_io              | 496 ms                                                   | 318 ms: 1.56x faster                                                    |
| async_tree_io_tg                 | 480 ms                                                   | 315 ms: 1.53x faster                                                    |
| async_tree_eager_io_tg           | 446 ms                                                   | 307 ms: 1.45x faster                                                    |
| async_tree_io                    | 459 ms                                                   | 323 ms: 1.42x faster                                                    |
| deepcopy                         | 161 us                                                   | 122 us: 1.32x faster                                                    |
| xml_etree_iterparse              | 51.6 ms                                                  | 41.4 ms: 1.24x faster                                                   |
| async_generators                 | 206 ms                                                   | 167 ms: 1.24x faster                                                    |
| async_tree_memoization_tg        | 231 ms                                                   | 187 ms: 1.23x faster                                                    |
| regex_effbot                     | 1.67 ms                                                  | 1.37 ms: 1.21x faster                                                   |
| xml_etree_parse                  | 67.9 ms                                                  | 56.9 ms: 1.19x faster                                                   |
| sqlite_synth                     | 967 ns                                                   | 820 ns: 1.18x faster                                                    |
| typing_runtime_protocols         | 71.0 us                                                  | 61.2 us: 1.16x faster                                                   |
| async_tree_none_tg               | 172 ms                                                   | 149 ms: 1.16x faster                                                    |
| deepcopy_memo                    | 18.3 us                                                  | 16.0 us: 1.14x faster                                                   |
| deepcopy_reduce                  | 1.46 us                                                  | 1.28 us: 1.14x faster                                                   |
| json_dumps                       | 4.26 ms                                                  | 3.78 ms: 1.13x faster                                                   |
| async_tree_cpu_io_mixed_tg       | 338 ms                                                   | 301 ms: 1.13x faster                                                    |
| bpe_tokeniser                    | 2.24 sec                                                 | 1.99 sec: 1.12x faster                                                  |
| pathlib                          | 12.4 ms                                                  | 11.2 ms: 1.10x faster                                                   |
| comprehensions                   | 9.84 us                                                  | 8.94 us: 1.10x faster                                                   |
| k_core                           | 1.12 sec                                                 | 1.02 sec: 1.09x faster                                                  |
| async_tree_memoization           | 223 ms                                                   | 205 ms: 1.09x faster                                                    |
| async_tree_none                  | 178 ms                                                   | 164 ms: 1.08x faster                                                    |
| regex_dna                        | 99.6 ms                                                  | 93.1 ms: 1.07x faster                                                   |
| async_tree_cpu_io_mixed          | 333 ms                                                   | 312 ms: 1.07x faster                                                    |
| dulwich_log                      | 21.3 ms                                                  | 19.9 ms: 1.07x faster                                                   |
| float                            | 37.9 ms                                                  | 35.5 ms: 1.07x faster                                                   |
| coroutines                       | 13.6 ms                                                  | 12.8 ms: 1.06x faster                                                   |
| scimark_fft                      | 142 ms                                                   | 136 ms: 1.04x faster                                                    |
| spectral_norm                    | 54.4 ms                                                  | 52.3 ms: 1.04x faster                                                   |
| regex_v8                         | 9.59 ms                                                  | 9.23 ms: 1.04x faster                                                   |
| go                               | 70.0 ms                                                  | 67.4 ms: 1.04x faster                                                   |
| asyncio_websockets               | 190 ms                                                   | 186 ms: 1.03x faster                                                    |
| pyflate                          | 216 ms                                                   | 214 ms: 1.01x faster                                                    |
| pycparser                        | 497 ms                                                   | 503 ms: 1.01x slower                                                    |
| scimark_sparse_mat_mult          | 2.08 ms                                                  | 2.10 ms: 1.01x slower                                                   |
| scimark_sor                      | 61.0 ms                                                  | 62.1 ms: 1.02x slower                                                   |
| raytrace                         | 145 ms                                                   | 148 ms: 1.02x slower                                                    |
| json_loads                       | 10.9 us                                                  | 11.1 us: 1.02x slower                                                   |
| xml_etree_generate               | 38.9 ms                                                  | 39.9 ms: 1.03x slower                                                   |
| json                             | 1.93 ms                                                  | 1.99 ms: 1.03x slower                                                   |
| nqueens                          | 43.5 ms                                                  | 44.9 ms: 1.03x slower                                                   |
| logging_simple                   | 2.57 us                                                  | 2.68 us: 1.04x slower                                                   |
| pidigits                         | 161 ms                                                   | 168 ms: 1.04x slower                                                    |
| html5lib                         | 23.0 ms                                                  | 24.2 ms: 1.05x slower                                                   |
| logging_silent                   | 50.9 ns                                                  | 53.5 ns: 1.05x slower                                                   |
| logging_format                   | 2.80 us                                                  | 2.95 us: 1.05x slower                                                   |
| sympy_integrate                  | 8.02 ms                                                  | 8.56 ms: 1.07x slower                                                   |
| chaos                            | 28.9 ms                                                  | 30.9 ms: 1.07x slower                                                   |
| sphinx                           | 434 ms                                                   | 464 ms: 1.07x slower                                                    |
| fannkuch                         | 176 ms                                                   | 188 ms: 1.07x slower                                                    |
| docutils                         | 1.02 sec                                                 | 1.11 sec: 1.08x slower                                                  |
| crypto_pyaes                     | 38.8 ms                                                  | 42.4 ms: 1.09x slower                                                   |
| sympy_sum                        | 57.6 ms                                                  | 63.3 ms: 1.10x slower                                                   |
| scimark_monte_carlo              | 32.2 ms                                                  | 35.5 ms: 1.10x slower                                                   |
| sympy_str                        | 104 ms                                                   | 115 ms: 1.11x slower                                                    |
| thrift                           | 322 us                                                   | 360 us: 1.12x slower                                                    |
| bench_mp_pool                    | 39.7 ms                                                  | 44.7 ms: 1.13x slower                                                   |
| async_tree_eager_cpu_io_mixed    | 231 ms                                                   | 262 ms: 1.13x slower                                                    |
| unpickle_pure_python             | 103 us                                                   | 117 us: 1.14x slower                                                    |
| nbody                            | 54.2 ms                                                  | 61.9 ms: 1.14x slower                                                   |
| async_tree_eager_memoization     | 132 ms                                                   | 152 ms: 1.15x slower                                                    |
| generators                       | 21.9 ms                                                  | 25.3 ms: 1.15x slower                                                   |
| meteor_contest                   | 47.7 ms                                                  | 55.3 ms: 1.16x slower                                                   |
| shortest_path                    | 219 ms                                                   | 255 ms: 1.16x slower                                                    |
| pprint_safe_repr                 | 328 ms                                                   | 382 ms: 1.16x slower                                                    |
| pprint_pformat                   | 665 ms                                                   | 779 ms: 1.17x slower                                                    |
| hexiom                           | 3.04 ms                                                  | 3.56 ms: 1.17x slower                                                   |
| xml_etree_process                | 26.7 ms                                                  | 31.5 ms: 1.18x slower                                                   |
| deltablue                        | 1.73 ms                                                  | 2.03 ms: 1.18x slower                                                   |
| sympy_expand                     | 167 ms                                                   | 197 ms: 1.18x slower                                                    |
| pickle_pure_python               | 139 us                                                   | 167 us: 1.20x slower                                                    |
| 2to3                             | 114 ms                                                   | 137 ms: 1.20x slower                                                    |
| telco                            | 2.61 ms                                                  | 3.13 ms: 1.20x slower                                                   |
| richards                         | 22.4 ms                                                  | 27.0 ms: 1.21x slower                                                   |
| richards_super                   | 25.4 ms                                                  | 30.6 ms: 1.21x slower                                                   |
| connected_components             | 201 ms                                                   | 244 ms: 1.22x slower                                                    |
| regex_compile                    | 54.6 ms                                                  | 69.8 ms: 1.28x slower                                                   |
| mako                             | 4.77 ms                                                  | 6.14 ms: 1.29x slower                                                   |
| python_startup                   | 8.01 ms                                                  | 10.4 ms: 1.30x slower                                                   |
| async_tree_eager_cpu_io_mixed_tg | 213 ms                                                   | 280 ms: 1.32x slower                                                    |
| django_template                  | 13.6 ms                                                  | 18.0 ms: 1.32x slower                                                   |
| python_startup_no_site           | 5.71 ms                                                  | 7.66 ms: 1.34x slower                                                   |
| bench_thread_pool                | 419 us                                                   | 567 us: 1.35x slower                                                    |
| scimark_lu                       | 51.3 ms                                                  | 70.9 ms: 1.38x slower                                                   |
| many_optionals                   | 195 us                                                   | 274 us: 1.41x slower                                                    |
| async_tree_eager_memoization_tg  | 113 ms                                                   | 168 ms: 1.49x slower                                                    |
| coverage                         | 26.9 ms                                                  | 40.9 ms: 1.52x slower                                                   |
| async_tree_eager                 | 45.6 ms                                                  | 71.4 ms: 1.57x slower                                                   |
| async_tree_eager_tg              | 32.1 ms                                                  | 120 ms: 3.75x slower                                                    |
| Geometric mean                   | (ref)                                                    | 1.00x slower                                                            |

Benchmark hidden because not significant (1): tomli_loads
Ignored benchmarks (13) of results/bm-20240906-3.12.6-a4a2d2b/bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b.json: chameleon, dask, djangocms, genshi_text, genshi_xml, gevent_hub, sqlalchemy_declarative, sqlalchemy_imperative, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http
Ignored benchmarks (4) of results/bm-20260914-3.16.0a0-89fe985-NOGIL/bm-20260914-macm4pro-arm64-python-89fe9857dd0832de9b59-3.16.0a0-89fe985.json: sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile

- Geometric mean (including insignificant results): 1.001x faster

# HPT report

- Reliability score: 81.06% likely to be slow
- 90% likely to have a slowdown of 1.00x
- 95% likely to have a slowdown of 1.00x
- 99% likely to have a slowdown of 1.00x

# Memory
- memory change: 1.35x