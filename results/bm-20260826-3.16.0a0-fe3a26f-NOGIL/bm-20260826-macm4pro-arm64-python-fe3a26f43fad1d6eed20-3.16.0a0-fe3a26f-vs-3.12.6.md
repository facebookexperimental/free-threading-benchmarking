# Results vs. 3.12.6

- fork: python
- ref: fe3a26f43fad1d6eed20
- machine: darwin-arm64
- commit hash: fe3a26f
- commit date: 2026-08-26
- overall geometric mean: 1.015x faster
- HPT reliability: 64.86%
- HPT 99th percentile: 1.00x slower
- Memory change: 1.31x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260826-macm4pro-arm64-python-fe3a26f43fad1d6eed20-3.16.0a0-fe3a26f |
|----------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| 2to3           | 114 ms                                                   | 137 ms: 1.20x slower                                                    |
| docutils       | 1.02 sec                                                 | 1.10 sec: 1.07x slower                                                  |
| html5lib       | 23.0 ms                                                  | 24.2 ms: 1.05x slower                                                   |
| sphinx         | 434 ms                                                   | 459 ms: 1.06x slower                                                    |
| Geometric mean | (ref)                                                    | 1.10x slower                                                            |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                        | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260826-macm4pro-arm64-python-fe3a26f43fad1d6eed20-3.16.0a0-fe3a26f |
|----------------------------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| async_tree_eager_io              | 496 ms                                                   | 315 ms: 1.58x faster                                                    |
| async_tree_io_tg                 | 480 ms                                                   | 315 ms: 1.52x faster                                                    |
| async_tree_io                    | 459 ms                                                   | 320 ms: 1.44x faster                                                    |
| async_tree_eager_io_tg           | 446 ms                                                   | 311 ms: 1.43x faster                                                    |
| async_generators                 | 206 ms                                                   | 166 ms: 1.24x faster                                                    |
| async_tree_memoization_tg        | 231 ms                                                   | 187 ms: 1.23x faster                                                    |
| async_tree_none                  | 178 ms                                                   | 149 ms: 1.20x faster                                                    |
| async_tree_none_tg               | 172 ms                                                   | 147 ms: 1.17x faster                                                    |
| async_tree_memoization           | 223 ms                                                   | 192 ms: 1.16x faster                                                    |
| async_tree_cpu_io_mixed_tg       | 338 ms                                                   | 303 ms: 1.12x faster                                                    |
| async_tree_cpu_io_mixed          | 333 ms                                                   | 308 ms: 1.08x faster                                                    |
| coroutines                       | 13.6 ms                                                  | 13.1 ms: 1.03x faster                                                   |
| async_tree_eager_memoization     | 132 ms                                                   | 128 ms: 1.03x faster                                                    |
| asyncio_websockets               | 190 ms                                                   | 185 ms: 1.03x faster                                                    |
| async_tree_eager_cpu_io_mixed    | 231 ms                                                   | 238 ms: 1.03x slower                                                    |
| async_tree_eager                 | 45.6 ms                                                  | 51.9 ms: 1.14x slower                                                   |
| async_tree_eager_cpu_io_mixed_tg | 213 ms                                                   | 281 ms: 1.32x slower                                                    |
| async_tree_eager_memoization_tg  | 113 ms                                                   | 160 ms: 1.42x slower                                                    |
| async_tree_eager_tg              | 32.1 ms                                                  | 119 ms: 3.71x slower                                                    |
| Geometric mean                   | (ref)                                                    | 1.04x faster                                                            |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260826-macm4pro-arm64-python-fe3a26f43fad1d6eed20-3.16.0a0-fe3a26f |
|----------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| float          | 37.9 ms                                                  | 35.0 ms: 1.08x faster                                                   |
| pidigits       | 161 ms                                                   | 165 ms: 1.02x slower                                                    |
| nbody          | 54.2 ms                                                  | 62.0 ms: 1.14x slower                                                   |
| Geometric mean | (ref)                                                    | 1.03x slower                                                            |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260826-macm4pro-arm64-python-fe3a26f43fad1d6eed20-3.16.0a0-fe3a26f |
|----------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| regex_effbot   | 1.67 ms                                                  | 1.37 ms: 1.21x faster                                                   |
| regex_dna      | 99.6 ms                                                  | 92.2 ms: 1.08x faster                                                   |
| regex_v8       | 9.59 ms                                                  | 9.09 ms: 1.05x faster                                                   |
| regex_compile  | 54.6 ms                                                  | 68.4 ms: 1.25x slower                                                   |
| Geometric mean | (ref)                                                    | 1.02x faster                                                            |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260826-macm4pro-arm64-python-fe3a26f43fad1d6eed20-3.16.0a0-fe3a26f |
|----------------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| xml_etree_iterparse  | 51.6 ms                                                  | 43.0 ms: 1.20x faster                                                   |
| json_dumps           | 4.26 ms                                                  | 3.76 ms: 1.13x faster                                                   |
| xml_etree_parse      | 67.9 ms                                                  | 62.1 ms: 1.09x faster                                                   |
| tomli_loads          | 957 ms                                                   | 944 ms: 1.01x faster                                                    |
| xml_etree_generate   | 38.9 ms                                                  | 39.4 ms: 1.01x slower                                                   |
| unpickle_pure_python | 103 us                                                   | 119 us: 1.16x slower                                                    |
| xml_etree_process    | 26.7 ms                                                  | 31.4 ms: 1.17x slower                                                   |
| pickle_pure_python   | 139 us                                                   | 164 us: 1.18x slower                                                    |
| Geometric mean       | (ref)                                                    | 1.01x slower                                                            |

Benchmark hidden because not significant (1): json_loads

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260826-macm4pro-arm64-python-fe3a26f43fad1d6eed20-3.16.0a0-fe3a26f |
|------------------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| python_startup         | 8.01 ms                                                  | 10.2 ms: 1.27x slower                                                   |
| python_startup_no_site | 5.71 ms                                                  | 7.51 ms: 1.32x slower                                                   |
| Geometric mean         | (ref)                                                    | 1.29x slower                                                            |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260826-macm4pro-arm64-python-fe3a26f43fad1d6eed20-3.16.0a0-fe3a26f |
|-----------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| mako            | 4.77 ms                                                  | 5.85 ms: 1.22x slower                                                   |
| django_template | 13.6 ms                                                  | 17.9 ms: 1.32x slower                                                   |
| Geometric mean  | (ref)                                                    | 1.27x slower                                                            |

All benchmarks:
===============

| Benchmark                        | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260826-macm4pro-arm64-python-fe3a26f43fad1d6eed20-3.16.0a0-fe3a26f |
|----------------------------------|:--------------------------------------------------------:|:-----------------------------------------------------------------------:|
| subparsers                       | 20.8 ms                                                  | 4.55 ms: 4.56x faster                                                   |
| gc_traversal                     | 2.01 ms                                                  | 773 us: 2.60x faster                                                    |
| pylint                           | 128 ms                                                   | 54.4 ms: 2.35x faster                                                   |
| mdp                              | 1.09 sec                                                 | 624 ms: 1.75x faster                                                    |
| create_gc_cycles                 | 830 us                                                   | 484 us: 1.71x faster                                                    |
| async_tree_eager_io              | 496 ms                                                   | 315 ms: 1.58x faster                                                    |
| async_tree_io_tg                 | 480 ms                                                   | 315 ms: 1.52x faster                                                    |
| async_tree_io                    | 459 ms                                                   | 320 ms: 1.44x faster                                                    |
| async_tree_eager_io_tg           | 446 ms                                                   | 311 ms: 1.43x faster                                                    |
| deepcopy                         | 161 us                                                   | 118 us: 1.37x faster                                                    |
| async_generators                 | 206 ms                                                   | 166 ms: 1.24x faster                                                    |
| async_tree_memoization_tg        | 231 ms                                                   | 187 ms: 1.23x faster                                                    |
| regex_effbot                     | 1.67 ms                                                  | 1.37 ms: 1.21x faster                                                   |
| deepcopy_memo                    | 18.3 us                                                  | 15.2 us: 1.21x faster                                                   |
| sqlite_synth                     | 967 ns                                                   | 805 ns: 1.20x faster                                                    |
| async_tree_none                  | 178 ms                                                   | 149 ms: 1.20x faster                                                    |
| xml_etree_iterparse              | 51.6 ms                                                  | 43.0 ms: 1.20x faster                                                   |
| deepcopy_reduce                  | 1.46 us                                                  | 1.24 us: 1.17x faster                                                   |
| async_tree_none_tg               | 172 ms                                                   | 147 ms: 1.17x faster                                                    |
| typing_runtime_protocols         | 71.0 us                                                  | 61.1 us: 1.16x faster                                                   |
| async_tree_memoization           | 223 ms                                                   | 192 ms: 1.16x faster                                                    |
| bpe_tokeniser                    | 2.24 sec                                                 | 1.96 sec: 1.14x faster                                                  |
| json_dumps                       | 4.26 ms                                                  | 3.76 ms: 1.13x faster                                                   |
| comprehensions                   | 9.84 us                                                  | 8.77 us: 1.12x faster                                                   |
| pathlib                          | 12.4 ms                                                  | 11.0 ms: 1.12x faster                                                   |
| k_core                           | 1.12 sec                                                 | 1.00 sec: 1.12x faster                                                  |
| async_tree_cpu_io_mixed_tg       | 338 ms                                                   | 303 ms: 1.12x faster                                                    |
| xml_etree_parse                  | 67.9 ms                                                  | 62.1 ms: 1.09x faster                                                   |
| async_tree_cpu_io_mixed          | 333 ms                                                   | 308 ms: 1.08x faster                                                    |
| float                            | 37.9 ms                                                  | 35.0 ms: 1.08x faster                                                   |
| regex_dna                        | 99.6 ms                                                  | 92.2 ms: 1.08x faster                                                   |
| dulwich_log                      | 21.3 ms                                                  | 19.9 ms: 1.07x faster                                                   |
| regex_v8                         | 9.59 ms                                                  | 9.09 ms: 1.05x faster                                                   |
| go                               | 70.0 ms                                                  | 66.8 ms: 1.05x faster                                                   |
| scimark_fft                      | 142 ms                                                   | 136 ms: 1.05x faster                                                    |
| coroutines                       | 13.6 ms                                                  | 13.1 ms: 1.03x faster                                                   |
| async_tree_eager_memoization     | 132 ms                                                   | 128 ms: 1.03x faster                                                    |
| asyncio_websockets               | 190 ms                                                   | 185 ms: 1.03x faster                                                    |
| pyflate                          | 216 ms                                                   | 211 ms: 1.02x faster                                                    |
| json                             | 1.93 ms                                                  | 1.90 ms: 1.02x faster                                                   |
| tomli_loads                      | 957 ms                                                   | 944 ms: 1.01x faster                                                    |
| spectral_norm                    | 54.4 ms                                                  | 53.7 ms: 1.01x faster                                                   |
| nqueens                          | 43.5 ms                                                  | 44.0 ms: 1.01x slower                                                   |
| xml_etree_generate               | 38.9 ms                                                  | 39.4 ms: 1.01x slower                                                   |
| pidigits                         | 161 ms                                                   | 165 ms: 1.02x slower                                                    |
| pycparser                        | 497 ms                                                   | 511 ms: 1.03x slower                                                    |
| async_tree_eager_cpu_io_mixed    | 231 ms                                                   | 238 ms: 1.03x slower                                                    |
| logging_simple                   | 2.57 us                                                  | 2.67 us: 1.04x slower                                                   |
| scimark_sparse_mat_mult          | 2.08 ms                                                  | 2.18 ms: 1.05x slower                                                   |
| logging_format                   | 2.80 us                                                  | 2.94 us: 1.05x slower                                                   |
| html5lib                         | 23.0 ms                                                  | 24.2 ms: 1.05x slower                                                   |
| sympy_integrate                  | 8.02 ms                                                  | 8.49 ms: 1.06x slower                                                   |
| sphinx                           | 434 ms                                                   | 459 ms: 1.06x slower                                                    |
| docutils                         | 1.02 sec                                                 | 1.10 sec: 1.07x slower                                                  |
| fannkuch                         | 176 ms                                                   | 190 ms: 1.08x slower                                                    |
| raytrace                         | 145 ms                                                   | 157 ms: 1.08x slower                                                    |
| sympy_sum                        | 57.6 ms                                                  | 62.7 ms: 1.09x slower                                                   |
| scimark_monte_carlo              | 32.2 ms                                                  | 35.4 ms: 1.10x slower                                                   |
| chaos                            | 28.9 ms                                                  | 31.8 ms: 1.10x slower                                                   |
| sympy_str                        | 104 ms                                                   | 114 ms: 1.10x slower                                                    |
| thrift                           | 322 us                                                   | 356 us: 1.10x slower                                                    |
| crypto_pyaes                     | 38.8 ms                                                  | 42.9 ms: 1.11x slower                                                   |
| bench_mp_pool                    | 39.7 ms                                                  | 44.1 ms: 1.11x slower                                                   |
| pprint_safe_repr                 | 328 ms                                                   | 367 ms: 1.12x slower                                                    |
| generators                       | 21.9 ms                                                  | 24.8 ms: 1.13x slower                                                   |
| pprint_pformat                   | 665 ms                                                   | 753 ms: 1.13x slower                                                    |
| async_tree_eager                 | 45.6 ms                                                  | 51.9 ms: 1.14x slower                                                   |
| meteor_contest                   | 47.7 ms                                                  | 54.4 ms: 1.14x slower                                                   |
| nbody                            | 54.2 ms                                                  | 62.0 ms: 1.14x slower                                                   |
| unpickle_pure_python             | 103 us                                                   | 119 us: 1.16x slower                                                    |
| shortest_path                    | 219 ms                                                   | 254 ms: 1.16x slower                                                    |
| sympy_expand                     | 167 ms                                                   | 194 ms: 1.16x slower                                                    |
| hexiom                           | 3.04 ms                                                  | 3.54 ms: 1.16x slower                                                   |
| xml_etree_process                | 26.7 ms                                                  | 31.4 ms: 1.17x slower                                                   |
| pickle_pure_python               | 139 us                                                   | 164 us: 1.18x slower                                                    |
| telco                            | 2.61 ms                                                  | 3.09 ms: 1.19x slower                                                   |
| richards                         | 22.4 ms                                                  | 26.9 ms: 1.20x slower                                                   |
| 2to3                             | 114 ms                                                   | 137 ms: 1.20x slower                                                    |
| richards_super                   | 25.4 ms                                                  | 30.5 ms: 1.20x slower                                                   |
| deltablue                        | 1.73 ms                                                  | 2.09 ms: 1.21x slower                                                   |
| connected_components             | 201 ms                                                   | 245 ms: 1.22x slower                                                    |
| mako                             | 4.77 ms                                                  | 5.85 ms: 1.22x slower                                                   |
| regex_compile                    | 54.6 ms                                                  | 68.4 ms: 1.25x slower                                                   |
| python_startup                   | 8.01 ms                                                  | 10.2 ms: 1.27x slower                                                   |
| django_template                  | 13.6 ms                                                  | 17.9 ms: 1.32x slower                                                   |
| python_startup_no_site           | 5.71 ms                                                  | 7.51 ms: 1.32x slower                                                   |
| async_tree_eager_cpu_io_mixed_tg | 213 ms                                                   | 281 ms: 1.32x slower                                                    |
| scimark_lu                       | 51.3 ms                                                  | 70.4 ms: 1.37x slower                                                   |
| bench_thread_pool                | 419 us                                                   | 581 us: 1.39x slower                                                    |
| many_optionals                   | 195 us                                                   | 275 us: 1.41x slower                                                    |
| async_tree_eager_memoization_tg  | 113 ms                                                   | 160 ms: 1.42x slower                                                    |
| coverage                         | 26.9 ms                                                  | 41.4 ms: 1.54x slower                                                   |
| async_tree_eager_tg              | 32.1 ms                                                  | 119 ms: 3.71x slower                                                    |
| Geometric mean                   | (ref)                                                    | 1.01x faster                                                            |

Benchmark hidden because not significant (3): logging_silent, json_loads, scimark_sor
Ignored benchmarks (13) of results/bm-20240906-3.12.6-a4a2d2b/bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b.json: chameleon, dask, djangocms, genshi_text, genshi_xml, gevent_hub, sqlalchemy_declarative, sqlalchemy_imperative, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http
Ignored benchmarks (4) of results/bm-20260826-3.16.0a0-fe3a26f-NOGIL/bm-20260826-macm4pro-arm64-python-fe3a26f43fad1d6eed20-3.16.0a0-fe3a26f.json: sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile

- Geometric mean (including insignificant results): 1.015x faster

# HPT report

- Reliability score: 64.86% likely to be slow
- 90% likely to have a slowdown of 1.00x
- 95% likely to have a slowdown of 1.00x
- 99% likely to have a slowdown of 1.00x

# Memory
- memory change: 1.31x