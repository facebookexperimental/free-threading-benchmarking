# Results vs. 3.12.6

- fork: python
- ref: ff7f62eb2333ac2a2ce2
- machine: linux-x86_64
- commit hash: ff7f62e
- commit date: 2025-12-22
- overall geometric mean: 1.117x faster
- HPT reliability: 100.00%
- HPT 99th percentile: 1.06x faster
- Memory change: 1.19x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20251222-vultr-x86_64-python-ff7f62eb2333ac2a2ce2-3.15.0a3+-ff7f62e |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| 2to3           | 264 ms                                                 | 248 ms: 1.06x faster                                                   |
| docutils       | 2.64 sec                                               | 2.68 sec: 1.01x slower                                                 |
| Geometric mean | (ref)                                                  | 1.01x faster                                                           |

Benchmark hidden because not significant (1): html5lib

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20251222-vultr-x86_64-python-ff7f62eb2333ac2a2ce2-3.15.0a3+-ff7f62e |
|----------------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| async_tree_io              | 1.08 sec                                               | 558 ms: 1.94x faster                                                   |
| async_tree_none            | 464 ms                                                 | 242 ms: 1.92x faster                                                   |
| async_tree_io_tg           | 1.11 sec                                               | 592 ms: 1.87x faster                                                   |
| async_tree_none_tg         | 446 ms                                                 | 245 ms: 1.82x faster                                                   |
| async_tree_memoization_tg  | 560 ms                                                 | 309 ms: 1.81x faster                                                   |
| async_tree_memoization     | 555 ms                                                 | 307 ms: 1.81x faster                                                   |
| async_tree_cpu_io_mixed_tg | 723 ms                                                 | 487 ms: 1.48x faster                                                   |
| async_tree_cpu_io_mixed    | 715 ms                                                 | 488 ms: 1.47x faster                                                   |
| async_generators           | 384 ms                                                 | 359 ms: 1.07x faster                                                   |
| coroutines                 | 23.9 ms                                                | 24.1 ms: 1.01x slower                                                  |
| asyncio_websockets         | 517 ms                                                 | 545 ms: 1.05x slower                                                   |
| Geometric mean             | (ref)                                                  | 1.51x faster                                                           |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20251222-vultr-x86_64-python-ff7f62eb2333ac2a2ce2-3.15.0a3+-ff7f62e |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| float          | 80.8 ms                                                | 60.5 ms: 1.33x faster                                                  |
| nbody          | 89.3 ms                                                | 73.3 ms: 1.22x faster                                                  |
| pidigits       | 184 ms                                                 | 201 ms: 1.09x slower                                                   |
| Geometric mean | (ref)                                                  | 1.14x faster                                                           |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20251222-vultr-x86_64-python-ff7f62eb2333ac2a2ce2-3.15.0a3+-ff7f62e |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| regex_effbot   | 3.17 ms                                                | 2.63 ms: 1.20x faster                                                  |
| regex_compile  | 142 ms                                                 | 121 ms: 1.18x faster                                                   |
| regex_v8       | 20.6 ms                                                | 21.2 ms: 1.03x slower                                                  |
| regex_dna      | 168 ms                                                 | 173 ms: 1.03x slower                                                   |
| Geometric mean | (ref)                                                  | 1.07x faster                                                           |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20251222-vultr-x86_64-python-ff7f62eb2333ac2a2ce2-3.15.0a3+-ff7f62e |
|----------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| unpickle_pure_python | 221 us                                                 | 176 us: 1.25x faster                                                   |
| xml_etree_iterparse  | 96.7 ms                                                | 83.7 ms: 1.16x faster                                                  |
| pickle_pure_python   | 308 us                                                 | 278 us: 1.11x faster                                                   |
| json_dumps           | 10.4 ms                                                | 9.47 ms: 1.09x faster                                                  |
| xml_etree_generate   | 85.2 ms                                                | 78.0 ms: 1.09x faster                                                  |
| xml_etree_process    | 59.0 ms                                                | 54.1 ms: 1.09x faster                                                  |
| xml_etree_parse      | 139 ms                                                 | 129 ms: 1.07x faster                                                   |
| tomli_loads          | 2.11 sec                                               | 1.99 sec: 1.06x faster                                                 |
| json_loads           | 26.5 us                                                | 28.0 us: 1.05x slower                                                  |
| Geometric mean       | (ref)                                                  | 1.09x faster                                                           |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20251222-vultr-x86_64-python-ff7f62eb2333ac2a2ce2-3.15.0a3+-ff7f62e |
|------------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| python_startup_no_site | 7.16 ms                                                | 7.37 ms: 1.03x slower                                                  |
| python_startup         | 9.93 ms                                                | 12.6 ms: 1.27x slower                                                  |
| Geometric mean         | (ref)                                                  | 1.14x slower                                                           |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20251222-vultr-x86_64-python-ff7f62eb2333ac2a2ce2-3.15.0a3+-ff7f62e |
|-----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| genshi_text     | 22.8 ms                                                | 21.2 ms: 1.08x faster                                                  |
| mako            | 11.0 ms                                                | 10.6 ms: 1.04x faster                                                  |
| django_template | 34.7 ms                                                | 35.0 ms: 1.01x slower                                                  |
| genshi_xml      | 50.2 ms                                                | 53.4 ms: 1.06x slower                                                  |
| Geometric mean  | (ref)                                                  | 1.01x faster                                                           |

All benchmarks:
===============

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20251222-vultr-x86_64-python-ff7f62eb2333ac2a2ce2-3.15.0a3+-ff7f62e |
|----------------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| richards                   | 45.9 ms                                                | 18.4 ms: 2.50x faster                                                  |
| richards_super             | 51.9 ms                                                | 22.0 ms: 2.36x faster                                                  |
| async_tree_io              | 1.08 sec                                               | 558 ms: 1.94x faster                                                   |
| async_tree_none            | 464 ms                                                 | 242 ms: 1.92x faster                                                   |
| async_tree_io_tg           | 1.11 sec                                               | 592 ms: 1.87x faster                                                   |
| async_tree_none_tg         | 446 ms                                                 | 245 ms: 1.82x faster                                                   |
| async_tree_memoization_tg  | 560 ms                                                 | 309 ms: 1.81x faster                                                   |
| async_tree_memoization     | 555 ms                                                 | 307 ms: 1.81x faster                                                   |
| mdp                        | 2.42 sec                                               | 1.34 sec: 1.80x faster                                                 |
| deepcopy_memo              | 40.3 us                                                | 25.6 us: 1.57x faster                                                  |
| go                         | 139 ms                                                 | 91.8 ms: 1.52x faster                                                  |
| async_tree_cpu_io_mixed_tg | 723 ms                                                 | 487 ms: 1.48x faster                                                   |
| async_tree_cpu_io_mixed    | 715 ms                                                 | 488 ms: 1.47x faster                                                   |
| deepcopy                   | 352 us                                                 | 244 us: 1.44x faster                                                   |
| float                      | 80.8 ms                                                | 60.5 ms: 1.33x faster                                                  |
| scimark_fft                | 342 ms                                                 | 258 ms: 1.33x faster                                                   |
| logging_silent             | 109 ns                                                 | 82.4 ns: 1.32x faster                                                  |
| scimark_monte_carlo        | 68.4 ms                                                | 51.8 ms: 1.32x faster                                                  |
| deltablue                  | 3.45 ms                                                | 2.63 ms: 1.31x faster                                                  |
| scimark_sor                | 130 ms                                                 | 99.6 ms: 1.30x faster                                                  |
| spectral_norm              | 110 ms                                                 | 85.6 ms: 1.29x faster                                                  |
| unpickle_pure_python       | 221 us                                                 | 176 us: 1.25x faster                                                   |
| deepcopy_reduce            | 3.08 us                                                | 2.50 us: 1.23x faster                                                  |
| pyflate                    | 448 ms                                                 | 365 ms: 1.23x faster                                                   |
| comprehensions             | 19.8 us                                                | 16.2 us: 1.22x faster                                                  |
| nbody                      | 89.3 ms                                                | 73.3 ms: 1.22x faster                                                  |
| pathlib                    | 21.5 ms                                                | 17.8 ms: 1.21x faster                                                  |
| regex_effbot               | 3.17 ms                                                | 2.63 ms: 1.20x faster                                                  |
| bpe_tokeniser              | 4.74 sec                                               | 3.99 sec: 1.19x faster                                                 |
| chaos                      | 62.8 ms                                                | 53.2 ms: 1.18x faster                                                  |
| regex_compile              | 142 ms                                                 | 121 ms: 1.18x faster                                                   |
| pprint_pformat             | 1.52 sec                                               | 1.30 sec: 1.17x faster                                                 |
| pprint_safe_repr           | 743 ms                                                 | 639 ms: 1.16x faster                                                   |
| crypto_pyaes               | 76.6 ms                                                | 66.0 ms: 1.16x faster                                                  |
| xml_etree_iterparse        | 96.7 ms                                                | 83.7 ms: 1.16x faster                                                  |
| raytrace                   | 299 ms                                                 | 262 ms: 1.14x faster                                                   |
| logging_simple             | 6.63 us                                                | 5.93 us: 1.12x faster                                                  |
| logging_format             | 7.35 us                                                | 6.63 us: 1.11x faster                                                  |
| pickle_pure_python         | 308 us                                                 | 278 us: 1.11x faster                                                   |
| dulwich_log                | 78.9 ms                                                | 71.5 ms: 1.10x faster                                                  |
| generators                 | 32.2 ms                                                | 29.5 ms: 1.09x faster                                                  |
| json_dumps                 | 10.4 ms                                                | 9.47 ms: 1.09x faster                                                  |
| xml_etree_generate         | 85.2 ms                                                | 78.0 ms: 1.09x faster                                                  |
| xml_etree_process          | 59.0 ms                                                | 54.1 ms: 1.09x faster                                                  |
| scimark_lu                 | 114 ms                                                 | 105 ms: 1.09x faster                                                   |
| thrift                     | 791 us                                                 | 732 us: 1.08x faster                                                   |
| genshi_text                | 22.8 ms                                                | 21.2 ms: 1.08x faster                                                  |
| async_generators           | 384 ms                                                 | 359 ms: 1.07x faster                                                   |
| xml_etree_parse            | 139 ms                                                 | 129 ms: 1.07x faster                                                   |
| 2to3                       | 264 ms                                                 | 248 ms: 1.06x faster                                                   |
| tomli_loads                | 2.11 sec                                               | 1.99 sec: 1.06x faster                                                 |
| meteor_contest             | 104 ms                                                 | 98.1 ms: 1.06x faster                                                  |
| fannkuch                   | 372 ms                                                 | 354 ms: 1.05x faster                                                   |
| pycparser                  | 1.17 sec                                               | 1.11 sec: 1.05x faster                                                 |
| typing_runtime_protocols   | 163 us                                                 | 156 us: 1.05x faster                                                   |
| mako                       | 11.0 ms                                                | 10.6 ms: 1.04x faster                                                  |
| pylint                     | 319 ms                                                 | 308 ms: 1.03x faster                                                   |
| hexiom                     | 6.17 ms                                                | 6.06 ms: 1.02x faster                                                  |
| json                       | 5.02 ms                                                | 4.95 ms: 1.01x faster                                                  |
| sympy_integrate            | 20.5 ms                                                | 20.4 ms: 1.01x faster                                                  |
| coroutines                 | 23.9 ms                                                | 24.1 ms: 1.01x slower                                                  |
| django_template            | 34.7 ms                                                | 35.0 ms: 1.01x slower                                                  |
| docutils                   | 2.64 sec                                               | 2.68 sec: 1.01x slower                                                 |
| sympy_sum                  | 166 ms                                                 | 170 ms: 1.03x slower                                                   |
| python_startup_no_site     | 7.16 ms                                                | 7.37 ms: 1.03x slower                                                  |
| regex_v8                   | 20.6 ms                                                | 21.2 ms: 1.03x slower                                                  |
| regex_dna                  | 168 ms                                                 | 173 ms: 1.03x slower                                                   |
| nqueens                    | 80.1 ms                                                | 84.4 ms: 1.05x slower                                                  |
| asyncio_websockets         | 517 ms                                                 | 545 ms: 1.05x slower                                                   |
| json_loads                 | 26.5 us                                                | 28.0 us: 1.05x slower                                                  |
| sympy_str                  | 292 ms                                                 | 310 ms: 1.06x slower                                                   |
| genshi_xml                 | 50.2 ms                                                | 53.4 ms: 1.06x slower                                                  |
| sympy_expand               | 468 ms                                                 | 507 ms: 1.09x slower                                                   |
| pidigits                   | 184 ms                                                 | 201 ms: 1.09x slower                                                   |
| coverage                   | 71.4 ms                                                | 81.2 ms: 1.14x slower                                                  |
| python_startup             | 9.93 ms                                                | 12.6 ms: 1.27x slower                                                  |
| gc_traversal               | 3.46 ms                                                | 4.66 ms: 1.35x slower                                                  |
| bench_thread_pool          | 941 us                                                 | 1.31 ms: 1.39x slower                                                  |
| create_gc_cycles           | 1.09 ms                                                | 1.91 ms: 1.75x slower                                                  |
| bench_mp_pool              | 10.8 ms                                                | 244 ms: 22.56x slower                                                  |
| telco                      | 6.53 ms                                                | 164 ms: 25.09x slower                                                  |
| Geometric mean             | (ref)                                                  | 1.07x faster                                                           |

Benchmark hidden because not significant (3): scimark_sparse_mat_mult, sqlite_synth, html5lib
Ignored benchmarks (21) of results/bm-20240906-3.12.6-a4a2d2b/bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b.json: aiohttp, asyncio_tcp, asyncio_tcp_ssl, chameleon, dask, flaskblogging, gunicorn, mypy2, pickle, pickle_dict, pickle_list, sqlalchemy_declarative, sqlalchemy_imperative, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http, unpack_sequence, unpickle, unpickle_list
Ignored benchmarks (10) of results/bm-20251222-3.15.0a3+-ff7f62e-JIT/bm-20251222-vultr-x86_64-python-ff7f62eb2333ac2a2ce2-3.15.0a3+-ff7f62e.json: connected_components, k_core, many_optionals, shortest_path, sphinx, sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile, subparsers

- Geometric mean (including insignificant results): 1.117x faster

# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.08x
- 95% likely to have a speedup of 1.07x
- 99% likely to have a speedup of 1.06x

# Memory
- memory change: 1.19x