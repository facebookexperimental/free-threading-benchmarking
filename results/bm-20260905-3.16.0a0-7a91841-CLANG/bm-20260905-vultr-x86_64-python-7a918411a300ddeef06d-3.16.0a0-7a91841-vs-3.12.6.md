# Results vs. 3.12.6

- fork: python
- ref: 7a918411a300ddeef06d
- machine: linux-x86_64
- commit hash: 7a91841
- commit date: 2026-09-05
- overall geometric mean: 1.095x faster
- HPT reliability: 100.00%
- HPT 99th percentile: 1.05x faster
- Memory change: 1.18x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260905-vultr-x86_64-python-7a918411a300ddeef06d-3.16.0a0-7a91841 |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| 2to3           | 264 ms                                                 | 259 ms: 1.02x faster                                                  |
| docutils       | 2.64 sec                                               | 2.32 sec: 1.14x faster                                                |
| html5lib       | 63.6 ms                                                | 59.3 ms: 1.07x faster                                                 |
| Geometric mean | (ref)                                                  | 1.07x faster                                                          |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260905-vultr-x86_64-python-7a918411a300ddeef06d-3.16.0a0-7a91841 |
|----------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| async_tree_none            | 464 ms                                                 | 283 ms: 1.64x faster                                                  |
| async_tree_memoization_tg  | 560 ms                                                 | 358 ms: 1.56x faster                                                  |
| async_tree_io              | 1.08 sec                                               | 708 ms: 1.53x faster                                                  |
| async_tree_none_tg         | 446 ms                                                 | 292 ms: 1.53x faster                                                  |
| async_tree_memoization     | 555 ms                                                 | 366 ms: 1.52x faster                                                  |
| async_tree_io_tg           | 1.11 sec                                               | 758 ms: 1.46x faster                                                  |
| async_tree_cpu_io_mixed    | 715 ms                                                 | 519 ms: 1.38x faster                                                  |
| async_tree_cpu_io_mixed_tg | 723 ms                                                 | 537 ms: 1.35x faster                                                  |
| async_generators           | 384 ms                                                 | 325 ms: 1.18x faster                                                  |
| asyncio_tcp                | 519 ms                                                 | 450 ms: 1.15x faster                                                  |
| coroutines                 | 23.9 ms                                                | 22.0 ms: 1.09x faster                                                 |
| asyncio_tcp_ssl            | 1.51 sec                                               | 1.44 sec: 1.04x faster                                                |
| asyncio_websockets         | 517 ms                                                 | 544 ms: 1.05x slower                                                  |
| Geometric mean             | (ref)                                                  | 1.32x faster                                                          |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260905-vultr-x86_64-python-7a918411a300ddeef06d-3.16.0a0-7a91841 |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| float          | 80.8 ms                                                | 71.0 ms: 1.14x faster                                                 |
| nbody          | 89.3 ms                                                | 83.2 ms: 1.07x faster                                                 |
| pidigits       | 184 ms                                                 | 191 ms: 1.04x slower                                                  |
| Geometric mean | (ref)                                                  | 1.06x faster                                                          |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260905-vultr-x86_64-python-7a918411a300ddeef06d-3.16.0a0-7a91841 |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| regex_compile  | 142 ms                                                 | 145 ms: 1.02x slower                                                  |
| regex_effbot   | 3.17 ms                                                | 3.33 ms: 1.05x slower                                                 |
| regex_v8       | 20.6 ms                                                | 23.6 ms: 1.15x slower                                                 |
| regex_dna      | 168 ms                                                 | 201 ms: 1.20x slower                                                  |
| Geometric mean | (ref)                                                  | 1.10x slower                                                          |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260905-vultr-x86_64-python-7a918411a300ddeef06d-3.16.0a0-7a91841 |
|----------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| json_dumps           | 10.4 ms                                                | 8.85 ms: 1.17x faster                                                 |
| tomli_loads          | 2.11 sec                                               | 1.80 sec: 1.17x faster                                                |
| unpickle_pure_python | 221 us                                                 | 202 us: 1.09x faster                                                  |
| pickle_pure_python   | 308 us                                                 | 294 us: 1.05x faster                                                  |
| xml_etree_iterparse  | 96.7 ms                                                | 92.6 ms: 1.04x faster                                                 |
| unpickle_list        | 4.67 us                                                | 4.56 us: 1.02x faster                                                 |
| pickle_dict          | 31.8 us                                                | 31.4 us: 1.01x faster                                                 |
| json_loads           | 26.5 us                                                | 26.9 us: 1.01x slower                                                 |
| unpickle             | 14.1 us                                                | 14.3 us: 1.02x slower                                                 |
| xml_etree_parse      | 139 ms                                                 | 159 ms: 1.15x slower                                                  |
| pickle_list          | 4.77 us                                                | 5.48 us: 1.15x slower                                                 |
| pickle               | 10.9 us                                                | 13.7 us: 1.25x slower                                                 |
| Geometric mean       | (ref)                                                  | 1.00x slower                                                          |

Benchmark hidden because not significant (2): xml_etree_generate, xml_etree_process

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260905-vultr-x86_64-python-7a918411a300ddeef06d-3.16.0a0-7a91841 |
|------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| python_startup_no_site | 7.16 ms                                                | 7.88 ms: 1.10x slower                                                 |
| python_startup         | 9.93 ms                                                | 13.1 ms: 1.32x slower                                                 |
| Geometric mean         | (ref)                                                  | 1.20x slower                                                          |

Benchmarks with tag 'template':
===============================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260905-vultr-x86_64-python-7a918411a300ddeef06d-3.16.0a0-7a91841 |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| mako           | 11.0 ms                                                | 11.9 ms: 1.08x slower                                                 |
| Geometric mean | (ref)                                                  | 1.04x slower                                                          |

Benchmark hidden because not significant (1): django_template

All benchmarks:
===============

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260905-vultr-x86_64-python-7a918411a300ddeef06d-3.16.0a0-7a91841 |
|----------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| pylint                     | 319 ms                                                 | 111 ms: 2.88x faster                                                  |
| mdp                        | 2.42 sec                                               | 1.08 sec: 2.24x faster                                                |
| deepcopy                   | 352 us                                                 | 214 us: 1.64x faster                                                  |
| async_tree_none            | 464 ms                                                 | 283 ms: 1.64x faster                                                  |
| deepcopy_memo              | 40.3 us                                                | 24.8 us: 1.62x faster                                                 |
| async_tree_memoization_tg  | 560 ms                                                 | 358 ms: 1.56x faster                                                  |
| async_tree_io              | 1.08 sec                                               | 708 ms: 1.53x faster                                                  |
| async_tree_none_tg         | 446 ms                                                 | 292 ms: 1.53x faster                                                  |
| async_tree_memoization     | 555 ms                                                 | 366 ms: 1.52x faster                                                  |
| typing_runtime_protocols   | 163 us                                                 | 109 us: 1.49x faster                                                  |
| async_tree_io_tg           | 1.11 sec                                               | 758 ms: 1.46x faster                                                  |
| comprehensions             | 19.8 us                                                | 14.2 us: 1.40x faster                                                 |
| go                         | 139 ms                                                 | 99.8 ms: 1.39x faster                                                 |
| async_tree_cpu_io_mixed    | 715 ms                                                 | 519 ms: 1.38x faster                                                  |
| async_tree_cpu_io_mixed_tg | 723 ms                                                 | 537 ms: 1.35x faster                                                  |
| deepcopy_reduce            | 3.08 us                                                | 2.33 us: 1.32x faster                                                 |
| logging_silent             | 109 ns                                                 | 84.7 ns: 1.29x faster                                                 |
| spectral_norm              | 110 ms                                                 | 86.3 ms: 1.28x faster                                                 |
| scimark_sor                | 130 ms                                                 | 103 ms: 1.26x faster                                                  |
| pathlib                    | 21.5 ms                                                | 17.1 ms: 1.26x faster                                                 |
| scimark_fft                | 342 ms                                                 | 274 ms: 1.25x faster                                                  |
| raytrace                   | 299 ms                                                 | 243 ms: 1.23x faster                                                  |
| chaos                      | 62.8 ms                                                | 51.7 ms: 1.22x faster                                                 |
| unpack_sequence            | 52.1 ns                                                | 42.8 ns: 1.22x faster                                                 |
| logging_simple             | 6.63 us                                                | 5.54 us: 1.20x faster                                                 |
| async_generators           | 384 ms                                                 | 325 ms: 1.18x faster                                                  |
| richards                   | 45.9 ms                                                | 38.8 ms: 1.18x faster                                                 |
| scimark_monte_carlo        | 68.4 ms                                                | 58.1 ms: 1.18x faster                                                 |
| dulwich_log                | 78.9 ms                                                | 66.9 ms: 1.18x faster                                                 |
| json_dumps                 | 10.4 ms                                                | 8.85 ms: 1.17x faster                                                 |
| bpe_tokeniser              | 4.74 sec                                               | 4.05 sec: 1.17x faster                                                |
| tomli_loads                | 2.11 sec                                               | 1.80 sec: 1.17x faster                                                |
| logging_format             | 7.35 us                                                | 6.31 us: 1.17x faster                                                 |
| pyflate                    | 448 ms                                                 | 386 ms: 1.16x faster                                                  |
| richards_super             | 51.9 ms                                                | 44.8 ms: 1.16x faster                                                 |
| asyncio_tcp                | 519 ms                                                 | 450 ms: 1.15x faster                                                  |
| nqueens                    | 80.1 ms                                                | 69.6 ms: 1.15x faster                                                 |
| generators                 | 32.2 ms                                                | 28.3 ms: 1.14x faster                                                 |
| docutils                   | 2.64 sec                                               | 2.32 sec: 1.14x faster                                                |
| float                      | 80.8 ms                                                | 71.0 ms: 1.14x faster                                                 |
| hexiom                     | 6.17 ms                                                | 5.45 ms: 1.13x faster                                                 |
| deltablue                  | 3.45 ms                                                | 3.07 ms: 1.12x faster                                                 |
| sympy_integrate            | 20.5 ms                                                | 18.3 ms: 1.12x faster                                                 |
| crypto_pyaes               | 76.6 ms                                                | 68.8 ms: 1.11x faster                                                 |
| unpickle_pure_python       | 221 us                                                 | 202 us: 1.09x faster                                                  |
| sympy_sum                  | 166 ms                                                 | 152 ms: 1.09x faster                                                  |
| coroutines                 | 23.9 ms                                                | 22.0 ms: 1.09x faster                                                 |
| sympy_str                  | 292 ms                                                 | 269 ms: 1.08x faster                                                  |
| nbody                      | 89.3 ms                                                | 83.2 ms: 1.07x faster                                                 |
| html5lib                   | 63.6 ms                                                | 59.3 ms: 1.07x faster                                                 |
| scimark_sparse_mat_mult    | 4.39 ms                                                | 4.11 ms: 1.07x faster                                                 |
| pickle_pure_python         | 308 us                                                 | 294 us: 1.05x faster                                                  |
| xml_etree_iterparse        | 96.7 ms                                                | 92.6 ms: 1.04x faster                                                 |
| asyncio_tcp_ssl            | 1.51 sec                                               | 1.44 sec: 1.04x faster                                                |
| scimark_lu                 | 114 ms                                                 | 110 ms: 1.04x faster                                                  |
| pprint_safe_repr           | 743 ms                                                 | 718 ms: 1.04x faster                                                  |
| pprint_pformat             | 1.52 sec                                               | 1.47 sec: 1.03x faster                                                |
| fannkuch                   | 372 ms                                                 | 360 ms: 1.03x faster                                                  |
| thrift                     | 791 us                                                 | 769 us: 1.03x faster                                                  |
| sympy_expand               | 468 ms                                                 | 456 ms: 1.03x faster                                                  |
| unpickle_list              | 4.67 us                                                | 4.56 us: 1.02x faster                                                 |
| 2to3                       | 264 ms                                                 | 259 ms: 1.02x faster                                                  |
| pickle_dict                | 31.8 us                                                | 31.4 us: 1.01x faster                                                 |
| meteor_contest             | 104 ms                                                 | 105 ms: 1.01x slower                                                  |
| json_loads                 | 26.5 us                                                | 26.9 us: 1.01x slower                                                 |
| regex_compile              | 142 ms                                                 | 145 ms: 1.02x slower                                                  |
| unpickle                   | 14.1 us                                                | 14.3 us: 1.02x slower                                                 |
| pidigits                   | 184 ms                                                 | 191 ms: 1.04x slower                                                  |
| asyncio_websockets         | 517 ms                                                 | 544 ms: 1.05x slower                                                  |
| regex_effbot               | 3.17 ms                                                | 3.33 ms: 1.05x slower                                                 |
| mako                       | 11.0 ms                                                | 11.9 ms: 1.08x slower                                                 |
| python_startup_no_site     | 7.16 ms                                                | 7.88 ms: 1.10x slower                                                 |
| coverage                   | 71.4 ms                                                | 80.1 ms: 1.12x slower                                                 |
| gc_traversal               | 3.46 ms                                                | 3.92 ms: 1.13x slower                                                 |
| regex_v8                   | 20.6 ms                                                | 23.6 ms: 1.15x slower                                                 |
| xml_etree_parse            | 139 ms                                                 | 159 ms: 1.15x slower                                                  |
| pickle_list                | 4.77 us                                                | 5.48 us: 1.15x slower                                                 |
| regex_dna                  | 168 ms                                                 | 201 ms: 1.20x slower                                                  |
| pickle                     | 10.9 us                                                | 13.7 us: 1.25x slower                                                 |
| python_startup             | 9.93 ms                                                | 13.1 ms: 1.32x slower                                                 |
| bench_thread_pool          | 941 us                                                 | 1.31 ms: 1.39x slower                                                 |
| create_gc_cycles           | 1.09 ms                                                | 1.82 ms: 1.67x slower                                                 |
| bench_mp_pool              | 10.8 ms                                                | 258 ms: 23.90x slower                                                 |
| telco                      | 6.53 ms                                                | 157 ms: 24.10x slower                                                 |
| Geometric mean             | (ref)                                                  | 1.05x faster                                                          |

Benchmark hidden because not significant (6): sqlite_synth, pycparser, xml_etree_generate, django_template, xml_etree_process, json
Ignored benchmarks (15) of results/bm-20240906-3.12.6-a4a2d2b/bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b.json: aiohttp, chameleon, dask, flaskblogging, genshi_text, genshi_xml, gunicorn, mypy2, sqlalchemy_declarative, sqlalchemy_imperative, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http
Ignored benchmarks (10) of results/bm-20260905-3.16.0a0-7a91841-CLANG/bm-20260905-vultr-x86_64-python-7a918411a300ddeef06d-3.16.0a0-7a91841.json: connected_components, k_core, many_optionals, shortest_path, sphinx, sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile, subparsers

- Geometric mean (including insignificant results): 1.095x faster

# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.08x
- 95% likely to have a speedup of 1.07x
- 99% likely to have a speedup of 1.05x

# Memory
- memory change: 1.18x