# Results vs. 3.13.0rc2

- fork: python
- ref: ccbe41e27cdb44103318
- machine: darwin-arm64
- commit hash: ccbe41e
- commit date: 2026-01-30
- overall geometric mean: 1.177x faster
- HPT reliability: 100.00%
- HPT 99th percentile: 1.07x faster
- Memory change: 1.21x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-macm4pro-arm64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260130-macm4pro-arm64-python-ccbe41e27cdb44103318-3.15.0a5+-ccbe41e |
|----------------|:--------------------------------------------------------------:|:------------------------------------------------------------------------:|
| 2to3           | 112 ms                                                         | 110 ms: 1.02x faster                                                     |
| docutils       | 1.05 sec                                                       | 973 ms: 1.08x faster                                                     |
| html5lib       | 23.1 ms                                                        | 18.9 ms: 1.23x faster                                                    |
| sphinx         | 409 ms                                                         | 377 ms: 1.08x faster                                                     |
| Geometric mean | (ref)                                                          | 1.10x faster                                                             |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                        | bm-20240906-macm4pro-arm64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260130-macm4pro-arm64-python-ccbe41e27cdb44103318-3.15.0a5+-ccbe41e |
|----------------------------------|:--------------------------------------------------------------:|:------------------------------------------------------------------------:|
| async_tree_eager_io              | 525 ms                                                         | 214 ms: 2.45x faster                                                     |
| async_tree_eager_io_tg           | 521 ms                                                         | 217 ms: 2.40x faster                                                     |
| async_tree_io_tg                 | 405 ms                                                         | 221 ms: 1.83x faster                                                     |
| async_tree_io                    | 386 ms                                                         | 218 ms: 1.77x faster                                                     |
| async_tree_none                  | 142 ms                                                         | 96.6 ms: 1.47x faster                                                    |
| async_tree_none_tg               | 133 ms                                                         | 93.8 ms: 1.41x faster                                                    |
| async_tree_memoization_tg        | 186 ms                                                         | 132 ms: 1.40x faster                                                     |
| async_tree_memoization           | 184 ms                                                         | 133 ms: 1.39x faster                                                     |
| async_tree_cpu_io_mixed_tg       | 301 ms                                                         | 246 ms: 1.22x faster                                                     |
| async_tree_cpu_io_mixed          | 294 ms                                                         | 250 ms: 1.17x faster                                                     |
| async_tree_eager                 | 43.2 ms                                                        | 37.0 ms: 1.17x faster                                                    |
| async_tree_eager_memoization     | 122 ms                                                         | 106 ms: 1.15x faster                                                     |
| async_generators                 | 193 ms                                                         | 181 ms: 1.07x faster                                                     |
| coroutines                       | 10.8 ms                                                        | 10.2 ms: 1.05x faster                                                    |
| asyncio_websockets               | 194 ms                                                         | 185 ms: 1.05x faster                                                     |
| async_tree_eager_cpu_io_mixed    | 225 ms                                                         | 217 ms: 1.04x faster                                                     |
| async_tree_eager_cpu_io_mixed_tg | 208 ms                                                         | 232 ms: 1.12x slower                                                     |
| async_tree_eager_memoization_tg  | 103 ms                                                         | 119 ms: 1.16x slower                                                     |
| async_tree_eager_tg              | 28.9 ms                                                        | 77.7 ms: 2.68x slower                                                    |
| Geometric mean                   | (ref)                                                          | 1.23x faster                                                             |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-macm4pro-arm64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260130-macm4pro-arm64-python-ccbe41e27cdb44103318-3.15.0a5+-ccbe41e |
|----------------|:--------------------------------------------------------------:|:------------------------------------------------------------------------:|
| float          | 31.4 ms                                                        | 22.3 ms: 1.41x faster                                                    |
| nbody          | 42.5 ms                                                        | 39.5 ms: 1.08x faster                                                    |
| pidigits       | 166 ms                                                         | 157 ms: 1.06x faster                                                     |
| Geometric mean | (ref)                                                          | 1.17x faster                                                             |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-macm4pro-arm64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260130-macm4pro-arm64-python-ccbe41e27cdb44103318-3.15.0a5+-ccbe41e |
|----------------|:--------------------------------------------------------------:|:------------------------------------------------------------------------:|
| regex_compile  | 47.9 ms                                                        | 40.2 ms: 1.19x faster                                                    |
| regex_effbot   | 1.61 ms                                                        | 1.41 ms: 1.14x faster                                                    |
| regex_v8       | 10.7 ms                                                        | 9.39 ms: 1.14x faster                                                    |
| regex_dna      | 94.6 ms                                                        | 92.3 ms: 1.03x faster                                                    |
| Geometric mean | (ref)                                                          | 1.12x faster                                                             |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-macm4pro-arm64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260130-macm4pro-arm64-python-ccbe41e27cdb44103318-3.15.0a5+-ccbe41e |
|----------------------|:--------------------------------------------------------------:|:------------------------------------------------------------------------:|
| tomli_loads          | 1000 ms                                                        | 664 ms: 1.51x faster                                                     |
| unpickle_pure_python | 99.5 us                                                        | 72.9 us: 1.36x faster                                                    |
| json_dumps           | 4.65 ms                                                        | 3.64 ms: 1.28x faster                                                    |
| xml_etree_iterparse  | 46.1 ms                                                        | 36.9 ms: 1.25x faster                                                    |
| pickle_pure_python   | 130 us                                                         | 111 us: 1.18x faster                                                     |
| xml_etree_process    | 25.4 ms                                                        | 21.7 ms: 1.17x faster                                                    |
| xml_etree_generate   | 35.8 ms                                                        | 31.0 ms: 1.15x faster                                                    |
| json_loads           | 10.8 us                                                        | 10.5 us: 1.04x faster                                                    |
| xml_etree_parse      | 62.4 ms                                                        | 60.5 ms: 1.03x faster                                                    |
| Geometric mean       | (ref)                                                          | 1.21x faster                                                             |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240906-macm4pro-arm64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260130-macm4pro-arm64-python-ccbe41e27cdb44103318-3.15.0a5+-ccbe41e |
|------------------------|:--------------------------------------------------------------:|:------------------------------------------------------------------------:|
| python_startup         | 8.63 ms                                                        | 8.65 ms: 1.00x slower                                                    |
| python_startup_no_site | 5.95 ms                                                        | 6.26 ms: 1.05x slower                                                    |
| Geometric mean         | (ref)                                                          | 1.03x slower                                                             |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240906-macm4pro-arm64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260130-macm4pro-arm64-python-ccbe41e27cdb44103318-3.15.0a5+-ccbe41e |
|-----------------|:--------------------------------------------------------------:|:------------------------------------------------------------------------:|
| mako            | 4.41 ms                                                        | 3.89 ms: 1.14x faster                                                    |
| genshi_text     | 9.97 ms                                                        | 8.80 ms: 1.13x faster                                                    |
| genshi_xml      | 21.4 ms                                                        | 20.4 ms: 1.05x faster                                                    |
| django_template | 12.5 ms                                                        | 13.8 ms: 1.10x slower                                                    |
| Geometric mean  | (ref)                                                          | 1.05x faster                                                             |

All benchmarks:
===============

| Benchmark                        | bm-20240906-macm4pro-arm64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260130-macm4pro-arm64-python-ccbe41e27cdb44103318-3.15.0a5+-ccbe41e |
|----------------------------------|:--------------------------------------------------------------:|:------------------------------------------------------------------------:|
| async_tree_eager_io              | 525 ms                                                         | 214 ms: 2.45x faster                                                     |
| async_tree_eager_io_tg           | 521 ms                                                         | 217 ms: 2.40x faster                                                     |
| richards                         | 22.1 ms                                                        | 9.21 ms: 2.40x faster                                                    |
| richards_super                   | 24.7 ms                                                        | 11.0 ms: 2.25x faster                                                    |
| async_tree_io_tg                 | 405 ms                                                         | 221 ms: 1.83x faster                                                     |
| mdp                              | 1.06 sec                                                       | 590 ms: 1.79x faster                                                     |
| async_tree_io                    | 386 ms                                                         | 218 ms: 1.77x faster                                                     |
| k_core                           | 1.46 sec                                                       | 859 ms: 1.70x faster                                                     |
| scimark_sor                      | 64.0 ms                                                        | 38.4 ms: 1.67x faster                                                    |
| subparsers                       | 6.26 ms                                                        | 3.88 ms: 1.61x faster                                                    |
| go                               | 72.6 ms                                                        | 45.4 ms: 1.60x faster                                                    |
| deepcopy_memo                    | 16.5 us                                                        | 10.3 us: 1.60x faster                                                    |
| deepcopy                         | 145 us                                                         | 91.4 us: 1.59x faster                                                    |
| scimark_lu                       | 42.8 ms                                                        | 28.0 ms: 1.53x faster                                                    |
| pyflate                          | 222 ms                                                         | 146 ms: 1.52x faster                                                     |
| tomli_loads                      | 1000 ms                                                        | 664 ms: 1.51x faster                                                     |
| async_tree_none                  | 142 ms                                                         | 96.6 ms: 1.47x faster                                                    |
| async_tree_none_tg               | 133 ms                                                         | 93.8 ms: 1.41x faster                                                    |
| float                            | 31.4 ms                                                        | 22.3 ms: 1.41x faster                                                    |
| async_tree_memoization_tg        | 186 ms                                                         | 132 ms: 1.40x faster                                                     |
| async_tree_memoization           | 184 ms                                                         | 133 ms: 1.39x faster                                                     |
| unpickle_pure_python             | 99.5 us                                                        | 72.9 us: 1.36x faster                                                    |
| deepcopy_reduce                  | 1.30 us                                                        | 967 ns: 1.34x faster                                                     |
| scimark_monte_carlo              | 29.9 ms                                                        | 22.8 ms: 1.31x faster                                                    |
| deltablue                        | 1.45 ms                                                        | 1.12 ms: 1.30x faster                                                    |
| pprint_pformat                   | 650 ms                                                         | 507 ms: 1.28x faster                                                     |
| json_dumps                       | 4.65 ms                                                        | 3.64 ms: 1.28x faster                                                    |
| pprint_safe_repr                 | 322 ms                                                         | 252 ms: 1.28x faster                                                     |
| scimark_fft                      | 124 ms                                                         | 98.4 ms: 1.26x faster                                                    |
| xml_etree_iterparse              | 46.1 ms                                                        | 36.9 ms: 1.25x faster                                                    |
| html5lib                         | 23.1 ms                                                        | 18.9 ms: 1.23x faster                                                    |
| async_tree_cpu_io_mixed_tg       | 301 ms                                                         | 246 ms: 1.22x faster                                                     |
| regex_compile                    | 47.9 ms                                                        | 40.2 ms: 1.19x faster                                                    |
| fannkuch                         | 179 ms                                                         | 150 ms: 1.19x faster                                                     |
| telco                            | 3.07 ms                                                        | 2.61 ms: 1.18x faster                                                    |
| pickle_pure_python               | 130 us                                                         | 111 us: 1.18x faster                                                     |
| async_tree_cpu_io_mixed          | 294 ms                                                         | 250 ms: 1.17x faster                                                     |
| logging_silent                   | 40.6 ns                                                        | 34.7 ns: 1.17x faster                                                    |
| bpe_tokeniser                    | 2.13 sec                                                       | 1.82 sec: 1.17x faster                                                   |
| xml_etree_process                | 25.4 ms                                                        | 21.7 ms: 1.17x faster                                                    |
| async_tree_eager                 | 43.2 ms                                                        | 37.0 ms: 1.17x faster                                                    |
| xml_etree_generate               | 35.8 ms                                                        | 31.0 ms: 1.15x faster                                                    |
| async_tree_eager_memoization     | 122 ms                                                         | 106 ms: 1.15x faster                                                     |
| regex_effbot                     | 1.61 ms                                                        | 1.41 ms: 1.14x faster                                                    |
| regex_v8                         | 10.7 ms                                                        | 9.39 ms: 1.14x faster                                                    |
| mako                             | 4.41 ms                                                        | 3.89 ms: 1.14x faster                                                    |
| genshi_text                      | 9.97 ms                                                        | 8.80 ms: 1.13x faster                                                    |
| create_gc_cycles                 | 993 us                                                         | 884 us: 1.12x faster                                                     |
| spectral_norm                    | 43.7 ms                                                        | 39.2 ms: 1.11x faster                                                    |
| chaos                            | 24.3 ms                                                        | 21.8 ms: 1.11x faster                                                    |
| pycparser                        | 470 ms                                                         | 428 ms: 1.10x faster                                                     |
| comprehensions                   | 6.80 us                                                        | 6.21 us: 1.10x faster                                                    |
| sphinx                           | 409 ms                                                         | 377 ms: 1.08x faster                                                     |
| dulwich_log                      | 19.8 ms                                                        | 18.3 ms: 1.08x faster                                                    |
| logging_format                   | 2.45 us                                                        | 2.26 us: 1.08x faster                                                    |
| logging_simple                   | 2.24 us                                                        | 2.07 us: 1.08x faster                                                    |
| nbody                            | 42.5 ms                                                        | 39.5 ms: 1.08x faster                                                    |
| meteor_contest                   | 47.9 ms                                                        | 44.5 ms: 1.08x faster                                                    |
| docutils                         | 1.05 sec                                                       | 973 ms: 1.08x faster                                                     |
| json                             | 1.94 ms                                                        | 1.81 ms: 1.07x faster                                                    |
| crypto_pyaes                     | 33.6 ms                                                        | 31.4 ms: 1.07x faster                                                    |
| pathlib                          | 11.1 ms                                                        | 10.4 ms: 1.07x faster                                                    |
| async_generators                 | 193 ms                                                         | 181 ms: 1.07x faster                                                     |
| nqueens                          | 37.2 ms                                                        | 34.9 ms: 1.07x faster                                                    |
| typing_runtime_protocols         | 64.6 us                                                        | 60.9 us: 1.06x faster                                                    |
| connected_components             | 208 ms                                                         | 196 ms: 1.06x faster                                                     |
| pidigits                         | 166 ms                                                         | 157 ms: 1.06x faster                                                     |
| hexiom                           | 2.85 ms                                                        | 2.70 ms: 1.05x faster                                                    |
| coroutines                       | 10.8 ms                                                        | 10.2 ms: 1.05x faster                                                    |
| asyncio_websockets               | 194 ms                                                         | 185 ms: 1.05x faster                                                     |
| genshi_xml                       | 21.4 ms                                                        | 20.4 ms: 1.05x faster                                                    |
| shortest_path                    | 225 ms                                                         | 215 ms: 1.04x faster                                                     |
| raytrace                         | 109 ms                                                         | 105 ms: 1.04x faster                                                     |
| async_tree_eager_cpu_io_mixed    | 225 ms                                                         | 217 ms: 1.04x faster                                                     |
| json_loads                       | 10.8 us                                                        | 10.5 us: 1.04x faster                                                    |
| thrift                           | 309 us                                                         | 299 us: 1.03x faster                                                     |
| xml_etree_parse                  | 62.4 ms                                                        | 60.5 ms: 1.03x faster                                                    |
| regex_dna                        | 94.6 ms                                                        | 92.3 ms: 1.03x faster                                                    |
| 2to3                             | 112 ms                                                         | 110 ms: 1.02x faster                                                     |
| sympy_integrate                  | 7.53 ms                                                        | 7.48 ms: 1.01x faster                                                    |
| dask                             | 525 ms                                                         | 522 ms: 1.01x faster                                                     |
| python_startup                   | 8.63 ms                                                        | 8.65 ms: 1.00x slower                                                    |
| gc_traversal                     | 2.04 ms                                                        | 2.09 ms: 1.02x slower                                                    |
| scimark_sparse_mat_mult          | 1.78 ms                                                        | 1.83 ms: 1.03x slower                                                    |
| coverage                         | 31.2 ms                                                        | 32.6 ms: 1.04x slower                                                    |
| bench_thread_pool                | 412 us                                                         | 432 us: 1.05x slower                                                     |
| python_startup_no_site           | 5.95 ms                                                        | 6.26 ms: 1.05x slower                                                    |
| sympy_sum                        | 52.3 ms                                                        | 55.8 ms: 1.07x slower                                                    |
| sympy_expand                     | 159 ms                                                         | 172 ms: 1.08x slower                                                     |
| django_template                  | 12.5 ms                                                        | 13.8 ms: 1.10x slower                                                    |
| sympy_str                        | 95.5 ms                                                        | 106 ms: 1.11x slower                                                     |
| async_tree_eager_cpu_io_mixed_tg | 208 ms                                                         | 232 ms: 1.12x slower                                                     |
| pylint                           | 106 ms                                                         | 118 ms: 1.12x slower                                                     |
| async_tree_eager_memoization_tg  | 103 ms                                                         | 119 ms: 1.16x slower                                                     |
| bench_mp_pool                    | 37.8 ms                                                        | 43.8 ms: 1.16x slower                                                    |
| generators                       | 15.7 ms                                                        | 18.3 ms: 1.16x slower                                                    |
| many_optionals                   | 200 us                                                         | 246 us: 1.23x slower                                                     |
| async_tree_eager_tg              | 28.9 ms                                                        | 77.7 ms: 2.68x slower                                                    |
| Geometric mean                   | (ref)                                                          | 1.17x faster                                                             |

Benchmark hidden because not significant (1): sqlite_synth
Ignored benchmarks (10) of results/bm-20240906-3.13.0rc2-ec61006/bm-20240906-macm4pro-arm64-python-v3.13.0rc2-3.13.0rc2-ec61006.json: chameleon, djangocms, gevent_hub, sqlalchemy_declarative, sqlalchemy_imperative, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http
Ignored benchmarks (12) of results/bm-20260130-3.15.0a5+-ccbe41e-JIT/bm-20260130-macm4pro-arm64-python-ccbe41e27cdb44103318-3.15.0a5+-ccbe41e.json: asyncio_tcp, asyncio_tcp_ssl, pickle, pickle_dict, pickle_list, sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile, unpack_sequence, unpickle, unpickle_list

- Geometric mean (including insignificant results): 1.177x faster

# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.10x
- 95% likely to have a speedup of 1.09x
- 99% likely to have a speedup of 1.07x

# Memory
- memory change: 1.21x