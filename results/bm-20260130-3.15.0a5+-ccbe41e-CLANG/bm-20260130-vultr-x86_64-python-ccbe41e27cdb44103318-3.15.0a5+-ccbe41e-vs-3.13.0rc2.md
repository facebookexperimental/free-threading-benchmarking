# Results vs. 3.13.0rc2

- fork: python
- ref: ccbe41e27cdb44103318
- machine: linux-x86_64
- commit hash: ccbe41e
- commit date: 2026-01-30
- overall geometric mean: 1.088x faster
- HPT reliability: 100.00%
- HPT 99th percentile: 1.06x faster
- Memory change: 1.18x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260130-vultr-x86_64-python-ccbe41e27cdb44103318-3.15.0a5+-ccbe41e |
|----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| 2to3           | 260 ms                                                       | 245 ms: 1.06x faster                                                   |
| docutils       | 2.62 sec                                                     | 2.48 sec: 1.05x faster                                                 |
| html5lib       | 67.0 ms                                                      | 58.8 ms: 1.14x faster                                                  |
| Geometric mean | (ref)                                                        | 1.08x faster                                                           |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260130-vultr-x86_64-python-ccbe41e27cdb44103318-3.15.0a5+-ccbe41e |
|----------------------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| async_tree_io_tg           | 913 ms                                                       | 536 ms: 1.70x faster                                                   |
| async_tree_memoization     | 461 ms                                                       | 282 ms: 1.64x faster                                                   |
| async_tree_io              | 876 ms                                                       | 544 ms: 1.61x faster                                                   |
| async_tree_none            | 354 ms                                                       | 237 ms: 1.49x faster                                                   |
| async_tree_memoization_tg  | 414 ms                                                       | 283 ms: 1.46x faster                                                   |
| async_tree_none_tg         | 336 ms                                                       | 235 ms: 1.43x faster                                                   |
| async_tree_cpu_io_mixed    | 666 ms                                                       | 467 ms: 1.43x faster                                                   |
| async_tree_cpu_io_mixed_tg | 638 ms                                                       | 464 ms: 1.37x faster                                                   |
| async_generators           | 377 ms                                                       | 312 ms: 1.21x faster                                                   |
| asyncio_tcp                | 505 ms                                                       | 437 ms: 1.16x faster                                                   |
| coroutines                 | 23.6 ms                                                      | 22.5 ms: 1.05x faster                                                  |
| asyncio_tcp_ssl            | 1.51 sec                                                     | 1.45 sec: 1.04x faster                                                 |
| asyncio_websockets         | 520 ms                                                       | 543 ms: 1.04x slower                                                   |
| Geometric mean             | (ref)                                                        | 1.33x faster                                                           |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260130-vultr-x86_64-python-ccbe41e27cdb44103318-3.15.0a5+-ccbe41e |
|----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| float          | 77.5 ms                                                      | 65.6 ms: 1.18x faster                                                  |
| pidigits       | 217 ms                                                       | 195 ms: 1.11x faster                                                   |
| nbody          | 85.1 ms                                                      | 79.4 ms: 1.07x faster                                                  |
| Geometric mean | (ref)                                                        | 1.12x faster                                                           |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260130-vultr-x86_64-python-ccbe41e27cdb44103318-3.15.0a5+-ccbe41e |
|----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| regex_compile  | 132 ms                                                       | 119 ms: 1.11x faster                                                   |
| regex_effbot   | 3.08 ms                                                      | 3.05 ms: 1.01x faster                                                  |
| regex_v8       | 22.7 ms                                                      | 22.9 ms: 1.01x slower                                                  |
| regex_dna      | 180 ms                                                       | 190 ms: 1.05x slower                                                   |
| Geometric mean | (ref)                                                        | 1.02x faster                                                           |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260130-vultr-x86_64-python-ccbe41e27cdb44103318-3.15.0a5+-ccbe41e |
|----------------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| json_dumps           | 10.5 ms                                                      | 9.23 ms: 1.14x faster                                                  |
| tomli_loads          | 2.01 sec                                                     | 1.78 sec: 1.13x faster                                                 |
| xml_etree_process    | 59.3 ms                                                      | 53.9 ms: 1.10x faster                                                  |
| xml_etree_iterparse  | 94.9 ms                                                      | 86.6 ms: 1.10x faster                                                  |
| xml_etree_generate   | 85.4 ms                                                      | 79.4 ms: 1.08x faster                                                  |
| json_loads           | 27.0 us                                                      | 25.6 us: 1.05x faster                                                  |
| unpickle_pure_python | 210 us                                                       | 206 us: 1.02x faster                                                   |
| pickle_dict          | 32.5 us                                                      | 31.9 us: 1.02x faster                                                  |
| unpickle_list        | 4.71 us                                                      | 4.64 us: 1.02x faster                                                  |
| pickle_list          | 4.93 us                                                      | 5.00 us: 1.01x slower                                                  |
| unpickle             | 14.3 us                                                      | 15.1 us: 1.05x slower                                                  |
| xml_etree_parse      | 136 ms                                                       | 144 ms: 1.06x slower                                                   |
| pickle               | 11.3 us                                                      | 13.0 us: 1.15x slower                                                  |
| Geometric mean       | (ref)                                                        | 1.03x faster                                                           |

Benchmark hidden because not significant (1): pickle_pure_python

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260130-vultr-x86_64-python-ccbe41e27cdb44103318-3.15.0a5+-ccbe41e |
|------------------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| python_startup_no_site | 7.39 ms                                                      | 7.34 ms: 1.01x faster                                                  |
| python_startup         | 11.0 ms                                                      | 12.5 ms: 1.13x slower                                                  |
| Geometric mean         | (ref)                                                        | 1.06x slower                                                           |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260130-vultr-x86_64-python-ccbe41e27cdb44103318-3.15.0a5+-ccbe41e |
|-----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| genshi_text     | 21.5 ms                                                      | 19.8 ms: 1.09x faster                                                  |
| django_template | 34.1 ms                                                      | 32.2 ms: 1.06x faster                                                  |
| genshi_xml      | 48.8 ms                                                      | 46.7 ms: 1.04x faster                                                  |
| mako            | 11.3 ms                                                      | 11.9 ms: 1.05x slower                                                  |
| Geometric mean  | (ref)                                                        | 1.04x faster                                                           |

All benchmarks:
===============

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260130-vultr-x86_64-python-ccbe41e27cdb44103318-3.15.0a5+-ccbe41e |
|----------------------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| mdp                        | 2.36 sec                                                     | 1.10 sec: 2.14x faster                                                 |
| deepcopy                   | 355 us                                                       | 208 us: 1.71x faster                                                   |
| async_tree_io_tg           | 913 ms                                                       | 536 ms: 1.70x faster                                                   |
| async_tree_memoization     | 461 ms                                                       | 282 ms: 1.64x faster                                                   |
| async_tree_io              | 876 ms                                                       | 544 ms: 1.61x faster                                                   |
| deepcopy_memo              | 39.1 us                                                      | 24.7 us: 1.58x faster                                                  |
| async_tree_none            | 354 ms                                                       | 237 ms: 1.49x faster                                                   |
| async_tree_memoization_tg  | 414 ms                                                       | 283 ms: 1.46x faster                                                   |
| async_tree_none_tg         | 336 ms                                                       | 235 ms: 1.43x faster                                                   |
| go                         | 141 ms                                                       | 98.5 ms: 1.43x faster                                                  |
| async_tree_cpu_io_mixed    | 666 ms                                                       | 467 ms: 1.43x faster                                                   |
| spectral_norm              | 111 ms                                                       | 80.0 ms: 1.39x faster                                                  |
| async_tree_cpu_io_mixed_tg | 638 ms                                                       | 464 ms: 1.37x faster                                                   |
| deepcopy_reduce            | 3.11 us                                                      | 2.28 us: 1.37x faster                                                  |
| scimark_sor                | 134 ms                                                       | 99.7 ms: 1.35x faster                                                  |
| scimark_fft                | 349 ms                                                       | 263 ms: 1.33x faster                                                   |
| logging_silent             | 103 ns                                                       | 84.8 ns: 1.21x faster                                                  |
| async_generators           | 377 ms                                                       | 312 ms: 1.21x faster                                                   |
| scimark_sparse_mat_mult    | 4.71 ms                                                      | 3.96 ms: 1.19x faster                                                  |
| float                      | 77.5 ms                                                      | 65.6 ms: 1.18x faster                                                  |
| scimark_monte_carlo        | 65.4 ms                                                      | 55.9 ms: 1.17x faster                                                  |
| richards                   | 45.2 ms                                                      | 39.0 ms: 1.16x faster                                                  |
| asyncio_tcp                | 505 ms                                                       | 437 ms: 1.16x faster                                                   |
| richards_super             | 51.6 ms                                                      | 44.8 ms: 1.15x faster                                                  |
| pylint                     | 317 ms                                                       | 277 ms: 1.15x faster                                                   |
| pathlib                    | 19.2 ms                                                      | 16.8 ms: 1.14x faster                                                  |
| json_dumps                 | 10.5 ms                                                      | 9.23 ms: 1.14x faster                                                  |
| html5lib                   | 67.0 ms                                                      | 58.8 ms: 1.14x faster                                                  |
| pyflate                    | 449 ms                                                       | 394 ms: 1.14x faster                                                   |
| tomli_loads                | 2.01 sec                                                     | 1.78 sec: 1.13x faster                                                 |
| comprehensions             | 16.5 us                                                      | 14.6 us: 1.12x faster                                                  |
| chaos                      | 57.3 ms                                                      | 51.3 ms: 1.12x faster                                                  |
| regex_compile              | 132 ms                                                       | 119 ms: 1.11x faster                                                   |
| dulwich_log                | 74.8 ms                                                      | 67.3 ms: 1.11x faster                                                  |
| logging_simple             | 6.16 us                                                      | 5.55 us: 1.11x faster                                                  |
| pidigits                   | 217 ms                                                       | 195 ms: 1.11x faster                                                   |
| scimark_lu                 | 113 ms                                                       | 102 ms: 1.10x faster                                                   |
| xml_etree_process          | 59.3 ms                                                      | 53.9 ms: 1.10x faster                                                  |
| bpe_tokeniser              | 4.45 sec                                                     | 4.05 sec: 1.10x faster                                                 |
| coverage                   | 83.0 ms                                                      | 75.6 ms: 1.10x faster                                                  |
| xml_etree_iterparse        | 94.9 ms                                                      | 86.6 ms: 1.10x faster                                                  |
| sympy_integrate            | 19.8 ms                                                      | 18.1 ms: 1.09x faster                                                  |
| logging_format             | 6.84 us                                                      | 6.25 us: 1.09x faster                                                  |
| hexiom                     | 5.99 ms                                                      | 5.48 ms: 1.09x faster                                                  |
| deltablue                  | 3.12 ms                                                      | 2.87 ms: 1.09x faster                                                  |
| genshi_text                | 21.5 ms                                                      | 19.8 ms: 1.09x faster                                                  |
| typing_runtime_protocols   | 155 us                                                       | 142 us: 1.09x faster                                                   |
| xml_etree_generate         | 85.4 ms                                                      | 79.4 ms: 1.08x faster                                                  |
| nbody                      | 85.1 ms                                                      | 79.4 ms: 1.07x faster                                                  |
| pprint_safe_repr           | 738 ms                                                       | 690 ms: 1.07x faster                                                   |
| pprint_pformat             | 1.50 sec                                                     | 1.40 sec: 1.07x faster                                                 |
| raytrace                   | 253 ms                                                       | 238 ms: 1.06x faster                                                   |
| 2to3                       | 260 ms                                                       | 245 ms: 1.06x faster                                                   |
| django_template            | 34.1 ms                                                      | 32.2 ms: 1.06x faster                                                  |
| generators                 | 28.8 ms                                                      | 27.2 ms: 1.06x faster                                                  |
| thrift                     | 778 us                                                       | 737 us: 1.06x faster                                                   |
| docutils                   | 2.62 sec                                                     | 2.48 sec: 1.05x faster                                                 |
| json_loads                 | 27.0 us                                                      | 25.6 us: 1.05x faster                                                  |
| sympy_sum                  | 156 ms                                                       | 148 ms: 1.05x faster                                                   |
| nqueens                    | 78.6 ms                                                      | 75.0 ms: 1.05x faster                                                  |
| coroutines                 | 23.6 ms                                                      | 22.5 ms: 1.05x faster                                                  |
| genshi_xml                 | 48.8 ms                                                      | 46.7 ms: 1.04x faster                                                  |
| asyncio_tcp_ssl            | 1.51 sec                                                     | 1.45 sec: 1.04x faster                                                 |
| sympy_str                  | 275 ms                                                       | 264 ms: 1.04x faster                                                   |
| pycparser                  | 1.12 sec                                                     | 1.07 sec: 1.04x faster                                                 |
| sympy_expand               | 457 ms                                                       | 445 ms: 1.03x faster                                                   |
| unpickle_pure_python       | 210 us                                                       | 206 us: 1.02x faster                                                   |
| pickle_dict                | 32.5 us                                                      | 31.9 us: 1.02x faster                                                  |
| unpickle_list              | 4.71 us                                                      | 4.64 us: 1.02x faster                                                  |
| json                       | 4.93 ms                                                      | 4.85 ms: 1.02x faster                                                  |
| regex_effbot               | 3.08 ms                                                      | 3.05 ms: 1.01x faster                                                  |
| python_startup_no_site     | 7.39 ms                                                      | 7.34 ms: 1.01x faster                                                  |
| fannkuch                   | 370 ms                                                       | 368 ms: 1.00x faster                                                   |
| regex_v8                   | 22.7 ms                                                      | 22.9 ms: 1.01x slower                                                  |
| pickle_list                | 4.93 us                                                      | 5.00 us: 1.01x slower                                                  |
| meteor_contest             | 102 ms                                                       | 103 ms: 1.02x slower                                                   |
| asyncio_websockets         | 520 ms                                                       | 543 ms: 1.04x slower                                                   |
| mako                       | 11.3 ms                                                      | 11.9 ms: 1.05x slower                                                  |
| regex_dna                  | 180 ms                                                       | 190 ms: 1.05x slower                                                   |
| unpickle                   | 14.3 us                                                      | 15.1 us: 1.05x slower                                                  |
| xml_etree_parse            | 136 ms                                                       | 144 ms: 1.06x slower                                                   |
| unpack_sequence            | 44.8 ns                                                      | 47.8 ns: 1.07x slower                                                  |
| python_startup             | 11.0 ms                                                      | 12.5 ms: 1.13x slower                                                  |
| pickle                     | 11.3 us                                                      | 13.0 us: 1.15x slower                                                  |
| bench_thread_pool          | 919 us                                                       | 1.26 ms: 1.38x slower                                                  |
| gc_traversal               | 3.14 ms                                                      | 4.41 ms: 1.40x slower                                                  |
| create_gc_cycles           | 1.34 ms                                                      | 1.90 ms: 1.42x slower                                                  |
| telco                      | 7.82 ms                                                      | 155 ms: 19.76x slower                                                  |
| bench_mp_pool              | 11.0 ms                                                      | 278 ms: 25.25x slower                                                  |
| Geometric mean             | (ref)                                                        | 1.04x faster                                                           |

Benchmark hidden because not significant (3): pickle_pure_python, crypto_pyaes, sqlite_synth
Ignored benchmarks (10) of results/bm-20240906-3.13.0rc2-ec61006/bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006.json: aiohttp, chameleon, dask, flaskblogging, gunicorn, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http
Ignored benchmarks (10) of results/bm-20260130-3.15.0a5+-ccbe41e-CLANG/bm-20260130-vultr-x86_64-python-ccbe41e27cdb44103318-3.15.0a5+-ccbe41e.json: connected_components, k_core, many_optionals, shortest_path, sphinx, sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile, subparsers

- Geometric mean (including insignificant results): 1.088x faster

# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.07x
- 95% likely to have a speedup of 1.07x
- 99% likely to have a speedup of 1.06x

# Memory
- memory change: 1.18x