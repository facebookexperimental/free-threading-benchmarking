# Results vs. 3.12.6

- fork: python
- ref: 1fd66eadd258223a0e34
- machine: linux-x86_64
- commit hash: 1fd66ea
- commit date: 2026-03-28
- overall geometric mean: 1.152x faster
- HPT reliability: 100.00%
- HPT 99th percentile: 1.07x faster
- Memory change: 1.21x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260328-vultr-x86_64-python-1fd66eadd258223a0e34-3.15.0a7+-1fd66ea |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| 2to3           | 264 ms                                                 | 250 ms: 1.05x faster                                                   |
| docutils       | 2.64 sec                                               | 2.61 sec: 1.01x faster                                                 |
| Geometric mean | (ref)                                                  | 1.02x faster                                                           |

Benchmark hidden because not significant (1): html5lib

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260328-vultr-x86_64-python-1fd66eadd258223a0e34-3.15.0a7+-1fd66ea |
|----------------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| async_tree_io_tg           | 1.11 sec                                               | 555 ms: 2.00x faster                                                   |
| async_tree_none            | 464 ms                                                 | 234 ms: 1.99x faster                                                   |
| async_tree_memoization_tg  | 560 ms                                                 | 293 ms: 1.91x faster                                                   |
| async_tree_io              | 1.08 sec                                               | 568 ms: 1.91x faster                                                   |
| async_tree_none_tg         | 446 ms                                                 | 235 ms: 1.90x faster                                                   |
| async_tree_memoization     | 555 ms                                                 | 295 ms: 1.88x faster                                                   |
| async_tree_cpu_io_mixed    | 715 ms                                                 | 499 ms: 1.43x faster                                                   |
| async_tree_cpu_io_mixed_tg | 723 ms                                                 | 509 ms: 1.42x faster                                                   |
| asyncio_tcp                | 519 ms                                                 | 450 ms: 1.15x faster                                                   |
| async_generators           | 384 ms                                                 | 360 ms: 1.07x faster                                                   |
| asyncio_tcp_ssl            | 1.51 sec                                               | 1.46 sec: 1.03x faster                                                 |
| asyncio_websockets         | 517 ms                                                 | 544 ms: 1.05x slower                                                   |
| Geometric mean             | (ref)                                                  | 1.45x faster                                                           |

Benchmark hidden because not significant (1): coroutines

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260328-vultr-x86_64-python-1fd66eadd258223a0e34-3.15.0a7+-1fd66ea |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| float          | 80.8 ms                                                | 58.4 ms: 1.38x faster                                                  |
| nbody          | 89.3 ms                                                | 65.1 ms: 1.37x faster                                                  |
| pidigits       | 184 ms                                                 | 197 ms: 1.07x slower                                                   |
| Geometric mean | (ref)                                                  | 1.21x faster                                                           |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260328-vultr-x86_64-python-1fd66eadd258223a0e34-3.15.0a7+-1fd66ea |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| regex_effbot   | 3.17 ms                                                | 2.73 ms: 1.16x faster                                                  |
| regex_compile  | 142 ms                                                 | 128 ms: 1.11x faster                                                   |
| regex_v8       | 20.6 ms                                                | 20.8 ms: 1.01x slower                                                  |
| regex_dna      | 168 ms                                                 | 175 ms: 1.04x slower                                                   |
| Geometric mean | (ref)                                                  | 1.05x faster                                                           |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260328-vultr-x86_64-python-1fd66eadd258223a0e34-3.15.0a7+-1fd66ea |
|----------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| tomli_loads          | 2.11 sec                                               | 1.52 sec: 1.39x faster                                                 |
| json_dumps           | 10.4 ms                                                | 8.78 ms: 1.18x faster                                                  |
| xml_etree_iterparse  | 96.7 ms                                                | 84.3 ms: 1.15x faster                                                  |
| unpickle_pure_python | 221 us                                                 | 200 us: 1.10x faster                                                   |
| xml_etree_process    | 59.0 ms                                                | 53.5 ms: 1.10x faster                                                  |
| xml_etree_parse      | 139 ms                                                 | 131 ms: 1.06x faster                                                   |
| xml_etree_generate   | 85.2 ms                                                | 80.3 ms: 1.06x faster                                                  |
| pickle_dict          | 31.8 us                                                | 31.2 us: 1.02x faster                                                  |
| pickle_list          | 4.77 us                                                | 4.69 us: 1.02x faster                                                  |
| pickle_pure_python   | 308 us                                                 | 316 us: 1.03x slower                                                   |
| json_loads           | 26.5 us                                                | 27.8 us: 1.05x slower                                                  |
| unpickle             | 14.1 us                                                | 15.1 us: 1.07x slower                                                  |
| pickle               | 10.9 us                                                | 12.3 us: 1.12x slower                                                  |
| unpickle_list        | 4.67 us                                                | 5.37 us: 1.15x slower                                                  |
| Geometric mean       | (ref)                                                  | 1.04x faster                                                           |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260328-vultr-x86_64-python-1fd66eadd258223a0e34-3.15.0a7+-1fd66ea |
|------------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| python_startup_no_site | 7.16 ms                                                | 7.53 ms: 1.05x slower                                                  |
| python_startup         | 9.93 ms                                                | 12.8 ms: 1.29x slower                                                  |
| Geometric mean         | (ref)                                                  | 1.16x slower                                                           |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260328-vultr-x86_64-python-1fd66eadd258223a0e34-3.15.0a7+-1fd66ea |
|-----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| genshi_text     | 22.8 ms                                                | 20.5 ms: 1.11x faster                                                  |
| django_template | 34.7 ms                                                | 32.3 ms: 1.07x faster                                                  |
| Geometric mean  | (ref)                                                  | 1.04x faster                                                           |

Benchmark hidden because not significant (2): mako, genshi_xml

All benchmarks:
===============

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260328-vultr-x86_64-python-1fd66eadd258223a0e34-3.15.0a7+-1fd66ea |
|----------------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| richards                   | 45.9 ms                                                | 18.1 ms: 2.54x faster                                                  |
| richards_super             | 51.9 ms                                                | 21.8 ms: 2.38x faster                                                  |
| mdp                        | 2.42 sec                                               | 1.17 sec: 2.07x faster                                                 |
| async_tree_io_tg           | 1.11 sec                                               | 555 ms: 2.00x faster                                                   |
| async_tree_none            | 464 ms                                                 | 234 ms: 1.99x faster                                                   |
| async_tree_memoization_tg  | 560 ms                                                 | 293 ms: 1.91x faster                                                   |
| async_tree_io              | 1.08 sec                                               | 568 ms: 1.91x faster                                                   |
| async_tree_none_tg         | 446 ms                                                 | 235 ms: 1.90x faster                                                   |
| async_tree_memoization     | 555 ms                                                 | 295 ms: 1.88x faster                                                   |
| deepcopy_memo              | 40.3 us                                                | 22.8 us: 1.76x faster                                                  |
| scimark_sor                | 130 ms                                                 | 78.6 ms: 1.65x faster                                                  |
| spectral_norm              | 110 ms                                                 | 70.8 ms: 1.56x faster                                                  |
| deepcopy                   | 352 us                                                 | 230 us: 1.53x faster                                                   |
| scimark_lu                 | 114 ms                                                 | 77.6 ms: 1.47x faster                                                  |
| async_tree_cpu_io_mixed    | 715 ms                                                 | 499 ms: 1.43x faster                                                   |
| async_tree_cpu_io_mixed_tg | 723 ms                                                 | 509 ms: 1.42x faster                                                   |
| go                         | 139 ms                                                 | 100.0 ms: 1.39x faster                                                 |
| tomli_loads                | 2.11 sec                                               | 1.52 sec: 1.39x faster                                                 |
| float                      | 80.8 ms                                                | 58.4 ms: 1.38x faster                                                  |
| logging_silent             | 109 ns                                                 | 79.3 ns: 1.37x faster                                                  |
| nbody                      | 89.3 ms                                                | 65.1 ms: 1.37x faster                                                  |
| scimark_fft                | 342 ms                                                 | 249 ms: 1.37x faster                                                   |
| scimark_monte_carlo        | 68.4 ms                                                | 50.5 ms: 1.36x faster                                                  |
| comprehensions             | 19.8 us                                                | 14.7 us: 1.34x faster                                                  |
| deltablue                  | 3.45 ms                                                | 2.66 ms: 1.30x faster                                                  |
| pyflate                    | 448 ms                                                 | 356 ms: 1.26x faster                                                   |
| chaos                      | 62.8 ms                                                | 50.0 ms: 1.26x faster                                                  |
| logging_simple             | 6.63 us                                                | 5.37 us: 1.24x faster                                                  |
| deepcopy_reduce            | 3.08 us                                                | 2.50 us: 1.23x faster                                                  |
| logging_format             | 7.35 us                                                | 6.05 us: 1.21x faster                                                  |
| bpe_tokeniser              | 4.74 sec                                               | 3.90 sec: 1.21x faster                                                 |
| pathlib                    | 21.5 ms                                                | 17.9 ms: 1.20x faster                                                  |
| pprint_pformat             | 1.52 sec                                               | 1.27 sec: 1.20x faster                                                 |
| pprint_safe_repr           | 743 ms                                                 | 623 ms: 1.19x faster                                                   |
| json_dumps                 | 10.4 ms                                                | 8.78 ms: 1.18x faster                                                  |
| crypto_pyaes               | 76.6 ms                                                | 65.1 ms: 1.18x faster                                                  |
| regex_effbot               | 3.17 ms                                                | 2.73 ms: 1.16x faster                                                  |
| asyncio_tcp                | 519 ms                                                 | 450 ms: 1.15x faster                                                   |
| xml_etree_iterparse        | 96.7 ms                                                | 84.3 ms: 1.15x faster                                                  |
| nqueens                    | 80.1 ms                                                | 70.0 ms: 1.14x faster                                                  |
| dulwich_log                | 78.9 ms                                                | 69.7 ms: 1.13x faster                                                  |
| fannkuch                   | 372 ms                                                 | 333 ms: 1.12x faster                                                   |
| genshi_text                | 22.8 ms                                                | 20.5 ms: 1.11x faster                                                  |
| regex_compile              | 142 ms                                                 | 128 ms: 1.11x faster                                                   |
| generators                 | 32.2 ms                                                | 29.0 ms: 1.11x faster                                                  |
| unpickle_pure_python       | 221 us                                                 | 200 us: 1.10x faster                                                   |
| xml_etree_process          | 59.0 ms                                                | 53.5 ms: 1.10x faster                                                  |
| scimark_sparse_mat_mult    | 4.39 ms                                                | 4.03 ms: 1.09x faster                                                  |
| pylint                     | 319 ms                                                 | 295 ms: 1.08x faster                                                   |
| typing_runtime_protocols   | 163 us                                                 | 152 us: 1.08x faster                                                   |
| django_template            | 34.7 ms                                                | 32.3 ms: 1.07x faster                                                  |
| hexiom                     | 6.17 ms                                                | 5.75 ms: 1.07x faster                                                  |
| async_generators           | 384 ms                                                 | 360 ms: 1.07x faster                                                   |
| xml_etree_parse            | 139 ms                                                 | 131 ms: 1.06x faster                                                   |
| xml_etree_generate         | 85.2 ms                                                | 80.3 ms: 1.06x faster                                                  |
| 2to3                       | 264 ms                                                 | 250 ms: 1.05x faster                                                   |
| meteor_contest             | 104 ms                                                 | 98.6 ms: 1.05x faster                                                  |
| asyncio_tcp_ssl            | 1.51 sec                                               | 1.46 sec: 1.03x faster                                                 |
| raytrace                   | 299 ms                                                 | 292 ms: 1.02x faster                                                   |
| pickle_dict                | 31.8 us                                                | 31.2 us: 1.02x faster                                                  |
| sympy_integrate            | 20.5 ms                                                | 20.2 ms: 1.02x faster                                                  |
| pickle_list                | 4.77 us                                                | 4.69 us: 1.02x faster                                                  |
| docutils                   | 2.64 sec                                               | 2.61 sec: 1.01x faster                                                 |
| json                       | 5.02 ms                                                | 4.97 ms: 1.01x faster                                                  |
| sqlite_synth               | 2.20 us                                                | 2.18 us: 1.01x faster                                                  |
| regex_v8                   | 20.6 ms                                                | 20.8 ms: 1.01x slower                                                  |
| thrift                     | 791 us                                                 | 801 us: 1.01x slower                                                   |
| sympy_str                  | 292 ms                                                 | 297 ms: 1.02x slower                                                   |
| sympy_sum                  | 166 ms                                                 | 170 ms: 1.02x slower                                                   |
| pickle_pure_python         | 308 us                                                 | 316 us: 1.03x slower                                                   |
| sympy_expand               | 468 ms                                                 | 487 ms: 1.04x slower                                                   |
| regex_dna                  | 168 ms                                                 | 175 ms: 1.04x slower                                                   |
| json_loads                 | 26.5 us                                                | 27.8 us: 1.05x slower                                                  |
| python_startup_no_site     | 7.16 ms                                                | 7.53 ms: 1.05x slower                                                  |
| asyncio_websockets         | 517 ms                                                 | 544 ms: 1.05x slower                                                   |
| pidigits                   | 184 ms                                                 | 197 ms: 1.07x slower                                                   |
| unpickle                   | 14.1 us                                                | 15.1 us: 1.07x slower                                                  |
| pickle                     | 10.9 us                                                | 12.3 us: 1.12x slower                                                  |
| coverage                   | 71.4 ms                                                | 81.8 ms: 1.15x slower                                                  |
| unpickle_list              | 4.67 us                                                | 5.37 us: 1.15x slower                                                  |
| gc_traversal               | 3.46 ms                                                | 4.14 ms: 1.20x slower                                                  |
| python_startup             | 9.93 ms                                                | 12.8 ms: 1.29x slower                                                  |
| unpack_sequence            | 52.1 ns                                                | 71.4 ns: 1.37x slower                                                  |
| bench_thread_pool          | 941 us                                                 | 1.33 ms: 1.41x slower                                                  |
| create_gc_cycles           | 1.09 ms                                                | 1.88 ms: 1.73x slower                                                  |
| bench_mp_pool              | 10.8 ms                                                | 163 ms: 15.05x slower                                                  |
| telco                      | 6.53 ms                                                | 153 ms: 23.42x slower                                                  |
| Geometric mean             | (ref)                                                  | 1.09x faster                                                           |

Benchmark hidden because not significant (5): html5lib, pycparser, mako, coroutines, genshi_xml
Ignored benchmarks (13) of results/bm-20240906-3.12.6-a4a2d2b/bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b.json: aiohttp, chameleon, dask, flaskblogging, gunicorn, mypy2, sqlalchemy_declarative, sqlalchemy_imperative, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http
Ignored benchmarks (10) of results/bm-20260328-3.15.0a7+-1fd66ea-JIT/bm-20260328-vultr-x86_64-python-1fd66eadd258223a0e34-3.15.0a7+-1fd66ea.json: connected_components, k_core, many_optionals, shortest_path, sphinx, sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile, subparsers

- Geometric mean (including insignificant results): 1.152x faster

# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.11x
- 95% likely to have a speedup of 1.09x
- 99% likely to have a speedup of 1.07x

# Memory
- memory change: 1.21x