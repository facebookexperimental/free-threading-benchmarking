# Results vs. 3.12.6

- fork: python
- ref: 18347417b06d09fa7eea
- machine: darwin-arm64
- commit hash: 1834741
- commit date: 2026-02-01
- overall geometric mean: 1.124x faster
- HPT reliability: 100.00%
- HPT 99th percentile: 1.03x faster
- Memory change: 1.36x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260201-macm4pro-arm64-python-18347417b06d09fa7eea-3.15.0a5+-1834741 |
|----------------|:--------------------------------------------------------:|:------------------------------------------------------------------------:|
| 2to3           | 114 ms                                                   | 115 ms: 1.01x slower                                                     |
| docutils       | 1.02 sec                                                 | 970 ms: 1.05x faster                                                     |
| html5lib       | 23.0 ms                                                  | 22.3 ms: 1.03x faster                                                    |
| sphinx         | 434 ms                                                   | 409 ms: 1.06x faster                                                     |
| Geometric mean | (ref)                                                    | 1.03x faster                                                             |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                        | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260201-macm4pro-arm64-python-18347417b06d09fa7eea-3.15.0a5+-1834741 |
|----------------------------------|:--------------------------------------------------------:|:------------------------------------------------------------------------:|
| async_tree_eager_io              | 496 ms                                                   | 238 ms: 2.08x faster                                                     |
| async_tree_io_tg                 | 480 ms                                                   | 242 ms: 1.99x faster                                                     |
| async_tree_io                    | 459 ms                                                   | 239 ms: 1.92x faster                                                     |
| async_tree_eager_io_tg           | 446 ms                                                   | 236 ms: 1.89x faster                                                     |
| async_tree_memoization_tg        | 231 ms                                                   | 141 ms: 1.63x faster                                                     |
| async_tree_none                  | 178 ms                                                   | 110 ms: 1.62x faster                                                     |
| async_tree_none_tg               | 172 ms                                                   | 107 ms: 1.61x faster                                                     |
| async_tree_memoization           | 223 ms                                                   | 148 ms: 1.51x faster                                                     |
| async_tree_cpu_io_mixed_tg       | 338 ms                                                   | 250 ms: 1.35x faster                                                     |
| coroutines                       | 13.6 ms                                                  | 10.1 ms: 1.34x faster                                                    |
| async_tree_cpu_io_mixed          | 333 ms                                                   | 257 ms: 1.30x faster                                                     |
| async_tree_eager_memoization     | 132 ms                                                   | 110 ms: 1.20x faster                                                     |
| async_generators                 | 206 ms                                                   | 179 ms: 1.15x faster                                                     |
| async_tree_eager_cpu_io_mixed    | 231 ms                                                   | 217 ms: 1.07x faster                                                     |
| asyncio_websockets               | 190 ms                                                   | 183 ms: 1.04x faster                                                     |
| async_tree_eager                 | 45.6 ms                                                  | 44.8 ms: 1.02x faster                                                    |
| async_tree_eager_memoization_tg  | 113 ms                                                   | 125 ms: 1.11x slower                                                     |
| async_tree_eager_cpu_io_mixed_tg | 213 ms                                                   | 238 ms: 1.12x slower                                                     |
| async_tree_eager_tg              | 32.1 ms                                                  | 90.5 ms: 2.82x slower                                                    |
| Geometric mean                   | (ref)                                                    | 1.27x faster                                                             |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260201-macm4pro-arm64-python-18347417b06d09fa7eea-3.15.0a5+-1834741 |
|----------------|:--------------------------------------------------------:|:------------------------------------------------------------------------:|
| float          | 37.9 ms                                                  | 28.3 ms: 1.34x faster                                                    |
| pidigits       | 161 ms                                                   | 157 ms: 1.03x faster                                                     |
| nbody          | 54.2 ms                                                  | 55.3 ms: 1.02x slower                                                    |
| Geometric mean | (ref)                                                    | 1.10x faster                                                             |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260201-macm4pro-arm64-python-18347417b06d09fa7eea-3.15.0a5+-1834741 |
|----------------|:--------------------------------------------------------:|:------------------------------------------------------------------------:|
| regex_effbot   | 1.67 ms                                                  | 1.41 ms: 1.18x faster                                                    |
| regex_compile  | 54.6 ms                                                  | 49.2 ms: 1.11x faster                                                    |
| regex_dna      | 99.6 ms                                                  | 91.5 ms: 1.09x faster                                                    |
| regex_v8       | 9.59 ms                                                  | 9.02 ms: 1.06x faster                                                    |
| Geometric mean | (ref)                                                    | 1.11x faster                                                             |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260201-macm4pro-arm64-python-18347417b06d09fa7eea-3.15.0a5+-1834741 |
|----------------------|:--------------------------------------------------------:|:------------------------------------------------------------------------:|
| xml_etree_iterparse  | 51.6 ms                                                  | 37.3 ms: 1.38x faster                                                    |
| xml_etree_parse      | 67.9 ms                                                  | 56.7 ms: 1.20x faster                                                    |
| tomli_loads          | 957 ms                                                   | 843 ms: 1.14x faster                                                     |
| json_dumps           | 4.26 ms                                                  | 3.86 ms: 1.10x faster                                                    |
| xml_etree_generate   | 38.9 ms                                                  | 36.5 ms: 1.07x faster                                                    |
| xml_etree_process    | 26.7 ms                                                  | 26.4 ms: 1.01x faster                                                    |
| unpickle_pure_python | 103 us                                                   | 105 us: 1.02x slower                                                     |
| pickle_pure_python   | 139 us                                                   | 142 us: 1.02x slower                                                     |
| json_loads           | 10.9 us                                                  | 11.2 us: 1.03x slower                                                    |
| Geometric mean       | (ref)                                                    | 1.09x faster                                                             |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260201-macm4pro-arm64-python-18347417b06d09fa7eea-3.15.0a5+-1834741 |
|------------------------|:--------------------------------------------------------:|:------------------------------------------------------------------------:|
| python_startup         | 8.01 ms                                                  | 9.60 ms: 1.20x slower                                                    |
| python_startup_no_site | 5.71 ms                                                  | 6.99 ms: 1.23x slower                                                    |
| Geometric mean         | (ref)                                                    | 1.21x slower                                                             |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260201-macm4pro-arm64-python-18347417b06d09fa7eea-3.15.0a5+-1834741 |
|-----------------|:--------------------------------------------------------:|:------------------------------------------------------------------------:|
| genshi_xml      | 21.8 ms                                                  | 21.5 ms: 1.01x faster                                                    |
| django_template | 13.6 ms                                                  | 15.0 ms: 1.10x slower                                                    |
| genshi_text     | 10.5 ms                                                  | 11.6 ms: 1.10x slower                                                    |
| mako            | 4.77 ms                                                  | 5.37 ms: 1.13x slower                                                    |
| Geometric mean  | (ref)                                                    | 1.08x slower                                                             |

All benchmarks:
===============

| Benchmark                        | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260201-macm4pro-arm64-python-18347417b06d09fa7eea-3.15.0a5+-1834741 |
|----------------------------------|:--------------------------------------------------------:|:------------------------------------------------------------------------:|
| subparsers                       | 20.8 ms                                                  | 4.00 ms: 5.20x faster                                                    |
| gc_traversal                     | 2.01 ms                                                  | 761 us: 2.64x faster                                                     |
| async_tree_eager_io              | 496 ms                                                   | 238 ms: 2.08x faster                                                     |
| async_tree_io_tg                 | 480 ms                                                   | 242 ms: 1.99x faster                                                     |
| async_tree_io                    | 459 ms                                                   | 239 ms: 1.92x faster                                                     |
| async_tree_eager_io_tg           | 446 ms                                                   | 236 ms: 1.89x faster                                                     |
| mdp                              | 1.09 sec                                                 | 592 ms: 1.84x faster                                                     |
| create_gc_cycles                 | 830 us                                                   | 500 us: 1.66x faster                                                     |
| async_tree_memoization_tg        | 231 ms                                                   | 141 ms: 1.63x faster                                                     |
| async_tree_none                  | 178 ms                                                   | 110 ms: 1.62x faster                                                     |
| async_tree_none_tg               | 172 ms                                                   | 107 ms: 1.61x faster                                                     |
| deepcopy                         | 161 us                                                   | 103 us: 1.57x faster                                                     |
| async_tree_memoization           | 223 ms                                                   | 148 ms: 1.51x faster                                                     |
| deepcopy_memo                    | 18.3 us                                                  | 12.2 us: 1.50x faster                                                    |
| xml_etree_iterparse              | 51.6 ms                                                  | 37.3 ms: 1.38x faster                                                    |
| async_tree_cpu_io_mixed_tg       | 338 ms                                                   | 250 ms: 1.35x faster                                                     |
| coroutines                       | 13.6 ms                                                  | 10.1 ms: 1.34x faster                                                    |
| float                            | 37.9 ms                                                  | 28.3 ms: 1.34x faster                                                    |
| async_tree_cpu_io_mixed          | 333 ms                                                   | 257 ms: 1.30x faster                                                     |
| deepcopy_reduce                  | 1.46 us                                                  | 1.13 us: 1.29x faster                                                    |
| comprehensions                   | 9.84 us                                                  | 7.97 us: 1.24x faster                                                    |
| go                               | 70.0 ms                                                  | 58.1 ms: 1.21x faster                                                    |
| xml_etree_parse                  | 67.9 ms                                                  | 56.7 ms: 1.20x faster                                                    |
| async_tree_eager_memoization     | 132 ms                                                   | 110 ms: 1.20x faster                                                     |
| sqlite_synth                     | 967 ns                                                   | 810 ns: 1.19x faster                                                     |
| dulwich_log                      | 21.3 ms                                                  | 17.8 ms: 1.19x faster                                                    |
| regex_effbot                     | 1.67 ms                                                  | 1.41 ms: 1.18x faster                                                    |
| pathlib                          | 12.4 ms                                                  | 10.6 ms: 1.17x faster                                                    |
| raytrace                         | 145 ms                                                   | 125 ms: 1.16x faster                                                     |
| async_generators                 | 206 ms                                                   | 179 ms: 1.15x faster                                                     |
| bpe_tokeniser                    | 2.24 sec                                                 | 1.94 sec: 1.15x faster                                                   |
| logging_silent                   | 50.9 ns                                                  | 44.4 ns: 1.15x faster                                                    |
| pyflate                          | 216 ms                                                   | 189 ms: 1.14x faster                                                     |
| scimark_fft                      | 142 ms                                                   | 124 ms: 1.14x faster                                                     |
| tomli_loads                      | 957 ms                                                   | 843 ms: 1.14x faster                                                     |
| k_core                           | 1.12 sec                                                 | 988 ms: 1.13x faster                                                     |
| scimark_sor                      | 61.0 ms                                                  | 54.3 ms: 1.12x faster                                                    |
| pylint                           | 128 ms                                                   | 115 ms: 1.11x faster                                                     |
| regex_compile                    | 54.6 ms                                                  | 49.2 ms: 1.11x faster                                                    |
| pycparser                        | 497 ms                                                   | 450 ms: 1.11x faster                                                     |
| json_dumps                       | 4.26 ms                                                  | 3.86 ms: 1.10x faster                                                    |
| logging_simple                   | 2.57 us                                                  | 2.34 us: 1.10x faster                                                    |
| regex_dna                        | 99.6 ms                                                  | 91.5 ms: 1.09x faster                                                    |
| spectral_norm                    | 54.4 ms                                                  | 50.0 ms: 1.09x faster                                                    |
| generators                       | 21.9 ms                                                  | 20.2 ms: 1.09x faster                                                    |
| deltablue                        | 1.73 ms                                                  | 1.60 ms: 1.08x faster                                                    |
| logging_format                   | 2.80 us                                                  | 2.60 us: 1.08x faster                                                    |
| chaos                            | 28.9 ms                                                  | 27.0 ms: 1.07x faster                                                    |
| async_tree_eager_cpu_io_mixed    | 231 ms                                                   | 217 ms: 1.07x faster                                                     |
| xml_etree_generate               | 38.9 ms                                                  | 36.5 ms: 1.07x faster                                                    |
| regex_v8                         | 9.59 ms                                                  | 9.02 ms: 1.06x faster                                                    |
| sphinx                           | 434 ms                                                   | 409 ms: 1.06x faster                                                     |
| docutils                         | 1.02 sec                                                 | 970 ms: 1.05x faster                                                     |
| fannkuch                         | 176 ms                                                   | 167 ms: 1.05x faster                                                     |
| nqueens                          | 43.5 ms                                                  | 41.5 ms: 1.05x faster                                                    |
| asyncio_websockets               | 190 ms                                                   | 183 ms: 1.04x faster                                                     |
| html5lib                         | 23.0 ms                                                  | 22.3 ms: 1.03x faster                                                    |
| pidigits                         | 161 ms                                                   | 157 ms: 1.03x faster                                                     |
| typing_runtime_protocols         | 71.0 us                                                  | 69.5 us: 1.02x faster                                                    |
| async_tree_eager                 | 45.6 ms                                                  | 44.8 ms: 1.02x faster                                                    |
| richards                         | 22.4 ms                                                  | 22.1 ms: 1.01x faster                                                    |
| pprint_safe_repr                 | 328 ms                                                   | 323 ms: 1.01x faster                                                     |
| genshi_xml                       | 21.8 ms                                                  | 21.5 ms: 1.01x faster                                                    |
| scimark_monte_carlo              | 32.2 ms                                                  | 31.8 ms: 1.01x faster                                                    |
| xml_etree_process                | 26.7 ms                                                  | 26.4 ms: 1.01x faster                                                    |
| sympy_integrate                  | 8.02 ms                                                  | 7.96 ms: 1.01x faster                                                    |
| richards_super                   | 25.4 ms                                                  | 25.2 ms: 1.01x faster                                                    |
| pprint_pformat                   | 665 ms                                                   | 660 ms: 1.01x faster                                                     |
| scimark_sparse_mat_mult          | 2.08 ms                                                  | 2.09 ms: 1.01x slower                                                    |
| hexiom                           | 3.04 ms                                                  | 3.06 ms: 1.01x slower                                                    |
| thrift                           | 322 us                                                   | 326 us: 1.01x slower                                                     |
| 2to3                             | 114 ms                                                   | 115 ms: 1.01x slower                                                     |
| unpickle_pure_python             | 103 us                                                   | 105 us: 1.02x slower                                                     |
| pickle_pure_python               | 139 us                                                   | 142 us: 1.02x slower                                                     |
| nbody                            | 54.2 ms                                                  | 55.3 ms: 1.02x slower                                                    |
| json_loads                       | 10.9 us                                                  | 11.2 us: 1.03x slower                                                    |
| sympy_sum                        | 57.6 ms                                                  | 59.3 ms: 1.03x slower                                                    |
| sympy_str                        | 104 ms                                                   | 108 ms: 1.03x slower                                                     |
| crypto_pyaes                     | 38.8 ms                                                  | 41.5 ms: 1.07x slower                                                    |
| sympy_expand                     | 167 ms                                                   | 182 ms: 1.09x slower                                                     |
| django_template                  | 13.6 ms                                                  | 15.0 ms: 1.10x slower                                                    |
| genshi_text                      | 10.5 ms                                                  | 11.6 ms: 1.10x slower                                                    |
| async_tree_eager_memoization_tg  | 113 ms                                                   | 125 ms: 1.11x slower                                                     |
| meteor_contest                   | 47.7 ms                                                  | 53.1 ms: 1.11x slower                                                    |
| async_tree_eager_cpu_io_mixed_tg | 213 ms                                                   | 238 ms: 1.12x slower                                                     |
| bench_mp_pool                    | 39.7 ms                                                  | 44.7 ms: 1.13x slower                                                    |
| mako                             | 4.77 ms                                                  | 5.37 ms: 1.13x slower                                                    |
| scimark_lu                       | 51.3 ms                                                  | 58.7 ms: 1.15x slower                                                    |
| telco                            | 2.61 ms                                                  | 3.00 ms: 1.15x slower                                                    |
| shortest_path                    | 219 ms                                                   | 253 ms: 1.15x slower                                                     |
| python_startup                   | 8.01 ms                                                  | 9.60 ms: 1.20x slower                                                    |
| connected_components             | 201 ms                                                   | 245 ms: 1.22x slower                                                     |
| python_startup_no_site           | 5.71 ms                                                  | 6.99 ms: 1.23x slower                                                    |
| many_optionals                   | 195 us                                                   | 254 us: 1.30x slower                                                     |
| bench_thread_pool                | 419 us                                                   | 565 us: 1.35x slower                                                     |
| coverage                         | 26.9 ms                                                  | 38.2 ms: 1.42x slower                                                    |
| async_tree_eager_tg              | 32.1 ms                                                  | 90.5 ms: 2.82x slower                                                    |
| Geometric mean                   | (ref)                                                    | 1.12x faster                                                             |

Benchmark hidden because not significant (2): json, dask
Ignored benchmarks (10) of results/bm-20240906-3.12.6-a4a2d2b/bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b.json: chameleon, djangocms, gevent_hub, sqlalchemy_declarative, sqlalchemy_imperative, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http
Ignored benchmarks (4) of results/bm-20260201-3.15.0a5+-1834741-NOGIL/bm-20260201-macm4pro-arm64-python-18347417b06d09fa7eea-3.15.0a5+-1834741.json: sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile

- Geometric mean (including insignificant results): 1.124x faster

# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.05x
- 95% likely to have a speedup of 1.04x
- 99% likely to have a speedup of 1.03x

# Memory
- memory change: 1.36x