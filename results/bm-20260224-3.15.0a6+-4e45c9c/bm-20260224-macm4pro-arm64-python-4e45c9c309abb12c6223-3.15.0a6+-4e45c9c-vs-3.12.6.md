# Results vs. 3.12.6

- fork: python
- ref: 4e45c9c309abb12c6223
- machine: darwin-arm64
- commit hash: 4e45c9c
- commit date: 2026-02-24
- overall geometric mean: 1.119x faster
- HPT reliability: 100.00%
- HPT 99th percentile: 1.03x faster
- Memory change: 1.23x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260224-macm4pro-arm64-python-4e45c9c309abb12c6223-3.15.0a6+-4e45c9c |
|----------------|:--------------------------------------------------------:|:------------------------------------------------------------------------:|
| 2to3           | 114 ms                                                   | 118 ms: 1.03x slower                                                     |
| docutils       | 1.02 sec                                                 | 1.06 sec: 1.04x slower                                                   |
| html5lib       | 23.0 ms                                                  | 22.8 ms: 1.01x faster                                                    |
| sphinx         | 434 ms                                                   | 409 ms: 1.06x faster                                                     |
| Geometric mean | (ref)                                                    | 1.00x faster                                                             |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                        | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260224-macm4pro-arm64-python-4e45c9c309abb12c6223-3.15.0a6+-4e45c9c |
|----------------------------------|:--------------------------------------------------------:|:------------------------------------------------------------------------:|
| async_tree_eager_io              | 496 ms                                                   | 233 ms: 2.13x faster                                                     |
| async_tree_io_tg                 | 480 ms                                                   | 246 ms: 1.95x faster                                                     |
| async_tree_eager_io_tg           | 446 ms                                                   | 233 ms: 1.92x faster                                                     |
| async_tree_io                    | 459 ms                                                   | 241 ms: 1.90x faster                                                     |
| async_tree_none_tg               | 172 ms                                                   | 102 ms: 1.68x faster                                                     |
| async_tree_none                  | 178 ms                                                   | 106 ms: 1.67x faster                                                     |
| async_tree_memoization_tg        | 231 ms                                                   | 143 ms: 1.62x faster                                                     |
| async_tree_memoization           | 223 ms                                                   | 140 ms: 1.58x faster                                                     |
| async_tree_cpu_io_mixed_tg       | 338 ms                                                   | 257 ms: 1.32x faster                                                     |
| async_tree_cpu_io_mixed          | 333 ms                                                   | 256 ms: 1.30x faster                                                     |
| coroutines                       | 13.6 ms                                                  | 10.6 ms: 1.27x faster                                                    |
| async_tree_eager_memoization     | 132 ms                                                   | 109 ms: 1.21x faster                                                     |
| async_generators                 | 206 ms                                                   | 181 ms: 1.14x faster                                                     |
| async_tree_eager                 | 45.6 ms                                                  | 42.8 ms: 1.07x faster                                                    |
| async_tree_eager_cpu_io_mixed    | 231 ms                                                   | 224 ms: 1.03x faster                                                     |
| asyncio_websockets               | 190 ms                                                   | 189 ms: 1.01x faster                                                     |
| async_tree_eager_memoization_tg  | 113 ms                                                   | 125 ms: 1.11x slower                                                     |
| async_tree_eager_cpu_io_mixed_tg | 213 ms                                                   | 244 ms: 1.15x slower                                                     |
| async_tree_eager_tg              | 32.1 ms                                                  | 84.1 ms: 2.62x slower                                                    |
| Geometric mean                   | (ref)                                                    | 1.28x faster                                                             |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260224-macm4pro-arm64-python-4e45c9c309abb12c6223-3.15.0a6+-4e45c9c |
|----------------|:--------------------------------------------------------:|:------------------------------------------------------------------------:|
| float          | 37.9 ms                                                  | 28.6 ms: 1.33x faster                                                    |
| nbody          | 54.2 ms                                                  | 48.0 ms: 1.13x faster                                                    |
| pidigits       | 161 ms                                                   | 168 ms: 1.04x slower                                                     |
| Geometric mean | (ref)                                                    | 1.13x faster                                                             |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260224-macm4pro-arm64-python-4e45c9c309abb12c6223-3.15.0a6+-4e45c9c |
|----------------|:--------------------------------------------------------:|:------------------------------------------------------------------------:|
| regex_compile  | 54.6 ms                                                  | 48.0 ms: 1.14x faster                                                    |
| regex_effbot   | 1.67 ms                                                  | 1.48 ms: 1.13x faster                                                    |
| regex_dna      | 99.6 ms                                                  | 96.5 ms: 1.03x faster                                                    |
| regex_v8       | 9.59 ms                                                  | 9.80 ms: 1.02x slower                                                    |
| Geometric mean | (ref)                                                    | 1.07x faster                                                             |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260224-macm4pro-arm64-python-4e45c9c309abb12c6223-3.15.0a6+-4e45c9c |
|----------------------|:--------------------------------------------------------:|:------------------------------------------------------------------------:|
| xml_etree_iterparse  | 51.6 ms                                                  | 41.3 ms: 1.25x faster                                                    |
| json_dumps           | 4.26 ms                                                  | 3.82 ms: 1.12x faster                                                    |
| tomli_loads          | 957 ms                                                   | 876 ms: 1.09x faster                                                     |
| xml_etree_parse      | 67.9 ms                                                  | 62.3 ms: 1.09x faster                                                    |
| xml_etree_generate   | 38.9 ms                                                  | 36.0 ms: 1.08x faster                                                    |
| xml_etree_process    | 26.7 ms                                                  | 25.8 ms: 1.03x faster                                                    |
| unpickle_pure_python | 103 us                                                   | 104 us: 1.01x slower                                                     |
| pickle_pure_python   | 139 us                                                   | 148 us: 1.06x slower                                                     |
| Geometric mean       | (ref)                                                    | 1.06x faster                                                             |

Benchmark hidden because not significant (1): json_loads

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260224-macm4pro-arm64-python-4e45c9c309abb12c6223-3.15.0a6+-4e45c9c |
|------------------------|:--------------------------------------------------------:|:------------------------------------------------------------------------:|
| python_startup         | 8.01 ms                                                  | 9.06 ms: 1.13x slower                                                    |
| python_startup_no_site | 5.71 ms                                                  | 6.53 ms: 1.15x slower                                                    |
| Geometric mean         | (ref)                                                    | 1.14x slower                                                             |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260224-macm4pro-arm64-python-4e45c9c309abb12c6223-3.15.0a6+-4e45c9c |
|-----------------|:--------------------------------------------------------:|:------------------------------------------------------------------------:|
| genshi_text     | 10.5 ms                                                  | 10.1 ms: 1.03x faster                                                    |
| mako            | 4.77 ms                                                  | 4.80 ms: 1.01x slower                                                    |
| genshi_xml      | 21.8 ms                                                  | 22.0 ms: 1.01x slower                                                    |
| django_template | 13.6 ms                                                  | 15.4 ms: 1.13x slower                                                    |
| Geometric mean  | (ref)                                                    | 1.03x slower                                                             |

All benchmarks:
===============

| Benchmark                        | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260224-macm4pro-arm64-python-4e45c9c309abb12c6223-3.15.0a6+-4e45c9c |
|----------------------------------|:--------------------------------------------------------:|:------------------------------------------------------------------------:|
| subparsers                       | 20.8 ms                                                  | 4.21 ms: 4.93x faster                                                    |
| async_tree_eager_io              | 496 ms                                                   | 233 ms: 2.13x faster                                                     |
| async_tree_io_tg                 | 480 ms                                                   | 246 ms: 1.95x faster                                                     |
| mdp                              | 1.09 sec                                                 | 560 ms: 1.95x faster                                                     |
| async_tree_eager_io_tg           | 446 ms                                                   | 233 ms: 1.92x faster                                                     |
| async_tree_io                    | 459 ms                                                   | 241 ms: 1.90x faster                                                     |
| async_tree_none_tg               | 172 ms                                                   | 102 ms: 1.68x faster                                                     |
| async_tree_none                  | 178 ms                                                   | 106 ms: 1.67x faster                                                     |
| async_tree_memoization_tg        | 231 ms                                                   | 143 ms: 1.62x faster                                                     |
| async_tree_memoization           | 223 ms                                                   | 140 ms: 1.58x faster                                                     |
| deepcopy                         | 161 us                                                   | 103 us: 1.57x faster                                                     |
| deepcopy_memo                    | 18.3 us                                                  | 11.9 us: 1.54x faster                                                    |
| float                            | 37.9 ms                                                  | 28.6 ms: 1.33x faster                                                    |
| async_tree_cpu_io_mixed_tg       | 338 ms                                                   | 257 ms: 1.32x faster                                                     |
| comprehensions                   | 9.84 us                                                  | 7.54 us: 1.31x faster                                                    |
| async_tree_cpu_io_mixed          | 333 ms                                                   | 256 ms: 1.30x faster                                                     |
| deepcopy_reduce                  | 1.46 us                                                  | 1.13 us: 1.29x faster                                                    |
| coroutines                       | 13.6 ms                                                  | 10.6 ms: 1.27x faster                                                    |
| xml_etree_iterparse              | 51.6 ms                                                  | 41.3 ms: 1.25x faster                                                    |
| go                               | 70.0 ms                                                  | 56.9 ms: 1.23x faster                                                    |
| spectral_norm                    | 54.4 ms                                                  | 44.3 ms: 1.23x faster                                                    |
| k_core                           | 1.12 sec                                                 | 914 ms: 1.22x faster                                                     |
| async_tree_eager_memoization     | 132 ms                                                   | 109 ms: 1.21x faster                                                     |
| logging_silent                   | 50.9 ns                                                  | 42.5 ns: 1.20x faster                                                    |
| scimark_sor                      | 61.0 ms                                                  | 51.1 ms: 1.19x faster                                                    |
| raytrace                         | 145 ms                                                   | 123 ms: 1.18x faster                                                     |
| scimark_fft                      | 142 ms                                                   | 121 ms: 1.17x faster                                                     |
| async_generators                 | 206 ms                                                   | 181 ms: 1.14x faster                                                     |
| regex_compile                    | 54.6 ms                                                  | 48.0 ms: 1.14x faster                                                    |
| nbody                            | 54.2 ms                                                  | 48.0 ms: 1.13x faster                                                    |
| regex_effbot                     | 1.67 ms                                                  | 1.48 ms: 1.13x faster                                                    |
| chaos                            | 28.9 ms                                                  | 25.8 ms: 1.12x faster                                                    |
| pathlib                          | 12.4 ms                                                  | 11.1 ms: 1.12x faster                                                    |
| json_dumps                       | 4.26 ms                                                  | 3.82 ms: 1.12x faster                                                    |
| deltablue                        | 1.73 ms                                                  | 1.55 ms: 1.11x faster                                                    |
| pyflate                          | 216 ms                                                   | 195 ms: 1.11x faster                                                     |
| bpe_tokeniser                    | 2.24 sec                                                 | 2.02 sec: 1.11x faster                                                   |
| nqueens                          | 43.5 ms                                                  | 39.4 ms: 1.10x faster                                                    |
| scimark_monte_carlo              | 32.2 ms                                                  | 29.2 ms: 1.10x faster                                                    |
| dulwich_log                      | 21.3 ms                                                  | 19.3 ms: 1.10x faster                                                    |
| logging_format                   | 2.80 us                                                  | 2.55 us: 1.10x faster                                                    |
| scimark_sparse_mat_mult          | 2.08 ms                                                  | 1.89 ms: 1.10x faster                                                    |
| logging_simple                   | 2.57 us                                                  | 2.34 us: 1.10x faster                                                    |
| tomli_loads                      | 957 ms                                                   | 876 ms: 1.09x faster                                                     |
| xml_etree_parse                  | 67.9 ms                                                  | 62.3 ms: 1.09x faster                                                    |
| xml_etree_generate               | 38.9 ms                                                  | 36.0 ms: 1.08x faster                                                    |
| pylint                           | 128 ms                                                   | 120 ms: 1.07x faster                                                     |
| async_tree_eager                 | 45.6 ms                                                  | 42.8 ms: 1.07x faster                                                    |
| typing_runtime_protocols         | 71.0 us                                                  | 66.8 us: 1.06x faster                                                    |
| sphinx                           | 434 ms                                                   | 409 ms: 1.06x faster                                                     |
| richards_super                   | 25.4 ms                                                  | 23.9 ms: 1.06x faster                                                    |
| richards                         | 22.4 ms                                                  | 21.2 ms: 1.06x faster                                                    |
| scimark_lu                       | 51.3 ms                                                  | 48.6 ms: 1.05x faster                                                    |
| hexiom                           | 3.04 ms                                                  | 2.91 ms: 1.04x faster                                                    |
| generators                       | 21.9 ms                                                  | 21.1 ms: 1.04x faster                                                    |
| pycparser                        | 497 ms                                                   | 481 ms: 1.03x faster                                                     |
| xml_etree_process                | 26.7 ms                                                  | 25.8 ms: 1.03x faster                                                    |
| genshi_text                      | 10.5 ms                                                  | 10.1 ms: 1.03x faster                                                    |
| regex_dna                        | 99.6 ms                                                  | 96.5 ms: 1.03x faster                                                    |
| async_tree_eager_cpu_io_mixed    | 231 ms                                                   | 224 ms: 1.03x faster                                                     |
| sympy_integrate                  | 8.02 ms                                                  | 7.83 ms: 1.03x faster                                                    |
| thrift                           | 322 us                                                   | 317 us: 1.02x faster                                                     |
| html5lib                         | 23.0 ms                                                  | 22.8 ms: 1.01x faster                                                    |
| fannkuch                         | 176 ms                                                   | 174 ms: 1.01x faster                                                     |
| dask                             | 528 ms                                                   | 523 ms: 1.01x faster                                                     |
| asyncio_websockets               | 190 ms                                                   | 189 ms: 1.01x faster                                                     |
| sqlite_synth                     | 967 ns                                                   | 960 ns: 1.01x faster                                                     |
| mako                             | 4.77 ms                                                  | 4.80 ms: 1.01x slower                                                    |
| pprint_pformat                   | 665 ms                                                   | 670 ms: 1.01x slower                                                     |
| sympy_str                        | 104 ms                                                   | 105 ms: 1.01x slower                                                     |
| genshi_xml                       | 21.8 ms                                                  | 22.0 ms: 1.01x slower                                                    |
| pprint_safe_repr                 | 328 ms                                                   | 332 ms: 1.01x slower                                                     |
| unpickle_pure_python             | 103 us                                                   | 104 us: 1.01x slower                                                     |
| sympy_sum                        | 57.6 ms                                                  | 58.4 ms: 1.01x slower                                                    |
| regex_v8                         | 9.59 ms                                                  | 9.80 ms: 1.02x slower                                                    |
| crypto_pyaes                     | 38.8 ms                                                  | 39.8 ms: 1.02x slower                                                    |
| 2to3                             | 114 ms                                                   | 118 ms: 1.03x slower                                                     |
| docutils                         | 1.02 sec                                                 | 1.06 sec: 1.04x slower                                                   |
| shortest_path                    | 219 ms                                                   | 227 ms: 1.04x slower                                                     |
| pidigits                         | 161 ms                                                   | 168 ms: 1.04x slower                                                     |
| bench_thread_pool                | 419 us                                                   | 436 us: 1.04x slower                                                     |
| gc_traversal                     | 2.01 ms                                                  | 2.09 ms: 1.04x slower                                                    |
| connected_components             | 201 ms                                                   | 210 ms: 1.05x slower                                                     |
| sympy_expand                     | 167 ms                                                   | 176 ms: 1.06x slower                                                     |
| pickle_pure_python               | 139 us                                                   | 148 us: 1.06x slower                                                     |
| meteor_contest                   | 47.7 ms                                                  | 51.2 ms: 1.07x slower                                                    |
| telco                            | 2.61 ms                                                  | 2.85 ms: 1.09x slower                                                    |
| async_tree_eager_memoization_tg  | 113 ms                                                   | 125 ms: 1.11x slower                                                     |
| django_template                  | 13.6 ms                                                  | 15.4 ms: 1.13x slower                                                    |
| python_startup                   | 8.01 ms                                                  | 9.06 ms: 1.13x slower                                                    |
| create_gc_cycles                 | 830 us                                                   | 940 us: 1.13x slower                                                     |
| python_startup_no_site           | 5.71 ms                                                  | 6.53 ms: 1.15x slower                                                    |
| async_tree_eager_cpu_io_mixed_tg | 213 ms                                                   | 244 ms: 1.15x slower                                                     |
| bench_mp_pool                    | 39.7 ms                                                  | 46.4 ms: 1.17x slower                                                    |
| coverage                         | 26.9 ms                                                  | 32.1 ms: 1.19x slower                                                    |
| many_optionals                   | 195 us                                                   | 261 us: 1.34x slower                                                     |
| async_tree_eager_tg              | 32.1 ms                                                  | 84.1 ms: 2.62x slower                                                    |
| Geometric mean                   | (ref)                                                    | 1.12x faster                                                             |

Benchmark hidden because not significant (2): json, json_loads
Ignored benchmarks (10) of results/bm-20240906-3.12.6-a4a2d2b/bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b.json: chameleon, djangocms, gevent_hub, sqlalchemy_declarative, sqlalchemy_imperative, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http
Ignored benchmarks (4) of results/bm-20260224-3.15.0a6+-4e45c9c/bm-20260224-macm4pro-arm64-python-4e45c9c309abb12c6223-3.15.0a6+-4e45c9c.json: sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile

- Geometric mean (including insignificant results): 1.119x faster

# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.04x
- 95% likely to have a speedup of 1.04x
- 99% likely to have a speedup of 1.03x

# Memory
- memory change: 1.23x