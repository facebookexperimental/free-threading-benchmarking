# Results vs. 3.12.6

- fork: python
- ref: 149c4657507d17f78dd0
- machine: darwin-arm64
- commit hash: 149c465
- commit date: 2026-03-07
- overall geometric mean: 1.253x faster
- HPT reliability: 100.00%
- HPT 99th percentile: 1.12x faster
- Memory change: 1.28x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260307-macm4pro-arm64-python-149c4657507d17f78dd0-3.15.0a6+-149c465 |
|----------------|:--------------------------------------------------------:|:------------------------------------------------------------------------:|
| 2to3           | 114 ms                                                   | 116 ms: 1.02x slower                                                     |
| docutils       | 1.02 sec                                                 | 1.02 sec: 1.01x faster                                                   |
| html5lib       | 23.0 ms                                                  | 20.0 ms: 1.15x faster                                                    |
| sphinx         | 434 ms                                                   | 390 ms: 1.11x faster                                                     |
| Geometric mean | (ref)                                                    | 1.06x faster                                                             |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                        | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260307-macm4pro-arm64-python-149c4657507d17f78dd0-3.15.0a6+-149c465 |
|----------------------------------|:--------------------------------------------------------:|:------------------------------------------------------------------------:|
| async_tree_eager_io              | 496 ms                                                   | 222 ms: 2.24x faster                                                     |
| async_tree_io_tg                 | 480 ms                                                   | 226 ms: 2.13x faster                                                     |
| async_tree_eager_io_tg           | 446 ms                                                   | 219 ms: 2.04x faster                                                     |
| async_tree_io                    | 459 ms                                                   | 229 ms: 2.01x faster                                                     |
| async_tree_none                  | 178 ms                                                   | 96.4 ms: 1.85x faster                                                    |
| async_tree_none_tg               | 172 ms                                                   | 98.9 ms: 1.74x faster                                                    |
| async_tree_memoization_tg        | 231 ms                                                   | 136 ms: 1.70x faster                                                     |
| async_tree_memoization           | 223 ms                                                   | 133 ms: 1.67x faster                                                     |
| async_tree_cpu_io_mixed_tg       | 338 ms                                                   | 250 ms: 1.36x faster                                                     |
| async_tree_cpu_io_mixed          | 333 ms                                                   | 248 ms: 1.34x faster                                                     |
| async_tree_eager_memoization     | 132 ms                                                   | 103 ms: 1.28x faster                                                     |
| coroutines                       | 13.6 ms                                                  | 10.9 ms: 1.25x faster                                                    |
| async_tree_eager                 | 45.6 ms                                                  | 37.5 ms: 1.21x faster                                                    |
| async_generators                 | 206 ms                                                   | 190 ms: 1.09x faster                                                     |
| async_tree_eager_cpu_io_mixed    | 231 ms                                                   | 216 ms: 1.07x faster                                                     |
| asyncio_websockets               | 190 ms                                                   | 187 ms: 1.02x faster                                                     |
| async_tree_eager_memoization_tg  | 113 ms                                                   | 121 ms: 1.07x slower                                                     |
| async_tree_eager_cpu_io_mixed_tg | 213 ms                                                   | 240 ms: 1.13x slower                                                     |
| async_tree_eager_tg              | 32.1 ms                                                  | 82.2 ms: 2.56x slower                                                    |
| Geometric mean                   | (ref)                                                    | 1.33x faster                                                             |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260307-macm4pro-arm64-python-149c4657507d17f78dd0-3.15.0a6+-149c465 |
|----------------|:--------------------------------------------------------:|:------------------------------------------------------------------------:|
| float          | 37.9 ms                                                  | 23.5 ms: 1.61x faster                                                    |
| nbody          | 54.2 ms                                                  | 39.9 ms: 1.36x faster                                                    |
| Geometric mean | (ref)                                                    | 1.30x faster                                                             |

Benchmark hidden because not significant (1): pidigits

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260307-macm4pro-arm64-python-149c4657507d17f78dd0-3.15.0a6+-149c465 |
|----------------|:--------------------------------------------------------:|:------------------------------------------------------------------------:|
| regex_compile  | 54.6 ms                                                  | 41.1 ms: 1.33x faster                                                    |
| regex_effbot   | 1.67 ms                                                  | 1.46 ms: 1.14x faster                                                    |
| regex_dna      | 99.6 ms                                                  | 94.2 ms: 1.06x faster                                                    |
| Geometric mean | (ref)                                                    | 1.12x faster                                                             |

Benchmark hidden because not significant (1): regex_v8

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260307-macm4pro-arm64-python-149c4657507d17f78dd0-3.15.0a6+-149c465 |
|----------------------|:--------------------------------------------------------:|:------------------------------------------------------------------------:|
| tomli_loads          | 957 ms                                                   | 637 ms: 1.50x faster                                                     |
| xml_etree_iterparse  | 51.6 ms                                                  | 38.1 ms: 1.35x faster                                                    |
| xml_etree_generate   | 38.9 ms                                                  | 31.4 ms: 1.24x faster                                                    |
| unpickle_pure_python | 103 us                                                   | 85.2 us: 1.21x faster                                                    |
| json_dumps           | 4.26 ms                                                  | 3.53 ms: 1.21x faster                                                    |
| xml_etree_process    | 26.7 ms                                                  | 22.5 ms: 1.19x faster                                                    |
| xml_etree_parse      | 67.9 ms                                                  | 61.3 ms: 1.11x faster                                                    |
| pickle_pure_python   | 139 us                                                   | 127 us: 1.09x faster                                                     |
| json_loads           | 10.9 us                                                  | 10.6 us: 1.03x faster                                                    |
| Geometric mean       | (ref)                                                    | 1.21x faster                                                             |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260307-macm4pro-arm64-python-149c4657507d17f78dd0-3.15.0a6+-149c465 |
|------------------------|:--------------------------------------------------------:|:------------------------------------------------------------------------:|
| python_startup         | 8.01 ms                                                  | 8.87 ms: 1.11x slower                                                    |
| python_startup_no_site | 5.71 ms                                                  | 6.41 ms: 1.12x slower                                                    |
| Geometric mean         | (ref)                                                    | 1.11x slower                                                             |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260307-macm4pro-arm64-python-149c4657507d17f78dd0-3.15.0a6+-149c465 |
|-----------------|:--------------------------------------------------------:|:------------------------------------------------------------------------:|
| mako            | 4.77 ms                                                  | 3.97 ms: 1.20x faster                                                    |
| django_template | 13.6 ms                                                  | 13.8 ms: 1.02x slower                                                    |
| Geometric mean  | (ref)                                                    | 1.09x faster                                                             |

All benchmarks:
===============

| Benchmark                        | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260307-macm4pro-arm64-python-149c4657507d17f78dd0-3.15.0a6+-149c465 |
|----------------------------------|:--------------------------------------------------------:|:------------------------------------------------------------------------:|
| subparsers                       | 20.8 ms                                                  | 3.62 ms: 5.74x faster                                                    |
| richards                         | 22.4 ms                                                  | 9.25 ms: 2.42x faster                                                    |
| richards_super                   | 25.4 ms                                                  | 11.1 ms: 2.30x faster                                                    |
| async_tree_eager_io              | 496 ms                                                   | 222 ms: 2.24x faster                                                     |
| async_tree_io_tg                 | 480 ms                                                   | 226 ms: 2.13x faster                                                     |
| async_tree_eager_io_tg           | 446 ms                                                   | 219 ms: 2.04x faster                                                     |
| mdp                              | 1.09 sec                                                 | 543 ms: 2.01x faster                                                     |
| async_tree_io                    | 459 ms                                                   | 229 ms: 2.01x faster                                                     |
| async_tree_none                  | 178 ms                                                   | 96.4 ms: 1.85x faster                                                    |
| scimark_lu                       | 51.3 ms                                                  | 28.0 ms: 1.83x faster                                                    |
| async_tree_none_tg               | 172 ms                                                   | 98.9 ms: 1.74x faster                                                    |
| deepcopy_memo                    | 18.3 us                                                  | 10.6 us: 1.72x faster                                                    |
| deepcopy                         | 161 us                                                   | 94.4 us: 1.71x faster                                                    |
| async_tree_memoization_tg        | 231 ms                                                   | 136 ms: 1.70x faster                                                     |
| comprehensions                   | 9.84 us                                                  | 5.86 us: 1.68x faster                                                    |
| async_tree_memoization           | 223 ms                                                   | 133 ms: 1.67x faster                                                     |
| float                            | 37.9 ms                                                  | 23.5 ms: 1.61x faster                                                    |
| scimark_sor                      | 61.0 ms                                                  | 38.9 ms: 1.57x faster                                                    |
| go                               | 70.0 ms                                                  | 46.6 ms: 1.50x faster                                                    |
| tomli_loads                      | 957 ms                                                   | 637 ms: 1.50x faster                                                     |
| pyflate                          | 216 ms                                                   | 146 ms: 1.48x faster                                                     |
| logging_silent                   | 50.9 ns                                                  | 34.6 ns: 1.47x faster                                                    |
| deltablue                        | 1.73 ms                                                  | 1.18 ms: 1.46x faster                                                    |
| scimark_fft                      | 142 ms                                                   | 98.3 ms: 1.44x faster                                                    |
| deepcopy_reduce                  | 1.46 us                                                  | 1.02 us: 1.43x faster                                                    |
| spectral_norm                    | 54.4 ms                                                  | 38.5 ms: 1.41x faster                                                    |
| scimark_monte_carlo              | 32.2 ms                                                  | 23.2 ms: 1.39x faster                                                    |
| nqueens                          | 43.5 ms                                                  | 31.7 ms: 1.37x faster                                                    |
| nbody                            | 54.2 ms                                                  | 39.9 ms: 1.36x faster                                                    |
| async_tree_cpu_io_mixed_tg       | 338 ms                                                   | 250 ms: 1.36x faster                                                     |
| xml_etree_iterparse              | 51.6 ms                                                  | 38.1 ms: 1.35x faster                                                    |
| async_tree_cpu_io_mixed          | 333 ms                                                   | 248 ms: 1.34x faster                                                     |
| regex_compile                    | 54.6 ms                                                  | 41.1 ms: 1.33x faster                                                    |
| k_core                           | 1.12 sec                                                 | 862 ms: 1.30x faster                                                     |
| chaos                            | 28.9 ms                                                  | 22.4 ms: 1.29x faster                                                    |
| async_tree_eager_memoization     | 132 ms                                                   | 103 ms: 1.28x faster                                                     |
| raytrace                         | 145 ms                                                   | 115 ms: 1.26x faster                                                     |
| pprint_pformat                   | 665 ms                                                   | 530 ms: 1.25x faster                                                     |
| bpe_tokeniser                    | 2.24 sec                                                 | 1.79 sec: 1.25x faster                                                   |
| pprint_safe_repr                 | 328 ms                                                   | 263 ms: 1.25x faster                                                     |
| hexiom                           | 3.04 ms                                                  | 2.44 ms: 1.25x faster                                                    |
| coroutines                       | 13.6 ms                                                  | 10.9 ms: 1.25x faster                                                    |
| xml_etree_generate               | 38.9 ms                                                  | 31.4 ms: 1.24x faster                                                    |
| async_tree_eager                 | 45.6 ms                                                  | 37.5 ms: 1.21x faster                                                    |
| unpickle_pure_python             | 103 us                                                   | 85.2 us: 1.21x faster                                                    |
| crypto_pyaes                     | 38.8 ms                                                  | 32.1 ms: 1.21x faster                                                    |
| json_dumps                       | 4.26 ms                                                  | 3.53 ms: 1.21x faster                                                    |
| mako                             | 4.77 ms                                                  | 3.97 ms: 1.20x faster                                                    |
| logging_simple                   | 2.57 us                                                  | 2.14 us: 1.20x faster                                                    |
| logging_format                   | 2.80 us                                                  | 2.34 us: 1.20x faster                                                    |
| xml_etree_process                | 26.7 ms                                                  | 22.5 ms: 1.19x faster                                                    |
| fannkuch                         | 176 ms                                                   | 150 ms: 1.17x faster                                                     |
| pathlib                          | 12.4 ms                                                  | 10.7 ms: 1.16x faster                                                    |
| html5lib                         | 23.0 ms                                                  | 20.0 ms: 1.15x faster                                                    |
| regex_effbot                     | 1.67 ms                                                  | 1.46 ms: 1.14x faster                                                    |
| dulwich_log                      | 21.3 ms                                                  | 18.7 ms: 1.14x faster                                                    |
| scimark_sparse_mat_mult          | 2.08 ms                                                  | 1.85 ms: 1.12x faster                                                    |
| sphinx                           | 434 ms                                                   | 390 ms: 1.11x faster                                                     |
| pycparser                        | 497 ms                                                   | 448 ms: 1.11x faster                                                     |
| xml_etree_parse                  | 67.9 ms                                                  | 61.3 ms: 1.11x faster                                                    |
| pylint                           | 128 ms                                                   | 117 ms: 1.10x faster                                                     |
| pickle_pure_python               | 139 us                                                   | 127 us: 1.09x faster                                                     |
| async_generators                 | 206 ms                                                   | 190 ms: 1.09x faster                                                     |
| typing_runtime_protocols         | 71.0 us                                                  | 65.2 us: 1.09x faster                                                    |
| sympy_integrate                  | 8.02 ms                                                  | 7.46 ms: 1.08x faster                                                    |
| async_tree_eager_cpu_io_mixed    | 231 ms                                                   | 216 ms: 1.07x faster                                                     |
| meteor_contest                   | 47.7 ms                                                  | 44.9 ms: 1.06x faster                                                    |
| regex_dna                        | 99.6 ms                                                  | 94.2 ms: 1.06x faster                                                    |
| generators                       | 21.9 ms                                                  | 20.8 ms: 1.06x faster                                                    |
| sympy_sum                        | 57.6 ms                                                  | 54.5 ms: 1.06x faster                                                    |
| json                             | 1.93 ms                                                  | 1.83 ms: 1.05x faster                                                    |
| sympy_str                        | 104 ms                                                   | 99.1 ms: 1.05x faster                                                    |
| thrift                           | 322 us                                                   | 309 us: 1.04x faster                                                     |
| json_loads                       | 10.9 us                                                  | 10.6 us: 1.03x faster                                                    |
| connected_components             | 201 ms                                                   | 196 ms: 1.03x faster                                                     |
| asyncio_websockets               | 190 ms                                                   | 187 ms: 1.02x faster                                                     |
| sqlite_synth                     | 967 ns                                                   | 951 ns: 1.02x faster                                                     |
| shortest_path                    | 219 ms                                                   | 216 ms: 1.02x faster                                                     |
| dask                             | 528 ms                                                   | 523 ms: 1.01x faster                                                     |
| docutils                         | 1.02 sec                                                 | 1.02 sec: 1.01x faster                                                   |
| telco                            | 2.61 ms                                                  | 2.62 ms: 1.00x slower                                                    |
| sympy_expand                     | 167 ms                                                   | 168 ms: 1.01x slower                                                     |
| gc_traversal                     | 2.01 ms                                                  | 2.03 ms: 1.01x slower                                                    |
| django_template                  | 13.6 ms                                                  | 13.8 ms: 1.02x slower                                                    |
| 2to3                             | 114 ms                                                   | 116 ms: 1.02x slower                                                     |
| bench_thread_pool                | 419 us                                                   | 430 us: 1.03x slower                                                     |
| async_tree_eager_memoization_tg  | 113 ms                                                   | 121 ms: 1.07x slower                                                     |
| create_gc_cycles                 | 830 us                                                   | 902 us: 1.09x slower                                                     |
| python_startup                   | 8.01 ms                                                  | 8.87 ms: 1.11x slower                                                    |
| python_startup_no_site           | 5.71 ms                                                  | 6.41 ms: 1.12x slower                                                    |
| async_tree_eager_cpu_io_mixed_tg | 213 ms                                                   | 240 ms: 1.13x slower                                                     |
| bench_mp_pool                    | 39.7 ms                                                  | 45.7 ms: 1.15x slower                                                    |
| many_optionals                   | 195 us                                                   | 239 us: 1.22x slower                                                     |
| coverage                         | 26.9 ms                                                  | 33.6 ms: 1.25x slower                                                    |
| async_tree_eager_tg              | 32.1 ms                                                  | 82.2 ms: 2.56x slower                                                    |
| Geometric mean                   | (ref)                                                    | 1.25x faster                                                             |

Benchmark hidden because not significant (2): pidigits, regex_v8
Ignored benchmarks (12) of results/bm-20240906-3.12.6-a4a2d2b/bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b.json: chameleon, djangocms, genshi_text, genshi_xml, gevent_hub, sqlalchemy_declarative, sqlalchemy_imperative, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http
Ignored benchmarks (10) of results/bm-20260307-3.15.0a6+-149c465-JIT/bm-20260307-macm4pro-arm64-python-149c4657507d17f78dd0-3.15.0a6+-149c465.json: pickle, pickle_dict, pickle_list, sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile, unpack_sequence, unpickle, unpickle_list

- Geometric mean (including insignificant results): 1.253x faster

# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.17x
- 95% likely to have a speedup of 1.15x
- 99% likely to have a speedup of 1.12x

# Memory
- memory change: 1.28x