# Results vs. 3.13.0rc2

- fork: python
- ref: c5fcdb4a9bd04b88f363
- machine: linux-x86_64
- commit hash: c5fcdb4
- commit date: 2026-04-25
- overall geometric mean: 1.161x faster
- HPT reliability: 100.00%
- HPT 99th percentile: 1.08x faster
- Memory change: 1.20x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260425-vultr-x86_64-python-c5fcdb4a9bd04b88f363-3.15.0a8+-c5fcdb4 |
|----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| 2to3           | 260 ms                                                       | 255 ms: 1.02x faster                                                   |
| docutils       | 2.62 sec                                                     | 2.56 sec: 1.02x faster                                                 |
| html5lib       | 67.0 ms                                                      | 61.0 ms: 1.10x faster                                                  |
| Geometric mean | (ref)                                                        | 1.05x faster                                                           |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260425-vultr-x86_64-python-c5fcdb4a9bd04b88f363-3.15.0a8+-c5fcdb4 |
|----------------------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| async_tree_io_tg           | 913 ms                                                       | 551 ms: 1.66x faster                                                   |
| async_tree_io              | 876 ms                                                       | 538 ms: 1.63x faster                                                   |
| async_tree_memoization     | 461 ms                                                       | 286 ms: 1.61x faster                                                   |
| async_tree_none            | 354 ms                                                       | 222 ms: 1.59x faster                                                   |
| async_tree_memoization_tg  | 414 ms                                                       | 280 ms: 1.48x faster                                                   |
| async_tree_none_tg         | 336 ms                                                       | 234 ms: 1.44x faster                                                   |
| async_tree_cpu_io_mixed    | 666 ms                                                       | 472 ms: 1.41x faster                                                   |
| async_tree_cpu_io_mixed_tg | 638 ms                                                       | 469 ms: 1.36x faster                                                   |
| asyncio_tcp                | 505 ms                                                       | 445 ms: 1.14x faster                                                   |
| async_generators           | 377 ms                                                       | 354 ms: 1.07x faster                                                   |
| asyncio_tcp_ssl            | 1.51 sec                                                     | 1.46 sec: 1.03x faster                                                 |
| coroutines                 | 23.6 ms                                                      | 24.3 ms: 1.03x slower                                                  |
| asyncio_websockets         | 520 ms                                                       | 545 ms: 1.05x slower                                                   |
| Geometric mean             | (ref)                                                        | 1.31x faster                                                           |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260425-vultr-x86_64-python-c5fcdb4a9bd04b88f363-3.15.0a8+-c5fcdb4 |
|----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| float          | 77.5 ms                                                      | 52.7 ms: 1.47x faster                                                  |
| nbody          | 85.1 ms                                                      | 67.8 ms: 1.26x faster                                                  |
| pidigits       | 217 ms                                                       | 184 ms: 1.18x faster                                                   |
| Geometric mean | (ref)                                                        | 1.29x faster                                                           |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260425-vultr-x86_64-python-c5fcdb4a9bd04b88f363-3.15.0a8+-c5fcdb4 |
|----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| regex_effbot   | 3.08 ms                                                      | 2.61 ms: 1.18x faster                                                  |
| regex_dna      | 180 ms                                                       | 165 ms: 1.09x faster                                                   |
| regex_compile  | 132 ms                                                       | 121 ms: 1.09x faster                                                   |
| regex_v8       | 22.7 ms                                                      | 21.3 ms: 1.07x faster                                                  |
| Geometric mean | (ref)                                                        | 1.11x faster                                                           |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260425-vultr-x86_64-python-c5fcdb4a9bd04b88f363-3.15.0a8+-c5fcdb4 |
|----------------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| tomli_loads          | 2.01 sec                                                     | 1.50 sec: 1.34x faster                                                 |
| unpickle_pure_python | 210 us                                                       | 170 us: 1.24x faster                                                   |
| json_dumps           | 10.5 ms                                                      | 8.76 ms: 1.20x faster                                                  |
| xml_etree_iterparse  | 94.9 ms                                                      | 81.7 ms: 1.16x faster                                                  |
| xml_etree_process    | 59.3 ms                                                      | 53.0 ms: 1.12x faster                                                  |
| xml_etree_generate   | 85.4 ms                                                      | 79.0 ms: 1.08x faster                                                  |
| pickle_pure_python   | 294 us                                                       | 273 us: 1.08x faster                                                   |
| xml_etree_parse      | 136 ms                                                       | 129 ms: 1.05x faster                                                   |
| pickle_dict          | 32.5 us                                                      | 31.1 us: 1.05x faster                                                  |
| pickle_list          | 4.93 us                                                      | 4.96 us: 1.01x slower                                                  |
| unpickle             | 14.3 us                                                      | 14.7 us: 1.02x slower                                                  |
| unpickle_list        | 4.71 us                                                      | 5.25 us: 1.11x slower                                                  |
| pickle               | 11.3 us                                                      | 12.7 us: 1.12x slower                                                  |
| Geometric mean       | (ref)                                                        | 1.07x faster                                                           |

Benchmark hidden because not significant (1): json_loads

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260425-vultr-x86_64-python-c5fcdb4a9bd04b88f363-3.15.0a8+-c5fcdb4 |
|------------------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| python_startup_no_site | 7.39 ms                                                      | 6.85 ms: 1.08x faster                                                  |
| python_startup         | 11.0 ms                                                      | 12.4 ms: 1.13x slower                                                  |
| Geometric mean         | (ref)                                                        | 1.02x slower                                                           |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260425-vultr-x86_64-python-c5fcdb4a9bd04b88f363-3.15.0a8+-c5fcdb4 |
|-----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| mako            | 11.3 ms                                                      | 10.1 ms: 1.12x faster                                                  |
| genshi_text     | 21.5 ms                                                      | 19.3 ms: 1.12x faster                                                  |
| django_template | 34.1 ms                                                      | 32.4 ms: 1.05x faster                                                  |
| genshi_xml      | 48.8 ms                                                      | 49.6 ms: 1.02x slower                                                  |
| Geometric mean  | (ref)                                                        | 1.07x faster                                                           |

All benchmarks:
===============

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260425-vultr-x86_64-python-c5fcdb4a9bd04b88f363-3.15.0a8+-c5fcdb4 |
|----------------------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| pylint                     | 317 ms                                                       | 115 ms: 2.76x faster                                                   |
| richards_super             | 51.6 ms                                                      | 21.2 ms: 2.43x faster                                                  |
| richards                   | 45.2 ms                                                      | 18.9 ms: 2.39x faster                                                  |
| mdp                        | 2.36 sec                                                     | 1.17 sec: 2.02x faster                                                 |
| scimark_sor                | 134 ms                                                       | 76.2 ms: 1.76x faster                                                  |
| deepcopy_memo              | 39.1 us                                                      | 22.3 us: 1.75x faster                                                  |
| async_tree_io_tg           | 913 ms                                                       | 551 ms: 1.66x faster                                                   |
| spectral_norm              | 111 ms                                                       | 67.6 ms: 1.64x faster                                                  |
| async_tree_io              | 876 ms                                                       | 538 ms: 1.63x faster                                                   |
| async_tree_memoization     | 461 ms                                                       | 286 ms: 1.61x faster                                                   |
| async_tree_none            | 354 ms                                                       | 222 ms: 1.59x faster                                                   |
| deepcopy                   | 355 us                                                       | 225 us: 1.58x faster                                                   |
| async_tree_memoization_tg  | 414 ms                                                       | 280 ms: 1.48x faster                                                   |
| float                      | 77.5 ms                                                      | 52.7 ms: 1.47x faster                                                  |
| scimark_lu                 | 113 ms                                                       | 76.7 ms: 1.47x faster                                                  |
| async_tree_none_tg         | 336 ms                                                       | 234 ms: 1.44x faster                                                   |
| scimark_fft                | 349 ms                                                       | 246 ms: 1.42x faster                                                   |
| async_tree_cpu_io_mixed    | 666 ms                                                       | 472 ms: 1.41x faster                                                   |
| go                         | 141 ms                                                       | 99.9 ms: 1.41x faster                                                  |
| async_tree_cpu_io_mixed_tg | 638 ms                                                       | 469 ms: 1.36x faster                                                   |
| logging_silent             | 103 ns                                                       | 75.8 ns: 1.35x faster                                                  |
| tomli_loads                | 2.01 sec                                                     | 1.50 sec: 1.34x faster                                                 |
| scimark_monte_carlo        | 65.4 ms                                                      | 50.4 ms: 1.30x faster                                                  |
| pyflate                    | 449 ms                                                       | 353 ms: 1.27x faster                                                   |
| deepcopy_reduce            | 3.11 us                                                      | 2.45 us: 1.27x faster                                                  |
| nbody                      | 85.1 ms                                                      | 67.8 ms: 1.26x faster                                                  |
| scimark_sparse_mat_mult    | 4.71 ms                                                      | 3.76 ms: 1.25x faster                                                  |
| unpickle_pure_python       | 210 us                                                       | 170 us: 1.24x faster                                                   |
| pprint_safe_repr           | 738 ms                                                       | 610 ms: 1.21x faster                                                   |
| pprint_pformat             | 1.50 sec                                                     | 1.24 sec: 1.20x faster                                                 |
| json_dumps                 | 10.5 ms                                                      | 8.76 ms: 1.20x faster                                                  |
| nqueens                    | 78.6 ms                                                      | 66.6 ms: 1.18x faster                                                  |
| chaos                      | 57.3 ms                                                      | 48.6 ms: 1.18x faster                                                  |
| regex_effbot               | 3.08 ms                                                      | 2.61 ms: 1.18x faster                                                  |
| pidigits                   | 217 ms                                                       | 184 ms: 1.18x faster                                                   |
| logging_simple             | 6.16 us                                                      | 5.26 us: 1.17x faster                                                  |
| deltablue                  | 3.12 ms                                                      | 2.67 ms: 1.17x faster                                                  |
| comprehensions             | 16.5 us                                                      | 14.1 us: 1.17x faster                                                  |
| logging_format             | 6.84 us                                                      | 5.86 us: 1.17x faster                                                  |
| xml_etree_iterparse        | 94.9 ms                                                      | 81.7 ms: 1.16x faster                                                  |
| bpe_tokeniser              | 4.45 sec                                                     | 3.87 sec: 1.15x faster                                                 |
| asyncio_tcp                | 505 ms                                                       | 445 ms: 1.14x faster                                                   |
| pathlib                    | 19.2 ms                                                      | 17.0 ms: 1.13x faster                                                  |
| xml_etree_process          | 59.3 ms                                                      | 53.0 ms: 1.12x faster                                                  |
| mako                       | 11.3 ms                                                      | 10.1 ms: 1.12x faster                                                  |
| genshi_text                | 21.5 ms                                                      | 19.3 ms: 1.12x faster                                                  |
| fannkuch                   | 370 ms                                                       | 332 ms: 1.11x faster                                                   |
| html5lib                   | 67.0 ms                                                      | 61.0 ms: 1.10x faster                                                  |
| regex_dna                  | 180 ms                                                       | 165 ms: 1.09x faster                                                   |
| regex_compile              | 132 ms                                                       | 121 ms: 1.09x faster                                                   |
| xml_etree_generate         | 85.4 ms                                                      | 79.0 ms: 1.08x faster                                                  |
| python_startup_no_site     | 7.39 ms                                                      | 6.85 ms: 1.08x faster                                                  |
| pickle_pure_python         | 294 us                                                       | 273 us: 1.08x faster                                                   |
| dulwich_log                | 74.8 ms                                                      | 69.8 ms: 1.07x faster                                                  |
| regex_v8                   | 22.7 ms                                                      | 21.3 ms: 1.07x faster                                                  |
| async_generators           | 377 ms                                                       | 354 ms: 1.07x faster                                                   |
| xml_etree_parse            | 136 ms                                                       | 129 ms: 1.05x faster                                                   |
| django_template            | 34.1 ms                                                      | 32.4 ms: 1.05x faster                                                  |
| hexiom                     | 5.99 ms                                                      | 5.69 ms: 1.05x faster                                                  |
| meteor_contest             | 102 ms                                                       | 96.6 ms: 1.05x faster                                                  |
| pickle_dict                | 32.5 us                                                      | 31.1 us: 1.05x faster                                                  |
| typing_runtime_protocols   | 155 us                                                       | 148 us: 1.04x faster                                                   |
| sqlite_synth               | 2.21 us                                                      | 2.12 us: 1.04x faster                                                  |
| asyncio_tcp_ssl            | 1.51 sec                                                     | 1.46 sec: 1.03x faster                                                 |
| crypto_pyaes               | 67.9 ms                                                      | 65.8 ms: 1.03x faster                                                  |
| coverage                   | 83.0 ms                                                      | 81.1 ms: 1.02x faster                                                  |
| docutils                   | 2.62 sec                                                     | 2.56 sec: 1.02x faster                                                 |
| raytrace                   | 253 ms                                                       | 248 ms: 1.02x faster                                                   |
| 2to3                       | 260 ms                                                       | 255 ms: 1.02x faster                                                   |
| json                       | 4.93 ms                                                      | 4.85 ms: 1.02x faster                                                  |
| thrift                     | 778 us                                                       | 773 us: 1.01x faster                                                   |
| generators                 | 28.8 ms                                                      | 28.9 ms: 1.00x slower                                                  |
| pickle_list                | 4.93 us                                                      | 4.96 us: 1.01x slower                                                  |
| genshi_xml                 | 48.8 ms                                                      | 49.6 ms: 1.02x slower                                                  |
| unpickle                   | 14.3 us                                                      | 14.7 us: 1.02x slower                                                  |
| coroutines                 | 23.6 ms                                                      | 24.3 ms: 1.03x slower                                                  |
| asyncio_websockets         | 520 ms                                                       | 545 ms: 1.05x slower                                                   |
| sympy_str                  | 275 ms                                                       | 289 ms: 1.05x slower                                                   |
| sympy_sum                  | 156 ms                                                       | 167 ms: 1.07x slower                                                   |
| unpickle_list              | 4.71 us                                                      | 5.25 us: 1.11x slower                                                  |
| pickle                     | 11.3 us                                                      | 12.7 us: 1.12x slower                                                  |
| python_startup             | 11.0 ms                                                      | 12.4 ms: 1.13x slower                                                  |
| gc_traversal               | 3.14 ms                                                      | 4.33 ms: 1.38x slower                                                  |
| create_gc_cycles           | 1.34 ms                                                      | 1.87 ms: 1.40x slower                                                  |
| bench_thread_pool          | 919 us                                                       | 1.29 ms: 1.40x slower                                                  |
| unpack_sequence            | 44.8 ns                                                      | 65.5 ns: 1.46x slower                                                  |
| bench_mp_pool              | 11.0 ms                                                      | 184 ms: 16.77x slower                                                  |
| telco                      | 7.82 ms                                                      | 145 ms: 18.58x slower                                                  |
| Geometric mean             | (ref)                                                        | 1.10x faster                                                           |

Benchmark hidden because not significant (4): sympy_integrate, pycparser, sympy_expand, json_loads
Ignored benchmarks (10) of results/bm-20240906-3.13.0rc2-ec61006/bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006.json: aiohttp, chameleon, dask, flaskblogging, gunicorn, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http
Ignored benchmarks (10) of results/bm-20260425-3.15.0a8+-c5fcdb4-JIT/bm-20260425-vultr-x86_64-python-c5fcdb4a9bd04b88f363-3.15.0a8+-c5fcdb4.json: connected_components, k_core, many_optionals, shortest_path, sphinx, sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile, subparsers

- Geometric mean (including insignificant results): 1.161x faster

# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.12x
- 95% likely to have a speedup of 1.10x
- 99% likely to have a speedup of 1.08x

# Memory
- memory change: 1.20x