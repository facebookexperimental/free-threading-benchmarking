# Results vs. 3.12.6

- fork: python
- ref: e167e06f8c6b24f7b54e
- machine: linux-x86_64
- commit hash: e167e06
- commit date: 2026-03-15
- overall geometric mean: 1.137x faster
- HPT reliability: 100.00%
- HPT 99th percentile: 1.07x faster
- Memory change: 1.20x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260315-vultr-x86_64-python-e167e06f8c6b24f7b54e-3.15.0a7+-e167e06 |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| 2to3           | 264 ms                                                 | 249 ms: 1.06x faster                                                   |
| docutils       | 2.64 sec                                               | 2.61 sec: 1.01x faster                                                 |
| html5lib       | 63.6 ms                                                | 62.2 ms: 1.02x faster                                                  |
| Geometric mean | (ref)                                                  | 1.03x faster                                                           |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260315-vultr-x86_64-python-e167e06f8c6b24f7b54e-3.15.0a7+-e167e06 |
|----------------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| async_tree_io_tg           | 1.11 sec                                               | 561 ms: 1.98x faster                                                   |
| async_tree_none            | 464 ms                                                 | 239 ms: 1.94x faster                                                   |
| async_tree_memoization_tg  | 560 ms                                                 | 298 ms: 1.88x faster                                                   |
| async_tree_none_tg         | 446 ms                                                 | 238 ms: 1.87x faster                                                   |
| async_tree_io              | 1.08 sec                                               | 580 ms: 1.87x faster                                                   |
| async_tree_memoization     | 555 ms                                                 | 300 ms: 1.85x faster                                                   |
| async_tree_cpu_io_mixed    | 715 ms                                                 | 497 ms: 1.44x faster                                                   |
| async_tree_cpu_io_mixed_tg | 723 ms                                                 | 505 ms: 1.43x faster                                                   |
| coroutines                 | 23.9 ms                                                | 23.7 ms: 1.01x faster                                                  |
| async_generators           | 384 ms                                                 | 391 ms: 1.02x slower                                                   |
| asyncio_websockets         | 517 ms                                                 | 545 ms: 1.05x slower                                                   |
| Geometric mean             | (ref)                                                  | 1.51x faster                                                           |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260315-vultr-x86_64-python-e167e06f8c6b24f7b54e-3.15.0a7+-e167e06 |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| float          | 80.8 ms                                                | 59.4 ms: 1.36x faster                                                  |
| nbody          | 89.3 ms                                                | 76.7 ms: 1.16x faster                                                  |
| pidigits       | 184 ms                                                 | 204 ms: 1.11x slower                                                   |
| Geometric mean | (ref)                                                  | 1.13x faster                                                           |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260315-vultr-x86_64-python-e167e06f8c6b24f7b54e-3.15.0a7+-e167e06 |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| regex_effbot   | 3.17 ms                                                | 2.76 ms: 1.15x faster                                                  |
| regex_compile  | 142 ms                                                 | 125 ms: 1.14x faster                                                   |
| regex_dna      | 168 ms                                                 | 183 ms: 1.09x slower                                                   |
| regex_v8       | 20.6 ms                                                | 23.5 ms: 1.14x slower                                                  |
| Geometric mean | (ref)                                                  | 1.01x faster                                                           |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260315-vultr-x86_64-python-e167e06f8c6b24f7b54e-3.15.0a7+-e167e06 |
|----------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| tomli_loads          | 2.11 sec                                               | 1.53 sec: 1.38x faster                                                 |
| json_dumps           | 10.4 ms                                                | 8.95 ms: 1.16x faster                                                  |
| xml_etree_iterparse  | 96.7 ms                                                | 83.6 ms: 1.16x faster                                                  |
| unpickle_pure_python | 221 us                                                 | 197 us: 1.12x faster                                                   |
| xml_etree_generate   | 85.2 ms                                                | 77.2 ms: 1.10x faster                                                  |
| xml_etree_process    | 59.0 ms                                                | 54.2 ms: 1.09x faster                                                  |
| xml_etree_parse      | 139 ms                                                 | 130 ms: 1.06x faster                                                   |
| pickle_pure_python   | 308 us                                                 | 310 us: 1.01x slower                                                   |
| json_loads           | 26.5 us                                                | 27.6 us: 1.04x slower                                                  |
| Geometric mean       | (ref)                                                  | 1.11x faster                                                           |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260315-vultr-x86_64-python-e167e06f8c6b24f7b54e-3.15.0a7+-e167e06 |
|------------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| python_startup_no_site | 7.16 ms                                                | 7.48 ms: 1.05x slower                                                  |
| python_startup         | 9.93 ms                                                | 12.7 ms: 1.28x slower                                                  |
| Geometric mean         | (ref)                                                  | 1.16x slower                                                           |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260315-vultr-x86_64-python-e167e06f8c6b24f7b54e-3.15.0a7+-e167e06 |
|-----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| genshi_text     | 22.8 ms                                                | 20.2 ms: 1.13x faster                                                  |
| mako            | 11.0 ms                                                | 10.6 ms: 1.04x faster                                                  |
| django_template | 34.7 ms                                                | 35.0 ms: 1.01x slower                                                  |
| genshi_xml      | 50.2 ms                                                | 52.9 ms: 1.05x slower                                                  |
| Geometric mean  | (ref)                                                  | 1.03x faster                                                           |

All benchmarks:
===============

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260315-vultr-x86_64-python-e167e06f8c6b24f7b54e-3.15.0a7+-e167e06 |
|----------------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| richards                   | 45.9 ms                                                | 17.7 ms: 2.60x faster                                                  |
| richards_super             | 51.9 ms                                                | 21.5 ms: 2.41x faster                                                  |
| mdp                        | 2.42 sec                                               | 1.20 sec: 2.01x faster                                                 |
| async_tree_io_tg           | 1.11 sec                                               | 561 ms: 1.98x faster                                                   |
| async_tree_none            | 464 ms                                                 | 239 ms: 1.94x faster                                                   |
| async_tree_memoization_tg  | 560 ms                                                 | 298 ms: 1.88x faster                                                   |
| async_tree_none_tg         | 446 ms                                                 | 238 ms: 1.87x faster                                                   |
| async_tree_io              | 1.08 sec                                               | 580 ms: 1.87x faster                                                   |
| async_tree_memoization     | 555 ms                                                 | 300 ms: 1.85x faster                                                   |
| deepcopy_memo              | 40.3 us                                                | 22.6 us: 1.78x faster                                                  |
| deepcopy                   | 352 us                                                 | 225 us: 1.56x faster                                                   |
| scimark_sor                | 130 ms                                                 | 85.1 ms: 1.52x faster                                                  |
| async_tree_cpu_io_mixed    | 715 ms                                                 | 497 ms: 1.44x faster                                                   |
| scimark_lu                 | 114 ms                                                 | 79.6 ms: 1.43x faster                                                  |
| spectral_norm              | 110 ms                                                 | 76.8 ms: 1.43x faster                                                  |
| async_tree_cpu_io_mixed_tg | 723 ms                                                 | 505 ms: 1.43x faster                                                   |
| go                         | 139 ms                                                 | 99.7 ms: 1.40x faster                                                  |
| tomli_loads                | 2.11 sec                                               | 1.53 sec: 1.38x faster                                                 |
| float                      | 80.8 ms                                                | 59.4 ms: 1.36x faster                                                  |
| logging_silent             | 109 ns                                                 | 80.1 ns: 1.36x faster                                                  |
| comprehensions             | 19.8 us                                                | 14.8 us: 1.34x faster                                                  |
| scimark_monte_carlo        | 68.4 ms                                                | 52.0 ms: 1.32x faster                                                  |
| scimark_fft                | 342 ms                                                 | 261 ms: 1.31x faster                                                   |
| deepcopy_reduce            | 3.08 us                                                | 2.38 us: 1.29x faster                                                  |
| deltablue                  | 3.45 ms                                                | 2.72 ms: 1.27x faster                                                  |
| pathlib                    | 21.5 ms                                                | 17.3 ms: 1.24x faster                                                  |
| pyflate                    | 448 ms                                                 | 361 ms: 1.24x faster                                                   |
| bpe_tokeniser              | 4.74 sec                                               | 3.92 sec: 1.21x faster                                                 |
| chaos                      | 62.8 ms                                                | 52.6 ms: 1.19x faster                                                  |
| pprint_safe_repr           | 743 ms                                                 | 629 ms: 1.18x faster                                                   |
| crypto_pyaes               | 76.6 ms                                                | 65.6 ms: 1.17x faster                                                  |
| nbody                      | 89.3 ms                                                | 76.7 ms: 1.16x faster                                                  |
| json_dumps                 | 10.4 ms                                                | 8.95 ms: 1.16x faster                                                  |
| xml_etree_iterparse        | 96.7 ms                                                | 83.6 ms: 1.16x faster                                                  |
| logging_simple             | 6.63 us                                                | 5.75 us: 1.15x faster                                                  |
| pprint_pformat             | 1.52 sec                                               | 1.32 sec: 1.15x faster                                                 |
| regex_effbot               | 3.17 ms                                                | 2.76 ms: 1.15x faster                                                  |
| regex_compile              | 142 ms                                                 | 125 ms: 1.14x faster                                                   |
| dulwich_log                | 78.9 ms                                                | 69.3 ms: 1.14x faster                                                  |
| logging_format             | 7.35 us                                                | 6.46 us: 1.14x faster                                                  |
| genshi_text                | 22.8 ms                                                | 20.2 ms: 1.13x faster                                                  |
| unpickle_pure_python       | 221 us                                                 | 197 us: 1.12x faster                                                   |
| generators                 | 32.2 ms                                                | 29.0 ms: 1.11x faster                                                  |
| xml_etree_generate         | 85.2 ms                                                | 77.2 ms: 1.10x faster                                                  |
| fannkuch                   | 372 ms                                                 | 338 ms: 1.10x faster                                                   |
| pylint                     | 319 ms                                                 | 291 ms: 1.09x faster                                                   |
| xml_etree_process          | 59.0 ms                                                | 54.2 ms: 1.09x faster                                                  |
| nqueens                    | 80.1 ms                                                | 73.7 ms: 1.09x faster                                                  |
| raytrace                   | 299 ms                                                 | 277 ms: 1.08x faster                                                   |
| pycparser                  | 1.17 sec                                               | 1.08 sec: 1.08x faster                                                 |
| xml_etree_parse            | 139 ms                                                 | 130 ms: 1.06x faster                                                   |
| 2to3                       | 264 ms                                                 | 249 ms: 1.06x faster                                                   |
| hexiom                     | 6.17 ms                                                | 5.86 ms: 1.05x faster                                                  |
| sympy_integrate            | 20.5 ms                                                | 19.7 ms: 1.04x faster                                                  |
| mako                       | 11.0 ms                                                | 10.6 ms: 1.04x faster                                                  |
| meteor_contest             | 104 ms                                                 | 99.5 ms: 1.04x faster                                                  |
| scimark_sparse_mat_mult    | 4.39 ms                                                | 4.26 ms: 1.03x faster                                                  |
| sqlite_synth               | 2.20 us                                                | 2.14 us: 1.03x faster                                                  |
| json                       | 5.02 ms                                                | 4.90 ms: 1.03x faster                                                  |
| html5lib                   | 63.6 ms                                                | 62.2 ms: 1.02x faster                                                  |
| typing_runtime_protocols   | 163 us                                                 | 161 us: 1.02x faster                                                   |
| sympy_sum                  | 166 ms                                                 | 164 ms: 1.01x faster                                                   |
| docutils                   | 2.64 sec                                               | 2.61 sec: 1.01x faster                                                 |
| coroutines                 | 23.9 ms                                                | 23.7 ms: 1.01x faster                                                  |
| sympy_str                  | 292 ms                                                 | 289 ms: 1.01x faster                                                   |
| pickle_pure_python         | 308 us                                                 | 310 us: 1.01x slower                                                   |
| django_template            | 34.7 ms                                                | 35.0 ms: 1.01x slower                                                  |
| async_generators           | 384 ms                                                 | 391 ms: 1.02x slower                                                   |
| json_loads                 | 26.5 us                                                | 27.6 us: 1.04x slower                                                  |
| python_startup_no_site     | 7.16 ms                                                | 7.48 ms: 1.05x slower                                                  |
| sympy_expand               | 468 ms                                                 | 491 ms: 1.05x slower                                                   |
| asyncio_websockets         | 517 ms                                                 | 545 ms: 1.05x slower                                                   |
| genshi_xml                 | 50.2 ms                                                | 52.9 ms: 1.05x slower                                                  |
| regex_dna                  | 168 ms                                                 | 183 ms: 1.09x slower                                                   |
| pidigits                   | 184 ms                                                 | 204 ms: 1.11x slower                                                   |
| regex_v8                   | 20.6 ms                                                | 23.5 ms: 1.14x slower                                                  |
| coverage                   | 71.4 ms                                                | 82.0 ms: 1.15x slower                                                  |
| gc_traversal               | 3.46 ms                                                | 4.23 ms: 1.22x slower                                                  |
| python_startup             | 9.93 ms                                                | 12.7 ms: 1.28x slower                                                  |
| bench_thread_pool          | 941 us                                                 | 1.31 ms: 1.39x slower                                                  |
| create_gc_cycles           | 1.09 ms                                                | 1.87 ms: 1.71x slower                                                  |
| bench_mp_pool              | 10.8 ms                                                | 195 ms: 18.08x slower                                                  |
| telco                      | 6.53 ms                                                | 159 ms: 24.37x slower                                                  |
| Geometric mean             | (ref)                                                  | 1.09x faster                                                           |

Benchmark hidden because not significant (1): thrift
Ignored benchmarks (21) of results/bm-20240906-3.12.6-a4a2d2b/bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b.json: aiohttp, asyncio_tcp, asyncio_tcp_ssl, chameleon, dask, flaskblogging, gunicorn, mypy2, pickle, pickle_dict, pickle_list, sqlalchemy_declarative, sqlalchemy_imperative, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http, unpack_sequence, unpickle, unpickle_list
Ignored benchmarks (10) of results/bm-20260315-3.15.0a7+-e167e06-JIT/bm-20260315-vultr-x86_64-python-e167e06f8c6b24f7b54e-3.15.0a7+-e167e06.json: connected_components, k_core, many_optionals, shortest_path, sphinx, sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile, subparsers

- Geometric mean (including insignificant results): 1.137x faster

# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.10x
- 95% likely to have a speedup of 1.09x
- 99% likely to have a speedup of 1.07x

# Memory
- memory change: 1.20x