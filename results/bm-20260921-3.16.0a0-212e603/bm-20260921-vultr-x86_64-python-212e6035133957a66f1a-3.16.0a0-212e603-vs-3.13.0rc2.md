# Results vs. 3.13.0rc2

- fork: python
- ref: 212e6035133957a66f1a
- machine: linux-x86_64
- commit hash: 212e603
- commit date: 2026-09-21
- overall geometric mean: 1.029x faster
- HPT reliability: 99.98%
- HPT 99th percentile: 1.01x faster
- Memory change: 1.15x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260921-vultr-x86_64-python-212e6035133957a66f1a-3.16.0a0-212e603 |
|----------------|:------------------------------------------------------------:|:---------------------------------------------------------------------:|
| 2to3           | 260 ms                                                       | 257 ms: 1.01x faster                                                  |
| docutils       | 2.62 sec                                                     | 2.37 sec: 1.10x faster                                                |
| html5lib       | 67.0 ms                                                      | 59.4 ms: 1.13x faster                                                 |
| Geometric mean | (ref)                                                        | 1.08x faster                                                          |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260921-vultr-x86_64-python-212e6035133957a66f1a-3.16.0a0-212e603 |
|----------------------------|:------------------------------------------------------------:|:---------------------------------------------------------------------:|
| async_tree_io_tg           | 913 ms                                                       | 757 ms: 1.21x faster                                                  |
| async_tree_cpu_io_mixed    | 666 ms                                                       | 565 ms: 1.18x faster                                                  |
| async_tree_memoization     | 461 ms                                                       | 402 ms: 1.15x faster                                                  |
| async_tree_cpu_io_mixed_tg | 638 ms                                                       | 558 ms: 1.14x faster                                                  |
| async_tree_memoization_tg  | 414 ms                                                       | 363 ms: 1.14x faster                                                  |
| async_tree_io              | 876 ms                                                       | 776 ms: 1.13x faster                                                  |
| async_tree_none_tg         | 336 ms                                                       | 300 ms: 1.12x faster                                                  |
| async_generators           | 377 ms                                                       | 342 ms: 1.10x faster                                                  |
| async_tree_none            | 354 ms                                                       | 335 ms: 1.06x faster                                                  |
| asyncio_websockets         | 520 ms                                                       | 544 ms: 1.05x slower                                                  |
| Geometric mean             | (ref)                                                        | 1.10x faster                                                          |

Benchmark hidden because not significant (1): coroutines

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260921-vultr-x86_64-python-212e6035133957a66f1a-3.16.0a0-212e603 |
|----------------|:------------------------------------------------------------:|:---------------------------------------------------------------------:|
| pidigits       | 217 ms                                                       | 191 ms: 1.13x faster                                                  |
| float          | 77.5 ms                                                      | 73.6 ms: 1.05x faster                                                 |
| nbody          | 85.1 ms                                                      | 90.8 ms: 1.07x slower                                                 |
| Geometric mean | (ref)                                                        | 1.04x faster                                                          |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260921-vultr-x86_64-python-212e6035133957a66f1a-3.16.0a0-212e603 |
|----------------|:------------------------------------------------------------:|:---------------------------------------------------------------------:|
| regex_effbot   | 3.08 ms                                                      | 2.65 ms: 1.16x faster                                                 |
| regex_v8       | 22.7 ms                                                      | 21.1 ms: 1.07x faster                                                 |
| regex_dna      | 180 ms                                                       | 179 ms: 1.01x faster                                                  |
| regex_compile  | 132 ms                                                       | 149 ms: 1.13x slower                                                  |
| Geometric mean | (ref)                                                        | 1.03x faster                                                          |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260921-vultr-x86_64-python-212e6035133957a66f1a-3.16.0a0-212e603 |
|----------------------|:------------------------------------------------------------:|:---------------------------------------------------------------------:|
| json_dumps           | 10.5 ms                                                      | 9.11 ms: 1.16x faster                                                 |
| tomli_loads          | 2.01 sec                                                     | 1.83 sec: 1.10x faster                                                |
| xml_etree_iterparse  | 94.9 ms                                                      | 92.5 ms: 1.03x faster                                                 |
| unpickle_pure_python | 210 us                                                       | 209 us: 1.00x faster                                                  |
| xml_etree_generate   | 85.4 ms                                                      | 88.6 ms: 1.04x slower                                                 |
| pickle_pure_python   | 294 us                                                       | 306 us: 1.04x slower                                                  |
| xml_etree_process    | 59.3 ms                                                      | 62.9 ms: 1.06x slower                                                 |
| xml_etree_parse      | 136 ms                                                       | 145 ms: 1.06x slower                                                  |
| Geometric mean       | (ref)                                                        | 1.01x faster                                                          |

Benchmark hidden because not significant (1): json_loads

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260921-vultr-x86_64-python-212e6035133957a66f1a-3.16.0a0-212e603 |
|------------------------|:------------------------------------------------------------:|:---------------------------------------------------------------------:|
| python_startup_no_site | 7.39 ms                                                      | 7.72 ms: 1.04x slower                                                 |
| python_startup         | 11.0 ms                                                      | 13.0 ms: 1.18x slower                                                 |
| Geometric mean         | (ref)                                                        | 1.11x slower                                                          |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260921-vultr-x86_64-python-212e6035133957a66f1a-3.16.0a0-212e603 |
|-----------------|:------------------------------------------------------------:|:---------------------------------------------------------------------:|
| mako            | 11.3 ms                                                      | 11.7 ms: 1.04x slower                                                 |
| django_template | 34.1 ms                                                      | 36.4 ms: 1.07x slower                                                 |
| Geometric mean  | (ref)                                                        | 1.05x slower                                                          |

All benchmarks:
===============

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260921-vultr-x86_64-python-212e6035133957a66f1a-3.16.0a0-212e603 |
|----------------------------|:------------------------------------------------------------:|:---------------------------------------------------------------------:|
| pylint                     | 317 ms                                                       | 112 ms: 2.83x faster                                                  |
| mdp                        | 2.36 sec                                                     | 1.13 sec: 2.09x faster                                                |
| deepcopy                   | 355 us                                                       | 231 us: 1.54x faster                                                  |
| deepcopy_memo              | 39.1 us                                                      | 27.9 us: 1.40x faster                                                 |
| go                         | 141 ms                                                       | 102 ms: 1.39x faster                                                  |
| typing_runtime_protocols   | 155 us                                                       | 122 us: 1.27x faster                                                  |
| scimark_sor                | 134 ms                                                       | 106 ms: 1.27x faster                                                  |
| deepcopy_reduce            | 3.11 us                                                      | 2.52 us: 1.23x faster                                                 |
| spectral_norm              | 111 ms                                                       | 90.7 ms: 1.22x faster                                                 |
| async_tree_io_tg           | 913 ms                                                       | 757 ms: 1.21x faster                                                  |
| async_tree_cpu_io_mixed    | 666 ms                                                       | 565 ms: 1.18x faster                                                  |
| regex_effbot               | 3.08 ms                                                      | 2.65 ms: 1.16x faster                                                 |
| json_dumps                 | 10.5 ms                                                      | 9.11 ms: 1.16x faster                                                 |
| async_tree_memoization     | 461 ms                                                       | 402 ms: 1.15x faster                                                  |
| pyflate                    | 449 ms                                                       | 392 ms: 1.14x faster                                                  |
| async_tree_cpu_io_mixed_tg | 638 ms                                                       | 558 ms: 1.14x faster                                                  |
| async_tree_memoization_tg  | 414 ms                                                       | 363 ms: 1.14x faster                                                  |
| pidigits                   | 217 ms                                                       | 191 ms: 1.13x faster                                                  |
| scimark_fft                | 349 ms                                                       | 309 ms: 1.13x faster                                                  |
| async_tree_io              | 876 ms                                                       | 776 ms: 1.13x faster                                                  |
| html5lib                   | 67.0 ms                                                      | 59.4 ms: 1.13x faster                                                 |
| async_tree_none_tg         | 336 ms                                                       | 300 ms: 1.12x faster                                                  |
| docutils                   | 2.62 sec                                                     | 2.37 sec: 1.10x faster                                                |
| async_generators           | 377 ms                                                       | 342 ms: 1.10x faster                                                  |
| tomli_loads                | 2.01 sec                                                     | 1.83 sec: 1.10x faster                                                |
| dulwich_log                | 74.8 ms                                                      | 69.0 ms: 1.08x faster                                                 |
| chaos                      | 57.3 ms                                                      | 52.9 ms: 1.08x faster                                                 |
| logging_silent             | 103 ns                                                       | 94.7 ns: 1.08x faster                                                 |
| nqueens                    | 78.6 ms                                                      | 72.7 ms: 1.08x faster                                                 |
| hexiom                     | 5.99 ms                                                      | 5.55 ms: 1.08x faster                                                 |
| regex_v8                   | 22.7 ms                                                      | 21.1 ms: 1.07x faster                                                 |
| bpe_tokeniser              | 4.45 sec                                                     | 4.19 sec: 1.06x faster                                                |
| pathlib                    | 19.2 ms                                                      | 18.1 ms: 1.06x faster                                                 |
| async_tree_none            | 354 ms                                                       | 335 ms: 1.06x faster                                                  |
| comprehensions             | 16.5 us                                                      | 15.6 us: 1.05x faster                                                 |
| float                      | 77.5 ms                                                      | 73.6 ms: 1.05x faster                                                 |
| scimark_monte_carlo        | 65.4 ms                                                      | 62.2 ms: 1.05x faster                                                 |
| logging_simple             | 6.16 us                                                      | 5.89 us: 1.05x faster                                                 |
| sympy_integrate            | 19.8 ms                                                      | 19.2 ms: 1.03x faster                                                 |
| xml_etree_iterparse        | 94.9 ms                                                      | 92.5 ms: 1.03x faster                                                 |
| richards                   | 45.2 ms                                                      | 44.3 ms: 1.02x faster                                                 |
| richards_super             | 51.6 ms                                                      | 50.7 ms: 1.02x faster                                                 |
| raytrace                   | 253 ms                                                       | 248 ms: 1.02x faster                                                  |
| crypto_pyaes               | 67.9 ms                                                      | 66.7 ms: 1.02x faster                                                 |
| pprint_safe_repr           | 738 ms                                                       | 728 ms: 1.01x faster                                                  |
| meteor_contest             | 102 ms                                                       | 101 ms: 1.01x faster                                                  |
| 2to3                       | 260 ms                                                       | 257 ms: 1.01x faster                                                  |
| logging_format             | 6.84 us                                                      | 6.79 us: 1.01x faster                                                 |
| generators                 | 28.8 ms                                                      | 28.6 ms: 1.01x faster                                                 |
| regex_dna                  | 180 ms                                                       | 179 ms: 1.01x faster                                                  |
| fannkuch                   | 370 ms                                                       | 368 ms: 1.01x faster                                                  |
| unpickle_pure_python       | 210 us                                                       | 209 us: 1.00x faster                                                  |
| pycparser                  | 1.12 sec                                                     | 1.13 sec: 1.01x slower                                                |
| sympy_str                  | 275 ms                                                       | 280 ms: 1.02x slower                                                  |
| json                       | 4.93 ms                                                      | 5.03 ms: 1.02x slower                                                 |
| sympy_sum                  | 156 ms                                                       | 159 ms: 1.02x slower                                                  |
| thrift                     | 778 us                                                       | 800 us: 1.03x slower                                                  |
| mako                       | 11.3 ms                                                      | 11.7 ms: 1.04x slower                                                 |
| xml_etree_generate         | 85.4 ms                                                      | 88.6 ms: 1.04x slower                                                 |
| sympy_expand               | 457 ms                                                       | 475 ms: 1.04x slower                                                  |
| pickle_pure_python         | 294 us                                                       | 306 us: 1.04x slower                                                  |
| python_startup_no_site     | 7.39 ms                                                      | 7.72 ms: 1.04x slower                                                 |
| deltablue                  | 3.12 ms                                                      | 3.26 ms: 1.04x slower                                                 |
| asyncio_websockets         | 520 ms                                                       | 544 ms: 1.05x slower                                                  |
| scimark_lu                 | 113 ms                                                       | 119 ms: 1.06x slower                                                  |
| xml_etree_process          | 59.3 ms                                                      | 62.9 ms: 1.06x slower                                                 |
| xml_etree_parse            | 136 ms                                                       | 145 ms: 1.06x slower                                                  |
| django_template            | 34.1 ms                                                      | 36.4 ms: 1.07x slower                                                 |
| nbody                      | 85.1 ms                                                      | 90.8 ms: 1.07x slower                                                 |
| regex_compile              | 132 ms                                                       | 149 ms: 1.13x slower                                                  |
| gc_traversal               | 3.14 ms                                                      | 3.71 ms: 1.18x slower                                                 |
| python_startup             | 11.0 ms                                                      | 13.0 ms: 1.18x slower                                                 |
| create_gc_cycles           | 1.34 ms                                                      | 1.68 ms: 1.26x slower                                                 |
| bench_thread_pool          | 919 us                                                       | 1.34 ms: 1.46x slower                                                 |
| telco                      | 7.82 ms                                                      | 160 ms: 20.42x slower                                                 |
| bench_mp_pool              | 11.0 ms                                                      | 247 ms: 22.44x slower                                                 |
| Geometric mean             | (ref)                                                        | 1.01x slower                                                          |

Benchmark hidden because not significant (6): sqlite_synth, pprint_pformat, json_loads, coverage, coroutines, scimark_sparse_mat_mult
Ignored benchmarks (20) of results/bm-20240906-3.13.0rc2-ec61006/bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006.json: aiohttp, asyncio_tcp, asyncio_tcp_ssl, chameleon, dask, flaskblogging, genshi_text, genshi_xml, gunicorn, pickle, pickle_dict, pickle_list, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http, unpack_sequence, unpickle, unpickle_list
Ignored benchmarks (10) of results/bm-20260921-3.16.0a0-212e603/bm-20260921-vultr-x86_64-python-212e6035133957a66f1a-3.16.0a0-212e603.json: connected_components, k_core, many_optionals, shortest_path, sphinx, sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile, subparsers

- Geometric mean (including insignificant results): 1.029x faster

# HPT report

- Reliability score: 99.98% likely to be faster
- 90% likely to have a speedup of 1.01x
- 95% likely to have a speedup of 1.01x
- 99% likely to have a speedup of 1.01x

# Memory
- memory change: 1.15x