# Results vs. 3.12.6

- fork: python
- ref: 76f22853410d3ded872c
- machine: linux-x86_64
- commit hash: 76f2285
- commit date: 2026-05-12
- overall geometric mean: 1.037x slower
- HPT reliability: 89.27%
- HPT 99th percentile: 1.00x slower
- Memory change: 1.39x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260512-vultr-x86_64-python-76f22853410d3ded872c-3.16.0a0-76f2285 |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| docutils       | 2.64 sec                                               | 2.98 sec: 1.13x slower                                                |
| html5lib       | 63.6 ms                                                | 69.4 ms: 1.09x slower                                                 |
| Geometric mean | (ref)                                                  | 1.11x slower                                                          |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260512-vultr-x86_64-python-76f22853410d3ded872c-3.16.0a0-76f2285 |
|----------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| async_tree_io_tg           | 1.11 sec                                               | 690 ms: 1.61x faster                                                  |
| async_tree_io              | 1.08 sec                                               | 713 ms: 1.52x faster                                                  |
| async_tree_memoization_tg  | 560 ms                                                 | 396 ms: 1.41x faster                                                  |
| async_tree_none_tg         | 446 ms                                                 | 326 ms: 1.37x faster                                                  |
| async_tree_none            | 464 ms                                                 | 343 ms: 1.35x faster                                                  |
| async_tree_memoization     | 555 ms                                                 | 422 ms: 1.32x faster                                                  |
| async_tree_cpu_io_mixed_tg | 723 ms                                                 | 578 ms: 1.25x faster                                                  |
| async_tree_cpu_io_mixed    | 715 ms                                                 | 592 ms: 1.21x faster                                                  |
| asyncio_websockets         | 517 ms                                                 | 509 ms: 1.02x faster                                                  |
| async_generators           | 384 ms                                                 | 390 ms: 1.01x slower                                                  |
| coroutines                 | 23.9 ms                                                | 25.3 ms: 1.05x slower                                                 |
| Geometric mean             | (ref)                                                  | 1.25x faster                                                          |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260512-vultr-x86_64-python-76f22853410d3ded872c-3.16.0a0-76f2285 |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| pidigits       | 184 ms                                                 | 190 ms: 1.03x slower                                                  |
| float          | 80.8 ms                                                | 88.2 ms: 1.09x slower                                                 |
| nbody          | 89.3 ms                                                | 126 ms: 1.41x slower                                                  |
| Geometric mean | (ref)                                                  | 1.17x slower                                                          |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260512-vultr-x86_64-python-76f22853410d3ded872c-3.16.0a0-76f2285 |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| regex_effbot   | 3.17 ms                                                | 3.09 ms: 1.02x faster                                                 |
| regex_compile  | 142 ms                                                 | 145 ms: 1.02x slower                                                  |
| regex_v8       | 20.6 ms                                                | 21.0 ms: 1.02x slower                                                 |
| regex_dna      | 168 ms                                                 | 179 ms: 1.07x slower                                                  |
| Geometric mean | (ref)                                                  | 1.02x slower                                                          |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260512-vultr-x86_64-python-76f22853410d3ded872c-3.16.0a0-76f2285 |
|----------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| tomli_loads          | 2.11 sec                                               | 1.95 sec: 1.08x faster                                                |
| xml_etree_iterparse  | 96.7 ms                                                | 95.9 ms: 1.01x faster                                                 |
| xml_etree_parse      | 139 ms                                                 | 140 ms: 1.01x slower                                                  |
| json_dumps           | 10.4 ms                                                | 10.8 ms: 1.04x slower                                                 |
| unpickle_pure_python | 221 us                                                 | 242 us: 1.10x slower                                                  |
| pickle_pure_python   | 308 us                                                 | 346 us: 1.12x slower                                                  |
| json_loads           | 26.5 us                                                | 30.7 us: 1.16x slower                                                 |
| xml_etree_generate   | 85.2 ms                                                | 101 ms: 1.18x slower                                                  |
| xml_etree_process    | 59.0 ms                                                | 75.0 ms: 1.27x slower                                                 |
| Geometric mean       | (ref)                                                  | 1.08x slower                                                          |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260512-vultr-x86_64-python-76f22853410d3ded872c-3.16.0a0-76f2285 |
|-----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| django_template | 34.7 ms                                                | 40.0 ms: 1.15x slower                                                 |
| mako            | 11.0 ms                                                | 16.2 ms: 1.47x slower                                                 |
| Geometric mean  | (ref)                                                  | 1.30x slower                                                          |

All benchmarks:
===============

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260512-vultr-x86_64-python-76f22853410d3ded872c-3.16.0a0-76f2285 |
|----------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| pylint                     | 319 ms                                                 | 133 ms: 2.40x faster                                                  |
| mdp                        | 2.42 sec                                               | 1.33 sec: 1.82x faster                                                |
| gc_traversal               | 3.46 ms                                                | 1.90 ms: 1.82x faster                                                 |
| async_tree_io_tg           | 1.11 sec                                               | 690 ms: 1.61x faster                                                  |
| async_tree_io              | 1.08 sec                                               | 713 ms: 1.52x faster                                                  |
| async_tree_memoization_tg  | 560 ms                                                 | 396 ms: 1.41x faster                                                  |
| async_tree_none_tg         | 446 ms                                                 | 326 ms: 1.37x faster                                                  |
| async_tree_none            | 464 ms                                                 | 343 ms: 1.35x faster                                                  |
| async_tree_memoization     | 555 ms                                                 | 422 ms: 1.32x faster                                                  |
| deepcopy                   | 352 us                                                 | 273 us: 1.29x faster                                                  |
| deepcopy_memo              | 40.3 us                                                | 31.7 us: 1.27x faster                                                 |
| async_tree_cpu_io_mixed_tg | 723 ms                                                 | 578 ms: 1.25x faster                                                  |
| async_tree_cpu_io_mixed    | 715 ms                                                 | 592 ms: 1.21x faster                                                  |
| pathlib                    | 21.5 ms                                                | 18.2 ms: 1.18x faster                                                 |
| go                         | 139 ms                                                 | 123 ms: 1.13x faster                                                  |
| sqlite_synth               | 2.20 us                                                | 1.95 us: 1.13x faster                                                 |
| typing_runtime_protocols   | 163 us                                                 | 145 us: 1.13x faster                                                  |
| comprehensions             | 19.8 us                                                | 17.7 us: 1.12x faster                                                 |
| dulwich_log                | 78.9 ms                                                | 70.5 ms: 1.12x faster                                                 |
| tomli_loads                | 2.11 sec                                               | 1.95 sec: 1.08x faster                                                |
| bpe_tokeniser              | 4.74 sec                                               | 4.42 sec: 1.07x faster                                                |
| scimark_sor                | 130 ms                                                 | 123 ms: 1.06x faster                                                  |
| deepcopy_reduce            | 3.08 us                                                | 2.97 us: 1.04x faster                                                 |
| pycparser                  | 1.17 sec                                               | 1.14 sec: 1.03x faster                                                |
| spectral_norm              | 110 ms                                                 | 107 ms: 1.03x faster                                                  |
| regex_effbot               | 3.17 ms                                                | 3.09 ms: 1.02x faster                                                 |
| logging_silent             | 109 ns                                                 | 106 ns: 1.02x faster                                                  |
| asyncio_websockets         | 517 ms                                                 | 509 ms: 1.02x faster                                                  |
| xml_etree_iterparse        | 96.7 ms                                                | 95.9 ms: 1.01x faster                                                 |
| chaos                      | 62.8 ms                                                | 62.7 ms: 1.00x faster                                                 |
| xml_etree_parse            | 139 ms                                                 | 140 ms: 1.01x slower                                                  |
| async_generators           | 384 ms                                                 | 390 ms: 1.01x slower                                                  |
| scimark_fft                | 342 ms                                                 | 347 ms: 1.01x slower                                                  |
| regex_compile              | 142 ms                                                 | 145 ms: 1.02x slower                                                  |
| raytrace                   | 299 ms                                                 | 305 ms: 1.02x slower                                                  |
| regex_v8                   | 20.6 ms                                                | 21.0 ms: 1.02x slower                                                 |
| pidigits                   | 184 ms                                                 | 190 ms: 1.03x slower                                                  |
| pyflate                    | 448 ms                                                 | 464 ms: 1.04x slower                                                  |
| json_dumps                 | 10.4 ms                                                | 10.8 ms: 1.04x slower                                                 |
| generators                 | 32.2 ms                                                | 33.7 ms: 1.04x slower                                                 |
| coroutines                 | 23.9 ms                                                | 25.3 ms: 1.05x slower                                                 |
| logging_simple             | 6.63 us                                                | 7.02 us: 1.06x slower                                                 |
| json                       | 5.02 ms                                                | 5.35 ms: 1.06x slower                                                 |
| regex_dna                  | 168 ms                                                 | 179 ms: 1.07x slower                                                  |
| hexiom                     | 6.17 ms                                                | 6.65 ms: 1.08x slower                                                 |
| sympy_integrate            | 20.5 ms                                                | 22.3 ms: 1.09x slower                                                 |
| logging_format             | 7.35 us                                                | 7.98 us: 1.09x slower                                                 |
| float                      | 80.8 ms                                                | 88.2 ms: 1.09x slower                                                 |
| html5lib                   | 63.6 ms                                                | 69.4 ms: 1.09x slower                                                 |
| unpickle_pure_python       | 221 us                                                 | 242 us: 1.10x slower                                                  |
| deltablue                  | 3.45 ms                                                | 3.79 ms: 1.10x slower                                                 |
| sympy_sum                  | 166 ms                                                 | 183 ms: 1.10x slower                                                  |
| pprint_safe_repr           | 743 ms                                                 | 823 ms: 1.11x slower                                                  |
| nqueens                    | 80.1 ms                                                | 88.8 ms: 1.11x slower                                                 |
| sympy_str                  | 292 ms                                                 | 325 ms: 1.11x slower                                                  |
| pickle_pure_python         | 308 us                                                 | 346 us: 1.12x slower                                                  |
| pprint_pformat             | 1.52 sec                                               | 1.71 sec: 1.12x slower                                                |
| docutils                   | 2.64 sec                                               | 2.98 sec: 1.13x slower                                                |
| scimark_lu                 | 114 ms                                                 | 130 ms: 1.14x slower                                                  |
| richards                   | 45.9 ms                                                | 52.7 ms: 1.15x slower                                                 |
| django_template            | 34.7 ms                                                | 40.0 ms: 1.15x slower                                                 |
| json_loads                 | 26.5 us                                                | 30.7 us: 1.16x slower                                                 |
| richards_super             | 51.9 ms                                                | 60.0 ms: 1.16x slower                                                 |
| scimark_monte_carlo        | 68.4 ms                                                | 79.4 ms: 1.16x slower                                                 |
| sympy_expand               | 468 ms                                                 | 543 ms: 1.16x slower                                                  |
| thrift                     | 791 us                                                 | 922 us: 1.16x slower                                                  |
| crypto_pyaes               | 76.6 ms                                                | 89.3 ms: 1.17x slower                                                 |
| xml_etree_generate         | 85.2 ms                                                | 101 ms: 1.18x slower                                                  |
| scimark_sparse_mat_mult    | 4.39 ms                                                | 5.29 ms: 1.20x slower                                                 |
| meteor_contest             | 104 ms                                                 | 129 ms: 1.25x slower                                                  |
| fannkuch                   | 372 ms                                                 | 469 ms: 1.26x slower                                                  |
| xml_etree_process          | 59.0 ms                                                | 75.0 ms: 1.27x slower                                                 |
| bench_mp_pool              | 10.8 ms                                                | 13.8 ms: 1.28x slower                                                 |
| create_gc_cycles           | 1.09 ms                                                | 1.47 ms: 1.35x slower                                                 |
| nbody                      | 89.3 ms                                                | 126 ms: 1.41x slower                                                  |
| mako                       | 11.0 ms                                                | 16.2 ms: 1.47x slower                                                 |
| bench_thread_pool          | 941 us                                                 | 1.47 ms: 1.56x slower                                                 |
| coverage                   | 71.4 ms                                                | 115 ms: 1.61x slower                                                  |
| telco                      | 6.53 ms                                                | 176 ms: 27.02x slower                                                 |
| Geometric mean             | (ref)                                                  | 1.05x slower                                                          |
Ignored benchmarks (26) of results/bm-20240906-3.12.6-a4a2d2b/bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b.json: 2to3, aiohttp, asyncio_tcp, asyncio_tcp_ssl, chameleon, dask, flaskblogging, genshi_text, genshi_xml, gunicorn, mypy2, pickle, pickle_dict, pickle_list, python_startup, python_startup_no_site, sqlalchemy_declarative, sqlalchemy_imperative, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http, unpack_sequence, unpickle, unpickle_list
Ignored benchmarks (10) of results/bm-20260512-3.16.0a0-76f2285-NOGIL/bm-20260512-vultr-x86_64-python-76f22853410d3ded872c-3.16.0a0-76f2285.json: connected_components, k_core, many_optionals, shortest_path, sphinx, sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile, subparsers

- Geometric mean (including insignificant results): 1.037x slower

# HPT report

- Reliability score: 89.27% likely to be slow
- 90% likely to have a slowdown of 1.00x
- 95% likely to have a slowdown of 1.00x
- 99% likely to have a slowdown of 1.00x

# Memory
- memory change: 1.39x