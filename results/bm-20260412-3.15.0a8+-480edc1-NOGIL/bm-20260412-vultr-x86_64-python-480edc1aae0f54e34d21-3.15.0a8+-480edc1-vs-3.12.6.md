# Results vs. 3.12.6

- fork: python
- ref: 480edc1aae0f54e34d21
- machine: linux-x86_64
- commit hash: 480edc1
- commit date: 2026-04-12
- overall geometric mean: 1.025x slower
- HPT reliability: 62.06%
- HPT 99th percentile: 1.00x slower
- Memory change: 1.43x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260412-vultr-x86_64-python-480edc1aae0f54e34d21-3.15.0a8+-480edc1 |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| 2to3           | 264 ms                                                 | 282 ms: 1.07x slower                                                   |
| docutils       | 2.64 sec                                               | 2.61 sec: 1.01x faster                                                 |
| html5lib       | 63.6 ms                                                | 66.9 ms: 1.05x slower                                                  |
| Geometric mean | (ref)                                                  | 1.04x slower                                                           |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260412-vultr-x86_64-python-480edc1aae0f54e34d21-3.15.0a8+-480edc1 |
|----------------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| async_tree_io_tg           | 1.11 sec                                               | 610 ms: 1.82x faster                                                   |
| async_tree_memoization_tg  | 560 ms                                                 | 323 ms: 1.73x faster                                                   |
| async_tree_none_tg         | 446 ms                                                 | 260 ms: 1.72x faster                                                   |
| async_tree_io              | 1.08 sec                                               | 633 ms: 1.71x faster                                                   |
| async_tree_none            | 464 ms                                                 | 287 ms: 1.62x faster                                                   |
| async_tree_memoization     | 555 ms                                                 | 351 ms: 1.58x faster                                                   |
| async_tree_cpu_io_mixed_tg | 723 ms                                                 | 506 ms: 1.43x faster                                                   |
| async_tree_cpu_io_mixed    | 715 ms                                                 | 533 ms: 1.34x faster                                                   |
| async_generators           | 384 ms                                                 | 377 ms: 1.02x faster                                                   |
| asyncio_websockets         | 517 ms                                                 | 510 ms: 1.01x faster                                                   |
| Geometric mean             | (ref)                                                  | 1.42x faster                                                           |

Benchmark hidden because not significant (1): coroutines

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260412-vultr-x86_64-python-480edc1aae0f54e34d21-3.15.0a8+-480edc1 |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| float          | 80.8 ms                                                | 74.0 ms: 1.09x faster                                                  |
| pidigits       | 184 ms                                                 | 180 ms: 1.02x faster                                                   |
| nbody          | 89.3 ms                                                | 126 ms: 1.41x slower                                                   |
| Geometric mean | (ref)                                                  | 1.08x slower                                                           |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260412-vultr-x86_64-python-480edc1aae0f54e34d21-3.15.0a8+-480edc1 |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| regex_effbot   | 3.17 ms                                                | 2.91 ms: 1.09x faster                                                  |
| regex_compile  | 142 ms                                                 | 141 ms: 1.01x faster                                                   |
| regex_v8       | 20.6 ms                                                | 21.0 ms: 1.02x slower                                                  |
| regex_dna      | 168 ms                                                 | 175 ms: 1.04x slower                                                   |
| Geometric mean | (ref)                                                  | 1.01x faster                                                           |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260412-vultr-x86_64-python-480edc1aae0f54e34d21-3.15.0a8+-480edc1 |
|----------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| xml_etree_iterparse  | 96.7 ms                                                | 87.3 ms: 1.11x faster                                                  |
| tomli_loads          | 2.11 sec                                               | 1.96 sec: 1.07x faster                                                 |
| xml_etree_parse      | 139 ms                                                 | 131 ms: 1.06x faster                                                   |
| json_dumps           | 10.4 ms                                                | 10.7 ms: 1.03x slower                                                  |
| unpickle_pure_python | 221 us                                                 | 236 us: 1.07x slower                                                   |
| pickle_pure_python   | 308 us                                                 | 335 us: 1.09x slower                                                   |
| json_loads           | 26.5 us                                                | 28.9 us: 1.09x slower                                                  |
| xml_etree_generate   | 85.2 ms                                                | 95.6 ms: 1.12x slower                                                  |
| xml_etree_process    | 59.0 ms                                                | 68.3 ms: 1.16x slower                                                  |
| Geometric mean       | (ref)                                                  | 1.03x slower                                                           |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260412-vultr-x86_64-python-480edc1aae0f54e34d21-3.15.0a8+-480edc1 |
|------------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| python_startup_no_site | 7.16 ms                                                | 9.92 ms: 1.39x slower                                                  |
| python_startup         | 9.93 ms                                                | 16.2 ms: 1.63x slower                                                  |
| Geometric mean         | (ref)                                                  | 1.50x slower                                                           |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260412-vultr-x86_64-python-480edc1aae0f54e34d21-3.15.0a8+-480edc1 |
|-----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| genshi_xml      | 50.2 ms                                                | 54.2 ms: 1.08x slower                                                  |
| genshi_text     | 22.8 ms                                                | 25.2 ms: 1.10x slower                                                  |
| django_template | 34.7 ms                                                | 39.6 ms: 1.14x slower                                                  |
| mako            | 11.0 ms                                                | 15.8 ms: 1.44x slower                                                  |
| Geometric mean  | (ref)                                                  | 1.18x slower                                                           |

All benchmarks:
===============

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260412-vultr-x86_64-python-480edc1aae0f54e34d21-3.15.0a8+-480edc1 |
|----------------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| mdp                        | 2.42 sec                                               | 1.32 sec: 1.84x faster                                                 |
| async_tree_io_tg           | 1.11 sec                                               | 610 ms: 1.82x faster                                                   |
| gc_traversal               | 3.46 ms                                                | 1.97 ms: 1.76x faster                                                  |
| async_tree_memoization_tg  | 560 ms                                                 | 323 ms: 1.73x faster                                                   |
| async_tree_none_tg         | 446 ms                                                 | 260 ms: 1.72x faster                                                   |
| async_tree_io              | 1.08 sec                                               | 633 ms: 1.71x faster                                                   |
| async_tree_none            | 464 ms                                                 | 287 ms: 1.62x faster                                                   |
| async_tree_memoization     | 555 ms                                                 | 351 ms: 1.58x faster                                                   |
| bench_mp_pool              | 10.8 ms                                                | 7.06 ms: 1.53x faster                                                  |
| async_tree_cpu_io_mixed_tg | 723 ms                                                 | 506 ms: 1.43x faster                                                   |
| async_tree_cpu_io_mixed    | 715 ms                                                 | 533 ms: 1.34x faster                                                   |
| deepcopy                   | 352 us                                                 | 266 us: 1.32x faster                                                   |
| deepcopy_memo              | 40.3 us                                                | 31.6 us: 1.28x faster                                                  |
| pathlib                    | 21.5 ms                                                | 17.9 ms: 1.20x faster                                                  |
| sqlite_synth               | 2.20 us                                                | 1.92 us: 1.15x faster                                                  |
| go                         | 139 ms                                                 | 122 ms: 1.14x faster                                                   |
| comprehensions             | 19.8 us                                                | 17.8 us: 1.12x faster                                                  |
| dulwich_log                | 78.9 ms                                                | 70.7 ms: 1.12x faster                                                  |
| xml_etree_iterparse        | 96.7 ms                                                | 87.3 ms: 1.11x faster                                                  |
| float                      | 80.8 ms                                                | 74.0 ms: 1.09x faster                                                  |
| regex_effbot               | 3.17 ms                                                | 2.91 ms: 1.09x faster                                                  |
| bpe_tokeniser              | 4.74 sec                                               | 4.37 sec: 1.08x faster                                                 |
| tomli_loads                | 2.11 sec                                               | 1.96 sec: 1.07x faster                                                 |
| pylint                     | 319 ms                                                 | 298 ms: 1.07x faster                                                   |
| scimark_sor                | 130 ms                                                 | 122 ms: 1.06x faster                                                   |
| xml_etree_parse            | 139 ms                                                 | 131 ms: 1.06x faster                                                   |
| pycparser                  | 1.17 sec                                               | 1.11 sec: 1.05x faster                                                 |
| deepcopy_reduce            | 3.08 us                                                | 2.93 us: 1.05x faster                                                  |
| logging_silent             | 109 ns                                                 | 105 ns: 1.04x faster                                                   |
| pidigits                   | 184 ms                                                 | 180 ms: 1.02x faster                                                   |
| async_generators           | 384 ms                                                 | 377 ms: 1.02x faster                                                   |
| asyncio_websockets         | 517 ms                                                 | 510 ms: 1.01x faster                                                   |
| regex_compile              | 142 ms                                                 | 141 ms: 1.01x faster                                                   |
| docutils                   | 2.64 sec                                               | 2.61 sec: 1.01x faster                                                 |
| chaos                      | 62.8 ms                                                | 62.6 ms: 1.00x faster                                                  |
| raytrace                   | 299 ms                                                 | 302 ms: 1.01x slower                                                   |
| logging_simple             | 6.63 us                                                | 6.74 us: 1.02x slower                                                  |
| regex_v8                   | 20.6 ms                                                | 21.0 ms: 1.02x slower                                                  |
| deltablue                  | 3.45 ms                                                | 3.51 ms: 1.02x slower                                                  |
| pyflate                    | 448 ms                                                 | 458 ms: 1.02x slower                                                   |
| json_dumps                 | 10.4 ms                                                | 10.7 ms: 1.03x slower                                                  |
| generators                 | 32.2 ms                                                | 33.4 ms: 1.04x slower                                                  |
| regex_dna                  | 168 ms                                                 | 175 ms: 1.04x slower                                                   |
| logging_format             | 7.35 us                                                | 7.70 us: 1.05x slower                                                  |
| scimark_fft                | 342 ms                                                 | 358 ms: 1.05x slower                                                   |
| html5lib                   | 63.6 ms                                                | 66.9 ms: 1.05x slower                                                  |
| sympy_integrate            | 20.5 ms                                                | 21.8 ms: 1.06x slower                                                  |
| sympy_str                  | 292 ms                                                 | 310 ms: 1.06x slower                                                   |
| sympy_sum                  | 166 ms                                                 | 177 ms: 1.07x slower                                                   |
| unpickle_pure_python       | 221 us                                                 | 236 us: 1.07x slower                                                   |
| 2to3                       | 264 ms                                                 | 282 ms: 1.07x slower                                                   |
| pprint_safe_repr           | 743 ms                                                 | 797 ms: 1.07x slower                                                   |
| hexiom                     | 6.17 ms                                                | 6.65 ms: 1.08x slower                                                  |
| genshi_xml                 | 50.2 ms                                                | 54.2 ms: 1.08x slower                                                  |
| pprint_pformat             | 1.52 sec                                               | 1.64 sec: 1.08x slower                                                 |
| pickle_pure_python         | 308 us                                                 | 335 us: 1.09x slower                                                   |
| json_loads                 | 26.5 us                                                | 28.9 us: 1.09x slower                                                  |
| nqueens                    | 80.1 ms                                                | 87.8 ms: 1.10x slower                                                  |
| genshi_text                | 22.8 ms                                                | 25.2 ms: 1.10x slower                                                  |
| scimark_lu                 | 114 ms                                                 | 128 ms: 1.12x slower                                                   |
| sympy_expand               | 468 ms                                                 | 524 ms: 1.12x slower                                                   |
| xml_etree_generate         | 85.2 ms                                                | 95.6 ms: 1.12x slower                                                  |
| richards                   | 45.9 ms                                                | 51.6 ms: 1.12x slower                                                  |
| thrift                     | 791 us                                                 | 899 us: 1.14x slower                                                   |
| richards_super             | 51.9 ms                                                | 59.2 ms: 1.14x slower                                                  |
| django_template            | 34.7 ms                                                | 39.6 ms: 1.14x slower                                                  |
| crypto_pyaes               | 76.6 ms                                                | 88.7 ms: 1.16x slower                                                  |
| xml_etree_process          | 59.0 ms                                                | 68.3 ms: 1.16x slower                                                  |
| scimark_monte_carlo        | 68.4 ms                                                | 79.7 ms: 1.17x slower                                                  |
| typing_runtime_protocols   | 163 us                                                 | 193 us: 1.18x slower                                                   |
| meteor_contest             | 104 ms                                                 | 123 ms: 1.19x slower                                                   |
| fannkuch                   | 372 ms                                                 | 461 ms: 1.24x slower                                                   |
| scimark_sparse_mat_mult    | 4.39 ms                                                | 5.65 ms: 1.29x slower                                                  |
| python_startup_no_site     | 7.16 ms                                                | 9.92 ms: 1.39x slower                                                  |
| nbody                      | 89.3 ms                                                | 126 ms: 1.41x slower                                                   |
| mako                       | 11.0 ms                                                | 15.8 ms: 1.44x slower                                                  |
| create_gc_cycles           | 1.09 ms                                                | 1.57 ms: 1.44x slower                                                  |
| coverage                   | 71.4 ms                                                | 110 ms: 1.53x slower                                                   |
| bench_thread_pool          | 941 us                                                 | 1.46 ms: 1.55x slower                                                  |
| python_startup             | 9.93 ms                                                | 16.2 ms: 1.63x slower                                                  |
| telco                      | 6.53 ms                                                | 172 ms: 26.30x slower                                                  |
| Geometric mean             | (ref)                                                  | 1.02x slower                                                           |

Benchmark hidden because not significant (3): spectral_norm, coroutines, json
Ignored benchmarks (21) of results/bm-20240906-3.12.6-a4a2d2b/bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b.json: aiohttp, asyncio_tcp, asyncio_tcp_ssl, chameleon, dask, flaskblogging, gunicorn, mypy2, pickle, pickle_dict, pickle_list, sqlalchemy_declarative, sqlalchemy_imperative, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http, unpack_sequence, unpickle, unpickle_list
Ignored benchmarks (10) of results/bm-20260412-3.15.0a8+-480edc1-NOGIL/bm-20260412-vultr-x86_64-python-480edc1aae0f54e34d21-3.15.0a8+-480edc1.json: connected_components, k_core, many_optionals, shortest_path, sphinx, sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile, subparsers

- Geometric mean (including insignificant results): 1.025x slower

# HPT report

- Reliability score: 62.06% likely to be slow
- 90% likely to have a slowdown of 1.00x
- 95% likely to have a slowdown of 1.00x
- 99% likely to have a slowdown of 1.00x

# Memory
- memory change: 1.43x