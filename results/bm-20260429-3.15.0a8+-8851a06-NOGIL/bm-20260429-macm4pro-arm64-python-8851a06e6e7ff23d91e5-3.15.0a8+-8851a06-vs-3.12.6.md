# Results vs. 3.12.6

- fork: python
- ref: 8851a06e6e7ff23d91e5
- machine: darwin-arm64
- commit hash: 8851a06
- commit date: 2026-04-29
- overall geometric mean: 1.120x faster
- HPT reliability: 99.97%
- HPT 99th percentile: 1.02x faster
- Memory change: 1.37x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260429-macm4pro-arm64-python-8851a06e6e7ff23d91e5-3.15.0a8+-8851a06 |
|----------------|:--------------------------------------------------------:|:------------------------------------------------------------------------:|
| 2to3           | 114 ms                                                   | 123 ms: 1.08x slower                                                     |
| docutils       | 1.02 sec                                                 | 978 ms: 1.05x faster                                                     |
| html5lib       | 23.0 ms                                                  | 22.5 ms: 1.02x faster                                                    |
| sphinx         | 434 ms                                                   | 416 ms: 1.04x faster                                                     |
| Geometric mean | (ref)                                                    | 1.01x faster                                                             |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                        | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260429-macm4pro-arm64-python-8851a06e6e7ff23d91e5-3.15.0a8+-8851a06 |
|----------------------------------|:--------------------------------------------------------:|:------------------------------------------------------------------------:|
| async_tree_eager_io              | 496 ms                                                   | 243 ms: 2.04x faster                                                     |
| async_tree_io_tg                 | 480 ms                                                   | 247 ms: 1.94x faster                                                     |
| async_tree_eager_io_tg           | 446 ms                                                   | 237 ms: 1.88x faster                                                     |
| async_tree_io                    | 459 ms                                                   | 248 ms: 1.85x faster                                                     |
| async_tree_memoization_tg        | 231 ms                                                   | 144 ms: 1.61x faster                                                     |
| async_tree_none_tg               | 172 ms                                                   | 109 ms: 1.57x faster                                                     |
| async_tree_none                  | 178 ms                                                   | 116 ms: 1.54x faster                                                     |
| async_tree_memoization           | 223 ms                                                   | 150 ms: 1.49x faster                                                     |
| async_tree_cpu_io_mixed_tg       | 338 ms                                                   | 256 ms: 1.32x faster                                                     |
| async_tree_cpu_io_mixed          | 333 ms                                                   | 263 ms: 1.27x faster                                                     |
| coroutines                       | 13.6 ms                                                  | 10.8 ms: 1.26x faster                                                    |
| async_tree_eager_memoization     | 132 ms                                                   | 113 ms: 1.17x faster                                                     |
| async_generators                 | 206 ms                                                   | 184 ms: 1.12x faster                                                     |
| asyncio_websockets               | 190 ms                                                   | 183 ms: 1.04x faster                                                     |
| async_tree_eager_cpu_io_mixed    | 231 ms                                                   | 223 ms: 1.04x faster                                                     |
| async_tree_eager                 | 45.6 ms                                                  | 46.9 ms: 1.03x slower                                                    |
| async_tree_eager_memoization_tg  | 113 ms                                                   | 128 ms: 1.13x slower                                                     |
| async_tree_eager_cpu_io_mixed_tg | 213 ms                                                   | 246 ms: 1.16x slower                                                     |
| async_tree_eager_tg              | 32.1 ms                                                  | 92.4 ms: 2.88x slower                                                    |
| Geometric mean                   | (ref)                                                    | 1.24x faster                                                             |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260429-macm4pro-arm64-python-8851a06e6e7ff23d91e5-3.15.0a8+-8851a06 |
|----------------|:--------------------------------------------------------:|:------------------------------------------------------------------------:|
| float          | 37.9 ms                                                  | 28.5 ms: 1.33x faster                                                    |
| pidigits       | 161 ms                                                   | 159 ms: 1.01x faster                                                     |
| nbody          | 54.2 ms                                                  | 55.3 ms: 1.02x slower                                                    |
| Geometric mean | (ref)                                                    | 1.10x faster                                                             |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260429-macm4pro-arm64-python-8851a06e6e7ff23d91e5-3.15.0a8+-8851a06 |
|----------------|:--------------------------------------------------------:|:------------------------------------------------------------------------:|
| regex_effbot   | 1.67 ms                                                  | 1.43 ms: 1.17x faster                                                    |
| regex_compile  | 54.6 ms                                                  | 50.5 ms: 1.08x faster                                                    |
| regex_dna      | 99.6 ms                                                  | 96.4 ms: 1.03x faster                                                    |
| regex_v8       | 9.59 ms                                                  | 9.30 ms: 1.03x faster                                                    |
| Geometric mean | (ref)                                                    | 1.08x faster                                                             |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260429-macm4pro-arm64-python-8851a06e6e7ff23d91e5-3.15.0a8+-8851a06 |
|----------------------|:--------------------------------------------------------:|:------------------------------------------------------------------------:|
| xml_etree_iterparse  | 51.6 ms                                                  | 36.6 ms: 1.41x faster                                                    |
| xml_etree_parse      | 67.9 ms                                                  | 55.4 ms: 1.23x faster                                                    |
| json_dumps           | 4.26 ms                                                  | 3.77 ms: 1.13x faster                                                    |
| tomli_loads          | 957 ms                                                   | 863 ms: 1.11x faster                                                     |
| xml_etree_generate   | 38.9 ms                                                  | 36.7 ms: 1.06x faster                                                    |
| xml_etree_process    | 26.7 ms                                                  | 26.5 ms: 1.01x faster                                                    |
| json_loads           | 10.9 us                                                  | 11.3 us: 1.04x slower                                                    |
| unpickle_pure_python | 103 us                                                   | 108 us: 1.05x slower                                                     |
| pickle_pure_python   | 139 us                                                   | 148 us: 1.06x slower                                                     |
| Geometric mean       | (ref)                                                    | 1.08x faster                                                             |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260429-macm4pro-arm64-python-8851a06e6e7ff23d91e5-3.15.0a8+-8851a06 |
|------------------------|:--------------------------------------------------------:|:------------------------------------------------------------------------:|
| python_startup_no_site | 5.71 ms                                                  | 6.66 ms: 1.17x slower                                                    |
| python_startup         | 8.01 ms                                                  | 9.52 ms: 1.19x slower                                                    |
| Geometric mean         | (ref)                                                    | 1.18x slower                                                             |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260429-macm4pro-arm64-python-8851a06e6e7ff23d91e5-3.15.0a8+-8851a06 |
|-----------------|:--------------------------------------------------------:|:------------------------------------------------------------------------:|
| django_template | 13.6 ms                                                  | 15.5 ms: 1.14x slower                                                    |
| mako            | 4.77 ms                                                  | 5.65 ms: 1.18x slower                                                    |
| Geometric mean  | (ref)                                                    | 1.16x slower                                                             |

All benchmarks:
===============

| Benchmark                        | bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260429-macm4pro-arm64-python-8851a06e6e7ff23d91e5-3.15.0a8+-8851a06 |
|----------------------------------|:--------------------------------------------------------:|:------------------------------------------------------------------------:|
| subparsers                       | 20.8 ms                                                  | 4.10 ms: 5.07x faster                                                    |
| gc_traversal                     | 2.01 ms                                                  | 771 us: 2.61x faster                                                     |
| pylint                           | 128 ms                                                   | 51.0 ms: 2.51x faster                                                    |
| async_tree_eager_io              | 496 ms                                                   | 243 ms: 2.04x faster                                                     |
| async_tree_io_tg                 | 480 ms                                                   | 247 ms: 1.94x faster                                                     |
| async_tree_eager_io_tg           | 446 ms                                                   | 237 ms: 1.88x faster                                                     |
| async_tree_io                    | 459 ms                                                   | 248 ms: 1.85x faster                                                     |
| mdp                              | 1.09 sec                                                 | 606 ms: 1.80x faster                                                     |
| create_gc_cycles                 | 830 us                                                   | 485 us: 1.71x faster                                                     |
| async_tree_memoization_tg        | 231 ms                                                   | 144 ms: 1.61x faster                                                     |
| async_tree_none_tg               | 172 ms                                                   | 109 ms: 1.57x faster                                                     |
| async_tree_none                  | 178 ms                                                   | 116 ms: 1.54x faster                                                     |
| deepcopy                         | 161 us                                                   | 107 us: 1.52x faster                                                     |
| async_tree_memoization           | 223 ms                                                   | 150 ms: 1.49x faster                                                     |
| deepcopy_memo                    | 18.3 us                                                  | 12.9 us: 1.41x faster                                                    |
| xml_etree_iterparse              | 51.6 ms                                                  | 36.6 ms: 1.41x faster                                                    |
| float                            | 37.9 ms                                                  | 28.5 ms: 1.33x faster                                                    |
| async_tree_cpu_io_mixed_tg       | 338 ms                                                   | 256 ms: 1.32x faster                                                     |
| async_tree_cpu_io_mixed          | 333 ms                                                   | 263 ms: 1.27x faster                                                     |
| coroutines                       | 13.6 ms                                                  | 10.8 ms: 1.26x faster                                                    |
| deepcopy_reduce                  | 1.46 us                                                  | 1.17 us: 1.25x faster                                                    |
| xml_etree_parse                  | 67.9 ms                                                  | 55.4 ms: 1.23x faster                                                    |
| sqlite_synth                     | 967 ns                                                   | 792 ns: 1.22x faster                                                     |
| comprehensions                   | 9.84 us                                                  | 8.12 us: 1.21x faster                                                    |
| go                               | 70.0 ms                                                  | 58.8 ms: 1.19x faster                                                    |
| logging_silent                   | 50.9 ns                                                  | 42.9 ns: 1.19x faster                                                    |
| regex_effbot                     | 1.67 ms                                                  | 1.43 ms: 1.17x faster                                                    |
| async_tree_eager_memoization     | 132 ms                                                   | 113 ms: 1.17x faster                                                     |
| pathlib                          | 12.4 ms                                                  | 10.7 ms: 1.16x faster                                                    |
| dulwich_log                      | 21.3 ms                                                  | 18.4 ms: 1.15x faster                                                    |
| raytrace                         | 145 ms                                                   | 126 ms: 1.15x faster                                                     |
| pyflate                          | 216 ms                                                   | 191 ms: 1.13x faster                                                     |
| json_dumps                       | 4.26 ms                                                  | 3.77 ms: 1.13x faster                                                    |
| k_core                           | 1.12 sec                                                 | 992 ms: 1.13x faster                                                     |
| scimark_fft                      | 142 ms                                                   | 126 ms: 1.12x faster                                                     |
| async_generators                 | 206 ms                                                   | 184 ms: 1.12x faster                                                     |
| scimark_sor                      | 61.0 ms                                                  | 54.7 ms: 1.11x faster                                                    |
| tomli_loads                      | 957 ms                                                   | 863 ms: 1.11x faster                                                     |
| bpe_tokeniser                    | 2.24 sec                                                 | 2.02 sec: 1.11x faster                                                   |
| logging_simple                   | 2.57 us                                                  | 2.34 us: 1.10x faster                                                    |
| pycparser                        | 497 ms                                                   | 455 ms: 1.09x faster                                                     |
| logging_format                   | 2.80 us                                                  | 2.58 us: 1.09x faster                                                    |
| regex_compile                    | 54.6 ms                                                  | 50.5 ms: 1.08x faster                                                    |
| spectral_norm                    | 54.4 ms                                                  | 50.4 ms: 1.08x faster                                                    |
| chaos                            | 28.9 ms                                                  | 27.0 ms: 1.07x faster                                                    |
| nqueens                          | 43.5 ms                                                  | 40.5 ms: 1.07x faster                                                    |
| deltablue                        | 1.73 ms                                                  | 1.62 ms: 1.06x faster                                                    |
| xml_etree_generate               | 38.9 ms                                                  | 36.7 ms: 1.06x faster                                                    |
| generators                       | 21.9 ms                                                  | 20.8 ms: 1.05x faster                                                    |
| docutils                         | 1.02 sec                                                 | 978 ms: 1.05x faster                                                     |
| sphinx                           | 434 ms                                                   | 416 ms: 1.04x faster                                                     |
| asyncio_websockets               | 190 ms                                                   | 183 ms: 1.04x faster                                                     |
| async_tree_eager_cpu_io_mixed    | 231 ms                                                   | 223 ms: 1.04x faster                                                     |
| regex_dna                        | 99.6 ms                                                  | 96.4 ms: 1.03x faster                                                    |
| regex_v8                         | 9.59 ms                                                  | 9.30 ms: 1.03x faster                                                    |
| fannkuch                         | 176 ms                                                   | 171 ms: 1.03x faster                                                     |
| html5lib                         | 23.0 ms                                                  | 22.5 ms: 1.02x faster                                                    |
| pidigits                         | 161 ms                                                   | 159 ms: 1.01x faster                                                     |
| xml_etree_process                | 26.7 ms                                                  | 26.5 ms: 1.01x faster                                                    |
| sympy_integrate                  | 8.02 ms                                                  | 8.04 ms: 1.00x slower                                                    |
| pprint_safe_repr                 | 328 ms                                                   | 332 ms: 1.01x slower                                                     |
| scimark_monte_carlo              | 32.2 ms                                                  | 32.6 ms: 1.01x slower                                                    |
| pprint_pformat                   | 665 ms                                                   | 675 ms: 1.02x slower                                                     |
| nbody                            | 54.2 ms                                                  | 55.3 ms: 1.02x slower                                                    |
| scimark_sparse_mat_mult          | 2.08 ms                                                  | 2.13 ms: 1.02x slower                                                    |
| scimark_lu                       | 51.3 ms                                                  | 52.6 ms: 1.03x slower                                                    |
| richards                         | 22.4 ms                                                  | 23.0 ms: 1.03x slower                                                    |
| async_tree_eager                 | 45.6 ms                                                  | 46.9 ms: 1.03x slower                                                    |
| richards_super                   | 25.4 ms                                                  | 26.2 ms: 1.03x slower                                                    |
| hexiom                           | 3.04 ms                                                  | 3.14 ms: 1.03x slower                                                    |
| json_loads                       | 10.9 us                                                  | 11.3 us: 1.04x slower                                                    |
| unpickle_pure_python             | 103 us                                                   | 108 us: 1.05x slower                                                     |
| thrift                           | 322 us                                                   | 338 us: 1.05x slower                                                     |
| sympy_sum                        | 57.6 ms                                                  | 60.5 ms: 1.05x slower                                                    |
| sympy_str                        | 104 ms                                                   | 110 ms: 1.05x slower                                                     |
| pickle_pure_python               | 139 us                                                   | 148 us: 1.06x slower                                                     |
| 2to3                             | 114 ms                                                   | 123 ms: 1.08x slower                                                     |
| sympy_expand                     | 167 ms                                                   | 186 ms: 1.12x slower                                                     |
| shortest_path                    | 219 ms                                                   | 248 ms: 1.13x slower                                                     |
| async_tree_eager_memoization_tg  | 113 ms                                                   | 128 ms: 1.13x slower                                                     |
| crypto_pyaes                     | 38.8 ms                                                  | 44.1 ms: 1.14x slower                                                    |
| django_template                  | 13.6 ms                                                  | 15.5 ms: 1.14x slower                                                    |
| meteor_contest                   | 47.7 ms                                                  | 54.6 ms: 1.14x slower                                                    |
| async_tree_eager_cpu_io_mixed_tg | 213 ms                                                   | 246 ms: 1.16x slower                                                     |
| python_startup_no_site           | 5.71 ms                                                  | 6.66 ms: 1.17x slower                                                    |
| bench_mp_pool                    | 39.7 ms                                                  | 46.5 ms: 1.17x slower                                                    |
| mako                             | 4.77 ms                                                  | 5.65 ms: 1.18x slower                                                    |
| telco                            | 2.61 ms                                                  | 3.09 ms: 1.18x slower                                                    |
| python_startup                   | 8.01 ms                                                  | 9.52 ms: 1.19x slower                                                    |
| connected_components             | 201 ms                                                   | 244 ms: 1.22x slower                                                     |
| bench_thread_pool                | 419 us                                                   | 566 us: 1.35x slower                                                     |
| many_optionals                   | 195 us                                                   | 265 us: 1.36x slower                                                     |
| coverage                         | 26.9 ms                                                  | 39.6 ms: 1.47x slower                                                    |
| async_tree_eager_tg              | 32.1 ms                                                  | 92.4 ms: 2.88x slower                                                    |
| Geometric mean                   | (ref)                                                    | 1.11x faster                                                             |

Benchmark hidden because not significant (3): dask, typing_runtime_protocols, json
Ignored benchmarks (12) of results/bm-20240906-3.12.6-a4a2d2b/bm-20240906-macm4pro-arm64-python-v3.12.6-3.12.6-a4a2d2b.json: chameleon, djangocms, genshi_text, genshi_xml, gevent_hub, sqlalchemy_declarative, sqlalchemy_imperative, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http
Ignored benchmarks (4) of results/bm-20260429-3.15.0a8+-8851a06-NOGIL/bm-20260429-macm4pro-arm64-python-8851a06e6e7ff23d91e5-3.15.0a8+-8851a06.json: sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile

- Geometric mean (including insignificant results): 1.120x faster

# HPT report

- Reliability score: 99.97% likely to be faster
- 90% likely to have a speedup of 1.04x
- 95% likely to have a speedup of 1.03x
- 99% likely to have a speedup of 1.02x

# Memory
- memory change: 1.37x