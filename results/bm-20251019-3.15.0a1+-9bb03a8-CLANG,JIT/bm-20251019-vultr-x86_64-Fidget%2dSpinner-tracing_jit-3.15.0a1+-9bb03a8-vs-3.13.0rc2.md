# Results vs. 3.13.0rc2

- fork: Fidget-Spinner
- ref: tracing_jit
- machine: linux-x86_64
- commit hash: 9bb03a8
- commit date: 2025-10-19
- overall geometric mean: 1.075x faster
- HPT reliability: 100.00%
- HPT 99th percentile: 1.03x faster
- Memory change: 1.20x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5 | bm-20251019-vultr-x86_64-Fidget%2dSpinner-tracing_jit-3.15.0a1+-9bb03a8 |
|----------------|:----------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| 2to3           | 259 ms                                                                 | 251 ms: 1.03x faster                                                    |
| docutils       | 2.63 sec                                                               | 2.57 sec: 1.02x faster                                                  |
| html5lib       | 68.6 ms                                                                | 65.7 ms: 1.04x faster                                                   |
| Geometric mean | (ref)                                                                  | 1.03x faster                                                            |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5 | bm-20251019-vultr-x86_64-Fidget%2dSpinner-tracing_jit-3.15.0a1+-9bb03a8 |
|----------------------------|:----------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| async_tree_memoization     | 459 ms                                                                 | 303 ms: 1.51x faster                                                    |
| async_tree_io              | 881 ms                                                                 | 588 ms: 1.50x faster                                                    |
| async_tree_io_tg           | 901 ms                                                                 | 608 ms: 1.48x faster                                                    |
| async_tree_cpu_io_mixed    | 666 ms                                                                 | 469 ms: 1.42x faster                                                    |
| async_tree_none            | 353 ms                                                                 | 256 ms: 1.38x faster                                                    |
| async_tree_none_tg         | 333 ms                                                                 | 243 ms: 1.37x faster                                                    |
| async_tree_cpu_io_mixed_tg | 634 ms                                                                 | 469 ms: 1.35x faster                                                    |
| async_tree_memoization_tg  | 410 ms                                                                 | 305 ms: 1.34x faster                                                    |
| async_generators           | 375 ms                                                                 | 331 ms: 1.13x faster                                                    |
| coroutines                 | 23.3 ms                                                                | 22.7 ms: 1.03x faster                                                   |
| asyncio_websockets         | 517 ms                                                                 | 545 ms: 1.05x slower                                                    |
| Geometric mean             | (ref)                                                                  | 1.30x faster                                                            |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5 | bm-20251019-vultr-x86_64-Fidget%2dSpinner-tracing_jit-3.15.0a1+-9bb03a8 |
|----------------|:----------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| float          | 76.7 ms                                                                | 62.2 ms: 1.23x faster                                                   |
| pidigits       | 216 ms                                                                 | 192 ms: 1.13x faster                                                    |
| nbody          | 85.3 ms                                                                | 87.3 ms: 1.02x slower                                                   |
| Geometric mean | (ref)                                                                  | 1.11x faster                                                            |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5 | bm-20251019-vultr-x86_64-Fidget%2dSpinner-tracing_jit-3.15.0a1+-9bb03a8 |
|----------------|:----------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| regex_effbot   | 3.21 ms                                                                | 2.94 ms: 1.09x faster                                                   |
| regex_v8       | 23.2 ms                                                                | 21.6 ms: 1.07x faster                                                   |
| regex_dna      | 189 ms                                                                 | 177 ms: 1.06x faster                                                    |
| regex_compile  | 131 ms                                                                 | 136 ms: 1.04x slower                                                    |
| Geometric mean | (ref)                                                                  | 1.05x faster                                                            |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5 | bm-20251019-vultr-x86_64-Fidget%2dSpinner-tracing_jit-3.15.0a1+-9bb03a8 |
|----------------------|:----------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| json_dumps           | 10.6 ms                                                                | 8.87 ms: 1.20x faster                                                   |
| tomli_loads          | 2.01 sec                                                               | 1.76 sec: 1.14x faster                                                  |
| unpickle_pure_python | 208 us                                                                 | 183 us: 1.14x faster                                                    |
| xml_etree_process    | 59.2 ms                                                                | 52.0 ms: 1.14x faster                                                   |
| xml_etree_generate   | 85.4 ms                                                                | 76.2 ms: 1.12x faster                                                   |
| json_loads           | 27.3 us                                                                | 26.0 us: 1.05x faster                                                   |
| xml_etree_iterparse  | 94.4 ms                                                                | 93.5 ms: 1.01x faster                                                   |
| pickle_pure_python   | 292 us                                                                 | 302 us: 1.04x slower                                                    |
| xml_etree_parse      | 136 ms                                                                 | 144 ms: 1.06x slower                                                    |
| Geometric mean       | (ref)                                                                  | 1.08x faster                                                            |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5 | bm-20251019-vultr-x86_64-Fidget%2dSpinner-tracing_jit-3.15.0a1+-9bb03a8 |
|------------------------|:----------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| python_startup_no_site | 7.41 ms                                                                | 7.39 ms: 1.00x faster                                                   |
| python_startup         | 11.0 ms                                                                | 12.7 ms: 1.15x slower                                                   |
| Geometric mean         | (ref)                                                                  | 1.07x slower                                                            |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5 | bm-20251019-vultr-x86_64-Fidget%2dSpinner-tracing_jit-3.15.0a1+-9bb03a8 |
|-----------------|:----------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| genshi_text     | 21.7 ms                                                                | 20.0 ms: 1.09x faster                                                   |
| mako            | 11.2 ms                                                                | 11.4 ms: 1.02x slower                                                   |
| django_template | 34.2 ms                                                                | 35.2 ms: 1.03x slower                                                   |
| Geometric mean  | (ref)                                                                  | 1.01x faster                                                            |

Benchmark hidden because not significant (1): genshi_xml

All benchmarks:
===============

| Benchmark                  | bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5 | bm-20251019-vultr-x86_64-Fidget%2dSpinner-tracing_jit-3.15.0a1+-9bb03a8 |
|----------------------------|:----------------------------------------------------------------------:|:-----------------------------------------------------------------------:|
| mdp                        | 2.34 sec                                                               | 1.18 sec: 1.99x faster                                                  |
| deepcopy_memo              | 38.1 us                                                                | 21.9 us: 1.74x faster                                                   |
| deepcopy                   | 357 us                                                                 | 233 us: 1.53x faster                                                    |
| richards                   | 44.4 ms                                                                | 29.0 ms: 1.53x faster                                                   |
| async_tree_memoization     | 459 ms                                                                 | 303 ms: 1.51x faster                                                    |
| richards_super             | 50.4 ms                                                                | 33.5 ms: 1.50x faster                                                   |
| async_tree_io              | 881 ms                                                                 | 588 ms: 1.50x faster                                                    |
| async_tree_io_tg           | 901 ms                                                                 | 608 ms: 1.48x faster                                                    |
| scimark_fft                | 348 ms                                                                 | 242 ms: 1.44x faster                                                    |
| async_tree_cpu_io_mixed    | 666 ms                                                                 | 469 ms: 1.42x faster                                                    |
| async_tree_none            | 353 ms                                                                 | 256 ms: 1.38x faster                                                    |
| async_tree_none_tg         | 333 ms                                                                 | 243 ms: 1.37x faster                                                    |
| scimark_sor                | 134 ms                                                                 | 98.8 ms: 1.35x faster                                                   |
| async_tree_cpu_io_mixed_tg | 634 ms                                                                 | 469 ms: 1.35x faster                                                    |
| async_tree_memoization_tg  | 410 ms                                                                 | 305 ms: 1.34x faster                                                    |
| deepcopy_reduce            | 3.12 us                                                                | 2.38 us: 1.31x faster                                                   |
| spectral_norm              | 108 ms                                                                 | 83.1 ms: 1.29x faster                                                   |
| logging_silent             | 98.2 ns                                                                | 78.0 ns: 1.26x faster                                                   |
| float                      | 76.7 ms                                                                | 62.2 ms: 1.23x faster                                                   |
| scimark_sparse_mat_mult    | 4.75 ms                                                                | 3.95 ms: 1.20x faster                                                   |
| json_dumps                 | 10.6 ms                                                                | 8.87 ms: 1.20x faster                                                   |
| scimark_monte_carlo        | 65.8 ms                                                                | 55.5 ms: 1.19x faster                                                   |
| go                         | 141 ms                                                                 | 120 ms: 1.17x faster                                                    |
| pyflate                    | 449 ms                                                                 | 391 ms: 1.15x faster                                                    |
| pylint                     | 317 ms                                                                 | 276 ms: 1.15x faster                                                    |
| tomli_loads                | 2.01 sec                                                               | 1.76 sec: 1.14x faster                                                  |
| unpickle_pure_python       | 208 us                                                                 | 183 us: 1.14x faster                                                    |
| xml_etree_process          | 59.2 ms                                                                | 52.0 ms: 1.14x faster                                                   |
| async_generators           | 375 ms                                                                 | 331 ms: 1.13x faster                                                    |
| comprehensions             | 16.6 us                                                                | 14.7 us: 1.13x faster                                                   |
| pidigits                   | 216 ms                                                                 | 192 ms: 1.13x faster                                                    |
| bpe_tokeniser              | 4.46 sec                                                               | 3.97 sec: 1.12x faster                                                  |
| xml_etree_generate         | 85.4 ms                                                                | 76.2 ms: 1.12x faster                                                   |
| deltablue                  | 3.10 ms                                                                | 2.78 ms: 1.11x faster                                                   |
| scimark_lu                 | 112 ms                                                                 | 101 ms: 1.11x faster                                                    |
| coverage                   | 82.5 ms                                                                | 74.5 ms: 1.11x faster                                                   |
| pprint_safe_repr           | 719 ms                                                                 | 649 ms: 1.11x faster                                                    |
| regex_effbot               | 3.21 ms                                                                | 2.94 ms: 1.09x faster                                                   |
| pprint_pformat             | 1.46 sec                                                               | 1.34 sec: 1.09x faster                                                  |
| genshi_text                | 21.7 ms                                                                | 20.0 ms: 1.09x faster                                                   |
| chaos                      | 56.3 ms                                                                | 52.3 ms: 1.08x faster                                                   |
| regex_v8                   | 23.2 ms                                                                | 21.6 ms: 1.07x faster                                                   |
| regex_dna                  | 189 ms                                                                 | 177 ms: 1.06x faster                                                    |
| fannkuch                   | 376 ms                                                                 | 355 ms: 1.06x faster                                                    |
| crypto_pyaes               | 68.2 ms                                                                | 64.6 ms: 1.06x faster                                                   |
| typing_runtime_protocols   | 156 us                                                                 | 147 us: 1.06x faster                                                    |
| sympy_integrate            | 19.7 ms                                                                | 18.7 ms: 1.05x faster                                                   |
| json_loads                 | 27.3 us                                                                | 26.0 us: 1.05x faster                                                   |
| logging_format             | 6.92 us                                                                | 6.57 us: 1.05x faster                                                   |
| sqlite_synth               | 2.25 us                                                                | 2.14 us: 1.05x faster                                                   |
| html5lib                   | 68.6 ms                                                                | 65.7 ms: 1.04x faster                                                   |
| logging_simple             | 6.14 us                                                                | 5.88 us: 1.04x faster                                                   |
| hexiom                     | 5.95 ms                                                                | 5.74 ms: 1.04x faster                                                   |
| thrift                     | 772 us                                                                 | 744 us: 1.04x faster                                                    |
| 2to3                       | 259 ms                                                                 | 251 ms: 1.03x faster                                                    |
| coroutines                 | 23.3 ms                                                                | 22.7 ms: 1.03x faster                                                   |
| docutils                   | 2.63 sec                                                               | 2.57 sec: 1.02x faster                                                  |
| generators                 | 28.5 ms                                                                | 27.9 ms: 1.02x faster                                                   |
| json                       | 4.98 ms                                                                | 4.90 ms: 1.02x faster                                                   |
| xml_etree_iterparse        | 94.4 ms                                                                | 93.5 ms: 1.01x faster                                                   |
| python_startup_no_site     | 7.41 ms                                                                | 7.39 ms: 1.00x faster                                                   |
| mako                       | 11.2 ms                                                                | 11.4 ms: 1.02x slower                                                   |
| sympy_str                  | 274 ms                                                                 | 280 ms: 1.02x slower                                                    |
| nbody                      | 85.3 ms                                                                | 87.3 ms: 1.02x slower                                                   |
| meteor_contest             | 101 ms                                                                 | 104 ms: 1.03x slower                                                    |
| django_template            | 34.2 ms                                                                | 35.2 ms: 1.03x slower                                                   |
| nqueens                    | 77.7 ms                                                                | 80.1 ms: 1.03x slower                                                   |
| pickle_pure_python         | 292 us                                                                 | 302 us: 1.04x slower                                                    |
| regex_compile              | 131 ms                                                                 | 136 ms: 1.04x slower                                                    |
| bench_thread_pool          | 924 us                                                                 | 963 us: 1.04x slower                                                    |
| asyncio_websockets         | 517 ms                                                                 | 545 ms: 1.05x slower                                                    |
| sympy_expand               | 454 ms                                                                 | 479 ms: 1.06x slower                                                    |
| xml_etree_parse            | 136 ms                                                                 | 144 ms: 1.06x slower                                                    |
| raytrace                   | 250 ms                                                                 | 267 ms: 1.07x slower                                                    |
| bench_mp_pool              | 11.0 ms                                                                | 12.3 ms: 1.12x slower                                                   |
| python_startup             | 11.0 ms                                                                | 12.7 ms: 1.15x slower                                                   |
| gc_traversal               | 3.32 ms                                                                | 4.27 ms: 1.29x slower                                                   |
| create_gc_cycles           | 1.33 ms                                                                | 2.03 ms: 1.52x slower                                                   |
| telco                      | 7.77 ms                                                                | 158 ms: 20.26x slower                                                   |
| Geometric mean             | (ref)                                                                  | 1.07x faster                                                            |

Benchmark hidden because not significant (5): sympy_sum, pathlib, dulwich_log, pycparser, genshi_xml
Ignored benchmarks (18) of results/bm-20240920-3.13.0rc2-4981ec5/bm-20240920-vultr-x86_64-python-4981ec59ded050919eb2-3.13.0rc2-4981ec5.json: aiohttp, asyncio_tcp, asyncio_tcp_ssl, chameleon, dask, flaskblogging, gunicorn, pickle, pickle_dict, pickle_list, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http, unpack_sequence, unpickle, unpickle_list
Ignored benchmarks (10) of results/bm-20251019-3.15.0a1+-9bb03a8-CLANG,JIT/bm-20251019-vultr-x86_64-Fidget%2dSpinner-tracing_jit-3.15.0a1+-9bb03a8.json: connected_components, k_core, many_optionals, shortest_path, sphinx, sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile, subparsers

- Geometric mean (including insignificant results): 1.075x faster

# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.06x
- 95% likely to have a speedup of 1.05x
- 99% likely to have a speedup of 1.03x

# Memory
- memory change: 1.20x