# Results vs. 3.13.0rc2

- fork: python
- ref: 0adc7289c3ab097b5608
- machine: linux-x86_64
- commit hash: 0adc728
- commit date: 2026-03-12
- overall geometric mean: 1.098x faster
- HPT reliability: 100.00%
- HPT 99th percentile: 1.04x faster
- Memory change: 1.18x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260312-vultr-x86_64-python-0adc7289c3ab097b5608-3.15.0a7+-0adc728 |
|----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| 2to3           | 260 ms                                                       | 250 ms: 1.04x faster                                                   |
| html5lib       | 67.0 ms                                                      | 63.6 ms: 1.05x faster                                                  |
| Geometric mean | (ref)                                                        | 1.03x faster                                                           |

Benchmark hidden because not significant (1): docutils

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260312-vultr-x86_64-python-0adc7289c3ab097b5608-3.15.0a7+-0adc728 |
|----------------------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| async_tree_io_tg           | 913 ms                                                       | 564 ms: 1.62x faster                                                   |
| async_tree_memoization     | 461 ms                                                       | 299 ms: 1.54x faster                                                   |
| async_tree_io              | 876 ms                                                       | 582 ms: 1.51x faster                                                   |
| async_tree_none            | 354 ms                                                       | 240 ms: 1.47x faster                                                   |
| async_tree_none_tg         | 336 ms                                                       | 237 ms: 1.42x faster                                                   |
| async_tree_memoization_tg  | 414 ms                                                       | 297 ms: 1.40x faster                                                   |
| async_tree_cpu_io_mixed    | 666 ms                                                       | 493 ms: 1.35x faster                                                   |
| async_tree_cpu_io_mixed_tg | 638 ms                                                       | 504 ms: 1.26x faster                                                   |
| coroutines                 | 23.6 ms                                                      | 23.1 ms: 1.02x faster                                                  |
| asyncio_websockets         | 520 ms                                                       | 545 ms: 1.05x slower                                                   |
| async_generators           | 377 ms                                                       | 403 ms: 1.07x slower                                                   |
| Geometric mean             | (ref)                                                        | 1.29x faster                                                           |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260312-vultr-x86_64-python-0adc7289c3ab097b5608-3.15.0a7+-0adc728 |
|----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| float          | 77.5 ms                                                      | 59.2 ms: 1.31x faster                                                  |
| nbody          | 85.1 ms                                                      | 76.3 ms: 1.11x faster                                                  |
| pidigits       | 217 ms                                                       | 200 ms: 1.08x faster                                                   |
| Geometric mean | (ref)                                                        | 1.17x faster                                                           |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260312-vultr-x86_64-python-0adc7289c3ab097b5608-3.15.0a7+-0adc728 |
|----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| regex_effbot   | 3.08 ms                                                      | 2.89 ms: 1.07x faster                                                  |
| regex_compile  | 132 ms                                                       | 124 ms: 1.06x faster                                                   |
| regex_v8       | 22.7 ms                                                      | 23.2 ms: 1.02x slower                                                  |
| regex_dna      | 180 ms                                                       | 189 ms: 1.05x slower                                                   |
| Geometric mean | (ref)                                                        | 1.01x faster                                                           |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260312-vultr-x86_64-python-0adc7289c3ab097b5608-3.15.0a7+-0adc728 |
|----------------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| tomli_loads          | 2.01 sec                                                     | 1.54 sec: 1.30x faster                                                 |
| json_dumps           | 10.5 ms                                                      | 9.16 ms: 1.15x faster                                                  |
| xml_etree_iterparse  | 94.9 ms                                                      | 82.6 ms: 1.15x faster                                                  |
| xml_etree_generate   | 85.4 ms                                                      | 77.5 ms: 1.10x faster                                                  |
| xml_etree_process    | 59.3 ms                                                      | 53.9 ms: 1.10x faster                                                  |
| unpickle_pure_python | 210 us                                                       | 197 us: 1.07x faster                                                   |
| xml_etree_parse      | 136 ms                                                       | 129 ms: 1.05x faster                                                   |
| json_loads           | 27.0 us                                                      | 27.5 us: 1.02x slower                                                  |
| pickle_pure_python   | 294 us                                                       | 315 us: 1.07x slower                                                   |
| Geometric mean       | (ref)                                                        | 1.09x faster                                                           |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260312-vultr-x86_64-python-0adc7289c3ab097b5608-3.15.0a7+-0adc728 |
|------------------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| python_startup_no_site | 7.39 ms                                                      | 7.46 ms: 1.01x slower                                                  |
| python_startup         | 11.0 ms                                                      | 12.6 ms: 1.15x slower                                                  |
| Geometric mean         | (ref)                                                        | 1.08x slower                                                           |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260312-vultr-x86_64-python-0adc7289c3ab097b5608-3.15.0a7+-0adc728 |
|-----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| mako            | 11.3 ms                                                      | 10.5 ms: 1.08x faster                                                  |
| genshi_text     | 21.5 ms                                                      | 20.7 ms: 1.04x faster                                                  |
| django_template | 34.1 ms                                                      | 35.1 ms: 1.03x slower                                                  |
| genshi_xml      | 48.8 ms                                                      | 51.8 ms: 1.06x slower                                                  |
| Geometric mean  | (ref)                                                        | 1.01x faster                                                           |

All benchmarks:
===============

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20260312-vultr-x86_64-python-0adc7289c3ab097b5608-3.15.0a7+-0adc728 |
|----------------------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| richards                   | 45.2 ms                                                      | 17.5 ms: 2.58x faster                                                  |
| richards_super             | 51.6 ms                                                      | 21.4 ms: 2.41x faster                                                  |
| mdp                        | 2.36 sec                                                     | 1.21 sec: 1.94x faster                                                 |
| deepcopy_memo              | 39.1 us                                                      | 22.4 us: 1.74x faster                                                  |
| async_tree_io_tg           | 913 ms                                                       | 564 ms: 1.62x faster                                                   |
| deepcopy                   | 355 us                                                       | 225 us: 1.58x faster                                                   |
| scimark_sor                | 134 ms                                                       | 86.9 ms: 1.55x faster                                                  |
| async_tree_memoization     | 461 ms                                                       | 299 ms: 1.54x faster                                                   |
| async_tree_io              | 876 ms                                                       | 582 ms: 1.51x faster                                                   |
| async_tree_none            | 354 ms                                                       | 240 ms: 1.47x faster                                                   |
| scimark_lu                 | 113 ms                                                       | 78.8 ms: 1.43x faster                                                  |
| async_tree_none_tg         | 336 ms                                                       | 237 ms: 1.42x faster                                                   |
| spectral_norm              | 111 ms                                                       | 78.6 ms: 1.41x faster                                                  |
| async_tree_memoization_tg  | 414 ms                                                       | 297 ms: 1.40x faster                                                   |
| go                         | 141 ms                                                       | 101 ms: 1.39x faster                                                   |
| async_tree_cpu_io_mixed    | 666 ms                                                       | 493 ms: 1.35x faster                                                   |
| scimark_fft                | 349 ms                                                       | 259 ms: 1.35x faster                                                   |
| float                      | 77.5 ms                                                      | 59.2 ms: 1.31x faster                                                  |
| tomli_loads                | 2.01 sec                                                     | 1.54 sec: 1.30x faster                                                 |
| logging_silent             | 103 ns                                                       | 80.6 ns: 1.27x faster                                                  |
| deepcopy_reduce            | 3.11 us                                                      | 2.45 us: 1.27x faster                                                  |
| async_tree_cpu_io_mixed_tg | 638 ms                                                       | 504 ms: 1.26x faster                                                   |
| pyflate                    | 449 ms                                                       | 358 ms: 1.25x faster                                                   |
| scimark_monte_carlo        | 65.4 ms                                                      | 52.2 ms: 1.25x faster                                                  |
| deltablue                  | 3.12 ms                                                      | 2.70 ms: 1.16x faster                                                  |
| json_dumps                 | 10.5 ms                                                      | 9.16 ms: 1.15x faster                                                  |
| xml_etree_iterparse        | 94.9 ms                                                      | 82.6 ms: 1.15x faster                                                  |
| pprint_safe_repr           | 738 ms                                                       | 645 ms: 1.14x faster                                                   |
| pprint_pformat             | 1.50 sec                                                     | 1.31 sec: 1.14x faster                                                 |
| bpe_tokeniser              | 4.45 sec                                                     | 3.92 sec: 1.13x faster                                                 |
| scimark_sparse_mat_mult    | 4.71 ms                                                      | 4.22 ms: 1.11x faster                                                  |
| nbody                      | 85.1 ms                                                      | 76.3 ms: 1.11x faster                                                  |
| comprehensions             | 16.5 us                                                      | 14.9 us: 1.11x faster                                                  |
| xml_etree_generate         | 85.4 ms                                                      | 77.5 ms: 1.10x faster                                                  |
| xml_etree_process          | 59.3 ms                                                      | 53.9 ms: 1.10x faster                                                  |
| pylint                     | 317 ms                                                       | 290 ms: 1.09x faster                                                   |
| pathlib                    | 19.2 ms                                                      | 17.6 ms: 1.09x faster                                                  |
| fannkuch                   | 370 ms                                                       | 341 ms: 1.09x faster                                                   |
| pidigits                   | 217 ms                                                       | 200 ms: 1.08x faster                                                   |
| nqueens                    | 78.6 ms                                                      | 72.6 ms: 1.08x faster                                                  |
| dulwich_log                | 74.8 ms                                                      | 69.2 ms: 1.08x faster                                                  |
| mako                       | 11.3 ms                                                      | 10.5 ms: 1.08x faster                                                  |
| chaos                      | 57.3 ms                                                      | 53.4 ms: 1.07x faster                                                  |
| unpickle_pure_python       | 210 us                                                       | 197 us: 1.07x faster                                                   |
| regex_effbot               | 3.08 ms                                                      | 2.89 ms: 1.07x faster                                                  |
| regex_compile              | 132 ms                                                       | 124 ms: 1.06x faster                                                   |
| xml_etree_parse            | 136 ms                                                       | 129 ms: 1.05x faster                                                   |
| hexiom                     | 5.99 ms                                                      | 5.69 ms: 1.05x faster                                                  |
| html5lib                   | 67.0 ms                                                      | 63.6 ms: 1.05x faster                                                  |
| logging_simple             | 6.16 us                                                      | 5.85 us: 1.05x faster                                                  |
| crypto_pyaes               | 67.9 ms                                                      | 65.0 ms: 1.04x faster                                                  |
| genshi_text                | 21.5 ms                                                      | 20.7 ms: 1.04x faster                                                  |
| 2to3                       | 260 ms                                                       | 250 ms: 1.04x faster                                                   |
| logging_format             | 6.84 us                                                      | 6.58 us: 1.04x faster                                                  |
| sqlite_synth               | 2.21 us                                                      | 2.16 us: 1.02x faster                                                  |
| meteor_contest             | 102 ms                                                       | 99.5 ms: 1.02x faster                                                  |
| coroutines                 | 23.6 ms                                                      | 23.1 ms: 1.02x faster                                                  |
| json                       | 4.93 ms                                                      | 4.85 ms: 1.02x faster                                                  |
| sympy_integrate            | 19.8 ms                                                      | 19.6 ms: 1.01x faster                                                  |
| pycparser                  | 1.12 sec                                                     | 1.10 sec: 1.01x faster                                                 |
| coverage                   | 83.0 ms                                                      | 82.0 ms: 1.01x faster                                                  |
| generators                 | 28.8 ms                                                      | 28.5 ms: 1.01x faster                                                  |
| python_startup_no_site     | 7.39 ms                                                      | 7.46 ms: 1.01x slower                                                  |
| typing_runtime_protocols   | 155 us                                                       | 157 us: 1.01x slower                                                   |
| json_loads                 | 27.0 us                                                      | 27.5 us: 1.02x slower                                                  |
| regex_v8                   | 22.7 ms                                                      | 23.2 ms: 1.02x slower                                                  |
| thrift                     | 778 us                                                       | 797 us: 1.02x slower                                                   |
| django_template            | 34.1 ms                                                      | 35.1 ms: 1.03x slower                                                  |
| asyncio_websockets         | 520 ms                                                       | 545 ms: 1.05x slower                                                   |
| sympy_sum                  | 156 ms                                                       | 163 ms: 1.05x slower                                                   |
| sympy_str                  | 275 ms                                                       | 288 ms: 1.05x slower                                                   |
| regex_dna                  | 180 ms                                                       | 189 ms: 1.05x slower                                                   |
| genshi_xml                 | 48.8 ms                                                      | 51.8 ms: 1.06x slower                                                  |
| sympy_expand               | 457 ms                                                       | 486 ms: 1.06x slower                                                   |
| async_generators           | 377 ms                                                       | 403 ms: 1.07x slower                                                   |
| pickle_pure_python         | 294 us                                                       | 315 us: 1.07x slower                                                   |
| raytrace                   | 253 ms                                                       | 280 ms: 1.11x slower                                                   |
| python_startup             | 11.0 ms                                                      | 12.6 ms: 1.15x slower                                                  |
| gc_traversal               | 3.14 ms                                                      | 4.15 ms: 1.32x slower                                                  |
| create_gc_cycles           | 1.34 ms                                                      | 1.86 ms: 1.39x slower                                                  |
| bench_thread_pool          | 919 us                                                       | 1.32 ms: 1.43x slower                                                  |
| bench_mp_pool              | 11.0 ms                                                      | 209 ms: 18.98x slower                                                  |
| telco                      | 7.82 ms                                                      | 160 ms: 20.45x slower                                                  |
| Geometric mean             | (ref)                                                        | 1.06x faster                                                           |

Benchmark hidden because not significant (1): docutils
Ignored benchmarks (18) of results/bm-20240906-3.13.0rc2-ec61006/bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006.json: aiohttp, asyncio_tcp, asyncio_tcp_ssl, chameleon, dask, flaskblogging, gunicorn, pickle, pickle_dict, pickle_list, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http, unpack_sequence, unpickle, unpickle_list
Ignored benchmarks (10) of results/bm-20260312-3.15.0a7+-0adc728-JIT/bm-20260312-vultr-x86_64-python-0adc7289c3ab097b5608-3.15.0a7+-0adc728.json: connected_components, k_core, many_optionals, shortest_path, sphinx, sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile, subparsers

- Geometric mean (including insignificant results): 1.098x faster

# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.07x
- 95% likely to have a speedup of 1.06x
- 99% likely to have a speedup of 1.04x

# Memory
- memory change: 1.18x