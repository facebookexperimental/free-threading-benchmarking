# Results vs. 3.12.6

- fork: python
- ref: 012c498035a9adfe9fd2
- machine: linux-x86_64
- commit hash: 012c498
- commit date: 2026-01-24
- overall geometric mean: 1.139x faster
- HPT reliability: 100.00%
- HPT 99th percentile: 1.07x faster
- Memory change: 1.19x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260124-vultr-x86_64-python-012c498035a9adfe9fd2-3.15.0a5+-012c498 |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| 2to3           | 264 ms                                                 | 250 ms: 1.06x faster                                                   |
| docutils       | 2.64 sec                                               | 2.65 sec: 1.01x slower                                                 |
| html5lib       | 63.6 ms                                                | 62.0 ms: 1.03x faster                                                  |
| Geometric mean | (ref)                                                  | 1.03x faster                                                           |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260124-vultr-x86_64-python-012c498035a9adfe9fd2-3.15.0a5+-012c498 |
|----------------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| async_tree_io              | 1.08 sec                                               | 533 ms: 2.03x faster                                                   |
| async_tree_io_tg           | 1.11 sec                                               | 551 ms: 2.01x faster                                                   |
| async_tree_none            | 464 ms                                                 | 233 ms: 1.99x faster                                                   |
| async_tree_memoization_tg  | 560 ms                                                 | 286 ms: 1.96x faster                                                   |
| async_tree_memoization     | 555 ms                                                 | 291 ms: 1.91x faster                                                   |
| async_tree_none_tg         | 446 ms                                                 | 239 ms: 1.87x faster                                                   |
| async_tree_cpu_io_mixed    | 715 ms                                                 | 471 ms: 1.52x faster                                                   |
| async_tree_cpu_io_mixed_tg | 723 ms                                                 | 481 ms: 1.50x faster                                                   |
| async_generators           | 384 ms                                                 | 359 ms: 1.07x faster                                                   |
| coroutines                 | 23.9 ms                                                | 23.5 ms: 1.02x faster                                                  |
| asyncio_websockets         | 517 ms                                                 | 544 ms: 1.05x slower                                                   |
| Geometric mean             | (ref)                                                  | 1.56x faster                                                           |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260124-vultr-x86_64-python-012c498035a9adfe9fd2-3.15.0a5+-012c498 |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| float          | 80.8 ms                                                | 57.7 ms: 1.40x faster                                                  |
| nbody          | 89.3 ms                                                | 74.3 ms: 1.20x faster                                                  |
| pidigits       | 184 ms                                                 | 192 ms: 1.04x slower                                                   |
| Geometric mean | (ref)                                                  | 1.17x faster                                                           |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260124-vultr-x86_64-python-012c498035a9adfe9fd2-3.15.0a5+-012c498 |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| regex_compile  | 142 ms                                                 | 121 ms: 1.18x faster                                                   |
| regex_effbot   | 3.17 ms                                                | 2.76 ms: 1.15x faster                                                  |
| regex_v8       | 20.6 ms                                                | 21.0 ms: 1.02x slower                                                  |
| regex_dna      | 168 ms                                                 | 177 ms: 1.06x slower                                                   |
| Geometric mean | (ref)                                                  | 1.06x faster                                                           |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260124-vultr-x86_64-python-012c498035a9adfe9fd2-3.15.0a5+-012c498 |
|----------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| tomli_loads          | 2.11 sec                                               | 1.64 sec: 1.28x faster                                                 |
| unpickle_pure_python | 221 us                                                 | 175 us: 1.26x faster                                                   |
| xml_etree_iterparse  | 96.7 ms                                                | 84.5 ms: 1.14x faster                                                  |
| json_dumps           | 10.4 ms                                                | 9.20 ms: 1.13x faster                                                  |
| pickle_pure_python   | 308 us                                                 | 276 us: 1.12x faster                                                   |
| xml_etree_generate   | 85.2 ms                                                | 77.3 ms: 1.10x faster                                                  |
| xml_etree_process    | 59.0 ms                                                | 54.2 ms: 1.09x faster                                                  |
| xml_etree_parse      | 139 ms                                                 | 131 ms: 1.06x faster                                                   |
| json_loads           | 26.5 us                                                | 28.3 us: 1.07x slower                                                  |
| Geometric mean       | (ref)                                                  | 1.12x faster                                                           |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260124-vultr-x86_64-python-012c498035a9adfe9fd2-3.15.0a5+-012c498 |
|------------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| python_startup_no_site | 7.16 ms                                                | 7.38 ms: 1.03x slower                                                  |
| python_startup         | 9.93 ms                                                | 12.6 ms: 1.27x slower                                                  |
| Geometric mean         | (ref)                                                  | 1.14x slower                                                           |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260124-vultr-x86_64-python-012c498035a9adfe9fd2-3.15.0a5+-012c498 |
|-----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| genshi_text     | 22.8 ms                                                | 20.5 ms: 1.11x faster                                                  |
| mako            | 11.0 ms                                                | 10.6 ms: 1.04x faster                                                  |
| django_template | 34.7 ms                                                | 35.5 ms: 1.02x slower                                                  |
| genshi_xml      | 50.2 ms                                                | 51.4 ms: 1.02x slower                                                  |
| Geometric mean  | (ref)                                                  | 1.03x faster                                                           |

All benchmarks:
===============

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260124-vultr-x86_64-python-012c498035a9adfe9fd2-3.15.0a5+-012c498 |
|----------------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| richards                   | 45.9 ms                                                | 17.8 ms: 2.58x faster                                                  |
| richards_super             | 51.9 ms                                                | 21.5 ms: 2.41x faster                                                  |
| async_tree_io              | 1.08 sec                                               | 533 ms: 2.03x faster                                                   |
| async_tree_io_tg           | 1.11 sec                                               | 551 ms: 2.01x faster                                                   |
| async_tree_none            | 464 ms                                                 | 233 ms: 1.99x faster                                                   |
| async_tree_memoization_tg  | 560 ms                                                 | 286 ms: 1.96x faster                                                   |
| async_tree_memoization     | 555 ms                                                 | 291 ms: 1.91x faster                                                   |
| async_tree_none_tg         | 446 ms                                                 | 239 ms: 1.87x faster                                                   |
| deepcopy_memo              | 40.3 us                                                | 22.4 us: 1.80x faster                                                  |
| mdp                        | 2.42 sec                                               | 1.35 sec: 1.79x faster                                                 |
| deepcopy                   | 352 us                                                 | 221 us: 1.59x faster                                                   |
| async_tree_cpu_io_mixed    | 715 ms                                                 | 471 ms: 1.52x faster                                                   |
| async_tree_cpu_io_mixed_tg | 723 ms                                                 | 481 ms: 1.50x faster                                                   |
| scimark_sor                | 130 ms                                                 | 86.9 ms: 1.49x faster                                                  |
| scimark_lu                 | 114 ms                                                 | 77.8 ms: 1.47x faster                                                  |
| float                      | 80.8 ms                                                | 57.7 ms: 1.40x faster                                                  |
| go                         | 139 ms                                                 | 101 ms: 1.38x faster                                                   |
| spectral_norm              | 110 ms                                                 | 81.4 ms: 1.35x faster                                                  |
| scimark_fft                | 342 ms                                                 | 257 ms: 1.33x faster                                                   |
| scimark_monte_carlo        | 68.4 ms                                                | 51.7 ms: 1.32x faster                                                  |
| logging_silent             | 109 ns                                                 | 83.2 ns: 1.31x faster                                                  |
| deepcopy_reduce            | 3.08 us                                                | 2.39 us: 1.29x faster                                                  |
| tomli_loads                | 2.11 sec                                               | 1.64 sec: 1.28x faster                                                 |
| deltablue                  | 3.45 ms                                                | 2.73 ms: 1.26x faster                                                  |
| pathlib                    | 21.5 ms                                                | 17.0 ms: 1.26x faster                                                  |
| unpickle_pure_python       | 221 us                                                 | 175 us: 1.26x faster                                                   |
| comprehensions             | 19.8 us                                                | 15.8 us: 1.25x faster                                                  |
| pyflate                    | 448 ms                                                 | 368 ms: 1.22x faster                                                   |
| nbody                      | 89.3 ms                                                | 74.3 ms: 1.20x faster                                                  |
| chaos                      | 62.8 ms                                                | 53.0 ms: 1.19x faster                                                  |
| bpe_tokeniser              | 4.74 sec                                               | 4.02 sec: 1.18x faster                                                 |
| regex_compile              | 142 ms                                                 | 121 ms: 1.18x faster                                                   |
| pprint_pformat             | 1.52 sec                                               | 1.30 sec: 1.17x faster                                                 |
| pprint_safe_repr           | 743 ms                                                 | 636 ms: 1.17x faster                                                   |
| raytrace                   | 299 ms                                                 | 260 ms: 1.15x faster                                                   |
| regex_effbot               | 3.17 ms                                                | 2.76 ms: 1.15x faster                                                  |
| xml_etree_iterparse        | 96.7 ms                                                | 84.5 ms: 1.14x faster                                                  |
| logging_simple             | 6.63 us                                                | 5.80 us: 1.14x faster                                                  |
| crypto_pyaes               | 76.6 ms                                                | 67.2 ms: 1.14x faster                                                  |
| json_dumps                 | 10.4 ms                                                | 9.20 ms: 1.13x faster                                                  |
| logging_format             | 7.35 us                                                | 6.57 us: 1.12x faster                                                  |
| pickle_pure_python         | 308 us                                                 | 276 us: 1.12x faster                                                   |
| genshi_text                | 22.8 ms                                                | 20.5 ms: 1.11x faster                                                  |
| xml_etree_generate         | 85.2 ms                                                | 77.3 ms: 1.10x faster                                                  |
| dulwich_log                | 78.9 ms                                                | 71.8 ms: 1.10x faster                                                  |
| generators                 | 32.2 ms                                                | 29.5 ms: 1.09x faster                                                  |
| xml_etree_process          | 59.0 ms                                                | 54.2 ms: 1.09x faster                                                  |
| fannkuch                   | 372 ms                                                 | 346 ms: 1.08x faster                                                   |
| async_generators           | 384 ms                                                 | 359 ms: 1.07x faster                                                   |
| meteor_contest             | 104 ms                                                 | 97.4 ms: 1.06x faster                                                  |
| scimark_sparse_mat_mult    | 4.39 ms                                                | 4.15 ms: 1.06x faster                                                  |
| xml_etree_parse            | 139 ms                                                 | 131 ms: 1.06x faster                                                   |
| 2to3                       | 264 ms                                                 | 250 ms: 1.06x faster                                                   |
| pycparser                  | 1.17 sec                                               | 1.11 sec: 1.05x faster                                                 |
| mako                       | 11.0 ms                                                | 10.6 ms: 1.04x faster                                                  |
| thrift                     | 791 us                                                 | 760 us: 1.04x faster                                                   |
| pylint                     | 319 ms                                                 | 307 ms: 1.04x faster                                                   |
| json                       | 5.02 ms                                                | 4.89 ms: 1.03x faster                                                  |
| html5lib                   | 63.6 ms                                                | 62.0 ms: 1.03x faster                                                  |
| sympy_integrate            | 20.5 ms                                                | 20.1 ms: 1.02x faster                                                  |
| coroutines                 | 23.9 ms                                                | 23.5 ms: 1.02x faster                                                  |
| hexiom                     | 6.17 ms                                                | 6.06 ms: 1.02x faster                                                  |
| typing_runtime_protocols   | 163 us                                                 | 162 us: 1.01x faster                                                   |
| docutils                   | 2.64 sec                                               | 2.65 sec: 1.01x slower                                                 |
| sympy_sum                  | 166 ms                                                 | 168 ms: 1.01x slower                                                   |
| regex_v8                   | 20.6 ms                                                | 21.0 ms: 1.02x slower                                                  |
| django_template            | 34.7 ms                                                | 35.5 ms: 1.02x slower                                                  |
| genshi_xml                 | 50.2 ms                                                | 51.4 ms: 1.02x slower                                                  |
| python_startup_no_site     | 7.16 ms                                                | 7.38 ms: 1.03x slower                                                  |
| sympy_str                  | 292 ms                                                 | 303 ms: 1.04x slower                                                   |
| pidigits                   | 184 ms                                                 | 192 ms: 1.04x slower                                                   |
| asyncio_websockets         | 517 ms                                                 | 544 ms: 1.05x slower                                                   |
| regex_dna                  | 168 ms                                                 | 177 ms: 1.06x slower                                                   |
| json_loads                 | 26.5 us                                                | 28.3 us: 1.07x slower                                                  |
| sympy_expand               | 468 ms                                                 | 502 ms: 1.07x slower                                                   |
| coverage                   | 71.4 ms                                                | 81.3 ms: 1.14x slower                                                  |
| gc_traversal               | 3.46 ms                                                | 4.28 ms: 1.24x slower                                                  |
| python_startup             | 9.93 ms                                                | 12.6 ms: 1.27x slower                                                  |
| bench_thread_pool          | 941 us                                                 | 1.31 ms: 1.39x slower                                                  |
| create_gc_cycles           | 1.09 ms                                                | 1.95 ms: 1.79x slower                                                  |
| bench_mp_pool              | 10.8 ms                                                | 226 ms: 20.96x slower                                                  |
| telco                      | 6.53 ms                                                | 166 ms: 25.44x slower                                                  |
| Geometric mean             | (ref)                                                  | 1.09x faster                                                           |

Benchmark hidden because not significant (2): nqueens, sqlite_synth
Ignored benchmarks (21) of results/bm-20240906-3.12.6-a4a2d2b/bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b.json: aiohttp, asyncio_tcp, asyncio_tcp_ssl, chameleon, dask, flaskblogging, gunicorn, mypy2, pickle, pickle_dict, pickle_list, sqlalchemy_declarative, sqlalchemy_imperative, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http, unpack_sequence, unpickle, unpickle_list
Ignored benchmarks (10) of results/bm-20260124-3.15.0a5+-012c498-JIT/bm-20260124-vultr-x86_64-python-012c498035a9adfe9fd2-3.15.0a5+-012c498.json: connected_components, k_core, many_optionals, shortest_path, sphinx, sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile, subparsers

- Geometric mean (including insignificant results): 1.139x faster

# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.10x
- 95% likely to have a speedup of 1.08x
- 99% likely to have a speedup of 1.07x

# Memory
- memory change: 1.19x