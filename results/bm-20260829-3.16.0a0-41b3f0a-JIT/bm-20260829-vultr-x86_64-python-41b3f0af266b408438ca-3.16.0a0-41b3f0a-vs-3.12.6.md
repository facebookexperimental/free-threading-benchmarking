# Results vs. 3.12.6

- fork: python
- ref: 41b3f0af266b408438ca
- machine: linux-x86_64
- commit hash: 41b3f0a
- commit date: 2026-08-29
- overall geometric mean: 1.170x faster
- HPT reliability: 100.00%
- HPT 99th percentile: 1.09x faster
- Memory change: 1.20x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260829-vultr-x86_64-python-41b3f0af266b408438ca-3.16.0a0-41b3f0a |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| 2to3           | 264 ms                                                 | 258 ms: 1.02x faster                                                  |
| docutils       | 2.64 sec                                               | 2.46 sec: 1.07x faster                                                |
| html5lib       | 63.6 ms                                                | 59.0 ms: 1.08x faster                                                 |
| Geometric mean | (ref)                                                  | 1.06x faster                                                          |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260829-vultr-x86_64-python-41b3f0af266b408438ca-3.16.0a0-41b3f0a |
|----------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| async_tree_none            | 464 ms                                                 | 271 ms: 1.71x faster                                                  |
| async_tree_memoization_tg  | 560 ms                                                 | 340 ms: 1.64x faster                                                  |
| async_tree_io              | 1.08 sec                                               | 678 ms: 1.60x faster                                                  |
| async_tree_none_tg         | 446 ms                                                 | 280 ms: 1.59x faster                                                  |
| async_tree_memoization     | 555 ms                                                 | 353 ms: 1.57x faster                                                  |
| async_tree_io_tg           | 1.11 sec                                               | 724 ms: 1.53x faster                                                  |
| async_tree_cpu_io_mixed    | 715 ms                                                 | 521 ms: 1.37x faster                                                  |
| async_tree_cpu_io_mixed_tg | 723 ms                                                 | 530 ms: 1.36x faster                                                  |
| asyncio_tcp                | 519 ms                                                 | 447 ms: 1.16x faster                                                  |
| async_generators           | 384 ms                                                 | 359 ms: 1.07x faster                                                  |
| asyncio_tcp_ssl            | 1.51 sec                                               | 1.46 sec: 1.03x faster                                                |
| coroutines                 | 23.9 ms                                                | 23.5 ms: 1.02x faster                                                 |
| asyncio_websockets         | 517 ms                                                 | 544 ms: 1.05x slower                                                  |
| Geometric mean             | (ref)                                                  | 1.33x faster                                                          |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260829-vultr-x86_64-python-41b3f0af266b408438ca-3.16.0a0-41b3f0a |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| nbody          | 89.3 ms                                                | 58.7 ms: 1.52x faster                                                 |
| float          | 80.8 ms                                                | 54.5 ms: 1.48x faster                                                 |
| pidigits       | 184 ms                                                 | 187 ms: 1.02x slower                                                  |
| Geometric mean | (ref)                                                  | 1.30x faster                                                          |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260829-vultr-x86_64-python-41b3f0af266b408438ca-3.16.0a0-41b3f0a |
|----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| regex_effbot   | 3.17 ms                                                | 2.62 ms: 1.21x faster                                                 |
| regex_dna      | 168 ms                                                 | 165 ms: 1.02x faster                                                  |
| regex_compile  | 142 ms                                                 | 142 ms: 1.01x faster                                                  |
| regex_v8       | 20.6 ms                                                | 21.2 ms: 1.03x slower                                                 |
| Geometric mean | (ref)                                                  | 1.05x faster                                                          |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260829-vultr-x86_64-python-41b3f0af266b408438ca-3.16.0a0-41b3f0a |
|----------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| unpickle_pure_python | 221 us                                                 | 179 us: 1.23x faster                                                  |
| pickle_pure_python   | 308 us                                                 | 261 us: 1.18x faster                                                  |
| json_dumps           | 10.4 ms                                                | 8.82 ms: 1.17x faster                                                 |
| tomli_loads          | 2.11 sec                                               | 1.81 sec: 1.16x faster                                                |
| xml_etree_iterparse  | 96.7 ms                                                | 88.8 ms: 1.09x faster                                                 |
| json_loads           | 26.5 us                                                | 24.8 us: 1.07x faster                                                 |
| xml_etree_process    | 59.0 ms                                                | 58.2 ms: 1.01x faster                                                 |
| xml_etree_parse      | 139 ms                                                 | 142 ms: 1.02x slower                                                  |
| unpickle             | 14.1 us                                                | 15.1 us: 1.07x slower                                                 |
| pickle_dict          | 31.8 us                                                | 34.4 us: 1.08x slower                                                 |
| unpickle_list        | 4.67 us                                                | 5.26 us: 1.13x slower                                                 |
| pickle_list          | 4.77 us                                                | 5.67 us: 1.19x slower                                                 |
| pickle               | 10.9 us                                                | 13.4 us: 1.22x slower                                                 |
| Geometric mean       | (ref)                                                  | 1.01x faster                                                          |

Benchmark hidden because not significant (1): xml_etree_generate

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260829-vultr-x86_64-python-41b3f0af266b408438ca-3.16.0a0-41b3f0a |
|------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| python_startup_no_site | 7.16 ms                                                | 7.83 ms: 1.09x slower                                                 |
| python_startup         | 9.93 ms                                                | 13.1 ms: 1.32x slower                                                 |
| Geometric mean         | (ref)                                                  | 1.20x slower                                                          |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260829-vultr-x86_64-python-41b3f0af266b408438ca-3.16.0a0-41b3f0a |
|-----------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| mako            | 11.0 ms                                                | 10.1 ms: 1.09x faster                                                 |
| django_template | 34.7 ms                                                | 40.8 ms: 1.18x slower                                                 |
| Geometric mean  | (ref)                                                  | 1.04x slower                                                          |

All benchmarks:
===============

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260829-vultr-x86_64-python-41b3f0af266b408438ca-3.16.0a0-41b3f0a |
|----------------------------|:------------------------------------------------------:|:---------------------------------------------------------------------:|
| pylint                     | 319 ms                                                 | 113 ms: 2.81x faster                                                  |
| richards_super             | 51.9 ms                                                | 20.4 ms: 2.55x faster                                                 |
| richards                   | 45.9 ms                                                | 18.2 ms: 2.52x faster                                                 |
| mdp                        | 2.42 sec                                               | 1.23 sec: 1.96x faster                                                |
| deepcopy_memo              | 40.3 us                                                | 22.0 us: 1.84x faster                                                 |
| async_tree_none            | 464 ms                                                 | 271 ms: 1.71x faster                                                  |
| scimark_sor                | 130 ms                                                 | 77.8 ms: 1.67x faster                                                 |
| async_tree_memoization_tg  | 560 ms                                                 | 340 ms: 1.64x faster                                                  |
| async_tree_io              | 1.08 sec                                               | 678 ms: 1.60x faster                                                  |
| async_tree_none_tg         | 446 ms                                                 | 280 ms: 1.59x faster                                                  |
| async_tree_memoization     | 555 ms                                                 | 353 ms: 1.57x faster                                                  |
| spectral_norm              | 110 ms                                                 | 70.6 ms: 1.56x faster                                                 |
| async_tree_io_tg           | 1.11 sec                                               | 724 ms: 1.53x faster                                                  |
| nbody                      | 89.3 ms                                                | 58.7 ms: 1.52x faster                                                 |
| typing_runtime_protocols   | 163 us                                                 | 109 us: 1.50x faster                                                  |
| deepcopy                   | 352 us                                                 | 236 us: 1.49x faster                                                  |
| float                      | 80.8 ms                                                | 54.5 ms: 1.48x faster                                                 |
| go                         | 139 ms                                                 | 98.4 ms: 1.42x faster                                                 |
| async_tree_cpu_io_mixed    | 715 ms                                                 | 521 ms: 1.37x faster                                                  |
| scimark_fft                | 342 ms                                                 | 250 ms: 1.36x faster                                                  |
| async_tree_cpu_io_mixed_tg | 723 ms                                                 | 530 ms: 1.36x faster                                                  |
| scimark_lu                 | 114 ms                                                 | 83.8 ms: 1.36x faster                                                 |
| comprehensions             | 19.8 us                                                | 14.6 us: 1.36x faster                                                 |
| chaos                      | 62.8 ms                                                | 47.7 ms: 1.32x faster                                                 |
| scimark_monte_carlo        | 68.4 ms                                                | 52.9 ms: 1.29x faster                                                 |
| deltablue                  | 3.45 ms                                                | 2.71 ms: 1.27x faster                                                 |
| pyflate                    | 448 ms                                                 | 354 ms: 1.27x faster                                                  |
| nqueens                    | 80.1 ms                                                | 64.6 ms: 1.24x faster                                                 |
| unpickle_pure_python       | 221 us                                                 | 179 us: 1.23x faster                                                  |
| logging_simple             | 6.63 us                                                | 5.38 us: 1.23x faster                                                 |
| crypto_pyaes               | 76.6 ms                                                | 62.3 ms: 1.23x faster                                                 |
| pathlib                    | 21.5 ms                                                | 17.5 ms: 1.23x faster                                                 |
| raytrace                   | 299 ms                                                 | 245 ms: 1.22x faster                                                  |
| logging_format             | 7.35 us                                                | 6.01 us: 1.22x faster                                                 |
| deepcopy_reduce            | 3.08 us                                                | 2.53 us: 1.22x faster                                                 |
| regex_effbot               | 3.17 ms                                                | 2.62 ms: 1.21x faster                                                 |
| bpe_tokeniser              | 4.74 sec                                               | 3.98 sec: 1.19x faster                                                |
| pickle_pure_python         | 308 us                                                 | 261 us: 1.18x faster                                                  |
| pprint_pformat             | 1.52 sec                                               | 1.29 sec: 1.18x faster                                                |
| json_dumps                 | 10.4 ms                                                | 8.82 ms: 1.17x faster                                                 |
| pprint_safe_repr           | 743 ms                                                 | 638 ms: 1.16x faster                                                  |
| tomli_loads                | 2.11 sec                                               | 1.81 sec: 1.16x faster                                                |
| asyncio_tcp                | 519 ms                                                 | 447 ms: 1.16x faster                                                  |
| dulwich_log                | 78.9 ms                                                | 68.3 ms: 1.15x faster                                                 |
| fannkuch                   | 372 ms                                                 | 329 ms: 1.13x faster                                                  |
| scimark_sparse_mat_mult    | 4.39 ms                                                | 3.90 ms: 1.13x faster                                                 |
| logging_silent             | 109 ns                                                 | 96.9 ns: 1.12x faster                                                 |
| json                       | 5.02 ms                                                | 4.51 ms: 1.11x faster                                                 |
| hexiom                     | 6.17 ms                                                | 5.60 ms: 1.10x faster                                                 |
| mako                       | 11.0 ms                                                | 10.1 ms: 1.09x faster                                                 |
| xml_etree_iterparse        | 96.7 ms                                                | 88.8 ms: 1.09x faster                                                 |
| generators                 | 32.2 ms                                                | 29.7 ms: 1.09x faster                                                 |
| html5lib                   | 63.6 ms                                                | 59.0 ms: 1.08x faster                                                 |
| docutils                   | 2.64 sec                                               | 2.46 sec: 1.07x faster                                                |
| json_loads                 | 26.5 us                                                | 24.8 us: 1.07x faster                                                 |
| meteor_contest             | 104 ms                                                 | 96.7 ms: 1.07x faster                                                 |
| async_generators           | 384 ms                                                 | 359 ms: 1.07x faster                                                  |
| sympy_integrate            | 20.5 ms                                                | 19.8 ms: 1.04x faster                                                 |
| asyncio_tcp_ssl            | 1.51 sec                                               | 1.46 sec: 1.03x faster                                                |
| sqlite_synth               | 2.20 us                                                | 2.13 us: 1.03x faster                                                 |
| 2to3                       | 264 ms                                                 | 258 ms: 1.02x faster                                                  |
| coroutines                 | 23.9 ms                                                | 23.5 ms: 1.02x faster                                                 |
| regex_dna                  | 168 ms                                                 | 165 ms: 1.02x faster                                                  |
| thrift                     | 791 us                                                 | 779 us: 1.02x faster                                                  |
| xml_etree_process          | 59.0 ms                                                | 58.2 ms: 1.01x faster                                                 |
| regex_compile              | 142 ms                                                 | 142 ms: 1.01x faster                                                  |
| sympy_sum                  | 166 ms                                                 | 167 ms: 1.01x slower                                                  |
| pidigits                   | 184 ms                                                 | 187 ms: 1.02x slower                                                  |
| xml_etree_parse            | 139 ms                                                 | 142 ms: 1.02x slower                                                  |
| regex_v8                   | 20.6 ms                                                | 21.2 ms: 1.03x slower                                                 |
| asyncio_websockets         | 517 ms                                                 | 544 ms: 1.05x slower                                                  |
| unpickle                   | 14.1 us                                                | 15.1 us: 1.07x slower                                                 |
| pickle_dict                | 31.8 us                                                | 34.4 us: 1.08x slower                                                 |
| python_startup_no_site     | 7.16 ms                                                | 7.83 ms: 1.09x slower                                                 |
| unpickle_list              | 4.67 us                                                | 5.26 us: 1.13x slower                                                 |
| coverage                   | 71.4 ms                                                | 82.4 ms: 1.15x slower                                                 |
| gc_traversal               | 3.46 ms                                                | 4.02 ms: 1.16x slower                                                 |
| django_template            | 34.7 ms                                                | 40.8 ms: 1.18x slower                                                 |
| pickle_list                | 4.77 us                                                | 5.67 us: 1.19x slower                                                 |
| unpack_sequence            | 52.1 ns                                                | 63.4 ns: 1.22x slower                                                 |
| pickle                     | 10.9 us                                                | 13.4 us: 1.22x slower                                                 |
| python_startup             | 9.93 ms                                                | 13.1 ms: 1.32x slower                                                 |
| bench_thread_pool          | 941 us                                                 | 1.33 ms: 1.41x slower                                                 |
| create_gc_cycles           | 1.09 ms                                                | 1.67 ms: 1.53x slower                                                 |
| bench_mp_pool              | 10.8 ms                                                | 168 ms: 15.55x slower                                                 |
| telco                      | 6.53 ms                                                | 135 ms: 20.61x slower                                                 |
| Geometric mean             | (ref)                                                  | 1.10x faster                                                          |

Benchmark hidden because not significant (4): pycparser, sympy_str, sympy_expand, xml_etree_generate
Ignored benchmarks (15) of results/bm-20240906-3.12.6-a4a2d2b/bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b.json: aiohttp, chameleon, dask, flaskblogging, genshi_text, genshi_xml, gunicorn, mypy2, sqlalchemy_declarative, sqlalchemy_imperative, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http
Ignored benchmarks (10) of results/bm-20260829-3.16.0a0-41b3f0a-JIT/bm-20260829-vultr-x86_64-python-41b3f0af266b408438ca-3.16.0a0-41b3f0a.json: connected_components, k_core, many_optionals, shortest_path, sphinx, sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile, subparsers

- Geometric mean (including insignificant results): 1.170x faster

# HPT report

- Reliability score: 100.00% likely to be faster
- 90% likely to have a speedup of 1.14x
- 95% likely to have a speedup of 1.12x
- 99% likely to have a speedup of 1.09x

# Memory
- memory change: 1.20x