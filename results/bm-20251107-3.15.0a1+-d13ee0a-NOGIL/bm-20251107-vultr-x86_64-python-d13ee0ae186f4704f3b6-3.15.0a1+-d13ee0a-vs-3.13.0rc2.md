# Results vs. 3.13.0rc2

- fork: python
- ref: d13ee0ae186f4704f3b6
- machine: linux-x86_64
- commit hash: d13ee0a
- commit date: 2025-11-07
- overall geometric mean: 1.050x slower
- HPT reliability: 96.47%
- HPT 99th percentile: 1.00x slower
- Memory change: 1.40x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20251107-vultr-x86_64-python-d13ee0ae186f4704f3b6-3.15.0a1+-d13ee0a |
|----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| 2to3           | 260 ms                                                       | 280 ms: 1.08x slower                                                   |
| docutils       | 2.62 sec                                                     | 2.69 sec: 1.03x slower                                                 |
| Geometric mean | (ref)                                                        | 1.04x slower                                                           |

Benchmark hidden because not significant (1): html5lib

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20251107-vultr-x86_64-python-d13ee0ae186f4704f3b6-3.15.0a1+-d13ee0a |
|----------------------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| async_tree_io_tg           | 913 ms                                                       | 512 ms: 1.79x faster                                                   |
| async_tree_io              | 876 ms                                                       | 540 ms: 1.62x faster                                                   |
| async_tree_none_tg         | 336 ms                                                       | 224 ms: 1.50x faster                                                   |
| async_tree_memoization     | 461 ms                                                       | 309 ms: 1.49x faster                                                   |
| async_tree_memoization_tg  | 414 ms                                                       | 283 ms: 1.46x faster                                                   |
| async_tree_none            | 354 ms                                                       | 254 ms: 1.39x faster                                                   |
| async_tree_cpu_io_mixed_tg | 638 ms                                                       | 474 ms: 1.34x faster                                                   |
| async_tree_cpu_io_mixed    | 666 ms                                                       | 500 ms: 1.33x faster                                                   |
| asyncio_websockets         | 520 ms                                                       | 511 ms: 1.02x faster                                                   |
| async_generators           | 377 ms                                                       | 372 ms: 1.01x faster                                                   |
| Geometric mean             | (ref)                                                        | 1.34x faster                                                           |

Benchmark hidden because not significant (1): coroutines

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20251107-vultr-x86_64-python-d13ee0ae186f4704f3b6-3.15.0a1+-d13ee0a |
|----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| pidigits       | 217 ms                                                       | 188 ms: 1.15x faster                                                   |
| float          | 77.5 ms                                                      | 70.4 ms: 1.10x faster                                                  |
| nbody          | 85.1 ms                                                      | 128 ms: 1.50x slower                                                   |
| Geometric mean | (ref)                                                        | 1.06x slower                                                           |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20251107-vultr-x86_64-python-d13ee0ae186f4704f3b6-3.15.0a1+-d13ee0a |
|----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| regex_v8       | 22.7 ms                                                      | 21.2 ms: 1.07x faster                                                  |
| regex_effbot   | 3.08 ms                                                      | 2.91 ms: 1.06x faster                                                  |
| regex_dna      | 180 ms                                                       | 178 ms: 1.01x faster                                                   |
| regex_compile  | 132 ms                                                       | 143 ms: 1.08x slower                                                   |
| Geometric mean | (ref)                                                        | 1.02x faster                                                           |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20251107-vultr-x86_64-python-d13ee0ae186f4704f3b6-3.15.0a1+-d13ee0a |
|----------------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| xml_etree_iterparse  | 94.9 ms                                                      | 87.6 ms: 1.08x faster                                                  |
| xml_etree_parse      | 136 ms                                                       | 132 ms: 1.03x faster                                                   |
| tomli_loads          | 2.01 sec                                                     | 2.03 sec: 1.01x slower                                                 |
| json_dumps           | 10.5 ms                                                      | 11.1 ms: 1.05x slower                                                  |
| unpickle_pure_python | 210 us                                                       | 232 us: 1.10x slower                                                   |
| xml_etree_generate   | 85.4 ms                                                      | 96.0 ms: 1.12x slower                                                  |
| xml_etree_process    | 59.3 ms                                                      | 69.1 ms: 1.16x slower                                                  |
| pickle_pure_python   | 294 us                                                       | 343 us: 1.17x slower                                                   |
| json_loads           | 27.0 us                                                      | 31.5 us: 1.17x slower                                                  |
| Geometric mean       | (ref)                                                        | 1.07x slower                                                           |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20251107-vultr-x86_64-python-d13ee0ae186f4704f3b6-3.15.0a1+-d13ee0a |
|------------------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| python_startup_no_site | 7.39 ms                                                      | 9.58 ms: 1.30x slower                                                  |
| python_startup         | 11.0 ms                                                      | 15.7 ms: 1.43x slower                                                  |
| Geometric mean         | (ref)                                                        | 1.36x slower                                                           |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20251107-vultr-x86_64-python-d13ee0ae186f4704f3b6-3.15.0a1+-d13ee0a |
|-----------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| django_template | 34.1 ms                                                      | 40.3 ms: 1.18x slower                                                  |
| genshi_xml      | 48.8 ms                                                      | 58.8 ms: 1.21x slower                                                  |
| genshi_text     | 21.5 ms                                                      | 26.4 ms: 1.23x slower                                                  |
| mako            | 11.3 ms                                                      | 15.6 ms: 1.37x slower                                                  |
| Geometric mean  | (ref)                                                        | 1.24x slower                                                           |

All benchmarks:
===============

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006 | bm-20251107-vultr-x86_64-python-d13ee0ae186f4704f3b6-3.15.0a1+-d13ee0a |
|----------------------------|:------------------------------------------------------------:|:----------------------------------------------------------------------:|
| async_tree_io_tg           | 913 ms                                                       | 512 ms: 1.79x faster                                                   |
| mdp                        | 2.36 sec                                                     | 1.32 sec: 1.78x faster                                                 |
| gc_traversal               | 3.14 ms                                                      | 1.93 ms: 1.63x faster                                                  |
| async_tree_io              | 876 ms                                                       | 540 ms: 1.62x faster                                                   |
| async_tree_none_tg         | 336 ms                                                       | 224 ms: 1.50x faster                                                   |
| async_tree_memoization     | 461 ms                                                       | 309 ms: 1.49x faster                                                   |
| async_tree_memoization_tg  | 414 ms                                                       | 283 ms: 1.46x faster                                                   |
| async_tree_none            | 354 ms                                                       | 254 ms: 1.39x faster                                                   |
| async_tree_cpu_io_mixed_tg | 638 ms                                                       | 474 ms: 1.34x faster                                                   |
| async_tree_cpu_io_mixed    | 666 ms                                                       | 500 ms: 1.33x faster                                                   |
| deepcopy_memo              | 39.1 us                                                      | 30.7 us: 1.27x faster                                                  |
| deepcopy                   | 355 us                                                       | 280 us: 1.27x faster                                                   |
| pidigits                   | 217 ms                                                       | 188 ms: 1.15x faster                                                   |
| go                         | 141 ms                                                       | 122 ms: 1.15x faster                                                   |
| float                      | 77.5 ms                                                      | 70.4 ms: 1.10x faster                                                  |
| pathlib                    | 19.2 ms                                                      | 17.6 ms: 1.09x faster                                                  |
| scimark_sor                | 134 ms                                                       | 124 ms: 1.09x faster                                                   |
| xml_etree_iterparse        | 94.9 ms                                                      | 87.6 ms: 1.08x faster                                                  |
| regex_v8                   | 22.7 ms                                                      | 21.2 ms: 1.07x faster                                                  |
| sqlite_synth               | 2.21 us                                                      | 2.07 us: 1.07x faster                                                  |
| regex_effbot               | 3.08 ms                                                      | 2.91 ms: 1.06x faster                                                  |
| xml_etree_parse            | 136 ms                                                       | 132 ms: 1.03x faster                                                   |
| deepcopy_reduce            | 3.11 us                                                      | 3.04 us: 1.02x faster                                                  |
| dulwich_log                | 74.8 ms                                                      | 73.3 ms: 1.02x faster                                                  |
| asyncio_websockets         | 520 ms                                                       | 511 ms: 1.02x faster                                                   |
| async_generators           | 377 ms                                                       | 372 ms: 1.01x faster                                                   |
| regex_dna                  | 180 ms                                                       | 178 ms: 1.01x faster                                                   |
| bpe_tokeniser              | 4.45 sec                                                     | 4.43 sec: 1.00x faster                                                 |
| logging_silent             | 103 ns                                                       | 103 ns: 1.01x slower                                                   |
| tomli_loads                | 2.01 sec                                                     | 2.03 sec: 1.01x slower                                                 |
| pycparser                  | 1.12 sec                                                     | 1.14 sec: 1.02x slower                                                 |
| pyflate                    | 449 ms                                                       | 461 ms: 1.03x slower                                                   |
| docutils                   | 2.62 sec                                                     | 2.69 sec: 1.03x slower                                                 |
| generators                 | 28.8 ms                                                      | 29.9 ms: 1.04x slower                                                  |
| spectral_norm              | 111 ms                                                       | 116 ms: 1.04x slower                                                   |
| pprint_safe_repr           | 738 ms                                                       | 770 ms: 1.04x slower                                                   |
| json_dumps                 | 10.5 ms                                                      | 11.1 ms: 1.05x slower                                                  |
| pprint_pformat             | 1.50 sec                                                     | 1.60 sec: 1.07x slower                                                 |
| hexiom                     | 5.99 ms                                                      | 6.44 ms: 1.07x slower                                                  |
| 2to3                       | 260 ms                                                       | 280 ms: 1.08x slower                                                   |
| regex_compile              | 132 ms                                                       | 143 ms: 1.08x slower                                                   |
| json                       | 4.93 ms                                                      | 5.38 ms: 1.09x slower                                                  |
| comprehensions             | 16.5 us                                                      | 18.0 us: 1.09x slower                                                  |
| unpickle_pure_python       | 210 us                                                       | 232 us: 1.10x slower                                                   |
| sympy_integrate            | 19.8 ms                                                      | 21.9 ms: 1.10x slower                                                  |
| logging_simple             | 6.16 us                                                      | 6.81 us: 1.11x slower                                                  |
| nqueens                    | 78.6 ms                                                      | 87.9 ms: 1.12x slower                                                  |
| richards                   | 45.2 ms                                                      | 50.6 ms: 1.12x slower                                                  |
| logging_format             | 6.84 us                                                      | 7.67 us: 1.12x slower                                                  |
| thrift                     | 778 us                                                       | 874 us: 1.12x slower                                                   |
| xml_etree_generate         | 85.4 ms                                                      | 96.0 ms: 1.12x slower                                                  |
| create_gc_cycles           | 1.34 ms                                                      | 1.50 ms: 1.13x slower                                                  |
| chaos                      | 57.3 ms                                                      | 64.5 ms: 1.13x slower                                                  |
| richards_super             | 51.6 ms                                                      | 58.3 ms: 1.13x slower                                                  |
| sympy_str                  | 275 ms                                                       | 310 ms: 1.13x slower                                                   |
| sympy_expand               | 457 ms                                                       | 518 ms: 1.13x slower                                                   |
| sympy_sum                  | 156 ms                                                       | 178 ms: 1.14x slower                                                   |
| deltablue                  | 3.12 ms                                                      | 3.62 ms: 1.16x slower                                                  |
| scimark_sparse_mat_mult    | 4.71 ms                                                      | 5.47 ms: 1.16x slower                                                  |
| xml_etree_process          | 59.3 ms                                                      | 69.1 ms: 1.16x slower                                                  |
| meteor_contest             | 102 ms                                                       | 118 ms: 1.17x slower                                                   |
| pickle_pure_python         | 294 us                                                       | 343 us: 1.17x slower                                                   |
| scimark_lu                 | 113 ms                                                       | 131 ms: 1.17x slower                                                   |
| json_loads                 | 27.0 us                                                      | 31.5 us: 1.17x slower                                                  |
| django_template            | 34.1 ms                                                      | 40.3 ms: 1.18x slower                                                  |
| raytrace                   | 253 ms                                                       | 299 ms: 1.18x slower                                                   |
| genshi_xml                 | 48.8 ms                                                      | 58.8 ms: 1.21x slower                                                  |
| scimark_monte_carlo        | 65.4 ms                                                      | 78.9 ms: 1.21x slower                                                  |
| genshi_text                | 21.5 ms                                                      | 26.4 ms: 1.23x slower                                                  |
| typing_runtime_protocols   | 155 us                                                       | 190 us: 1.23x slower                                                   |
| fannkuch                   | 370 ms                                                       | 458 ms: 1.24x slower                                                   |
| crypto_pyaes               | 67.9 ms                                                      | 86.5 ms: 1.27x slower                                                  |
| python_startup_no_site     | 7.39 ms                                                      | 9.58 ms: 1.30x slower                                                  |
| coverage                   | 83.0 ms                                                      | 111 ms: 1.33x slower                                                   |
| mako                       | 11.3 ms                                                      | 15.6 ms: 1.37x slower                                                  |
| bench_mp_pool              | 11.0 ms                                                      | 15.1 ms: 1.37x slower                                                  |
| python_startup             | 11.0 ms                                                      | 15.7 ms: 1.43x slower                                                  |
| nbody                      | 85.1 ms                                                      | 128 ms: 1.50x slower                                                   |
| bench_thread_pool          | 919 us                                                       | 3.06 ms: 3.33x slower                                                  |
| telco                      | 7.82 ms                                                      | 174 ms: 22.24x slower                                                  |
| Geometric mean             | (ref)                                                        | 1.07x slower                                                           |

Benchmark hidden because not significant (4): pylint, coroutines, scimark_fft, html5lib
Ignored benchmarks (18) of results/bm-20240906-3.13.0rc2-ec61006/bm-20240906-vultr-x86_64-python-v3.13.0rc2-3.13.0rc2-ec61006.json: aiohttp, asyncio_tcp, asyncio_tcp_ssl, chameleon, dask, flaskblogging, gunicorn, pickle, pickle_dict, pickle_list, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http, unpack_sequence, unpickle, unpickle_list
Ignored benchmarks (10) of results/bm-20251107-3.15.0a1+-d13ee0a-NOGIL/bm-20251107-vultr-x86_64-python-d13ee0ae186f4704f3b6-3.15.0a1+-d13ee0a.json: connected_components, k_core, many_optionals, shortest_path, sphinx, sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile, subparsers

- Geometric mean (including insignificant results): 1.050x slower

# HPT report

- Reliability score: 96.47% likely to be slow
- 90% likely to have a slowdown of 1.00x
- 95% likely to have a slowdown of 1.00x
- 99% likely to have a slowdown of 1.00x

# Memory
- memory change: 1.40x