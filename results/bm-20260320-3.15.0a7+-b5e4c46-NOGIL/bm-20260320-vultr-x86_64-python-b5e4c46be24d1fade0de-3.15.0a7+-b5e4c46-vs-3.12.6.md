# Results vs. 3.12.6

- fork: python
- ref: b5e4c46be24d1fade0de
- machine: linux-x86_64
- commit hash: b5e4c46
- commit date: 2026-03-20
- overall geometric mean: 1.022x slower
- HPT reliability: 70.94%
- HPT 99th percentile: 1.00x slower
- Memory change: 1.42x

Benchmarks with tag 'apps':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260320-vultr-x86_64-python-b5e4c46be24d1fade0de-3.15.0a7+-b5e4c46 |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| 2to3           | 264 ms                                                 | 281 ms: 1.07x slower                                                   |
| docutils       | 2.64 sec                                               | 2.65 sec: 1.01x slower                                                 |
| html5lib       | 63.6 ms                                                | 66.4 ms: 1.04x slower                                                  |
| Geometric mean | (ref)                                                  | 1.04x slower                                                           |

Benchmarks with tag 'asyncio':
==============================

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260320-vultr-x86_64-python-b5e4c46be24d1fade0de-3.15.0a7+-b5e4c46 |
|----------------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| async_tree_io_tg           | 1.11 sec                                               | 607 ms: 1.83x faster                                                   |
| async_tree_memoization_tg  | 560 ms                                                 | 322 ms: 1.74x faster                                                   |
| async_tree_none_tg         | 446 ms                                                 | 260 ms: 1.72x faster                                                   |
| async_tree_io              | 1.08 sec                                               | 638 ms: 1.70x faster                                                   |
| async_tree_none            | 464 ms                                                 | 287 ms: 1.62x faster                                                   |
| async_tree_memoization     | 555 ms                                                 | 351 ms: 1.58x faster                                                   |
| async_tree_cpu_io_mixed_tg | 723 ms                                                 | 516 ms: 1.40x faster                                                   |
| async_tree_cpu_io_mixed    | 715 ms                                                 | 541 ms: 1.32x faster                                                   |
| async_generators           | 384 ms                                                 | 373 ms: 1.03x faster                                                   |
| asyncio_websockets         | 517 ms                                                 | 513 ms: 1.01x faster                                                   |
| coroutines                 | 23.9 ms                                                | 24.1 ms: 1.01x slower                                                  |
| Geometric mean             | (ref)                                                  | 1.41x faster                                                           |

Benchmarks with tag 'math':
===========================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260320-vultr-x86_64-python-b5e4c46be24d1fade0de-3.15.0a7+-b5e4c46 |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| float          | 80.8 ms                                                | 73.2 ms: 1.10x faster                                                  |
| pidigits       | 184 ms                                                 | 195 ms: 1.06x slower                                                   |
| nbody          | 89.3 ms                                                | 123 ms: 1.37x slower                                                   |
| Geometric mean | (ref)                                                  | 1.10x slower                                                           |

Benchmarks with tag 'regex':
============================

| Benchmark      | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260320-vultr-x86_64-python-b5e4c46be24d1fade0de-3.15.0a7+-b5e4c46 |
|----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| regex_effbot   | 3.17 ms                                                | 2.88 ms: 1.10x faster                                                  |
| regex_compile  | 142 ms                                                 | 141 ms: 1.01x faster                                                   |
| regex_v8       | 20.6 ms                                                | 21.3 ms: 1.03x slower                                                  |
| regex_dna      | 168 ms                                                 | 176 ms: 1.05x slower                                                   |
| Geometric mean | (ref)                                                  | 1.01x faster                                                           |

Benchmarks with tag 'serialize':
================================

| Benchmark            | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260320-vultr-x86_64-python-b5e4c46be24d1fade0de-3.15.0a7+-b5e4c46 |
|----------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| xml_etree_iterparse  | 96.7 ms                                                | 88.8 ms: 1.09x faster                                                  |
| tomli_loads          | 2.11 sec                                               | 1.94 sec: 1.09x faster                                                 |
| xml_etree_parse      | 139 ms                                                 | 134 ms: 1.04x faster                                                   |
| json_dumps           | 10.4 ms                                                | 10.8 ms: 1.04x slower                                                  |
| unpickle_pure_python | 221 us                                                 | 237 us: 1.07x slower                                                   |
| pickle_pure_python   | 308 us                                                 | 332 us: 1.08x slower                                                   |
| xml_etree_generate   | 85.2 ms                                                | 95.2 ms: 1.12x slower                                                  |
| xml_etree_process    | 59.0 ms                                                | 67.8 ms: 1.15x slower                                                  |
| json_loads           | 26.5 us                                                | 30.7 us: 1.16x slower                                                  |
| Geometric mean       | (ref)                                                  | 1.04x slower                                                           |

Benchmarks with tag 'startup':
==============================

| Benchmark              | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260320-vultr-x86_64-python-b5e4c46be24d1fade0de-3.15.0a7+-b5e4c46 |
|------------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| python_startup_no_site | 7.16 ms                                                | 9.76 ms: 1.36x slower                                                  |
| python_startup         | 9.93 ms                                                | 15.9 ms: 1.60x slower                                                  |
| Geometric mean         | (ref)                                                  | 1.48x slower                                                           |

Benchmarks with tag 'template':
===============================

| Benchmark       | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260320-vultr-x86_64-python-b5e4c46be24d1fade0de-3.15.0a7+-b5e4c46 |
|-----------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| genshi_xml      | 50.2 ms                                                | 53.9 ms: 1.08x slower                                                  |
| genshi_text     | 22.8 ms                                                | 25.4 ms: 1.11x slower                                                  |
| django_template | 34.7 ms                                                | 39.6 ms: 1.14x slower                                                  |
| mako            | 11.0 ms                                                | 15.6 ms: 1.42x slower                                                  |
| Geometric mean  | (ref)                                                  | 1.18x slower                                                           |

All benchmarks:
===============

| Benchmark                  | bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b | bm-20260320-vultr-x86_64-python-b5e4c46be24d1fade0de-3.15.0a7+-b5e4c46 |
|----------------------------|:------------------------------------------------------:|:----------------------------------------------------------------------:|
| mdp                        | 2.42 sec                                               | 1.30 sec: 1.86x faster                                                 |
| async_tree_io_tg           | 1.11 sec                                               | 607 ms: 1.83x faster                                                   |
| gc_traversal               | 3.46 ms                                                | 1.92 ms: 1.80x faster                                                  |
| async_tree_memoization_tg  | 560 ms                                                 | 322 ms: 1.74x faster                                                   |
| async_tree_none_tg         | 446 ms                                                 | 260 ms: 1.72x faster                                                   |
| async_tree_io              | 1.08 sec                                               | 638 ms: 1.70x faster                                                   |
| async_tree_none            | 464 ms                                                 | 287 ms: 1.62x faster                                                   |
| async_tree_memoization     | 555 ms                                                 | 351 ms: 1.58x faster                                                   |
| bench_mp_pool              | 10.8 ms                                                | 7.04 ms: 1.53x faster                                                  |
| async_tree_cpu_io_mixed_tg | 723 ms                                                 | 516 ms: 1.40x faster                                                   |
| deepcopy                   | 352 us                                                 | 265 us: 1.33x faster                                                   |
| async_tree_cpu_io_mixed    | 715 ms                                                 | 541 ms: 1.32x faster                                                   |
| deepcopy_memo              | 40.3 us                                                | 31.0 us: 1.30x faster                                                  |
| pathlib                    | 21.5 ms                                                | 17.6 ms: 1.22x faster                                                  |
| go                         | 139 ms                                                 | 120 ms: 1.16x faster                                                   |
| sqlite_synth               | 2.20 us                                                | 1.90 us: 1.16x faster                                                  |
| dulwich_log                | 78.9 ms                                                | 69.5 ms: 1.14x faster                                                  |
| float                      | 80.8 ms                                                | 73.2 ms: 1.10x faster                                                  |
| regex_effbot               | 3.17 ms                                                | 2.88 ms: 1.10x faster                                                  |
| bpe_tokeniser              | 4.74 sec                                               | 4.33 sec: 1.09x faster                                                 |
| comprehensions             | 19.8 us                                                | 18.1 us: 1.09x faster                                                  |
| xml_etree_iterparse        | 96.7 ms                                                | 88.8 ms: 1.09x faster                                                  |
| tomli_loads                | 2.11 sec                                               | 1.94 sec: 1.09x faster                                                 |
| scimark_sor                | 130 ms                                                 | 121 ms: 1.08x faster                                                   |
| pylint                     | 319 ms                                                 | 300 ms: 1.06x faster                                                   |
| deepcopy_reduce            | 3.08 us                                                | 2.91 us: 1.06x faster                                                  |
| logging_silent             | 109 ns                                                 | 104 ns: 1.05x faster                                                   |
| pycparser                  | 1.17 sec                                               | 1.12 sec: 1.04x faster                                                 |
| xml_etree_parse            | 139 ms                                                 | 134 ms: 1.04x faster                                                   |
| async_generators           | 384 ms                                                 | 373 ms: 1.03x faster                                                   |
| chaos                      | 62.8 ms                                                | 61.3 ms: 1.02x faster                                                  |
| regex_compile              | 142 ms                                                 | 141 ms: 1.01x faster                                                   |
| asyncio_websockets         | 517 ms                                                 | 513 ms: 1.01x faster                                                   |
| docutils                   | 2.64 sec                                               | 2.65 sec: 1.01x slower                                                 |
| coroutines                 | 23.9 ms                                                | 24.1 ms: 1.01x slower                                                  |
| pyflate                    | 448 ms                                                 | 455 ms: 1.01x slower                                                   |
| generators                 | 32.2 ms                                                | 32.9 ms: 1.02x slower                                                  |
| logging_format             | 7.35 us                                                | 7.54 us: 1.03x slower                                                  |
| regex_v8                   | 20.6 ms                                                | 21.3 ms: 1.03x slower                                                  |
| json_dumps                 | 10.4 ms                                                | 10.8 ms: 1.04x slower                                                  |
| html5lib                   | 63.6 ms                                                | 66.4 ms: 1.04x slower                                                  |
| regex_dna                  | 168 ms                                                 | 176 ms: 1.05x slower                                                   |
| hexiom                     | 6.17 ms                                                | 6.48 ms: 1.05x slower                                                  |
| sympy_integrate            | 20.5 ms                                                | 21.7 ms: 1.06x slower                                                  |
| pprint_safe_repr           | 743 ms                                                 | 785 ms: 1.06x slower                                                   |
| pidigits                   | 184 ms                                                 | 195 ms: 1.06x slower                                                   |
| sympy_sum                  | 166 ms                                                 | 176 ms: 1.06x slower                                                   |
| sympy_str                  | 292 ms                                                 | 311 ms: 1.07x slower                                                   |
| json                       | 5.02 ms                                                | 5.35 ms: 1.07x slower                                                  |
| 2to3                       | 264 ms                                                 | 281 ms: 1.07x slower                                                   |
| unpickle_pure_python       | 221 us                                                 | 237 us: 1.07x slower                                                   |
| pprint_pformat             | 1.52 sec                                               | 1.63 sec: 1.07x slower                                                 |
| genshi_xml                 | 50.2 ms                                                | 53.9 ms: 1.08x slower                                                  |
| pickle_pure_python         | 308 us                                                 | 332 us: 1.08x slower                                                   |
| scimark_lu                 | 114 ms                                                 | 123 ms: 1.08x slower                                                   |
| nqueens                    | 80.1 ms                                                | 86.8 ms: 1.08x slower                                                  |
| sympy_expand               | 468 ms                                                 | 518 ms: 1.11x slower                                                   |
| genshi_text                | 22.8 ms                                                | 25.4 ms: 1.11x slower                                                  |
| xml_etree_generate         | 85.2 ms                                                | 95.2 ms: 1.12x slower                                                  |
| scimark_monte_carlo        | 68.4 ms                                                | 77.5 ms: 1.13x slower                                                  |
| crypto_pyaes               | 76.6 ms                                                | 86.9 ms: 1.13x slower                                                  |
| django_template            | 34.7 ms                                                | 39.6 ms: 1.14x slower                                                  |
| xml_etree_process          | 59.0 ms                                                | 67.8 ms: 1.15x slower                                                  |
| thrift                     | 791 us                                                 | 914 us: 1.16x slower                                                   |
| json_loads                 | 26.5 us                                                | 30.7 us: 1.16x slower                                                  |
| richards                   | 45.9 ms                                                | 53.4 ms: 1.16x slower                                                  |
| richards_super             | 51.9 ms                                                | 60.6 ms: 1.17x slower                                                  |
| meteor_contest             | 104 ms                                                 | 122 ms: 1.18x slower                                                   |
| typing_runtime_protocols   | 163 us                                                 | 193 us: 1.18x slower                                                   |
| fannkuch                   | 372 ms                                                 | 448 ms: 1.20x slower                                                   |
| scimark_sparse_mat_mult    | 4.39 ms                                                | 5.42 ms: 1.23x slower                                                  |
| python_startup_no_site     | 7.16 ms                                                | 9.76 ms: 1.36x slower                                                  |
| create_gc_cycles           | 1.09 ms                                                | 1.49 ms: 1.36x slower                                                  |
| nbody                      | 89.3 ms                                                | 123 ms: 1.37x slower                                                   |
| mako                       | 11.0 ms                                                | 15.6 ms: 1.42x slower                                                  |
| coverage                   | 71.4 ms                                                | 114 ms: 1.59x slower                                                   |
| python_startup             | 9.93 ms                                                | 15.9 ms: 1.60x slower                                                  |
| bench_thread_pool          | 941 us                                                 | 1.63 ms: 1.73x slower                                                  |
| telco                      | 6.53 ms                                                | 172 ms: 26.40x slower                                                  |
| Geometric mean             | (ref)                                                  | 1.02x slower                                                           |

Benchmark hidden because not significant (5): spectral_norm, scimark_fft, raytrace, logging_simple, deltablue
Ignored benchmarks (21) of results/bm-20240906-3.12.6-a4a2d2b/bm-20240906-vultr-x86_64-python-v3.12.6-3.12.6-a4a2d2b.json: aiohttp, asyncio_tcp, asyncio_tcp_ssl, chameleon, dask, flaskblogging, gunicorn, mypy2, pickle, pickle_dict, pickle_list, sqlalchemy_declarative, sqlalchemy_imperative, sqlglot_normalize, sqlglot_optimize, sqlglot_parse, sqlglot_transpile, tornado_http, unpack_sequence, unpickle, unpickle_list
Ignored benchmarks (10) of results/bm-20260320-3.15.0a7+-b5e4c46-NOGIL/bm-20260320-vultr-x86_64-python-b5e4c46be24d1fade0de-3.15.0a7+-b5e4c46.json: connected_components, k_core, many_optionals, shortest_path, sphinx, sqlglot_v2_normalize, sqlglot_v2_optimize, sqlglot_v2_parse, sqlglot_v2_transpile, subparsers

- Geometric mean (including insignificant results): 1.022x slower

# HPT report

- Reliability score: 70.94% likely to be slow
- 90% likely to have a slowdown of 1.00x
- 95% likely to have a slowdown of 1.00x
- 99% likely to have a slowdown of 1.00x

# Memory
- memory change: 1.42x